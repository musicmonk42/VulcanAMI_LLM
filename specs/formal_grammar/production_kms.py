...
class ProductionKMS:
    ...
    def __init__(
        ...
        self.logger.info(
            f"ProductionKMS initialized with keystore at {self.keystore_path}"
        )
        ...
        if master_key_b64:
            try:
                ...
                if len(master_key) == 32:  # 256 bits
                    ...
                    self.logger.warning(
                        "Invalid master key in environment, generating new one"
                    )
    ...
    def _encrypt_data(self, data: bytes) -> bytes:
        ...
        # Create cipher
        cipher = Cipher(
            algorithms.AES(self._master_key), modes.CBC(iv), backend=default_backend()
        )
    ...
    def _save_metadata(self):
        ...
        with self._lock:
            try:
                metadata = {"revoked": list(self._revoked_keys)}
