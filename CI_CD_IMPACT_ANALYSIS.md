...
2. ⚠️ **Documentation reviewed** (see "Documentation References" section above):
   - 9 documentation files reviewed
   - All references are instructional/educational only
   - No configuration dependencies found
3. ℹ️ **Review documentation** if future changes include:
...
```bash
# 1. Test Docker build (requires Makefile target 'docker-build')
make docker-build
# Or directly: docker build --build-arg REJECT_INSECURE_JWT=ack -t vulcanami-main:test .

# 2. Test Docker Compose (requires Makefile target 'validate-docker')
make validate-docker
# Or directly: docker compose -f docker-compose.dev.yml config

# 3. Test Kubernetes manifests (requires kubectl and kustomize)
kubectl apply --dry-run=client -k k8s/base/

# 4. Test Helm charts (requires helm 3.10+)
helm template vulcanami ./helm/vulcanami --debug

# 5. Run comprehensive tests (if available)
./test_full_cicd.sh

# 6. Quick validation (if available)
./quick_test.sh quick
```

**Note:** The `make` commands reference Makefile targets. Verify these targets exist in your Makefile before using them.
