...
class ScheduledAdversarialTester:
    ...
    def run_attack(self, attack_type: str, epsilon: float) -> Dict[str, Any]:
        ...
        try:
            ...
            test_action = {"type": "test_action", "parameters": {"test": True}}
            test_context = {
                "test_mode": True,
                "scheduled": True,
                "attack_type": attack_type,
            ...
            try:
                ...
                report = self.validator.validate_robustness(
                    test_action, test_context, mock_validator
                )
                ...
                if "unpack" in str(e):
                    ...
                    if attack_type == "trojan":
                        logger.warning(
                            f"Skipping trojan attack due to known issue: {e}"
                        )
                        ...
                        try:
                            report = self.validator.validate_robustness(
                                mock_validator, test_action, test_context
                            )
                        except:
                            # Try without the validator function
                            try:
                                report = self.validator.validate_robustness(
                                    test_action, test_context
                                )
                            except Exception as e2:
                                error_msg = (
                                    f"Failed with multiple parameter combinations: {e2}"
                                )
                else:
                    error_msg = str(e)
    ...
    def run_scheduled_tests(
        ...
        if SIGNAL_AVAILABLE:
            ...
            logger.warning(
                "Signal-based timeout not available on Windows, using manual timeout check"
            )
        ...
        try:
            ...
            logger.info(
                f"Success: {summary['successful_tests']}, Failed: {summary['failed_tests']}"
            )
    ...
    def _check_and_alert(self, result: Dict[str, Any]):
        ...
        if robustness is not None and robustness < threshold:
            logger.warning(
                f"ALERT: Robustness below threshold! "
