#!/usr/bin/env bash
set -euo pipefail
IMAGE_REF="${IMAGE_REF:?IMAGE_REF must identify the frozen candidate by repository@sha256:digest}"
[[ "$IMAGE_REF" =~ @sha256:[0-9a-f]{64}$ ]] || { echo "mutable image references are prohibited" >&2; exit 2; }
...
start(){
  CID=$(docker run -d --rm -p "127.0.0.1:${PORT}:8000" -v "${VOL}:/var/lib/vulcan" \
    ...
    --read-only --tmpfs /tmp:rw,noexec,nosuid,size=16m "$IMAGE_REF")
...
chat=$(curl -fsS "http://127.0.0.1:${PORT}/v1/chat" "${hdr[@]}" -d '{"message":"2 + 2","conversation_id":"conv-e2e"}')
...
printf '%s' "$chat" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); raise SystemExit(0 if d.get("response") == "The computed result is 4." else 1)'
...
printf '{"schema":"vulcan-gate-e-observation/v2","subject":"%s","result":"PASS"}\n' "$IMAGE_REF"
