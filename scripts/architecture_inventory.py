...
def main() -> int:
    ...
    parser.add_argument(
        "--markdown", default="docs/generated/architecture-inventory.md"
    )
    ...
    expected_readme = replace_readme_capabilities(
        readme, render_readme_capabilities(inventory)
    )
    if args.check:
        ...
        if readme != expected_readme:
            raise SystemExit("README.md capability claims are not current")
        return 0
