    ...
    def generate_multilingual_field(self, field_name: str, base_schema: Dict) -> Dict:
        ...
        multilingual_schema = {
            "oneOf": [
                ...
                {
                    "type": "object",
                    "patternProperties": {"^[a-z]{2}(-[A-Z]{2})?$": base_schema},
...
def extract_grammar_sections(md_text: str) -> str:
    ...
    ebnf_blocks = re.findall(
        r"```(?:ebnf|grammar|bnf)(.*?)```", md_text, re.DOTALL | re.IGNORECASE
    )
            ...
            if expr["type"] == "sequence":
                ...
                if current_seq:
                    if len(current_seq) > 1:
                        alternation_items.append(
                            {"type": "sequence", "items": current_seq}
                        )
                    else:
...
def build_json_schema_from_productions(
    productions: Dict[str, Any],
    strict: bool = False,
    multilingual_generator: Optional[MultilingualSchemaGenerator] = None,
) -> Dict[str, Any]:
    """
    ...
    for tname, expr in productions.items():
        if expr is not None:
            schema["definitions"][tname] = build_schema_from_expr(
                expr, strict, multilingual_generator
            )

    return schema


def build_schema_from_expr(
    expr: Optional[Dict[str, Any]],
    strict: bool,
    multilingual_generator: Optional[MultilingualSchemaGenerator],
) -> Dict[str, Any]:
    ...
    if etype == "ref":
        ...
        if expr["value"] in TYPE_MAP:
            ...
            if expr["value"] == "STRING" and multilingual_generator:
                return multilingual_generator.generate_multilingual_field(
                    expr["value"], base_schema
                )
            return base_schema
        else:
            return {"$ref": f"#/definitions/{expr['value']}"}
        ...
        if all(item.get("type") == "sequence" for item in items):
            ...
            for item in items:
                ...
                if (
                    ...
                    and len(item_items) >= 2
                    and isinstance(item_items[0], dict)
                    and isinstance(item_items[1], dict)
                    and item_items[1].get("type") == "ident"
                ):
                    prop_name = item_items[0].get("value")
                    if prop_name:
                        prop_type_expr = item_items[1]
                        properties[prop_name] = build_schema_from_expr(
                            prop_type_expr, strict, multilingual_generator
                        )
        ...
        for item in items:
            if isinstance(item, dict) and "value" in item:
                ...
                if prop_name and isinstance(prop_name, str):
                    properties[prop_name] = build_schema_from_expr(
                        item, strict, multilingual_generator
                    )
        ...
        inner_schema = build_schema_from_expr(
            expr.get("value"), strict, multilingual_generator
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Graphix IR EBNF → JSON schema auto-generator (Upgraded v2.0.0)"
    )
    parser.add_argument(
        "--in", dest="input", required=True, help="Path to formal_grammar.md"
    )
    parser.add_argument(
        "--out", dest="output", required=True, help="Output path for JSON schema"
    )
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Set additionalProperties: false for strict validation",
    )
    parser.add_argument(
        "--sign-key",
        dest="sign_key",
        default=None,
        help="Path to ECDSA private key PEM for signing provenance",
    )
    parser.add_argument(
        "--test", action="store_true", help="Run self-test with sample grammar"
    )
...
<NUMBER> ::= [+-] ( "0" | "1".."9" {"0".."9"} ) [ "." {"0".."9"} ];
        ...
        try:
            ...
            schema = build_json_schema_from_productions(
                productions, args.strict, multilingual_generator
            )
            schema["provenance"]["generated_at"] = datetime.utcnow().isoformat() + "Z"
            schema["provenance"]["grammar_hash"] = grammar_hash
            print(json.dumps(schema, indent=2))
            print(
                "Self-test passed. Generated schema is a valid representation of the sample grammar."
            )
