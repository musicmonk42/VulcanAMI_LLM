...
class TaskManager:
    ...
    async def register_task(
        self, task_id: str, task: asyncio.Task, metadata: Dict[str, Any] = None
    ):
        ...
        def _cleanup_task(fut: asyncio.Future):
            ...
            try:
                ...
                if not fut.cancelled() and fut.exception():
                    exc = fut.exception()
                    logger.error(
                        f"Task {task_id} finished with an unhandled exception: {exc}",
                        exc_info=exc,
                    )
...
class SchedulerNode:
    ...
    async def execute(
        ...
        logger.info(
            f"Executing SchedulerNode with trigger={trigger}, interval_ms={interval_ms}, task_id={task_id}"
        )
            ...
            if trigger == "periodic":
                    ...
                    try:
                        while context.get("running", True):
                            ...
                            if (
                                ...
                                logger.info(
                                    f"Task {task_id} reached max iterations: {max_iterations}"
                                )
                                break
                            ...
                            logger.info(
                                f"Running task {task_id} iteration {iteration} at {context['tasks'][task_id]['last_run']}"
                            )
                            ...
                            context["tasks"][task_id].update(
                                {
                                    ...
                                    "compression_ok": compression_result[
                                        "compression_ok"
                                    ],
                                }
                            )
                        ...
                        # Task was cancelled
                        logger.info(
                            f"Task {task_id} was cancelled after {iteration} iterations"
                        )
                ...
                async def event_task():
                    try:
                        ...
                        logger.info(
                            f"Event task {task_id} triggered by {context['event_signal']}"
                        )
...
async def async_dispatch_scheduler_node(
