...
class Groth16Proof:
    ...
    def to_bytes(self) -> bytes:
        ...
        a_bytes = int(self.A[0]).to_bytes(32, 'big') + int(self.A[1]).to_bytes(32, 'big')
        ...
        c_bytes = int(self.C[0]).to_bytes(32, 'big') + int(self.C[1]).to_bytes(32, 'big')
...
class Groth16Prover:
    ...
    def setup(self, toxic_waste: Optional[Dict[str, int]] = None) -> Tuple[ProvingKey, VerificationKey]:
        """
    ...
    def verify(
        ...
        if len(public_inputs) != self.circuit.num_public_inputs:
            logger.error(f"Expected {self.circuit.num_public_inputs} public inputs, got {len(public_inputs)}")
            return False
...
def create_unlearning_circuit(
    ...
    num_private = num_samples + model_size + num_samples  # samples + weights + gradients
    ...
    logger.info(f"Created unlearning circuit: {len(constraints)} constraints, {num_variables} variables")
