...
class ZKProof:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> ZKProof:
        ...
        return cls(
            ...
            proof_bytes=bytes.fromhex(data['proof_bytes']) if data.get('proof_bytes') else None,
...
class ZKVerifier:
    ...
    def register_verifier(
        self,
        proof_system: str,
        verifier_func: Callable[[ZKProof], bool]
    ) -> None:
        """
    ...
    def _verify_groth16(self, proof: ZKProof) -> bool:
        ...
        vk = proof.verification_key or self.registry.get_verification_key(proof.circuit_hash)
        if not vk:
    ...
    def _verify_bulletproofs(self, proof: ZKProof) -> bool:
        ...
        if len(proof.proof_bytes) < 32:
            logger.error(f"Bulletproofs proof too small: {len(proof.proof_bytes)} bytes")
            return False
