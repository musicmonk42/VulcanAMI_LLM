...
class RepackScheduler:
    ...
    def __init__(self, config: RepackConfig):
        ...
        logger.info(f"Initialized RepackScheduler with max_packs={config.max_packs_per_artifact}")
    ...
    def schedule_task(self, task: RepackTask) -> None:
        ...
        heapq.heappush(self.pending_tasks, (task.priority.value, self.task_counter, task))
        self.task_counter += 1
        
        logger.info(f"Scheduled repack task: {task.task_id} "
    ...
    def complete_task(self, result: RepackResult) -> None:
        ...
        logger.info(f"Completed repack task: {result.task_id} "
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_io = sum(r.bytes_read + r.bytes_written for r in self.completed_tasks if r.success)
...
class AdaptiveRepacker:
    ...
    def analyze_artifact(self, artifact: ArtifactInfo,
                        packs: Dict[str, PackMetadata]) -> Tuple[bool, Optional[RepackReason]]:
        """
    ...
    def create_repack_task(self, artifact_id: str, pack_ids: List[str],
                          reason: RepackReason,
                          packs: Dict[str, PackMetadata]) -> RepackTask:
        """
        ...
        task = RepackTask(
            ...
            metadata={
                ...
                'avg_fragmentation': sum(p.fragmentation_ratio for p in task_packs) / len(task_packs)
            }
        )
        
        return task
    
    def _determine_priority(self, reason: RepackReason,
                          packs: List[PackMetadata]) -> RepackPriority:
        """Determine priority for repack task"""
        avg_frag = sum(p.fragmentation_ratio for p in packs) / len(packs) if packs else 0.0
    ...
    def select_packs_for_repack(self, artifact_id: str,
                               packs: Dict[str, PackMetadata]) -> List[str]:
        """
        ...
        """
        # Get artifact packs
        artifact_packs = {pid: p for pid, p in packs.items() 
        ...
        sorted_packs = sorted(artifact_packs.items(),
                            key=lambda x: x[1].fragmentation_ratio,
                            reverse=True)
...
def adaptive_repack(pack_id: str,
                   packs: Optional[Dict[str, PackMetadata]] = None,
                   config: Optional[RepackConfig] = None) -> Dict[str, Any]:
    """
...
class RepackMonitor:
    ...
    def get_statistics(self, window_hours: Optional[int] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_io_bytes': total_io,
            'total_io_mb': total_io / (1024 * 1024),
            'avg_throughput_mbps': sum(r.throughput_mbps for r in repacks) / len(repacks),
