...
class PackStats:
    ...
    def __post_init__(self):
        ...
        if self.live_bytes > self.total_bytes:
            raise ValueError(f"live_bytes ({self.live_bytes}) cannot exceed total_bytes ({self.total_bytes})")
        
        if not 0 <= self.fragmentation_ratio <= 1:
            raise ValueError(f"fragmentation_ratio must be in [0, 1]: {self.fragmentation_ratio}")
...
class TimeWindowedCompaction:
    ...
    def __init__(self, window_size_hours: int = 24,
        ...
        logger.info(f"Initialized TimeWindowedCompaction: "
    ...
    def _group_by_time_window(self, packs: List[PackStats]) -> Dict[datetime, List[PackStats]]:
        """Group packs by time window"""
...
class LeveledCompaction:
    ...
    def __init__(self, aggressive: bool = False,
        ...
        logger.info(f"Initialized LeveledCompaction: "
    ...
    def _create_l0_compaction_task(self, l0_packs: List[PackStats],
                                   l1_packs: List[PackStats]) -> CompactionTask:
    ...
    def _create_level_compaction_task(self, level: int,
                                     level_packs: List[PackStats],
                                     next_level_packs: List[PackStats]) -> Optional[CompactionTask]:
...
class TieredCompaction:
    ...
    def __init__(self, min_threshold: int = 4,
        ...
        logger.info(f"Initialized TieredCompaction: "
...
class HybridCompaction:
    ...
    def __init__(self, tier_levels: List[int] = None,
        ...
        logger.info(f"Initialized HybridCompaction: "
...
class AdaptiveCompaction:
    ...
    def _analyze_workload(self, packs: List[PackStats]) -> Dict[str, float]:
        ...
        return {
            ...
            'avg_fragmentation': sum(p.fragmentation_ratio for p in packs) / len(packs),
            'time_series_ratio': sum(1 for p in packs if p.domain in ['metrics', 'audit']) / len(packs),
...
class CompactionCostModel:
    ...
    def __init__(self, io_cost_per_mb: float = 1.0,
                 cpu_cost_per_mb: float = 0.1):
        """
    ...
    def estimate_task_cost(self, task: CompactionTask,
                          packs: Dict[str, PackStats]) -> float:
        """
    ...
    def estimate_write_amplification(self, packs: List[PackStats],
                                    strategy: CompactionStrategy) -> float:
        """
...
class CompactionPlanner:
    ...
    def select_compaction_candidates(
            ...
            if current_amplification > self.size_amplification_limit:
                ...
                for pack in packs:
                    if pack.pack_id not in urgent_packs:
                        dead_ratio = pack.dead_bytes / pack.total_bytes if pack.total_bytes > 0 else 0
        ...
        if self.enable_throttling:
            ...
            available_slots = self.max_concurrent_compactions - len(self.active_compactions)
    ...
    def mark_compaction_started(self, pack_id: str) -> None:
        ...
        logger.debug(f"Marked {pack_id} as compacting (active: {len(self.active_compactions)})")
    ...
    def mark_compaction_finished(self, pack_id: str) -> None:
        ...
        if pack_id in self.active_compactions:
            ...
            logger.debug(f"Marked {pack_id} as finished (active: {len(self.active_compactions)})")
    ...
    def estimate_compaction_benefit(
        ...
        total_bytes = sum(p.total_bytes for p in packs)
        current_frag = sum(p.fragmentation_ratio * p.total_bytes for p in packs) / total_bytes if total_bytes > 0 else 0
...
if __name__ == "__main__":
    ...
    print("Sample Packs:")
    for pack in packs:
        print(f"  {pack.pack_id}: {pack.domain}, "
              f"level={pack.level}, "
