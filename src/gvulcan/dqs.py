...
class DQSComponents:
    ...
    def __post_init__(self):
        ...
        for field_name in ['pii_confidence', 'graph_completeness', 'syntactic_completeness']:
...
def compute_dqs(comp: DQSComponents,
...
def compute_dqs_v2(comp: DQSComponents,
                   weights: Optional[Dict[str, float]] = None,
                   pii_penalty_factor: float = 1.5) -> float:
    """
...
class DQSScorer:
    ...
    def __init__(self,
        ...
        logger.info(f"Initialized DQS Scorer with model={model}, "
...
class DQSTracker:
    ...
    def record(self, result: DQSResult) -> None:
        ...
        logger.debug(f"Recorded DQS result: {result.score:.3f} -> {result.gate_decision}")
