...
def merkle_root(leaves: Iterable[bytes], algorithm: HashAlgorithm = HashAlgorithm.SHA256) -> bytes:
    """
...
class MerkleTree:
    ...
    def build(self, leaves: List[bytes]) -> None:
        ...
        logger.info(f"Built Merkle tree with {len(self.leaves)} leaves, "
...
class MerkleLSMDAG:
    ...
    def checkpoint(self) -> None:
        ...
        logger.info(f"Created checkpoint at leaf {self.leaf_count}, "
                   f"root: {root.hex()[:16]}...")
    ...
    def load(cls, path: Path) -> MerkleLSMDAG:
        ...
        dag.checkpoints = [(count, bytes.fromhex(root)) 
                          for count, root in state['checkpoints']]
...
def batch_verify_proofs(proofs: List[MerkleProof], 
                       algorithm: HashAlgorithm = HashAlgorithm.SHA256) -> List[bool]:
    """
...
def compute_merkle_root_streaming(data_stream: Iterable[bytes],
                                  chunk_size: int = 1000,
...
def verify_data_integrity(data: List[bytes], 
                         expected_root: bytes,
