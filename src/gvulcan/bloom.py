...
class BloomFilter:
    ...
    def __init__(self, size_bytes: int = 128, k: int = 7):
        ...
        logger.info(f"Initialized BloomFilter: {self.size} bytes, {self.k} hash functions")
    
    @classmethod
    def create_optimal(cls, expected_items: int, 
        ...
        logger.info(f"Created optimal BloomFilter for {expected_items} items "
    ...
    def __repr__(self) -> str:
        ...
        return (f"BloomFilter(size={self.size}B, k={self.k}, "
...
class CountingBloomFilter:
    ...
    def __init__(self, size_bytes: int = 128, k: int = 7, 
        ...
        logger.info(f"Initialized CountingBloomFilter: "
...
class ScalableBloomFilter:
    ...
    def add(self, item: bytes) -> None:
        ...
        if current.get_fill_ratio() > 0.5:
            new_capacity = self.initial_capacity * (self.growth_factor ** len(self.filters))
...
def test_false_positive_rate(bf: BloomFilter, 
                            test_items: Set[bytes],
                            non_member_items: Set[bytes]) -> Tuple[float, int, int]:
    """
    ...
    false_positives = sum(1 for item in non_member_items 
