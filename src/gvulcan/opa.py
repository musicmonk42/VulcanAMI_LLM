...
class OPAClient:
    ...
    def __init__(self, bundle_version: str, 
        ...
        logger.info(f"Initialized OPA Client with bundle version {bundle_version}, "
    ...
    def _evaluate_write_barrier_impl(self, data: WriteBarrierInput) -> WriteBarrierResult:
        """
        ...
        if data.dqs < 0.30:
            return WriteBarrierResult(
                allow=False,
                quarantine=False,
                deny_reason="DQS below reject threshold"
            )
    ...
    def evaluate_policy(self, policy_name: str, 
    ...
    def _evaluate_policy_impl(self, policy_name: str, 
                            input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def evaluate_batch(self, policy_name: str, 
    ...
    def get_audit_log(self, policy_name: Optional[str] = None,
                     limit: Optional[int] = None) -> List[PolicyEvaluation]:
        """
    ...
    def export_audit_log(self, path: Path,
...
"""
        ...
        for policy, count in sorted(stats['policy_counts'].items(), 
                                   key=lambda x: x[1], reverse=True):
            pct = count / stats['total_evaluations'] * 100 if stats['total_evaluations'] > 0 else 0
...
class PolicyRegistry:
    ...
    def register_client(self, name: str, client: OPAClient,
...
if __name__ == "__main__":
    ...
    print(f"Write Barrier Result: allow={result.allow}, "
    ...
    for dqs in [0.25, 0.35, 0.50]:
        ...
        print(f"DQS {dqs:.2f} -> {test_result.allow}, "
              f"quarantine={test_result.quarantine}")
