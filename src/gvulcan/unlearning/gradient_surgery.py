...
class GradientSurgeryUnlearner:
    ...
    def compute_forget_gradient(
        ...
        logger.debug(f"Computed forget gradient: norm={np.linalg.norm(forget_grad):.4f}")
        return forget_grad
    ...
    def _project_gradient(
        self,
        forget_grad: np.ndarray,
        retain_grad: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def _generate_zk_proof(
        ...
        return {
            ...
            "public_inputs": {
                ...
                "forget_loss_increase": metrics.forget_loss_after - metrics.forget_loss_before,
                "retain_loss_delta": abs(metrics.retain_loss_after - metrics.retain_loss_before),
...
def pcgrad(grads: List[np.ndarray]) -> List[np.ndarray]:
    ...
    for i in range(n_tasks):
        ...
        for j in range(n_tasks):
            ...
            if cos_sim < 0:
                logger.debug(
                    f"Conflict detected between task {i} and {j}: "
