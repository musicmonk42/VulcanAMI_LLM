...
MAX_PRIORITY_QUEUE_SIZE = (
    10000  # Maximum items per priority queue to prevent unbounded growth
)
...
class SmartPurger:
    ...
    def __init__(
        ...
        self.queues = {
            priority: deque(maxlen=MAX_PRIORITY_QUEUE_SIZE)
            for priority in PurgePriority
        }
        ...
        self.invalidation_timestamps: deque = deque(
            maxlen=max_invalidations_per_hour * RATE_LIMITER_WINDOW_MULTIPLIER
        )
        self.last_flush = 0.0
    ...
    def enqueue(
        ...
        with self.lock:
            ...
            request = PurgeRequest(
                path=path, priority=priority, reason=reason, callback=callback
            )
    ...
    def _create_invalidation(
        self, paths: List[str], caller_reference: str
    ) -> InvalidationResult:
        """
    ...
    def maybe_flush(self) -> List[InvalidationResult]:
        ...
        with self.lock:
            ...
            should_flush = total_pending >= self.max_batch_size or (
...
def create_purger(distribution_id: str, **kwargs) -> SmartPurger:
    """
