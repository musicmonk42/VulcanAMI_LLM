...
def rotational_8bit(
    fp16: np.ndarray,
    use_optimal_rotation: bool = True
    ...
    meta = {
        ...
        'rotation_matrix': rotation_matrix.tolist() if rotation_matrix is not None and rotation_matrix.size < 10000 else None,
...
def dequantize_rotational_8bit(
    codes: np.ndarray,
    meta: Dict[str, Any]
) -> np.ndarray:
    """
    ...
    if verify_ecc and 'ecc_parity' in meta:
        ...
        for i, row in enumerate(packed):
            ...
            if i < len(stored_parity) and computed_parity != stored_parity[i]:
                logger.warning(f"ECC parity mismatch for row {i}, data may be corrupted")
...
def product_quantization(
    ...
    if d % n_subspaces != 0:
        raise ValueError(f"Dimension {d} must be divisible by n_subspaces {n_subspaces}")
    ...
    for i in range(n_subspaces):
        ...
        distances = np.linalg.norm(
            subspace[:, np.newaxis, :] - centroids[np.newaxis, :, :],
            axis=2
        )
