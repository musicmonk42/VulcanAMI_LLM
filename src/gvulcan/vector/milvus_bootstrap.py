    ...
    try:
        ...
        for f in cfg["defaults"].get("extra_fields", []):
            ...
            if t == "INT64":
                ...
                logger.warning(f"Unsupported field type: {t}, skipping field {f['name']}")
                continue
        
        # Create schema
        description = tier.get("description", f"Collection for tier {tier.get('name', 'unknown')}")
        ...
        raise CollectionCreationError(f"Failed to create collection {coll_name}: {e}") from e
    except Exception as e:
        raise CollectionCreationError(f"Unexpected error creating collection {coll_name}: {e}") from e
...
def load_collection(coll_name: str, timeout: float = 30.0) -> Collection:
    ...
    try:
        ...
        while time.time() - start_time < timeout:
            if coll.num_entities > 0 or utility.get_loading_progress(coll_name).get("progress", 0) == 100:
                logger.info(f"Collection loaded: {coll_name} ({coll.num_entities} entities)")
                return coll
            time.sleep(0.5)
...
def main(
    ...
    if config_path is None:
        config_path = Path(__file__).resolve().parents[3] / "configs" / "vector" / "milvus" / "collections.yaml"
