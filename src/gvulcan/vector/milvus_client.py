...
class TierConfig:
    ...
    def __post_init__(self):
        if not self.collection_name:
            self.collection_name = f"vectors_tier_{self.name.lower().replace('+', 'plus')}"
...
class MilvusIndex:
    ...
    def __init__(
        ...
        logger.info(f"Initialized MilvusIndex: uri={uri}, default_tier={default_tier.value}")
        ...
        for tier in self.TIERS:
            ...
            if utility.has_collection(collection_name):
                try:
                    ...
                    logger.info(f"Loaded collection for tier {tier.name}: {collection_name}")
                except Exception as e:
                    logger.warning(f"Failed to load collection {collection_name}: {e}")
            else:
                logger.info(f"Collection {collection_name} does not exist for tier {tier.name}")
    ...
    def add_vectors(
        ...
        results = {}
        target_tiers = [tier] if tier else [SearchTier.A_PLUS, SearchTier.A, SearchTier.B, SearchTier.C]
        ...
        for search_tier in target_tiers:
            ...
            if not collection:
                logger.warning(f"Collection not loaded for tier {search_tier.value}, skipping")
                continue
                ...
                for fallback_tier in self.TIERS:
                    ...
                    try:
                        ...
                        result = self._search_tier(
                            query_vec,
                            k,
                            SearchTier(fallback_tier.name),
                            search_params
                        )
                        return result
                    except Exception as fallback_e:
                        logger.error(f"Fallback to {fallback_tier.name} failed: {fallback_e}")
    ...
    def _search_tier(
        ...
        self.stats['tier_usage'][tier.value] = self.stats['tier_usage'].get(tier.value, 0) + 1
