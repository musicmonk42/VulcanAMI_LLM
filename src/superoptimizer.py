...
class Superoptimizer:
    ...
    def __init__(
        ...
        if self.use_llm:
            try:
                ...
                logger.warning(f"Failed to initialize LLM client: {e}. Using fallback generation.")
                self.use_llm = False
        ...
        self.supported_backends = {
            "cuda", "llvm", "cpu", "photonic", "memristor", 
            "fpga", "gpu", "x86", "arm", "aarch64"
        }
    ...
    def generate_kernel(
        ...
        logger.info(f"Generating kernel for backend '{backend}' (opt level {optimization_level})")
        ...
        kernel_result = self._generate_kernel_internal(subgraph, backend, optimization_level)
    ...
    def _generate_kernel_internal(
        ...
        if self.use_emulator and result["kernel_code"]:
            ...
            result["valid"] = result["valid"] and result["test_results"].get("valid", False)
    ...
    def _generate_with_llm(self, subgraph: Dict[str, Any], backend: str) -> Optional[str]:
...
"""
        ...
        try:
            ...
            if backend == "photonic":
                ...
                if not analog_photonic_emulator:
                    test_results["errors"].append("Emulator instance is None, cannot test")
                    return test_results
                ...
                result = analog_photonic_emulator.emulate_photonic_mvm(test_matrix, test_vector)
                
                test_results["tested"] = True
                test_results["valid"] = result is not None and result.shape == test_vector.shape
    ...
    def _extract_tensor_ops(self, subgraph: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for node in subgraph.get("nodes", []):
            ...
            if node_type in ["MVM", "MEMRISTOR_MVM", "PhotonicMVMNode", "ADD", "MUL", "CONV"]:
                ops.append({
                    "type": node_type,
                    "params": node.get("params", {})
                })
