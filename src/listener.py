...
class MockAgentRegistry:
    ...
    def verify_signature(self, agent_id: str, message: str, signature: str) -> bool:
        ...
        """
        logger.warning(f"Using mock verification for agent '{agent_id}' (always returns True)")
        return True
    ...
    def do_POST(self):
        ...
        try:
            ...
            if len(agent_id) > MAX_AGENT_ID_LENGTH:
                self.send_error_response(
                    400,
                    f"X-Agent-ID too long (max {MAX_AGENT_ID_LENGTH} characters)"
                )
                return
            ...
            if len(signature) > MAX_SIGNATURE_LENGTH:
                self.send_error_response(
                    400,
                    f"X-Signature too long (max {MAX_SIGNATURE_LENGTH} characters)"
                )
                return
            ...
            if content_length < MIN_CONTENT_LENGTH:
                self.send_error_response(
                    400,
                    f"Content-Length too small (min {MIN_CONTENT_LENGTH} bytes)"
                )
                return
            ...
            try:
                ...
                self.send_error_response(
                    408,
                    f"Request timeout or read error: {e}"
                )
                return
            ...
            if validation_error:
                self.send_error_response(
                    400,
                    f"Invalid graph structure: {validation_error}"
                )
                return
            ...
            logger.error(
                f"Unexpected error in request handler: {e}",
                exc_info=True
            )
            self.send_error_response(
                500,
...
class GraphixListener:
    ...
    def start(self):
        ...
        try:
            ...
            logger.info(f"Rate limit: {self.rate_limiter.max_requests} requests per minute")
