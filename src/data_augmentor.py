...
class GraphValidator:
    ...
    @staticmethod
    def validate_edge(edge: Dict[str, Any], node_ids: Set[str]) -> Tuple[bool, Optional[str]]:
...
class SemanticMutator:
    ...
    @classmethod
    def mutate_node_semantic(cls, node: Dict[str, Any], rng: random.Random) -> Dict[str, Any]:
        """
    ...
    def create_semantic_edge(
        ...
        return {
            ...
            "properties": {
                "semantic": True,
                "inverse_relation": config['inverse']
            }
        }
...
class DataAugmentor:
    ...
    def _calculate_quality_score(self, proposal: Dict[str, Any]) -> float:
        ...
        augmented_count = sum(
            1 for n in nodes
            if n.get("properties", {}).get("augmented")
        )
        if nodes:
    ...
    def _update_metrics(self, kind: str, quality: float):
        ...
        with self.lock:
            ...
            self.metrics.avg_quality_score = (
                (self.metrics.avg_quality_score * (n - 1) + quality) / n
            )
    ...
    def generate_synthetic_proposal(
        ...
        for mutation_idx in range(complexity):
            ...
            if nodes:
                operations.extend(["mutate_node", "mutate_edge" if edges else "add_edge"])
            ...
            if op == "mutate_node" and nodes:
                ...
                # Add semantic node
                new_node_id = f"syn_node_{self.rng.randint(1000, 9999)}_{counter}_{mutation_idx}"
                ...
                if len(available_nodes) >= 2:
                    ...
                    target_candidates = [n for n in available_nodes if n["id"] != src_id]
        ...
        if nodes:
            for i, node in enumerate(nodes):
                if i == 0 or invert_all:
                    ...
                    negation_strategies = {
                        'NOT_': lambda l: l.replace('NOT_', '') if 'NOT_' in l else f'NOT_{l}',
                        'Anti': lambda l: l.replace('Anti', '') if l.startswith('Anti') else f'Anti{l}',
                        'Inverse': lambda l: l.replace('Inverse', '') if l.startswith('Inverse') else f'Inverse{l}'
                    }
    ...
    def adversarial_proposal(
        self,
        base_graph: Dict[str, Any],
        targeted_node: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        adversarial_labels = [
            "ADVERSARY", "MALICIOUS", "CORRUPTED", "POISONED",
            "BACKDOOR", "TROJAN", "EXPLOIT"
        ]
        ...
        if targeted_node:
            ...
            if not target_found:
                logger.warning(f"Targeted node '{targeted_node}' not found, targeting random")
            ...
            if strategy == "replace_label" and nodes:
                ...
                if len(nodes) > 1:
                    for _ in range(min(3, len(nodes) - 1)):
                        target = self.rng.choice([n for n in nodes if n["id"] != adv_node_id])
                        edges.append({
                            "from": adv_node_id,
                            "to": target["id"],
    ...
    def curriculum_batch(
        ...
        for i in range(num_synthetic):
            try:
                ...
                proposal = self.generate_synthetic_proposal(base_graph, complexity=complexity)
        ...
        for i in range(num_counterfactual):
            try:
                invert_all = (i % 2 == 0)
                proposal = self.counterfactual_proposal(base_graph, invert_all=invert_all)
                batch.append(proposal)
            except Exception as e:
