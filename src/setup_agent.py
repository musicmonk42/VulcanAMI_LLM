...
def validate_agent_id(agent_id: str) -> None:
    ...
    if len(agent_id) < 3:
        raise ValidationError(
            f"Agent ID must be at least 3 characters, got {len(agent_id)}"
        )

    if len(agent_id) > 64:
        raise ValidationError(
            f"Agent ID must be at most 64 characters, got {len(agent_id)}"
        )
...
def validate_roles(roles: List[str]) -> List[str]:
    ...
    logger.debug(
        f"Validated {len(normalized_roles)} unique role(s): {normalized_roles}"
    )
...
def setup(agent_id: str, roles: List[str]) -> bool:
    ...
    try:
        ...
        try:
            ...
            registration_result = registry.register_agent(
                agent_id, agent_id, validated_roles
            )
