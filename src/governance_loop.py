...
class GovernanceLoop:
            ...
            rules=[
                {
                    ...
                    'condition': {'field': 'action_type', 'operator': 'not_in', 
    ...
    def add_policy(self, policy: Policy):
        ...
        if len(policy.id) > MAX_POLICY_ID_LENGTH:
            raise ValueError(f"Policy ID too long: {len(policy.id)} > {MAX_POLICY_ID_LENGTH}")
        
        if not policy.name or not isinstance(policy.name, str):
            raise ValueError("Policy name must be a non-empty string")
        
        if len(policy.name) > MAX_POLICY_NAME_LENGTH:
            raise ValueError(f"Policy name too long: {len(policy.name)} > {MAX_POLICY_NAME_LENGTH}")
                    ...
                    try:
                        ...
                        with self.lock:
                            for violation in reversed(self.violations):
                                if violation.policy_id == policy_id and not violation.resolved:
                                    violation.resolved = success
                                    violation.resolution = "Auto-enforced" if success else "Failed"
                                    break
                    except Exception as e:
        ...
        with self.policy_effectiveness_lock:
            for policy_id, (compliant, _) in results.items():
                ...
                if len(self.policy_effectiveness) > MAX_POLICY_EFFECTIVENESS_ENTRIES:
                    ...
                    oldest = min(
                        self.policy_effectiveness.items(),
                        key=lambda x: x[1]['total']
                    )
                    del self.policy_effectiveness[oldest[0]]
                    logger.debug(f"Removed least-used policy effectiveness entry: {oldest[0]}")
                ...
                for policy_id, (compliant, reason) in results.items():
                    if not compliant and self.policies.get(policy_id):
                        if self.policies[policy_id].priority == PolicyPriority.CRITICAL:
                            ...
                            logger.warning(f"Action blocked by policy {policy_id}: {reason}")
                            break
    ...
    def _analyze_violation_trends(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'resolution_rate': sum(1 for v in violations_copy if v.resolved) / len(violations_copy)
        }
        ...
        try:
            ...
            for policy_id, data in policies_data.items():
                try:
                    ...
                    if missing_fields:
                        errors.append(f"Policy {policy_id}: Missing fields {missing_fields}")
                        continue
...
if __name__ == "__main__":
    ...
    print(f"   Enabled policies: {sum(1 for p in governance.policies.values() if p.enabled)}")
