...
def _register_or_get_metric(
    metric_type, name: str, description: str, base_name: str = None
):
    """
    ...
    try:
        ...
        if metric is None:
            raise RuntimeError(
                f"Failed to retrieve existing metric '{search_name}' from registry"
            )
        return metric
...
diversity_penalty_events = _register_or_get_metric(
    Counter,
    "tournament_diversity_penalty_total",
    "Total diversity penalties applied",
...
adaptive_penalty_gauge = _register_or_get_metric(
    Gauge,
    "tournament_adaptive_penalty_value",
...
class TournamentManager:
    ...
    def __init__(
        ...
        self.last_innovation_score = (
            0.5  # Start with a neutral assumption of past performance
        )
        ...
        if self.sharder:
            logger.info(
                "TournamentManager initialized with distributed sharder support."
            )
    ...
    def _adapt_penalty(self, trace: str) -> None:
        ...
        try:
            ...
            adjustment_factor = 1.0 + (
                error * 0.1
            )  # Small, gentle adjustment (10% of error)
            ...
            self.current_diversity_penalty = max(
                0.05, min(self.current_diversity_penalty, 0.5)
            )
            ...
            logger.error(
                f"[{trace}] Error adapting penalty: {e}. Using previous penalty."
            )

    def _validate_inputs(
        self, proposals: List[Dict[str, Any]], fitness: List[float], trace: str
    ) -> None:
        """
    ...
    def _validate_embeddings(
        self, embeddings: np.ndarray, expected_count: int, trace: str
    ) -> None:
        """
    ...
    def _compute_embeddings(
        ...
        if self.sharder and n > self.sharder_threshold:
            logger.info(
                f"[{trace}] Using distributed sharder for embedding {n} proposals."
            )
            try:
                ...
                logger.error(
                    f"[{trace}] Distributed sharding failed: {e}. Falling back to local computation."
                )
    ...
    def run_adaptive_tournament(
        ...
        try:
            ...
            num_winners = max(
                self.min_winners, int(np.ceil(self.winner_percentage * n))
            )
            ...
            logger.error(
                f"[{trace}] Unexpected error in tournament: {e}", exc_info=True
            )
