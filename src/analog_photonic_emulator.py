    ...
    def validate_corrections(self, expected_shape: Tuple[int, ...]) -> bool:
        ...
        if self.amplitude_corrections is not None:
            if self.amplitude_corrections.shape != expected_shape:
                logger.warning(f"Amplitude corrections shape mismatch: {self.amplitude_corrections.shape} vs {expected_shape}")
                return False
        return True
...
class PhotonicNoise:
    ...
    def amplifier_noise(self, signal: np.ndarray, nf_db: float = 5.0) -> np.ndarray:
        ...
        bandwidth = self.params.bandwidth
        ase_power = (nf - 1) * PLANCK_CONSTANT * SPEED_OF_LIGHT / self.params.wavelength * bandwidth * gain_linear
    ...
    def apply_realistic_noise(self, signal: np.ndarray, noise_models: List[NoiseModel]) -> np.ndarray:
...
class WaveguideSimulator:
    ...
    def couple(self, field1: np.ndarray, field2: np.ndarray, 
               coupling_ratio: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
        """Directional coupler."""
...
class MemristorEmulator:
    ...
    def __init__(self, rows: int = 128, cols: int = 128, state_file: Optional[str] = None):
    ...
    def write(self, row: int, col: int, value: float):
        ...
        with self.lock:
            ...
            conductance = self.min_conductance + (self.max_conductance - self.min_conductance) * value
            
            noise = np.random.normal(0, self.write_noise * conductance)
            conductance = np.clip(conductance + noise, self.min_conductance, self.max_conductance)
...
class QuantumPhotonicProcessor:
    ...
    def beam_splitter(self, mode1: int, mode2: int, reflectivity: float = 0.5) -> np.ndarray:
    ...
    def apply_unitary(self, unitary: np.ndarray):
        ...
        if unitary.shape != (self.state_dim, self.state_dim):
            raise ValueError(f"Unitary shape {unitary.shape} doesn't match state dim {self.state_dim}")
...
class HardwareAccelerator:
    ...
    def compile_model(self, model: nn.Module, input_shape: Tuple, 
        ...
        with self.lock:
            ...
            if self.backend == PhotonicBackend.FPGA and VITIS_AVAILABLE:
                ...
                try:
                    compile_to_fpga(model, target="aws-f1", opt_level=optimization_level)
                    self.compiled_models[model_hash] = {"type": "fpga", "model": "fpga_bitstream"}
                ...
                try:
                    ...
                    self.compiled_models[model_hash] = {"type": "tensorrt", "model": "tensorrt_engine"}
                except Exception as e:
                ...
                try:
                    ...
                    self.compiled_models[model_hash] = {"type": "onnx", "model": onnx_path}
...
class AnalogPhotonicEmulator:
    ...
    def __init__(self,
        ...
        # Metrics (thread-safe)
        self.metrics = defaultdict(lambda: {"count": 0, "total_time": 0.0, "energy_nj": 0.0})
    ...
    def emulate_photonic_mvm(self,
                            tensor1: Union[np.ndarray, torch.Tensor],
                            tensor2: Union[np.ndarray, torch.Tensor],
                            multiplexing: MultiplexingMode = MultiplexingMode.WAVELENGTH) -> torch.Tensor:
        ...
        try:
            if self.backend in [PhotonicBackend.CPU, PhotonicBackend.CUDA]:
                ...
                result_np = np.array(results).reshape(t1_mux.shape[:-1] + (t2.shape[-1],))
    ...
    def emulate_memristor_cim(self,
    ...
    def _apply_multiplexing(self, tensor: torch.Tensor, 
                           mode: MultiplexingMode) -> torch.Tensor:
        ...
        try:
            if mode == MultiplexingMode.WAVELENGTH:
                ...
                if tensor.shape[0] > 0:
                    time_slots = torch.linspace(0, 2*np.pi, tensor.shape[0], device=self.device)
                ...
                for i in range(min(3, tensor.shape[0])):
                    if tensor.shape[-1] > 0:
                        ...
                        tensor[i] *= torch.tensor(mode_profile, device=self.device, dtype=tensor.dtype)
                        
            elif mode == MultiplexingMode.SPACE_TIME_WAVELENGTH:
                if tensor.shape[-1] > 0:
                    wavelength_mod = torch.linspace(0.8, 1.2, tensor.shape[-1], device=self.device)
                    tensor *= wavelength_mod
                
                if tensor.dim() > 1 and tensor.shape[0] > 0:
                    time_mod = torch.linspace(0.9, 1.1, tensor.shape[0], device=self.device)
                    for _ in range(tensor.dim() - 1):
                        time_mod = time_mod.unsqueeze(-1)
                    tensor *= time_mod
                
                if tensor.dim() > 2 and tensor.shape[1] > 0:
                    space_mod = torch.linspace(0.95, 1.05, tensor.shape[1], device=self.device)
    ...
    def _propagate_optical(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            propagated = self.waveguide.propagate(np_array, self.params.propagation_length)
            
            # Convert back, preserving dtype if possible
            if torch.is_complex(tensor):
                return torch.tensor(propagated, device=self.device, dtype=tensor.dtype)
            else:
                return torch.tensor(np.real(propagated), device=self.device, dtype=tensor.dtype)
    ...
    def _quantum_process(self, tensor1: torch.Tensor, tensor2: torch.Tensor) -> torch.Tensor:
    ...
    def _apply_noise(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        try:
            np_array = tensor.cpu().numpy()
            noisy = self.noise_generator.apply_realistic_noise(np_array, self.noise_models)
            
            # Check for invalid values
            if np.isnan(noisy).any() or np.isinf(noisy).any():
                logger.warning("Noise application produced NaN/Inf, using original tensor")
                return tensor
    ...
    def train_in_situ(self, graph: Any, target: torch.Tensor, 
                     learning_rate: float = 0.1, iterations: int = 100) -> float:
        """In-situ training using photonic hardware."""
        try:
            if isinstance(graph, np.ndarray):
                self._validate_tensor(graph, "graph")
                mat = torch.tensor(graph, dtype=torch.float32, requires_grad=True, device=self.device)
            else:
    ...
    def deploy_to_fpga(self, model: nn.Module, input_shape: Tuple, 
                       target_fpga: str = "aws-f1") -> str:
    ...
    def save_state(self, directory: str):
        ...
        with self.calibration_lock:
            ...
            if self.calibration.amplitude_corrections is not None:
                calib_data["amplitude_corrections"] = self.calibration.amplitude_corrections.tolist()
    ...
    def load_state(self, directory: str):
        ...
        if os.path.exists(calib_path):
            ...
            with self.calibration_lock:
                ...
                self.calibration.temperature_coefficient = calib_data["temperature_coefficient"]
                self.calibration.wavelength_response = calib_data["wavelength_response"]
                if "amplitude_corrections" in calib_data:
                    self.calibration.amplitude_corrections = np.array(calib_data["amplitude_corrections"])
    ...
    for backend in backends:
        ...
        try:
            ...
            result = emulator.emulate_photonic_mvm(
                input_matrix,
                weight_matrix,
                multiplexing=MultiplexingMode.WAVELENGTH
            )
            ...
            final_loss = emulator.train_in_situ(
                weight_matrix,
                target,
                iterations=50
            )
