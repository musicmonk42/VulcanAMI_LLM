...
class EvolutionEngine:
    ...
    ALLOWED_NODE_TYPES = [
        'input', 'output', 'transform', 'aggregate', 'filter',
        'map', 'reduce', 'join', 'split', 'cache', 'optimize',
        'normalize', 'encode', 'decode', 'merge', 'route'
    ]
    ...
    def __init__(
        ...
        self.node_types = [t for t in self.ALLOWED_NODE_TYPES if t not in ['input', 'output']]
    ...
    def _sanitize_params(self, params: Dict) -> Dict:
        ...
        for key, value in params.items():
            ...
            if isinstance(value, bool):
                ...
                safe_params[safe_key] = self._sanitize_string(value, self.MAX_PARAM_LENGTH)
    ...
    def _generate_minimal_graph(self) -> Dict:
        ...
        return {
            ...
            'edges': [
                {'source': 'input', 'target': 'output', 'weight': 1.0}
            ],
            'metadata': {}
        }
            ...
            if seed_graph:
                ...
                for i in range(self.population_size):
                    individual = Individual(
                        graph=copy.deepcopy(validated_seed),
                        generation=0
                    )
    ...
    def evolve(
        ...
        for gen in range(generations):
            ...
            if diversity < self.diversity_threshold:
                logger.warning(f"Low diversity ({diversity:.4f}), injecting random individuals")
    ...
    def _mutate_remove_node(self, graph: Dict) -> Dict:
        ...
        removable = [n for n in graph['nodes'] 
                    if n.get('type') not in ['input', 'output']]
    ...
    def _mutate_swap_nodes(self, graph: Dict) -> Dict:
        ...
        swappable = [n for n in graph['nodes'] 
                    if n.get('type') not in ['input', 'output']]
    ...
    def _mutate_duplicate_subgraph(self, graph: Dict) -> Dict:
        ...
        duplicable = [n for n in graph['nodes'] 
                     if n.get('type') not in ['input', 'output']]
    ...
    def _crossover_single_point(self, graph1: Dict, graph2: Dict) -> Tuple[Dict, Dict]:
        ...
        child1['nodes'] = graph1['nodes'][:point] + graph2['nodes'][point:self.max_nodes]
        child2['nodes'] = graph2['nodes'][:point] + graph1['nodes'][point:self.max_nodes]
        ...
        if 'edges' in child1:
            child1['edges'] = [e for e in child1['edges'] 
                              if e['source'] in child1_ids and e['target'] in child1_ids][:self.max_edges]
        
        if 'edges' in child2:
            child2['edges'] = [e for e in child2['edges'] 
                              if e['source'] in child2_ids and e['target'] in child2_ids][:self.max_edges]
        ...
        for node_id in node_ids:
            if node_id not in visited:
                ...
                non_io_nodes = [
                    ...
                    if any(n['id'] == nid and n.get('type') not in ['input', 'output']
    ...
    def _exchange_subgraph(
        ...
        nodes_to_add = [copy.deepcopy(n) for n in graph2['nodes'] if n['id'] in subgraph2]
        ...
        child['edges'] = [
            e for e in child.get('edges', [])
            if e['source'] in child_node_ids and e['target'] in child_node_ids
        ]
        
        # Add edges from subgraph2
        edges_to_add = [
            copy.deepcopy(e) for e in graph2.get('edges', [])
            if e['source'] in subgraph2 and e['target'] in subgraph2
            and e['source'] in child_node_ids and e['target'] in child_node_ids
        ]
    ...
    def _generate_random_graph(self) -> Dict:
        ...
        for _ in range(extra_edges):
            ...
            if source['id'] != target['id']:
                ...
                edge_exists = any(e['source'] == source['id'] and e['target'] == target['id'] 
                                 for e in edges)
                if not edge_exists:
                    edges.append({
                        'source': source['id'],
                ...
                try:
                    ...
                    for ind_data in population_data['population']:
                        ind = Individual(
                            ...
                            id=ind_data.get('id', hashlib.md5(str(time.time()).encode()).hexdigest()[:8]),
                    ...
                    if population_data.get('best_individual'):
                        ...
                        self.best_individual = Individual(
                            ...
                            id=best_data.get('id', hashlib.md5(str(time.time()).encode()).hexdigest()[:8]),
                    ...
                    if 'config' in population_data:
                        ...
                        self.elitism_rate = config.get('elitism_rate', self.elitism_rate)
                    ...
                    logger.error(f"Error loading population data, restoring previous state: {e}")
