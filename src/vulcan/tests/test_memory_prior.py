...
class TestMemoryIndex:
    ...
    def test_size_limit(self):
        ...
        for i in range(15):
            entry = MemoryEntry(
                f"e{i}", np.random.rand(5), "tool_a", True, 0.9, 100, 50, time.time()
            )
            index.add(entry)
    ...
    def test_search_cosine(self):
        ...
        for i in range(5):
            ...
            entry = MemoryEntry(
                f"e{i}", features, "tool_a", True, 0.9, 100, 50, time.time()
            )
            index.add(entry)
...
class TestBayesianMemoryPrior:
    ...
    def test_dirichlet_prior(self, prior):
        ...
        # Add history
        for i in range(10):
            prior.update(np.random.rand(3), "tool_a", True, 0.9, 100, 50)
...
class TestAdaptivePriorSelector:
    ...
    def test_prior_type_selection_with_data(self, selector):
        ...
        for i in range(250):
            beta_prior.update(np.random.rand(3), "tool_a", True, 0.9, 100, 50)
...
class TestIntegration:
    ...
    def test_end_to_end_prior_workflow(self):
        ...
        prior = BayesianMemoryPrior(memory_system=None, prior_type=PriorType.BETA)
    ...
    def test_adaptive_selector_workflow(self):
        ...
        for i in range(30):
            selector.update_all(
                np.random.rand(3), "tool_a" if i < 20 else "tool_b", True, 0.9, 100, 50
            )
    ...
    def test_recency_weighting(self):
        ...
        for i in range(5):
            prior.update(
                features + np.random.rand(3) * 0.1, "tool_b", True, 0.9, 100, 50
            )
