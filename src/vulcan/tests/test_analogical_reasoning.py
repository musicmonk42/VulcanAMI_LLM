...
def solar_system_domain():
    ...
    return {
        ...
        "entities": [
            Entity(
                name="Sun",
                attributes={"type": "star", "mass": "large"},
                entity_type="star",
            ),
            Entity(
                name="Earth",
                attributes={"type": "planet", "orbit": "circular"},
                entity_type="planet",
            ),
            Entity(
                name="Mars",
        ...
        "relations": [
            Relation(
                predicate="orbits", arguments=["Earth", "Sun"], relation_type="binary"
            ),
            Relation(
                predicate="orbits", arguments=["Mars", "Sun"], relation_type="binary"
            ),
            Relation(
                predicate="attracts", arguments=["Sun", "Earth"], relation_type="binary"
            ),
        ],
        "attributes": {
...
def atom_domain():
    ...
    return {
        "entities": [
            Entity(
                name="Nucleus",
                attributes={"type": "nucleus", "mass": "large"},
                entity_type="nucleus",
            ),
            Entity(
                name="Electron1",
                attributes={"type": "electron", "orbit": "circular"},
                entity_type="electron",
            ),
            Entity(
                name="Electron2",
                attributes={"type": "electron", "orbit": "circular"},
        ...
        "relations": [
            Relation(
                predicate="orbits",
                arguments=["Electron1", "Nucleus"],
                relation_type="binary",
            ),
            Relation(
                predicate="orbits",
                arguments=["Electron2", "Nucleus"],
                relation_type="binary",
            ),
            Relation(
                predicate="attracts",
                arguments=["Nucleus", "Electron1"],
                relation_type="binary",
            ),
        ],
        "attributes": {
...
class TestBasicFunctionality:
    ...
    def test_entity_similarity(self):
        ...
        entity1 = Entity(
            name="Sun", attributes={"type": "star", "mass": "large"}, entity_type="star"
        )
        entity2 = Entity(
            name="Nucleus",
            attributes={"type": "nucleus", "mass": "large"},
        ...
        entity3 = Entity(
            name="Earth",
            attributes={"type": "planet", "mass": "small"},
            entity_type="planet",
        )
        ...
        entity5 = Entity(
            name="Jupiter",
            attributes={"type": "planet", "mass": "large"},
            entity_type="planet",
        )
    ...
    def test_find_structural_analogy(
        self, basic_reasoner, solar_system_domain, atom_domain
    ):
...
class TestEdgeCases:
    ...
    def test_mapping_with_no_entities(self, basic_reasoner):
        ...
        basic_reasoner.add_domain(
            "empty", {"entities": set(), "relations": [], "attributes": {}}
        )
...
class TestCacheManagement:
    ...
    def test_cache_size_limit(self):
        ...
        reasoner = AnalogicalReasoner(enable_caching=True, enable_learning=True)
        ...
        for i in range(20):
            domain = {
                "entities": [
                    Entity(name=f"E_{i}", attributes={"id": i}, entity_type="test")
                ],
                "relations": [],
                "attributes": {},
            }
            ...
            target = {
                "entities": [
                    Entity(name=f"T_{i}", attributes={"id": i}, entity_type="test")
                ],
                "relations": [],
                "attributes": {},
            }
...
class TestThreadSafety:
    ...
    def test_concurrent_domain_additions(self, basic_reasoner):
        ...
        def add_domain(i):
            domain = {
                "entities": [
                    Entity(name=f"Entity_{i}", attributes={"id": i}, entity_type="test")
                ],
                "relations": [],
                "attributes": {},
            }
        ...
        def update_cache(i):
            for j in range(10):
                basic_reasoner.update_cache(
                    f"source_{i}_{j}", f"target_{i}_{j}", {"mapping": j}, 0.8
                )
...
class TestNumericalStability:
    ...
    def test_attribute_similarity_with_none(self, basic_reasoner):
        ...
        try:
            sim3 = test_entity._attribute_similarity({"key": None}, {"key": None})
...
class TestLearning:
    ...
    def test_learn_from_successful_mapping(self, basic_reasoner, solar_system_domain):
        ...
        # Perform mapping
        result = basic_reasoner.find_structural_analogy(
            "solar_system", solar_system_domain, mapping_type=MappingType.STRUCTURAL
        )
...
class TestIntegration:
    ...
    def test_complete_analogy_workflow(
        self, basic_reasoner, solar_system_domain, atom_domain
    ):
    ...
    def test_multiple_analogies(self, basic_reasoner, solar_system_domain):
        ...
        target = {
            "entities": [
                Entity(name="X", attributes={"type": "center"}, entity_type="test")
            ],
            "relations": [],
            "attributes": {},
        }
    ...
    def test_cross_domain_transfer(self, basic_reasoner, solar_system_domain):
        ...
        result = basic_reasoner.cross_domain_transfer("solar_system", "atom", "Sun")
...
class TestPerformance:
    ...
    def test_repeated_queries_performance(self, basic_reasoner, solar_system_domain):
        ...
        target = {
            "entities": [
                Entity(name="X", attributes={"type": "test"}, entity_type="test")
            ],
            "relations": [],
            "attributes": {},
        }
    ...
    def test_memory_usage_bounded(self, basic_reasoner):
        ...
        assert (
            len(basic_reasoner.analogy_cache) <= basic_reasoner.max_analogy_cache_size
        )
        assert len(basic_reasoner.mapping_cache) <= basic_reasoner.max_cache_size
...
class TestPersistence:
    ...
    def test_statistics_preservation(
        self, basic_reasoner, solar_system_domain, tmp_path
    ):
...
class TestRegressions:
    ...
    def test_division_by_zero_in_surface_mapping(self, basic_reasoner):
        ...
        # This should not raise ZeroDivisionError
        result = basic_reasoner.find_structural_analogy(
            "empty", domain, mapping_type=MappingType.SURFACE
        )
...
# ============================================================================
# BUG M FIX Tests - Natural Language Analogical Queries
# ============================================================================


class TestBugMFixAnalogicalQueries:
    """Tests for BUG M FIX: Natural language analogical query handling"""

    def test_reason_with_query_string(self):
        """Test that reason() handles string input as natural language query"""
        engine = AnalogicalReasoningEngine()

        # String input should be treated as a natural language query
        result = engine.reason("The atom is like a solar system")

        # BUG M FIX: String input now triggers analogical query analysis
        assert isinstance(result, dict)
        assert result.get("confidence", 0) > 0
        assert "reasoning_type" in result or "found" in result

    def test_reason_with_query_dict(self):
        """Test that reason() handles dict with 'query' key"""
        engine = AnalogicalReasoningEngine()

        result = engine.reason({"query": "Map software concepts to biology"})

        # BUG M FIX: Dict with 'query' triggers analogical query analysis
        assert isinstance(result, dict)
        assert result.get("confidence", 0) > 0

    def test_domain_structure_extraction(self):
        """Test that _parse_domains_from_query extracts structure correctly"""
        engine = AnalogicalReasoningEngine()

        query = """
        Domain S (software): leader election, quorum, split brain, fencing token
        Domain T (biology): control centers, hormone cascades, metabolic stability
        Task: Map concepts from software to biology
        """

        result = engine.reason({"query": query})

        # Should extract domain structure
        assert isinstance(result, dict)
        assert result.get("found", False) or result.get("reasoning_type") == "analogical"
        # Should have source and target domains
        if "source_domain" in result:
            assert result["source_domain"] in ("source", "software")
        if "target_domain" in result:
            assert result["target_domain"] in ("target", "biology")

    def test_effect_of_change_analysis(self):
        """Test analogical reasoning for effect-of-change questions"""
        engine = AnalogicalReasoningEngine()

        query = """
        Domain S (distributed systems): quorum size affects stability
        Domain T (biology): consensus threshold affects metabolic stability
        Task: If we increase quorum size in the source domain, what happens
        to the analogous concept in the target domain?
        """

        result = engine.reason({"query": query})

        # Should handle effect-of-change analysis
        assert isinstance(result, dict)
        assert result.get("confidence", 0) > 0

    def test_minimum_confidence_floor(self):
        """Test that reason() returns minimum confidence floor even for unsupported input"""
        engine = AnalogicalReasoningEngine()

        # Unsupported input format (neither dict with query nor string)
        result = engine.reason(12345)  # Number input

        # BUG M FIX: Should return minimum confidence floor (0.15)
        assert isinstance(result, dict)
        assert result.get("confidence", 0) >= 0.15
