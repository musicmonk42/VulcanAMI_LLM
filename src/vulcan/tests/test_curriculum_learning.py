...
class TestCurriculumLearner:
    ...
    def test_self_paced_strategy(self):
        ...
        for i in range(15):
            learner.get_next_batch(performance=0.88 + np.random.normal(0, 0.02), batch_size=1)
    ...
    def test_task_sampling_with_clusters(self, learner):
        ...
        learner.task_clusters = {
            str({'id': i}): i % 3 for i in range(20)
        }
...
class TestEdgeCases:
    ...
    def test_callable_difficulty_estimator(self):
        ...
        assert curriculum[0][0]['custom_difficulty'] <= curriculum[-1][-1]['custom_difficulty']
