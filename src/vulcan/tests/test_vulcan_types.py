...
from src.vulcan.vulcan_types import (
    ...
    # IR types
    IRNodeType, IREdgeType, IRNode, IREdge, IRGraph,
    
    # Agent types
    AgentRole, AgentCapability, AgentProfile,
    
    # Action types
    ActionCategory, ActionSpecification, ActionResult,
    
    # Event types
    EventPriority, EventCategory, EventMetadata, Event,
    
    # System state types
    ComponentHealth, SystemHealth, KnowledgeState,
    CommunicationState, SecurityState, CompleteSystemState,
    
    # Orchestrator-specific types
    SystemState, Episode, ProvRecord, SA_Latents, HealthSnapshot,
    
    # Validation
    TypeValidator, enforce_types,
    
    # Schemas
    IRSchemas, SchemaMigrator,
    
    # Registry
    TypeRegistry,
    
    # Serialization
    EnhancedJSONEncoder, EnhancedJSONDecoder
)
...
class TestIRNode:
    ...
    def test_node_validation_missing_required_params(self):
        ...
        assert node.validate() is False
        assert any("Parameter validation failed" in err for err in node.validation_errors)
    ...
    def test_node_validation_negative_resources(self):
        ...
        assert node.validate() is False
        assert any("Invalid memory requirement" in err for err in node.validation_errors)
...
class TestIRGraph:
    ...
    def test_graph_validation_edge_to_nonexistent_node(self):
        ...
        graph = IRGraph(
            ...
            edges=[
                IREdge(source="node1", target="node2", type=IREdgeType.DATA)  # node2 doesn't exist
            ]
        )
    ...
    def test_graph_validation_incompatible_version(self):
        ...
        assert graph.validate() is False
        assert any("Incompatible grammar version" in err for err in graph.validation_errors)
    
    def test_graph_safety_cycle_detection(self):
        """Test detection of cycles in safety-critical paths."""
        nodes = [
            IRNode(id="node1", type=IRNodeType.SAFETY_CHECK, params={'safety_level': 'high', 'constraints': []}),
            IRNode(id="node2", type=IRNodeType.SAFETY_CHECK, params={'safety_level': 'high', 'constraints': []}),
            IRNode(id="node3", type=IRNodeType.SAFETY_CHECK, params={'safety_level': 'high', 'constraints': []})
        ]
        edges = [
            IREdge(source="node1", target="node2", type=IREdgeType.SAFETY),
            IREdge(source="node2", target="node3", type=IREdgeType.SAFETY),
            IREdge(source="node3", target="node1", type=IREdgeType.SAFETY)  # Creates cycle
        ]
        graph = IRGraph(
            grammar_version="1.3.1",
            nodes=nodes,
        ...
        assert graph.validate() is False
        assert any("Cycle detected in safety-critical path" in err for err in graph.validation_errors)
    ...
    def test_complete_workflow(self):
        ...
        graph = IRGraph(
            ...
            nodes=[
                ...
                IRNode(id="compute", type=IRNodeType.COMPUTE, params={'operation': 'add'}),
...
class TestKnowledgeState:
    ...
    def test_knowledge_state_learning_statistics(self):
        ...
        knowledge = KnowledgeState(
            ...
            positive_experiences=700,
            negative_experiences=300
        )
...
class TestCommunicationState:
    ...
    def test_communication_state_protocol_usage(self):
        ...
        comm = CommunicationState(
            protocol_usage={'http': 500, 'websocket': 200}
        )
...
class TestIRGraphEdgeCases:
    ...
    def test_graph_with_disconnected_nodes(self):
        ...
        graph = IRGraph(
            grammar_version=SchemaVersion.get_version(),
            nodes=[
                IRNode(id="node1", type=IRNodeType.COMPUTE, params={'operation': 'add'}),
                IRNode(id="node2", type=IRNodeType.COMPUTE, params={'operation': 'mul'}),
                IRNode(id="node3", type=IRNodeType.COMPUTE, params={'operation': 'sub'})
            ],
            edges=[
                IREdge(source="node1", target="node2", type=IREdgeType.DATA)
    ...
    def test_graph_with_self_loop_non_safety(self):
        ...
        graph = IRGraph(
            grammar_version=SchemaVersion.get_version(),
            nodes=[
                IRNode(id="node1", type=IRNodeType.COMPUTE, params={'operation': 'test'})
            ],
            edges=[
                IREdge(source="node1", target="node1", type=IREdgeType.DATA)
            ]
        )
    ...
    def test_graph_with_multiple_edge_types(self):
        ...
        graph = IRGraph(
            ...
            nodes=[
                IRNode(id="node1", type=IRNodeType.COMPUTE, params={'operation': 'test'}),
                IRNode(id="node2", type=IRNodeType.COMPUTE, params={'operation': 'test'})
            ],
            edges=[
                IREdge(source="node1", target="node2", type=IREdgeType.DATA),
