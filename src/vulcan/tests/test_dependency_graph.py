...
class TestDependencyEdge:
    ...
    def test_edge_cost_calculation(self, mock_gap):
        ...
        gap1 = mock_gap
        gap2 = KnowledgeGap(type="test", domain="test2", priority=0.5, estimated_cost=10.0)
...
class TestCycleAwareDependencyGraph:
    ...
    def test_add_edge_without_cycle(self, dependency_graph):
        ...
        gap1 = KnowledgeGap(type="test1", domain="d1", priority=0.8, estimated_cost=10.0)
        gap2 = KnowledgeGap(type="test2", domain="d2", priority=0.7, estimated_cost=15.0)
    ...
    def test_descendants(self, dependency_graph):
        ...
        gap1 = KnowledgeGap(type="test1", domain="d1", priority=0.8, estimated_cost=10.0)
        gap2 = KnowledgeGap(type="test2", domain="d2", priority=0.7, estimated_cost=15.0)
        gap3 = KnowledgeGap(type="test3", domain="d3", priority=0.6, estimated_cost=12.0)
    ...
    def test_thread_safety(self, dependency_graph):
        ...
        threads = [threading.Thread(target=add_gaps_and_edges, args=(i,)) 
                  for i in range(3)]
...
class TestDependencyAnalyzer:
    ...
    def test_identify_root_causes(self, dependency_analyzer, dependency_graph):
        ...
        gap1 = KnowledgeGap(type="test1", domain="d1", priority=0.9, estimated_cost=10.0)
        gap2 = KnowledgeGap(type="test2", domain="d2", priority=0.8, estimated_cost=15.0)
        gap3 = KnowledgeGap(type="test3", domain="d3", priority=0.7, estimated_cost=12.0)
    ...
    def test_calculate_unlock_potential(self, dependency_analyzer, dependency_graph):
        ...
        potential = dependency_analyzer.calculate_unlock_potential(gap1, dependency_graph)
...
class TestROICalculator:
    ...
    def test_calculate_base_roi(self, roi_calculator):
        ...
        assert base_roi > 0
        assert base_roi == pytest.approx(0.8 / 10.0 * 1.1, rel=0.01)  # 1.1 is decomposition bonus
    ...
    def test_apply_dependency_penalty(self, roi_calculator):
        ...
        roi_with_penalty = roi_calculator.apply_dependency_penalty(gap, ancestors_count=5)
        base_roi = roi_calculator.calculate_base_roi(gap)
