...
class TestAdversarialValidator:
    ...
    def test_random_seed_reproducibility(self):
        ...
        result1 = validator1._fgsm_attack(action, context, validator1.default_attack_config)
        
        validator2 = AdversarialValidator(epsilon=0.1, num_attacks=3, random_seed=42)
        result2 = validator2._fgsm_attack(action, context, validator2.default_attack_config)
    ...
    def test_validate_robustness_basic(self, adversarial_validator, sample_action, sample_context):
    ...
    def test_validate_robustness_timeout(self, adversarial_validator, sample_action, sample_context):
    ...
    def test_fgsm_attack(self, adversarial_validator, sample_action, sample_context):
        ...
        perturbed = adversarial_validator._fgsm_attack(sample_action, sample_context, config)
    ...
    def test_pgd_attack(self, adversarial_validator, sample_action, sample_context):
        ...
        perturbed = adversarial_validator._pgd_attack(sample_action, sample_context, config)
        
        assert len(perturbed) > 0
        assert all(p.get('attack_type') == 'pgd' for p in perturbed)
        assert all('iterations' in p for p in perturbed)
    
    def test_semantic_attack(self, adversarial_validator, sample_action, sample_context):
        """Test semantic attack generation."""
        config = AttackConfig(epsilon=0.1)
        perturbed = adversarial_validator._semantic_attack(sample_action, sample_context, config)
        
        # Should generate type substitutions
        assert len(perturbed) > 0
        type_changes = [p for p in perturbed if p.get('type') != sample_action.get('type')]
    ...
    def test_semantic_attack_text(self, adversarial_validator, sample_context):
        ...
        config = AttackConfig(epsilon=0.1)
        perturbed = adversarial_validator._semantic_attack(action, sample_context, config)
        
        # Should substitute words
        text_changes = [p for p in perturbed if p.get('text') != action.get('text')]
        assert len(text_changes) > 0
    
    def test_boundary_attack(self, adversarial_validator, sample_action, sample_context):
        """Test boundary attack generation."""
        config = AttackConfig(epsilon=0.1)
        perturbed = adversarial_validator._boundary_attack(sample_action, sample_context, config)
        
        assert len(perturbed) > 0
        
        # Check for extreme values
        confidence_values = [p.get('confidence') for p in perturbed if 'confidence' in p]
    ...
    def test_boundary_attack_overflow(self, adversarial_validator, sample_context):
        ...
        config = AttackConfig(epsilon=0.1)
        perturbed = adversarial_validator._boundary_attack(action, sample_context, config)
    ...
    def test_trojan_attack(self, adversarial_validator, sample_action, sample_context):
        ...
        config = AttackConfig(epsilon=0.1)
        perturbed = adversarial_validator._trojan_attack(sample_action, sample_context, config)
    ...
    def test_deepfool_attack(self, adversarial_validator, sample_action, sample_context):
        """Test DeepFool attack generation."""
        config = AttackConfig(epsilon=0.1, max_iterations=20, early_stop=True)
        perturbed = adversarial_validator._deepfool_attack(sample_action, sample_context, config)
        
        assert len(perturbed) > 0
        assert all(p.get('attack_type') == 'deepfool' for p in perturbed)
        assert all('iterations_used' in p for p in perturbed)
    
    def test_deepfool_convergence(self, adversarial_validator, sample_action, sample_context):
    ...
    def test_carlini_wagner_attack(self, adversarial_validator, sample_action, sample_context):
        """Test Carlini & Wagner attack generation."""
        config = AttackConfig(epsilon=0.1, max_iterations=50, alpha=0.01)
        perturbed = adversarial_validator._carlini_wagner_attack(sample_action, sample_context, config)
    ...
    def test_jsma_attack(self, adversarial_validator, sample_action, sample_context):
        ...
        perturbed = adversarial_validator._jsma_attack(sample_action, sample_context, config)
        
        assert len(perturbed) > 0
        assert all(p.get('attack_type') == 'jsma' for p in perturbed)
        assert all('features_modified' in p for p in perturbed)
    
    def test_universal_attack(self, adversarial_validator, sample_action, sample_context):
        """Test universal adversarial perturbation."""
        config = AttackConfig(epsilon=0.1)
        perturbed = adversarial_validator._universal_attack(sample_action, sample_context, config)
        
        assert len(perturbed) > 0
        assert all(p.get('attack_type') == 'universal' for p in perturbed)
        assert all('perturbation_scale' in p for p in perturbed)
    
    def test_targeted_attack(self, adversarial_validator, sample_action, sample_context):
        """Test targeted misclassification attack."""
        config = AttackConfig(epsilon=0.1, targeted=True)
        perturbed = adversarial_validator._targeted_attack(sample_action, sample_context, config)
        
        assert len(perturbed) > 0
        assert all(p.get('attack_type') == 'targeted' for p in perturbed)
        assert all('target_class' in p for p in perturbed)
    
    def test_default_validate(self, adversarial_validator, sample_action, sample_context):
    ...
    def test_default_validate_detects_adversarial(self, adversarial_validator, sample_context):
    ...
    def test_default_validate_detects_malicious(self, adversarial_validator, sample_context):
    ...
    def test_identify_vulnerability_patterns(self, adversarial_validator):
        ...
        patterns = adversarial_validator._identify_vulnerability_patterns(violations, attack_results)
    ...
    def test_get_attack_stats(self, adversarial_validator, sample_action, sample_context):
...
class TestFormalVerifier:
    ...
    def test_verify_action_model_checking(self, formal_verifier, sample_action, sample_context):
    ...
    def test_verify_action_theorem_proving(self, formal_verifier, sample_action, sample_context):
    ...
    def test_verify_action_symbolic_execution(self, formal_verifier, sample_action, sample_context):
    ...
    def test_verify_action_abstract_interpretation(self, formal_verifier, sample_action, sample_context):
    ...
    def test_verify_action_caching(self, formal_verifier, sample_action, sample_context):
    ...
    def test_probabilistic_property_checking(self, formal_verifier, sample_action, sample_context):
    ...
    def test_generate_proof_certificate_success(self, formal_verifier, sample_action, sample_context):
        """Test proof certificate generation for verified action."""
        certificate = formal_verifier.generate_proof_certificate(sample_action, sample_context)
    ...
    def test_get_verification_stats(self, formal_verifier, sample_action, sample_context):
...
class TestEdgeCases:
    ...
    def test_invalid_attack_type(self, adversarial_validator, sample_action, sample_context):
        ...
        report = adversarial_validator.validate_robustness(
            sample_action,
            sample_context,
            attack_types=[FakeAttackType()]
        )
        
        assert isinstance(report, SafetyReport)
    
    def test_property_exception_handling(self, formal_verifier, sample_action, sample_context):
    ...
    def test_zero_epsilon_attack(self):
        ...
        perturbed = validator._fgsm_attack(action, context, validator.default_attack_config)
