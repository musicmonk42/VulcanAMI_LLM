...
from vulcan.reasoning.selection.tool_selector import (
    ToolSelector, SelectionRequest, SelectionResult, SelectionMode,
    StochasticCostModel, MultiTierFeatureExtractor, CalibratedDecisionMaker,
    ValueOfInformationGate, DistributionMonitor, ToolSelectionBandit,
...
class TestDistributionMonitor:
    ...
    def test_shift_detection_after_warmup(self):
        ...
        for i in range(150):
            features = base_features + np.random.randn(128) * 0.01  # Low variance, similar
...
class TestToolSelectionBandit:
    ...
    def test_update_from_execution(self):
        ...
        if not bandit.is_enabled:
            pytest.skip("Contextual bandit module not available, skipping orchestrator test.")
        ...
        bandit.update_from_execution(
            features, 'symbolic', 
            quality=0.9, time_ms=1500, energy_mj=150,
            constraints={}
        )
    ...
    def test_get_statistics(self):
        ...
        for i in range(5):
            ...
            bandit.update_from_execution(
                features, 'symbolic',
                quality=0.8, time_ms=1000, energy_mj=100,
                constraints={}
            )
...
class TestToolSelector:
    ...
    def test_available_tools(self):
        ...
        expected_tools = ['symbolic', 'probabilistic', 'causal', 
                         'analogical', 'multimodal']
    ...
    def test_select_and_execute_basic(self):
        ...
        assert result.selected_tool in selector.tool_names or result.selected_tool == 'none'
...
class TestToolSelectorIntegration:
    ...
    def test_learning_updates(self):
        ...
        # ('arm_stats' or 'status').
        assert ('arm_stats' in bandit_stats or 
                'status' in bandit_stats or 
                'active_bandit' in bandit_stats)
        # END CRITICAL FIX
        # **************************************************************************
