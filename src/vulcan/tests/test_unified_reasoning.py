...
def force_shutdown_reasoner(reasoner, timeout=0.5):
    ...
    component_names = ['portfolio_executor', 'warm_pool', 'cache', 
                       'tool_selector', 'safety_governor', 'tool_monitor',
                       'processor', 'runtime']
...
class TestUnifiedReasoner:
    ...
    def test_sequential_reasoning(self, reasoner):
        ...
        assert result.conclusion is not None
        assert result.reasoning_chain is not None
        assert len(result.reasoning_chain.steps) >= 2  # Should have steps from both tasks
    ...
    def test_ensemble_reasoning(self, reasoner):
        ...
        mock_prob = Mock()
        mock_prob.reason_with_uncertainty = Mock(return_value={
            'conclusion': 'prob_result',
            'confidence': 0.7
        })
        
        mock_symbolic = Mock()
        mock_symbolic.add_rule = Mock()
        mock_symbolic.query = Mock(return_value={
    ...
    def test_topological_sort(self, reasoner):
        ...
        tasks = [
            ReasoningTask(task_id="t1", task_type=ReasoningType.PROBABILISTIC,
                         input_data="test1", query={}),
            ReasoningTask(task_id="t2", task_type=ReasoningType.PROBABILISTIC,
                         input_data="test2", query={}),
            ReasoningTask(task_id="t3", task_type=ReasoningType.PROBABILISTIC,
                         input_data="test3", query={})
        ]
...
class TestSpecializedReasoning:
    ...
    def test_reason_by_analogy(self, reasoner):
        ...
        mock_analogical.find_structural_analogy = Mock(return_value={
            'found': True,
            'confidence': 0.8,
