    ...
    def detect(self, x_var: str, y_var: str) -> Optional[Invariant]:
        ...
        if r_squared >= self.r_squared_threshold:
            return Invariant(
                ...
                parameters={'slope': slope, 'intercept': intercept, 'r_squared': r_squared}
            )
        return None
...
def multiple_invariants():
    return [
        Invariant(type=InvariantType.CONSERVATION, expression="a + b = 5",
                  variables=['a', 'b'], confidence=0.9,
                  parameters={'conserved_value': 5.0, 'tolerance': 0.01}),
        Invariant(type=InvariantType.CONSTRAINT, expression="c >= 0",
                  variables=['c'], confidence=0.95,
                  parameters={'bound_type': 'lower', 'bound_value': 0.0}),
        Invariant(type=InvariantType.LINEAR, expression="d = 3.0 * e + 2.0",
                  variables=['d', 'e'], confidence=0.92,
                  parameters={'slope': 3.0, 'intercept': 2.0, 'tolerance': 0.1}),
        Invariant(type=InvariantType.SYMMETRY, expression="f ≈ g",
                  variables=['f', 'g'], confidence=0.88,
