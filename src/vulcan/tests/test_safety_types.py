...
class TestCondition:
    ...
    def test_condition_to_dict(self):
        ...
        condition = Condition(
            field="temp", operator=">=", value=100, description="Temperature check"
        )
    ...
    def test_condition_string_representation(self):
        ...
        condition = Condition(
            field="speed", operator="<=", value=100, description="Speed limit"
        )
...
class TestSafetyReport:
    ...
    def test_safety_report_creation(self):
        ...
        report = SafetyReport(safe=True, confidence=0.95, violations=[], reasons=[])
    ...
    def test_severity_calculation(self):
        ...
        # Critical violations
        report = SafetyReport(
            safe=False, confidence=0.5, violations=[SafetyViolationType.ADVERSARIAL]
        )
    ...
    def test_merge_reports(self):
        ...
        report1 = SafetyReport(
            safe=True, confidence=0.9, violations=[], reasons=["All good"]
        )
...
class TestSafetyConstraint:
    ...
    def test_constraint_check_boolean(self):
        ...
        constraint = SafetyConstraint(
            name="test", type="hard", check_function=check_func
        )
    ...
    def test_constraint_check_tuple(self):
        ...
        constraint = SafetyConstraint(
            name="confidence_check", type="soft", check_function=check_func
        )
    ...
    def test_constraint_check_exception_handling(self):
        ...
        constraint = SafetyConstraint(
            name="failing", type="hard", check_function=failing_check
        )
...
class TestToolSafetyContract:
    ...
    def test_validate_preconditions(self):
        ...
        # All conditions met
        valid, failures = contract.validate_preconditions({"ready": True, "load": 0.5})
        assert valid is True
        assert len(failures) == 0

        # One condition fails
        valid, failures = contract.validate_preconditions({"ready": False, "load": 0.5})
    ...
    def test_check_invariants(self):
        ...
        valid, failures = contract.check_invariants({"database_connected": True})
        assert valid is True

        valid, failures = contract.check_invariants({"database_connected": False})
        assert valid is False
    ...
    def test_check_veto(self):
        ...
        # No veto triggered
        vetoed, reasons = contract.check_veto({"emergency_mode": False})
        assert vetoed is False

        # Veto triggered
        vetoed, reasons = contract.check_veto({"emergency_mode": True})
...
class TestSafetyConfig:
    ...
    def test_config_serialization(self):
        ...
        original = SafetyConfig(
            enable_audit_logging=True, safety_thresholds={"test": 0.7}
        )
...
class TestSafetyMetrics:
    ...
    def test_metrics_update_unsafe(self):
        ...
        report = SafetyReport(
            safe=False, confidence=0.6, violations=[SafetyViolationType.BIAS]
        )
        metrics.update(report)
    ...
    def test_top_violations(self):
        ...
        for _ in range(5):
            metrics.update(
                SafetyReport(
                    safe=False, confidence=0.5, violations=[SafetyViolationType.BIAS]
                )
            )
...
class TestSafetyException:
    ...
    def test_exception_creation(self):
        ...
        exception = SafetyException(
            "Test error", SafetyViolationType.ADVERSARIAL, metadata={"details": "test"}
        )
    ...
    def test_exception_to_dict(self):
        ...
        exception = SafetyException(
            "Error message", SafetyViolationType.COMPLIANCE, metadata={"code": 123}
        )
    ...
    def test_exception_raise(self):
        ...
        with pytest.raises(SafetyException) as exc_info:
            raise SafetyException("Critical error", SafetyViolationType.ADVERSARIAL)
