...
class TestCurriculumLearning:
    ...
    def test_curriculum_generation(self):
        ...
        learner = CurriculumLearner(pacing_strategy=PacingStrategy.THRESHOLD)

        # Create tasks with varying difficulty
        tasks = []
        for i in range(30):
            tasks.append(
                {"id": f"task_{i}", "difficulty": i / 30.0, "complexity": i / 30.0}
            )
...
class TestMetaLearning:
    ...
    def test_maml_adaptation(self, fast_learning_config):
        ...
        try:
            ...
            meta_learner = MetaLearner(
                base_model=base_model, config=fast_learning_config
            )
...
class TestMetacognition:
    ...
    def test_learning_efficiency_analysis(self):
        ...
        for i in range(20):
            monitor.update_self_model({"loss": np.random.random(), "modality": "test"})
...
class TestWorldModel:
    ...
    def test_dynamics_prediction(self):
        ...
        model = UnifiedWorldModel(
            state_dim=TEST_EMBEDDING_DIM, ensemble_size=2, use_attention=False
        )
    ...
    def test_planning_algorithms(self):
        ...
        candidate_actions = [torch.randn(TEST_EMBEDDING_DIM) for _ in range(3)]

        # Test each algorithm
        for algo in [PlanningAlgorithm.GREEDY, PlanningAlgorithm.BEAM_SEARCH]:
            action, info = model.plan_actions(
                current_state, candidate_actions, horizon=2, algorithm=algo
            )
...
class TestParameterHistory:
    ...
    def test_checkpoint_saving(self, temp_dir, fast_learning_config):
        ...
        manager = ParameterHistoryManager(
            base_path=temp_dir, config=fast_learning_config
        )
    ...
    def test_trajectory_recording(self, temp_dir, fast_learning_config):
        ...
        # Start trajectory
        traj_id = manager.start_trajectory(task_id="test_task", agent_id="test_agent")
