...
def mock_objective_hierarchy():
    ...
    hierarchy.get_priority_order = Mock(return_value=[
        'prediction_accuracy', 'safety', 'efficiency'
    ])
...
class TestNegotiateMultiAgentProposals:
    ...
    def test_validation_performed(self, negotiator, sample_proposals):
        ...
        if result.outcome != NegotiationOutcome.DEADLOCK:
            ...
            is_valid = negotiator.validate_negotiated_objectives(result.agreed_objectives)
...
class TestFindParetoFrontier:
    ...
    def test_find_frontier_basic(self, negotiator):
        ...
        objective_space = {
            'objectives': ['prediction_accuracy', 'efficiency']
        }
...
class TestNegotiationStrategies:
    ...
    def test_weighted_average_strategy(self, negotiator, sample_proposals):
        ...
        assert isinstance(result, NegotiationResult)
        assert result.outcome in [NegotiationOutcome.COMPROMISE, NegotiationOutcome.CONSENSUS]
...
class TestHelperMethods:
    ...
    def test_assess_difficulty(self, negotiator):
        ...
        analysis = {'conflicts': conflicts, 'avg_flexibility': 0.75, 'has_conflicts': False}
...
class TestEdgeCases:
    ...
    def test_all_proposals_same_objective(self, negotiator):
        ...
        proposals = [
            {'agent_id': f'agent_{i}', 'objective': 'prediction_accuracy', 
             'target_value': 0.95, 'weight': 1.0}
            for i in range(3)
        ]
    ...
    def test_extreme_flexibility(self, negotiator):
        ...
        proposals = [
            {'agent_id': 'a1', 'objective': 'obj1', 'target_value': 0.9, 
             'weight': 1.0, 'flexibility': 0.0},
            {'agent_id': 'a2', 'objective': 'obj2', 'target_value': 0.8, 
             'weight': 1.0, 'flexibility': 1.0}
        ]
    ...
    def test_many_objectives(self, negotiator):
        ...
        proposals = [
            {'agent_id': f'a{i}', 'objective': f'obj{i}', 
             'target_value': 0.8, 'weight': 1.0}
            for i in range(10)
        ]
