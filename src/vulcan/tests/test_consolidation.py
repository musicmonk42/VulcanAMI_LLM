...
from vulcan.memory.consolidation import (
    ConsolidationStrategy, ClusteringAlgorithm,
    KMeansClustering, DBSCANClustering, HierarchicalClustering,
    MemoryConsolidator, MemoryOptimizer, IndexManager, CacheManager
)
...
class TestDBSCANClustering:
    ...
    def test_cluster_with_noise(self):
        ...
        embeddings = np.array([
            [0, 0], [0, 0.1], [0.1, 0],  # Dense cluster
            [5, 5], [5, 5.1], [5.1, 5],  # Another dense cluster
            [10, 10]  # Noise point
        ])
...
class TestMemoryConsolidator:
    ...
    def test_select_cluster_representative(self, consolidator, memories_with_embeddings):
        ...
        assert representative in cluster_memories or representative.id.startswith('merged_')
...
class TestMemoryOptimizer:
    ...
    def test_remove_duplicates(self, optimizer):
        ...
        memories = {
            "1": Memory(id="1", type=MemoryType.SEMANTIC, content="Same", importance=0.5),
            "2": Memory(id="2", type=MemoryType.SEMANTIC, content="Same", importance=0.7),
            "3": Memory(id="3", type=MemoryType.SEMANTIC, content="Different", importance=0.6)
        }
    ...
    def test_update_indices(self, optimizer):
        """Test index updating."""
        memories = {
            "1": Memory(id="1", type=MemoryType.SEMANTIC, content="Test")
        }
...
class TestIndexManager:
    ...
    def test_indices_sorting(self):
        ...
        manager = IndexManager()
        memories = {
            "1": Memory(id="1", type=MemoryType.SEMANTIC, content="A", 
                       timestamp=100, importance=0.5),
            "2": Memory(id="2", type=MemoryType.SEMANTIC, content="B", 
                       timestamp=200, importance=0.8),
            "3": Memory(id="3", type=MemoryType.SEMANTIC, content="C", 
                       timestamp=150, importance=0.3)
        }
...
class TestCacheManager:
    ...
    def test_optimize_caches(self, sample_memories):
        ...
        memories_with_embeddings = sum(1 for m in sample_memories if m.embedding is not None)
        assert len(manager.embedding_cache) <= min(memories_with_embeddings, manager.max_cache_size // 2)
...
class TestIntegration:
    ...
    def test_consolidation_with_optimization(self, consolidator, optimizer, sample_memories):
    ...
    def test_multiple_consolidations(self, consolidator, sample_memories):
        ...
        for strategy in [ConsolidationStrategy.IMPORTANCE_BASED,
            ...
            if len(result) > 3:
                result = consolidator.consolidate(
                    result,
                    strategy=strategy,
                    target_count=max(3, len(result) // 2)
                )
