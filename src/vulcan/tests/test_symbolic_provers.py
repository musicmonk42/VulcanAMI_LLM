...
from src.vulcan.reasoning.symbolic.core import (
    Variable, Constant, Function, Literal, Clause, ProofNode
)
...
class TestTableauProver:
    ...
    def test_tableau_collect_ground_terms(self):
        ...
        clause = Clause([
            Literal('P', [Constant('a'), Constant('b')], negated=False)
        ])
...
class TestParallelProver:
    ...
    def test_parallel_simple_proof(self):
        ...
        assert proven
        assert proof is not None
        assert method in ['tableau', 'resolution', 'model_elimination', 'connection', 'natural_deduction']
