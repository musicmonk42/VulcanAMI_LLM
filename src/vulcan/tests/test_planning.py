...
from src.vulcan.planning import (
    PlanningMethod, PlanStep, Plan, MCTSNode, MonteCarloTreeSearch,
    EnhancedHierarchicalPlanner, PlanningState, PlanLibrary, PlanMonitor,
    PlanRepairer, ResourceAwareCompute, ResourceAllocator, DistributedCoordinator,
    ConsensusProtocol
)
from src.vulcan.config import ActionType, GoalType, AgentConfig
...
class TestPlan:
    ...
    def test_plan_optimization(self):
        ...
        step3 = PlanStep(step_id='step_3', action='action_3', dependencies=['step_1', 'step_2'])
...
class TestMonteCarloTreeSearch:
    ...
    async def test_mcts_search_async(self):
        ...
        # Pass actions to search
        best_action, value = await mcts.search_async(initial_state, expected_actions.copy())
...
class TestEnhancedHierarchicalPlanner:
    ...
    def test_identify_dependencies(self, enhanced_planner):
        ...
        deps = enhanced_planner._identify_dependencies('optimize_model', previous_subgoals)
...
class TestPlanRepairer:
    ...
    def test_create_recovery_plan(self, test_plan, plan_step):
        ...
        recovery = repairer.create_recovery_plan(
            test_plan,
            plan_step,
            'precondition failure: missing data'
        )
    ...
    def test_classify_failure(self):
        ...
        assert repairer._classify_failure('precondition failed') == 'precondition_failure'
        assert repairer._classify_failure('insufficient resources') == 'resource_shortage'
