...
def mock_memory_system(memory_config):
    ...
    class MockMemorySystem(BaseMemorySystem):
        def store(self, content, **kwargs):
            return Memory(id="mock_1", type=MemoryType.SEMANTIC, content=content)

        def retrieve(self, query):
            return RetrievalResult(
                memories=[], scores=[], query_time_ms=1.0, total_matches=0
            )
...
class TestMemory:
    ...
    def test_memory_creation_minimal(self):
        ...
        memory = Memory(id="test1", type=MemoryType.SEMANTIC, content="Test")
    ...
    def test_memory_validation_importance(self, caplog):
        ...
        memory = Memory(
            id="test", type=MemoryType.SEMANTIC, content="", importance=-0.5
        )
    ...
    def test_memory_validation_decay_rate(self, caplog):
        ...
        memory = Memory(
            id="test", type=MemoryType.SEMANTIC, content="", decay_rate=-0.1
        )
    ...
    def test_memory_validation_access_count(self, caplog):
        ...
        memory = Memory(
            id="test", type=MemoryType.SEMANTIC, content="", access_count=-5
        )
        assert memory.access_count == 0
    ...
    def test_memory_validation_timestamp(self, mock_time, caplog):
        ...
        memory = Memory(
            id="test", type=MemoryType.SEMANTIC, content="", timestamp=1100.0
        )
    ...
    def test_compute_salience_edge_cases(self, caplog):
        ...
        memory = Memory(
            id="test", type=MemoryType.SEMANTIC, content="", timestamp=1000.0
        )
...
class TestMemoryConfig:
    ...
    def test_config_validation_capacities(self, caplog):
        ...
        config = MemoryConfig(max_working_memory=0, max_short_term=-5, max_long_term=0)
    ...
    def test_config_validation_thresholds(self, caplog):
        ...
        config = MemoryConfig(consolidation_threshold=1.5, similarity_threshold=-0.5)
    ...
    def test_config_validation_intervals(self, caplog):
        ...
        config = MemoryConfig(consolidation_interval=-10.0, checkpoint_interval=-5.0)
...
class TestRetrievalResult:
    ...
    def test_result_creation(self):
        ...
        result = RetrievalResult(
            memories=memories, scores=scores, query_time_ms=5.5, total_matches=10
        )
    ...
    def test_result_validation_length_mismatch(self, caplog):
        ...
        result = RetrievalResult(
            memories=memories, scores=scores, query_time_ms=1.0, total_matches=2
        )
    ...
    def test_result_validation_negative_values(self, caplog):
        ...
        result = RetrievalResult(
            memories=[], scores=[], query_time_ms=-5.0, total_matches=-10
        )
...
class TestMemoryStats:
    ...
    def test_stats_update(self):
        ...
        stats1 = MemoryStats(
            total_memories=10, total_queries=5, avg_retrieval_time_ms=10.0
        )
        stats1.by_type[MemoryType.SEMANTIC] = 5

        stats2 = MemoryStats(
            total_memories=5, total_queries=3, avg_retrieval_time_ms=20.0
        )
    ...
    def test_stats_reset(self):
        ...
        stats = MemoryStats(total_memories=100, total_queries=50)
...
class TestBaseMemorySystem:
    ...
    def test_shutdown(self, mock_memory_system, caplog):
        ...
        assert (
            "Shutting down memory system" in caplog.text
            or "Memory system shutdown complete" in caplog.text
            or mock_memory_system.is_shutdown() is True
        )
...
class TestPerformance:
    ...
    def test_memory_creation_performance(self):
        ...
        def create_memory():
            return Memory(
                id=f"perf_{time.time()}", type=MemoryType.SEMANTIC, content="test"
            )
