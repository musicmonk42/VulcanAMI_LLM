...
def test_preference_learner_predict_preferred_option_greedy(learner):
    learner.preferences["color:red"] = Preference(feature="color", preferred_value="red", alpha=10, beta=1)
    options = [{"color": "red"}, {"color": "blue"}]
    context = {"env": "test"}
...
def test_preference_learner_update_from_feedback(learner):
    ...
    assert bandit[learner._option_to_id("A")].failures > 0  # Penalty for wrong prediction
...
def test_preference_learner_get_preference_for_feature(learner):
    learner.add_signal(PreferenceSignal(PreferenceSignalType.EXPLICIT_CHOICE, {"color": "red"}))
...
def test_preference_learner_private_methods(learner):
    ...
    context = {}
    selected_id = learner._thompson_select_by_id(option_by_id, option_scores_by_id, context)
    ...
    dist = learner._build_preference_distribution([PreferenceSignal(PreferenceSignalType.EXPLICIT_CHOICE, "A")])
...
def test_preference_learner_non_matching_context(learner):
    learner.preferences["color:red"] = Preference("color", "red", context_conditions={"env": "prod"})
    options = [{"color": "red"}, {"color": "blue"}]
...
def test_preference_learner_drift_identify_changed(learner):
    learner.add_signal(PreferenceSignal(PreferenceSignalType.EXPLICIT_CHOICE, {"feature1": "value1"}))
    old_dist = learner._build_preference_distribution(learner.signals[:1])
    learner.add_signal(PreferenceSignal(PreferenceSignalType.EXPLICIT_CHOICE, {"feature2": "value2"}))
...
def test_preference_learner_transparency_integration(learner):
    with patch.object(learner.transparency_interface, 'record_preference_update') as mock_update:
        learner.add_signal(PreferenceSignal(PreferenceSignalType.EXPLICIT_CHOICE, "A"))
        mock_update.assert_called()

def test_preference_learner_validation_tracker_integration(learner):
    with patch.object(learner.validation_tracker, 'record_preference_signal') as mock_record:
        learner.add_signal(PreferenceSignal(PreferenceSignalType.EXPLICIT_CHOICE, "A"))
