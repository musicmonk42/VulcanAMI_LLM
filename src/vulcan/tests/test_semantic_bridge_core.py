...
class MockSemanticBridge:
    ...
    def transfer_concept(
        ...
        if hasattr(concept, "success_rate"):
            transferred.success_rate = (
                concept.success_rate * self.TRANSFER_SUCCESS_RATE_DECAY
            )
...
class TestSemanticBridgeOperations:
    ...
    def test_transfer_concept(self):
        ...
        concept = Concept(
            pattern_signature="malware_detection", confidence=0.85, success_rate=0.9
        )
        ...
        assert (
            "biosecurity" in transferred.domains
        ), "Target domain should be in domains"
        assert (
            transferred.pattern_signature == concept.pattern_signature
        ), "Pattern should be preserved"
        assert (
            transferred.confidence <= concept.confidence
        ), "Confidence may decrease after transfer"

        # Test that same domain returns None
        no_transfer = bridge.transfer_concept(
            concept, "cyber_security", "cyber_security"
        )
