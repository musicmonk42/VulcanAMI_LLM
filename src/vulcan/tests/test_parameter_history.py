...
class TestParameterHistoryManager:
    ...
    def config(self):
        """Create test config"""
        return LearningConfig(max_checkpoints=5, checkpoint_frequency=10)
    ...
    def test_end_trajectory(self, manager):
        ...
        for i in range(5):
            manager.record_step(
                np.random.randn(10), f"action_{i}", 0.1 * i, 1.0 - 0.1 * i
            )
    ...
    def test_cleanup_old_trajectories(self, manager):
        ...
        manager.current_trajectory.start_time = time.time() - (
            35 * 24 * 3600
        )  # 35 days old
        manager.end_trajectory(save=True)
        ...
        def save_checkpoints():
            for i in range(5):
                manager.save_checkpoint(
                    model, {"thread": threading.current_thread().name}
                )
                time.sleep(0.01)
    ...
    @patch(
        "vulcan.learning.parameter_history.ParameterHistoryManager.validate_checkpoint"
    )
    def test_checkpoint_integrity(self, mock_validate, manager, model):
        ...
        checkpoint["model_state_dict"]["fc1.weight"] = checkpoint["model_state_dict"][
            "fc1.weight"
        ].clone()
