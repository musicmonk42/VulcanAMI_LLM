...
def mock_correlation_tracker_safety_validator():
    ...
    mock_validator_instance.clamp_to_safe_region.side_effect = (
        lambda state_vec, *args, **kwargs: state_vec
    )
...
def cleanup_session_resources():
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            warnings.filterwarnings(
                "ignore",
                message=".*torch.distributed.ReduceOp.*",
                category=FutureWarning,
            )
            ...
            for obj in gc.get_objects():
                if (
                    ...
                    and obj.__class__.__name__ == "EnhancedResourceMonitor"
                ):
                    try:
