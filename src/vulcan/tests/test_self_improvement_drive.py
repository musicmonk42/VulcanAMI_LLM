...
def config_path(temp_dir):
    ...
    config = {
        ...
        "global_settings": {
            "conflict_resolution": {
                "simultaneous_triggers": {"jitter_milliseconds": 100}
            }
        },
    }
...
def drive(config_path, state_path):
    ...
    return SelfImprovementDrive(config_path=config_path, state_path=state_path)
...
class TestCSIU:
    ...
    def test_csiu_regularize_plan(self, drive):
        ...
        plan = {"objective_weights": {"accuracy": 0.8, "efficiency": 0.7}}
...
class TestThreadSafety:
    ...
    def test_concurrent_step_calls(self, drive):
        ...
        def call_step():
            try:
                result = drive.step(
                    {"is_startup": True, "other_drives_total_priority": 999.0}
                )
        ...
        minimal_config = {
            "drives": {
                "self_improvement": {
                    "enabled": True,
                    "objectives": [
                        {"type": "test", "weight": 1.0, "auto_apply": False}
                    ],
...
class TestIntegration:
    ...
    def test_full_failure_cycle(self, drive):
        ...
        drive.record_outcome(
            obj_type, success=False, details={"error": "validation_failed"}
        )
            ...
            if prev_telemetry and cur_telemetry:
                ...
                if U_now > U_prev:  # Only update on gains
                    ...
                    feature_deltas = {
                        ...
                        - prev_telemetry.get("A", 0.85),
                        "dH": cur_telemetry.get("H", 0.06)
                        - prev_telemetry.get("H", 0.06),
