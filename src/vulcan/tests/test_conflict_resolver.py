...
class MockEvidenceWeightedResolver:
    ...
    def add_evidence(self, concept_id: str, evidence: Evidence):
        with self._lock:
            ...
            if len(self.evidence_store[concept_id]) >= self.max_evidence_per_concept:
                self.evidence_store[concept_id] = self.evidence_store[concept_id][
                    -self.max_evidence_per_concept + 1 :
                ]
    ...
    def resolve_conflict(self, conflict) -> Dict[str, Any]:
        ...
        existing_weight = (
            self.calculate_evidence_weight(existing_concept) if existing_concept else 0
        )
    ...
    def merge_concepts(
        self, concept_a: MockConcept, concept_b: MockConcept
    ) -> MockConcept:
        merged = MockConcept(
    ...
    def create_concept_variant(
        self, base_concept: MockConcept, new_pattern: Dict
    ) -> MockConcept:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        success_rate = self.successful_resolutions / max(self.total_resolutions, 1)
...
class TestEvidenceManagement:
    ...
    def test_evidence_weight_with_domain(self, resolver):
        ...
        weight_engineering = evidence.get_weight(
            domain="engineering", domain_weights=resolver.domain_evidence_weights
        )
    ...
    def test_evidence_store_size_limit(self, fresh_resolver):
        ...
        assert (
            len(fresh_resolver.evidence_store) <= fresh_resolver.max_evidence_concepts
        )
    ...
    def test_evidence_per_concept_limit(self, fresh_resolver):
        ...
        assert (
            len(fresh_resolver.evidence_store["concept_per_limit"])
            <= fresh_resolver.max_evidence_per_concept
        )
...
class TestConflictResolution:
    ...
    def test_resolve_conflict_basic(self, resolver):
        new_concept = MockConcept(
            concept_id="new_basic", pattern_signature="pattern_new", confidence=0.8
        )
    ...
    def test_resolve_conflict_with_dict(self, resolver):
        new_concept = MockConcept(
            concept_id="new_dict", pattern_signature="pattern_new"
        )

        existing_concept = MockConcept(
            concept_id="existing_dict", pattern_signature="pattern_existing"
        )
    ...
    def test_resolution_tracking(self, fresh_resolver):
        new_concept = MockConcept(
            concept_id="new_track", pattern_signature="pattern_new"
        )

        existing_concept = MockConcept(
            concept_id="existing_track", pattern_signature="pattern_existing"
        )
...
class TestConceptMerging:
    ...
    def test_merge_combines_features(self, resolver):
        concept_a = MockConcept(concept_id="feat_a", pattern_signature="pattern_a")
        concept_a.features = {
            "numeric": 10.0,
            "set_data": {1, 2, 3},
            "list_data": [1, 2],
        ...
        concept_b = MockConcept(concept_id="feat_b", pattern_signature="pattern_b")
        concept_b.features = {
            "numeric": 20.0,
            "set_data": {3, 4, 5},
            "list_data": [3, 4],
...
class TestConceptVariants:
    ...
    def test_create_variant_basic(self, resolver):
        base_concept = MockConcept(
            concept_id="base_var", pattern_signature="base_pattern", confidence=0.8
        )
    ...
    def test_variant_has_lower_confidence(self, resolver):
        base_concept = MockConcept(
            concept_id="base_conf", pattern_signature="base_pattern", confidence=0.9
        )
    ...
    def test_variant_resets_usage(self, resolver):
        base_concept = MockConcept(
            concept_id="base_usage", pattern_signature="base_pattern", usage_count=100
        )
    ...
    def test_variant_relationship_tracking(self, fresh_resolver):
        base_concept = MockConcept(
            concept_id="base_rel", pattern_signature="base_pattern"
        )
        ...
        if base_concept.concept_id in fresh_resolver.concept_relationships:
            relationships = fresh_resolver.concept_relationships[
                base_concept.concept_id
            ]
...
class TestSemanticSimilarity:
    ...
    def test_semantic_similarity_identical(self, resolver):
        concept = MockConcept(concept_id="sim_identical", pattern_signature="pattern")
    ...
    def test_semantic_similarity_different(self, resolver):
        concept_a = MockConcept(concept_id="sim_diff_a", pattern_signature="pattern_a")
        concept_a.features = {"a": 1, "b": 2}

        concept_b = MockConcept(concept_id="sim_diff_b", pattern_signature="pattern_b")
    ...
    def test_semantic_similarity_partial_overlap(self, resolver):
        concept_a = MockConcept(
            concept_id="sim_partial_a", pattern_signature="pattern_a"
        )
        concept_a.features = {"a": 10, "b": 20, "c": 30}

        concept_b = MockConcept(
            concept_id="sim_partial_b", pattern_signature="pattern_b"
        )
...
class TestConceptComparison:
    ...
    def test_compare_concepts_basic(self, resolver):
        concept_a = MockConcept(
            concept_id="cmp_a", pattern_signature="pattern_a", success_rate=0.9
        )
        concept_a.features = {"a": 1, "b": 2}
        concept_a.domains = {"optimization"}

        concept_b = MockConcept(
            concept_id="cmp_b", pattern_signature="pattern_b", success_rate=0.7
        )
    ...
    def test_compare_concepts_performance_diff(self, resolver):
        concept_a = MockConcept(
            concept_id="perf_a", pattern_signature="pattern_a", success_rate=0.95
        )

        concept_b = MockConcept(
            concept_id="perf_b", pattern_signature="pattern_b", success_rate=0.70
        )
...
class TestStatistics:
    ...
    def test_get_statistics(self, fresh_resolver):
        ...
        for i in range(2):
            new_concept = MockConcept(
                concept_id=f"new_stats_{i}", pattern_signature=f"pattern_new_{i}"
            )
...
class TestEdgeCases:
    ...
    def test_merge_with_missing_attributes(self, resolver):
        concept_a = MockConcept(concept_id="edge_a", pattern_signature="pattern_a")

        concept_b = MockConcept(concept_id="edge_b", pattern_signature="pattern_b")
