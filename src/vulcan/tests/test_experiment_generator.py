...
class TestExperimentTracker:
    ...
    def test_track_active(self, experiment_tracker, sample_experiment):
        ...
        assert experiment_tracker.active_experiments[sample_experiment.experiment_id]['status'] == 'running'
    ...
    def test_complete_experiment(self, experiment_tracker, sample_experiment):
        ...
        assert sample_experiment.experiment_id not in experiment_tracker.active_experiments
...
class TestExperimentGenerator:
    ...
    def test_generate_for_decomposition_gap(self, experiment_generator, mock_gap):
        ...
        assert all(exp.experiment_type == ExperimentType.DECOMPOSITION for exp in experiments)
    ...
    def test_generate_for_transfer_gap(self, experiment_generator, transfer_gap):
        ...
        assert len(experiments) > 0
        assert all(exp.experiment_type == ExperimentType.TRANSFER for exp in experiments)
    ...
    def test_track_experiment(self, experiment_generator, sample_experiment):
        ...
        assert sample_experiment.experiment_id in experiment_generator.tracker.active_experiments
    
    def test_complete_experiment(self, experiment_generator, sample_experiment):
        """Test completing experiment"""
        experiment_generator.track_experiment(sample_experiment)
        experiment_generator.complete_experiment(sample_experiment.experiment_id, {'success': True})
        
        assert sample_experiment.experiment_id not in experiment_generator.tracker.active_experiments
    ...
    def test_thread_safety(self, experiment_generator):
        ...
        threads = [threading.Thread(target=generate_experiments, args=(i,)) for i in range(3)]
...
class TestFailureAnalyzer:
    ...
    def test_generate_adjustments_too_simple(self, failure_analyzer):
        ...
        adjustments = failure_analyzer.generate_adjustments(
...
class TestParameterAdjuster:
    ...
    def test_adjust_for_insufficient_data(self, parameter_adjuster):
        ...
        adjusted = parameter_adjuster.adjust_for_failure(params, FailureType.INSUFFICIENT_DATA)
    ...
    def test_adjust_for_unstable_output(self, parameter_adjuster):
        ...
        adjusted = parameter_adjuster.adjust_for_failure(params, FailureType.UNSTABLE_OUTPUT)
...
class TestIterativeExperimentDesigner:
    ...
    def test_generate_iterative_experiments(self, iterative_designer, mock_gap):
        ...
        experiments = iterative_designer.generate_iterative_experiments(mock_gap, max_iterations=3)
        
        assert len(experiments) == 3
        assert all(exp.experiment_type == ExperimentType.ITERATIVE for exp in experiments)
    ...
    def test_pivot_experiment_strategy(self, iterative_designer, sample_experiment):
        ...
        analysis = FailureAnalysis(
            type=FailureType.WRONG_APPROACH,
            recovery_possible=True
        )
        
        pivoted = iterative_designer.pivot_experiment_strategy(sample_experiment, analysis)
    ...
    def test_parse_adjustment_set(self, iterative_designer):
        ...
        key, value, operation = iterative_designer._parse_adjustment('strategy:hierarchical')
    ...
    def test_parse_adjustment_boolean(self, iterative_designer):
        ...
        key, value, operation = iterative_designer._parse_adjustment('early_stopping:True')
