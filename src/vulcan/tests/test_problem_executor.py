...
def basic_executor(mock_safety_validator):
    ...
    with patch('vulcan.problem_decomposer.problem_executor.SAFETY_VALIDATOR_AVAILABLE', True):
        with patch('vulcan.problem_decomposer.problem_executor.EnhancedSafetyValidator', return_value=mock_safety_validator):
...
def executor_no_safety():
    ...
    with patch('vulcan.problem_decomposer.problem_executor.SAFETY_VALIDATOR_AVAILABLE', False):
        executor = ProblemExecutor()
        return executor
...
def simple_problem_graph():
    ...
    return ProblemGraph(
        nodes={
            'node1': {'type': 'operation', 'operation': 'sum', 'parameters': {'input': 'data'}},
            'node2': {'type': 'transform', 'transform': 'scale', 'parameters': {'factor': 2.0}},
            'node3': {'type': 'decision', 'condition': {'variable': 'x', 'operator': '>', 'threshold': 5}}
        },
        edges=[
            ('node1', 'node2', {}),
            ('node2', 'node3', {})
...
def simple_plan():
    ...
    return DecompositionPlan(
        steps=[
            {'type': 'structural_match', 'structure': 'hierarchical', 'nodes': ['node1', 'node2'], 'confidence': 0.8},
            {'type': 'exact_match', 'pattern_id': 'linear', 'nodes': ['node3'], 'confidence': 0.9}
        ],
...
def complex_plan():
    ...
    return DecompositionPlan(
        steps=[
            {'type': 'structural_match', 'structure': 'modular', 'nodes': ['node1'], 'confidence': 0.7},
            {'type': 'semantic_match', 'concept': 'semantic_cluster', 'similarity': 0.75, 'nodes': ['node2'], 'confidence': 0.6},
            {'type': 'synthetic_bridge', 'template': 'parallel', 'confidence': 0.8},
            {'type': 'analogical', 'source_domain': 'sorting', 'target_mapping': {'input': 'data'}, 'confidence': 0.7}
        ],
        strategy=None,
        confidence=0.7
    )
...
def test_executor_initialization_with_safety():
    ...
    with patch('vulcan.problem_decomposer.problem_executor.SAFETY_VALIDATOR_AVAILABLE', True):
        with patch('vulcan.problem_decomposer.problem_executor.EnhancedSafetyValidator') as mock_validator_class:
            mock_validator = Mock()
...
def test_executor_with_custom_config():
    ...
    with patch('vulcan.problem_decomposer.problem_executor.SAFETY_VALIDATOR_AVAILABLE', True):
        with patch('vulcan.problem_decomposer.problem_executor.SafetyConfig') as mock_config_class:
            with patch('vulcan.problem_decomposer.problem_executor.EnhancedSafetyValidator') as mock_validator_class:
...
def test_execute_plan_blocks_unsafe_plan(basic_executor, simple_problem_graph):
    ...
    unsafe_plan = DecompositionPlan(
        steps=[
            {'type': 'shell_command', 'command': 'rm -rf /', 'confidence': 0.9}
        ],
        confidence=0.9
    )
...
def test_execute_plan_updates_statistics(basic_executor, simple_problem_graph, simple_plan):
...
def test_convert_structural_step(basic_executor, simple_problem_graph):
    ...
    principles = basic_executor._convert_steps_to_principles(steps, simple_problem_graph)
...
def test_convert_semantic_step(basic_executor, simple_problem_graph):
    ...
    steps = [
        {'type': 'semantic_match', 'concept': 'clustering', 'similarity': 0.75, 'nodes': ['node1'], 'confidence': 0.7}
    ]
...
def test_convert_all_step_types(basic_executor, simple_problem_graph):
    ...
    steps = [
        {'type': 'structural_match', 'structure': 'hierarchical', 'nodes': ['node1'], 'confidence': 0.8},
        {'type': 'semantic_match', 'concept': 'test', 'similarity': 0.7, 'nodes': ['node2'], 'confidence': 0.7},
        {'type': 'exact_match', 'pattern_id': 'linear', 'nodes': ['node3'], 'confidence': 0.9},
        {'type': 'synthetic_bridge', 'template': 'simple', 'confidence': 0.6},
        {'type': 'analogical', 'source_domain': 'sorting', 'target_mapping': {}, 'confidence': 0.7},
...
def test_determine_strategy_iterative(basic_executor):
    ...
    plan = DecompositionPlan(
        steps=[
            {'type': 'recursive', 'structure': 'recursive'}
        ],
        strategy=None,
        confidence=0.8
    )
...
def test_execute_sequential_failure(basic_executor, simple_problem_graph):
    ...
    principles = [
        Principle(id='p1', core_pattern={}, confidence=0.8, 
                 execution_logic=lambda inputs: {'success': True}),
        Principle(id='p2', core_pattern={}, confidence=0.8, 
                 execution_logic=failing_logic),
        Principle(id='p3', core_pattern={}, confidence=0.8, 
                 execution_logic=lambda inputs: {'success': True})
    ]
...
def test_execute_parallel_all_success(basic_executor, simple_problem_graph):
    ...
    principles = [
        Principle(id=f'p{i}', core_pattern={}, confidence=0.8,
                 execution_logic=lambda inputs: {'success': True, 'result': i})
        for i in range(3)
    ]
...
def test_execute_iterative_converges(basic_executor, simple_problem_graph):
    ...
    principles = [
        Principle(id='iter_p', core_pattern={}, confidence=0.8,
...
def test_solve_hierarchical(basic_executor, hierarchical_problem_graph):
    ...
    result = basic_executor._solve_hierarchical(nodes, inputs, hierarchical_problem_graph)
...
def test_execute_and_validate_success(basic_executor, simple_problem_graph, simple_plan):
...
def test_execute_and_validate_no_validator(basic_executor, simple_problem_graph, simple_plan):
...
def test_full_execution_pipeline(basic_executor, hierarchical_problem_graph):
    ...
    plan = DecompositionPlan(
        steps=[
            {'type': 'structural_match', 'structure': 'hierarchical', 
             'nodes': list(hierarchical_problem_graph.nodes.keys()), 'confidence': 0.8},
            {'type': 'exact_match', 'pattern_id': 'tree', 
             'nodes': ['root'], 'confidence': 0.9}
        ],
        strategy=None,
        confidence=0.85
    )
