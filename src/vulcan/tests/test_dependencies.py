...
class TestValidation(unittest.TestCase):
        ...
        for category, missing_deps in report.items():
            ...
            if category == DependencyCategory.CORE:
                self.assertEqual(len(missing_deps), 0,
                 ...
                 missing_initialized = [d for d in missing_deps if "(import failed)" not in d]
                 self.assertEqual(len(missing_initialized), 0,
    ...
    def test_get_missing_dependencies(self):
        ...
        missing = [dep for cat_deps in status['missing_by_category'].values() for dep in cat_deps]
...
class TestShutdown(unittest.TestCase):
    ...
    def test_shutdown_with_missing_shutdown_method(self):
        ...
        deps = EnhancedCollectiveDeps(
            safety_validator=mock_component
        )
...
class TestIntegration(unittest.TestCase):
    ...
    def test_complex_dependency_setup(self):
        ...
        self.assertTrue(deps.compressed_memory._shutdown_called, "compressed_memory shutdown should be called")
        
        # Learning systems
        self.assertTrue(deps.continual._shutdown_called, "continual shutdown should be called")
        self.assertTrue(deps.world_model._shutdown_called, "world_model shutdown should be called")
        self.assertTrue(deps.meta_cognitive._shutdown_called, "meta_cognitive shutdown should be called")
        ...
        self.assertTrue(deps.safety_validator._shutdown_called, 
                        "safety_validator *is* in the shutdown loop")
        self.assertTrue(deps.governance._shutdown_called,
                        "governance *is* in the shutdown loop")
        self.assertTrue(deps.multimodal._shutdown_called,
                        "multimodal *is* in the shutdown loop")
