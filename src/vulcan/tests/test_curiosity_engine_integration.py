...
class TestExperimentGeneratorToBudget:
    ...
    def test_budget_constrained_experiment_selection(self):
        ...
        if len(experiments) > 0:
            assert len(selected) > 0, f"Should have selected at least one experiment from {len(experiments)} available with budget 200.0"
...
class TestFullCuriosityEnginePipeline:
    ...
    def test_end_to_end_learning_cycle(self):
        ...
        engine.gap_analyzer.record_failure('decomposition', {
            'pattern': 'hierarchical',
            'domain': 'planning',
            'complexity': 0.7,
            'missing_concepts': ['goal_decomposition']
        ...
        for i in range(5):
            engine.gap_analyzer.record_failure('prediction', {
                'cause': 'temperature',
                'effect': 'pressure',
                'magnitude': 0.4,
                'domain': 'physics',
    ...
    def test_strategy_selection_adaptation(self):
        ...
        for context in contexts:
            ...
            assert strategy in ['gap_driven', 'minimal', 'efficient', 'balanced', 'comprehensive']
...
class TestExplorationFrontier:
    ...
    def test_frontier_tracking_with_experiments(self):
        ...
        region1 = frontier.add_explored_region('planning', {'hierarchical', 'goal_oriented'})
    ...
    def test_frontier_update_with_new_knowledge(self):
        ...
        new_knowledge = {
            'planning': {
                'patterns': ['temporal', 'resource_allocation'],
                'value': 0.8
            }
        }
...
class TestIterativeExperimentDesign:
    ...
    def test_experiment_pivoting_on_failure(self):
        ...
        assert pivoted.experiment_type != experiment.experiment_type or \
               pivoted.parameters != experiment.parameters
