...
class TestRegionManager:
    ...
    def test_add_empty_patterns_bug(self, region_manager):
        ...
        assert region_id1 != region_id2, "Empty patterns should not match existing regions"
...
class TestExplorationFrontier:
    ...
    def test_pattern_conversion(self, exploration_frontier):
        ...
        # Test string pattern
        region_id1 = exploration_frontier.add_explored_region(
            "test",
            "single_pattern"
        )
...
class TestSafeExperimentExecutor:
    ...
    def test_execute_with_mock_decomposer(self, mock_experiment):
        ...
        mock_decomposer.test_decomposition = Mock(return_value={
            'success': True,
...
def mock_decomposer():
    """Create mock decomposer"""
    decomposer = Mock()
    decomposer.test_decomposition = Mock(return_value={
        'success': True,
        'components': ['c1', 'c2']
    })
...
def mock_world_model():
    ...
    wm = Mock()
    wm.test_intervention = Mock(return_value={
        'causal_strength': 0.5,
        'p_value': 0.03
    })
