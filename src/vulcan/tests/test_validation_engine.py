...
class TestPrinciple:
    ...
    def test_principle_creation_basic(self):
        ...
        p = Principle(id="test_1", core_pattern=Mock(), confidence=0.8)
        assert p.id == "test_1"
...
"""
    ...
    def test_principle_execute_rule_spec(self):
        ...
        rules = {"output_val": {"type": "direct", "source": "input_val"}}
...
class TestValidationResult:
    ...
    def test_validation_result_creation(self):
        ...
        vr = ValidationResult(
            is_valid=True, confidence=0.9, errors=[], warnings=["Minor issue"]
        )
        assert vr.is_valid
    ...
    def test_validation_result_to_dict(self):
        ...
        vr = ValidationResult(
            is_valid=False, confidence=0.3, errors=["Error 1", "Error 2"]
        )
        d = vr.to_dict()
...
class TestValidationResults:
    ...
    def test_add_failure(self):
        ...
        failure = FailureAnalysis(
            failure_type="test_failure", error_message="Test error"
        )
...
class TestKnowledgeValidator:
    ...
    def test_validator_initialization(self):
        ...
        validator = KnowledgeValidator(min_confidence=0.7, consistency_threshold=0.8)
...
class TestDomainValidator:
    ...
    def test_run_sandboxed_test_success(self, mock_popen):
        ...
        test_case = DomainTestCase(
            domain="general", test_id="test_1", inputs={"x": 1}, timeout=10.0
        )
    ...
    def test_run_sandboxed_test_timeout(self, mock_popen):
        ...
        principle = Principle(
            id="test", core_pattern=Mock(), confidence=0.8, execution_logic=lambda x: x
        )

        test_case = DomainTestCase(
            domain="general", test_id="test_1", inputs={"x": 1}, timeout=1.0
        )
