        ...
        if len(self.observation_buffer) >= 5:
            for var in observation.variables:
                ...
                if len(values) >= 5:
                    ...
                    patterns['trends'][var] = 'increasing' if trend > 0.1 else 'decreasing' if trend < -0.1 else 'stable'
...
class MockInterventionManager:
    ...
    def schedule_intervention(self, variable: str, value: float, 
                               scheduled_time: float = None) -> Intervention:
        with self._lock:
    ...
    def get_statistics(self) -> Dict:
        return {
            ...
            'pending': len([i for i in self.interventions.values() if i.status == 'pending']),
            'executed': len([i for i in self.interventions.values() if i.status == 'executed'])
        }
...
class MockPredictionManager:
    ...
    def predict_correlation(self, target: str, context: ModelContext) -> Prediction:
        confidence = 0.7
        value = np.mean(self.correlation_data.get(target, [0.0])) if self.correlation_data.get(target) else 0.0
    ...
    def predict_causal(self, target: str, intervention: Dict, context: ModelContext) -> Prediction:
        confidence = 0.6
...
class MockWorldModel:
    ...
    def process_observation(self, observation: Observation) -> Dict:
        with self._lock:
            # Validate
            is_valid, error = self.observation_processor.validate_observation(observation)
            if not is_valid:
    ...
    def predict_intervention(self, target: str, intervention: Dict,
                              context: ModelContext = None) -> Prediction:
        context = context or ModelContext(domain="default", targets=[target])
...
class TestObservationProcessor:
    ...
    def test_detect_intervention_from_metadata(self, observation_processor):
        obs = Observation(
            timestamp=time.time(),
            variables={'x': 5.0},
            metadata={'is_intervention': True, 'intervened_var': 'x', 'intervention_val': 5.0}
        )
    ...
    def test_validate_observation_invalid_timestamp(self, observation_processor, invalid_observation):
        is_valid, error = observation_processor.validate_observation(invalid_observation)
        assert is_valid == False
    
    def test_validate_observation_nan(self, observation_processor, observation_with_nan):
