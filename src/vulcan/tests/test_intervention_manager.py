...
class TestInterventionCandidate:
    ...
    def test_candidate_ordering(self, sample_correlation, weak_correlation):
        ...
        cand1 = InterventionCandidate(sample_correlation, priority=2.0, cost=10, info_gain=20)
        cand2 = InterventionCandidate(weak_correlation, priority=1.0, cost=10, info_gain=10)
...
class TestInformationGainEstimator:
    ...
    def test_estimate_strong_vs_weak(self, info_estimator, strong_correlation, weak_correlation):
    ...
    def test_novelty_bonus(self, info_estimator, sample_correlation):
        ...
        # Mark as tested
        info_estimator.mark_as_tested(sample_correlation.var_a, sample_correlation.var_b)
...
class TestInterventionScheduler:
    ...
    def test_schedule_within_budget(self, scheduler, info_estimator, cost_estimator, multiple_correlations):
        ...
        assert len(candidates) > 0, "Scheduler should return at least one candidate with reasonable budget"
        
        # Total cost should not exceed budget
        total_cost = sum(c.cost for c in candidates)
        assert total_cost <= budget
    
    def test_schedule_prioritization(self, scheduler, info_estimator, cost_estimator, multiple_correlations):
    ...
    def test_schedule_zero_budget(self, scheduler, info_estimator, cost_estimator, multiple_correlations):
    ...
    def test_create_batches(self, scheduler, multiple_correlations, info_estimator, cost_estimator):
...
class TestConfounderDetector:
    ...
    def test_identify_confounders_high_variance(self, confounder_detector, sample_correlation):
        ...
        confounders = confounder_detector.identify_confounders(sample_correlation, result)
...
class TestInterventionPrioritizer:
    ...
    def test_prioritize_interventions(self, prioritizer, multiple_correlations):
        ...
        assert len(candidates) > 0, "Prioritizer should return at least one candidate with reasonable budget"
...
class TestInterventionExecutor:
    ...
    def test_create_controlled_intervention(self, executor, sample_correlation):
        ...
        controlled = executor.create_controlled_intervention(
            candidate,
            control_for=['confounder1', 'confounder2']
        )
    ...
    def test_real_intervention_requires_safety(self):
        ...
        with pytest.raises(RuntimeError, match="SAFETY CRITICAL: Real intervention execution requires safety_validator."):
...
class TestSafetyIntegration:
    ...
    def test_safety_validation_in_execution(self, executor_with_safety, sample_correlation):
...
class TestEdgeCases:
    ...
    def test_insufficient_budget(self, prioritizer, multiple_correlations):
        ...
        candidates = prioritizer.prioritize_interventions(multiple_correlations, budget=0.5)
    ...
    def test_single_correlation(self, prioritizer, sample_correlation):
        ...
        candidates = prioritizer.prioritize_interventions([sample_correlation], budget=100.0)
...
class TestThreadSafety:
    ...
    def test_concurrent_scheduling(self, scheduler, info_estimator, cost_estimator, multiple_correlations):
...
class TestIntegration:
    ...
    def test_full_intervention_workflow(self, multiple_correlations):
        ...
        candidates = prioritizer.prioritize_interventions(multiple_correlations, budget=100.0)
        
        # FIXED: Should get candidates with fixed scheduling logic
        assert len(candidates) > 0, "Should get at least one candidate with reasonable budget"
...
class TestPerformance:
    ...
    def test_large_scale_prioritization(self):
        ...
        # FIXED: Should get candidates with fixed scheduling logic
        assert len(candidates) > 0, "Should get at least one candidate from 100 correlations"
