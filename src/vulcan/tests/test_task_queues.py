...
class TestTaskQueue(TaskQueueInterface):
    ...
    def submit_task(
        self, task: dict, priority: int = 0, timeout: float = None, max_retries: int = 3
    ) -> str:
...
class TestTaskMetadata(unittest.TestCase):
    ...
    def test_task_metadata_creation(self):
        ...
        metadata = TaskMetadata(
            task_id="test_123", status=TaskStatus.PENDING, submitted_at=time.time()
        )
    ...
    def test_get_duration_incomplete_task(self):
        ...
        metadata = TaskMetadata(
            task_id="test_123", status=TaskStatus.RUNNING, submitted_at=time.time()
        )
    ...
    def test_get_wait_time_not_started(self):
        ...
        current_time = time.time()
        metadata = TaskMetadata(
            task_id="test_123", status=TaskStatus.PENDING, submitted_at=current_time - 1
        )
    ...
    def test_should_retry_completed_task(self):
        ...
        metadata = TaskMetadata(
            task_id="test_123", status=TaskStatus.COMPLETED, submitted_at=time.time()
        )
...
class TestRayTaskQueue(unittest.TestCase):
    ...
    def test_cancel_task(self, mock_ray):
        ...
        queue.ray_futures[task_id] = mock_future
        metadata = TaskMetadata(
            task_id=task_id, status=TaskStatus.RUNNING, submitted_at=time.time()
        )
        queue.pending_tasks[task_id] = metadata
