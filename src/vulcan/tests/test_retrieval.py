...
def sample_memories():
    ...
    for i in range(10):
        mem = Memory(
            ...
            timestamp=current_time - (i * 1800),  # Spread over 4.5 hours (1800 seconds = 30 minutes each)
...
class TestAttentionMechanism:
    ...
    def test_learned_attention(self, sample_embedding, sample_embeddings):
        ...
        keys = torch.tensor(np.array(sample_embeddings[:5]), dtype=torch.float32).unsqueeze(0)
...
class TestMemorySearch:
    ...
    def test_metadata_search(self, temp_dir, sample_memories, memory_dict):
        ...
        # Search by metadata
        results = search.metadata_search(
            memory_dict,
            {'category': 'test'}
        )
    ...
    def test_causal_search(self, temp_dir, memory_dict):
        ...
        effect = Memory(
            ...
            embedding=np.random.randn(512).astype(np.float32) * 0.9 + cause.embedding * 0.1
        )
    ...
    def test_save_and_load_indices(self, temp_dir, sample_memories):
        ...
        assert "test_index" in new_search.indices or len(list(Path(temp_dir).glob("*.map"))) > 0
...
class TestIntegration:
    ...
    def test_full_search_workflow(self, temp_dir, sample_memories):
        ...
        # 3. Temporal search
        temporal_results = search.temporal_search(
            memory_dict,
            hours_back=12
        )
    ...
    def test_search_with_updates(self, temp_dir, sample_memories):
        ...
        results1 = search.semantic_search(
            sample_memories[0].embedding,
            {m.id: m for m in sample_memories[:5]},
            k=5
        )
        ...
        results2 = search.semantic_search(
            sample_memories[0].embedding,
            {m.id: m for m in sample_memories[:8]},
            k=5
        )
