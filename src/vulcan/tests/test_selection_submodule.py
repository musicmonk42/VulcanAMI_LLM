...
try:
    from vulcan.reasoning.selection.admission_control import AdmissionControlIntegration, Request, RequestPriority, AdmissionDecision
    from vulcan.reasoning.selection.portfolio_executor import PortfolioExecutor, ExecutionStrategy, PortfolioResult
    from vulcan.reasoning.selection.warm_pool import WarmStartPool
    from vulcan.reasoning.selection.utility_model import UtilityModel, UtilityContext, ContextMode
except ImportError as e:
    print(f"Failed to import selection modules: {e}")
    print("Please ensure this test script is placed correctly and all dependencies are available.")
    sys.exit(1)
    ...
    def test_reject_overloaded_system(self):
        ...
        with self.admission_control.controller.resource_monitor._history_lock:
            ...
            for _ in range(60):  # Fill the entire deque
                self.admission_control.controller.resource_monitor.cpu_history.append(96.0)
                self.admission_control.controller.resource_monitor.memory_history.append(50.0)
...
class TestPortfolioExecutor(unittest.TestCase):
    ...
    def setUp(self):
        self.mock_tools = {
            ...
            'slow_tool': MockTool(name='slow_tool', execution_time=0.5, confidence=0.95),
            'cheap_tool': MockTool(name='cheap_tool', execution_time=0.05, confidence=0.6)
        }
    ...
    def test_speculative_parallel_fastest_wins(self):
        ...
        self.assertIsInstance(result, PortfolioResult)
        self.assertLess(result.execution_time, 0.4) # Should be much faster than the slow tool (0.5s)
...
class TestUtilityModel(unittest.TestCase):
    ...
    def test_rush_mode_prioritizes_time(self):
        ...
        self.assertGreater(utility_fast, utility_slow, "In RUSH mode, the faster option should have higher utility.")
    ...
    def test_accurate_mode_prioritizes_quality(self):
        ...
        self.assertGreater(utility_slow, utility_fast, "In ACCURATE mode, the higher quality option should have higher utility.")
