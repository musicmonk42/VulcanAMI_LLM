...
def complex_problem():
    ...
    problem = ProblemGraph(
        ...
        edges=edges,
        root="node_0",
        metadata={"domain": "complex_planning", "constraints": ["c1", "c2", "c3"]},
    )
...
def sample_plan():
    ...
    plan = DecompositionPlan(
        steps=[step1, step2], estimated_complexity=2.5, confidence=0.8
    )
...
def successful_outcome():
    ...
    return ExecutionOutcome(
        ...
        metrics={"actual_complexity": 2.0, "solution_quality": 0.9},
        errors=[],
    )
...
def mock_decomposer():
    ...
    decomposer = Mock()
    decomposer.get_statistics = Mock(
        return_value={
            "decomposition_stats": {"success_rate": 0.75, "total_decompositions": 100}
        }
    )
...
def mock_curriculum_learner():
    ...
    learner.get_curriculum_stats = Mock(
        return_value={"difficulty_adjustments": [0.5, 0.6, 0.7]}
    )
...
def mock_meta_learner():
    ...
    learner.get_statistics = Mock(return_value={"meta_updates": 10})
    return learner
...
def mock_metacognition():
    ...
    monitor.analyze_learning_efficiency = Mock(
        return_value={"recommendations": [{"suggestion": "Test recommendation"}]}
    )
    return monitor
...
def mock_rlhf_manager():
    ...
    manager.get_statistics = Mock(return_value={"total_feedback": 50})
    return manager
...
def mock_principle_learner():
    ...
    learner = Mock()
    learner.extract_and_promote = Mock(
        return_value={
    ...
    learner.get_learning_statistics = Mock(
        return_value={
            "promotion": {"promoter_stats": {"promotion_rate": 0.5}},
...
class TestProblemToExperienceConverter:
    ...
    def test_cache_size_limit(self):
        ...
        for i in range(15):
            problem = ProblemGraph(
                nodes={f"node_{i}": {}}, edges=[], metadata={"id": i}
            )
    ...
    def test_convert_to_experience_success(
        ...
        assert (
            experience["reward"] > 0
        )  # Successful outcome should have positive reward

        logger.info("✓ Convert to experience (success) test passed")

    def test_convert_to_experience_failure(
        ...
        experience = converter.convert_to_experience(
            simple_problem, sample_plan, failed_outcome
        )
    ...
    def test_reward_calculation(self, simple_problem, sample_plan):
        ...
        outcome1 = ExecutionOutcome(
            success=True, execution_time=30.0, metrics={"solution_quality": 0.9}
        )

        outcome2 = ExecutionOutcome(
            success=True, execution_time=30.0, metrics={"solution_quality": 0.5}
        )
    ...
    def test_get_statistics(self, simple_problem, sample_plan, successful_outcome):
        ...
        for _ in range(3):
            converter.convert_to_experience(
                simple_problem, sample_plan, successful_outcome
            )
...
class TestRLHFFeedbackRouter:
    ...
    def test_route_outcome_to_feedback(
        self, simple_problem, sample_plan, successful_outcome, mock_rlhf_manager
    ):
        """Test routing outcome to RLHF feedback"""
        router = RLHFFeedbackRouter(mock_rlhf_manager)

        feedback = router.route_outcome_to_feedback(
            simple_problem, sample_plan, successful_outcome
        )
...
class TestIntegratedLearningCoordinator:
    ...
    def test_coordinator_initialization(self, mock_decomposer):
        ...
        coordinator = IntegratedLearningCoordinator(decomposer=mock_decomposer)
    ...
    def test_coordinator_with_all_systems(
        self,
        mock_decomposer,
        mock_continual_learner,
        mock_curriculum_learner,
        mock_meta_learner,
        mock_metacognition,
        mock_rlhf_manager,
    ...
    def test_get_next_problems_curriculum(
        self, mock_decomposer, mock_curriculum_learner, simple_problem
    ):
        """Test getting next problems from curriculum"""
        coordinator = IntegratedLearningCoordinator(
            decomposer=mock_decomposer, curriculum_learner=mock_curriculum_learner
        )

        # Add problem to queue
        coordinator.problem_queue.append(
            {"problem": simple_problem, "difficulty": 0.5, "timestamp": time.time()}
        )
    ...
    def test_analyze_learning_effectiveness(
        self, mock_decomposer, mock_continual_learner
    ):
        """Test analyzing learning effectiveness"""
        coordinator = IntegratedLearningCoordinator(
            decomposer=mock_decomposer, continual_learner=mock_continual_learner
        )
    ...
    def test_get_recommendations(
        self, mock_decomposer, mock_continual_learner, mock_metacognition
    ):
...
class TestUnifiedDecomposerLearner:
    ...
    def test_create_unified_decomposer(self):
        ...
        learner = create_unified_decomposer(
            enable_all=False, enable_principle_learning=False
        )
...
class TestLearningIntegration:
    ...
    def test_error_handling(
        self, simple_problem, sample_plan, successful_outcome, mock_decomposer
    ):
        ...
        coordinator = IntegratedLearningCoordinator(
            decomposer=mock_decomposer, continual_learner=faulty_learner
        )
...
class TestPerformance:
    ...
    def test_difficulty_estimation_performance(self, complex_problem):
        ...
        logger.info(
            "✓ Difficulty estimation performance test passed (%.3f seconds)", elapsed
        )
