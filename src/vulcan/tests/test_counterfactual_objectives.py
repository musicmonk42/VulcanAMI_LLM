...
def reasoner(mock_world_model):
    ...
    with patch('vulcan.world_model.meta_reasoning.counterfactual_objectives.np.random.normal', return_value=0.0):
...
class TestPredictUnderObjective:
    ...
    def test_different_contexts_different_cache(self, reasoner):
        ...
        assert reasoner._get_cache_key('prediction_accuracy', context1) != \
...
class TestAlternativeProposals:
    ...
    def test_alternatives_differ_from_original(self, reasoner):
        ...
        assert all(alt.get('objective') != current_proposal.get('objective') 
...
class TestEdgeCases:
    ...
    def test_conflicting_objectives_comparison(self, reasoner):
        ...
        scenario = {'current_state': {}}
        with patch('vulcan.world_model.meta_reasoning.counterfactual_objectives.np.random.normal', return_value=0.0):
            comparison = reasoner.compare_objectives('efficiency', 'prediction_accuracy', scenario)
