...
def mock_principle():
    ...
    pattern.to_dict = Mock(return_value={
...
class TestDecompositionToTraceConverter:
    ...
    def test_convert_successful_execution(self, simple_problem, sample_plan, successful_outcome):
    ...
    def test_convert_failed_execution(self, simple_problem, sample_plan, failed_outcome):
...
class TestPrinciplePromoter:
    ...
    def test_evaluate_for_promotion(self, decomposition_library, mock_principle, 
                                   mock_validation_results):
        """Test evaluating principle for promotion"""
        promoter = PrinciplePromoter(decomposition_library)
        
        candidate = promoter.evaluate_for_promotion(mock_principle, mock_validation_results)
    ...
    def test_promote_high_score(self, decomposition_library, mock_principle, 
                               mock_validation_results):
    ...
    def test_reject_low_score(self, decomposition_library, mock_principle, 
                             mock_validation_results):
...
class TestPrincipleLearner:
    ...
    def test_extract_and_promote_no_components(self, decomposition_library,
                                               simple_problem, sample_plan, 
                                               successful_outcome):
        ...
        if not learner.components_available:
            results = learner.extract_and_promote(simple_problem, sample_plan, successful_outcome)
    ...
    def test_extract_and_promote_with_mocks(self, mock_validator_class, 
                                           mock_crystallizer_class,
                                           decomposition_library, simple_problem, 
...
class TestPrincipleLearningIntegration:
    ...
    def test_full_learning_cycle_mock(self, decomposition_library, simple_problem,
                                     sample_plan, successful_outcome):
...
class TestPerformance:
    ...
    def test_promotion_evaluation_performance(self, decomposition_library):
        ...
        logger.info("✓ Promotion evaluation performance test passed (%.3f seconds)", elapsed)
...
class TestErrorHandling:
    ...
    def test_promotion_with_missing_attributes(self, decomposition_library):
        ...
        # Should handle gracefully
        try:
            candidate = promoter.evaluate_for_promotion(minimal_principle, minimal_validation)
