...
def mock_objective_hierarchy():
    ...
    def mock_get_priority_order():
        return ['prediction_accuracy', 'safety', 'efficiency', 'exploration', 'exploitation']
...
class TestConflictDetection:
    ...
    def test_detect_tradeoff_conflict(self, detector):
        ...
        tradeoff_conflicts = [c for c in conflicts if c.conflict_type == ConflictType.TRADEOFF]
    ...
    def test_detect_direct_conflict(self, detector):
        ...
        direct_conflicts = [c for c in conflicts if c.conflict_type == ConflictType.DIRECT]
    ...
    def test_detect_constraint_violation(self, detector):
        ...
        constraint_conflicts = [c for c in conflicts if c.conflict_type == ConflictType.CONSTRAINT]
    ...
    def test_detect_resource_conflict(self, detector):
        ...
        resource_conflicts = [c for c in conflicts if c.conflict_type == ConflictType.INDIRECT]
...
class TestMultiObjectiveTension:
    ...
    def test_two_objective_tension(self, detector):
        ...
        analysis = detector.analyze_multi_objective_tension(['efficiency', 'prediction_accuracy'])
...
class TestTradeoffValidation:
    ...
    def test_critical_objective_sacrifice(self, detector):
        ...
        critical_issues = [i for i in result['issues'] if i['type'] == 'critical_sacrifice']
    ...
    def test_unnecessary_tradeoff(self, detector):
        ...
        unnecessary_issues = [i for i in result['issues'] if i['type'] == 'unnecessary_tradeoff']
...
class TestResolutionSuggestions:
    ...
    def test_suggest_resolution_for_tradeoff(self, detector):
        ...
        strategy_names = [r['strategy'] for r in resolutions]
        assert 'pareto_optimization' in strategy_names or 'bounded_optimization' in strategy_names
    ...
    def test_suggest_resolution_for_constraint(self, detector):
        ...
        strategy_names = [r['strategy'] for r in resolutions]
        assert 'constraint_relaxation' in strategy_names or 'constraint_reformulation' in strategy_names
...
class TestEdgeCases:
    ...
    def test_unknown_objective(self, detector):
        ...
        proposal = {
            'objectives': ['unknown_objective', 'another_unknown']
        }
