...
class MockProductionDeployment:
    def __init__(self, config=None):
        ...
        self.collective.deps.am.get_memory_summary = Mock(return_value={
            'total_episodes': 10,
            'short_term_size': 5,
            'long_term_size': 100
        })
...
class _FakeHttpClient:
    ...
    def _handle_request(self, method, path, json=None, headers=None):
        ...
        # Metrics endpoint
        if path == "/metrics":
            return MockResponse(200, content=b"# HELP vulcan_requests_total\n", 
...
def mock_deployment(test_config):
    ...
    deployment.step_with_monitoring = Mock(return_value={
        'action': {'type': 'explore'},
        'success': True,
        'uncertainty': 0.3,
        'status': 'completed'
    })
    deployment.get_status = Mock(return_value={
        'step': 10,
        'health': {
            'error_rate': 0.01,
            'energy_budget_left_nJ': 1000000,
...
class TestAPIEndpoints:
    ...
    def test_search_memory(self, client, mock_deployment):
        mock_deployment.collective.deps.ltm.search = Mock(return_value=[
...
class TestMiddleware:
    ...
    def test_cors_enabled(self, client):
        response = client.options("/v1/status", headers={"Origin": "http://localhost:3000"})
...
class TestBenchmark:
    ...
    def test_benchmark_report_generation(self, test_config):
        benchmark = PerformanceBenchmark(test_config)
        benchmark.results['latency'] = {'mean': 50.0, 'std': 10.0, 'p95': 70.0, 'p99': 80.0}
