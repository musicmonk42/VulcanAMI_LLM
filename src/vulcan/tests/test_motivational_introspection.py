...
class TestInitialization:
    ...
    def test_init_without_design_spec(self, mock_world_model):
        ...
        assert 'decision_quality' in introspector.active_objectives # CHANGED from 'safety'
...
class TestDetectObjectivePathology:
    ...
    def test_constraint_violation_detected(self, introspector):
        ...
        violation = next((p for p in result['pathologies'] 
                         if p['type'] == 'constraint_violation'), None)
    ...
    def test_goal_drift_detected(self, introspector):
        ...
        drift = next((p for p in result['pathologies'] 
                     if p.get('type') == 'goal_drift'), None)
    ...
    def test_critical_objective_sacrifice_detected(self, introspector):
        ...
        bad_tradeoff = next((p for p in result['pathologies'] 
                            if p.get('type') == 'unacceptable_tradeoff'), None)
...
class TestValidateProposalAlignment:
    ...
    def test_validation_includes_objective_analyses(self, introspector):
        ...
        assert all(isinstance(a, ObjectiveAnalysis) for a in validation.objective_analyses)
