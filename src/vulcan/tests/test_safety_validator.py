...
def basic_action():
    ...
    return {
        ...
        "confidence": 0.8,
        "uncertainty": 0.2,
        "resource_usage": {"cpu": 30, "memory": 50, "energy_nJ": 1000},
    }
...
def basic_context():
    ...
    return {
        "system_load": 0.5,
        "energy_budget": 10000,
        "resource_limits": {"cpu": 80, "memory": 90, "energy_nJ": 5000},
...
class TestConstraintManager:
    ...
    def test_add_constraint(self, constraint_manager):
        ...
        constraint = SafetyConstraint(
            name="test_constraint", type="hard", check_function=check_func, priority=5
        )
    ...
    def test_remove_constraint(self, constraint_manager):
        ...
        constraint = SafetyConstraint(
            name="removable", type="soft", check_function=lambda a, c: True
        )
    ...
    def test_activate_deactivate_constraint(self, constraint_manager):
        ...
        constraint = SafetyConstraint(
            name="toggle", type="soft", check_function=lambda a, c: True, active=False
        )
    ...
    def test_check_constraints_all_pass(
        self, constraint_manager, basic_action, basic_context
    ):
    ...
    def test_check_constraints_soft_failure(
        self, constraint_manager, basic_action, basic_context
    ):
    ...
    def test_check_constraints_hard_failure(
        self, constraint_manager, basic_action, basic_context
    ):
...
class TestEnhancedSafetyValidator:
    ...
    def test_validate_action_comprehensive(
        self, shared_safety_validator, basic_action, basic_context
    ):
        """Test comprehensive action validation."""
        report = shared_safety_validator.validate_action_comprehensive(
            basic_action, basic_context, create_snapshot=False
        )
...
class TestIntegration:
    ...
    def test_end_to_end_validation(self):
        ...
        context = {"energy_budget": 5000, "state": {}, "action_log": []}

        report = validator.validate_action_comprehensive(
            action, context, create_snapshot=False
        )
    ...
    def test_constraint_and_explanation_integration(self):
        ...
        explanation = explainer.execute(
            {"action": {"test": "action"}, "safety_report": report.to_audit_log()}, {}
        )
    ...
    async def test_async_validation_pipeline(self):
        ...
        config = SafetyConfig(
            enable_adversarial_testing=False, enable_compliance_checking=False
        )
