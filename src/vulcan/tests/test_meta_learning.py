...
class TestMetaLearner:
    ...
    def config(self):
        ...
        return LearningConfig(
            meta_lr=0.01, inner_lr=0.1, adaptation_steps=3, meta_batch_size=2
        )
    ...
    def test_adapt_maml(self, base_model, config):
        ...
        support_set = {"x": torch.randn(5, EMBEDDING_DIM), "y": torch.randn(5, 10)}
    ...
    def test_meta_update_maml(self, meta_learner):
        ...
        for i in range(2):
            task = {
                ...
                "query": {"x": torch.randn(3, EMBEDDING_DIM), "y": torch.randn(3, 10)},
                "task_id": f"task_{i}",
            }
            tasks.append(task)
        ...
        initial_params = [
            p.clone().detach() for p in meta_learner.base_model.parameters()
        ]
        ...
        for initial, current in zip(
            initial_params, meta_learner.base_model.parameters()
        ):
    ...
    def test_compute_loss_variations(self, meta_learner):
        ...
        # Regression loss
        data = {"x": torch.randn(5, EMBEDDING_DIM), "y": torch.randn(5, 10)}
