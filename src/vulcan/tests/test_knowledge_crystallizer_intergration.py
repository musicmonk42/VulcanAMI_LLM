...
from src.vulcan.knowledge_crystallizer.principle_extractor import (
    PrincipleExtractor, ExecutionTrace, Metric, MetricType,
    Pattern, PatternType, ExtractionStrategy
)
from src.vulcan.knowledge_crystallizer.validation_engine import (
    KnowledgeValidator, Principle, ValidationLevel, DomainTestCase
)
from src.vulcan.knowledge_crystallizer.contraindication_tracker import (
    ContraindicationDatabase, ContraindicationGraph, CascadeAnalyzer,
    Contraindication, FailureMode, Severity
)
...
from src.vulcan.knowledge_crystallizer.crystallization_selector import (
    CrystallizationSelector, CrystallizationMethod, TraceCharacteristics
)
from src.vulcan.knowledge_crystallizer.knowledge_crystallizer_core import (
    KnowledgeCrystallizer, ExecutionTrace as CoreExecutionTrace,
    CrystallizationMode, ApplicationMode
)
...
def sample_execution_trace():
    ...
    return ExecutionTrace(
        ...
        metrics=[
            Metric(name="accuracy", metric_type=MetricType.ACCURACY, value=0.95, threshold=0.9),
            Metric(name="latency", metric_type=MetricType.LATENCY, value=0.5, threshold=1.0)
        ],
        success=True,
        domain="optimization"
    )
...
class TestKnowledgeStorage:
    ...
    def test_store_principle(self, sample_principle):
        ...
        stored_id = kb.store(sample_principle, author="test_user", message="Initial version")
    ...
    def test_pruning(self, sample_principle):
        ...
        candidates = pruner.identify_low_confidence([sample_principle], confidence_threshold=0.9)
...
class TestCrystallizationSelector:
    ...
    def test_select_cascade_aware(self):
        ...
        selection = selector.select_method(trace, context={'cascade_failures_detected': True})
