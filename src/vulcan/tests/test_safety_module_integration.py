...
class SafetyReport:
    ...
    audit_id: str = field(default_factory=lambda: hashlib.md5(str(time.time()).encode()).hexdigest()[:16])
    ...
    def validate_preconditions(self, context: Dict) -> Tuple[bool, List[str]]:
        ...
        for cond in self.preconditions:
            if not cond.evaluate(context):
                failures.append(cond.description or f"{cond.field} {cond.operator} {cond.value}")
...
class SafetyConfig:
    ...
    rollback_config: Dict[str, Any] = field(default_factory=lambda: {'storage_path': '/tmp/rollback'})
    audit_config: Dict[str, Any] = field(default_factory=lambda: {'log_path': '/tmp/audit'})
...
class CausalSafetyValidator:
    ...
    def validate_causal_edge(self, source: str, target: str, strength: float) -> ValidationResult:
        if np.isnan(strength):
            return ValidationResult(safe=False, reason="NaN strength value", severity="critical")
        if abs(strength) > self.max_strength:
            return ValidationResult(safe=False, reason=f"Strength {strength} too large", severity="warning")
        if source == target:
            return ValidationResult(safe=False, reason="Self-loop detected", severity="warning")
        for pattern in self.unsafe_patterns:
            if pattern in source.lower() or pattern in target.lower():
                return ValidationResult(safe=False, reason=f"Detected unsafe pattern in edge", severity="critical")
        return ValidationResult(safe=True, reason="Valid causal edge")
    
    def validate_causal_path(self, nodes: List[str], strengths: List[float]) -> ValidationResult:
        if len(nodes) - 1 != len(strengths):
            return ValidationResult(safe=False, reason="Mismatch between nodes and strengths count")
        ...
        if total_amplification > self.max_amplification:
            return ValidationResult(safe=False, reason=f"Excessive amplification: {total_amplification}")
    ...
    def validate_causal_graph(self, adjacency: Dict) -> ValidationResult:
        ...
        for node in adjacency:
            if node not in visited:
                if has_cycle(node):
                    return ValidationResult(safe=False, reason="Detected cycle in graph")
...
class PredictionSafetyValidator:
    ...
    def validate_prediction(self, expected: float, lower: float, upper: float, 
                           variable: str) -> ValidationResult:
        if np.isnan(expected):
            return ValidationResult(safe=False, reason="NaN prediction", severity="critical")
        if lower > upper:
        ...
        if variable in self.safe_regions:
            ...
            if upper < safe_low or lower > safe_high:
                return ValidationResult(safe=False, reason=f"Prediction outside safe region for {variable}")
...
class ToolSafetyManager:
    def __init__(self):
        ...
        self.rate_limiters = {name: TokenBucket(rate=100.0, capacity=100.0) 
    ...
    def _create_default_contracts(self) -> Dict[str, ToolSafetyContract]:
        return {
            'probabilistic': ToolSafetyContract(
                ...
                veto_conditions=[
                    Condition('adversarial_detected', '==', True, "Adversarial detected"),
    ...
    def check_tool_safety(self, tool_name: str, context: Dict) -> Tuple[bool, SafetyReport]:
        with self._lock:
            if tool_name not in self.contracts:
                return False, SafetyReport(
                    safe=False, confidence=0.0,
    ...
    def veto_tool_selection(self, tools: List[str], context: Dict) -> Tuple[List[str], SafetyReport]:
        allowed = []
        all_violations = []
        all_reasons = []
...
class ToolSafetyGovernor:
    ...
    def govern_tool_selection(self, request: Dict, tools: List[str]
                             ) -> Tuple[List[str], Dict]:
        with self._lock:
            if self.emergency_stop:
                return [], {'status': 'emergency_stop', 'allowed_tools': [], 'veto_report': None}
            
            # Remove quarantined tools
            now = time.time()
            expired = [t for t, info in self.quarantine_list.items() 
                      if info['expires'] < now]
            for t in expired:
...
class RollbackManager:
    ...
    def create_snapshot(self, state: Dict, action_log: List) -> str:
        with self._lock:
            snapshot_id = hashlib.md5(f"{time.time()}{len(self.snapshots)}".encode()).hexdigest()[:16]
    ...
    def quarantine_action(self, action: Dict, reason: str, duration_seconds: float = 3600) -> str:
        with self._lock:
            quarantine_id = hashlib.md5(f"{time.time()}{action}".encode()).hexdigest()[:16]
    ...
    def review_quarantine(self, quarantine_id: str, approved: bool, reviewer: str) -> bool:
        with self._lock:
...
class AuditLogger:
    ...
    def log_event(self, event_type: str, data: Dict, severity: str = "info") -> str:
        with self._lock:
            entry_id = hashlib.md5(f"{time.time()}{event_type}".encode()).hexdigest()[:16]
            self.entries.append({
                'entry_id': entry_id,
                'type': event_type,
                'data': data,
...
class GovernanceManager:
    ...
    def request_approval(self, action: Dict) -> Dict:
        with self._lock:
            decision_id = hashlib.md5(f"{time.time()}{action}".encode()).hexdigest()[:16]
    ...
    def register_stakeholder(self, stakeholder_id: str, stakeholder_type: StakeholderType,
...
class ValueAlignmentSystem:
    ...
    def check_alignment(self, action: Dict, context: Dict) -> Dict:
        ...
        alignment_score = sum(
            value_scores.get(v, 0.5) * weight 
            for v, weight in self.core_values.items()
...
class HumanOversightInterface:
    ...
    def get_oversight_status(self) -> Dict:
        return {
            ...
            'active_alerts': len([a for a in self.alerts.values() if not a.get('acknowledged')])
        }
...
class EnhancedExplainabilityNode:
    ...
    def get_explanation_stats(self) -> Dict:
        return {
            ...
            'avg_quality': np.mean([e['quality_score'] for e in self.explanations]) if self.explanations else 0
        }
...
class ExplanationQualityScorer:
    ...
    def score(self, explanation: Dict) -> float:
        ...
        # Relevance
        has_factors = 'decision_factors' in explanation or 'feature_importance' in explanation
...
class EnhancedSafetyValidator:
    def __init__(self, config: SafetyConfig = None):
        ...
        self.tool_safety_manager = ToolSafetyManager() if self.config.enable_tool_safety else None
        
        self.rollback_manager = RollbackManager(
            config=self.config.rollback_config
        ) if self.config.enable_rollback else None
    ...
    def validate_action_comprehensive(self, action: Dict, context: Dict,
                                      create_snapshot: bool = False) -> SafetyReport:
        with self._lock:
            ...
            if action_confidence < self.config.safety_thresholds.get('confidence_min', 0.6):
    ...
    async def validate_action_comprehensive_async(self, action: Dict, context: Dict,
                                                  timeout_per_validator: float = 2.0,
                                                  total_timeout: float = 10.0) -> SafetyReport:
        # Run sync version in executor for async interface
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, 
            lambda: self.validate_action_comprehensive(action, context)
        )
    ...
    def validate_prediction_comprehensive(self, expected: float, lower: float, 
                                          upper: float, context: Dict) -> Dict:
        variable = context.get('target_variable', 'unknown')
        result = self.prediction_validator.validate_prediction(expected, lower, upper, variable)
        return {'safe': result.safe, 'reason': result.reason}
    
    def validate_tool_selection(self, tools: List[str], context: Dict
...
class TestDomainValidators:
    ...
    def test_optimization_params_validation(self):
        ...
        params = {
            'max_iterations': 1000, 'tolerance': 1e-6, 'learning_rate': 0.01,
    ...
    def test_data_processing_validation(self):
        ...
        df_info = {
            'rows': 1000, 'columns': 50, 'memory_mb': 10, 'missing_ratio': 0.1,
            'dtypes': {'col1': 'int64', 'col2': 'float64'}
        }
        result = validator.validate_dataframe(df_info)
        assert result.safe == True
        
        df_info = {'rows': 20000000, 'columns': 50, 'memory_mb': 10, 'missing_ratio': 0.1}
        result = validator.validate_dataframe(df_info)
        assert result.safe == False
...
class TestToolSafety:
    ...
    def test_tool_safety_check(self):
        ...
        context = {
            'confidence': 0.8, 'data_quality': 0.9, 'corrupted_data': False,
            'adversarial_detected': False, 'system_overload': False,
            'logic_valid': True, 'causal_graph_valid': True, 'sample_size': 100,
            'temporal_paradox': False, 'estimated_resources': {'memory_mb': 100, 'time_ms': 1000}
        }
        ...
        context = {'confidence': 0.3, 'adversarial_detected': False, 'system_overload': False}
        safe, report = manager.check_tool_safety('probabilistic', context)
        assert safe == False
    ...
    def test_tool_veto_selection(self):
        ...
        context = {
            'confidence': 0.8, 'data_quality': 0.9, 'corrupted_data': False,
            'adversarial_detected': False, 'system_overload': False,
            'logic_valid': True, 'axioms_count': 50, 'contradictory_axioms': False,
            'causal_graph_valid': True, 'sample_size': 100, 'temporal_paradox': False
        }
...
class TestGovernanceAlignment:
    def test_governance_manager_initialization(self, temp_dir):
        config = {'db_path': str(Path(temp_dir) / 'governance.db'), 'max_active_decisions': 100}
    ...
    def test_value_alignment_system(self):
        ...
        action = {
            'type': ActionType.EXPLORE, 'safety_score': 0.8,
            'explanation': 'Testing alignment', 'auditable': True
        }
        context = {}
    ...
    def test_value_alignment(self):
        ...
        action = {
            'type': ActionType.OPTIMIZE, 'safety_score': 0.85,
...
class TestSafetyValidator:
    def test_constraint_manager(self):
        ...
        constraint = SafetyConstraint(
            name='test_constraint', type='hard',
            check_function=lambda a, c: (a.get('value', 0) < 100, 0.9),
            threshold=0.0, priority=5
        )
    ...
    def test_explainability_node(self):
        ...
        data = {'decision': 'explore', 'confidence': 0.8, 'features': {'f1': 0.5, 'f2': 0.3}}
    ...
    def test_explanation_quality_scorer(self):
        ...
        explanation = {
            ...
            'method': 'neural', 'context': {'decision_type': 'explore'},
            'alternatives': [{'action': 'wait', 'reason': 'gather info'}],
            'confidence': 0.85, 'decision_factors': ['factor1', 'factor2'],
    ...
    def test_comprehensive_validation_sync(self, temp_dir, safety_config, sample_action, sample_context):
        ...
        validator = EnhancedSafetyValidator(config=safety_config)
        report = validator.validate_action_comprehensive(sample_action, sample_context, create_snapshot=True)
    ...
    def test_comprehensive_validation_async(self, temp_dir, safety_config, sample_action, sample_context):
        safety_config.rollback_config['storage_path'] = str(Path(temp_dir) / 'rollback')
        safety_config.audit_config['log_path'] = str(Path(temp_dir) / 'audit')
        safety_config.enable_adversarial_testing = False
        ...
        async def run_test():
            report = await validator.validate_action_comprehensive_async(
                sample_action, sample_context, timeout_per_validator=2.0, total_timeout=10.0
            )
            return report
    ...
    def test_domain_validator_delegation(self, temp_dir, safety_config):
        ...
        result = validator.validate_prediction_comprehensive(10.0, 8.0, 12.0, {'target_variable': 'temperature'})
        assert 'safe' in result
...
class TestIntegration:
    ...
    def test_end_to_end_unsafe_action(self, temp_dir, safety_config):
        ...
        unsafe_action = {
            'type': ActionType.OPTIMIZE, 'confidence': 0.3, 'uncertainty': 0.95,
    ...
    def test_tool_safety_integration(self, temp_dir, safety_config):
        ...
        action = {
            'type': ActionType.OPTIMIZE, 'tool_name': 'probabilistic',
            'confidence': 0.8, 'uncertainty': 0.2
        }
        context = {
            'data_quality': 0.9, 'corrupted_data': False,
            'adversarial_detected': False, 'system_overload': False
        }
    ...
    def test_concurrent_validations(self, temp_dir, safety_config):
        ...
        def validate_action(action_id):
            action = {
                'type': ActionType.OPTIMIZE, 'id': f'action_{action_id}',
                'confidence': 0.8, 'uncertainty': 0.2
            }
    ...
    def test_async_parallel_validations(self, temp_dir, safety_config):
        ...
        async def run_parallel():
            tasks = []
            for i in range(3):
                action = {'type': ActionType.OPTIMIZE, 'id': f'async_action_{i}', 'confidence': 0.8}
    ...
    def test_governance_integration(self, temp_dir, safety_config):
        ...
        action = {
            'type': ActionType.OPTIMIZE, 'risk_score': 0.3, 'safety_score': 0.8,
            'confidence': 0.85, 'explanation': 'Test governance integration', 'auditable': True
        }
...
class TestStress:
    ...
    def test_memory_bounded_structures(self):
        ...
        for i in range(2000):
            context = {'confidence': 0.8, 'adversarial_detected': False, 'system_overload': False}
    ...
    def test_constraint_manager_scale(self):
        ...
        for i in range(50):
            constraint = SafetyConstraint(
                name=f'constraint_{i}', type='soft',
                check_function=lambda a, c, i=i: (a.get('value', 0) < 100 + i, 0.9),
                threshold=0.0, priority=i
            )
            manager.add_constraint(constraint)
...
class TestErrorHandling:
    ...
    def test_database_error_recovery(self, temp_dir):
        manager = RollbackManager(config={'storage_path': '/invalid/path/that/does/not/exist'})
