...
VALID_TRANSITIONS = {
    ...
    AgentState.IDLE: {AgentState.WORKING, AgentState.PAUSED, AgentState.ERROR, AgentState.TERMINATED},
    AgentState.WORKING: {AgentState.IDLE, AgentState.PAUSED, AgentState.ERROR, AgentState.TERMINATED},
    AgentState.PAUSED: {AgentState.IDLE, AgentState.WORKING, AgentState.ERROR, AgentState.TERMINATED},
...
def create_agent_metadata(agent_id: str, capability: AgentCapability = AgentCapability.GENERAL,
                          location: str = "local") -> AgentMetadata:
    return AgentMetadata(agent_id=agent_id, capability=capability, location=location)


def create_job_provenance(job_id: str, graph_id: str, parameters: Dict = None,
                          priority: int = 0) -> JobProvenance:
    return JobProvenance(job_id=job_id, graph_id=graph_id, 
                         parameters=parameters or {}, priority=priority)
...
class AgentPoolManager:
    ...
    def spawn_agent(self, capability: AgentCapability = AgentCapability.GENERAL,
    ...
    def submit_job(self, graph: Dict, parameters: Dict = None, priority: int = 0,
        ...
        with self._lock:
            ...
            job = create_job_provenance(job_id, graph.get('id', 'unknown'), 
                                       parameters, priority)
            job.start_execution()
...
class FaultTolerantOrchestrator(VULCANAGICollective):
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        return {
            ...
            'success_rate': self.successful_attempts / self.total_attempts if self.total_attempts > 0 else 1.0
        }
...
class ProductionDeployment:
    def __init__(self, config, orchestrator_type: str = "basic"):
        ...
        if orchestrator_type == "parallel":
            ...
            self.collective = FaultTolerantOrchestrator(config, self.system_state, self.deps)
    ...
    def step_with_monitoring(self, history: List, context: Dict) -> Dict[str, Any]:
        ...
        if self.checkpoint_dir and self.system_state.step % checkpoint_interval == 0:
            auto_path = self.checkpoint_dir / f"auto_checkpoint_{self.system_state.step}.pkl"
...
class TestAgentLifecycle:
    def test_agent_creation_and_transitions(self):
        agent = create_agent_metadata("test_agent_001", capability=AgentCapability.REASONING, location="local")
    ...
    def test_job_provenance_tracking(self):
        job = create_job_provenance(job_id="job_001", graph_id="graph_001",
                                    parameters={"param1": "value1"}, priority=5)
...
class TestAgentPoolIntegration:
    ...
    def test_agent_spawning_and_retirement(self):
        ...
        try:
            ...
            agent_id = pool.spawn_agent(capability=AgentCapability.PERCEPTION, location="local")
    ...
    def test_job_submission_and_tracking(self):
        ...
        try:
            ...
            job_id = pool.submit_job(
                graph=graph, parameters={"test": "value"}, priority=1,
                capability_required=AgentCapability.GENERAL, timeout_seconds=5.0
            )
...
class TestDependenciesIntegration:
    ...
    def test_dependency_status_reporting(self):
        ...
        print(f"✓ Dependency status: {status['available_count']} available, {status['missing_count']} missing")
...
class TestProductionDeployment:
    ...
    def test_deployment_step_execution(self, minimal_config, temp_dir):
        ...
        try:
            results = []
            for i in range(2):
                result = deployment.step_with_monitoring([], {'high_level_goal': 'test'})
                assert result is not None
    ...
    def test_deployment_with_different_orchestrators(self, minimal_config, temp_dir):
        ...
        for orch_type in orchestrator_types:
            deployment = ProductionDeployment(minimal_config, orchestrator_type=orch_type)
...
class TestEndToEndWorkflows:
    ...
    def test_multi_agent_collaboration(self, minimal_config, temp_dir):
        ...
        try:
            tasks = []
            for i in range(2):
                context = {'high_level_goal': 'collaborate', 'task_id': i,
    ...
    def test_long_running_operation(self, minimal_config, temp_dir):
        ...
        try:
            ...
            for i in range(num_iterations):
                context = {'high_level_goal': 'explore' if i % 2 == 0 else 'optimize', 'iteration': i}
                result = deployment.step_with_monitoring(history, context)
                assert result is not None
                history.append(result.get('observation'))
...
class TestStressConditions:
    ...
    def test_error_recovery(self, minimal_config, temp_dir):
        ...
        deployment = ProductionDeployment(minimal_config, orchestrator_type="fault_tolerant")
        ...
        try:
            ...
            assert results_returned >= 8, f"Fault-tolerant system should handle most requests: {results_returned}/10"
...
class TestIntegrationSummary:
    def test_full_system_integration(self, minimal_config, temp_dir):
        ...
        try:
            for i in range(5):
                context = {'high_level_goal': 'full_test', 'iteration': i,
