...
class TestCostEstimate:
    ...
    def test_cost_estimate_negative_percentile(self):
        with pytest.raises(
            ValueError, match="Percentile value .* must be non-negative"
        ):
            CostEstimate(
                time_ms=100.0,
                energy_mj=50.0,
...
class TestValueOfInformation:
    ...
    def test_voi_empty_recommendation(self):
        with pytest.raises(
            ValueError, match="recommendation must be a non-empty string"
        ):
            ValueOfInformation(
                expected_value=10.0,
                information_gain=5.0,
...
class TestDistributionShift:
    ...
    def test_distribution_shift_negative_feature_index(self):
        with pytest.raises(
            ValueError, match="affected_features.* must be non-negative"
        ):
            DistributionShift(
                drift_detected=True,
                drift_type="SUDDEN",
