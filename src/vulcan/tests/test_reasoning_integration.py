...
class TestUnifiedReasoningIntegration:
    ...
    def reasoning_system(self, temp_dir):
        ...
        system = UnifiedReasoner(
            enable_learning=False, enable_safety=True, max_workers=2, config=config
        )
    ...
    def test_different_strategies(self, reasoning_system):
        ...
        for strategy in strategies:
            try:
                result = reasoning_system.reason(
                    input_data=input_data, query=query, strategy=strategy
                )
    ...
    def test_constraints_handling(self, reasoning_system):
        ...
        for constraint in constraints:
            result = reasoning_system.reason(
                input_data=input_data, query=query, constraints=constraint
            )
    ...
    def test_concurrent_reasoning(self, reasoning_system):
        ...
        def reason_task(task_id):
            ...
            result = reasoning_system.reason(
                input_data=input_data, query={"task_id": task_id}
            )
    ...
    def test_statistics_collection(self, reasoning_system):
        ...
        for i in range(3):
            reasoning_system.reason(input_data=[1, 2, 3, 4, 5], query={"iteration": i})
    ...
    def test_state_persistence(self, reasoning_system, temp_dir):
        ...
        for i in range(2):
            reasoning_system.reason(input_data=[1, 2, 3], query={"id": i})
    ...
    def test_error_recovery(self, reasoning_system):
        ...
        result = reasoning_system.reason(input_data=None, query={})
        assert result is not None, "Should handle None input"
        print("✅ Handled None input")

        # Test with empty input
        result = reasoning_system.reason(input_data=[], query={})
    ...
    def test_final_system_validation(self, reasoning_system):
        ...
        result = reasoning_system.reason(
            input_data=input_data, query=query, strategy=ReasoningStrategy.ADAPTIVE
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        tests = [
            ...
            (
                "State Persistence",
                lambda: test.test_state_persistence(reasoning_system, temp_dir),
            ),
