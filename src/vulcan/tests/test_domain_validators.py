...
class TestCausalSafetyValidator:
    ...
    def test_validate_graph_too_large(self, causal_validator):
        ...
        adjacency = {}
        for i in range(100):
            adjacency[f'node_{i}'] = [(f'node_{j}', 1.0) for j in range(i+1, min(i+20, 100))]
        
        result = causal_validator.validate_causal_graph(adjacency)
        
        # May fail due to size or edge violations
        if not result.safe:
            assert 'too large' in result.reason.lower() or 'unsafe edges' in result.reason.lower()
...
class TestPredictionSafetyValidator:
    ...
    def test_validate_prediction_bounds_extend_outside_safe_region(self, prediction_validator):
    ...
    def test_validate_prediction_batch_mixed(self, prediction_validator):
        ...
        predictions = [
            ...
            {'expected': 150.0, 'lower': 140.0, 'upper': 160.0, 'variable': 'temperature'},
        ]
