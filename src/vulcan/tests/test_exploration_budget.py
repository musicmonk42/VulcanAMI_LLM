...
def mock_experiment():
    ...
    exp.parameters = {"sample_size": 100, "iterations": 50}
    return exp
...
class TestLoadAdjuster:
    ...
    def test_calculate_adjustment_low_load(self, load_adjuster):
        ...
        new_budget = load_adjuster.calculate_adjustment(
            system_load=0.2, current_budget=100.0, min_budget=10.0, max_budget=200.0
        )
    ...
    def test_calculate_adjustment_high_load(self, load_adjuster):
        ...
        new_budget = load_adjuster.calculate_adjustment(
            system_load=0.9, current_budget=100.0, min_budget=10.0, max_budget=200.0
        )
    ...
    def test_calculate_adjustment_normal_load(self, load_adjuster):
        ...
        new_budget = load_adjuster.calculate_adjustment(
            system_load=0.5, current_budget=100.0, min_budget=10.0, max_budget=200.0
        )
    ...
    def test_calculate_adjustment_clamping(self, load_adjuster):
        ...
        # Test max limit
        new_budget = load_adjuster.calculate_adjustment(
            system_load=0.1, current_budget=190.0, min_budget=10.0, max_budget=200.0
        )
        assert new_budget <= 200.0

        # Test min limit
        new_budget = load_adjuster.calculate_adjustment(
            system_load=0.95, current_budget=15.0, min_budget=10.0, max_budget=200.0
        )
    ...
    def test_calculate_adjustment_invalid_load(self, load_adjuster):
        ...
        # Test load > 1.0
        new_budget = load_adjuster.calculate_adjustment(
            system_load=1.5, current_budget=100.0, min_budget=10.0, max_budget=200.0
        )
        assert new_budget > 0
...
class TestEfficiencyTracker:
    ...
    def test_calculate_base_adjustment_high_efficiency(self, efficiency_tracker):
        ...
        new_base = efficiency_tracker.calculate_base_adjustment(
            current_base=100.0, min_base=10.0, max_base=200.0
        )
...
class TestCostEstimator:
    ...
    def test_estimate_learning_cost(self, cost_estimator):
        ...
        cost = cost_estimator.estimate_learning_cost(
            gap_type="decomposition", complexity=0.5, priority=0.8, domain="test_domain"
        )
    ...
    def test_update_from_actual(self, cost_estimator):
        ...
        cost_estimator.update_from_actual(
            experiment_type="decomposition", predicted=10.0, actual=12.0, domain="test"
        )
...
class TestIntegration:
    ...
    def test_cost_estimation_and_calibration(self, cost_estimator, mock_experiment):
        ...
        # Update with actual
        cost_estimator.update_from_actual(
            "decomposition", predicted_cost, actual_cost, "test_domain"
        )
