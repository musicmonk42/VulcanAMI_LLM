...
class AgentMessage:
    ...
    def to_task(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "capability": self.receiver_agent if self.receiver_agent != "all" else "general",
            "nodes": [
                {
                    "id": "receive",
                ...
                {
                    "id": "process",
                    "type": self.receiver_agent if self.receiver_agent != "all" else "general",
...
class CollaborationSession:
    ...
    def is_active(self) -> bool:
        ...
        return self.status in (CollaborationStatus.PENDING, CollaborationStatus.IN_PROGRESS)
...
class AgentCollaborationManager:
    ...
    def __init__(
        ...
        self._on_complete_callback: Optional[Callable[[CollaborationSession], None]] = None
    ...
    def _cleanup_stale_sessions(self) -> None:
        ...
        with self._lock:
            for session_id, session in self._sessions.items():
                # Check for timeout
                if session.is_active and (current_time - session.start_time) > self._session_timeout:
        ...
        if sessions_to_remove:
            logger.debug(f"[Collaboration] Cleaned up {len(sessions_to_remove)} stale sessions")
    ...
    def set_on_complete_callback(self, callback: Callable[[CollaborationSession], None]) -> None:
        """
    ...
    def start_collaboration(
        ...
        with self._lock:
            ...
            self._stats["avg_agents_per_collaboration"] = (
                (current_avg * (total - 1) + len(agent_types)) / total
            )
    ...
    def _submit_message_to_pool(self, msg: AgentMessage, session_id: str) -> None:
        ...
        if not self._agent_pool:
            logger.warning("[Collaboration] Agent pool not available for task submission")
            return
        ...
        try:
            ...
            logger.error(f"[Collaboration] Failed to submit message: {e}", exc_info=True)
    ...
    def send_message(
        ...
        with self._lock:
            ...
            if len(session.messages) >= MAX_MESSAGES_PER_SESSION:
                logger.warning(f"[Collaboration] Session {session_id} message limit reached")
                return None
    ...
    def _complete_collaboration(self, session: CollaborationSession) -> None:
        ...
        with self._lock:
            ...
            self._stats["avg_duration_seconds"] = (
                (current_avg * (total - 1) + session.duration_seconds) / total
            )
        ...
        for agent in agents:
            msg = AgentMessage(
                ...
                content={
                    ...
                    "debate_round": len([m for m in session.messages if m.message_type == MessageType.DEBATE]) + 1,
                },
                context=session.original_query,
            )
        ...
        for agent in agents:
            msg = AgentMessage(
                ...
                content={
                    "options": options,
                    "voter": agent,
                    "vote_round": len([m for m in session.messages if m.message_type == MessageType.VOTE]) + 1,
                },
                context=session.original_query,
            )
