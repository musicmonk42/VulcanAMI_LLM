...
class TaskQueueActor:
    ...
    def __init__(self):
        self.status = {"queue_type": "ray", "total_tasks": 0, "pending": 0, "running": 0}
...
class TaskQueueInterface:
    ...
    def submit_task(self, task: Dict[str, Any], priority: int = 0, 
                ...
                if metadata.status == TaskStatus.RUNNING and metadata.timeout_seconds:
                    if metadata.started_at:
                        ...
                        if elapsed > metadata.timeout_seconds:
                            logger.warning(f"Task {task_id} timeout after {elapsed:.2f}s")
...
class CeleryTaskQueue(TaskQueueInterface):
    ...
    def __init__(self, broker_url: str = 'redis://localhost:6379',
        ...
        if not CELERY_AVAILABLE:
            raise ImportError("Celery is not available. Install with: pip install celery redis")
...
class CustomTaskQueue(TaskQueueInterface):
    ...
    def __init__(self, coordinator_address: str = "tcp://localhost:5555",
        ...
        if self.use_ray:
            try:
                ...
                ray.init(address=ray_config.get('address', 'auto'), ignore_reinit_error=True)
    ...
    def submit_task(self, task: Dict[str, Any], priority: int = 0,
        ...
        if self.use_ray and self.actor:
            try:
                ...
                metadata = TaskMetadata(
                    task_id=task_id, status=TaskStatus.PENDING, submitted_at=time.time(),
                    priority=priority, timeout_seconds=timeout, max_retries=max_retries
                )
                with self._lock:
                    self.pending_tasks[task_id] = metadata
        ...
        try:
            ...
            message = {
                "action": "submit", "task_id": task_id, "task": task,
            ...
            if response.get("status") == "accepted":
                ...
                raise RuntimeError(f"Task submission rejected: {response.get('reason', 'unknown')}")
    ...
    def get_result(self, task_id: str, timeout: Optional[float] = None) -> Any:
        ...
        if self.use_ray:
            logger.warning("get_result not implemented for Ray status actor mode in CustomTaskQueue.")
            return None
    ...
    def get_coordinator_status(self, retries: int = 2, initial_delay: float = 0.1) -> Dict[str, Any]:
        ...
        with self._coordinator_check_lock:
            if current_time - self._last_coordinator_check < self._coordinator_check_interval:
                logger.debug("Returning cached coordinator status (within cache interval)")
        ...
        for attempt in range(retries):
            try:
                ...
                if not self._socket_lock.acquire(timeout=0.05):
                    logger.warning("Could not acquire socket lock, skipping status check")
                ...
                try:
                    ...
                    if poller.poll(200):
                        ...
                        logger.debug("ZMQ status check timed out on attempt %d", attempt + 1)
                finally:
                ...
                logger.debug(f"Unexpected error on attempt {attempt+1}/{retries}: {e}")
        ...
        logger.debug("Failed to get coordinator status after %d retries, marking unavailable", retries)
        with self._coordinator_check_lock:
            self._coordinator_available = False
            return self._cached_coordinator_status or {}
