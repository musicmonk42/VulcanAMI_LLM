...
class EnhancedMetricsCollector:
    ...
    def record_step(self, duration: float, result: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self.timeseries['step_duration_ms_time'].append((time.time(), duration * 1000))
            ...
            if 'uncertainty' in result:
                ...
                self.timeseries['uncertainty_over_time'].append((time.time(), uncertainty))
    ...
    def record_timeseries(self, name: str, value: float, timestamp: Optional[float] = None):
        """
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for name, values in self.histograms.items():
                if values:
                    summary['histograms_summary'][name] = self._compute_histogram_stats(list(values))
            
            # Compute rates
            total_actions = self.counters.get('successful_actions', 0) + self.counters.get('failed_actions', 0)
            if total_actions > 0:
            ...
            if uptime > 0:
                summary['rates']['steps_per_second'] = self.counters.get('steps_total', 0) / uptime
            else:
    ...
    def get_timeseries(self, metric_name: str, last_n: Optional[int] = None) -> List[Tuple[float, float]]:
        """
    ...
    def _compute_health_score(self) -> float:
        ...
        if 'step_duration_ms' in self.histograms and len(self.histograms['step_duration_ms']) > 0:
        ...
        with self._lock:
            ...
            if 'histograms' in data:
                for name, values in data['histograms'].items():
                    self.histograms[name] = deque(values, maxlen=self.max_histogram_size)
            
            # Import timeseries
            if 'timeseries' in data:
                for name, series in data['timeseries'].items():
                    self.timeseries[name] = deque(series, maxlen=self.max_timeseries_size)
...
def compute_moving_average(timeseries: List[Tuple[float, float]], 
...
def compute_rate(timeseries: List[Tuple[float, float]], 
