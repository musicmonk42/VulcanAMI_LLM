...
class AgentPoolManager:
    ...
    def __init__(self, 
        ...
        self.task_queue: Optional[TaskQueueInterface] = create_task_queue(task_queue_type, **queue_config)
        ...
        try:
            ...
            try:
                if task_queue_type != "custom":
                    self.task_queue = create_task_queue("custom", **self.config.get('queue_config', {}))
    ...
    def _initialize_agent_pool(self):
        ...
        for i in range(self.min_agents):
            ...
            try:
                ...
                if agent_id:
                    logger.debug(f"Initialized agent {agent_id} with capability {capability.value}")
            except Exception as e:
    ...
    def spawn_agent(self, 
                   ...
                   location: str = "local",
                   hardware_spec: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """
        Spawn a new agent
        ...
        with self.lock:
            ...
            try:
                ...
                if location == "local":
                    ...
                    logger.warning(f"Unknown location '{location}', defaulting to local")
                    self._spawn_local_agent(agent_id, metadata)
    ...
    def retire_agent(self, agent_id: str, force: bool = False) -> bool:
        ...
        with self.lock:
            ...
            tasks_to_cancel = [
                tid for tid, aid in self.task_assignments.items()
                if aid == agent_id
            ]
            ...
            if metadata.state == AgentState.WORKING and not force:
                ...
                metadata.transition_state(
                    AgentState.RETIRING,
                    "Marked for retirement after current task"
                )
                logger.info(f"Agent {agent_id} marked for retirement after current task")
            else:
                # Immediate termination
                metadata.transition_state(AgentState.TERMINATED, "Forced retirement" if force else "Retirement")
    ...
    def recover_agent(self, agent_id: str) -> bool:
        ...
        with self.lock:
            ...
            if not metadata.should_recover():
                logger.info(f"Agent {agent_id} should not be recovered (too many errors)")
                return False
            
            # Validate state transition
            if not metadata.transition_state(AgentState.RECOVERING, "Recovery initiated"):
    ...
    def submit_job(self, 
        ...
        with self.lock:
            ...
            if agent_id:
                ...
                logger.warning(f"No agent available for job {job_id} within {timeout_seconds}s")
    ...
    def _assign_agent_with_timeout(self, 
    ...
    def _assign_agent(self, capability: AgentCapability) -> Optional[str]:
        ...
        best_agent = min(
            available_agents,
            key=lambda aid: self.agents[aid].tasks_failed / max(1, self.agents[aid].tasks_completed)
        )
        
        return best_agent
    
    def _assign_job_to_agent(self, 
                            job_id: str, 
                            agent_id: str, 
                            graph: Dict[str, Any], 
        ...
        if agent_id in self.agents:
            self.agents[agent_id].transition_state(AgentState.WORKING, f"Assigned job {job_id}")
    ...
    def _execute_agent_task(self, 
        ...
        try:
            ...
            logger.debug(f"Agent {agent_id} completed task {task_id} in {duration:.3f}s")
            return result
        ...
        with self.lock:
            ...
            if agent_id and agent_id in self.agents:
                ...
                if metadata.state == AgentState.WORKING:
                    metadata.transition_state(AgentState.IDLE, f"Task {task_id} cancelled")
        ...
        with self._archive_lock:
            try:
                ...
                if not CACHETOOLS_AVAILABLE:
                    ...
                    expired_jobs = [
                        job_id for job_id, create_time in self.provenance_creation_times.items()
                    ...
                    if expired_jobs:
                        ...
                        logger.debug(f"Cleaned up {len(expired_jobs)} expired provenance records")
                ...
                if len(self.provenance_records) > 9000:
                    ...
                    with open(archive_file, 'w') as f:
                        for job_id, prov in records_to_archive:
                            try:
                                ...
                                logger.error(f"Failed to serialize provenance {job_id}: {e}")
                    ...
                    with self.lock:
                        for job_id, _ in records_to_archive:
                            if job_id in self.provenance_records:
                                del self.provenance_records[job_id]
                            if not CACHETOOLS_AVAILABLE and job_id in self.provenance_creation_times:
                                del self.provenance_creation_times[job_id]
                    
                    self._last_archive_time = time.time()
                    logger.info(f"Archived {len(records_to_archive)} provenance records to {archive_file}")
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                with self.lock:
                    ...
                    stale_tasks = [
                        task_id for task_id, assign_time in self.task_assignment_times.items()
    ...
    def get_pool_status(self) -> Dict[str, Any]:
        ...
        if current_time - self.last_status_check < self.status_check_interval:
            logger.debug("Skipping queue status check due to throttling; returning cached agent data.")
            ...
            if self.task_queue and hasattr(self.task_queue, 'get_coordinator_status'):
                try:
                 ...
                 try:
                    ...
                    logger.warning(f"Failed to get queue status from get_queue_status: {e}")
...
class RecoveryManager:
    ...
    def recover_agent(self, agent_id: str) -> bool:
        ...
        logger.warning(f"No recovery strategy for agent {agent_id} in state {metadata.state}")
        return False
    ...
    def _recover_terminated_agent(self, agent_id: str, metadata: AgentMetadata) -> bool:
        ...
        if self.pool.get_pool_status()["total_agents"] < self.pool.min_agents:
            logger.info(f"Pool below minimum, spawning replacement for terminated agent {agent_id}")
