...
class StateTransitionRules:
    ...
    @classmethod
    def validate_transition(cls, from_state: AgentState, to_state: AgentState, 
                          agent_id: str = "unknown") -> bool:
        """Validate and log state transition"""
        if cls.is_valid_transition(from_state, to_state):
            logger.debug(f"Agent {agent_id}: Valid transition {from_state} -> {to_state}")
            return True
        else:
            logger.warning(
...
class AgentMetadata:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.capability, AgentCapability):
            raise ValueError(f"capability must be AgentCapability, got {type(self.capability)}")
    ...
    def transition_state(self, new_state: AgentState, reason: str = "") -> bool:
        ...
        if self.state == AgentState.TERMINATED and new_state == AgentState.TERMINATED:
            logger.debug(f"Agent {self.agent_id} already terminated, allowing idempotent transition")
            return True

        if StateTransitionRules.validate_transition(self.state, new_state, self.agent_id):
    ...
    def _record_state_change(self, new_state: AgentState, reason: str, 
                            old_state: Optional[AgentState] = None):
    ...
    def _update_performance_metrics(self):
        ...
        if total_tasks > 0:
            ...
            self.performance_metrics['failure_rate'] = (
                self.tasks_failed / total_tasks
            )
        else:
        ...
        self.performance_metrics['average_task_duration_s'] = self.average_task_duration_s
        ...
        if uptime_s > 0:
            self.performance_metrics['utilization'] = (
                self.total_runtime_s / uptime_s
            )
        else:
