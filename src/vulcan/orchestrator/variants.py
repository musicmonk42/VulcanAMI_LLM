    ...
    try:
        ...
        if SUPPORTS_EXECUTOR_TIMEOUT:
            try:
                ...
                logger.debug(f"{executor_name}: timeout parameter not supported, using manual shutdown")
    ...
    def step_with_recovery(self, history: List[Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"Perception error on attempt {attempt + 1}/{max_retries}: {e}")
                ...
                logger.warning(f"Reasoning error on attempt {attempt + 1}/{max_retries}: {e}")
                ...
                logger.warning(f"Execution error on attempt {attempt + 1}/{max_retries}: {e}")
...
class AdaptiveOrchestrator(VULCANAGICollective):
    ...
    def adaptive_step(self, history: List[Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Selected strategy: {strategy} (latency={metrics.get('avg_latency', 0):.1f}ms, "
    ...
    def get_adaptation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'current_strategy': adaptations[-1]['strategy'] if adaptations else 'balanced',
