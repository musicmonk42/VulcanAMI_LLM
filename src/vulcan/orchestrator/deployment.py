...
from .variants import ParallelOrchestrator, AdaptiveOrchestrator, FaultTolerantOrchestrator
...
class ProductionDeployment:
    ...
    def __init__(self, 
        ...
        logger.info(f"ProductionDeployment initialized (orchestrator: {orchestrator_type})")
    ...
    def initialize(self, checkpoint_path: Optional[str]):
        ...
        try:
            ...
            self.collective = self._create_orchestrator(self.orchestrator_type, sys, deps)
    ...
    def _import_components(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Using MagicMock for WorldModel (aliased as CausalWorldModel)")
        ...
        try:
            if CausalWorldModel and not isinstance(CausalWorldModel, MagicMock):
                ...
                cfg_file = getattr(self.config, 'intrinsic_drives_config_file', 'configs/intrinsic_drives.json')
                state_file = getattr(self.config, 'intrinsic_drives_state_file', 'data/agent_state.json')
                ...
                logger.info("WorldModel (causal) initialized with meta-reasoning=%s, self-improvement=%s",
                            True, enable_si)
            elif CausalWorldModel and isinstance(CausalWorldModel, MagicMock):
                 logger.warning("CausalWorldModel is a MagicMock, skipping initialization.")
                 components['world_model'] = None
            else:
                 # This else is redundant with the except ImportError, but good for clarity
                 logger.warning("CausalWorldModel not imported, skipping initialization.")
                 components['world_model'] = None

        except Exception as e:
            logger.warning(f"Causal WorldModel unavailable or failed to initialize: {e}")
            components['world_model'] = None
        ...
        try:
            ...
            if components['world_model'] is None and hasattr(learning_module, 'UnifiedWorldModel'):
                try:
                    components['world_model'] = learning_module.UnifiedWorldModel(components.get('multimodal'))
                    logger.info("UnifiedWorldModel (learning) loaded (no meta-reasoning/self-improvement)")
                except Exception as e:
            ...
            if hasattr(learning_module, 'MetaCognitiveMonitor'):
                try:
                    components['meta_cognitive'] = learning_module.MetaCognitiveMonitor()
            ...
            if hasattr(learning_module, 'CompositionalUnderstanding'):
                try:
                    components['compositional'] = learning_module.CompositionalUnderstanding()
                    logger.info("CompositionalUnderstanding loaded")
                except Exception as e:
                    logger.warning(f"Failed to initialize CompositionalUnderstanding: {e}")
            
            # Log summary
            available_learners = sum(1 for v in [
                components['continual'],
                components['meta_cognitive'],
                components['compositional'],
                components['world_model'] if components['world_model'] is not None and
                components['world_model'].__class__.__name__ == 'UnifiedWorldModel' else None
            ] if v is not None)
            logger.info(f"Learning components: {available_learners}/4 available")
                
        except ImportError as e:
            logger.error(f"Failed to import learning module: {e}")
            logger.info("Learning components will be unavailable - system will operate in degraded mode")
            components['continual'] = None
            components['meta_cognitive'] = None
            components['compositional'] = None
        ...
        try:
            ...
            from vulcan.safety.safety_types import GovernanceOrchestrator, NSOAligner, ExplainabilityNode
            ...
            logger.warning("Safety components will be unavailable - system will operate WITHOUT safety validation")
        ...
        try:
            ...
            try:
                ...
                class MinimalMemoryConfig:
                    def __init__(self):
                        self.max_working_memory = getattr(self.config, 'max_working_memory', 20)
                        self.max_short_term = getattr(self.config, 'short_term_capacity', 1000)
                        self.max_long_term = getattr(self.config, 'long_term_capacity', 100000)
                        self.consolidation_interval = getattr(self.config, 'consolidation_interval', 1000)
        ...
        logger.info(f"Component loading complete: {available_components}/{total_components} components available")
        
        return components
    
    def _create_dependencies(self, components: Dict[str, Any]) -> EnhancedCollectiveDeps:
        """
        ...
        if world_model and not isinstance(world_model, MagicMock):
            meta_components = {
                'self_improvement_drive': getattr(world_model, 'self_improvement_drive', None),
                'motivational_introspection': getattr(world_model, 'motivational_introspection', None),
                'objective_hierarchy': getattr(world_model, 'objective_hierarchy', None),
                'objective_negotiator': getattr(world_model, 'objective_negotiator', None),
                'goal_conflict_detector': getattr(world_model, 'conflict_detector', None),
                ...
                'ethical_boundary_monitor': getattr(world_model, 'ethical_boundary_monitor', None),
                'curiosity_reward_shaper': getattr(world_model, 'curiosity_reward_shaper', None),
                ...
                'counterfactual_objectives': getattr(world_model, 'counterfactual_reasoner', None),
        ...
        deps = EnhancedCollectiveDeps(
            env=components.get('env'), # Pass 'env' if you load it in _import_components
    ...
    def _create_system_state(self):
        ...
        try:
            ...
            elif isinstance(safety_policies, dict) and 'names_to_versions' in safety_policies:
                policies = safety_policies['names_to_versions']
            else:
                policies = {}
    ...
    def _create_orchestrator(self, 
                            ...
                            sys: Any,
                            deps: EnhancedCollectiveDeps) -> VULCANAGICollective:
        """
    ...
    def step_with_monitoring(self, history: List[Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if self.collective.deps.goal_system and hasattr(self.collective.deps.goal_system, 'generate_plan'):
                    planned_actions = self.collective.deps.goal_system.generate_plan(context)
                else:
                    planned_actions = context
                
                # Try to enforce policies
                if self.collective.deps.governance and hasattr(self.collective.deps.governance, 'enforce_policies'):
            ...
            if self.unified_runtime:
                try:
                    ...
                    logger.warning(f"UnifiedRuntime execution failed: {e}, falling back to orchestrator")
            ...
            if (current_step > 0 and 
                current_step % checkpoint_interval == 0 and 
                current_step != self._last_checkpointed_step):
    ...
    def _execute_with_orchestrator(self, history: List[Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        # Get goal status
        try:
            goal_status = self.collective.deps.goal_system.get_goal_status() if self.collective.deps.goal_system else {}
        except Exception as e:
        ...
        # Get safety report
        try:
            safety_report = self.collective.deps.safety_validator.get_safety_report() if self.collective.deps.safety_validator else {}
        except Exception as e:
    ...
    def save_checkpoint(self, checkpoint_path: Optional[str] = None) -> bool:
        ...
        with self._checkpoint_lock:
            if checkpoint_path is None:
                ...
                checkpoint_path = str(self.checkpoint_dir / f"checkpoint_{timestamp}_step{self.collective.sys.step}.pkl")
    ...
    def _health_check(self) -> bool:
        ...
        try:
            ...
            if health.error_rate > error_rate_threshold:
                logger.warning(f"Error rate {health.error_rate:.3f} exceeds SLO {error_rate_threshold}")
                return False
            ...
            if pool_status['total_agents'] < self.collective.agent_pool.min_agents:
                logger.warning(f"Agent pool below minimum capacity: {pool_status['total_agents']}")
                return False
    ...
    def _auto_checkpoint(self):
        ...
        # FIXED: Guard against step 0 and duplicate checkpoints
        if current_step <= 0:
            logger.debug(f"Skipping auto-checkpoint at step {current_step} (step must be > 0)")
            return
    
        # FIXED: Check if we already checkpointed this step
        # Acquire lock with timeout
        if not self._checkpoint_lock.acquire(timeout=5.0):  # Add timeout to prevent test hang
            logger.warning("Timeout acquiring checkpoint lock, skipping auto-checkpoint")
            return
        
        try:
            if current_step == self._last_checkpointed_step:
                logger.debug(f"Skipping duplicate auto-checkpoint at step {current_step}")
                return
    ...
    def _cleanup_old_checkpoints(self):
        ...
        try:
            ...
            if len(auto_checkpoints) > max_checkpoints:
                ...
                logger.debug(f"Cleaning up {len(checkpoints_to_remove)} old checkpoints.")
                for checkpoint in checkpoints_to_remove:
                    try:
                        checkpoint.unlink(missing_ok=True)  # Non-blocking, skip if not exists/locked
                        # Also remove metadata file
                        metadata_file = checkpoint.with_name(checkpoint.stem + '_metadata.json')
                        metadata_file.unlink(missing_ok=True) # Use missing_ok here too
                        logger.debug(f"Removed old checkpoint: {checkpoint}")
                    except OSError as e: # Catch OS errors like permission denied more specifically
                        logger.warning(f"Failed to remove old checkpoint {checkpoint} due to OS error: {e}")
                    except Exception as e:
                        logger.warning(f"Failed to remove old checkpoint {checkpoint}: {e}")
            else:
                 logger.debug(f"No old checkpoints to clean up ({len(auto_checkpoints)} <= {max_checkpoints}).")
            ...
            for checkpoint_file in sorted(
                ...
                if metadata_file.exists():
                    try:
                        ...
                        logger.debug(f"Failed to load metadata for {checkpoint_file}: {e}")
