...
from persistant_memory_v46 import PackfileStore, MerkleLSM, GraphRAG, UnlearningEngine, ZKProver

from .base import (
    Memory, MemoryType, MemoryConfig, MemoryQuery,
    RetrievalResult, MemoryStats, BaseMemorySystem,
...
class HierarchicalMemory(BaseMemorySystem):
    ...
    def _extract_concepts(self, prompt: str, generated: str) -> List[str]:
        ...
        return [word for word in text.split() if word and word[0].isupper() and len(word) > 3][:5]
    ...
    def _extract_pattern(self, reasoning_trace: Dict[str, Any]) -> str:
        ...
        if 'chain' in reasoning_trace and reasoning_trace['chain']:
            return "Pattern: " + " -> ".join(step.get('type', 'step') for step in reasoning_trace['chain'])
        return "Generic Pattern"
    
    def _merge(self, recent: List[Any], relevant: List[Any], patterns: List[Any]) -> Dict[str, Any]:
    ...
    def retrieve_context_for_generation(self, query_tokens: str, max_tokens: int = 2048) -> Dict[str, Any]:
        """
        ...
        with self._lock:
            ...
            if recent_level:
                ...
                recent_memories = [recent_level.memories[mid] for mid in list(recent_level.access_queue)][-20:]
            else:
                recent_memories = []
        ...
        for pattern in patterns:
            similarity = self._compute_feature_similarity(query_embedding, pattern.feature_signature)
            if similarity > 0.6:
    ...
    def store_generation(self, prompt: str, generated: str, reasoning_trace: Dict[str, Any]):
    ...
    def store_tool_selection(self, 
                           ...
                           success: bool = True,
                           utility_score: float = 0.0) -> ToolSelectionRecord:
        ...
        record = ToolSelectionRecord(
            ...
            timestamp=time.time(),
            problem_features=problem_features.copy() if problem_features is not None else np.zeros(self.embedding_dimension),
        ...
        with self._lock:
            ...
            memory = Memory(
                ...
                importance=0.5 + (utility_score * 0.5),  # Higher utility = higher importance
                metadata={
    ...
    def retrieve_similar_problems(self, 
                                 ...
                                 success_only: bool = False) -> List[Tuple[ToolSelectionRecord, float]]:
    ...
    def get_problem_patterns(self, 
                            min_occurrences: int = 5,
                            min_success_rate: float = 0.6) -> List[ProblemPattern]:
        ...
        with self._lock:
            for pattern in self.problem_patterns.values():
                if (pattern.occurrence_count >= min_occurrences and 
    ...
    def find_matching_pattern(self, 
                            problem_features: np.ndarray,
                            threshold: float = 0.7) -> Optional[ProblemPattern]:
    ...
    def get_recommended_tools(self,
        ...
        recommendations = []
        tool_scores = defaultdict(lambda: {'count': 0, 'total_utility': 0, 'successes': 0})
        ...
        for tool, scores in tool_scores.items():
            if scores['count'] > 0:
                ...
                success_rate = scores['successes'] / scores['count'] if scores['count'] > 0 else 0
    ...
    def _update_problem_patterns(self, record: ToolSelectionRecord):
        ...
        with self._lock:
            ...
            if matched_pattern:
                ...
                # Update success rate
                success_count = sum(1 for rid in matched_pattern.examples[-100:]
                                  if self._get_record_success(rid))
                matched_pattern.success_rate = success_count / min(100, len(matched_pattern.examples))
                
                # Update average utility
                total_utility = sum(self._get_record_utility(rid) 
                                  for rid in matched_pattern.examples[-100:])
                matched_pattern.avg_utility = total_utility / min(100, len(matched_pattern.examples))
                ...
                if tool_counts:
                    ...
                    matched_pattern.typical_tools = [tool for tool, _ in sorted_tools[:5]]
            
            else:
                # Create new pattern if enough similar records
                similar_records = self._find_similar_records(record.problem_features, threshold=0.8)
                ...
                if len(similar_records) >= 3:
                    ...
                    avg_utility = sum(r.utility_score for r in similar_records) / len(similar_records)
    ...
    def _find_similar_records(self, features: np.ndarray, 
                             threshold: float = 0.8) -> List[ToolSelectionRecord]:
        """Find records with similar features."""
        similar = []
        
        for record in self.tool_selection_history:
            if record.problem_features is not None:
                similarity = self._compute_feature_similarity(features, record.problem_features)
    ...
    def get_tool_selection_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_utility = sum(r.utility_score for r in self.tool_selection_history) / total_selections
    ...
    def store(self, content: Any, 
        ...
        with self._lock:
            ...
            if memory.embedding is not None and len(memory.embedding) != self.embedding_dimension:
                logger.warning(f"Embedding dimension mismatch: {len(memory.embedding)} vs {self.embedding_dimension}, regenerating")
                memory.embedding = self._generate_embedding(content)
    ...
    def retrieve(self, query: MemoryQuery) -> RetrievalResult:
        ...
        with self._lock:
            ...
            if len(filtered) > 0 and len(scores) > 0 and (query.embedding is not None or query.content is not None):
                
                # Get the embedding used for search
                final_query_embedding = query.embedding if query.embedding is not None else (query_embedding if 'query_embedding' in locals() else None)
                
                memory_embeddings = [m.embedding for m in filtered if m.embedding is not None]
                if memory_embeddings and len(memory_embeddings) == len(filtered) and final_query_embedding is not None:
                    try:
                        ...
                        if attention_weights is not None and len(attention_weights) == len(scores):
                            scores = [s * a for s, a in zip(scores, attention_weights)]
                    except Exception as e:
                        # FIX: Handle attention mechanism errors gracefully
                        logger.warning(f"Attention mechanism failed: {e}, using raw scores")
            ...
            for level in self.levels.values():
                if memory_id in level.memories:
                    ...
                    if memory.type in self.stats.by_type:
                        self.stats.by_type[memory.type] = max(0, self.stats.by_type[memory.type] - 1)
    ...
    def _apply_filters(self, memories: List[Memory], 
                      query: MemoryQuery) -> List[Memory]:
        ...
        if 'content_contains' in query.filters:
            ...
            filtered = [
                m for m in filtered
                if search_text in str(m.content).lower()
            ]
    ...
    def _compute_similarities(self, memories: List[Memory], 
        ...
        for memory in memories:
            if memory.embedding is not None:
                ...
                if mem_dim != query_dim:
                    logger.warning(f"Dimension mismatch: query={query_dim}, memory={mem_dim}")
        ...
        if consolidation_interval > 0:
            def consolidation_loop():
                while not self._shutdown_event.is_set(): # CHANGED
                    if self._shutdown_event.wait(timeout=consolidation_interval): # CHANGED
