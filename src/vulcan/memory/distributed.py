...
class RPCClient:
    ...
    async def send_request_async(
        self, node_id: str, msg_type: str, data: Any
    ) -> Optional[Any]:
        """Send request asynchronously."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            self.executor, self.send_request, node_id, msg_type, data
        )
    ...
    def _handle_connection(self, conn: socket.socket):
        ...
        try:
            ...
            if msg_len > MAX_MESSAGE_SIZE:
                logger.error(
                    f"Message too large: {msg_len} bytes (max: {MAX_MESSAGE_SIZE})"
                )
                return
...
class MemoryFederation:
    ...
    def elect_leader(self) -> Optional[str]:
        ...
        active_nodes = [nid for nid, node in self.nodes.items() if node.is_healthy()]
    ...
    def _migrate_from_node(self, node_id: str):
        ...
        for memory_id in node.primary_for:
            ...
            new_nodes = [item for item in new_nodes if item != node_id]
        ...
        def monitor_loop():
            while self.monitor_running:
                ...
                with self.lock:
                    ...
                    if self.leader_id:
                        if (
                            self.leader_id not in self.nodes
                            or not self.nodes[self.leader_id].is_healthy()
                        ):
                            self.elect_leader()
...
class DistributedMemory(BaseMemorySystem):
    ...
    def __init__(
        ...
        if REDIS_AVAILABLE:
            try:
                ...
                logger.warning(
                    "Redis server unavailable; operating in local-only/distributed mode."
                )
        ...
        self_node = MemoryNode(
            node_id=self.node_id, host=host, port=port, capacity=config.max_long_term
        )
    ...
    def store(self, content: Any, **kwargs) -> Memory:
        ...
        target_nodes = self.federation.get_nodes_for_key(
            memory.id, count=self.config.replication_factor
        )

        # FIX: Always store locally (either as target or fallback for single node)
        if (
            not target_nodes
            or len(self.federation.nodes) <= 1
            or self.node_id in target_nodes
        ):
        ...
        if target_nodes and len(target_nodes) > 1:
            ...
            if success_count < required:
                raise Exception(
                    f"Failed to achieve required consistency: {success_count}/{required}"
                )
    ...
    def retrieve(self, query: MemoryQuery) -> RetrievalResult:
        ...
        if (
            ...
            or len(local_results) < query.limit
        ):
            # Query other nodes
            ...
            for mem in memories:
                if mem.metadata.get("encrypted") and isinstance(mem.content, bytes):
                    try:
                        ...
                        logger.warning(
                            f"Failed to decrypt memory {mem.id}, returning encrypted content. Error: {e}"
                        )
    ...
    def _delete_from_node(self, memory_id: str, node_id: str) -> bool:
        ...
        try:
            result = self.rpc_client.send_request(
                node_id, "delete", {"memory_id": memory_id}
            )

            if result and result.get("success"):
                logger.debug(
                    f"Successfully deleted memory {memory_id} from node {node_id}"
                )
                return True
            else:
                logger.warning(
                    f"Failed to delete memory {memory_id} from node {node_id}"
                )
                return False
            ...
            for node_id, node in self.federation.nodes.items():
                if node_id != self.node_id and node.is_healthy():
                    future = executor.submit(
                        self.rpc_client.send_request, node_id, "search", query_data
                    )
    ...
    def _merge_results(
        self, local: List[Tuple[Memory, float]], remote: List[Tuple[Memory, float]]
    ...
    def _rebalance_data(self) -> int:
        ...
        total_memories = sum(
            node.memory_count for node in self.federation.nodes.values()
        )
        ...
        for memory_id, replica_nodes in list(self.replicas.items()):
            ...
            if len(healthy_replicas) < self.config.replication_factor:
                ...
                new_nodes = self.federation.get_nodes_for_key(
                    memory_id, count=self.config.replication_factor
                )
                    ...
                    for node_id in new_nodes:
                        if node_id not in healthy_replicas and node_id != self.node_id:
                            ...
                            if self._send_to_node(
                                self.local_storage[memory_id], node_id
                            ):
                                healthy_replicas.add(node_id)
                                cleaned += 1
