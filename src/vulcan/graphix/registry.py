...
from vulcan.graphix.dialects import CANONICAL_DIALECTS
...
class DialectRegistry:
    ...
    def migrate_to(
        self, envelope: GraphixEnvelope, target_version: int
    ) -> GraphixEnvelope:
        reg = self.require_supported(envelope)
        if target_version == envelope.schema_version:
            return envelope
        if (
            target_version not in reg.compatible_versions
            or target_version not in reg.migrations
        ):
            raise UnsupportedDialectError(
                "no explicit compatible migration for target version"
            )
        migrated = reg.migrations[target_version](envelope)
        if (
            migrated.dialect != envelope.dialect
            or migrated.schema_version != target_version
        ):
            raise GraphixCoreError("migration returned wrong dialect or schema version")
        return migrated


def canonical_runtime_registry(*, release_id: str) -> DialectRegistry:
    """Construct the immutable code-owned registry used by production Graphix."""
    registry = DialectRegistry(release_id=release_id)
    for dialect in sorted(CANONICAL_DIALECTS):
        registry.register(DialectRegistration(dialect, 1, release_id))
    registry.freeze()
    return registry
