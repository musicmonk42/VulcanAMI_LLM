    ...
    TOOL_SELECTION_SCHEMA = {
        ...
        "schema": {
            "default_selection_mode": {
                "type": "string",
                "allowed": [m.value for m in SelectionMode],
            },
...
class ConfigValidator:
    ...
    def __init__(
        self, validation_level: ConfigValidationLevel = ConfigValidationLevel.STRICT
    ):
    ...
    def validate(
        self, config: Dict[str, Any], schema: Dict[str, Any] = None
    ...
    def _validate_business_logic(self, config: Dict[str, Any]):
        ...
        if safety.get("safety_level", 0) >= 3 and not safety.get(
        ...
        if profile == "production" and not config.get("agent_config", {}).get(
        ...
        if (
            tool_selection.get("enable_portfolio", False)
            and tool_selection.get("max_parallel_tools", 1) < 2
        ):
            self.validation_warnings.append(
        ...
        if intrinsic_drives.get("enabled", False):
            if intrinsic_drives.get(
                "max_cost_usd_per_session", 0
            ) > intrinsic_drives.get("max_cost_usd_per_day", 0):
                self.validation_warnings.append(
...
class AgentConfig:
    ...
    def __post_init__(self):
        ...
        if not self.collective_id:
            self.collective_id = get_config(
                "agent_config.collective_id", "COLLECTIVE-001"
            )
        if not self.version:
    ...
    def __post_init__(self):
        ...
        if self.safety_level is None:
            self.safety_level = SafetyLevel(
                get_config("safety_policies.safety_level", 1)
            )
        if not self.require_human_approval:
            self.require_human_approval = get_config(
                "safety_policies.require_human_approval", False
            )
        if self.max_autonomy_level == 0:
            self.max_autonomy_level = get_config(
                "safety_policies.max_autonomy_level", 5
            )
        if self.rollback_threshold == 0.0:
            self.rollback_threshold = get_config(
                "safety_policies.rollback_threshold", 0.3
            )
        ...
        if not self.names_to_versions:
            self.names_to_versions = get_config(
                "safety_policies.names_to_versions",
                {"ITU_F748_53": "1.0", "safety_validator": "1.0", "governance": "1.0"},
            )
...
class ToolSelectionConfig:
    ...
    def __post_init__(self):
        ...
        if self.default_selection_mode is None:
            mode_str = get_config(
                "tool_selection_config.default_selection_mode", "balanced"
            )
            self.default_selection_mode = SelectionMode(mode_str)
        if self.confidence_threshold == 0.0:
            self.confidence_threshold = get_config(
                "tool_selection_config.confidence_threshold", 0.5
            )
        if self.veto_threshold == 0.0:
            self.veto_threshold = get_config(
                "tool_selection_config.veto_threshold", 0.8
            )
        if self.max_reasoning_time == 0.0:
            self.max_reasoning_time = get_config(
                "tool_selection_config.max_reasoning_time", 30.0
            )
        if self.max_parallel_tools == 0:
            self.max_parallel_tools = get_config(
                "tool_selection_config.max_parallel_tools", 4
            )
...
class IntrinsicDrivesConfig:
    ...
    def __post_init__(self):
        ...
        if not self.state_file:
            self.state_file = get_config(
                "intrinsic_drives_config.state_file", "data/agent_state.json"
            )
        if self.approval_required:
            self.approval_required = get_config(
                "intrinsic_drives_config.approval_required", True
            )
        if self.load_on_startup:
            self.load_on_startup = get_config(
                "intrinsic_drives_config.load_on_startup", True
            )
        if self.check_interval_seconds == 60:
            self.check_interval_seconds = get_config(
                "intrinsic_drives_config.check_interval_seconds", 60
            )
        if self.max_cost_usd_per_day == 20.0:
            self.max_cost_usd_per_day = get_config(
                "intrinsic_drives_config.max_cost_usd_per_day", 20.0
            )
        if self.max_cost_usd_per_session == 5.0:
            self.max_cost_usd_per_session = get_config(
                "intrinsic_drives_config.max_cost_usd_per_session", 5.0
            )
...
class WorldModelConfig:
    ...
    def __post_init__(self):
        ...
        if not self.enable_meta_reasoning:
            self.enable_meta_reasoning = get_config(
                "world_model.enable_meta_reasoning", False
            )
        if not self.enable_self_improvement:
            self.enable_self_improvement = get_config(
                "world_model.enable_self_improvement", False
            )
        if not self.meta_reasoning_config:
            self.meta_reasoning_config = get_config(
                "world_model.meta_reasoning_config", "configs/intrinsic_drives.json"
            )
        if not self.self_improvement_config:
            self.self_improvement_config = get_config(
                "world_model.self_improvement_config", "configs/intrinsic_drives.json"
            )
        if not self.improvement_state:
            self.improvement_state = get_config(
                "world_model.self_improvement_state", "data/agent_state.json"
            )
...
class HierarchicalGoalSystem:
    ...
    def decompose_goal(
        ...
        return [
            ...
            {"subgoal": f"execute_{goal}", "priority": 0.9, "estimated_cost": 0.7},
        ]
    ...
    def get_goal_status(self) -> Dict[str, Any]:
        ...
        return {"active_goals": 0, "completed_goals": 0, "failed_goals": 0}
...
class ConfigurationManager:
    ...
    def _setup_file_watcher(self, file_path: Path, layer: ConfigLayer):
        ...
        try:
            ...
            watch = self.observer.schedule(
                event_handler, str(file_path.parent), recursive=False
            )
    ...
    def load_from_file(
        self, file_path: Union[str, Path], layer: ConfigLayer = ConfigLayer.FILE
    ) -> bool:
    ...
    def load_from_environment(self, prefix: str = "VULCAN_") -> int:
        ...
        self._set_nested_config(
            env_config,
            "distributed.enabled",
            os.getenv("VULCAN_DISTRIBUTED_ENABLED", False),
        )
    ...
    def _create_default_profile(self, profile: ProfileType):
        ...
        profile_configs = {
            ProfileType.DEVELOPMENT: {
                ...
                "safety_policies": {"safety_level": SafetyLevel.MINIMAL.value},
            ...
            ProfileType.TESTING: {
                ...
                "resource_limits": {"max_memory_mb": 4000, "max_cpu_percent": 50.0},
                "tool_selection_config": {
            ...
            ProfileType.RESEARCH: {
                ...
                "learning_config": {"enable_meta_learning": True},
                "tool_selection_config": {
                    "default_selection_mode": SelectionMode.BALANCED.value,
                    "enable_voi": True,
                    "utility_weights": {"exploration_bonus": 0.3},
                },
                "intrinsic_drives_config": {
                    "enabled": True,
                    "approval_required": False,
            ...
            ProfileType.ENERGY_SAVING: {
                "resource_limits": {"energy_budget_nj": 1e8, "max_cpu_percent": 50.0},
                "processing_config": {
                ...
                "tool_selection_config": {
                    "default_selection_mode": SelectionMode.EFFICIENT.value,
                    "utility_weights": {"energy_penalty": 2.0},
                    "enable_warm_start": False,
                },
                "intrinsic_drives_config": {"enabled": False},
            },
    ...
    def set_runtime_override(
        self, key: str, value: Any, admin_token: str = None
    ) -> bool:
    ...
    def validate(self) -> Tuple[bool, List[str], List[str]]:
        ...
        with self.lock:
            ...
            is_valid, errors, warnings = self.validator.validate(
                self.current_config.get("agent_config", {}), ConfigSchema.AGENT_SCHEMA
            )
            ...
            self.metadata["validation_status"] = (
                "valid" if len(all_errors) == 0 else "invalid"
            )

            return len(all_errors) == 0, all_errors, all_warnings

    def export(
        self, file_path: Union[str, Path], include_metadata: bool = True
    ) -> bool:
        ...
        try:
            with self.lock:
                export_data = {"configuration": self.current_config}
...
def _dict_to_agent_config(raw: dict) -> AgentConfig:
    ...
    return AgentConfig(
        **filtered_cfg,
        ...
        or raw.get("agent_config", {}).get("enable_self_improvement", False),
        intrinsic_drives_config_file=intrinsic_drives.get(
            "config_file", "configs/intrinsic_drives.json"
        ),
        intrinsic_drives_state_file=intrinsic_drives.get(
            "state_file", "data/agent_state.json"
        ),
    )
...
def get_utility_weights() -> Dict[str, float]:
    ...
    return get_config(
        "tool_selection_config.utility_weights",
        {
            "quality": 1.0,
            "time_penalty": 1.0,
            "energy_penalty": 0.5,
...
def _validate_dependencies():
    ...
    if missing:
        ...
        config_manager.set_runtime_override(
            "degraded_features", unique_degraded_features
        )

        logger.warning(
            f"Missing optional dependencies: {list(set(missing))}. "
...
class ConfigurationAPI:
    ...
    async def set_config(
        ...
        return {"success": success, "key": key, "value": value if success else None}

    async def delete_config(self, key: str, admin_token: str = None) -> Dict[str, Any]:
...
def initialize_config(
    ...
    if validate:
        ...
        if not is_valid:
            ...
            if (
                config_manager.validator.validation_level.value
                >= ConfigValidationLevel.STRICT.value
            ):
                raise ValueError(
