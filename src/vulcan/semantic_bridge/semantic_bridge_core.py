            ...
            for cid, other in self.concepts.items():
                if cid != concept.concept_id:
                    if hasattr(concept, "domains") and hasattr(other, "domains"):
                        overlap = len(concept.domains & other.domains) / max(
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt < max_attempts - 1:
                        logger.debug(
                            "Attempt %d failed for %s, retrying: %s",
                            attempt + 1,
                            func.__name__,
                            e,
                        )
...
class SemanticBridge:
    ...
    def __init__(
        ...
        self.cache_manager.register_cache(
            "pattern_signature", self.pattern_signature_cache, priority=7
        )
        self.cache_manager.register_cache(
            "domain_concept", self.domain_concept_cache, priority=9
        )

        # Register component caches
        if hasattr(self.domain_registry, "distance_cache"):
            self.cache_manager.register_cache(
                "domain_distance", self.domain_registry.distance_cache, priority=6
            )
        ...
        logger.info(
            "SemanticBridge initialized (production-ready) with bounded data structures"
        )
    ...
    def _evict_oldest_index_domain(self):
        ...
        min_domain = min(
            self.domain_concept_index.keys(),
    ...
    def learn_concept_from_pattern(
        self, pattern, outcomes: List[PatternOutcome]
    ) -> Optional[Concept]:
        """
        ...
        if self.safety_validator:
            ...
            if not sig_validation["safe"]:
                logger.warning(
                    "Unsafe signature detected: %s", sig_validation["reason"]
                )
        ...
        if "map" in operations:
            ...
            with self._concept_lock:
                concept = self.concept_mapper.map_pattern_to_concept(
                    pattern, list(domains)[0] if domains else "general"
                )
        ...
        if "update_metrics" in operations:
            ...
            with self._concept_lock:
                ...
                for key in concept.features:
                    if (
                        isinstance(concept.features[key], list)
                        and concept.features[key]
                    ):
                        concept.features[key] = np.mean(concept.features[key])

        if "version" in operations:
            # Create version
            self._create_concept_version(
                concept, changes=["Initial creation from pattern"]
            )
        ...
        logger.info(
            "Learned concept from pattern (ops: %s, success_rate: %.2f, safety_filtered: %d)",
            operations,
            signature.success_rate,
            len(outcomes) - len(safe_outcomes),
        )
    ...
    def _link_concept_to_world_model(
        self, concept: Concept, outcomes: List[PatternOutcome]
    ):
        """
            ...
            with open(history_file, "a", encoding="utf-8") as f:
                for entry in list(self.operation_history):
                    ...
                    serialized = {
                        ...
                        "filtered_outcomes_count": entry.get(
                            "filtered_outcomes_count", 0
                        ),
                    }
                    f.write(json.dumps(serialized) + "\n")

            logger.debug(
                "Persisted %d operation history entries", len(self.operation_history)
            )
    ...
    def _load_operation_history(self):
        ...
        try:
            ...
            logger.info(
                "Loaded %d operation history entries", len(self.operation_history)
            )
        ...
        with self._concept_lock:
            ...
            for concept_id, concept in self.concept_mapper.concepts.items():
                if hasattr(concept, "domains"):
                    domains = (
                        concept.domains
                        if isinstance(concept.domains, (set, list))
                    ...
                    for domain in domains:
                        ...
                        if (
                            len(self.domain_concept_index[domain])
                            < self.max_concepts_per_domain
                        ):
                            self.domain_concept_index[domain].add(concept_id)

            self.index_dirty = False
            logger.debug(
                "Rebuilt domain-concept index with %d domains",
            ...
            try:
                ...
                if concept_vars and hasattr(
                    ...
                    insights["causal_depth"] = max(
                        [len(p.nodes) if hasattr(p, "nodes") else 0 for p in paths],
                        default=0,
                    )
    ...
    def _extract_pattern_signature(
        ...
        is_significant_change = (
            success_rate > 0.7
            or success_rate < 0.3  # Very good or very bad
            or len(outcomes) >= 10  # Enough evidence
            or len(domains) > 2  # Multi-domain
        )
        ...
        signature = PatternSignature(
            ...
            metadata={"domains": list(domains), "pattern_type": type(pattern).__name__},
        )
        ...
        if len(self.pattern_signature_cache) < self.max_pattern_cache_size:
            ...
            # Evict oldest entry
            oldest_key = min(
                self.pattern_signature_cache.keys(),
    ...
    def _select_operations(
        self, signature: PatternSignature, pattern: Any, outcomes: List[PatternOutcome]
    ) -> Set[str]:
        """
        ...
        # RULE 2: Skip mapping if not novel and no significant change
            ...
            logger.debug(
                "Existing pattern with no significant change: skipping all operations"
            )
        ...
        if signature.evidence_count < 3 and signature.confidence < 0.5:
            logger.debug(
                "Insufficient evidence and confidence: skipping all operations"
            )
        ...
        if (
            self.world_model
            and signature.is_significant_change
            and signature.confidence > 0.6
        ):
            operations.add("world_model_link")
            logger.debug(
                "Significant pattern with world model: linking to causal graph"
            )

        return operations

    def _estimate_pattern_complexity(
        ...
        if hasattr(pattern, "features"):
            feature_count = (
                len(pattern.features) if hasattr(pattern.features, "__len__") else 0
            )
    ...
    def _validate_signature_safety(self, signature: PatternSignature) -> Dict[str, Any]:
        ...
        if signature.pattern_complexity < 0 or signature.pattern_complexity > 1:
            violations.append(
                f"Invalid pattern complexity: {signature.pattern_complexity}"
            )
    ...
    def select_transfer_strategy(self, pattern: Any, target_domain: str) -> str:
        ...
        if hasattr(pattern, "source_domain"):
            ...
            source_domain = (
                list(pattern.domains)[0]
                if isinstance(pattern.domains, (set, list))
                else pattern.domains
            )
        ...
        if domain_similarity > 0.8:
            ...
            logger.debug(
                "Selected direct_transfer strategy (similarity: %.2f)",
                domain_similarity,
            )
        elif is_structural:
    ...
    def validate_transfer_compatibility(
        self, concept: Concept, source: str, target: str
        ...
        if (
            source_chars.get("complexity") == "low"
            and target_chars.get("complexity") == "very_high"
        ):
        ...
        logger.debug(
            "Validated transfer compatibility for concept: %.2f",
    ...
    def transfer_concept(
        ...
        """
        logger.info("Transferring concept from %s to %s", source_domain, target_domain)
        ...
        with self._concept_lock:
            ...
            for concept in all_concepts[:50]:  # Limit to top 50
                if hasattr(concept, "domains") and domain not in concept.domains:
                    if (
                        hasattr(concept, "confidence")
                        and concept.confidence >= min_confidence - 0.1
                    ):
                        ...
                        for source_domain in concept.domains:
                            ...
                            if (
                                ...
                                and compatibility.confidence >= min_confidence
                            ):
                                ...
                                transfer_decision = (
                                    self.transfer_engine.validate_full_transfer(
                                        concept, source_domain, domain
                                    )
                                )
            ...
            if len(self.domain_concept_cache) < self.max_domain_cache_size:
                ...
                # Evict oldest entry
                oldest_key = min(
                    self.domain_concept_cache.keys(),
            ...
            logger.debug(
                "Returning %d applicable concepts for domain %s",
                len(applicable),
                domain,
            )
    ...
    def _detect_conflict(
        self, new_concept: Concept, existing_concept: Concept
    ) -> Optional[ConceptConflict]:
        """Detect conflict between concepts"""
        # Calculate similarity
        similar_concepts = self.concept_mapper.find_similar_concepts(
            new_concept, top_k=10
        )
    ...
    def _apply_resolution(
        ...
        with self._concept_lock:
            if action == "replace":
                ...
                existing_id = getattr(
                    existing_concept, "concept_id", str(existing_concept)
                )
                ...
                # Merge concepts
                if hasattr(existing_concept, "features") and hasattr(
                    new_concept, "features"
                ):
                    existing_concept.features.update(new_concept.features)
                if hasattr(existing_concept, "domains") and hasattr(
                    new_concept, "domains"
                ):
                    existing_concept.domains.update(new_concept.domains)
                if hasattr(existing_concept, "confidence") and hasattr(
                    new_concept, "confidence"
                ):
                    existing_concept.confidence = (
                        existing_concept.confidence + new_concept.confidence
                    ) / 2
                self._create_concept_version(
                    existing_concept, ["Merged with new concept"]
                )
                self.index_dirty = True  # Mark index as needing rebuild
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "index_total_concepts": sum(
                len(s) for s in self.domain_concept_index.values()
            ),
