...
class TransferEngine:
    ...
    def __init__(
        ...
        if safety_validator is not None:
            ...
            logger.warning(
                "TransferEngine: Safety validator not available - operating without safety checks"
            )
        ...
        logger.info(
            "TransferEngine initialized (production-ready) with bounded data structures"
        )
    ...
    def _evict_least_used_effect(self):
        ...
        min_effect_id = min(
            self.effect_library.keys(), key=lambda k: self.effect_library[k].importance
        )
    ...
    def _evict_least_used_domain(self):
        ...
        min_domain = min(
            self.domain_characteristics.keys(),
    ...
    def calculate_effect_overlap(self, concept, target_domain: str) -> float:
        ...
        with self._lock:
            ...
            domain_chars = self.domain_characteristics.get(
                target_domain, DomainCharacteristics(name=target_domain)
            )
            ...
            logger.debug(
                "Effect overlap for concept in domain %s: %.2f", target_domain, overlap
            )

            return overlap

    def validate_full_transfer(
        self, concept, source: str, target: str
    ) -> TransferDecision:
        """
        ...
        with self._lock:
                ...
                try:
                    if hasattr(self.safety_validator, "validate_transfer"):
                        ...
                        if not transfer_check.get("safe", True):
                            logger.warning(
                                "Unsafe transfer blocked: %s",
                                transfer_check.get("reason", "unknown"),
                            )
                            ...
                            return TransferDecision(
                                ...
                                confidence=0.0,
                                reasoning=[
                                    f"Safety: {transfer_check.get('reason', 'unsafe transfer')}"
                                ],
                            )
                except Exception as e:
                    logger.debug("Error validating transfer: %s", e)

            decision = TransferDecision(type=TransferType.FULL, confidence=1.0)
            ...
            if overlap < self.full_transfer_threshold:
                ...
                decision.reasoning.append(
                    f"Effect overlap {overlap:.2f} below threshold {self.full_transfer_threshold}"
                )
                return decision
            ...
            if compatibility < 0.7:
                ...
                decision.reasoning.append(
                    f"Domain compatibility reduced confidence: {compatibility:.2f}"
                )
            ...
            if not self._verify_prerequisites(concept, target):
                ...
                decision.reasoning.append(
                    "Prerequisites not fully met - conditional transfer"
                )

            # FIXED: Query world model for causal dependencies
            if self.world_model:
                try:
                    self._check_world_model_constraints(
                        concept, source, target, decision
                    )
                except Exception as e:
                    logger.debug("Failed to check world model constraints: %s", e)

            decision.reasoning.append(
                f"Full transfer validated with {len(constraints)} constraints"
            )

            return decision

    def _check_world_model_constraints(
        self, concept, source: str, target: str, decision: TransferDecision
    ):
        """
        ...
        try:
            ...
            if self.world_model.causal_graph.has_node(
                source_node
            ) and self.world_model.causal_graph.has_node(target_node):
                ...
                logger.debug(
                    "Applied world model constraints for transfer %s -> %s",
                    source,
                    target,
                )
        except Exception as e:
    ...
    def validate_partial_transfer(
        ...
        with self._lock:
            ...
            if self.safety_validator:
                try:
                    if hasattr(self.safety_validator, "validate_transfer"):
                        ...
                        if not transfer_check.get("safe", True):
                            logger.warning(
                                "Unsafe partial transfer blocked: %s",
                                transfer_check.get("reason", "unknown"),
                            )
            ...
            decision = TransferDecision(type=TransferType.PARTIAL, confidence=0.5)
            ...
            if overlap < self.partial_transfer_threshold:
                ...
                decision.confidence = overlap
                decision.reasoning.append(
                    f"Effect overlap {overlap:.2f} below partial threshold"
                )
                return decision

            # Use partial transfer engine
            missing_effects = self.partial_engine.identify_missing_effects(
                concept, target
            )
            ...
            if missing_effects:
                ...
                mitigation_confidence = self._calculate_mitigation_confidence(
                    mitigations
                )
                decision.confidence = overlap * mitigation_confidence

                decision.reasoning.append(
                    f"Partial transfer with {len(mitigations)} mitigations"
                )
            else:
                ...
                decision.confidence = overlap
                decision.reasoning.append(
                    "No missing effects - upgraded to full transfer"
                )
    ...
    def execute_transfer(
        self, concept, decision: TransferDecision, target_domain: str
    ) -> Dict[str, Any]:
        """
        ...
        with self._lock:
                ...
                try:
                    if hasattr(self.safety_validator, "validate_concept"):
                        ...
                        if not concept_check.get("safe", True):
                            logger.warning(
                                "Unsafe concept detected before transfer: %s",
                                concept_check.get("reason", "unknown"),
                            )
                            self.safety_blocks["unsafe_concept_transfer"] += 1
                            result["warnings"].append(
                                f"Safety: {concept_check.get('reason', 'unsafe concept')}"
                            )
                            return result
                except Exception as e:
            ...
            for mitigation in decision.mitigations:
                ...
                if success:
                    ...
                    result["warnings"].append(
                        f"Failed to apply mitigation: {mitigation.mitigation_id}"
                    )
            ...
            if self.world_model:
                try:
                    self._update_world_model_for_transfer(
                        concept, transferred, target_domain, decision
                    )
                except Exception as e:
    ...
    def rollback_transfer(
        self, transferred_concept, original_concept, target_domain: str
    ) -> bool:
        """
        ...
        with self._lock:
            try:
                ...
                self.transfer_history.append(
                    {
                        "timestamp": time.time(),
                        "concept_id": getattr(
                            original_concept, "concept_id", str(original_concept)
                        ),
                ...
                if self.world_model:
                    try:
                        self._remove_transfer_from_world_model(
                            transferred_concept, target_domain
                        )
                    except Exception as e:
                        logger.debug(
                            "Failed to remove transfer from world model: %s", e
                        )
    ...
    def _update_world_model_for_transfer(
        ...
        try:
            ...
            transferred_id = getattr(
                transferred_concept, "concept_id", str(transferred_concept)
            )
            ...
            if original_id != transferred_id:
                if not self.world_model.causal_graph.has_edge(
                    original_id, transferred_id
                ):
                    self.world_model.causal_graph.add_edge(
                        original_id,
                        transferred_id,
    ...
    def _extract_concept_effects(self, concept) -> List[ConceptEffect]:
        ...
        if hasattr(concept, "grounded_effects") and concept.grounded_effects:
            for grounded_effect in concept.grounded_effects:
                effect = ConceptEffect(
                    effect_id=getattr(
                        grounded_effect, "effect_id", f"effect_{len(effects)}"
                    ),
                    effect_type=self._map_grounded_to_concept_effect_type(
                        grounded_effect
                    ),
        ...
        if hasattr(concept, "features") and concept.features:
            for feature_name, feature_value in concept.features.items():
                ...
                importance = 0.5
                if (
                    "critical" in feature_name.lower()
                    or "primary" in feature_name.lower()
                ):
                    importance = 0.9
        ...
        logger.warning(
            "No effects found for concept %s, creating default",
    ...
    def _is_effect_supported(
        self, effect: ConceptEffect, domain_chars: DomainCharacteristics
    ) -> bool:
    ...
    def _is_effect_compatible(
        ...
        for limitation in domain_chars.limitations:
            if (
                "incompatible" in limitation.lower()
                and effect.description.lower() in limitation.lower()
            ):
                return False
    ...
    def _assess_transfer_risk(
        ...
        if hasattr(concept, "complexity"):
            risks["complexity"] = (
                min(1.0, concept.complexity / 10) if concept.complexity > 0 else 0
            )
    ...
    def _apply_mitigation(
        self, concept, mitigation: Mitigation, target_domain: str
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to apply mitigation %s: %s", mitigation.mitigation_id, e
            )
            return False
    ...
    def _record_transfer(
        self, concept, decision: TransferDecision, target_domain: str, success: bool
    ):
...
class MitigationLearner:
    ...
    def record_mitigation_outcome(
        ...
        with self._lock:
            ...
            if (
                len(self.context_performance[key][context_key])
            ...
            self.context_performance[key][context_key].append(
                {"success": success, "metrics": metrics, "timestamp": time.time()}
            )

    def get_mitigation_confidence(
        self,
        mitigation_type: MitigationType,
        target_effect: str,
        ...
        with self._lock:
            ...
            if context:
                context_key = self._context_to_key(context)
                context_outcomes = self.context_performance.get(key, {}).get(
                    context_key, []
                )
    ...
    def suggest_best_mitigation(
        self, target_effect: str, context: Dict[str, Any]
    ) -> Optional[MitigationType]:
        """
        ...
        with self._lock:
            ...
            for mitigation_type in MitigationType:
                confidence = self.get_mitigation_confidence(
                    mitigation_type, target_effect, context
                )
...
class PartialTransferEngine:
    ...
    def identify_missing_effects(
        self, concept, target_domain: str
    ) -> List[ConceptEffect]:
        """
        ...
        logger.debug(
            "Identified %d missing effects for domain %s", len(missing), target_domain
        )

        return missing

    def generate_mitigations(
        self, missing_effects: List[ConceptEffect]
    ) -> List[Mitigation]:
        """
    ...
    def _create_mitigation(
        self, effect: ConceptEffect, mitigation_type: MitigationType
    ) -> Mitigation:
    ...
    def _generate_implementation(
        ...
        if mitigation_type == MitigationType.ADAPTATION:
            ...
            implementation["fallback"] = {"strategy": "default", "timeout": 30}
