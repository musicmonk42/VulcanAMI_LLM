...
try:
    ...
    logging.warning("safety_validator not available, concept_mapper operating without safety checks")
...
class MeasurableEffect:
    ...
    def is_consistent_with(self, other: 'MeasurableEffect', tolerance: float = 0.2) -> bool:
...
class Concept:
    ...
    def __init__(self, pattern_signature: str, grounded_effects: List[MeasurableEffect], 
                 confidence: float = 0.5):
        """
        Initialize concept
        ...
        logger.debug("Created concept %s with %d grounded effects", 
                    self.concept_id, len(grounded_effects))
    ...
    def update_evidence(self, new_outcomes: List[PatternOutcome]):
        ...
        logger.debug("Updated concept %s with %d new outcomes (success_rate: %.2f)",
                    self.concept_id, len(new_outcomes), self.success_rate)
    ...
    def calculate_stability_score(self) -> float:
        ...
        if self.evidence_history:
            recent_scores = [e['support_score'] for e in list(self.evidence_history)[-10:]]
            if recent_scores:
    ...
    def _update_confidence(self):
        ...
        if self.evidence_history:
            ...
            recent_success = sum(1 for e in recent_evidence if e['success']) / len(recent_evidence)
...
class ConceptMapper:
    ...
    def __init__(self, world_model=None, domain_registry=None, safety_config: Optional[Dict[str, Any]] = None):
        ...
        if SAFETY_VALIDATOR_AVAILABLE:
            ...
            logger.warning("ConceptMapper: Safety validator not available - operating without safety checks")
        ...
        logger.info("ConceptMapper initialized (production-ready) with bounded data structures")
        ...
        if hasattr(self, 'domain_registry') and self.domain_registry:
            ...
            if profile:
                ...
                return {
                    ...
                    'consistency': self.base_consistency_threshold + (criticality * 0.2),
                    'grounding_confidence': self.base_grounding_confidence + (criticality * 0.3)
                }
    ...
    def decay_unused_concepts(self, max_age_days: int = 30, min_usage: int = 5) -> List[str]:
        """
        ...
        with self._lock:
            ...
            for concept_id, concept in list(self.concepts.items()):
                ...
                if age > max_age_seconds and concept.usage_count < min_usage:
                    ...
                    logger.info("Removed unused concept %s (age: %.1f days, usage: %d)",
                               concept_id, age / (24*3600), concept.usage_count)
    ...
    def _archive_concept(self, concept: Concept):
        ...
        if len(self.archived_concepts) >= self.max_archived:
            ...
            oldest_key = min(self.archived_concepts.keys(), 
    ...
    def set_effect_category_override(self, measure_pattern: str, effect_type: EffectType):
        """
        ...
        logger.info("Added category override: %s -> %s", measure_pattern, effect_type.value)
    ...
    def map_pattern_to_concept(self, pattern, domain: str = "general") -> Concept:
        ...
        with self._lock:
            ...
            logger.debug("Mapped pattern to concept %s in domain %s", concept.concept_id, domain)
    ...
    def _evict_least_valuable_concept(self):
        ...
        for concept_id, concept in self.concepts.items():
            ...
            value_score = (concept.usage_count * 
    ...
    def find_similar_concepts(self, concept: Concept, top_k: int = 5) -> List[Tuple[Concept, float]]:
        """
            ...
            for other_sig, other in self.concepts.items():
                ...
                if hasattr(concept, 'domains') and hasattr(other, 'domains'):
                    if concept.domains and other.domains:
                        domain_overlap = len(concept.domains & other.domains) / max(1, len(concept.domains | other.domains))
                        similarity += domain_overlap * 0.3
                
                # Effect type overlap
                concept_effect_types = set(e.effect_type for e in concept.grounded_effects)
                other_effect_types = set(e.effect_type for e in other.grounded_effects)
                if concept_effect_types and other_effect_types:
                    effect_overlap = len(concept_effect_types & other_effect_types) / max(1, len(concept_effect_types | other_effect_types))
                    similarity += effect_overlap * 0.4
                
                # Success rate similarity
                if hasattr(concept, 'success_rate') and hasattr(other, 'success_rate'):
                    success_similarity = 1.0 - abs(concept.success_rate - other.success_rate)
    ...
    def extract_measurable_effects(self, outcomes: List[PatternOutcome]) -> List[MeasurableEffect]:
        """
            ...
            for outcome in outcomes:
                try:
                    ...
                    if outcome_check.get('safe', True):
                        ...
                        logger.debug("Filtered unsafe outcome: %s", outcome_check.get('reason', 'unknown'))
        ...
        for measure_name, measurements in effect_groups.items():
            if len(measurements) >= min_instances:
                ...
                if not np.isfinite(mean_value) or not np.isfinite(std_value):
                    logger.debug("Skipping effect with non-finite statistics: %s", measure_name)
                ...
                if std_value < mean_value * 0.3 or std_value < 0.1:  # Consistent enough
                    ...
                    if len(self.effect_library) >= self.max_effects:
                        ...
                        oldest_key = min(self.effect_library.keys(),
        ...
        logger.debug("Extracted %d measurable effects from %d outcomes", 
                    len(effects), len(outcomes))
        
        return effects
    
    def validate_effect_consistency(self, pattern: Any, effects: List[MeasurableEffect], 
                                   min_instances: int = None) -> bool:
        """
        ...
        if len(historical_outcomes) < min_instances:
            logger.debug("Insufficient instances for validation: %d < %d", 
                        len(historical_outcomes), min_instances)
            return False
        ...
        for new_effect in effects:
            ...
            matches = [h for h in historical_effects 
                      if h.effect_type == new_effect.effect_type]
        ...
        logger.debug("Effect consistency for pattern %s: %.2f (threshold: %.2f)",
                    pattern_sig[:8], overall_consistency, consistency_threshold)
    ...
    def calculate_grounding_confidence(self, effects: List[MeasurableEffect]) -> float:
            ...
            if len(type_effects) > 1:
                ...
                for i, e1 in enumerate(type_effects):
                    for e2 in type_effects[i+1:]:
                        consistency_scores.append(1.0 if e1.is_consistent_with(e2) else 0.0)
        ...
        critical_factor = (1.0 if has_performance else 0.0) * 0.5 + (1.0 if has_resource else 0.0) * 0.5
        ...
        logger.debug("Grounding confidence: %.2f (effects: %d, types: %d)",
                    grounding_confidence, len(effects), len(effect_types))
        
        return min(1.0, grounding_confidence)
    
    def create_concept(self, pattern: Any, effects: List[MeasurableEffect], 
                      evidence_count: int = 0) -> Concept:
        """
        ...
        with self._lock:
            ...
            # Update statistics
            self.total_concepts_created += 1
            if concept.grounding_status in [GroundingStatus.GROUNDED, GroundingStatus.STRONGLY_GROUNDED]:
            ...
            logger.info("Created concept %s with %d effects (confidence: %.2f, grounding: %s)",
                       concept.concept_id, len(effects), initial_confidence, 
                       concept.grounding_status.value)
            
            return concept
    
    def _link_concept_to_world_model(self, concept: Concept, effects: List[MeasurableEffect]):
        """
        ...
        for effect in effects:
            ...
            try:
                ...
                if not self.world_model.causal_graph.has_edge(pattern_var, effect_var):
                    ...
                    logger.debug("Linked concept to world model: %s -> %s", pattern_var, effect_var)
            except Exception as e:
                logger.debug("Error linking to world model: %s", e)
    
    def process_pattern_outcomes(self, pattern: Any, outcomes: List[PatternOutcome]) -> Optional[Concept]:
        """
        ...
        with self._lock:
            ...
            if pattern_sig not in self.pattern_outcomes:
                if len(self.pattern_outcomes) >= self.max_patterns:
                    ...
                    min_pattern = min(self.pattern_outcomes.keys(),
            ...
            domain = getattr(pattern, 'domain', outcomes[0].domain if outcomes else 'general')
            thresholds = self.get_thresholds_for_domain(domain)
            min_instances = thresholds['min_instances']
            ...
            if len(self.pattern_outcomes[pattern_sig]) >= min_instances:
                # Extract effects
                effects = self.extract_measurable_effects(self.pattern_outcomes[pattern_sig])
                ...
                if effects:
                    ...
                    if self.validate_effect_consistency(pattern, effects):
                        ...
                        concept = self.create_concept(
                            pattern, effects, 
                            len(self.pattern_outcomes[pattern_sig])
                        )
    ...
    def _categorize_effect(self, measure_name: str) -> EffectType:
        ...
        categorization_rules = [
            ...
            (EffectType.TEMPORAL, ['time', 'latency', 'duration', 'delay', 'speed', 'throughput']),
            # Resource (memory, CPU, etc.)
            (EffectType.RESOURCE, ['memory', 'cpu', 'resource', 'usage', 'consumption', 'allocation']),
            # Performance (accuracy, success)
            (EffectType.PERFORMANCE, ['accuracy', 'precision', 'recall', 'f1', 'success', 'performance', 
                                      'score', 'error_rate', 'quality']),
            # Behavioral
            (EffectType.BEHAVIORAL, ['behavior', 'behaviour', 'action', 'response', 'reaction', 
                                    'decision', 'choice']),
            # Structural (lowest priority - most generic)
            (EffectType.STRUCTURAL, ['structure', 'topology', 'architecture', 'layout', 'organization'])
        ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'grounding_rate': self.grounded_concepts / max(1, self.total_concepts_created),
