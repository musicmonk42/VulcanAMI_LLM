...
try:
    ...
    logging.warning("safety_validator not available, conflict_resolver operating without safety checks")
...
class Evidence:
    ...
    def get_weight(self, domain: str = None, domain_weights: Dict = None) -> float:
        ...
        if domain_weights and domain:
            type_weight = domain_weights.get(domain, domain_weights.get('default', {})).get(
                self.evidence_type, 0.5
            )
        else:
            # Default weights
            type_weights = {
...
class EvidenceWeightedResolver:
    ...
    def __init__(self, world_model=None, domain_registry=None, safety_config: Optional[Dict[str, Any]] = None):
        """
        Initialize evidence-weighted resolver - FIXED: Added world_model, domain_registry, and safety_config
        ...
        if SAFETY_VALIDATOR_AVAILABLE:
            ...
            logger.info("EvidenceWeightedResolver: Safety validator initialized")
        else:
            self.safety_validator = None
            logger.warning("EvidenceWeightedResolver: Safety validator not available - operating without safety checks")
        ...
        logger.info("EvidenceWeightedResolver initialized (production-ready) with bounded data structures")
    
    def _initialize_domain_evidence_weights(self) -> Dict[str, Dict[EvidenceType, float]]:
        """
    ...
    def calculate_evidence_weight(self, concept, domain: str = None) -> float:
        ...
        with self._lock:
            ...
            if domain is None and hasattr(concept, 'domains'):
                domains = concept.domains if isinstance(concept.domains, (list, set)) else [concept.domains]
            ...
            if not evidence_list:
                ...
                if concept_id not in self.evidence_store:
                    ...
                    self.evidence_store[concept_id] = deque(maxlen=self.max_evidence_per_concept)
    ...
    def resolve_conflict(self, conflict_data) -> Dict[str, Any]:
        ...
        if hasattr(conflict_data, 'new_concept'):
            ...
            new_pattern = conflict_data.get('new_concept') or conflict_data.get('new_pattern')
            ...
            try:
                if hasattr(self.safety_validator, 'validate_concept'):
                    ...
                    if not new_check.get('safe', True):
                        logger.warning("Unsafe new concept detected: %s", new_check.get('reason', 'unknown'))
                        self.safety_blocks['unsafe_new_concept'] += 1
                        return {
                            'action': 'reject',
                            'winner': None,
                            'confidence': 0.0,
                            'reasoning': [f"Safety: {new_check.get('reason', 'unsafe concept')}"]
                        }
            except Exception as e:
                logger.debug("Error validating new concept: %s", e)
        
        # Detect conflict type
        conflict_type = conflict_type_hint or self._detect_conflict_type(new_pattern, existing_concepts)
        
        # Calculate evidence weights
        new_weight = self._calculate_pattern_weight(new_pattern)
        existing_weights = [self.calculate_evidence_weight(c) for c in existing_concepts]
        
        # Determine resolution strategy
        resolution = self._determine_resolution_strategy(
            new_pattern, new_weight,
            existing_concepts, existing_weights,
            conflict_type
        )
        
        # Execute resolution
        resolution = self._execute_resolution(resolution, new_pattern, existing_concepts)
        ...
        logger.info("Resolved conflict with action: %s (confidence: %.2f)",
                   resolution.action.value, resolution.confidence)
        
        # Convert to dict format for semantic_bridge compatibility
        return {
            'action': resolution.action.value,
            'winner': existing_concepts[0] if existing_concepts and resolution.action == ResolutionAction.REJECT else None,
    ...
    def merge_concepts(self, concept_a, concept_b) -> Any:
        ...
        with self._lock:
            ...
            if self.safety_validator:
                try:
                    if hasattr(self.safety_validator, 'validate_concept'):
                        ...
                        if not a_check.get('safe', True) or not b_check.get('safe', True):
            ...
            if hasattr(merged, 'concept_id') and hasattr(concept_b, 'concept_id'):
                merged.concept_id = f"{concept_a.concept_id}_merged_{concept_b.concept_id}"
            ...
            if hasattr(merged, 'features') and hasattr(concept_b, 'features'):
                for key, value in concept_b.features.items():
                    if key in merged.features:
                        ...
                        if isinstance(value, (int, float)) and isinstance(merged.features[key], (int, float)):
                            merged.features[key] = (merged.features[key] + value) / 2
                        # Union for sets
                        elif isinstance(value, set) and isinstance(merged.features[key], set):
                            merged.features[key] = merged.features[key] | value
                        # Extend lists
                        elif isinstance(value, list) and isinstance(merged.features[key], list):
            ...
            if hasattr(concept_a, 'concept_id') and hasattr(concept_b, 'concept_id'):
                ...
                if merged.concept_id not in self.evidence_store:
                    if len(self.evidence_store) >= self.max_evidence_concepts:
                        self._evict_oldest_evidence_concept()
                    self.evidence_store[merged.concept_id] = deque(maxlen=self.max_evidence_per_concept)
                
                # Combine evidence from both concepts
                combined_evidence = list(self.evidence_store.get(concept_a.concept_id, []))
                combined_evidence.extend(self.evidence_store.get(concept_b.concept_id, []))
            ...
            logger.debug("Merged concepts into %s", getattr(merged, 'concept_id', 'merged'))
    ...
    def create_concept_variant(self, base_concept, new_pattern) -> Any:
        ...
        with self._lock:
            ...
            if hasattr(variant, 'features'):
                ...
                for key, value in pattern_features.items():
                    if key in variant.features:
                        # Blend features
                        if isinstance(value, (int, float)) and isinstance(variant.features[key], (int, float)):
                            variant.features[key] = 0.7 * variant.features[key] + 0.3 * value
                    else:
            ...
            if hasattr(variant, 'metadata'):
                variant.metadata['variant_of'] = getattr(base_concept, 'concept_id', str(base_concept))
            ...
            if base_id and variant_id:
                ...
                if base_id not in self.concept_relationships:
                    if len(self.concept_relationships) >= self.max_relationship_concepts:
                        self._evict_oldest_relationship_concept()
                    self.concept_relationships[base_id] = set()
                
                if len(self.concept_relationships[base_id]) < self.max_relationships_per_concept:
                ...
                if len(self.concept_relationships[variant_id]) < self.max_relationships_per_concept:
                    self.concept_relationships[variant_id].add(base_id)
            
            logger.debug("Created variant %s of concept", getattr(variant, 'concept_id', 'variant'))
    ...
    def _evict_oldest_relationship_concept(self):
        ...
        min_concept = min(self.concept_relationships.keys(),
    ...
    def add_evidence(self, concept_id: str, evidence: Evidence):
        ...
        with self._lock:
            ...
            logger.debug("Added %s evidence for concept %s", 
                        evidence.evidence_type.value, concept_id)
    ...
    def reverse_resolution(self, resolution_id: str, reason: str) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                # Record reversal
                self.resolution_history.append(ConflictResolution(
    ...
    def get_reversible_resolutions(self, max_age_hours: int = 24) -> List[Dict[str, Any]]:
        """
    ...
    def compare_concepts(self, concept_a, concept_b) -> Dict[str, float]:
        ...
        if hasattr(concept_a, 'domains') and hasattr(concept_b, 'domains'):
            domains_a = set(concept_a.domains) if not isinstance(concept_a.domains, set) else concept_a.domains
            domains_b = set(concept_b.domains) if not isinstance(concept_b.domains, set) else concept_b.domains
            if domains_a or domains_b:
                comparison['domain_overlap'] = len(domains_a & domains_b) / len(domains_a | domains_b)
        ...
        if hasattr(concept_a, 'success_rate') and hasattr(concept_b, 'success_rate'):
            comparison['performance_diff'] = concept_a.success_rate - concept_b.success_rate
    ...
    def _generate_evidence_from_concept(self, concept) -> List[Evidence]:
        ...
        if hasattr(concept, 'domains') and concept.domains:
            domains_set = concept.domains if isinstance(concept.domains, set) else set(concept.domains)
            evidence = Evidence(
    ...
    def _calculate_pattern_weight(self, pattern) -> float:
        ...
        if hasattr(pattern, 'complexity'):
            ...
            complexity_normalized = min(1.0, pattern.complexity / 10) if pattern.complexity > 0 else 0
    ...
    def _detect_conflict_type(self, new_pattern, existing_concepts) -> Optional[ConflictType]:
    ...
    def _calculate_semantic_similarity(self, pattern, concept) -> float:
        ...
        key_overlap = len(pattern_keys & concept_keys) / len(pattern_keys | concept_keys)
            ...
            if hasattr(concept, 'features'):
                for key in pattern_features:
                    if key in concept.features:
                        ...
                        if self._are_contradictory(pattern_features[key], concept.features[key]):
                            return True
    ...
    def _has_significant_overlap(self, pattern, concepts) -> bool:
        ...
        for concept in concepts:
            if hasattr(concept, 'features'):
                overlap = self._calculate_feature_overlap(pattern_features, concept.features)
                if overlap > 0.5:
                    return True
    ...
    def _determine_resolution_strategy(self, new_pattern, new_weight,
                                      existing_concepts, existing_weights,
                                      conflict_type: Optional[ConflictType]) -> ConflictResolution:
        ...
        if conflict_type == ConflictType.DUPLICATION:
            ...
            if new_weight > max_existing_weight * self.replace_threshold:
                ...
                resolution.action = ResolutionAction.REJECT
                resolution.confidence = max_existing_weight / (new_weight + max_existing_weight)
    ...
    def _execute_resolution(self, resolution: ConflictResolution,
                          new_pattern, existing_concepts) -> ConflictResolution:
    ...
    def _create_concept_from_pattern(self, pattern):
        ...
        concept.concept_id = f"concept_{hashlib.md5(str(pattern).encode()).hexdigest()[:8]}"
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'success_rate': self.successful_resolutions / max(1, self.total_resolutions),
            ...
            'total_relationships': sum(len(r) for r in self.concept_relationships.values()),
