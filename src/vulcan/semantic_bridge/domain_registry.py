...
try:
    ...
    class MockNX:
        ...
        def ancestors(graph, node):
            ...
            ancestors = set()
            to_visit = (
                graph.predecessors(node) if hasattr(graph, "predecessors") else []
            )
...
class DomainRegistry:
    ...
    def __init__(
        ...
        """
        Initialize domain registry - FIXED: Added world_model, safety_config, and safety_validator
        ...
        if safety_validator is not None:
            ...
            logger.info("DomainRegistry: Using shared safety validator instance")
            ...
            try:
                ...
                logger.info("DomainRegistry: Using singleton safety validator")
            ...
            logger.warning(
                "DomainRegistry: Safety validator not available - operating without safety checks"
            )
        ...
        logger.info(
            "DomainRegistry initialized (production-ready) with bounded data structures, %d domains",
            len(self.domains),
        )
    ...
    def _adaptive_cache_resize(self):
        ...
        if hit_rate > 0.8:
            ...
            logger.debug(
                "Increased cache size to %d (hit rate: %.2f)",
                self.max_cache_size,
                hit_rate,
            )
        elif hit_rate < 0.3:
            ...
            logger.debug(
                "Decreased cache size to %d (hit rate: %.2f)",
                self.max_cache_size,
                hit_rate,
            )
    ...
    def register_domain(
        ...
        with self._lock:
            ...
            if profile is None:
                ...
                if characteristics:
                    if "adaptability" in characteristics:
                        profile.metadata["adaptability"] = characteristics[
                            "adaptability"
                        ]
            ...
            if not (0 <= profile.criticality_score <= 1):
                logger.warning(
                    "Invalid criticality score %.2f, clamping to [0,1]",
            ...
            logger.info(
                "Registered domain: %s (criticality: %.2f)",
                name,
                profile.criticality_score,
            )
            return True
    ...
    def update_domain_performance(self, domain: str, success: bool):
        ...
        with self._lock:
            ...
            logger.debug(
                "Updated performance for domain %s: success_rate=%.2f", domain, new_rate
            )
    ...
    def calculate_domain_distance(self, domain_a: str, domain_b: str) -> float:
        ...
        with self._lock:
            ...
            if not profile_a or not profile_b:
                ...
                crit_dist = abs(
                    profile_a.criticality_score - profile_b.criticality_score
                )
                ...
                if effects_a or effects_b:
                    effect_similarity = len(effects_a & effects_b) / len(
                        effects_a | effects_b
                    )
                ...
                # Graph distance
                if self.domain_graph.has_node(domain_a) and self.domain_graph.has_node(
                    domain_b
                ):
                    try:
                        ...
                        path_length = self._find_shortest_path_length(
                            undirected, domain_a, domain_b
                        )
                        graph_distance = min(
                            1.0, path_length / 5
                        )  # Normalize to [0, 1]
                ...
                if distances:
                    weights = [
                        ...
                        0.25,
                        0.2,
                    ]  # Criticality, effects, capabilities, graph
                    weighted_distances = [
                        d * w for d, w in zip(distances, weights[: len(distances)])
                    ]
    ...
    def add_domain_relationship(
        self, source: str, target: str, relationship_type: str, strength: float = 0.5
    ):
        ...
        with self._lock:
            ...
            self.domain_graph.add_edge(
                source, target, type=relationship_type, weight=strength
            )
            ...
            logger.debug(
                "Added relationship: %s -> %s (%s)", source, target, relationship_type
            )
    ...
    def get_domain_hierarchy(self, domain_name: str) -> Dict[str, List[str]]:
        ...
        with self._lock:
            ...
            if self.domain_graph.has_node(domain_name):
                if NETWORKX_AVAILABLE:
                    hierarchy["ancestors"] = list(
                        nx.ancestors(self.domain_graph, domain_name)
                    )
                    hierarchy["descendants"] = list(
                        nx.descendants(self.domain_graph, domain_name)
                    )
                else:
                    hierarchy["ancestors"] = MockNX.ancestors(
                        self.domain_graph, domain_name
                    )
                    hierarchy["descendants"] = MockNX.descendants(
                        self.domain_graph, domain_name
                    )
    ...
    def get_similar_domains(
        self, domain_name: str, top_k: int = 5
    ...
    def update_domain_criticality(self, domain_name: str, new_criticality: float):
        ...
        with self._lock:
            if domain_name in self.domains:
                ...
                logger.info(
                    "Updated criticality for %s: %.2f -> %.2f",
                    domain_name,
                    old_criticality,
                    new_criticality,
                )
    ...
    def merge_domains(
        self, domain_a: str, domain_b: str, new_name: str = None
    ) -> DomainProfile:
        ...
        with self._lock:
            ...
            merged = DomainProfile(
                ...
                typical_patterns=profile_a.typical_patterns
                + profile_b.typical_patterns,
                capabilities=profile_a.capabilities | profile_b.capabilities,
                limitations=profile_a.limitations | profile_b.limitations,
                parent_domains=list(
                    set(profile_a.parent_domains + profile_b.parent_domains)
                ),
                child_domains=list(
                    set(profile_a.child_domains + profile_b.child_domains)
                ),
            )
            ...
            for rel in self.relationships:
                if rel.source in [domain_a, domain_b]:
                    self.add_domain_relationship(
                        merged_name, rel.target, rel.relationship_type, rel.strength
                    )
                elif rel.target in [domain_a, domain_b]:
                    self.add_domain_relationship(
                        rel.source, merged_name, rel.relationship_type, rel.strength
                    )

            logger.info(
                "Merged domains %s and %s into %s", domain_a, domain_b, merged_name
            )
    ...
    def _update_effect_statistics(self, effect: DomainEffect):
        ...
        stats["count"] = count + 1
        stats["avg_importance"] = (avg_importance * count + effect.importance) / (
            count + 1
        )
    ...
    def _initialize_default_domains(self):
        ...
        self.add_domain_relationship(
            "general", "machine_learning", "specialization", 0.6
        )
        ...
        if relationships_file.exists():
            try:
                with open(relationships_file, "r", encoding="utf-8") as f:
                    ...
                    for rel in rel_data:
                        self.add_domain_relationship(
                            rel["source"], rel["target"], rel["type"], rel["strength"]
                        )
            except Exception as e:
...
class RiskAdjuster:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        logger.info(
            "RiskAdjuster initialized with config from %s",
    ...
    def get_dynamic_thresholds(self, domain_profile: DomainProfile) -> Dict[str, float]:
        ...
        for threshold_name, base_value in self.base_thresholds.items():
            ...
            if threshold_name in ["confidence", "success_rate", "validation"]:
                ...
                adjusted[threshold_name] = min(
                    0.99, adjusted[threshold_name] + safety_margin
                )
    ...
    def assess_risk(
        self, domain_profile: DomainProfile, operation: str
    ) -> Dict[str, Any]:
        """
        ...
        assessment = {
            ...
            "safety_margin": self.calculate_safety_margin(
                domain_profile.criticality_score
            ),
            "recommended_actions": [],
        }
        ...
        if domain_profile.criticality_score >= DomainCriticality.HIGH.value:
            ...
            assessment["recommended_actions"].extend(
                ["standard_validation", "performance_monitoring"]
            )
