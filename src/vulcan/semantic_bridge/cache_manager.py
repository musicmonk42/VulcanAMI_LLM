...
class CacheManager:
    ...
    def register_cache(self, name: str, cache: Dict, priority: int = 5, 
    ...
    def check_memory(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            result = {
                ...
                'usage_percent': (total_size / self.max_memory * 100) if self.max_memory > 0 else 0,
            ...
            if total_size > self.max_memory:
                logger.info("Memory limit exceeded (%.1f / %.1f MB), evicting caches",
                           result['total_mb'], result['limit_mb'])
    ...
    def _evict_low_priority(self, cache_sizes: Dict[str, int], current_total: int) -> List[Dict[str, Any]]:
        """
        ...
        sorted_caches = sorted(
            ...
            key=lambda x: (x[1], -cache_sizes.get(x[0], 0))
        )
            ...
            if cache_size > 0:
                ...
                if callback:
                    try:
                        ...
                        logger.error("Cache clear callback failed for '%s': %s", name, e)
                ...
                logger.info("Evicted cache '%s' (priority %d, %d entries, %.1f MB freed)",
                           name, priority, original_size, cache_size / (1024 * 1024))
                
                # Check if we've freed enough memory
                if (current_total - freed_memory) < self.max_memory:
                    logger.info("Memory now under limit after evicting %d caches", len(evicted))
                    break
    ...
    def record_hit(self, cache_name: str):
        ...
        with self._lock:
            if cache_name in self.metadata:
                ...
                logger.debug("Cannot record hit for unregistered cache '%s'", cache_name)
    ...
    def record_miss(self, cache_name: str):
        ...
        with self._lock:
            if cache_name in self.metadata:
                ...
                logger.debug("Cannot record miss for unregistered cache '%s'", cache_name)
    ...
    def clear_cache(self, cache_name: str, force: bool = False):
        ...
        with self._lock:
            ...
            if not force and priority >= 9:
                logger.warning("Refusing to clear high-priority cache '%s' without force=True", 
                             cache_name)
                return
            ...
            if callback:
                try:
                    ...
                    logger.error("Cache clear callback failed for '%s': %s", cache_name, e)
    ...
    def get_cache_info(self, cache_name: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            return {
                ...
                'hit_rate': meta['hits'] / total_requests if total_requests > 0 else 0.0,
        ...
        with self._lock:
            ...
            return {
                ...
                'summary': {
                    ...
                    'overall_hit_rate': total_hits / overall_requests if overall_requests > 0 else 0.0,
    ...
    def update_priority(self, cache_name: str, new_priority: int):
        ...
        with self._lock:
            ...
            if not (1 <= new_priority <= 10):
                logger.warning("Priority %d out of range [1,10], clamping", new_priority)
                new_priority = max(1, min(10, new_priority))
            
            old_priority = self.priorities[cache_name]
            self.priorities[cache_name] = new_priority
            
            logger.info("Updated priority for cache '%s': %d -> %d", 
                       cache_name, old_priority, new_priority)
        ...
        with self._lock:
            for name in self.caches:
                ...
                if hit_rate > 0.8 and current_priority < 10:
                    ...
                    logger.debug("Increased priority for '%s' to %d (hit rate: %.2f)",
                               name, self.priorities[name], hit_rate)
                elif hit_rate < 0.3 and current_priority > 1:
                    # Low hit rate - decrease priority
                    self.priorities[name] = max(1, current_priority - 1)
                    logger.debug("Decreased priority for '%s' to %d (hit rate: %.2f)",
                               name, self.priorities[name], hit_rate)
