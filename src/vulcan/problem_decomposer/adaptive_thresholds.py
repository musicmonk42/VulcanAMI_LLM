...
class AdaptiveThresholds:
    ...
    def __init__(self, initial_values: Dict[str, float] = None):
        ...
        logger.info("AdaptiveThresholds initialized with %d thresholds", len(self.thresholds))
    ...
    def auto_calibrate(self, recent_performance: List[Dict[str, Any]]):
        ...
        with self._lock:
            ...
            complexities = [p.get('complexity', 0) for p in recent_performance if 'complexity' in p]
            if complexities:
            ...
            logger.info("Auto-calibrated thresholds based on %d performance records", total_count)
    
    def get_confidence_threshold(self) -> float:
        """Get confidence threshold (convenience method)"""
        return self.get_current(ThresholdType.CONFIDENCE.value)
    
    def update_from_outcome(self, complexity: float, success: bool, execution_time: float):
        """
    ...
    def _adjust_threshold(self, threshold_name: str, factor: float, direction: str):
        ...
        logger.debug("Adjusted %s threshold: %.3f -> %.3f", 
                    threshold_name, old_value, config.value)
...
class PerformanceTracker:
    ...
    def __init__(self, window_size: int = 100):
        ...
        self.problem_history = defaultdict(lambda: deque(maxlen=50))  # Was unbounded list
    ...
    def record_execution(self, problem, plan, outcome):
        ...
        with self._lock:
            ...
            if success:
                ...
                if errors:
                    failure_reason = errors[0] if isinstance(errors, list) else str(errors)
                else:
            ...
            logger.debug("Recorded execution for problem %s: success=%s, strategy=%s, time=%.2f",
                        problem_signature[:8], success, strategy_used, execution_time)
    
    def record_success(self, problem_signature: str, strategy_used: str, 
        ...
        with self._lock:
            ...
            logger.debug("Recorded success for problem %s using strategy %s",
                        problem_signature[:8], strategy_used)
    
    def record_failure(self, problem_signature: str, reason: str,
                      strategy_used: str = None, execution_time: float = 0.0):
        """
        ...
        with self._lock:
            ...
            logger.debug("Recorded failure for problem %s: %s", problem_signature[:8], reason)
    ...
    def get_strategy_success_rate(self, strategy_name: str) -> float:
        ...
        """
        with self._lock:
            total = self.strategy_successes[strategy_name] + self.strategy_failures[strategy_name]
    ...
    def get_failure_analysis(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'top_reason': max(self.failure_reasons, key=self.failure_reasons.get) if self.failure_reasons else None
            }
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'overall_success_rate': self.total_successes / max(1, self.total_attempts),
                'recent_success_rate': self.get_success_rate(),
                'unique_problems': len(self.problem_history),
                'strategies_used': len(set(list(self.strategy_successes.keys()) + list(self.strategy_failures.keys())))
            }
...
class StrategyProfiler:
    ...
    def update(self, strategy_name: str, latency: float, success: bool,
        ...
        with self._lock:
            ...
            logger.debug("Updated profile for strategy %s: latency=%.2f, success=%s",
                        strategy_name, latency, success)
    ...
    def recommend_strategy(self, problem_class: str = None,
                          max_latency: float = None) -> Optional[str]:
        """
