...
class ExecutionPlan:
    ...
    def add_components(self, components: List[DecompositionComponent], confidence: float = None):
        """
        ...
        for component in components:
            ...
            logger.debug("Added component %s to plan %s", component.component_id, self.plan_id)
    ...
    def validate_completeness(self) -> Tuple[bool, List[str]]:
        ...
        for component in self.components:
            if component.confidence < 0.2:
                issues.append(f"Very low confidence component: {component.component_id}")
        
        is_complete = len(issues) == 0
        
        return is_complete, issues
    
    def update_component_status(self, component_id: str, status: StrategyStatus, result: Any = None):
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'completion_rate': status_counts[StrategyStatus.SUCCESS.value] / max(1, len(self.components)),
...
class FallbackChain:
    ...
    def __init__(self, strategies: List[Any] = None):
        ...
        self.strategy_success_rates = defaultdict(lambda: 0.5)  # strategy_name -> success rate
        ...
        logger.info("FallbackChain initialized with %d strategies", len(self.strategies))
    
    def execute_with_fallbacks(self, problem_graph) -> Tuple[Optional[ExecutionPlan], DecompositionFailure]:
        """
        ...
        """
        problem_signature = problem_graph.get_signature() if hasattr(problem_graph, 'get_signature') else str(problem_graph)
    ...
    def reorder_by_cost(self, problem_class: str = None):
        ...
        with self._lock:
            ...
            for strategy in self.strategies:
                ...
                if problem_class:
                    success_rate = self._adjust_for_problem_class(strategy_name, problem_class, success_rate)
                
                # Calculate score (higher is better)
                score = (success_rate * self.success_weight) / (cost * self.cost_weight + 0.01)
    ...
    def handle_strategy_failure(self, strategy, exception: Exception) -> str:
        ...
        with self._lock:
            ...
            if strategy_name not in self.recovery_strategies:
                self.recovery_strategies[strategy_name] = self._determine_recovery_strategy(failure_reason)
            
            # Check if we should remove strategy from chain
            if self.failure_counts[strategy_name] > 5:
                logger.warning("Strategy %s has failed too many times, considering removal", strategy_name)
    ...
    def _adjust_for_problem_class(self, strategy_name: str, problem_class: str, 
    ...
    def _create_simplified_plan(self, problem_graph) -> Optional[ExecutionPlan]:
        ...
        signature = problem_graph.get_signature()[:8] if hasattr(problem_graph, 'get_signature') else "unknown"
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'overall_success_rate': successful_executions / max(1, total_executions),
