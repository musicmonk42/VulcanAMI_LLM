...
class PatternPerformance:
    ...
    def update(self, success: bool, execution_time: float, domain: str = None, 
...
class DecompositionLibrary:
    ...
    def __init__(self, storage_path: Optional[Path] = None):
        ...
        logger.info("DecompositionLibrary initialized with %d patterns and %d principles",
                   len(self.patterns), len(self.principles))
    ...
    def find_similar(self, subgraph, top_k: int = 5) -> List[Tuple[str, float]]:
        ...
        with self._lock:
            ...
            for pattern_id, pattern in self.patterns.items():
                # Check cache
                cache_key = (pattern_id, hashlib.md5(str(query_features).encode()).hexdigest())
                ...
                if cache_key in self.similarity_cache:
                    ...
                    if pattern_embedding is not None:
                        ...
                        similarity = self._calculate_structural_similarity(subgraph, pattern.structure)
    ...
    def reinforce_pattern(self, signature: str, decomposition: Any, performance: float):
        ...
        with self._lock:
            ...
            if signature not in self.performance:
                self.performance[signature] = PatternPerformance(pattern_signature=signature)
            ...
            perf.update(
                ...
                domain=decomposition.get('domain', 'general') if isinstance(decomposition, dict) else 'general'
            )
            ...
            if signature in self.pattern_index:
                ...
                if pattern:
                    ...
                    pattern.metadata['reinforcement_count'] = pattern.metadata.get('reinforcement_count', 0) + 1
                    pattern.metadata['avg_performance'] = (
                        (pattern.metadata.get('avg_performance', 0.5) * 
                         pattern.metadata.get('reinforcement_count', 1) + performance) /
            ...
            logger.debug("Reinforced pattern %s with performance %.2f", signature[:8], performance)
    ...
    def mark_failed_pattern(self, signature: str, decomposition: Any, reason: str):
        ...
        with self._lock:
            ...
            if signature in self.pattern_index:
                ...
                if pattern:
                    pattern.metadata['failure_count'] = pattern.metadata.get('failure_count', 0) + 1
    ...
    def add_principle(self, principle: DecompositionPrinciple):
        ...
        with self._lock:
            ...
            if isinstance(principle, dict):
                principle_id = principle.get('principle_id', principle.get('id', f"principle_{int(time.time())}"))
            ...
            if pattern:
                ...
                if isinstance(pattern, dict):
                    pattern_id = pattern.get('pattern_id', f"pattern_{int(time.time())}")
    ...
    def get_applicable_principles(self, context: Context) -> List[DecompositionPrinciple]:
        """
    ...
    def _add_pattern(self, pattern):
        ...
        if isinstance(pattern, dict):
            ...
            pattern_id = pattern.get('pattern_id', f"pattern_{int(time.time())}_{id(pattern)}")
            ...
            logger.warning("Pattern is neither dict nor has pattern_id attribute: %s", type(pattern))
    ...
    def _calculate_similarity(self, embedding1: np.ndarray, embedding2: np.ndarray) -> float:
        ...
        if SKLEARN_AVAILABLE:
            ...
            # Use fallback
            return float(cosine_similarity_fallback(
                embedding1.reshape(1, -1),
                embedding2.reshape(1, -1)
            )[0, 0])
    ...
    def _calculate_structural_similarity(self, graph1, graph2) -> float:
        ...
        # Calculate densities
        if NETWORKX_AVAILABLE and isinstance(graph1, nx.Graph) and isinstance(graph2, nx.Graph):
...
class StratifiedDecompositionLibrary(DecompositionLibrary):
    ...
    def get_patterns_by_frequency(self, min_count: int) -> List[Tuple[str, Pattern]]:
        ...
        with self._lock:
            ...
            frequent_patterns.sort(
                key=lambda x: self.pattern_frequency[x[0]], 
                reverse=True
            )
    ...
    def get_patterns_by_domain(self, domain: str) -> List[Pattern]:
        ...
        with self._lock:
            ...
            patterns.sort(
                key=lambda p: self.performance.get(self._get_pattern_signature(p), 
                    PatternPerformance(pattern_signature=self._get_pattern_signature(p))).get_success_rate(),
                reverse=True
            )
    ...
    def _get_pattern_signature(self, pattern) -> str:
        ...
        if isinstance(pattern, dict):
            return pattern.get('signature', f"sig_{pattern.get('pattern_id', 'unknown')}")
