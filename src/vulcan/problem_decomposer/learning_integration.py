...
try:
    from .problem_decomposer_core import (
        ProblemDecomposer, ProblemGraph, DecompositionPlan, 
        ExecutionOutcome, ProblemSignature, LearningGap
    )
...
try:
    ...
    try:
        ...
        logging.warning("PrincipleLearner not available - principle extraction disabled")
...
class ProblemToExperienceConverter:
    ...
    def convert_to_experience(self, problem: ProblemGraph, 
        ...
        try:
            ...
            if outcome.metrics:
                if 'actual_complexity' in outcome.metrics:
                    ...
                    complexity_bonus = max(0, problem.complexity_score - outcome.metrics['actual_complexity']) * 0.1
            ...
            experience = {
                ...
                'loss': 1.0 - success_rate if outcome.sub_results else (0.0 if outcome.success else 1.0),
...
class RLHFFeedbackRouter:
    ...
    def route_outcome_to_feedback(self, problem: ProblemGraph,
                                  plan: DecompositionPlan,
                                  outcome: ExecutionOutcome) -> Optional[FeedbackData]:
    ...
    def route_human_feedback(self, problem_signature: str,
                            rating: float,
...
class IntegratedLearningCoordinator:
    ...
    def __init__(self, decomposer: ProblemDecomposer,
        ...
        if continual_learner is not None and not hasattr(continual_learner, 'process_experience'):
            logger.error("continual_learner missing required method 'process_experience'")
            continual_learner = None
        
        if curriculum_learner is not None and not hasattr(curriculum_learner, 'generate_curriculum'):
            logger.error("curriculum_learner missing required method 'generate_curriculum'")
        ...
        if metacognition is not None and not hasattr(metacognition, 'update_self_model'):
            logger.error("metacognition missing required method 'update_self_model'")
            metacognition = None
        
        if principle_learner is not None and not hasattr(principle_learner, 'extract_and_promote'):
            logger.error("principle_learner missing required method 'extract_and_promote'")
        ...
        logger.info("IntegratedLearningCoordinator initialized (principle_learner: %s)", 
                   principle_learner is not None)
    
    def learn_integrated(self, problem: ProblemGraph,
                        plan: DecompositionPlan,
        ...
        with self._lock:
            ...
            try:
                ...
                experience = self.converter.convert_to_experience(problem, plan, outcome)
                
                # Step 2: Process with continual learner (gets task detection, EWC, replay)
                if self.continual_learner:
                    try:
                        continual_result = self.continual_learner.process_experience(experience)
                ...
                if self.rlhf_manager:
                    try:
                        self.feedback_router.route_outcome_to_feedback(problem, plan, outcome)
                ...
                if self.principle_learner:
                    try:
                        ...
                        # Update statistics
                        self.integration_stats['principles_extracted'] += principle_results.get('principles_extracted', 0)
                        self.integration_stats['principles_validated'] += principle_results.get('principles_validated', 0)
                        self.integration_stats['principles_promoted'] += principle_results.get('principles_promoted', 0)
                        
                        logger.debug("Principle learning: extracted=%d, validated=%d, promoted=%d",
    ...
    def get_next_problems_curriculum(self, batch_size: int = 10,
                                    performance: Optional[float] = None) -> List[ProblemGraph]:
    ...
    def analyze_learning_effectiveness(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self.curriculum_learner:
                analysis['curriculum_learning'] = self.curriculum_learner.get_curriculum_stats()
            ...
            if self.metacognition:
                analysis['metacognition'] = self.metacognition.analyze_learning_efficiency()
            ...
            if self.principle_learner:
                analysis['principle_learning'] = self.principle_learner.get_learning_statistics()
    ...
    def get_recommendations(self) -> List[str]:
        ...
        with self._lock:
            ...
            if self.metacognition:
                ...
                if efficiency.get('recommendations'):
                    for rec in efficiency['recommendations']:
                        recommendations.append(f"Metacognition: {rec.get('suggestion', 'Unknown')}")
            
            # Check continual learning
            if self.continual_learner:
                stats = self.continual_learner.get_statistics()
                
                forgetting = stats.get('continual_metrics', {}).get('forgetting_measure', 0)
                if forgetting > 0.3:
                    recommendations.append("High forgetting detected - increase EWC lambda or replay frequency")
                
                num_tasks = stats.get('num_tasks', 0)
                if num_tasks > 10 and stats.get('free_capacity', 1.0) < 0.2:
                    recommendations.append("Low capacity remaining - consider progressive network expansion")
            ...
            if self.curriculum_learner:
                ...
                if stats.get('difficulty_adjustments'):
                    ...
                    if len(recent_adjustments) == 5:
                        recommendations.append("Frequent difficulty adjustments - curriculum may need rebalancing")
            ...
            if success_rate < 0.5:
                recommendations.append("Low decomposition success rate - consider adding more strategies")
            elif success_rate > 0.95:
                recommendations.append("Very high success rate - problems may be too easy")
            ...
            if self.principle_learner:
                ...
                promotion_stats = principle_stats.get('promotion', {}).get('promoter_stats', {})
                promotion_rate = promotion_stats.get('promotion_rate', 0)
                
                if promotion_rate < 0.2:
                    recommendations.append("Low principle promotion rate - consider lowering promotion threshold")
                elif promotion_rate > 0.9:
                    recommendations.append("Very high promotion rate - consider raising quality standards")
                ...
                if total_principles > 1000:
                    recommendations.append("Large principle library - consider pruning low-quality principles")
                elif total_principles < 10 and self.integration_stats['total_learning_calls'] > 100:
                    recommendations.append("Low principle extraction rate - check extraction thresholds")
...
class UnifiedDecomposerLearner:
    ...
    def __init__(self, semantic_bridge=None, vulcan_memory=None, validator=None,
        ...
        if enable_curriculum and CurriculumLearner:
            try:
                self.curriculum_learner = CurriculumLearner(
                    config=self.config,
                    pacing_strategy=self.config.curriculum_stages
                )
        ...
        if enable_principle_learning and PrincipleLearner:
            try:
                ...
                if hasattr(self.decomposer, 'strategy_library'):
                    ...
                    library = None
                    logger.warning("No decomposition library available for principle learning")
        ...
        logger.info("UnifiedDecomposerLearner initialized with all systems (principle_learning: %s)", 
                   self.principle_learner is not None)
    
    def decompose_and_execute(self, problem: ProblemGraph,
    ...
    def provide_human_feedback(self, problem_signature: str, rating: float,
                              comments: Optional[str] = None):
    ...
    def prune_principles(self, age_threshold_days: int = 90,
                        confidence_threshold: float = 0.3) -> int:
        """
    ...
    def save_state(self, path: Optional[str] = None) -> Dict[str, str]:
        ...
        if self.continual_learner:
            ...
            try:
                saved_paths['continual_learner'] = self.continual_learner.save_state(str(cl_path))
            except Exception as e:
        ...
        if self.curriculum_learner:
            ...
            try:
                saved_paths['curriculum_learner'] = self.curriculum_learner.save_state(str(curr_path))
            except Exception as e:
        ...
        if self.metacognition:
            ...
            try:
                saved_paths['metacognition'] = self.metacognition.save_state(str(meta_path))
            except Exception as e:
        ...
        if self.principle_learner:
            ...
            try:
                principle_paths = self.principle_learner.knowledge_base.export(
                    principle_path / "knowledge_base.json", 
                    format='json'
                )
...
def create_unified_decomposer(semantic_bridge=None, vulcan_memory=None, validator=None,
                              config: Optional[LearningConfig] = None,
                              enable_all: bool = True,
                              enable_principle_learning: bool = True) -> UnifiedDecomposerLearner:
    """
...
def test_integration():
    ...
    try:
        ...
        logger.info(f"  Learning updates: {stats['integration_stats']['total_learning_calls']}")
        ...
        if stats['integration_stats']['principles_extracted'] >= 0:
            logger.info(f"  Principle extraction working: {stats['integration_stats']['principles_extracted']} extracted")
        if stats['integration_stats']['principles_promoted'] >= 0:
            logger.info(f"  Principle promotion working: {stats['integration_stats']['principles_promoted']} promoted")
