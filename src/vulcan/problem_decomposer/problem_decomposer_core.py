...
try:
    ...
    logging.warning("safety_validator not available, problem_decomposer operating without safety checks")
...
try:
    ...
    class ProblemExecutor:
        ...
        def execute_plan(self, problem_graph, plan):
            return ExecutionOutcome(success=False, execution_time=0.0, errors=["Executor not available"])
...
class PerformanceTracker:
    ...
    def get_strategy_success_rate(self, strategy_name: str) -> float:
        ...
        with self._lock:
            stats = self.strategy_performance.get(strategy_name, {'success': 0, 'failure': 0})
            total = stats['success'] + stats['failure']
            if total == 0:
                return 0.5  # Default
...
class StrategyProfiler:
    ...
    def profile_strategy(self, strategy: DecompositionStrategy) -> Dict[str, Any]:
        ...
        profile = {
            ...
            'type': strategy.strategy_type if hasattr(strategy, 'strategy_type') else 'unknown',
            'complexity_range': self._estimate_complexity_range(strategy),
            'domain_suitability': self._estimate_domain_suitability(strategy),
            'parallelizable': strategy.is_parallelizable() if hasattr(strategy, 'is_parallelizable') else False,
            'deterministic': strategy.is_deterministic() if hasattr(strategy, 'is_deterministic') else True,
    ...
    def update_affinity(self, strategy_name: str, domain: str, 
        ...
        if len(self.domain_affinity[strategy_name]) >= self.max_domains_per_strategy:
            ...
            if self.domain_affinity[strategy_name]:
                worst_domain = min(self.domain_affinity[strategy_name].items(), key=lambda x: x[1])
        ...
        if len(self.complexity_affinity[strategy_name]) >= self.max_domains_per_strategy:
            # Remove complexity bucket with lowest affinity
            if self.complexity_affinity[strategy_name]:
                worst_bucket = min(self.complexity_affinity[strategy_name].items(), key=lambda x: x[1])
    ...
    def _estimate_complexity_range(self, strategy: DecompositionStrategy) -> Tuple[float, float]:
    ...
    def _estimate_domain_suitability(self, strategy: DecompositionStrategy) -> List[str]:
    ...
    def _estimate_resource_requirements(self, strategy: DecompositionStrategy) -> Dict[str, Any]:
        """Estimate resource requirements"""
        return {
            'memory': 'medium',  # Could be calculated based on strategy
            'cpu': 'high' if (hasattr(strategy, 'is_parallelizable') and strategy.is_parallelizable()) else 'medium',
            'time': 'variable'
        }
...
class ProblemDecomposer:
    ...
    def __init__(self, semantic_bridge=None, vulcan_memory=None, validator=None, 
        ...
        if SAFETY_VALIDATOR_AVAILABLE:
            ...
            logger.warning("ProblemDecomposer: Safety validator not available - operating without safety checks")
    ...
    def decompose_and_execute(self, problem_graph: ProblemGraph, 
                              validate: bool = False) -> Tuple[DecompositionPlan, ExecutionOutcome]:
        """
        ...
        logger.info("Decomposing and executing problem %s", problem_graph.get_signature()[:8])
        ...
        if not problem_validation['safe']:
            ...
            failed_outcome = ExecutionOutcome(
                success=False,
                execution_time=0.0,
                errors=[f"Problem blocked by safety validator: {problem_validation['reason']}"],
        ...
        if not plan_validation['safe']:
            ...
            failed_outcome = ExecutionOutcome(
                success=False,
                execution_time=0.0,
                errors=[f"Plan blocked by safety validator: {plan_validation['reason']}"],
                metadata={'safety_blocked': True}
            )
        ...
        if validate and self.validator:
            outcome, validation_results = self.executor.execute_and_validate(problem_graph, plan)
            outcome.metadata['validation'] = validation_results
            logger.info("Execution completed with validation: success=%s, validated=%s",
                       outcome.success, validation_results.get('validated', False))
        else:
            outcome = self.executor.execute_plan(problem_graph, plan)
            logger.info("Execution completed: success=%s, time=%.2f",
                       outcome.success, outcome.execution_time)
        ...
        if self.safety_validator:
            ...
            if not outcome_validation['safe']:
                logger.warning("Unsafe outcome detected: %s", outcome_validation['reason'])
    ...
    def decompose_novel_problem(self, problem_graph: ProblemGraph) -> DecompositionPlan:
        ...
        self.prediction_history.append({
            'signature': signature,
            'predicted_strategy': strategy.name if hasattr(strategy, 'name') else 'unknown',
            'confidence': plan.confidence,
            'timestamp': time.time()
        })
        
        logger.info("Created decomposition plan for novel problem (complexity: %.2f, strategy: %s)", 
                   complexity, strategy.name if hasattr(strategy, 'name') else 'unknown')
        
        return plan
    
    def decompose_with_fallbacks(self, problem_graph: ProblemGraph) -> DecompositionPlan:
        """
        ...
        complexity = problem_graph.complexity_score or self._analyze_complexity(problem_graph)
        primary_plan = self._create_decomposition_plan(problem_graph, primary_strategy, complexity)
        ...
        if primary_plan.confidence >= confidence_threshold:
            ...
            logger.debug("Using primary strategy (confidence: %.2f >= %.2f)", 
                        primary_plan.confidence, confidence_threshold)
            return primary_plan
        
        # Primary is uncertain - generate ONE fallback as alternative
        logger.info("Primary confidence low (%.2f), generating fallback alternative", 
        ...
        if hasattr(best_plan, 'overall_confidence') and callable(best_plan.overall_confidence):
        ...
        # Assume it's a DecompositionPlan otherwise
            ...
            if hasattr(best_plan, 'strategy') and best_plan.strategy and hasattr(best_plan.strategy, 'name'):
                strategy_name = best_plan.strategy.name
            if hasattr(best_plan, 'confidence'):
                confidence_value = best_plan.confidence

        logger.info("Selected strategy: %s (confidence: %.2f)",
                   strategy_name,
                   confidence_value)
    ...
    def _validate_plan_safety(self, plan: DecompositionPlan, 
                             problem_graph: ProblemGraph) -> Dict[str, Any]:
        ...
        if plan.estimated_complexity > 100:
            violations.append(f"Excessive estimated complexity: {plan.estimated_complexity}")
    ...
    def _apply_outcome_corrections(self, outcome: ExecutionOutcome,
                                   validation: Dict[str, Any]) -> ExecutionOutcome:
    ...
    def _extract_problem_signature(self, problem_graph: ProblemGraph) -> ProblemSignature:
        """
    ...
    def _predict_best_strategy(self, problem_graph: ProblemGraph, 
                               signature: ProblemSignature) -> Optional[DecompositionStrategy]:
        """
        ...
        best_strategy_name = self.strategy_profiler.get_best_strategy_for_domain(signature.domain)
        ...
        if best_strategy_name:
            ...
            if best_strategy_name in self.strategy_profiler.complexity_affinity:
                ...
                complexity_affinity = self.strategy_profiler.complexity_affinity[best_strategy_name]
                ...
                if complexity_affinity.get(complexity_bucket, 0) > 0:
                    ...
                    if strategy:
                        logger.debug("Using learned strategy %s for domain %s", 
                                   best_strategy_name, signature.domain)
                        return strategy
        ...
        elif signature.has_constraints and len(signature.metadata.get('constraints', [])) > 5:
            predicted_type = 'constraint_based_decomposition'
            logger.debug("Predicted constraint-based (%d constraints)", 
        ...
        # Rule 4: Simple/low complexity
            ...
            logger.debug("Predicted direct (complexity=%.2f, nodes=%d)", 
                        signature.complexity, signature.node_count)
        ...
        # Rule 6: High branching
            ...
            logger.debug("Predicted parallel (branching=%.2f, nodes=%d)",
                        signature.branching_factor, signature.node_count)
        ...
        # Rule 8: Moderate complexity (default)
            ...
            predicted_type = 'hierarchical_decomposition'
            logger.debug("Predicted hierarchical (default, complexity=%.2f)", signature.complexity)
        ...
        if not strategy:
            ...
            logger.warning("Predicted strategy type %s not available, using fallback", predicted_type)
            # Try common types
            for fallback_type in ['hierarchical_decomposition', 'simple', 'direct_decomposition']:
    ...
    def learn_from_execution(self, problem: ProblemGraph, plan: DecompositionPlan, 
        ...
        if plan.strategy:
            ...
            strategy_name = plan.strategy.name if hasattr(plan.strategy, 'name') else 'unknown'
        ...
        logger.debug("Learned from execution: success=%s, time=%.2f",
                    outcome.success, outcome.execution_time)
    ...
    def _analyze_complexity(self, problem_graph: ProblemGraph) -> float:
        ...
        # Branching factor
        if problem_graph.root and problem_graph.root in list(G.nodes() if hasattr(G, 'nodes') else []):
            if NETWORKX_AVAILABLE:
                descendants = nx.descendants(G, problem_graph.root)
    ...
    def _select_strategy(self, problem_graph: ProblemGraph, 
                        complexity: float) -> Optional[DecompositionStrategy]:
    ...
    def _create_decomposition_plan(self, problem_graph: ProblemGraph,
                                   strategy: DecompositionStrategy,
                                   complexity: float) -> DecompositionPlan:
    ...
    def _get_available_domains(self) -> List[str]:
        ...
        # Default domains
        domains = [
            'general', 'optimization', 'classification', 'generation',
            'analysis', 'planning', 'control', 'reasoning', 'perception'
        ]
    ...
    def _calculate_gap_priority(self, problem: ProblemGraph) -> float:
        ...
        domain_frequency = len([
            h for h in self.performance_tracker.execution_history
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            'decomposition_stats': {
                ...
                'success_rate': self.successful_decompositions / max(1, self.total_decompositions),
                'cached_plans': len(self.decomposition_cache)
            },
            'execution_stats': self.executor.get_statistics() if hasattr(self.executor, 'get_statistics') else {},
            'performance_stats': {
                'complexity_accuracy': self.performance_tracker.get_complexity_estimation_accuracy(),
                'execution_history_size': len(self.performance_tracker.execution_history)
            },
            'learning_stats': {
...
class DomainSelector:
    ...
    def categorize_domains_by_data(self, domains: List[str]) -> Dict[DomainDataCategory, List[str]]:
        """
    ...
    def select_stratified_sample(self, frequent: List[str], medium: List[str], 
                                 rare: List[str]) -> List[str]:
        """
    ...
    def find_similar_domains(self, origin_domain: str, exclude: List[str] = []) -> List[str]:
        """
