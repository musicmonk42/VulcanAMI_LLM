...
try:
    from ..knowledge_crystallizer.knowledge_crystallizer_core import (
        KnowledgeCrystallizer, ExecutionTrace as CrystallizerTrace
    )
    from ..knowledge_crystallizer.principle_extractor import (
        Principle, ExecutionTrace, Metric, MetricType, Pattern, PatternType
    )
    ...
    from ..knowledge_crystallizer.knowledge_storage import (
        VersionedKnowledgeBase, KnowledgeIndex, KnowledgePruner
    )
except ImportError:
    try:
        from knowledge_crystallizer.knowledge_crystallizer_core import (
...
class DecompositionToTraceConverter:
    ...
    def convert(self, problem: ProblemGraph, plan: DecompositionPlan, 
                outcome: ExecutionOutcome) -> ExecutionTrace:
        """
    ...
    def _build_context(self, problem: ProblemGraph, plan: DecompositionPlan) -> Dict[str, Any]:
    ...
    def _extract_metrics(self, outcome: ExecutionOutcome, 
                        plan: DecompositionPlan) -> List[Metric]:
    ...
    def _detect_patterns(self, plan: DecompositionPlan, 
                        outcome: ExecutionOutcome) -> List[Pattern]:
...
class PromotionCandidate:
    ...
    def calculate_promotion_score(self):
        ...
        factors.append(('val_confidence', self.validation_results.overall_confidence, 0.05))
...
class PrinciplePromoter:
    ...
    def __init__(self, library: StratifiedDecompositionLibrary,
        ...
        logger.info("PrinciplePromoter initialized (threshold: %.2f)", promotion_threshold)
    
    def evaluate_for_promotion(self, principle: Principle,
                              validation_results: ValidationResults) -> PromotionCandidate:
        """
        ...
        logger.debug("Evaluated principle %s for promotion: score=%.2f",
                    principle.id, candidate.promotion_score)
    ...
    def promote(self, candidate: PromotionCandidate) -> bool:
        ...
        with self.lock:
            ...
            if candidate.promotion_score < self.promotion_threshold:
                ...
                logger.info("Rejected principle %s for promotion: score %.2f < threshold %.2f",
                           candidate.principle.id, candidate.promotion_score, 
            ...
            try:
                if hasattr(self.library, 'add_principle'):
                    ...
                    self.library.principles[library_principle_dict['id']] = library_principle
                else:
                ...
                logger.info("Promoted principle %s to library (score: %.2f, domains: %s)",
                           candidate.principle.id, candidate.promotion_score,
                ...
                logger.error("Failed to promote principle %s: %s", 
                           candidate.principle.id, e)
                import traceback
                traceback.print_exc()
                return False
    
    def _convert_to_library_format(self, candidate: PromotionCandidate) -> Dict[str, Any]:
        """Convert principle to library format"""
        ...
        try:
            if hasattr(principle, 'core_pattern') and principle.core_pattern is not None:
                ...
                if pattern == {'pattern_type': 'unknown'} or not isinstance(pattern, dict):
                    ...
                    for attr in ['components', 'structure', 'confidence', 'complexity', 'metadata']:
                        if hasattr(principle.core_pattern, attr):
                            ...
                            try:
                                ...
                                if isinstance(val, (list, tuple)):
                                    ...
                                    pattern_dict[attr] = {k: str(v) for k, v in val.items()}
                                else:
        ...
        library_principle = {
            ...
            'description': str(getattr(principle, 'description', 'Learned decomposition principle')),
        ...
        if hasattr(principle, 'execution_logic'):
            try:
                ...
                elif isinstance(exec_logic, (str, dict, list, int, float, bool, type(None))):
                    try:
                ...
                library_principle['execution_type'] = str(getattr(principle, 'execution_type', 'function'))
            except Exception as e:
        ...
        try:
            ...
            logger.warning("Library principle not fully serializable, applying deep cleaning: %s", e)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                'promotion_rate': self.promoted_count / max(1, self.promoted_count + self.rejected_count),
...
class PrincipleLearner:
    ...
    def __init__(self, 
        ...
        self.components_available = (self.crystallizer is not None and 
                                    self.validator is not None)
        
        if not self.components_available:
            logger.warning("Knowledge crystallizer components not available - "
                         "principle learning will be limited")
        else:
            logger.info("PrincipleLearner initialized with full capabilities")
    
    def extract_and_promote(self, problem: ProblemGraph,
        ...
        with self.lock:
            ...
            try:
                ...
                for principle in principles:
                    ...
                    self.knowledge_base.store(principle, author='system', 
                    ...
                    applicable_domains = self._get_applicable_domains(principle, problem)
                ...
                logger.info("Validated %d/%d principles successfully",
                           len(validated_principles), len(principles))
                    ...
                    for principle, validation_results in validated_principles:
                        ...
                        if self.promoter.promote(candidate):
                            ...
                            if hasattr(principle, 'core_pattern'):
                                try:
                                    if hasattr(principle.core_pattern, 'pattern_type'):
                                        if hasattr(principle.core_pattern.pattern_type, 'value'):
                                            pattern_type = principle.core_pattern.pattern_type.value
                                        elif hasattr(principle.core_pattern.pattern_type, 'name'):
                                            pattern_type = principle.core_pattern.pattern_type.name
                                        else:
                                            pattern_type = str(principle.core_pattern.pattern_type)
                ...
                logger.info("Principle learning complete: extracted=%d, validated=%d, promoted=%d",
    ...
    def prune_low_quality_principles(self, age_threshold_days: int = 90,
        ...
        with self.lock:
            stats = {
                ...
                'validation': {
                    ...
                    'validator_stats': self.validator.total_validations if self.validator else 0
                },
                'promotion': {
                ...
                'recent_learning': {
                    ...
                    'avg_extracted': np.mean([h['extracted'] for h in self.learning_history]) if self.learning_history else 0,
                    'avg_promoted': np.mean([h['promoted'] for h in self.learning_history]) if self.learning_history else 0
                }
            }
            
            # Calculate learning efficiency
            if self.learning_history:
                successful_sessions = sum(1 for h in self.learning_history if h.get('promoted', 0) > 0)
                stats['learning_efficiency'] = {
                    'successful_sessions': successful_sessions,
                    'session_success_rate': successful_sessions / len(self.learning_history)
                }
    ...
    def _get_applicable_domains(self, principle: Principle, 
                                problem: ProblemGraph) -> List[str]:
...
def integrate_principle_learning(decomposer, library: StratifiedDecompositionLibrary,
                                min_promotion_score: float = 0.7) -> PrincipleLearner:
    """
...
def test_principle_learning():
    ...
    try:
        ...
        # Create test problem
        from problem_decomposer_core import ProblemGraph, DecompositionPlan, ExecutionOutcome
        ...
        logger.info(f"  Knowledge base size: {stats['knowledge_base']['total_principles']}")
