...
class DecomposerBootstrap:
    ...
    def create_strategy_instances(self) -> Dict[str, DecompositionStrategy]:
        ...
        with self._lock:
            ...
            strategies["constraint_based_decomposition"] = (
                structural  # Constraints handled structurally
            )
            ...
            strategies["iterative_decomposition"] = (
                structural  # Iterative is structural
            )
            ...
            logger.info(
                "Created %d strategy instances with %d name mappings",
    ...
    def register_strategies_in_library(
        self,
        ...
        with self._lock:
            ...
            def get_strategy_by_type(strategy_type: str):
                if not isinstance(strategy_type, str):
                    logger.warning(
                        "get_strategy_by_type called with non-string type: %s",
                        type(strategy_type),
                    )
                    return None
            ...
            def get_strategy(strategy_name: str):
                if not isinstance(strategy_name, str):
                    logger.warning(
                        "get_strategy called with non-string name: %s",
                        type(strategy_name),
                    )
                    return None
    ...
    def populate_fallback_chain(
        self, fallback_chain: FallbackChain, strategies: List[DecompositionStrategy]
    ):
        """
        ...
        with self._lock:
            ...
            for strategy_key, cost, description in ordered_strategy_names:
                ...
                if strategy and id(strategy) not in added_strategies:
                    ...
                    added_count += 1
                    logger.debug(
                        "Added %s to fallback chain (cost=%.1f)",
                    ...
                    if not strategy:
                        logger.warning(
                            "Could not find strategy for key: %s", strategy_key
                        )
                    else:
            ...
            if added_count != len(strategies):
                logger.warning(
                    "Strategy count mismatch: added %d, expected %d",
                    added_count,
                    len(strategies),
                )
                logger.warning(
    ...
    def initialize_library_with_base_principles(
        ...
        with self._lock:
            ...
            contexts = {
                ...
                "analysis": Context(
                    domain="analysis", problem_type="exploratory", constraints={}
                ),
                "generation": Context(
                    domain="generation", problem_type="creative", constraints={}
                ),
            }
            ...
            logger.info(
                "Initialized library with %d base principles", len(principle_configs)
            )

    def configure_adaptive_thresholds(
        ...
        with self._lock:
            ...
            for threshold_name, value in initial_values.items():
                if (
                    hasattr(thresholds, "thresholds")
                    and threshold_name in thresholds.thresholds
                ):
    ...
    def create_initialized_decomposer(
        self,
        ...
        with self._lock:
            ...
            self.populate_fallback_chain(
                decomposer.fallback_chain, self.strategy_instances
            )
...
def run_bootstrap_test():
    ...
    try:
        ...
        logger.info(
            "  - Strategy count: %d", validation["checks"].get("strategy_count", 0)
        )
        logger.info(
            "  - Fallback chain: %d strategies",
            validation["checks"].get("fallback_chain_count", 0),
        )
