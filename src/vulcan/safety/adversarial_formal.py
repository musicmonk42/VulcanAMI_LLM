...
class AdversarialValidator:
    ...
    def __init__(
        ...
        # Metrics
        self.attack_metrics = defaultdict(
            lambda: {
                "attempts": 0,
                "successes": 0,
    ...
    def _initialize_attack_configs(self) -> Dict[AttackType, AttackConfig]:
        ...
        return {
            AttackType.FGSM: AttackConfig(epsilon=self.epsilon, num_iterations=1),
            AttackType.PGD: AttackConfig(
                epsilon=self.epsilon, alpha=self.epsilon / 4, num_iterations=20
            ),
            AttackType.DEEPFOOL: AttackConfig(
                epsilon=self.epsilon, max_iterations=50, early_stop=True
            ),
            AttackType.CARLINI_WAGNER: AttackConfig(
                confidence=0.0, max_iterations=100, alpha=0.01
            ),
            AttackType.JSMA: AttackConfig(epsilon=self.epsilon, max_iterations=100),
        }
    ...
    def _perturb_embedding(
        self, action: Dict[str, Any], epsilon: float = 0.1, num_perturbations: int = 5
    ) -> List[Dict[str, Any]]:
        """
    ...
    def validate_robustness(
        ...
        for attack_type in attack_types:
            ...
            attack_config = self.attack_configs.get(
                attack_type, self.default_attack_config
            )
            ...
            try:
                ...
                for i, perturbed in enumerate(perturbed_actions[: self.num_attacks]):
                    ...
                    if validator:
                        ...
                        safe, reason, confidence = self._default_validate(
                            perturbed, context
                        )
                    ...
                    if not safe:
                        ...
                        self._record_attack(
                            attack_type, action, perturbed, True, reason
                        )
                        ...
                        attack_results.append(
                            {
                                ...
                                "index": i,
                                "successful": True,
                                "perturbation_norm": self._calculate_perturbation_norm(
                                    action, perturbed
                                ),
                            }
                        )
                        ...
                        self._record_attack(
                            attack_type, action, perturbed, False, "Defense successful"
                        )
    ...
    def _fgsm_attack(
        self, action: Dict[str, Any], context: Dict[str, Any], config: AttackConfig
    ) -> List[Dict[str, Any]]:
        """
        ...
        if "embedding" in action and action["embedding"] is not None:
            ...
            for _ in range(min(2, self.num_attacks)):
                ...
                delta = np.random.uniform(
                    -config.epsilon, config.epsilon, embedding.shape
                )
    ...
    def _trojan_attack(
        ...
        for trigger in trojan_triggers[: self.num_attacks]:
            ...
            perturbed_action["trigger_type"] = (
                list(trigger.keys())[0] if trigger else "unknown"
            )
            ...
            for _ in range(min(2, self.num_attacks)):
                ...
                for iteration in range(config.max_iterations):
                    ...
                    for i in range(num_classes):
                        ...
                        if gradient_norm > 1e-10:  # FIXED: Prevent division by zero
                            ...
                            if distance < min_dist:
                                min_dist = distance
                                best_perturbation = (
                                    gradient / (gradient_norm**2) * distance
                                )
                    ...
                    if best_perturbation is not None:
                        ...
                        logger.debug(
                            f"DeepFool: No valid perturbation at iteration {iteration}"
                        )
                        break
                    ...
                    if abs(current_perturb_norm - previous_perturb_norm) < 1e-6:
                        ...
                        if no_improvement_count >= max_no_improvement:
                            logger.debug(
                                f"DeepFool stopped: no improvement for {max_no_improvement} iterations"
                            )
                            break
                    else:
                        no_improvement_count = 0
                    ...
                    if current_perturb_norm > config.epsilon * 10:
                        logger.warning(
                            f"DeepFool stopped: perturbation exploding ({current_perturb_norm:.2f})"
                        )
                        break
    ...
    def _carlini_wagner_attack(
            ...
            for _ in range(min(1, self.num_attacks)):  # C&W is expensive
                ...
                for binary_step in range(9):  # Binary search steps
                    ...
                    for iteration in range(
                        ...
                        adversarial_loss = const * max(
                            0, output_diff - config.confidence
                        )
                        ...
                        gradient = (
                            2 * delta + const * np.random.randn(*delta.shape) * 0.01
                        )
        ...
        # Update metrics (with lock)
        with self.lock:
            self.attack_metrics[AttackType.CARLINI_WAGNER]["attempts"] += len(
                perturbations
            )
            ...
            for _ in range(min(2, self.num_attacks)):
                ...
                for iteration in range(
                    ...
                    if jacobian[target_idx] > 0:
                        perturbed[target_idx] = min(
                            1, perturbed[target_idx] + config.epsilon
                        )
                    else:
                        perturbed[target_idx] = max(
                            -1, perturbed[target_idx] - config.epsilon
                        )
                    ...
                    if (
                        len(modified_indices) >= len(embedding) * 0.1
                    ):  # Max 10% of features
                        break
    ...
    def _universal_attack(
        ...
        if "embedding" in action and action["embedding"] is not None:
            ...
            universal_perturbation = universal_perturbation / (
                np.linalg.norm(universal_perturbation) + 1e-10
            )
    ...
    def _targeted_attack(
        ...
        for target in target_actions[: self.num_attacks]:
            ...
            if (
                "embedding" in perturbed_action
                and perturbed_action["embedding"] is not None
            ):
                ...
                target_direction = target_direction / (
                    np.linalg.norm(target_direction) + 1e-10
                )
    ...
    def _calculate_perturbation_norm(
        self, original: Dict[str, Any], perturbed: Dict[str, Any]
    ) -> float:
        """
    ...
    def _calculate_robustness_score(
        self, attack_results: Union[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]
    ) -> float:
        """
        ...
        if isinstance(attack_results, dict):
            ...
            for attack_type, result_data in attack_results.items():
                if isinstance(result_data, dict):
                    ...
                    successful = (
                        success_rate > 0.5
                    )  # Consider attack successful if >50% success rate
                    results_list.append(
                        {
    ...
    def _identify_vulnerability_patterns(
        self,
        ...
        if successful_attacks:
            ...
            perturbations = [
                r.get("perturbation_norm", 0)
                for r in successful_attacks
                if "perturbation_norm" in r
            ]
    ...
    def _extract_vulnerability_pattern(
        ...
        if "embedding" in original and "embedding" in perturbed:
            if (
                original.get("embedding") is not None
                and perturbed.get("embedding") is not None
            ):
                pattern["embedding_change"] = True
                pattern["perturbation_norm"] = self._calculate_perturbation_norm(
                    original, perturbed
                )
    ...
    def _suggest_mitigations(
        self, violations: List[SafetyViolationType], patterns: List[str]
    ) -> List[str]:
        """
        ...
        if SafetyViolationType.ADVERSARIAL in violations:
            mitigations.append(
                "Implement adversarial training with diverse attack samples"
            )
    ...
    def _defense_adversarial_training(self, validator: Callable) -> Callable:
        ...
        def robust_validator(action, context):
            ...
            adv_examples = self._fgsm_attack(
                action, context, self.default_attack_config
            )
    ...
    def get_attack_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            stats = {"total_attacks": len(self.attack_history), "attack_types": {}}
...
class FormalVerifier:
    ...
    def add_safety_property(
        self,
        property_fn: Callable[[Dict, Any], bool],
        name: str,
        description: str,
        priority: int = 1,
    ):
    ...
    def add_invariant(
        self, invariant_fn: Callable[[Any], bool], name: str, description: str
    ):
        """
    ...
    def add_temporal_property(
        self,
        property_fn: Callable[[List[Any]], bool],
        name: str,
        description: str,
        window: int = 10,
    ):
    ...
    def add_probabilistic_property(
        self,
        property_fn: Callable[[Any], float],
        name: str,
        description: str,
    ...
    def verify_action(
        self, action: Dict[str, Any], state: Any, method: str = "model_checking"
    ) -> SafetyReport:
        """
        ...
        with self.lock:
            ...
            if len(self.verification_cache) >= self.cache_max_size:
                # Remove oldest 20%
                sorted_items = sorted(
                    self.verification_cache.items(), key=lambda x: x[1]["timestamp"]
                )
    ...
    def _verify_model_checking(
        ...
        for prop in sorted(properties_to_check, key=lambda p: p.priority, reverse=True):
            try:
                if not prop.check_function(action, state):
                    ...
                    reasons.append(
                        f"Property '{prop.name}' violated: {prop.description}"
                    )
        ...
        for inv in invariants_to_check:
            try:
                ...
                if not inv.check_function(state):
                    violations.append(SafetyViolationType.FORMAL)
                    reasons.append(
                        f"Invariant '{inv.name}' violated: {inv.description}"
                    )
            ...
            if len(history) >= temp_prop["window"]:
                try:
                    if not prop.check_function(list(history)):
                        violations.append(SafetyViolationType.FORMAL)
                        reasons.append(
                            f"Temporal property '{prop.name}' violated: {prop.description}"
                        )
                        ...
                        with self.lock:
                            prop.verified_count += 1
                        proofs.append(
                            f"Temporal property '{prop.name}' verified over window"
                        )
        ...
        return SafetyReport(
            ...
            metadata={
                ...
                "properties_checked": len(properties_to_check)
                + len(invariants_to_check),
                "proofs": proofs[:5] if not violations else [],
                "counterexamples": len(self.counterexamples),
            },
        )

    def _verify_theorem_proving(
        self, action: Dict[str, Any], state: Any
    ) -> SafetyReport:
        """
    ...
    def _verify_symbolic_execution(
        self, action: Dict[str, Any], state: Any
    ) -> SafetyReport:
        """
    ...
    def _verify_abstract_interpretation(
        self, action: Dict[str, Any], state: Any
    ) -> SafetyReport:
        """
    ...
    def _generate_counterexample(
        self, property: FormalProperty, action: Dict[str, Any], state: Any
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_trace(
        ...
        # Add current action
        trace.append(
            {"action": action.get("type", "unknown"), "timestamp": time.time()}
        )
    ...
    def _explore_paths(
        self, constraints: List[Dict[str, Any]], max_paths: int = 100
    ) -> List[Dict[str, Any]]:
    ...
    def _check_path_satisfies_property(
        self, path: Dict[str, Any], property: FormalProperty
    ) -> bool:
    ...
    def _abstract_transition(
        self, abstract_state: Dict[str, Any], abstract_action: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _check_abstract_property(
        self, abstract_state: Dict[str, Any], property: FormalProperty
    ) -> bool:
        ...
        with self.lock:
            stats = {
                ...
                "total_verifications": sum(
                    p.verified_count + p.violation_count for p in self.properties
                ),
                "counterexamples": len(self.counterexamples),
            }
