...
class AnomalyDetector(nn.Module):
    ...
    def __init__(self, config: ModelConfig):
        ...
        for i in range(len(config.hidden_dims) - 2, -1, -1):
            decoder_layers.append(nn.Linear(config.hidden_dims[i + 1], config.hidden_dims[i]))
            decoder_layers.append(nn.ReLU())
    ...
    def _create_bayesian_layer(self, in_features: int, out_features: int) -> nn.Module:
        ...
        class BayesianLinear(nn.Module):
            def __init__(self, in_features, out_features):
                ...
                self.weight_sigma = nn.Parameter(torch.ones(out_features, in_features) * -3.0) # Init rho for small sigma
                
                # Bias parameters
                self.bias_mu = nn.Parameter(torch.zeros(out_features))
                self.bias_sigma = nn.Parameter(torch.ones(out_features) * -3.0) # Init rho for small sigma
...
class VariationalSafetyAutoencoder(nn.Module):
    ...
    def compute_loss(self, x: torch.Tensor, x_recon: torch.Tensor, 
                    ...
                    safety_pred: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
...
class NeuralSafetyValidator:
    ...
    def __init__(self, model_configs: Optional[Dict[ModelType, ModelConfig]] = None,
                 ensemble_size: int = 5, config: Optional[Dict[str, Any]] = None,
                 model_types=None, device="cpu"): # Added model_types and device for compatibility
        """
        ...
        self.model_types = model_types or ["toxicity", "bias", "risk", "perturbation", "consistency"]
        # self.device = device # Already set above
        logger.info(f"NeuralSafetyValidator initialized with {len(self.model_types)} model types on {self.device}")
    ...
    async def validate_async(self, action: Dict[str, Any], context: Dict[str, Any],
                            use_consensus: bool = True, timeout: float = 1.0) -> SafetyReport:
        """
    ...
    async def _run_consensus_inference_async(self, features: torch.Tensor) -> Dict[str, Any]:
            ...
            if ModelType.ANOMALY_DETECTOR in self.models:
                try:
                    ...
                    async def run_anomaly():
                        return float(model.compute_anomaly_score(features).cpu().numpy()[0])
    ...
    async def _run_single_inference_async(self, features: torch.Tensor) -> Dict[str, Any]:
    ...
    def validate(self, action: Dict[str, Any], context: Dict[str, Any],
                use_consensus: bool = True, timeout: float = 1.0) -> SafetyReport:
        """
    ...
    def _extract_features(self, action: Dict[str, Any], context: Dict[str, Any]) -> np.ndarray:
        ...
        try:
            ...
            action_types = ['explore', 'optimize', 'maintain', 'wait', 'emergency_stop',
            ...
            if 'resource_usage' in action and isinstance(action['resource_usage'], dict):
            ...
            if 'embedding' in action and action['embedding'] is not None:
                try:
                    embedding = np.array(action['embedding'], dtype=np.float32)[:64]  # Use first 64 dims
            ...
            if 'history' in context:
                ...
                if isinstance(history, list) and history:
                    recent_failures = sum(1 for h in history[-10:] if not h.get('safe', True))
    ...
    def _analyze_predictions(self, predictions: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'bayesian' in predictions:
            ...
            if bayesian['uncertainty'] > self.uncertainty_threshold:
                reasons.append(f"High prediction uncertainty: {bayesian['uncertainty']:.3f}")
        ...
        if 'vae' in predictions:
            ...
            if vae['reconstruction_error'] > 0.5:
                reasons.append(f"High reconstruction error: {vae['reconstruction_error']:.3f}")
        ...
        if safe_votes:
            ...
            if not is_safe:
                ...
                reasons.append(f"Consensus failed: {safe_ratio:.2f} < {self.consensus_threshold}")
        else:
            is_safe = False
            safe_ratio = 0.0
    ...
    def train(self, training_data: List[Tuple[Dict, Dict, bool]], 
        ...
        for model_type in model_types:
            ...
            if model_type == ModelType.CLASSIFIER:
                ...
                # Train transformer
                metrics = self._train_transformer(X_train, y_train, X_val, y_val, epochs)
                training_metrics[model_type.value] = metrics
    ...
    def _prepare_dataset(self, data: List[Tuple[Dict, Dict, bool]]) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _train_ensemble(self, X_train: torch.Tensor, y_train: torch.Tensor,
    ...
    def _train_anomaly_detector(self, X_train: torch.Tensor, epochs: int) -> Dict[str, Any]:
    ...
    def _train_bayesian(self, X_train: torch.Tensor, y_train: torch.Tensor,
                       X_val: Optional[torch.Tensor], y_val: Optional[torch.Tensor],
                       epochs: int) -> Dict[str, Any]:
    ...
    def _train_transformer(self, X_train: torch.Tensor, y_train: torch.Tensor,
                          X_val: Optional[torch.Tensor], y_val: Optional[torch.Tensor],
                          epochs: int) -> Dict[str, Any]:
    ...
    def _train_vae(self, X_train: torch.Tensor, y_train: torch.Tensor, 
                  epochs: int) -> Dict[str, Any]:
        ...
        with torch.set_grad_enabled(True):
            for epoch in range(epochs):
                ...
                for batch_X, batch_y in dataloader:
                    ...
                    # Compute losses
                    losses = model.compute_loss(batch_X, x_recon, mu, logvar, batch_y, safety_pred)
    ...
    def _record_inference_metrics(self, inference_time: float, 
                                 safety_decision: Dict[str, Any]):
        """Record metrics for performance tracking."""
        # Record inference time (with lock)
        with self.lock:
            for model_type in self.models.keys():
                self.performance_metrics[model_type]['inference_time'].append(inference_time * 1000)
                ...
                try:
                    if isinstance(model_dict, dict):
                        ...
                        for model_name, model in model_dict.items():
                            model_path = save_path / f"{model_type.value}_{model_name}.pth"
            ...
            for model_type, model_dict in self.models.items():
                try:
                    if isinstance(model_dict, dict):
                        # Load ensemble
                        for model_name, model in model_dict.items():
                            model_path = load_path / f"{model_type.value}_{model_name}.pth"
                        ...
                        if model_path.exists():
                            model_dict.load_state_dict(torch.load(model_path, map_location=self.device))
                except Exception as e:
    ...
    def get_model_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            stats = {
                'num_models': sum(
                    len(m) if isinstance(m, dict) else 1
            ...
            for model_type, metrics in self.performance_metrics.items():
                if metrics['inference_time']:
                    stats['performance'][model_type.value] = {
                        'avg_inference_time_ms': np.mean(list(metrics['inference_time']))
                    }
