...
class GovernanceManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.escalation_policies: Dict[str, Callable] = self._initialize_escalation_policies()
        ...
        # Metrics
        self.metrics: Dict[GovernanceLevel, Dict[str, Any]] = defaultdict(lambda: {
            'total_decisions': 0,
            'approved': 0,
            'rejected': 0,
            'escalated': 0,
    ...
    def _start_cleanup_thread(self):
        ...
        self._cleanup_thread = threading.Thread(target=cleanup_loop, daemon=True, name="GovernanceCleanup") # CHANGED
    ...
    def _cleanup_old_decisions(self):
        ...
        with self.lock:
            ...
            # Find old decisions
            old_decisions = [
                did for did, decision in self.active_decisions.items()
            ...
            if len(self.active_decisions) > self.max_active_decisions:
                ...
                logger.warning(f"Enforced max_active_decisions limit, removed {to_remove} decisions")
    ...
    def register_stakeholder(self, stakeholder_id: str, 
        ...
        with self.lock:
            ...
            if len(self.stakeholder_registry) >= self.max_stakeholders:
                logger.warning(f"Max stakeholders ({self.max_stakeholders}) reached, cannot register new stakeholder")
                return False
    ...
    def request_approval(self, action: Dict[str, Any], 
    ...
    def _select_policy(self, action: Dict[str, Any], 
                      context: Optional[Dict[str, Any]]) -> Optional[GovernancePolicy]:
        ...
        requires_human = context.get('require_human_approval', False) if context else False
        action_type = action.get('type')
        
        # Select based on risk and context
        if risk_score > 0.9 or (action_type and isinstance(action_type, ActionType) and action_type == ActionType.EMERGENCY_STOP):
    ...
    def _process_autonomous(self, decision_id: str, action: Dict[str, Any],
                          policy: GovernancePolicy) -> Dict[str, Any]:
    ...
    def _process_supervised(self, decision_id: str, action: Dict[str, Any],
                          policy: GovernancePolicy) -> Dict[str, Any]:
    ...
    def _process_human_assisted(self, decision_id: str, action: Dict[str, Any],
                               policy: GovernancePolicy) -> Dict[str, Any]:
    ...
    def _process_human_controlled(self, decision_id: str, action: Dict[str, Any],
                                 policy: GovernancePolicy) -> Dict[str, Any]:
        ...
        for stakeholder_type in policy.stakeholders:
            ...
            response = self._request_stakeholder_approval(
                decision_id, action, stakeholder_type,
                timeout=policy.timeout_seconds
            )
    ...
    def _process_committee(self, decision_id: str, action: Dict[str, Any],
                         policy: GovernancePolicy) -> Dict[str, Any]:
            ...
            if response:
                ...
                with self.lock:
                    if decision_id in self.active_decisions:
                        target_list = 'approvals' if response['approval'] else 'rejections'
    ...
    def _process_emergency(self, decision_id: str, action: Dict[str, Any],
                         policy: GovernancePolicy) -> Dict[str, Any]:
        """Process emergency governance decision."""
        # Emergency protocol - immediate conservative action
        action_type = action.get('type')
        emergency_action = (action_type and isinstance(action_type, ActionType) and
    ...
    def _request_human_feedback(self, decision_id: str, action: Dict[str, Any],
                               policy: GovernancePolicy, timeout: float,
                               ai_recommendation: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    ...
    def _request_stakeholder_approval(self, decision_id: str, action: Dict[str, Any],
                                     stakeholder_type: StakeholderType,
        ...
        with self.lock:
            stakeholders = [
                sid for sid, info in self.stakeholder_registry.items()
    ...
    def _escalate_to_committee(self, decision_id: str, reason: str):
        ...
        if committee_policy:
            ...
            self._record_escalation(escalation_id, decision_id, reason, 
                                   GovernanceLevel.COMMITTEE, result)
    ...
    def _escalate_to_emergency(self, decision_id: str, reason: str):
        ...
        # Record escalation
        self._record_escalation(escalation_id, decision_id, reason,
                               GovernanceLevel.EMERGENCY, result)
    ...
    def _escalate_to_regulatory(self, decision_id: str, reason: str):
        ...
        self._request_stakeholder_approval(
            decision_id, action, StakeholderType.REGULATOR,
            timeout=3600.0  # 1 hour timeout
        )
    
    def _record_decision(self, decision_id: str, action_id: str, 
                        policy_id: str, result: Dict[str, Any]):
        ...
        with self.db_lock:
            if self.conn is None:
                logger.error("Database connection not available, cannot record decision.")
                return
            try:
                self.conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (
                    decision_id, action_id, policy_id,
    ...
    def _record_escalation(self, escalation_id: str, decision_id: str,
                         reason: str, level: GovernanceLevel, result: Dict[str, Any]):
        """Record escalation in database."""
        with self.db_lock:
            if self.conn is None:
                logger.error("Database connection not available, cannot record escalation.")
                return
            try:
                self.conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (
                    escalation_id, decision_id, reason, level.value,
                    1, json.dumps(result), time.time()
                ))
                self.conn.commit()
            except sqlite3.Error as e:
                ...
                if total > 0:
                    stats['metrics_by_level'][level.value] = {
                        ...
                        'avg_response_time': (
                            ...
                            if metrics['average_response_time'] else 0
                        )
                    }
...
def initialize_governance():
    ...
    if _GOVERNANCE_INIT_DONE:
        logger.debug("GovernanceManager already initialized – skipping duplicate initialization cycle.")
    ...
    if not _GOVERNANCE_INIT_DONE:
        # Autonomous operation policy
        gm.add_policy(GovernancePolicy(
            policy_id="autonomous_default",
            name="Autonomous Operation",
            description="Default policy for autonomous operation",
            level=GovernanceLevel.AUTONOMOUS,
...
class ValueAlignmentSystem:
    ...
    def __init__(self, value_system: Optional[ValueSystem] = None,
        ...
        # Value learning
        self.learned_preferences: Dict[str, Dict[str, Any]] = defaultdict(lambda: {'weight': 0.5, 'samples': 0})
        ...
        # Metrics
        self.alignment_metrics: Dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))
    ...
    def check_alignment(self, action: Dict[str, Any], 
        ...
        overall_alignment = np.mean(list(alignment_scores.values())) if alignment_scores else 0
    ...
    def _calculate_value_alignment(self, action: Dict[str, Any], 
                                 value_name: str,
    ...
    def _generate_alignment_recommendation(self, violations: List[Dict[str, Any]],
        ...
        if critical_violations:
            recommendations.append("CRITICAL: Address safety and harm prevention issues immediately")
        ...
        return "; ".join(recommendations) if recommendations else "Minor adjustments recommended"
            ...
            if feedback.preferences:
                for pref_name, pref_value in feedback.preferences.items():
                    try:
                        ...
                        logger.warning(f"Invalid preference value for {pref_name}: {pref_value}")
    ...
    def get_alignment_report(self) -> Dict[str, Any]:
        ...
        with self.lock:
            recent_alignments = list(self.alignment_history)[-100:] if self.alignment_history else []
        ...
        if recent_alignments:
            ...
            report['metrics'] = {
                'average_alignment': np.mean([r['overall_alignment'] for r in recent_alignments if 'overall_alignment' in r]),
                'value_averages': {v: np.mean(s) for v, s in all_scores.items() if s},
                'violation_rate': sum(1 for r in recent_alignments if r.get('violations')) / len(recent_alignments)
            }
...
class HumanOversightInterface:
    ...
    def __init__(self, governance_manager: GovernanceManager,
        ...
        # Monitoring
        self.monitored_metrics: Dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))
    ...
    def request_human_approval(self, action: Dict[str, Any],
                              urgency: str = "normal",
                              context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def enable_human_override(self, duration_seconds: float = 3600):
        ...
        logger.warning(f"Human override enabled until {datetime.fromtimestamp(override_until)}")
        ...
        thread = threading.Thread(target=disable_override, daemon=True, name="OverrideDisable")
        thread.start()
    ...
    def get_oversight_status(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            active_alerts_count = len([a for a in self.alerts if not a.get('acknowledged')])
    ...
    def create_alert(self, alert_type: str, message: str, 
                    severity: str = "medium", data: Optional[Dict[str, Any]] = None):
    ...
    def _calculate_system_health(self) -> float:
        ...
        # Alignment health
        alignment_report = self.alignment.get_alignment_report()
        if 'metrics' in alignment_report and 'average_alignment' in alignment_report['metrics']:
...
class EnhancedNSOAligner:
    ...
    def align(self, *_, **__): return {"aligned": True, "reason": "shim"}

class SymbolicSafetyChecker:
    """Shim: present for compatibility; extend later if needed."""
    def __init__(self, *_, **__): ...
    def check(self, *_, **__): return {"safe": True, "reason": "shim"}
