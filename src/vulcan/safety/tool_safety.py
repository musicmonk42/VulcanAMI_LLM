...
class ToolSafetyManager:
    ...
    def _initialize_default_contracts(self):
        ...
        self.contracts["probabilistic"] = ToolSafetyContract(
            ...
            postconditions=[
                ...
                Condition(
                    "valid_distribution", "==", True, "Must produce valid distribution"
                ),
            ],
            invariants=[
        ...
        self.contracts["symbolic"] = ToolSafetyContract(
            ...
            postconditions=[
                Condition(
                    "proof_valid", "==", True, "Proof must be valid or low confidence"
                ),
        ...
        self.contracts["causal"] = ToolSafetyContract(
            ...
            preconditions=[
                Condition(
                    "causal_graph_valid", "==", True, "Causal graph must be valid"
                ),
            ...
            postconditions=[
                ...
                Condition(
                    "spurious_correlation", "==", False, "No spurious correlation"
                ),
            ],
            invariants=[
            ...
            veto_conditions=[
                Condition(
                    "spurious_correlation_risk",
                    "==",
                    True,
                    "No spurious correlation risk",
                ),
                Condition(
                    "simpson_paradox_detected", "==", True, "No Simpson's paradox"
                ),
            ],
            risk_score=0.4,
        ...
        self.contracts["data_processor"] = ToolSafetyContract(
            ...
            postconditions=[
                ...
                Condition(
                    "data_integrity_maintained", "==", True, "Data integrity maintained"
                ),
            ],
            invariants=[
    ...
    def register_contract(self, contract: ToolSafetyContract):
        ...
        with self.lock:
            ...
            rate_per_second = contract.max_frequency / 60.0
            capacity = contract.max_frequency
            self.rate_limiters[contract.tool_name] = TokenBucket(
                rate_per_second, capacity
            )
            logger.info(
                f"Registered safety contract for tool: {contract.tool_name} "
    ...
    def check_tool_safety(
        ...
        if tool_name not in self.contracts:
            logger.warning(
                f"No safety contract for tool: {tool_name} - using default safety"
            )
        ...
        if not vetoed:
            try:
                precond_valid, precond_failures = contract.validate_preconditions(
                    context
                )
        ...
        for resource, limit in contract.max_resource_usage.items():
            if resource in resource_usage and resource_usage[resource] > limit:
                violations.append(SafetyViolationType.TOOL_CONTRACT)
                reasons.append(
                    f"Resource limit exceeded for {tool_name}: "
        ...
        if confidence < contract.required_confidence:
            violations.append(SafetyViolationType.TOOL_CONTRACT)
            reasons.append(
                f"Insufficient confidence for {tool_name}: "
            ...
            if tool_name not in self.rate_limiters:
                if tool_name in self.contracts:
                    ...
                    self.rate_limiters[tool_name] = TokenBucket(
                        rate_per_second, contract.max_frequency
                    )
                else:
                    return True
    ...
    def _check_portfolio_invariant(self) -> bool:
        with self.lock:
            active_high_risk = sum(
                1
                for tool, contract in self.contracts.items()
    ...
    def _is_tool_active(self, tool_name: str) -> bool:
        ...
        recent = [
            h
            for h in self.usage_history[tool_name]
    ...
    def _calculate_safety_score(
        ...
        new_score = max(
            0.0, min(1.0, decayed_score - violation_penalty - history_penalty)
        )
        return new_score

    def _update_usage_history(
        self, tool_name: str, safe: bool, context: Dict[str, Any], violations: List
    ):
    ...
    def _record_veto(self, tool_name: str, reasons: List[str]):
        self.veto_history.append(
            {"tool": tool_name, "timestamp": time.time(), "reasons": reasons}
        )
    ...
    def _update_performance_metrics(
        self, tool_name: str, safe: bool, violation_count: int
    ):
        with self.lock:
    ...
    def get_tool_safety_report(self, tool_name: str) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            recent_vetos = [
                ...
                if v["tool"] == tool_name and v["timestamp"] > time.time() - 3600
            ]
            ...
            return {
                ...
                "performance": {
                    "safety_rate": perf_metrics.get("safe_uses", 0)
                    / max(1, perf_metrics.get("total_uses", 1)),
    ...
    def update_contract_risk(
        self, tool_name: str, new_risk_score: Optional[float] = None
    ):
        if self._shutdown:
            return
        ...
        with self.lock:
            if new_risk_score is not None:
                ...
                logger.info(
                    f"Updated risk score for {tool_name}: {old_score:.2f} -> {contract.risk_score:.2f}"
                )
            else:
                usage_count = len(self.usage_history[tool_name])
                if usage_count > 10:
                    ...
                    if old_score != contract.risk_score:
                        logger.info(
                            f"Auto-adjusted risk score for {tool_name}: "
    ...
    def get_global_safety_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                "average_safety_score": sum(self.safety_scores.values())
                / max(1, len(self.safety_scores)),
...
class ToolSafetyGovernor:
    ...
    def _initialize_policies(self) -> Dict[str, Any]:
        return {
            ...
            "require_approval_above_risk": self.config.get(
                "require_approval_above_risk", 0.7
            ),
            "auto_quarantine_on_failures": self.config.get(
                "auto_quarantine_on_failures", 3
            ),
    ...
    def govern_tool_selection(
        self, selection_request: Dict[str, Any], selected_tools: List[str]
        ...
        with self.lock:
            filtered_tools = [
                t for t in filtered_tools if t not in self.quarantine_list
            ]

        if len(filtered_tools) > self.governance_policies["max_parallel_tools"]:
            filtered_tools = self._select_best_tools(
                filtered_tools, self.governance_policies["max_parallel_tools"]
            )
    ...
    def validate_execution_result(
        ...
        valid, failures = self.tool_safety_manager.check_postconditions(
            tool_name, result
        )
        if not valid:
    ...
    def trigger_emergency_stop(self, reason: str):
        ...
        with self.lock:
            ...
            self.governance_history.append(
                {"timestamp": time.time(), "event": "emergency_stop", "reason": reason}
            )
    ...
    def clear_emergency_stop(self, authorized_by: str):
        ...
        with self.lock:
            ...
            self.emergency_stop_reason = None
            logger.info(
                f"Emergency stop cleared by: {authorized_by} (was: {previous_reason})"
            )
            self.governance_history.append(
                {
                    "timestamp": time.time(),
    ...
    def quarantine_tool(
        ...
        with self.lock:
            ...
            logger.warning(
                f"Tool {tool_name} quarantined for {duration_seconds}s: {reason}"
            )
            self.governance_history.append(
                {
                    "timestamp": time.time(),
        ...
        thread = threading.Thread(
            target=remove_quarantine, daemon=True, name=f"Quarantine-{tool_name}"
        )
        thread.start()
        with self.lock:
            self.quarantine_threads.append(thread)
            self.quarantine_threads = [
                t for t in self.quarantine_threads if t.is_alive()
            ]
    ...
    def _select_best_tools(self, tools: List[str], max_count: int) -> List[str]:
        scored_tools = [
            (t, self.tool_safety_manager.safety_scores.get(t, 0.5)) for t in tools
        ]
    ...
    def _track_tool_failure(self, tool_name: str):
        ...
        with self.lock:
            recent_failures = sum(
                ...
                and record.get("event") == "execution_failure"
                and record.get("timestamp", 0) > time.time() - 3600
            )
        if recent_failures >= failure_threshold:
            self.quarantine_tool(
                tool_name, f"Auto-quarantined after {recent_failures} failures"
            )
    ...
    def get_governance_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            total_vetos = sum(len(h.get("vetoed_tools", [])) for h in recent_history)
            approval_rate = self.governance_metrics["approved_requests"] / max(
                1, self.governance_metrics["total_requests"]
            )
            return {
    ...
    def update_governance_policy(self, policy_name: str, value: Any):
        ...
        with self.lock:
            if policy_name in self.governance_policies:
                ...
                logger.info(
                    f"Updated governance policy {policy_name}: {old_value} -> {value}"
                )
                self.governance_history.append(
                    {
                        "timestamp": time.time(),
