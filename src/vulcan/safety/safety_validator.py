...
try:
    ...
    logger.warning("Could not import SafetyLevel from common_types. This may be expected if the file is new.")
    SafetyLevel = None

# Lazy-load modules to avoid circular imports
try:
    from .tool_safety import ToolSafetyManager, ToolSafetyGovernor, initialize_tool_safety
except ImportError:
...
try:
    from .adversarial_formal import AdversarialValidator, FormalVerifier, initialize_adversarial
except ImportError:
...
try:
    ...
    import scipy
    import statsmodels
    logger.info(f"Neural dependencies checked: torch v{torch.__version__}, scipy v{scipy.__version__}, statsmodels v{statsmodels.__version__}")
    from .neural_safety import SafetyPredictor, FeatureExtractor, initialize_neural_safety
    logger.info("Neural safety modules (SafetyPredictor, FeatureExtractor, initialize_neural_safety) loaded.")
...
try:
    from .llm_validators import ToxicityValidator, HallucinationValidator, PromptInjectionValidator, StructuralValidator, EthicalValidator
    LLM_VALIDATORS_AVAILABLE = True
    logger.info("LLM safety validator modules loaded.")
except ImportError:
    class StructuralValidator:
        def check(self, token, context): return True
    class EthicalValidator:
        def check(self, token, context): return True
    class ToxicityValidator:
        def check(self, token, context): return True
        def get_safe_alternative(self, token, context): return " [SAFE_TOKEN] "
    class HallucinationValidator:
        def check(self, token, context): return True
        def get_safe_alternative(self, token, context): return " [CORRECTION] "
    class PromptInjectionValidator:
        def check(self, token, context): return True
        def get_safe_alternative(self, token, context): return " [REDACTED] "
    LLM_VALIDATORS_AVAILABLE = True  # Provide mock implementations so available = True for system continuity
...
class ConstraintManager:
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.constraint_metrics = defaultdict(lambda: {
            'checks': 0,
            'violations': 0,
            'last_checked': None
        })
    ...
    def check_constraints(self, action: Dict[str, Any], context: Dict[str, Any]) -> SafetyReport:
        violations = []
        reasons = []
        min_confidence = 1.0
        compliance_checks = {}
        ...
        for constraint in sorted_constraints:
            try:
                ...
                with self.lock:
                    ...
                    self.constraint_metrics[constraint.name]['last_checked'] = time.time()

                if not passed:
                    violations.append(SafetyViolationType.OPERATIONAL)
                    reasons.append(f"Constraint '{constraint.name}' violated: {constraint.description}")
                    with self.lock:
                        self.constraint_violations[constraint.name] += 1
                        self.constraint_metrics[constraint.name]['violations'] += 1
                    if constraint.type == 'hard':
                        min_confidence = 0.0
                        reasons.append(f"Hard constraint '{constraint.name}' failed - action blocked")
                        break
        ...
        with self.lock:
            self.constraint_history.append({
                ...
                'violated_constraints': [c.name for c in sorted_constraints
    ...
    def get_constraint_stats(self) -> Dict[str, Any]:
        with self.lock:
            ...
            for constraint in self.constraints:
                ...
                violation_rate = (metrics['violations'] / max(1, metrics['checks'])) if metrics['checks'] > 0 else 0
            ...
            stats['constraint_details'].sort(key=lambda x: x['violation_rate'], reverse=True)
        return stats
...
class EnhancedExplainabilityNode(ExplainabilityNode):
    ...
    def _get_interpretability_engine(self):
        ...
        if self._interpretability_engine_class is None:
            try:
                ...
                InterpretabilityEngine = getattr(interp_mod, "InterpretabilityEngine", None)
                self._interpretability_engine_class = InterpretabilityEngine
                if InterpretabilityEngine:
                    logger.info("InterpretabilityEngine lazy-loaded by ExplainabilityNode.")
                else:
                    logger.warning("InterpretabilityEngine not found in module src.interpretability_engine.")
            except Exception as e:
                logger.warning(f"InterpretabilityEngine lazy-load failed: {e}")
                self._interpretability_engine_class = "failed"
        if self._interpretability_engine_class and self._interpretability_engine_class != "failed":
            try:
    ...
    def execute(self, data: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if engine and 'tensor' in data:
            try:
                ...
                explanation['feature_importance'] = self._extract_feature_importance(shap_scores)
            except Exception as e:
        ...
        if quality_score < self.min_quality_threshold:
            ...
            explanation['quality_score'] = self.explanation_quality_scorer.score(explanation)
    ...
    def _extract_feature_importance(self, shap_scores: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(shap_scores, (list, np.ndarray)):
            for i, score in enumerate(shap_scores[:10]):
                features.append({'feature': f'feature_{i}', 'importance': float(score), 'rank': i + 1})
        return features
    ...
    def _generate_alternatives(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if decision in [ActionType.EXPLORE, 'explore']:
            alternatives = [
                {'action': 'optimize', 'reason': 'Could improve efficiency by 20-30%', 'risk': 'May miss new opportunities', 'confidence': 0.7},
                {'action': 'maintain', 'reason': 'Preserves system stability', 'risk': 'No performance improvement', 'confidence': 0.9}
            ]
        elif decision in [ActionType.OPTIMIZE, 'optimize']:
            alternatives = [
                {'action': 'explore', 'reason': 'Could discover better solutions', 'risk': 'Temporary performance decrease', 'confidence': 0.6},
                {'action': 'safe_fallback', 'reason': 'Guarantees safety', 'risk': 'Suboptimal performance', 'confidence': 0.95}
            ]
        else:
            alternatives = [
                {'action': 'wait', 'reason': 'Gather more information', 'risk': 'Delayed decision', 'confidence': 0.5},
                {'action': 'safe_fallback', 'reason': 'Minimize risk', 'risk': 'Conservative approach', 'confidence': 0.8}
            ]
        return alternatives

    def _compute_explanation_confidence(self, explanation: Dict[str, Any]) -> float:
        confidence = 0.5
        if explanation.get('feature_importance'): confidence += 0.15
        if explanation.get('shap_scores') is not None: confidence += 0.15
        if explanation.get('context'): confidence += 0.1
        if explanation.get('alternatives'): confidence += 0.1
        if explanation.get('decision_factors'): confidence += 0.1
    ...
    def _identify_decision_factors(self, data: Dict[str, Any]) -> List[str]:
        ...
        if 'features' in data and isinstance(data['features'], dict):
            sorted_features = sorted(
                [(k, v) for k, v in data['features'].items() if isinstance(v, (int, float))],
                key=lambda x: abs(x[1]), reverse=True
            )
            factors.extend([name for name, _ in sorted_features[:5]])
        if 'safety_report' in data:
            if not data['safety_report'].get('safe', True): factors.append('safety_violation_detected')
            if data['safety_report'].get('confidence', 0) < 0.5: factors.append('low_confidence')
        if 'violated_constraints' in data:
            factors.extend([f"constraint_{c}" for c in data['violated_constraints'][:3]])
        return factors

    def _analyze_contributing_factors(self, data: Dict[str, Any]) -> Dict[str, Any]:
        analysis = {'positive_factors': [], 'negative_factors': [], 'neutral_factors': []}
        if 'features' in data and isinstance(data['features'], dict):
            for feature, value in data['features'].items():
                if isinstance(value, (int, float)):
                    if value > 0.5:
                        analysis['positive_factors'].append({'factor': feature, 'contribution': value})
                    elif value < -0.5:
                        analysis['negative_factors'].append({'factor': feature, 'contribution': value})
                    else:
                        analysis['neutral_factors'].append({'factor': feature, 'contribution': value})
        for factor_list in analysis.values():
            factor_list.sort(key=lambda x: abs(x['contribution']), reverse=True)
        return analysis

    def _improve_explanation(self, explanation: Dict[str, Any], data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'explanation_summary' in explanation:
            explanation['explanation_summary'] = self._enhance_summary(explanation['explanation_summary'], data)
        return explanation
    ...
    def _generate_detailed_reasoning(self, data: Dict[str, Any]) -> str:
        ...
        if 'constraints' in data:
            reasoning_parts.append(f"Constraints considered: {len(data['constraints'])}")
        if 'safety_report' in data:
            safety = data['safety_report']
            reasoning_parts.append(f"Safety assessment: {'PASS' if safety.get('safe', True) else 'FAIL'} (confidence: {safety.get('confidence', 0):.2f})")
        return " ".join(reasoning_parts)

    def _generate_step_by_step(self, data: Dict[str, Any]) -> List[str]:
        steps = ["1. Received input data and context", "2. Evaluated safety constraints and requirements"]
        if 'features' in data: steps.append(f"3. Analyzed {len(data['features'])} features")
        if 'constraints' in data: steps.append(f"4. Checked {len(data['constraints'])} constraints")
        steps.extend(["5. Compared alternatives and selected optimal action", "6. Validated decision against safety policies", "7. Generated this explanation"])
        return steps
    ...
    def _generate_examples(self, data: Dict[str, Any]) -> List[str]:
        ...
        decision = data.get('decision', 'unknown')
        if decision == 'explore':
            examples = ["Similar to exploring new routes to find shortcuts", "Like trying different recipes to improve a dish"]
        elif decision == 'optimize':
            examples = ["Similar to fine-tuning a musical instrument", "Like adjusting a recipe you already know works"]
        elif decision == 'maintain':
            examples = ["Similar to keeping a steady pace in a marathon", "Like maintaining a proven routine"]
        return examples[:2]

    def _enhance_summary(self, summary: str, data: Dict[str, Any]) -> str:
        enhanced = summary
        if 'confidence' not in summary.lower():
            confidence = data.get('confidence', self._compute_explanation_confidence({}))
            enhanced += f" (Confidence: {confidence:.2f})"
        if 'safety_report' in data and 'safety' not in summary.lower():
            safety_status = "safe" if data['safety_report'].get('safe', True) else "requires review"
            enhanced += f" Safety status: {safety_status}."
        return enhanced

    def _assess_complexity(self, data: Dict[str, Any]) -> str:
        complexity_score = 0
        if 'features' in data: complexity_score += min(3, len(data.get('features', {})) / 10)
        if 'constraints' in data: complexity_score += min(2, len(data.get('constraints', [])) / 5)
        if 'alternatives' in data: complexity_score += min(1, len(data.get('alternatives', [])) / 3)
        if complexity_score < 2: return 'low'
        elif complexity_score < 4: return 'medium'
        else: return 'high'
...
class ExplanationQualityScorer:
    def __init__(self):
        ...
        self.quality_thresholds = {'excellent': 0.8, 'good': 0.6, 'acceptable': 0.4, 'poor': 0.2}
        self.lock = threading.RLock()
        self._shutdown = False
    ...
    def score(self, explanation: Dict[str, Any]) -> float:
        if self._shutdown:
            return 0.5
        score = 0.0
        weights = {'completeness': 0.3, 'clarity': 0.25, 'relevance': 0.25, 'usefulness': 0.2}
        completeness = self._score_completeness(explanation); score += completeness * weights['completeness']
        clarity = self._score_clarity(explanation); score += clarity * weights['clarity']
        relevance = self._score_relevance(explanation); score += relevance * weights['relevance']
        usefulness = self._score_usefulness(explanation); score += usefulness * weights['usefulness']
        with self.lock:
            self.scoring_history.append({'timestamp': time.time(), 'score': score, 'completeness': completeness, 'clarity': clarity, 'relevance': relevance, 'usefulness': usefulness})
        return min(1.0, score)

    def _score_completeness(self, explanation: Dict[str, Any]) -> float:
        completeness = 0.0
        if 'explanation_summary' in explanation: completeness += 0.2
        if 'method' in explanation: completeness += 0.1
        if 'context' in explanation: completeness += 0.15
        if 'alternatives' in explanation: completeness += 0.15
        if 'confidence' in explanation: completeness += 0.1
        if 'decision_factors' in explanation: completeness += 0.15
        if 'visual_aids' in explanation: completeness += 0.05
        if 'feature_importance' in explanation or 'shap_scores' in explanation: completeness += 0.1
        return completeness
    ...
    def _score_clarity(self, explanation: Dict[str, Any]) -> float:
        ...
        if 'explanation_summary' in explanation:
            ...
            if 50 <= summary_len <= 500: clarity += 0.2
            elif summary_len > 500: clarity -= 0.1
            else: clarity -= 0.2
            if '.' in summary: clarity += 0.1
        ...
        if 'step_by_step' in explanation: clarity += 0.1
        return min(1.0, max(0.0, clarity))

    def _score_relevance(self, explanation: Dict[str, Any]) -> float:
        relevance = 0.5
        if 'decision_factors' in explanation and explanation['decision_factors']: relevance += 0.25
        if 'contributing_factors' in explanation: relevance += 0.15
        if 'context' in explanation and 'safety_context' in explanation['context']: relevance += 0.1
        return min(1.0, relevance)

    def _score_usefulness(self, explanation: Dict[str, Any]) -> float:
        usefulness = 0.3
        if 'alternatives' in explanation and explanation['alternatives']: usefulness += 0.25
        if 'examples' in explanation and explanation['examples']: usefulness += 0.15
        if 'confidence' in explanation:
            confidence = explanation['confidence']
            if confidence > 0.8: usefulness += 0.2
            elif confidence > 0.6: usefulness += 0.1
        if 'visual_aids' in explanation: usefulness += 0.1
    ...
    def get_scoring_stats(self) -> Dict[str, Any]:
        with self.lock:
            ...
            distribution = {category: sum(1 for s in scores if self.get_quality_category(s) == category) for category in self.quality_thresholds}
            return {
    ...
    def _calculate_trend(self) -> str:
        with self.lock:
            ...
            older = list(self.scoring_history)[-20:-10] if len(self.scoring_history) >= 20 else []
            if not older:
                return 'insufficient_data'
            recent_avg = sum(h['score'] for h in recent) / len(recent)
            older_avg = sum(h['score'] for h in older) / len(older)
            if recent_avg > older_avg * 1.1: return 'improving'
            elif recent_avg < older_avg * 0.9: return 'declining'
            else: return 'stable'
...
class EnhancedSafetyValidator(SafetyValidator):
    ...
    def _initialize_llm_validators(self):
        if LLM_VALIDATORS_AVAILABLE:
            try:
                ...
                logger.info(f"Initialized {len(self.llm_validators)} LLM-specific validators.")
                class MockWorldModel:
                    def validate_generation(self, token, context): return True
                    def suggest_correction(self, token, context): return " [WORLD_MODEL_CORRECTION] "
    ...
    def _load_governance_modules(self):
        ...
        if self._governance_modules is None:
            try:
                gov_mod = importlib.import_module(".governance_alignment", package=__package__)
                ...
                logger.info("Governance modules (AdaptiveGovernance, EnhancedNSOAligner, SymbolicSafetyChecker) lazy-loaded.")
            except Exception as e:
        ...
        if self._security_nodes_class is None:
            try:
                ...
                if SecurityNodes:
                    ...
                    logger.warning("SecurityNodes not found in module src.security_nodes.")
            except Exception as e:
                logger.warning(f"SecurityNodes lazy-load failed: {e}")
                self._security_nodes_class = "failed"
        return self._security_nodes_class if self._security_nodes_class != "failed" else None
    ...
    def _get_interpretability_engine_class(self):
        ...
        if self._interpretability_engine_class is None:
            try:
                ...
                logger.warning(f"InterpretabilityEngine lazy-load failed: {e}")
                self._interpretability_engine_class = "failed"
        return self._interpretability_engine_class if self._interpretability_engine_class != "failed" else None
    ...
    def _initialize_components(self):
            ...
            if RollbackManager:
                try:
                    self.rollback_manager = RollbackManager(
                        max_snapshots=self.safety_config.rollback_config['max_snapshots']
                    )
                except Exception as e:
        ...
        try:
            self.adaptive_governance = _AdaptiveGovernance() if _AdaptiveGovernance else None
        except Exception as e:
        ...
        try:
            self.symbolic_checker = _SymbolicSafetyChecker() if _SymbolicSafetyChecker else None
        except Exception as e:
        ...
        try:
            self.interpretability_engine = _InterpretabilityEngine() if _InterpretabilityEngine else None
        except Exception as e:
    ...
    def _setup_default_constraints(self):
        self._safe_add_constraint(SafetyConstraint(
            name="energy_conservation",
            type="hard",
            check_function=lambda a, c: (a.get('resource_usage', {}).get('energy_nJ', 0) <= c.get('energy_budget', float('inf')), 0.9),
            threshold=0.0,
            priority=10,
            active=True,
        ...
        self._safe_add_constraint(SafetyConstraint(
            ...
            type="soft",
            check_function=lambda a, c: (a.get('uncertainty', 0) <= self.safety_config.safety_thresholds['uncertainty_max'], 1.0 - a.get('uncertainty', 0)),
        ...
        self._safe_add_constraint(SafetyConstraint(
            ...
            type="soft",
            check_function=lambda a, c: (a.get('confidence', 0) >= self.safety_config.safety_thresholds['confidence_min'], a.get('confidence', 0)),
        ...
        self._safe_add_constraint(SafetyConstraint(
            ...
            type="hard",
            check_function=lambda a, c: (all(a.get('resource_usage', {}).get(r, 0) <= limit for r, limit in c.get('resource_limits', {}).items()), 0.95),
            threshold=0.0,
            priority=9,
            active=True,
    ...
    def _setup_formal_properties(self):
        if not self.formal_verifier:
            return
        try:
            self._safe_add_property(lambda a, s: a.get('safe', True), "basic_safety", "Action must be marked as safe")
            self._safe_add_property(lambda a, s: not (a.get('explore', False) and a.get('exploit', False)), "action_consistency", "Cannot explore and exploit simultaneously")
            self._safe_add_invariant(lambda s: s.get('system_stable', True), "system_stability", "System must remain stable")
        except Exception as e:
    ...
    def validate_generation(self, token: Any, context: Dict[str, Any], world_model: Optional[Any] = None) -> Any:
        if self._shutdown:
            return token
        ...
        for validator in validators_to_use:
            try:
                ...
                logger.error(f"LLM validator {validator.__class__.__name__} failed: {e}")
                return " [VALIDATION_ERROR] "
        world_model_to_use = world_model if world_model is not None else self.world_model
    ...
    async def validate_action_comprehensive_async(
        ...
        if create_snapshot and self.rollback_manager:
            try:
                snapshot_id = self.rollback_manager.create_snapshot(
                    context.get('state', {}),
                    context.get('action_log', [])
                )
            except Exception as e:
                logger.error(f"Failed to create snapshot: {e}")

        tasks = []
        basic_safe, basic_reason, basic_confidence = self.validate_action(action, context)
        ...
        try:
            ...
            combined_report.metadata['validation_time_ms'] = (time.time() - start_time) * 1000

            if self.safety_predictor:
                future_safety = await self._run_with_timeout(
                    self._predict_safety_async,
                    timeout_per_validator,
                    action, context
                )
            ...
            if self.audit_logger:
                try:
                    audit_id = self.audit_logger.log_safety_decision(action, combined_report)
    ...
    async def _check_tool_safety_async(self, tool_name: str, context: Dict[str, Any]):
        ...
        return await loop.run_in_executor(
            None,
            self.tool_safety_manager.check_tool_safety,
            tool_name, context
        )

    async def _check_compliance_async(self, action: Dict[str, Any], context: Dict[str, Any],
                                      compliance_standards: List[ComplianceStandard]):
        loop = asyncio.get_event_loop()
        def sync_compliance():
            compliance_result = self.compliance_mapper.check_compliance(action, context, compliance_standards)
            return SafetyReport(
                safe=compliance_result['compliant'],
                confidence=0.9 if compliance_result['compliant'] else 0.3,
                violations=[] if compliance_result['compliant'] else [SafetyViolationType.COMPLIANCE],
    ...
    async def _detect_bias_async(self, action: Dict[str, Any], context: Dict[str, Any]):
        ...
        def sync_bias():
            ...
            return SafetyReport(
                ...
                violations=[] if not bias_result['bias_detected'] else [SafetyViolationType.BIAS],
    ...
    async def _validate_adversarial_async(self, action: Dict[str, Any], context: Dict[str, Any]):
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None,
    ...
    async def _verify_formal_async(self, action: Dict[str, Any], context: Dict[str, Any]):
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None,
            self.formal_verifier.verify_action,
            action, context.get('state', {})
        )

    async def _predict_safety_async(self, action: Dict[str, Any], context: Dict[str, Any]):
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None,
            self.safety_predictor.predict_safety,
            action, context
        )

    def validate_action_comprehensive(self, action: Dict[str, Any],
                                      context: Dict[str, Any],
                                      **kwargs) -> SafetyReport:
        if self._shutdown:
            return SafetyReport(
        ...
        if loop is not None and loop.is_running():
            ...
            return asyncio.run(self.validate_action_comprehensive_async(action, context, **kwargs))

    def _validate_action_comprehensive_sync(self, action: Dict[str, Any],
                                            context: Dict[str, Any],
                                            compliance_standards: Optional[List[ComplianceStandard]] = None,
                                            create_snapshot: bool = True) -> SafetyReport:
        if compliance_standards is None:
            compliance_standards = self.safety_config.compliance_standards
        snapshot_id = None
        if create_snapshot and self.rollback_manager:
            try:
                snapshot_id = self.rollback_manager.create_snapshot(context.get('state', {}), context.get('action_log', []))
            except Exception as e:
                logger.error(f"Failed to create snapshot: {e}")

        reports = []
        basic_safe, basic_reason, basic_confidence = self.validate_action(action, context)
        reports.append(SafetyReport(safe=basic_safe, confidence=basic_confidence, reasons=[basic_reason] if not basic_safe else []))
        constraint_report = self.constraint_manager.check_constraints(action, context)
        reports.append(constraint_report)

        if self.tool_safety_manager and 'tool_name' in action:
            try:
                tool_safe, tool_report = self.tool_safety_manager.check_tool_safety(action['tool_name'], context)
        ...
        if self.adversarial_validator and self.safety_config.enable_adversarial_testing:
            try:
                adv_report = self.adversarial_validator.validate_robustness(action, context, self.validate_action)
        ...
        if self.formal_verifier:
            try:
                formal_report = self.formal_verifier.verify_action(action, context.get('state', {}))
        ...
        try:
            explanation = self.explainability_node.execute({'action': action, 'context': context, 'safety_report': combined_report.to_audit_log()}, {})
            combined_report.metadata['explanation'] = explanation
        except Exception as e:
        ...
        with self.lock:
            self.validation_history.append({'timestamp': time.time(), 'action_type': action.get('type', 'unknown'), 'safe': combined_report.safe, 'confidence': combined_report.confidence, 'violations': [v.value for v in combined_report.violations]})
        return combined_report

    def validate_causal_edge(self, cause: str, effect: str, strength: float,
                             context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        if not self.causal_validator:
            return {'safe': True, 'reason': 'Causal validator not available'}
        try:
            result = self.causal_validator.validate_causal_edge(cause, effect, strength, context)
            return result.to_dict()
        except Exception as e:
            logger.error(f"Causal edge validation failed: {e}")
            return {'safe': False, 'reason': f"Validation error: {str(e)}"}

    def validate_causal_path(self, nodes: List[str], strengths: List[float],
                             context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        if not self.causal_validator:
            return {'safe': True, 'reason': 'Causal validator not available'}
        try:
            result = self.causal_validator.validate_causal_path(nodes, strengths, context)
            return result.to_dict()
        except Exception as e:
            logger.error(f"Causal path validation failed: {e}")
            return {'safe': False, 'reason': f"Validation error: {str(e)}"}

    def validate_prediction_comprehensive(self, expected: float, lower: float, upper: float,
        ...
        try:
            ...
            result = self.prediction_validator.validate_prediction(expected, lower, upper, variable, context)
            return result.to_dict()
        except Exception as e:
    ...
    def validate_pattern(self, pattern) -> Dict[str, Any]:
            ...
            for action in pattern.actions:
                if isinstance(action, dict) and action.get('type'):
                    try:
                        ...
                        action_check = self.validate_action_comprehensive(validated_action, minimal_context)
                        if not action_check.safe:
                            violations.append({'action': str(action), 'reason': ', '.join(action_check.reasons)})
                    except Exception as e:
        ...
        if hasattr(pattern, 'metadata'):
            ...
            if metadata.get('reward', 0) < -10:
                violations.append({'reason': f"Pattern has large negative reward: {metadata['reward']}"})
        if violations:
            return {'safe': False, 'reason': f"Pattern contains {len(violations)} unsafe elements", 'violations': violations}
        return {'safe': True}
            ...
            if hasattr(subproblem, 'actions'):
                for action in subproblem.actions:
                    try:
                        ...
                        if not action_check.safe:
                            violations.append({'subproblem_index': i, 'subproblem': str(subproblem), 'reason': ', '.join(action_check.reasons)})
                    except Exception as e:
                        logger.error(f"Action validation failed: {e}")
            if hasattr(subproblem, 'constraints'):
                for constraint in subproblem.constraints:
                    if constraint.get('type') == 'forbidden':
                        violations.append({'subproblem_index': i, 'reason': f"Forbidden constraint detected: {constraint}"})
        if 'steps' in plan:
            for step_idx, step in enumerate(plan['steps']):
                try:
                    step_check = self.validate_action_comprehensive(step, {})
                    if not step_check.safe:
                        violations.append({'step_index': step_idx, 'step': str(step), 'reason': ', '.join(step_check.reasons)})
                except Exception as e:
                    logger.error(f"Step validation failed: {e}")
        if 'resources' in plan:
            resources = plan['resources']
            if resources.get('energy', 0) > 100000:
                violations.append({'reason': f"Plan requires excessive energy: {resources['energy']}"})
        if violations:
            return {'safe': False, 'reason': f"Decomposition contains {len(violations)} unsafe elements", 'violations': violations}
        return {'safe': True}
        ...
        if data.get('type') == 'real_world':
            if 'intervened_variable' in data:
                ...
                if not var_check['safe']:
                    violations.append({'variable': var_name, 'value': var_value, 'reason': var_check['reason']})
        if 'outcomes' in data:
            for outcome in data['outcomes']:
                if isinstance(outcome, dict):
                    if outcome.get('harm_level', 0) > 0.5:
                        violations.append({'outcome': outcome, 'reason': f"Harmful outcome detected: harm_level={outcome['harm_level']}"})
        if data.get('reversible', True) == False:
            if not data.get('approved', False):
                violations.append({'reason': 'Irreversible intervention without approval'})
        if violations:
            return {'safe': False, 'reason': f"Intervention data contains {len(violations)} unsafe elements", 'violations': violations}
        return {'safe': True}
    ...
    def analyze_outcome_safety(self, outcome) -> Dict[str, Any]:
        ...
        if hasattr(outcome, 'metadata'):
            ...
            if metadata.get('error', False):
                violations.append(f"Outcome contains error: {metadata.get('error_message', 'unknown')}")
    ...
    def validate_problem(self, problem) -> Dict[str, Any]:
        ...
        if hasattr(problem, 'constraints'):
            for constraint in problem.constraints:
                if constraint.get('type') == 'forbidden':
                    violations.append(f"Problem contains forbidden constraint: {constraint}")
        ...
        if hasattr(problem, 'complexity_score'):
            if problem.complexity_score > 100:
                violations.append(f"Problem complexity too high: {problem.complexity_score}")
        if violations:
            return {'safe': False, 'reason': '; '.join(violations), 'violations': violations}
        return {'safe': True}
        ...
        if var_name in self.safe_regions:
            ...
            if isinstance(value, (int, float)):
                if not (min_val <= value <= max_val):
                    violations.append(f"Variable {var_name}={value} outside safe region [{min_val}, {max_val}]")
        if isinstance(value, (int, float)):
            if abs(value) > 1e10:
                violations.append(f"Variable {var_name} has extremely large value: {value}")
        if violations:
            return {'safe': False, 'reason': '; '.join(violations)}
        return {'safe': True}

    def validate_state_vector(self, state_vector: np.ndarray, variable_order: List[str]) -> Dict[str, Any]:
        ...
        for i, var_name in enumerate(variable_order):
            if i < len(state_vector):
                ...
                if not var_check['safe']:
                    violations.append(f"Variable {var_name}[{i}]: {var_check['reason']}")
    ...
    def validate_effect_magnitude(self, from_node: str, to_node: str, effect: float) -> Dict[str, Any]:
        violations = []
        ...
        if 'harm' in edge_key.lower() and effect > 0:
            ...
            safe_value = 0.0
        if violations:
            return {'safe': False, 'reason': '; '.join(violations), 'safe_value': safe_value}
    ...
    def validate_prediction_value(self, value: float, variable: str) -> Dict[str, Any]:
        ...
        if variable in self.safe_regions:
            ...
            if not (min_val <= value <= max_val):
                violations.append(f"Predicted value {value} for {variable} outside safe region [{min_val}, {max_val}]")
    ...
    def validate_intervention(self, cause: str, effect: str, intervention_type: str, metadata: Dict = None) -> Dict[str, Any]:
        violations = []
        ...
        if intervention_type == 'real_world' and metadata and not metadata.get('approved', False):
        ...
        for unsafe_pattern in self.unsafe_causal_patterns:
            if unsafe_pattern in edge_pattern:
                violations.append(f"Intervention matches unsafe pattern: {unsafe_pattern}")
        if metadata:
            if metadata.get('risk_level') == 'critical':
                violations.append("Intervention has critical risk level")
            if metadata.get('reversible', True) == False and not metadata.get('emergency', False):
                violations.append("Irreversible intervention requires emergency authorization")
        if violations:
            return {'safe': False, 'reason': '; '.join(violations), 'violations': violations}
        return {'safe': True}
    ...
    def analyze_observation_safety(self, observation) -> Dict[str, Any]:
        ...
        if hasattr(observation, 'confidence'):
            if observation.confidence < 0.3:
                violations.append(f"Observation has low confidence: {observation.confidence}")
        if hasattr(observation, 'intervention_data') and observation.intervention_data:
            intervention_check = self.validate_intervention_data(observation.intervention_data)
            if not intervention_check['safe']:
                violations.append(f"Unsafe intervention data: {intervention_check['reason']}")
        if violations:
            return {'safe': False, 'reason': '; '.join(violations), 'violations': violations}
        return {'safe': True}
    ...
    def validate(self, graph: Dict[str, Any]) -> SafetyReport:
        ...
        if 'nodes' in graph:
            ...
            try:
                ...
                validation_results['node_types'] = 'invalid' if invalid_nodes else 'valid'
                if invalid_nodes:
                    report.violations.append(SafetyViolationType.OPERATIONAL)
    ...
    def validate_tool_selection(self, tool_names: List[str], context: Dict[str, Any]) -> Tuple[List[str], SafetyReport]:
        ...
        try:
            ...
            allowed_tools, governance_result = self.tool_safety_governor.govern_tool_selection(selection_request, tool_names)
            ...
            report = SafetyReport(
                ...
                violations=[SafetyViolationType.TOOL_VETO] if veto_report_data.get('tool_vetoes') else [],
            ...
            if self.audit_logger:
                try:
                    self.audit_logger.log_safety_decision({'tool_selection': tool_names, 'context': context}, report)
                except Exception as e:
                    logger.error(f"Audit logging failed: {e}")
            return allowed_tools, report
        except Exception as e:
            logger.error(f"Tool selection validation failed: {e}")
            return tool_names, SafetyReport(safe=True, confidence=0.5, reasons=[f"Validation error: {str(e)}"])

    def _combine_reports(self, reports: List[SafetyReport]) -> SafetyReport:
        combined = SafetyReport(safe=all(r.safe for r in reports), confidence=min((r.confidence for r in reports), default=1.0))
        for report in reports:
            ...
            if 'constraints_checked' in report.metadata:
                combined.metadata['constraints_checked'] = report.metadata['constraints_checked']
            if 'constraint_names' in report.metadata:
                combined.metadata['constraint_names'] = report.metadata.get('constraint_names', [])
    ...
    def _handle_unsafe_action(self, action: Dict[str, Any], report: SafetyReport, snapshot_id: Optional[str]):
        if self.rollback_manager:
            try:
                quarantine_id = self.rollback_manager.quarantine_action(action, f"Safety violations: {', '.join([v.value for v in report.violations])}")
                report.metadata['quarantine_id'] = quarantine_id
            except Exception as e:
                logger.error(f"Quarantine failed: {e}")
        critical_violations = [SafetyViolationType.ADVERSARIAL, SafetyViolationType.COMPLIANCE, SafetyViolationType.PRIVACY, SafetyViolationType.TOOL_VETO]
        should_rollback = (self.safety_config.rollback_config['auto_rollback_on_critical'] and any(v in critical_violations for v in report.violations))
        if should_rollback and self.rollback_manager and snapshot_id:
            ...
            try:
                rollback_result = self.rollback_manager.rollback(snapshot_id, f"Critical violations: {report.violations}")
    ...
    def _validate_node_types(self, graph: Dict[str, Any]) -> List[str]:
        valid_types = {'input', 'output', 'compute', 'control', 'data', 'model', 'transform', 'aggregate', 'filter'}
        invalid = []
    ...
    def get_safety_stats(self) -> Dict[str, Any]:
        with self.lock:
            return {
                ...
                'validation_history': {'total': len(self.validation_history), 'recent_safety_rate': self._calculate_recent_safety_rate()},
...
def initialize_all_safety_components(config=None, reuse_existing=True, return_bundle=True):
    """
    ...
    with _SAFETY_SINGLETON_LOCK:
        if reuse_existing and _SAFETY_SINGLETON_READY and _SAFETY_SINGLETON_BUNDLE is not None:
