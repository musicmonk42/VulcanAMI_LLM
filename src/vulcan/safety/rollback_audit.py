...
class RollbackManager:
    ...
    def __init__(self, max_snapshots: int = 100, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"RollbackManager initialized with max_snapshots={self.max_snapshots}")
    ...
    def _start_cleanup_thread(self):
        ...
        def cleanup_worker():
            while not self._shutdown:
                try:
                    # CRITICAL FIX: Use Event.wait() instead of time.sleep()
                    # This allows immediate wake-up on shutdown
                    if self._shutdown_event.wait(timeout=3600):  # 1 hour, but interruptible
