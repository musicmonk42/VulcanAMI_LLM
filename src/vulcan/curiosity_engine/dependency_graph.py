        ...
        with self.lock:
            try:
                ...
                if edge_key in self.edges:
                    self.edges[edge_key].strength = max(self.edges[edge_key].strength, edge.strength)
                else:
        ...
        with self.lock:
            ...
            while self.has_cycles() and iteration < max_iterations:
                ...
                if min_edge:
                    ...
                    # REMEMBER
                    logger.info("Removed edge %s -> %s to break cycle (cost=%.2f)",
                               min_edge[0], min_edge[1], min_cost)
                else:
...
class CycleAwareDependencyGraph:
    ...
    def add_edge(self, source, target, dependency_type: DependencyType = DependencyType.PREREQUISITE, 
        ...
        with self.lock:
            try:
                ...
                if self.would_create_cycle(source_id, target_id):
                    logger.warning("Edge %s -> %s would create cycle", source_id, target_id)
                    return False
                ...
                logger.debug("Added edge %s -> %s (type=%s, strength=%.2f)", 
                            source_id, target_id, dependency_type.value, strength)
                return True
            except Exception as e:
                logger.error("Error adding edge: %s", e)
                return False
...
class DependencyAnalyzer:
    ...
    def calculate_unlock_potential(self, gap, graph: CycleAwareDependencyGraph) -> float:
        """
...
class ROICalculator:
    ...
    def get_adjusted_roi(self, gap, dependency_graph: CycleAwareDependencyGraph) -> float:
        """
        ...
        try:
            ...
            penalty_multiplier = 1.0 - min(0.5, ancestors_count * self.dependency_weight)
