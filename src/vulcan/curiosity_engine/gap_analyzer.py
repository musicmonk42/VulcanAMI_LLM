...
class SimpleAnomalyDetector:
    ...
    def fit(self, X: np.ndarray):
        ...
        try:
            ...
            self.threshold = sorted_distances[threshold_idx] if threshold_idx < len(sorted_distances) else np.inf
            ...
            try:
                if pattern_type:
                    if pattern_type in self.observed_patterns:
                        return {pattern_type: list(self.observed_patterns[pattern_type])}
                    return {}
    ...
    def get_gaps(self, addressed: Optional[bool] = None) -> List[KnowledgeGap]:
        ...
        with self.lock:
            try:
                ...
                return [g for g in self.identified_gaps.values() if g.addressed == addressed]
            except Exception as e:
        ...
        with self.lock:
            try:
                ...
                return {
                    ...
                    'active_gaps': len([g for g in self.identified_gaps.values() if not g.addressed]),
                    'addressed_gaps': len([g for g in self.identified_gaps.values() if g.addressed]),
...
class DecompositionAnalyzer:
    ...
    def analyze_failures(self, failures: List[Dict[str, Any]]) -> List[KnowledgeGap]:
        ...
        try:
            ...
            for pattern_key, pattern_failures in failure_patterns.items():
                # FIX: Check division by zero
                frequency = len(pattern_failures) / total_failures if total_failures > 0 else 0
                ...
                if frequency >= self.min_frequency:
                    ...
                    for domain in (domains or {'unknown'}):
                        gap = KnowledgeGap(
                            ...
                            estimated_cost=20 * np.mean(complexities) if complexities else 20,
    ...
    def _analyze_structural_failures(self, failures: List[Dict[str, Any]]) -> List[KnowledgeGap]:
...
class PredictionAnalyzer:
        ...
        try:
            ...
            for pair_key, pair_errors in error_patterns.items():
                ...
                if error_magnitudes:
                    ...
                    std_error = np.std(error_magnitudes) if len(error_magnitudes) > 1 else 0
    ...
    def _analyze_systematic_errors(self, errors: List[Dict[str, Any]]) -> List[KnowledgeGap]:
                        ...
                        for domain in (domains or {'systematic_bias'}):
                            gap = KnowledgeGap(
                                ...
                                metadata={
                                    ...
                                    'std_error': std_error,
                                    'error_type': 'over_prediction' if mean_error > 0 else 'under_prediction',
...
class TransferAnalyzer:
    ...
    def analyze_failures(self, failures: List[Dict[str, Any]]) -> List[KnowledgeGap]:
        ...
        try:
            ...
            for pair_key, pair_failures in transfer_patterns.items():
                # FIX: Check division by zero
                frequency = len(pair_failures) / total_failures if total_failures > 0 else 0
    ...
    def _analyze_domain_boundaries(self, failures: List[Dict[str, Any]]) -> List[KnowledgeGap]:
...
class AnomalyAnalyzer:
    ...
    def __init__(self, anomaly_threshold: float = 0.2):
        ...
        if SKLEARN_AVAILABLE:
            ...
            self.anomaly_detector = SimpleAnomalyDetector(contamination=anomaly_threshold)
            self.scaler = None
    
    def detect_anomalies(self, predictions: List[Dict[str, Any]], 
                        threshold: Optional[float] = None) -> List[Dict[str, Any]]:
        """Detect anomalies in predictions"""
        try:
            # FIX: Validate input
            if not predictions or not isinstance(predictions, list) or len(predictions) < 5:
                return []
            ...
            if SKLEARN_AVAILABLE:
                ...
                # Use simple detector
                if self.anomaly_detector is None:
                    self.anomaly_detector = SimpleAnomalyDetector(contamination=threshold)
                    ...
                    for idx, value in enumerate(features):
                        ...
                        if z_score > 3:  # 3-sigma rule
                            ...
                            anomalies.append({
                                ...
                                'value': value,
                                'z_score': z_score,
                                'frequency': 1 / len(features) if len(features) > 0 else 0.1,
                                'confidence': min(0.99, z_score / 10),
                                'score': z_score / 10,
                                'components': [observations[original_idx]] if original_idx < len(observations) else []
                            })
...
class LatentGapDetector:
    ...
    def detect_from_patterns(self, patterns: Dict[str, List[Any]], 
                            anomaly_analyzer: AnomalyAnalyzer) -> List[LatentGap]:
        ...
        try:
            ...
            for pattern_type, observations in patterns.items():
                ...
                anomalies = anomaly_analyzer.detect_pattern_anomalies(list(observations))
    ...
    def detect_latent_gaps(self) -> List[LatentGap]:
        ...
        with self._lock:
            try:
                ...
                gaps = self.latent_detector.detect_from_patterns(patterns, self.anomaly_analyzer)
