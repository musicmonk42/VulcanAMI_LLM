...
from .experiment_generator import ExperimentGenerator, Experiment, IterativeExperimentDesigner
...
class RegionManager:
    ...
    def _evict_oldest_region(self):
        ...
        try:
            ...
            oldest_id = min(self.explored_regions.keys(),
...
class ExplorationValueEstimator:
    ...
    def estimate_value(self, region: KnowledgeRegion, neighbors_count: int = 0) -> float:
        ...
        with self.lock:
            try:
                ...
                total_value = base_value + novelty_bonus + connectivity_bonus + historical_bonus
...
class ExplorationFrontier:
    ...
    def get_unexplored_neighbors(self, current_position: Optional[str] = None) -> List[Tuple[str, float]]:
                ...
                for domain, knowledge in new_knowledge.items():
                    ...
                    if patterns:
                        ...
                        if isinstance(knowledge, dict) and 'value' in knowledge:
                            self.value_estimator.update_history(domain, knowledge['value'])
            except Exception as e:
...
class SafeExperimentExecutor:
    ...
    def _execute_decomposition_experiment(self, experiment: 'Experiment', decomposer: Any) -> Dict[str, Any]:
        """Execute decomposition experiment"""
        result = {'success': False, 'data': {}}
        
        try:
            # FIX: Check both hasattr and callable
            if decomposer and hasattr(decomposer, 'test_decomposition') and callable(getattr(decomposer, 'test_decomposition', None)):
    ...
    def _execute_causal_experiment(self, experiment: 'Experiment', world_model: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # FIX: Check both hasattr and callable
            if world_model and hasattr(world_model, 'test_intervention') and callable(getattr(world_model, 'test_intervention', None)):
                ...
                test_result = world_model.test_intervention(
                    ...
                    control_variables=experiment.parameters.get('control_variables', [])
                )
    ...
    def _execute_transfer_experiment(self, experiment: 'Experiment', knowledge_base: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # FIX: Check both hasattr and callable
            if knowledge_base and hasattr(knowledge_base, 'test_transfer') and callable(getattr(knowledge_base, 'test_transfer', None)):
    ...
    def _execute_latent_experiment(self, experiment: 'Experiment', knowledge_base: Any) -> Dict[str, Any]:
        """Execute latent gap experiment"""
        result = {'success': False, 'data': {}}
        
        try:
            # FIX: Check both hasattr and callable
            if knowledge_base and hasattr(knowledge_base, 'explore_latent') and callable(getattr(knowledge_base, 'explore_latent', None)):
                # Use actual knowledge base
                ...
                result['data'] = {
                    'discoveries': [f'latent_pattern_{i}' for i in range(np.random.randint(0, 3))],
    ...
    def _execute_exploratory_experiment(self, experiment: 'Experiment') -> Dict[str, Any]:
    ...
    def select_strategy(self, 
        ...
        try:
            ...
            if recent_failures > self.recent_failures_threshold:
                ...
                return "efficient"
            elif current_load < self.low_load_threshold and available_budget > self.high_budget_threshold:
...
class GapPrioritizer:
    ...
    def prioritize_gaps(self, 
                       ...
                       dependency_graph: Any = None) -> List[LearningPriority]:
        ...
        with self.lock:
            try:
                ...
                for gap in gaps:
                    ...
                    if dependency_graph and hasattr(dependency_graph, 'descendants') and callable(getattr(dependency_graph, 'descendants', None)):
                        try:
                    ...
                    # Calculate priority
                    priority_score = self.calculate_priority(gap, descendants_count, ancestors_count)
                    
                    # FIX: Validate priority score
                    if priority_score is None or not isinstance(priority_score, (int, float)):
                        logger.warning("Invalid priority score for gap %s, using default", 
                ...
                try:
                    priorities.sort(key=lambda p: p.priority if p.priority is not None else 0, 
                                  reverse=True)
                except Exception as e:
...
class ExperimentManager:
    ...
    def run_experiment(self,
                      ...
                      resource_monitor: Any = None) -> ExperimentResult:
        ...
        with self.lock:
            try:
                ...
                # Get resource usage
                resource_usage = {}
                if resource_monitor and hasattr(resource_monitor, 'get_resource_snapshot') and callable(getattr(resource_monitor, 'get_resource_snapshot', None)):
                    try:
    ...
    def _extract_learned_knowledge(self, output: Any, experiment: Experiment) -> Dict[str, Any]:
        ...
        try:
            for result in results:
                if result.success and result.learned_knowledge:
                    ...
                    if knowledge_base:
                        self._update_knowledge_base(result.learned_knowledge, knowledge_base)
                    
                    # Update world model
                    if world_model and 'observations' in result.learned_knowledge:
                        self._update_world_model(result.learned_knowledge['observations'], world_model)
                    
                    # Update decomposer
                    if decomposer and 'patterns' in result.learned_knowledge:
                        self._update_decomposer(result.learned_knowledge['patterns'], decomposer)
                    ...
                    if hasattr(result.experiment.gap, 'mark_addressed') and callable(getattr(result.experiment.gap, 'mark_addressed', None)):
                        try:
        ...
        try:
            for key, value in knowledge.items():
                if key not in ['raw_output', 'experiment_type']:
                    if hasattr(knowledge_base, 'store_knowledge') and callable(getattr(knowledge_base, 'store_knowledge', None)):
                        try:
    ...
    def _update_world_model(self, observations: List[Any], world_model: Any):
        ...
        try:
            for obs in observations:
                if hasattr(world_model, 'update_from_observation') and callable(getattr(world_model, 'update_from_observation', None)):
                    try:
    ...
    def _update_decomposer(self, patterns: List[Any], decomposer: Any):
        ...
        try:
            for pattern in patterns:
                if hasattr(decomposer, 'learn_from_pattern') and callable(getattr(decomposer, 'learn_from_pattern', None)):
                    try:
...
class CuriosityEngine:
    ...
    def select_exploration_strategy(self, context: Optional[Dict[str, Any]] = None) -> str:
    ...
    def identify_knowledge_gaps(self, strategy: Optional[str] = None) -> List[KnowledgeGap]:
        ...
        try:
            ...
            if strategy in ["gap_driven", "comprehensive", "balanced"]:
                if strategy == "comprehensive":
                    decomposition_gaps = self.gap_analyzer.analyze_decomposition_failures()
            ...
            # REMEMBER
            logger.info("Identified %d knowledge gaps using strategy: %s", len(gaps), strategy)
            ...
            for gap in gaps:
                ...
                for dep in dependencies:
                    ...
                    if not self.gap_graph.would_create_cycle(gap, dep):
                        ...
                        logger.debug("Added weak edge to avoid cycle: %s -> %s", gap, dep)
        ...
        with self.lock:
            try:
                ...
                for priority in priorities:
                    priority.experiments = self.experiment_generator.generate_for_gap(priority.gap)
    ...
    def generate_targeted_experiments(self, gap: KnowledgeGap) -> List[Experiment]:
        ...
        try:
            ...
            if gap.type == "decomposition":
                experiments = self.experiment_generator.generate_decomposition_experiment(
                    gap,
                    complexity=0.5
                )
                ...
                experiments = self.experiment_generator.generate_transfer_experiment(gap)
            elif gap.type == "latent":
    ...
    def update_from_experiment_results(self, results: List[ExperimentResult]):
        ...
        try:
            ...
            logger.info("Updated from %d experiment results (success rate: %.2f)",
                       len(results), self.learning_rate)
        except Exception as e:
        ...
        with self.lock:
            try:
                ...
                summary = {
                    ...
                    'success_rate': success_count / experiments_run if experiments_run > 0 else 0,
                ...
                logger.info("Learning cycle complete: %d experiments, %.2f success rate",
                           experiments_run, summary['success_rate'])
