        ...
        with self.lock:
            try:
                ...
                if len(self.consumption_history) > 10:
                    recent_consumption = [c['cost'] for c in list(self.consumption_history)[-10:]]
                ...
                return {
                    ...
                    'consumption_rate': np.mean(recent_consumption) if recent_consumption else 0,
                    'budget_utilization': 1.0 - (self.current_budget / max(self.current_budget, 1))
                }
            except Exception as e:
...
class BudgetRecovery:
    ...
    def calculate_recovery(self, current_budget: float, base_allocation: float) -> float:
...
class LoadAdjuster:
    ...
    def calculate_adjustment(self, system_load: float, current_budget: float,
                           min_budget: float, max_budget: float) -> float:
...
class EfficiencyTracker:
    ...
    def calculate_base_adjustment(self, current_base: float, 
                                 min_base: float, max_base: float) -> float:
...
class DynamicBudget:
    ...
    def __init__(self, base_allocation: float = 100.0, 
        ...
        logger.info("DynamicBudget initialized (refactored) with base allocation: %.2f", 
                   base_allocation)
    ...
    def reserve(self, cost: float, reservation_id: Optional[str] = None) -> Optional[str]:
    ...
    def release_reservation(self, amount: float, reservation_id: Optional[str] = None):
        ...
        with self._lock:
            try:
                ...
                logger.debug("Released reservation: %.2f (ID: %s)", amount, reservation_id)
            except Exception as e:
                logger.error("Error releasing reservation: %s", e)
        ...
        with self._lock:
            try:
                ...
                if abs(new_budget - self.tracker.current_budget) > 0.01:
                    logger.debug("Adjusted budget for load %.2f: %.2f -> %.2f", 
                               system_load, self.tracker.current_budget, new_budget)
        ...
        with self._lock:
            try:
                ...
                if abs(new_base - self.base_allocation) > 0.01:
                    logger.debug("Adjusted base allocation: %.2f -> %.2f", 
    ...
    def reset(self):
        ...
        with self._lock:
            try:
                ...
                logger.info("Budget reset to base allocation: %.2f", self.base_allocation)
            except Exception as e:
...
class ResourcePredictor:
    ...
    def predict_load(self, horizon_minutes: float = 10, 
                     sampling_interval: float = 1.0) -> Tuple[float, float]:
            ...
            try:
                if len(self.history) < self.trend_window:
                    ...
                    if self.history:
                        ...
                        current_load = (latest.cpu_percent * 0.5 + latest.memory_percent * 0.5)
...
class ResourceAdvisor:
    ...
    def recommend_adjustment(self, snapshot: ResourceSnapshot, 
        ...
        try:
            ...
            elif snapshot.memory_percent > self.critical_thresholds[ResourceType.MEMORY]:
                recommendation['action'] = 'reduce'
    ...
    def get_current_load(self) -> float:
        ...
        with self.lock:
            try:
                ...
                current_time = time.time()
                if self._current_load_cache is not None and current_time - self._cache_time < self._cache_ttl:
    ...
    def recommend_budget_adjustment(self) -> Dict[str, Any]:
        ...
        with self.lock:
            try:
                ...
                # Get recommendation
                return self.advisor.recommend_adjustment(snapshot, predicted_load, confidence)
            except Exception as e:
    ...
    def get_resource_trends(self) -> Dict[str, Dict[str, float]]:
        ...
        with self.lock:
            try:
                ...
                recent = list(self.predictor.history)[-min(20, len(self.predictor.history)):]
...
class CostCalibrator:
    ...
    def calibrate(self, historical_costs: List[CostHistory],
                 base_costs: Dict[str, float],
                 domain_costs: Dict[str, float]) -> Tuple[Dict[str, float], Dict[str, float]]:
        ...
        with self.lock:
            try:
                ...
                for domain, records in domain_groups.items():
                    ...
                    if abs(avg_relative) > self.adaptation_threshold:
                        adjustment = 1.0 + avg_relative * self.learning_rate
                        updated_domain[domain] = updated_domain.get(domain, 1.0) * adjustment
                        updated_domain[domain] = max(0.5, min(2.0, updated_domain[domain]))
                ...
                if all_errors:
                    ...
                    if avg_error > self.adaptation_threshold:
                        self.calibration_factor *= (1 - min(0.2, avg_error * self.learning_rate))
...
class CostEstimator:
    ...
    def estimate_learning_cost(self, gap_type: str, complexity: float, 
                              priority: float, domain: str) -> float:
        """
            ...
            try:
                ...
                if cache_key:
                    ...
                    if len(self._estimate_cache) > 200:
                        ...
                        sorted_keys = sorted(all_keys, key=lambda k: self._estimate_cache[k][0])
    ...
    def calibrate_from_history(self, historical_costs: Optional[List[CostHistory]] = None):
        ...
        with self.lock:
            try:
                ...
                logger.info("Calibrated from %d historical records (avg error: %.2f%%)",
    ...
    def update_from_actual(self, experiment_type: str, predicted: float, 
                          actual: float, domain: Optional[str] = None):
            ...
            try:
                ...
                if domain:
                    ...
                    if record.error() > self.calibrator.adaptation_threshold:
                        current_novelty = self._get_domain_novelty(domain)
                        if actual > predicted:
                            self.domain_novelty[domain] = min(2.0, current_novelty * 1.1)
                        else:
                            self.domain_novelty[domain] = max(0.5, current_novelty * 0.9)
                ...
                for domain, domain_stats in self.domain_statistics.items():
                    if domain_stats['count'] > 0:
                        stats[domain] = {
                            ...
                            'avg_error': domain_stats['total_error'] / domain_stats['count'],
