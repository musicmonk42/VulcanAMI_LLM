    ...
    class AttentionFusion(nn.Module):
        ...
        def forward(self, features: List[torch.Tensor]) -> torch.Tensor:
            ...
            try:
                ...
                x = torch.stack(features).permute(
                    1, 0, 2
                )  # [batch, num_modalities, features]

                # CRITICAL: Check for NaN or Inf
                if torch.isnan(x).any() or torch.isinf(x).any():
                    logger.warning(
                        "NaN or Inf detected in attention input, replacing with zeros"
                    )
    ...
    class AdaptiveFeatureFusion(nn.Module):
        ...
        def __init__(self, input_dims: List[int], output_dim: int):
            ...
            self.output_projection = nn.Sequential(
                nn.Linear(output_dim, output_dim), nn.LayerNorm(output_dim), nn.ReLU()
            )
        ...
        def forward(
            ...
            fused = sum(
                w.unsqueeze(-1) * enc
                for w, enc in zip(fusion_weights.unbind(dim=-1), encoded)
            )
...
class MultiModalReasoningEngine:
    ...
    def attention_fusion_numpy(
        self, representations: Dict[ModalityType, np.ndarray]
    ) -> np.ndarray:
    ...
    ) -> Dict[ModalityType, ModalityData]:
    ...
    def _early_fusion(
        self, inputs: Dict[ModalityType, ModalityData], query: Dict[str, Any]
    ) -> Tuple[Any, float, List[ReasoningStep]]:
    ...
    def _late_fusion(
        ...
        for modality, data in inputs.items():
            if modality in self.modality_reasoners:
                try:
                    ...
                    logger.warning(
                        f"Modality reasoning failed for {modality.value}: {e}"
                    )
                    continue
        ...
        if self.enable_learning and modality_results:
            total_weight = sum(
                self.modality_importance[mod] for mod in modality_results.keys()
            )
        ...
        for modality, data in other.items():
            if modality in self.modality_reasoners:
                try:
                    ...
                    logger.warning(
                        f"Individual modality processing failed for {modality.value}: {e}"
                    )
    ...
    def _hierarchical_fusion(
        ...
        try:
            ...
            conclusion, confidence = self._hierarchical_reasoning(
                aligned_features, query
            )
        except Exception as e:
        ...
        try:
            if TORCH_AVAILABLE:
                ...
                if self.adaptive_fusion is None:
                    input_dims = [
                        self._get_feature_dim_for_modality(m) for m in inputs.keys()
                    ]
                ...
                for modality, data in inputs.items():
                    if data.embedding is not None:
                        ...
                        tensor = torch.randn(
                            1, self._get_feature_dim_for_modality(modality)
                        )
                ...
                if self.neural_reasoner is not None:
                    with torch.no_grad():
                        reasoning_output, neural_confidence = self.neural_reasoner(
                            fused_features
                        )
                ...
                conclusion, confidence, steps = self._advanced_numpy_gated_fusion(
                    inputs, query
                )
                return conclusion, confidence, steps
        except Exception as e:
    ...
    def _advanced_numpy_gated_fusion(
        self, inputs: Dict[ModalityType, ModalityData], query: Dict[str, Any]
    ) -> Tuple[Any, float, List[ReasoningStep]]:
        ...
        try:
            ...
            for i, (emb, modality) in enumerate(zip(normalized_embeddings, modalities)):
                ...
                # Learned importance
                importance = (
                    self.modality_importance.get(modality, 1.0)
            ...
            gate_confidence = 1.0 - (
                gate_entropy / max_entropy if max_entropy > 0 else 0
            )
    ...
    def _unified_reasoning(
        self, features: np.ndarray, query: Dict
    ) -> Tuple[Any, float]:
        ...
        try:
            ...
            query_specific_result = self._process_query_specific(
                transformed_features, query_type, query
            )

            # Stage 4: Confidence estimation
            confidence = self._estimate_reasoning_confidence(
                features, patterns, query_specific_result
            )
    ...
    def _detect_patterns_in_features(self, features: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(features) > 10:
                ...
                patterns["has_periodicity"] = (
                    np.max(np.abs(fft[1 : len(fft) // 2])) > np.mean(np.abs(fft)) * 2
                )
            ...
            if features.ndim == 1:
                ...
                patterns["trend"] = (
                    ...
                    else ("decreasing" if monotonic_decreasing else "mixed")
                )
    ...
    def _process_query_specific(
        self, features: np.ndarray, query_type: str, query: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            if query_type == "classification":
                ...
                result["self_similarity"] = float(
                    np.dot(features.flatten(), features.flatten())
                )
    ...
    def _estimate_reasoning_confidence(
        self, features: np.ndarray, patterns: Dict, query_result: Dict
    ) -> float:
        ...
        try:
            ...
            pattern_confidence = (
                0.8
                if patterns.get("has_periodicity") or patterns.get("is_clustered")
                else 0.5
            )

            # Factor 3: Query result confidence
            query_confidence = query_result.get(
                "answer_confidence", query_result.get("generation_quality", 0.5)
            )
    ...
    def _neural_reasoning(
        self, features: "torch.Tensor", query: Dict
    ) -> Tuple[Any, float]:
    ...
    def _extract_hierarchical_features(self, data: ModalityData, level: int) -> Dict:
        ...
        features = {"level": level, "raw": data.features}
        ...
        try:
            if data.embedding is not None:
                ...
                if level == 1:
                    ...
                    if reshaped_size > 0:
                        features["coarse"] = np.mean(
                            data.embedding.reshape(-1, reshaped_size), axis=1
                        )
                    else:
    ...
    def _compute_cross_modal_alignments(
        self, features: Dict
        ...
        for i, mod1 in enumerate(modalities):
            for mod2 in modalities[i + 1 :]:
                try:
                    ...
                    logger.warning(
                        f"Alignment computation failed for {mod1.value}-{mod2.value}: {e}"
                    )
    ...
    def _apply_alignments(
        self, features: Dict, alignments: List[CrossModalAlignment]
    ) -> Dict:
    ...
    def _hierarchical_reasoning(self, features: Dict, query: Dict) -> Tuple[Any, float]:
        ...
        try:
            ...
            for modality, feat_dict in features.items():
                if (
                    isinstance(feat_dict, dict)
                    and "fine" in feat_dict
                    and feat_dict["fine"] is not None
                ):
        ...
        try:
            if isinstance(data, str):
                ...
                if TRANSFORMERS_AVAILABLE:
                    ...
                    logger.info(
                        "Transformers not available, using simple text features"
                    )
                    features["embedding"] = self._simple_text_embedding(data)
                    features["confidence"] = 0.6
            else:
                features = {"embedding": np.zeros(self.embed_dim), "confidence": 0.1}

        except Exception as e:
            logger.error(f"Text feature extraction failed: {e}")
            features = {
                "embedding": np.zeros(
                    768 if TRANSFORMERS_AVAILABLE else self.embed_dim
                ),
                "confidence": 0.1,
            }
    ...
    def _extract_vision_features(self, data: Any) -> Dict:
        ...
        try:
            if VISION_AVAILABLE and TORCH_AVAILABLE:
                ...
                logger.info(
                    "Vision libraries not available, using placeholder features"
                )
    ...
    def _learn_from_fusion(
        self, modalities: List[ModalityType], strategy: str, confidence: float
    ):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats["modality_combinations"] = {
            str(k): v for k, v in stats["modality_combinations"].items()
        }
        ...
        try:
            ...
            if TORCH_AVAILABLE:
                if self.attention_fusion is not None:
                    torch.save(
                        self.attention_fusion.state_dict(), path / "attention_fusion.pt"
                    )

                if self.gated_fusion is not None:
                    torch.save(self.gated_fusion.state_dict(), path / "gated_fusion.pt")

                if self.adaptive_fusion is not None:
                    torch.save(
                        self.adaptive_fusion.state_dict(), path / "adaptive_fusion.pt"
                    )

                if self.neural_reasoner is not None:
                    torch.save(
                        self.neural_reasoner.state_dict(), path / "neural_reasoner.pt"
                    )
            ...
            if self.enable_learning:
                learning_data = {
                    ...
                    "modality_importance": {
                        k.value: v for k, v in self.modality_importance.items()
                    },
        ...
        try:
            ...
            if learning_path.exists() and self.enable_learning:
                ...
                self.fusion_weights = defaultdict(
                    lambda: 1.0, learning_data["fusion_weights"]
                )
                self.modality_importance = defaultdict(
                    lambda: 1.0,
                    {
                        ModalityType(k): v
                        for k, v in learning_data["modality_importance"].items()
                    },
                )
                self.successful_fusions = deque(
                    learning_data["successful_fusions"], maxlen=100
                )
...
class CrossModalReasoner:
    ...
    def find_cross_modal_correspondence(
                ...
                if "embedding" in inp:
                    ...
                    if self.processor:
                        ...
                        embeddings.append(
                            {
                                "embedding": getattr(
                                    result, "embedding", np.random.randn(256)
                                ),
                                "modality": getattr(
                                    result, "modality", ModalityType.UNKNOWN
                                ),
                                "metadata": inp,
                            }
                        )
                ...
                try:
                    if emb1["modality"] != emb2["modality"]:
                        ...
                        if similarity > self.correspondence_threshold:
                            ...
                            # Update associations
                            self.modality_associations[emb1["modality"]][
                                emb2["modality"]
                            ] += similarity

                            # Store pattern with cache size limit
                            pattern_key = (
                                f"{emb1['modality'].value}_{emb2['modality'].value}"
                            )
        ...
        if len(self.pattern_cache) > self.max_pattern_cache_size:
            keys_to_remove = list(self.pattern_cache.keys())[
                : self.max_pattern_cache_size // 5
            ]
            for key in keys_to_remove:
    ...
    def compute_cross_modal_attention(
        self, query_modality: ModalityData, key_modalities: List[ModalityData]
    ) -> np.ndarray:
    ...
    def register_transfer_function(
        self, source: ModalityType, target: ModalityType, function: Callable
    ):
    ...
    def learn_transfer_mapping(
        ...
        try:
            ...
            logger.info(
                f"Learned mapping for {transfer_key} with {len(mapping)} correspondences"
            )
        except Exception as e:
