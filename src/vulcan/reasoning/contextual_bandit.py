...
class AdvancedRewardModel:
    ...
    def __init__(self, context_dim: int, n_actions: int):
        ...
        self.poly_features = PolynomialFeatures(degree=2, include_bias=False) if SKLEARN_AVAILABLE else None
    ...
    def fit(self, history: List[BanditFeedback]):
        ...
        try:
            ...
            for action_id in range(self.n_actions):
                ...
                if len(X) < 10:
                    logger.warning(f"Insufficient data for action {action_id}: {len(X)} samples")
                    continue
                ...
                logger.info(f"Action {action_id}: Selected {best_model_type} model "
        ...
        try:
            ...
            for model_type in self.model_types:
                ...
                try:
                    ...
                    logger.debug(f"Action {action_id}, Model {model_type}: CV score = {avg_score:.3f}")
    ...
    def predict_with_uncertainty(self, context: BanditContext, action: BanditAction) -> Tuple[float, float]:
        """
...
class SimpleRewardModel:
    ...
    def fit(self, X, y):
        ...
        for features, reward in zip(X, y):
            key = str(features.tobytes()) if hasattr(features, 'tobytes') else str(features)
...
class ContextualBandit:
        ...
        try:
            ...
            for a in range(self.n_actions):
                if self.action_counts[a] == 0:
                    ...
                    exploration_bonus = min(exploration_bonus, self.max_exploration_bonus)
                    ucb_values[a] = q_values[a] + exploration_bonus
            
            if np.all(np.isinf(ucb_values)):
                action_id = np.random.randint(self.n_actions)
            else:
                finite_max = np.max(ucb_values[np.isfinite(ucb_values)]) if np.any(np.isfinite(ucb_values)) else 0
                ucb_values_safe = np.where(np.isinf(ucb_values), finite_max + 1, ucb_values)
            ...
            return BanditAction(
                ...
                action_id=action_id,
                expected_reward=float(q_values[action_id]),
                exploration_bonus=float(ucb_values[action_id] - q_values[action_id]) if np.isfinite(ucb_values[action_id]) else self.max_exploration_bonus,
                probability=self._compute_action_probability(action_id, context)
            )
        except Exception as e:
    ...
    def _thompson_sampling_selection(self, context: BanditContext) -> BanditAction:
        ...
        try:
            ...
            return BanditAction(
                ...
                exploration_bonus=float(samples[action_id] - self.q_values[context_key][action_id]),
                probability=self._compute_action_probability(action_id, context)
            )
        except Exception as e:
    ...
    def update(self, feedback: BanditFeedback):
        ...
        with self.lock:
            try:
                ...
                self.q_values[context_key][feedback.action.action_id] += lr * (reward - old_q)
                self.q_values[context_key][feedback.action.action_id] = np.clip(
                    self.q_values[context_key][feedback.action.action_id],
                    -1e6, 1e6
                )
                ...
                if self.stats['total_actions'] > 0:
                    self.stats['average_reward'] = self.stats['total_reward'] / self.stats['total_actions']
    ...
    def _compute_action_probability(self, action_id: int, context: BanditContext) -> float:
        ...
        try:
            if self.exploration_strategy == ExplorationStrategy.EPSILON_GREEDY:
                ...
                if action_id == best_action:
                    return (1 - self.exploration_rate) + self.exploration_rate / self.n_actions
                else:
    ...
    def select_action(self, context: BanditContext) -> BanditAction:
        ...
        try:
            ...
            return BanditAction(
                ...
                exploration_bonus=float(ucb_values[action_id] - float((x.T @ self.theta[action_id]).item())),
                probability=1.0
            )
        except Exception as e:
...
class NeuralContextualBandit(nn.Module):
    ...
    def forward(self, context: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            return torch.zeros(batch_size, self.n_actions), torch.ones(batch_size, self.n_actions)
    
    def select_action(self, context: BanditContext, exploration_rate: float = 0.1) -> BanditAction:
        """Select action"""
        try:
            ...
            return BanditAction(
                ...
                exploration_bonus=exploration_bonus,
                probability=self._compute_selection_probability(means[0], stds[0], action_id)
            )
        except Exception as e:
    ...
    def update(self, feedback: BanditFeedback):
        ...
        try:
            ...
            nll_loss = 0.5 * torch.log(2 * np.pi * pred_std**2) + \
                       0.5 * ((r - pred_mean)**2) / (pred_std**2 + self.eps)
    ...
    def _compute_selection_probability(self, means: torch.Tensor, stds: torch.Tensor, action_id: int) -> float:
        return 1.0 / self.n_actions
...
class OffPolicyEvaluator:
    ...
    def evaluate(self, 
                 history: List[BanditFeedback],
                 new_policy: Callable,
                 method: str = 'dr') -> Dict[str, float]:
        """Evaluate new policy"""
    ...
    def _importance_sampling(self, history: List[BanditFeedback], new_policy: Callable) -> Dict[str, float]:
        """IPS estimator"""
        try:
            total_weight = 0.0
            weighted_reward = 0.0
            ...
            for feedback in history:
                ...
                if new_action.action_id == feedback.action.action_id:
                    new_prob = max(self.propensity_model.predict_proba(
    ...
    def _doubly_robust(self, history: List[BanditFeedback], new_policy: Callable) -> Dict[str, float]:
        ...
        try:
            ...
            for feedback in history:
                ...
                predicted_reward = self.reward_model.predict(feedback.context, new_action)
    ...
    def _direct_method(self, history: List[BanditFeedback], new_policy: Callable) -> Dict[str, float]:
        """Direct method"""
        try:
            total_reward = 0.0
            for feedback in history:
    ...
    def _switch_estimator(self, history: List[BanditFeedback], new_policy: Callable) -> Dict[str, float]:
        """SWITCH estimator"""
        try:
            ...
            combined = (weight * ips_result['estimated_reward'] + 
    ...
    def _self_normalized_ips(self, history: List[BanditFeedback], new_policy: Callable) -> Dict[str, float]:
        """Self-normalized IPS"""
        try:
            numerator = 0.0
            denominator = 0.0
...
class AdaptiveBanditOrchestrator:
    ...
    def __init__(self, n_actions: int, context_dim: int):
        ...
        self.bandits = {
            'epsilon_greedy': ContextualBandit(n_actions, context_dim, ExplorationStrategy.EPSILON_GREEDY),
            'ucb': ContextualBandit(n_actions, context_dim, ExplorationStrategy.UCB),
            'thompson': ContextualBandit(n_actions, context_dim, ExplorationStrategy.THOMPSON_SAMPLING),
    ...
    def select_action(self, context: BanditContext) -> BanditAction:
        ...
        try:
            ...
            selected_bandit_name = bandit_names[meta_action.action_id % len(bandit_names)]
            ...
            for name, bandit in self.bandits.items():
                if hasattr(bandit, 'state_dict'):
                    ...
                    with open(save_path / f"{name}_state.pkl", 'wb') as f:
                        pickle.dump({
                            'q_values': dict(bandit.q_values) if hasattr(bandit, 'q_values') else {},
                            'action_counts': bandit.action_counts.tolist() if hasattr(bandit, 'action_counts') else [],
                            'stats': dict(bandit.stats) if hasattr(bandit, 'stats') else {}
                        }, f)
            
            with open(save_path / "meta_bandit.pkl", 'wb') as f:
                pickle.dump({
                    'performance_window': {k: list(v) for k, v in self.performance_window.items()},
        ...
        try:
            for name, bandit in self.bandits.items():
                ...
                if name in self.performance_window:
                    ...
                    if recent:
                        stats['bandit_performance'][name]['recent_average'] = float(np.mean(recent))
        except Exception as e:
...
class ToolSelectionBandit(AdaptiveBanditOrchestrator):
    ...
    def update_from_execution(self, features: np.ndarray, tool_name: str,
        ...
        try:
            ...
            action_id = self.tool_names.index(tool_name) if tool_name in self.tool_names else 0
    ...
    def _compute_reward(self, quality: float, time_ms: float,
                       energy_mj: float, constraints: Dict[str, float]) -> float:
        """Compute reward"""
        try:
