...
def _load_selection_components():
    ...
    try:
        ...
        try:
            from vulcan.reasoning.selection.confidence_calibration import CalibratedDecisionMaker
        except Exception:
...
class UnifiedReasoner:
    ...
    def __init__(self, enable_learning: bool = True, enable_safety: bool = True,
                 max_workers: Optional[int] = None, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            if 'ProbabilisticReasoner' in reasoning_components:
                self.reasoners[ReasoningType.PROBABILISTIC] = reasoning_components['ProbabilisticReasoner']()
            if 'SymbolicReasoner' in reasoning_components:
                self.reasoners[ReasoningType.SYMBOLIC] = reasoning_components['SymbolicReasoner']()
            if 'EnhancedCausalReasoning' in reasoning_components:
                self.reasoners[ReasoningType.CAUSAL] = reasoning_components['EnhancedCausalReasoning'](
        ...
        try:
            if 'CounterfactualReasoner' in reasoning_components and ReasoningType.CAUSAL in self.reasoners:
            ...
            if 'MultiModalReasoningEngine' in reasoning_components:
                ...
                self._register_modality_reasoners(reasoning_components.get('ModalityType'))
        except Exception as e:
        ...
        cache_config = config.get('cache_config', {}).copy() if config.get('cache_config') else {}
        
        # CRITICAL: Set cleanup_interval at TOP LEVEL - this is what SelectionCache.__init__ reads
        cache_config['cleanup_interval'] = 0.05  # 50ms - ultra-short for immediate test cleanup
        ...
        try:
            ...
            if 'CalibratedDecisionMaker' in selection_components and selection_components['CalibratedDecisionMaker'] is not None:
        ...
        if 'UnifiedRuntime' in optional_components:
            ...
            if not in_test:
                ...
                logger.info("Skipping UnifiedRuntime initialization (test environment detected)")
        ...
        logger.info("Enhanced Unified Reasoner initialized with production tool selection")
        ...
        try:
            ...
            for attr_name in thread_attrs:
                ...
                if thread and isinstance(thread, threading.Thread):
                    try:
                        thread.daemon = True
                        logger.debug(f"Daemonized {attr_name} in {type(component).__name__}")
                    except Exception as e:
    ...
    def reason(self, input_data: Any, query: Optional[Dict[str, Any]] = None,
              reasoning_type: Optional[ReasoningType] = None,
              strategy: ReasoningStrategy = ReasoningStrategy.ADAPTIVE,
              confidence_threshold: Optional[float] = None,
              constraints: Optional[Dict[str, Any]] = None) -> ReasoningResult:
        ...
        try:
            ...
            if self.distribution_monitor and task.features is not None:
                try:
                    if self.distribution_monitor.detect_shift(task.features):
                        logger.warning("Distribution shift detected - adjusting strategy")
            ...
            if self.voi_gate and task.features is not None:
                try:
                    ...
                    if should_gather:
                        logger.info(f"VOI suggests gathering more information: {voi_action}")
            ...
            if strategy in [ReasoningStrategy.PORTFOLIO, ReasoningStrategy.UTILITY_BASED]:
                if self.tool_selector:
                    try:
                        selection_result = self._select_tools_for_plan(plan, task)
                        plan.selected_tools = selection_result.selected_tool if hasattr(selection_result, 'selected_tool') else None
                        plan.execution_strategy = selection_result.strategy_used if hasattr(selection_result, 'strategy_used') else None
                    except Exception as e:
                        logger.warning(f"Tool selection failed: {e}")
            
            strategy_func = self.reasoning_strategies.get(strategy, self._adaptive_reasoning)
            result = self._execute_strategy_safe(strategy_func, plan, reasoning_chain, timeout=self.default_timeout)
            ...
            if self.calibrator and result.reasoning_type:
                try:
                    ...
                    if hasattr(self.calibrator, 'calibrators') and calibrator_key in self.calibrator.calibrators:
    ...
    def _execute_strategy_safe(self, strategy_func: Callable, plan: ReasoningPlan, reasoning_chain: ReasoningChain,
                               timeout: float = 30.0) -> Optional[ReasoningResult]:
        """Execute strategy with proper resource management and timeout"""
        
        future = None
        try:
            future = self.executor.submit(self._execute_strategy_impl, strategy_func, plan, reasoning_chain)
    ...
    def _execute_strategy_impl(self, strategy_func: Callable, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
    ...
    def _update_statistics_safe(self, result: ReasoningResult):
        ...
        if result and hasattr(result, 'reasoning_type') and result.reasoning_type:
            reasoning_stats = defaultdict(lambda: {'count': 0, 'successes': 0, 'avg_confidence': 0.0})
    ...
    def _create_utility_context(self, query: Optional[Dict[str, Any]],
    ...
    def _select_tools_for_plan(self, plan: ReasoningPlan,
                              task: ReasoningTask) -> Any:
    ...
    def _create_optimized_plan(self, task: ReasoningTask,
                              strategy: ReasoningStrategy) -> ReasoningPlan:
        ...
        try:
            if strategy == ReasoningStrategy.UTILITY_BASED and self.utility_model and self.cost_model:
                ...
                for reasoner_type in available_reasoners:
                    ...
                    features = task.features if task.features is not None else np.zeros(10)
                    cost_pred = self.cost_model.predict_cost(str(reasoner_type), features)
    ...
    def _portfolio_reasoning(self, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
    ...
    def _utility_based_reasoning(self, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
    ...
    def _enhance_task_with_voi(self, task: ReasoningTask,
                              voi_action: str) -> ReasoningTask:
    ...
    def _record_execution(self, task: ReasoningTask, result: ReasoningResult,
                        elapsed_time: float, from_cache: bool):
    ...
    def _convert_execution_to_reasoning_result(self, exec_result: Any) -> ReasoningResult:
    ...
    def reason_multimodal(self, inputs: Dict[Any, Any],
                         query: Optional[Dict[str, Any]] = None,
                         fusion_strategy: str = 'hybrid') -> ReasoningResult:
    ...
    def reason_counterfactual(self, factual_state: Dict[str, Any],
                            intervention: Dict[str, Any],
                            method: str = 'three_step') -> ReasoningResult:
        ...
        try:
            ...
            initial_step = ReasoningStep(
                ...
                output_data=cf_result.counterfactual if hasattr(cf_result, 'counterfactual') else None,
                confidence=cf_result.probability if hasattr(cf_result, 'probability') else 0.5,
                explanation=cf_result.explanation if hasattr(cf_result, 'explanation') else "Counterfactual reasoning"
            )
            ...
            chain = ReasoningChain(
                ...
                final_conclusion=cf_result.counterfactual if hasattr(cf_result, 'counterfactual') else None,
                total_confidence=cf_result.probability if hasattr(cf_result, 'probability') else 0.5,
            ...
            return ReasoningResult(
                conclusion=cf_result.counterfactual if hasattr(cf_result, 'counterfactual') else None,
                confidence=cf_result.probability if hasattr(cf_result, 'probability') else 0.5,
    ...
    def reason_by_analogy(self, target_problem: Dict[str, Any],
                         source_domain: Optional[str] = None) -> ReasoningResult:
        ...
        try:
            ...
            if source_domain:
                ...
                analogies = analogical_reasoner.find_multiple_analogies(target_problem, k=3)
                analogy_result = analogies[0] if analogies else {'found': False}
            
            confidence = analogy_result.get('confidence', 0.0) if analogy_result.get('found') else 0.0
    ...
    def _determine_reasoning_type(self, input_data: Any,
                                 query: Optional[Dict[str, Any]]) -> ReasoningType:
        """
        ...
        if isinstance(input_data, dict) and ModalityType:
            modality_types = [k for k in input_data.keys() if isinstance(k, ModalityType)]
            if len(modality_types) > 1:
                return ReasoningType.MULTIMODAL
        
        return self._classify_reasoning_task(input_data, query or {})

    def _classify_reasoning_task(self, input_data: Any, query: Dict[str, Any]) -> ReasoningType:
        """
        ...
        if isinstance(input_data, str):
            ...
            if any(key in input_data for key in ['graph', 'nodes', 'edges', 'evidence']):
        ...
        logger.debug(f"Reasoning type classifier scores: {dict(scores)}. Selected: {best_type}")
        return best_type

    def _sequential_reasoning(self, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
    ...
    def _parallel_reasoning(self, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
    ...
    def _ensemble_reasoning(self, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
        ...
        ensemble_confidence = np.average(
            [r[1].confidence for r in results],
    ...
    def _get_utility_weight(self, reasoning_type: ReasoningType,
                          context: Any) -> float:
    ...
    def _hierarchical_reasoning(self, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
    ...
    def _adaptive_reasoning(self, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
        ...
        try:
            ...
            if characteristics['complexity'] > 0.8:
                ...
                adaptive_plan = self._create_adaptive_plan(
                    plan.tasks[0],
                    [ReasoningType.PROBABILISTIC, ReasoningType.CAUSAL]
                )
    ...
    def _hybrid_reasoning(self, plan: ReasoningPlan, reasoning_chain: ReasoningChain) -> ReasoningResult:
    ...
    def _calculate_result_utility(self, result: ReasoningResult,
                                context: Any, execution_time_ms: float) -> float:
        """
    ...
    def _execute_reasoner(self, reasoner: Any, task: ReasoningTask) -> ReasoningResult:
        ...
        try:
            if task.task_type == ReasoningType.PROBABILISTIC:
                ...
                kb_data = task.input_data.get('kb', []) if isinstance(task.input_data, dict) else []
                ...
                result = ReasoningResult(
                    ...
                    confidence=query_result.get('confidence', 0.0) if isinstance(query_result, dict) else 0.0,
                    reasoning_type=task.task_type,
                    explanation=str(query_result.get('proof')) if isinstance(query_result, dict) else str(query_result)
                )
                ...
                if hasattr(reasoner, 'reason'):
                    ...
                    result = ReasoningResult(
                        ...
                        confidence=result_dict.get('confidence', 0.5) if isinstance(result_dict, dict) else 0.5,
                        reasoning_type=task.task_type,
                 ...
                 if not isinstance(result, ReasoningResult):
                     result = self._create_error_result("Language reasoner returned invalid type")
                ...
                if hasattr(reasoner, 'reason'):
                    ...
                    if isinstance(raw_result, ReasoningResult):
                        ...
                        result = ReasoningResult(
                            conclusion=raw_result.get('conclusion') if isinstance(raw_result, dict) else raw_result,
                            confidence=raw_result.get('confidence', 0.5) if isinstance(raw_result, dict) else 0.5,
                            reasoning_type=task.task_type,
                            explanation=str(raw_result)
                        )
                else:
            ...
            if result:
                ...
                if not result.reasoning_chain or not result.reasoning_chain.steps:
                    try:
                        is_error = isinstance(result.conclusion, dict) and 'error' in result.conclusion
                        if not is_error:
                            step = ReasoningStep(
                                ...
                                confidence=result.confidence,
                                explanation=result.explanation or f"Executed {task.task_type.value} reasoner."
                            )
                        ...
                        logger.warning(f"Failed to create reasoning chain for task {task.task_id}: {e}")
    ...
    def _analyze_input_characteristics(self, task: ReasoningTask) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(task.input_data, dict) and ModalityType:
                modality_count = sum(1 for k in task.input_data.keys() 
                                   if isinstance(k, ModalityType))
    ...
    def _create_adaptive_plan(self, task: ReasoningTask,
                            reasoning_types: List[ReasoningType]) -> ReasoningPlan:
    ...
    def _topological_sort(self, tasks: List[ReasoningTask],
                        dependencies: Dict[str, List[str]]) -> List[ReasoningTask]:
    ...
    def _merge_dependency_results(self, original_input: Any,
                                 dep_results: List[ReasoningResult]) -> Any:
    ...
    def _get_reasoning_type_weight(self, reasoning_type: ReasoningType) -> float:
        ...
        try:
            type_history = [h for h in self.reasoning_history 
    ...
    def _postprocess_result(self, result: ReasoningResult,
    ...
    def _update_metrics(self, result: ReasoningResult, elapsed_time: float,
                       strategy: ReasoningStrategy):
    ...
    def _add_audit_entry(self, task: ReasoningTask, result: ReasoningResult,
                        strategy: ReasoningStrategy, elapsed_time: float):
        ...
        try:
            audit_entry = {
                ...
                'utility_context': task.utility_context.mode.value if task.utility_context and hasattr(task.utility_context, 'mode') else None
            }
        ...
        try:
            ...
            for reasoning_type, reasoner in self.reasoners.items():
                if hasattr(reasoner, 'save_model'):
                    try:
                        reasoner.save_model(self.model_path / f"{name}_{reasoning_type.value}")
                    except Exception as e:
                        logger.warning(f"Failed to save {reasoning_type.value}: {e}")
            
            if self.tool_selector and hasattr(self.tool_selector, 'save_state'):
                try:
                    self.tool_selector.save_state(self.model_path / f"{name}_tool_selector")
                except Exception as e:
            ...
            if self.calibrator and hasattr(self.calibrator, 'save_calibration'):
                try:
                    self.calibrator.save_calibration(self.model_path / f"{name}_calibration")
                except Exception as e:
        ...
        try:
            ...
            for reasoning_type, reasoner in self.reasoners.items():
                if hasattr(reasoner, 'load_model'):
                    try:
                        reasoner.load_model(self.model_path / f"{name}_{reasoning_type.value}")
                    except Exception as e:
                        logger.warning(f"Could not load state for {reasoning_type.value}: {e}")
            
            if self.tool_selector and hasattr(self.tool_selector, 'load_state'):
                try:
                    self.tool_selector.load_state(self.model_path / f"{name}_tool_selector")
                except Exception as e:
            ...
            if self.calibrator and hasattr(self.calibrator, 'load_calibration'):
                try:
                    self.calibrator.load_calibration(self.model_path / f"{name}_calibration")
                except Exception as e:
        ...
        for reasoning_type, reasoner in self.reasoners.items():
            if hasattr(reasoner, 'get_statistics'):
                try:
                    ...
                    logger.warning(f"Failed to get stats for {reasoning_type.value}: {e}")
        ...
        for name, component in components_to_shutdown:
            if component and hasattr(component, 'shutdown'):
                if time.time() - start_time >= timeout:
                    logger.warning(f"Overall timeout reached, forcing remaining shutdowns")
                    break
