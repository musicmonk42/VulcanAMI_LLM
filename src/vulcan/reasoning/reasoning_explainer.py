...
class ReasoningExplainer:
    ...
    def explain_step(self, step: ReasoningStep) -> str:
        ...
        template = self.explanation_templates.get(
            step.step_type, "Performed {step_type} reasoning to get {output}"
        )
    ...
    def explain_chain(self, chain: ReasoningChain) -> str:
        ...
        reasoning_types_count = (
            len(chain.reasoning_types_used)
            if hasattr(chain, "reasoning_types_used")
            else 0
        )
        modalities_count = (
            len(chain.modalities_involved)
            if hasattr(chain, "modalities_involved")
            else 0
        )
        explanations.append(
            f"Using {reasoning_types_count} reasoning types across {modalities_count} modalities\n"
        )
        ...
        for i, step in enumerate(chain.steps, 1):
            try:
                ...
                if hasattr(step, "confidence") and step.confidence < 0.5:
                    explanations.append(
                        f"  Warning: Low confidence: {step.confidence:.2f}"
                    )
            except Exception as e:
        ...
        if hasattr(chain, "safety_checks") and chain.safety_checks:
            ...
            for check in chain.safety_checks:
                ...
                explanations.append(
                    f"  [{status}] {check.get('check_type', 'Unknown')}: {check.get('message', '')}"
                )
...
class SafetyAwareReasoning:
    ...
    def validate_input(self, input_data: Any) -> Dict[str, Any]:
        ...
        if input_data is None:
            return {"is_safe": False, "reason": "Null input", "sanitized_input": None}
        ...
        if hasattr(result, "explanation") and isinstance(result.explanation, str):
            ...
            for keyword in harmful_keywords:
                if keyword in explanation_lower:
                    ...
                    context_safe = any(
                        safe_word in explanation_lower
                        for safe_word in [
                            "prevent",
                            "protect",
                            "defend",
        ...
        if not safe:
            ...
            if hasattr(result, "conclusion"):
                ...
                result.conclusion = {
                    ...
                    "reason": [
                        check
                        for check in safety_status.get("checks_performed", [])
            ...
            if hasattr(result, "explanation"):
                ...
                safety_warning = (
                    f"[SAFETY FILTER APPLIED] Reasons: {'; '.join(failed_checks)}"
                )
                result.explanation = (
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        passed = sum(
            1 for check in self.safety_history if check.get("overall_safe", False)
        )
