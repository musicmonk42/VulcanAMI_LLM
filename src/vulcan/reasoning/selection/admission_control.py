...
class AdaptiveAdmissionController:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.processor_thread = threading.Thread(target=self._process_deferred, daemon=True)
            ...
            if not self._check_resource_availability(request):
                if request.priority == RequestPriority.CRITICAL:
                    ...
                    if request.priority.value <= RequestPriority.HIGH.value:
                        ...
                        with self._metrics_lock:
                            self.metrics.rejected_requests += 1
                        return AdmissionDecision.REJECT, {'reason': 'insufficient_resources'}
    ...
    def _check_resource_availability(self, request: Request) -> bool:
        ...
        try:
            ...
            estimated_memory_percent = (estimated_memory / total_memory_mb) * 100 if total_memory_mb > 0 else 0
    ...
    def _defer_request(self, request: Request) -> Tuple[AdmissionDecision, Dict[str, Any]]:
    ...
    def _process_deferred(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                if self.deferred_queue:
                    ...
                    if health == SystemHealth.HEALTHY:
                        ...
                        if decision == AdmissionDecision.ADMIT:
                            logger.info(f"Admitted deferred request {request.request_id}")
                ...
                consecutive_errors += 1
                logger.error(f"Error processing deferred requests ({consecutive_errors}): {e}")
            ...
            try:
                ...
                with self._metrics_lock:
                    ...
                    if len(self.metrics_window) > 1:
                        time_diff = self.metrics_window[-1]['time'] - self.metrics_window[0]['time']
                        request_diff = self.metrics_window[-1]['admitted'] - self.metrics_window[0]['admitted']
                        if time_diff > 0:
...
class AdmissionControlIntegration:
    ...
    def check_admission(self, 
                        ...
                        callback: Optional[Callable] = None) -> Tuple[bool, Dict[str, Any]]:
        """
            ...
            if decision == AdmissionDecision.ADMIT:
                ...
                if callback:
                    ...
                    timer = threading.Timer(
                        retry_delay,
                        lambda: self.check_admission(problem, constraints, priority, callback)
                    )
                    timer.daemon = True
                    timer.start()
                return False, info
