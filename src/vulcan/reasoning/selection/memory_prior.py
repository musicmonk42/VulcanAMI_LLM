...
class MemoryIndex:
    ...
    def build_index(self):
        ...
        try:
            ...
            self.features = np.vstack([e.problem_features for e in self.entries]).astype(np.float32)
    ...
    def search(self, query_features: np.ndarray, k: int = 10) -> List[Tuple[MemoryEntry, float]]:
            ...
            if self.use_faiss and self.faiss_index:
                ...
                for i, dist in zip(indices[0], distances[0]):
                    if i >= 0:  # FAISS returns -1 for not found
                        ...
                        if self.metric == SimilarityMetric.COSINE:
                            similarity = float(dist)  # Already similarity for inner product
                        else:
                ...
                if self.metric == SimilarityMetric.COSINE:
                    ...
                    features_norm = self.features / (np.linalg.norm(self.features, axis=1, keepdims=True) + 1e-10)
...
class BayesianMemoryPrior:
    ...
    def compute_prior(self, 
                     ...
                     context: Optional[Dict[str, Any]] = None) -> PriorDistribution:
        """
        ...
        if len(self.prior_cache) >= self.max_cache_size:
            ...
            expired_keys = [
                k for k, (_, ts) in self.prior_cache.items()
    ...
    def _beta_prior(self, features: np.ndarray, tools: List[str]) -> PriorDistribution:
        ...
        try:
            ...
            tool_data = defaultdict(lambda: {'successes': 1, 'failures': 1, 'weight': 0})
            ...
            if tool_probs:
                ...
                confidence = 1.0 - entropy / np.log(len(tools)) if len(tools) > 1 else 1.0
            else:
    ...
    def _dirichlet_prior(self, features: np.ndarray, tools: List[str]) -> PriorDistribution:
        ...
        try:
            ...
            alpha = np.array([
                self.alpha_prior + tool_counts.get(tool, 0) 
                for tool in tools
            ])
            ...
            confidence = total_weight / (total_weight + len(tools)) if (total_weight + len(tools)) > 0 else 0.0
    ...
    def _empirical_prior(self, features: np.ndarray, tools: List[str]) -> PriorDistribution:
        ...
        try:
            ...
            # CRITICAL FIX: Safe confidence calculation
            confidence = min(1.0, total_counts_sum / 10) if total_counts_sum > 0 else 0.0
    ...
    def _hierarchical_prior(self, features: np.ndarray, tools: List[str]) -> PriorDistribution:
...
class AdaptivePriorSelector:
    ...
    def select_prior_type(self, features: np.ndarray, available_tools: List[str]) -> PriorType:
    ...
    def compute_adaptive_prior(self,
                               features: np.ndarray,
                               available_tools: List[str]) -> PriorDistribution:
    ...
    def update_all(self, features: np.ndarray, tool_used: str, 
                   success: bool, confidence: float,
                   execution_time: float, energy_used: float,
                   context: Optional[Dict[str, Any]] = None):
                ...
                for prior_type in self._initialized_priors:
                    if prior_type in self.priors:
                        self.priors[prior_type].update(
                            features, tool_used, success, confidence, 
                            execution_time, energy_used, context
                        )
        except Exception as e:
