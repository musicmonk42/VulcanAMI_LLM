...
try:
    ...
    logger.warning("LightGBM not available. StochasticCostModel will use a simple average.")
...
try:
    ...
    logger.warning("sentence-transformers not available. MultiTierFeatureExtractor will have limited semantic capabilities.")
...
try:
    ...
    logger.warning("scikit-learn not available. CalibratedDecisionMaker will be disabled.")
...
try:
    ...
    from .portfolio_executor import PortfolioExecutor, ExecutionStrategy, ExecutionMonitor
...
try:
    from ..contextual_bandit import AdaptiveBanditOrchestrator, BanditContext, BanditFeedback, BanditAction
...
class StochasticCostModel:
    ...
    def predict_cost(self, tool_name: str, features: np.ndarray) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            try:
                ...
                time_pred = time_model.predict(features.reshape(1, -1))[0] if time_model else base['time']
                energy_pred = energy_model.predict(features.reshape(1, -1))[0] if energy_model else base['energy']
                
                # Use historical variance as uncertainty estimate
                time_values = [d['value'] for d in self.data_buffer.get(f'{tool_name}_time', [])]
                energy_values = [d['value'] for d in self.data_buffer.get(f'{tool_name}_energy', [])]
                
                time_std = np.std(time_values) if time_values else base['time'] * 0.3
                energy_std = np.std(energy_values) if energy_values else base['energy'] * 0.3
    ...
    def update(self, tool_name: str, component: str, value: float, features: Optional[np.ndarray] = None):
...
class MultiTierFeatureExtractor:
    ...
    def extract_tier3(self, problem: Any) -> np.ndarray:
        ...
        if not self.semantic_model:
            logger.warning("Semantic model not available, falling back to Tier 1 features.")
    ...
    def extract_tier4(self, problem: Any) -> np.ndarray:
        ...
        if isinstance(problem, dict) and any(k in problem for k in ['image', 'audio', 'video']):
    ...
    def extract_adaptive(self, problem: Any, time_budget: float) -> np.ndarray:
        ...
        if time_budget < 100 and not isinstance(problem, dict): # Fast path for simple problems
            return self.extract_tier1(problem)
...
class CalibratedDecisionMaker:
    ...
    def calibrate_confidence(self, tool_name: str, confidence: float, features: Optional[np.ndarray] = None) -> float:
    ...
    def update_calibration(self, tool_name: str, confidence: float, success: bool):
        ...
        with self.lock:
            self.data_buffer[tool_name].append({'confidence': confidence, 'success': int(success)})
...
class ValueOfInformationGate:
    ...
    def should_probe_deeper(self, features: np.ndarray, current_result: Any, budget: Dict[str, float]) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            for probe_action, probe_cost in self.cost_of_probing.items():
                ...
                value_of_information = (np.sqrt(utility_variance) - np.sqrt(reduced_variance))
    ...
    def detect_shift(self, features: np.ndarray, result: Any = None) -> bool:
        ...
        with self.lock:
            ...
            for i in range(self.reference_data.shape[1]):
                try:
                    stat, p_value = ks_2samp(self.reference_data[:, i], current_data[:, i])
                    if p_value < self.p_value_threshold:
                        logger.warning(f"Distribution shift detected in feature {i} (p-value: {p_value:.4f})")
...
class ToolSelectionBandit:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        config = config or {}
        self.tool_names = ['symbolic', 'probabilistic', 'causal', 'analogical', 'multimodal']
        ...
        if not self.is_enabled:
            logger.warning("ToolSelectionBandit is disabled; contextual_bandit module not found.")
        ...
        self.orchestrator = AdaptiveBanditOrchestrator(n_actions=num_tools, context_dim=feature_dim)
    ...
    def update_from_execution(self, features: np.ndarray, tool_name: str,
                             quality: float, time_ms: float, energy_mj: float,
        ...
        with self.update_lock:
            ...
            try:
                ...
                context = BanditContext(features=features, problem_type="tool_selection", constraints=constraints)
                try:
                ...
                action = BanditAction(tool_name=tool_name, action_id=action_id, expected_reward=0, probability=1.0/len(self.tool_names))
    ...
    def _compute_reward(self, quality: float, time_ms: float, energy_mj: float, constraints: Dict[str, float]) -> float:
...
class ToolSelector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.memory_prior = BayesianMemoryPrior(
            ...
            prior_type=PriorType.HIERARCHICAL
        )
        ...
        self.feature_extractor = MultiTierFeatureExtractor(config.get('feature_config', {}))
        ...
        self.distribution_monitor = DistributionMonitor(config.get('monitor_config', {}))
    ...
    def _warm_cache_loop(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                logger.error(f"Cache warming error ({consecutive_errors}/{max_errors}): {e}")
    ...
    def _update_statistics_loop(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                logger.debug(f"System statistics: {json.dumps(stats, default=str)[:500]}")
                consecutive_errors = 0
                
            except Exception as e:
                consecutive_errors += 1
                logger.error(f"Statistics update error ({consecutive_errors}/{max_errors}): {e}")
    ...
    def select_and_execute(self, request: SelectionRequest) -> SelectionResult:
        ...
        try:
            ...
            if not admitted:
                return self._create_rejection_result(admission_info.get('reason', 'Unknown'))
            ...
            execution_result = self._execute_portfolio(
                request, candidates, strategy
            )
    ...
    def _check_admission(self, request: SelectionRequest) -> Tuple[bool, Dict[str, Any]]:
    ...
    def _check_cache(self, request: SelectionRequest) -> Optional[SelectionResult]:
        ...
        try:
            ...
            if request.features is not None:
                cached = self.cache.get_cached_selection(request.features, request.constraints)
                if cached:
                    tool = cached['tool']
    ...
    def _check_voi(self, request: SelectionRequest, 
                   features: np.ndarray) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            return self.voi_gate.should_probe_deeper(
                features, None, budget_remaining
            )
        except Exception as e:
            logger.error(f"VOI check failed: {e}")
            return False, None
    ...
    def _generate_candidates(self, request: SelectionRequest, features: np.ndarray,
                           safe_tools: List[str], prior_dist: Any) -> List[Dict[str, Any]]:
        ...
        try:
            for tool_name in safe_tools:
                ...
                if cost_dist['time']['mean'] > request.constraints.get('time_budget_ms', float('inf')):
                    continue
    ...
    def _select_strategy(self, request: SelectionRequest, 
                        candidates: List[Dict[str, Any]]) -> ExecutionStrategy:
    ...
    def _execute_portfolio(self, request: SelectionRequest,
                          candidates: List[Dict[str, Any]],
                          strategy: ExecutionStrategy) -> Any:
    ...
    def _postprocess_result(self, request: SelectionRequest,
                          execution_result: Any,
                          start_time: float) -> SelectionResult:
        ...
        try:
            ...
            primary_tool = execution_result.tools_used[0] if execution_result.tools_used else 'unknown'
            ...
            if self.config.get('safety_enabled'):
                is_safe, safety_reason = self.safety_governor.validate_output(
                    primary_tool,
                    primary_result,
                    self._create_safety_context(request)
                )
            ...
            if len(execution_result.all_results) > 1:
                is_consistent, consensus_conf, details = self.safety_governor.check_consensus(
    ...
    def _update_statistics(self, result: SelectionResult):
        ...
        try:
            with self.stats_lock:
                ...
                tool_stats['avg_time'] = (
                    (1 - alpha) * tool_stats['avg_time'] + 
                    alpha * result.execution_time_ms
                )
                tool_stats['avg_energy'] = (
                    (1 - alpha) * tool_stats['avg_energy'] + 
                    alpha * result.energy_used_mj
                )
                tool_stats['avg_confidence'] = (
                    (1 - alpha) * tool_stats['avg_confidence'] + 
        ...
        try:
            with self.stats_lock:
                return {
                    ...
                    'bandit_stats': self.bandit.get_statistics() if hasattr(self.bandit, 'get_statistics') else {},
