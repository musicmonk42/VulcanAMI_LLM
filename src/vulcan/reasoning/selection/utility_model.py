...
class UtilityWeights:
    ...
    def normalize(self):
        ...
        total = (
            self.quality + self.time_penalty + self.energy_penalty + self.risk_penalty
        )
        if total > 0:
            self.quality /= total
...
class UtilityFunction:
    ...
    def gradient(self, value: float, context: Any) -> float:
        ...
        try:
            ...
            epsilon = 1e-6
            return (
                self.compute(value + epsilon, context)
                - self.compute(value - epsilon, context)
            ) / (2 * epsilon)
...
class ThresholdUtility(UtilityFunction):
    ...
    def __init__(
        self, threshold: float, above_value: float = 1.0, below_value: float = 0.0
    ):
...
class UtilityModel:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Statistics
        self.computation_stats = defaultdict(
            lambda: {"count": 0, "avg_utility": 0.0, "avg_time_ms": 0.0}
        )
    ...
    def compute_utility(
        ...
        energy: float,
        risk: float,
        context: Optional[Union[Dict[str, Any], UtilityContext]] = None,
    ) -> float:
        """
    ...
    def compute_components(
        ...
        try:
            # Normalize inputs
            norm_time = (
                min(1.0, time_ms / context.time_budget)
                if context.time_budget > 0
                else 1.0
            )
            norm_energy = (
                min(1.0, energy_mj / context.energy_budget)
                if context.energy_budget > 0
                else 1.0
            )
            ...
            with self.weights_lock:
                ...
                if context.mode in self.mode_weights:
                    weights = UtilityWeights(
                        **self.mode_weights[context.mode].to_dict()
                    )
                else:
                    weights = UtilityWeights()
                ...
                if context.mode in self.learned_weights:
                    self._apply_learned_weights(
                        weights, self.learned_weights[context.mode]
                    )
    ...
    def _apply_user_preferences(
        self, weights: UtilityWeights, preferences: Dict[str, float]
    ):
    ...
    def _apply_learned_weights(
        self, weights: UtilityWeights, learned: Dict[str, float]
    ):
    ...
    def compute_expected_utility(
        ...
        try:
            ...
            mean_utility = self.compute_utility(
                quality_dist[0], time_dist[0], energy_dist[0], risk_dist[0], context
            )
    ...
    def compute_marginal_utility(
        ...
        try:
            ...
            return (
                (new_utility - current_utility) / delta if abs(delta) > 1e-10 else 0.0
            )
        except Exception as e:
    ...
    def explain_utility(
        ...
        try:
            ...
            return {"total_utility": 0.0, "error": str(e)}
    ...
    def _dict_to_context(self, d: Dict[str, Any]) -> UtilityContext:
        ...
        try:
            ...
            if isinstance(mode_str, str):
                mode = (
                    ContextMode[mode_str.upper()]
                    if mode_str.upper() in ContextMode.__members__
    ...
    def _update_statistics(self, mode: ContextMode, utility: float, time_ms: float):
        ...
        try:
            with self.stats_lock:
                ...
                alpha = 0.1
                stats["avg_utility"] = (1 - alpha) * stats[
                    "avg_utility"
                ] + alpha * utility
                stats["avg_time_ms"] = (1 - alpha) * stats[
                    "avg_time_ms"
                ] + alpha * time_ms
        except Exception as e:
            ...
            with self.weights_lock:
                for mode_str, weights_dict in config.get("mode_weights", {}).items():
                    try:
                        ...
                        logger.warning(
                            f"Failed to load weights for mode {mode_str}: {e}"
                        )
                ...
                for mode_str, weights_val in config.get("learned_weights", {}).items():
                    try:
                        ...
                        logger.warning(
                            f"Failed to load learned weights for mode {mode_str}: {e}"
                        )
