...
class SafetyValidator:
    ...
    def __init__(self):
        ...
        self.safe_keywords = {
            ...
            'predict', 'classify', 'summarize', 'explain'
        }
...
class ConsistencyChecker:
    ...
    def _extract_comparable_value(self, output: Any) -> Any:
        ...
        try:
            ...
            if hasattr(output, 'value'):
                ...
                return output.get('value') or output.get('result') or output.get('conclusion')
            else:
                return output
        except Exception as e:
    ...
    def _check_boolean_consistency(self, values: List[Tuple[str, bool]]) -> Tuple[bool, float, str]:
        ...
        try:
            ...
            confidence = max(true_count, false_count) / len(values) if len(values) > 0 else 0.5
    ...
    def _check_numerical_consistency(self, values: List[Tuple[str, float]]) -> Tuple[bool, float, str]:
        ...
        try:
            ...
            if cv < 0.1:  # Less than 10% variation
                return True, float(np.clip(1.0 - cv, 0.0, 1.0)), "Numerical values consistent"
            elif cv < 0.3:  # Less than 30% variation
                return True, float(np.clip(1.0 - cv, 0.0, 1.0)), f"Moderate variation: CV={cv:.2f}"
            else:
                outliers = [name for name, v in values if abs(float(v) - mean) > 2 * std]
    ...
    def _check_string_consistency(self, values: List[Tuple[str, str]]) -> Tuple[bool, float, str]:
...
class SafetyGovernor:
    ...
    def check_safety(self, context: SafetyContext) -> Tuple[SafetyAction, Optional[str]]:
        """
        ...
        try:
            ...
            with self.cache_lock:
                ...
                if len(self.safety_cache) >= self.max_cache_size:
                    ...
                    expired_keys = [
                        k for k, (_, ts) in self.safety_cache.items()
                    ...
                    if len(self.safety_cache) >= self.max_cache_size:
                        sorted_items = sorted(
                            self.safety_cache.items(),
                            key=lambda x: x[1][1]
                        )
                        self.safety_cache = dict(sorted_items[-self.max_cache_size//2:])
    ...
    def filter_candidates(self, 
                         ...
                         context: SafetyContext) -> List[Dict[str, Any]]:
        ...
        try:
            for candidate in candidates:
                ...
                if action in [SafetyAction.ALLOW, SafetyAction.LOG_AND_ALLOW, SafetyAction.SANITIZE]:
    ...
    def validate_output(self, 
                       tool_name: str,
                       output: Any,
                       context: SafetyContext) -> Tuple[bool, Optional[str]]:
            ...
            if tool_name in self.contracts:
                ...
                if hasattr(output, 'confidence'):
                    ...
                    if output.confidence < min_conf:
                        reason = f"Confidence {output.confidence} below threshold {min_conf}"
                        self._record_violation(
                            tool_name,
    ...
    def check_consensus(self, outputs: Dict[str, Any]) -> Tuple[bool, float, str]:
        ...
        try:
            is_consistent, confidence, details = self.consistency_checker.check_consistency(outputs)
            ...
            if not is_consistent and confidence < self.veto_threshold:
                ...
                with self.lock:
                    for tool_name in outputs.keys():
                        self._record_violation(
                            tool_name,
                            VetoReason.INCONSISTENT_OUTPUT,
                            details
                        )
    ...
    def _check_contract_violation(self, 
                                 context: SafetyContext,
                                 contract: ToolContract) -> Optional[str]:
        ...
        try:
            ...
            if contract.required_inputs:
                ...
                missing = [req for req in contract.required_inputs 
                          if req not in problem_str]
                if missing:
            ...
            if contract.forbidden_inputs:
                problem_str = str(context.problem).lower()
                found = [forb for forb in contract.forbidden_inputs 
            ...
            if context.constraints:
                ...
                energy_budget = context.constraints.get('energy_budget_mj', float('inf'))
    ...
    def _record_violation(self, tool_name: str, reason: VetoReason, details: str):
        ...
        try:
            ...
            if len(self.violation_counts) > 1000:
                ...
                keys_to_remove = [k for k in self.violation_counts.keys() if k not in recent_tools]
                for k in keys_to_remove:
                    del self.violation_counts[k]
            
            logger.warning(f"Safety violation: {tool_name} - {reason.value}: {details[:200]}")
        except Exception as e:
            logger.error(f"Recording violation failed: {e}")
    
    def _add_audit_entry(self, context: SafetyContext, result: Tuple[SafetyAction, Optional[str]]):
        ...
        try:
            entry = {
                ...
                'reason': result[1][:500] if result[1] else None,  # CRITICAL FIX: Limit reason length
                'safety_level': context.safety_level.value,
                'problem_hash': hashlib.md5(str(context.problem)[:1000].encode()).hexdigest()[:8]
            }
