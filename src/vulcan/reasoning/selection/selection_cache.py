    ...
    def put(self, key: str, value: Any, size_bytes: int, ttl: Optional[float] = None):
        ...
        with self.lock:
            try:
                ...
                while len(self.cache) >= self.max_size or self.size_bytes + size_bytes > self.max_bytes:
                    if not self.cache:
                        break
...
class MultiLevelCache:
    ...
    def _load_disk_index(self):
        ...
        try:
            ...
            logger.info(f"Loaded disk cache index: {len(self.l3_index)} entries, {self.l3_current_size_mb:.2f} MB")
        except Exception as e:
        ...
        try:
            ...
            if self.l3_enabled:
                l3_stats.update({
                    ...
                    'utilization': self.l3_current_size_mb / self.l3_max_size_mb if self.l3_max_size_mb > 0 else 0
                })
...
class SelectionCache:
    ...
    def cache_features(self, problem: Any, features: np.ndarray, ttl: Optional[float] = None):
    ...
    def cache_selection(self, features: np.ndarray, constraints: Dict[str, float],
                       selection: str, confidence: float, ttl: Optional[float] = None):
        """Cache tool selection decision"""
        try:
            key = self._compute_selection_key(features, constraints)
            value = {'tool': selection, 'confidence': confidence, 'timestamp': time.time()}
    ...
    def get_cached_selection(self, features: np.ndarray, 
                            constraints: Dict[str, float]) -> Optional[Dict[str, Any]]:
    ...
    def cache_result(self, tool: str, problem: Any, result: Any, 
                    execution_time: float, energy: float, ttl: Optional[float] = None):
    ...
    def cache_similarity(self, features1: np.ndarray, features2: np.ndarray, 
                        similarity: float):
    ...
    def get_cached_similarity(self, features1: np.ndarray, 
                             features2: np.ndarray) -> Optional[float]:
    ...
    def invalidate_problem(self, problem: Any):
        ...
        try:
            ...
            for tool in ['symbolic', 'probabilistic', 'causal', 'analogical', 'multimodal']:
        ...
        try:
            ...
            for features, constraints, selection in list(self.warm_entries):
                ...
                if self.selection_cache.get(key) is None:
                    # Re-cache
                    self.cache_selection(features, constraints, selection, confidence=0.8)
        except Exception as e:
    ...
    def _compute_selection_key(self, features: np.ndarray, 
    ...
    def _compute_similarity_key(self, features1: np.ndarray, 
