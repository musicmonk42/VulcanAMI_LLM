...
class EWMA:
    ...
    def __init__(self, alpha: float = 0.25, init_mean: Optional[float] = None, init_var: Optional[float] = None):
...
class StochasticCostModel:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.enable_calibration = bool(cfg.get('enable_calibration', True and SKLEARN_AVAILABLE))
        ...
        self.ewma_stats: Dict[Tuple[str, str], Dict[str, EWMA]] = defaultdict(self._create_ewma_track)
        ...
        self.train_buffer: Dict[Tuple[str, str], List[ExecutionRecord]] = defaultdict(list)
        ...
        logger.info(f"StochasticCostModel initialized: type={self.effective_model_type}, "
    ...
    def _key(self, tool_name: str, context: Optional[Dict[str, Any]]) -> Tuple[str, str]:
    ...
    def update(
        ...
        with self.lock:
            ...
            if component == CostComponent.TIME_MS:
                ...
                logger.debug(f"Updated {tool_name} QUALITY: {value:.2f} (clamped: {clamped:.2f})")
            elif component == CostComponent.RISK:
                # Clamp to model bounds
                clamped = float(np.clip(value, self.min_risk, self.max_risk))
                ewma["risk"].update(clamped)
                logger.debug(f"Updated {tool_name} RISK: {value:.2f} (clamped: {clamped:.2f})")
            else:
    ...
    def predict(self, tool_name: str, context: Optional[Dict[str, Any]] = None) -> CostEstimate:
        """
        ...
        with self.lock:
            ...
            use_ml = (self.effective_model_type == 'lgbm' and 
                     LGBM_AVAILABLE and 
                     total_samples >= self.min_samples_for_ml and
    ...
    def _predict_ewma(self, ewma: Dict[str, EWMA], context: Dict[str, Any]) -> CostEstimate:
        ...
        if deadline_ms > 0.0 and time_scaled > deadline_ms:
            ...
            risk_mean = min(self.max_risk, risk_mean + min(0.06, lateness / max(1000.0, deadline_ms)))
            quality_mean *= 0.97
    ...
    def _apply_mode_nudges(self, mode_str: str, t: float, e: float, q: float, r: float) -> Tuple[float, float, float, float]:
    ...
    def observe(self, tool_name: str, context: Optional[Dict[str, Any]], actual: CostEstimate) -> None:
        """
        ...
        with self.lock:
            ...
            ewma['quality'].update(np.clip(actual.quality, self.min_quality, self.max_quality))
            ...
            if buffer_size >= self.batch_size and self.effective_model_type == 'lgbm' and LGBM_AVAILABLE:
                self._update_models()
        ...
        logger.debug(f"Observed {tool_name}: time={actual.time_ms:.1f}ms, "
    ...
    def _update_models(self) -> None:
        ...
        try:
            ...
            for record in recent_history:
                features = self.feature_extractor.extract(record.tool_name, record.context)
            ...
            self.models['time_ms'] = lgb.LGBMRegressor(**lgbm_params).fit(X_train, y_time)
            self.models['energy_mj'] = lgb.LGBMRegressor(**lgbm_params).fit(X_train, y_energy)
            self.models['quality'] = lgb.LGBMRegressor(**lgbm_params).fit(X_train, y_quality)
            self.models['risk'] = lgb.LGBMRegressor(**lgbm_params).fit(X_train, y_risk)
            
            # Calibration
            if self.enable_calibration and SKLEARN_AVAILABLE and len(recent_history) >= 30:
    ...
    def _calibrate_models(self, X: np.ndarray, y_quality: np.ndarray, y_risk: np.ndarray) -> None:
    ...
    def _check_drift(self) -> None:
        ...
        try:
            ...
            recent_error = np.mean([abs(r.actual_quality - self.default_quality) for r in recent])
            historical_error = np.mean([abs(r.actual_quality - self.default_quality) for r in historical])
            
            drift_magnitude = abs(recent_error - historical_error) / max(0.01, historical_error)
    ...
    def get_statistics(self, tool_name: str, mode: Optional[str] = None) -> Dict[str, Any]:
    ...
    def reset_statistics(self, tool_name: Optional[str] = None, mode: Optional[str] = None) -> None:
        ...
        with self.lock:
            if tool_name is None:
                ...
                keys_to_delete = [k for k in self.ewma_stats.keys() if k[0] == tool_name]
