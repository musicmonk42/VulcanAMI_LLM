...
class AbstractReasoner(ABC):
    ...
    @abstractmethod
    def reason(self, problem: Any, context: Optional['ReasoningContext'] = None) -> 'ReasoningResult':
...
class ReasoningStep:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.step_type, ReasoningType):
            raise TypeError(f"step_type must be ReasoningType enum, got {type(self.step_type)}")
        
        # Validate timestamp is reasonable
        if self.timestamp < 0 or self.timestamp > time.time() + 86400:  # Not more than 1 day in future
...
class ReasoningChain:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.total_confidence, (int, float)):
            raise TypeError(f"total_confidence must be numeric, got {type(self.total_confidence)}")
        
        if not 0.0 <= self.total_confidence <= 1.0:
            raise ValueError(f"total_confidence must be in [0, 1], got {self.total_confidence}")
...
class ReasoningResult:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.uncertainty, (int, float)):
            raise TypeError(f"uncertainty must be numeric, got {type(self.uncertainty)}")
        ...
        if self.reasoning_chain is not None and not isinstance(self.reasoning_chain, ReasoningChain):
...
class SelectionResult:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.calibrated_confidence, (int, float)):
            raise TypeError(f"calibrated_confidence must be numeric, got {type(self.calibrated_confidence)}")
        
        if not 0.0 <= self.calibrated_confidence <= 1.0:
            raise ValueError(f"calibrated_confidence must be in [0, 1], got {self.calibrated_confidence}")
        ...
        if self.energy_used_mj < 0:
            raise ValueError(f"energy_used_mj must be non-negative, got {self.energy_used_mj}")
        
        # Validate strategy
        if not isinstance(self.strategy_used, PortfolioStrategy):
            raise TypeError(f"strategy_used must be PortfolioStrategy enum, got {type(self.strategy_used)}")
...
class PortfolioResult:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.strategy, PortfolioStrategy):
            raise TypeError(f"strategy must be PortfolioStrategy enum, got {type(self.strategy)}")
        ...
        if self.energy_used < 0:
            raise ValueError(f"energy_used must be non-negative, got {self.energy_used}")
        ...
        for tool, score in self.confidence_scores.items():
            ...
            if not 0.0 <= score <= 1.0:
                raise ValueError(f"Confidence score for {tool} must be in [0, 1], got {score}")
...
class CostEstimate:
    ...
    def __post_init__(self):
        ...
        for percentile, value in self.percentiles.items():
            ...
            if value < 0:
                raise ValueError(f"Percentile value for {percentile} must be non-negative")
...
class SafetyAssessment:
    ...
    def __post_init__(self):
        ...
        if self.safety_level.upper() not in valid_levels:
            raise ValueError(f"safety_level must be one of {valid_levels}, got {self.safety_level}")
...
class CalibrationData:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.raw_confidence, (int, float)):
            raise TypeError(f"raw_confidence must be numeric, got {type(self.raw_confidence)}")
        
        if not 0.0 <= self.raw_confidence <= 1.0:
            raise ValueError(f"raw_confidence must be in [0, 1], got {self.raw_confidence}")
...
class DistributionShift:
    ...
    def __post_init__(self):
        ...
        if self.drift_type.upper() not in valid_drift_types:
            raise ValueError(f"drift_type must be one of {valid_drift_types}, got {self.drift_type}")
        ...
        for i, feature_idx in enumerate(self.affected_features):
            if not isinstance(feature_idx, int):
                raise TypeError(f"affected_features[{i}] must be an integer, got {type(feature_idx)}")
            if feature_idx < 0:
                raise ValueError(f"affected_features[{i}] must be non-negative, got {feature_idx}")
