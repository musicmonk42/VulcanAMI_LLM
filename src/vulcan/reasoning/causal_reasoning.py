...
try:
    ...
    logging.getLogger(__name__).warning("NetworkX not available, graph features disabled")
...
try:
    ...
    logging.getLogger(__name__).warning("DoWhy not available, advanced causal inference disabled")
...
try:
    ...
    logging.getLogger(__name__).warning("statsmodels not available. Granger causality test will be a simplified placeholder.")
...
try:
    ...
    logging.getLogger(__name__).warning("lingam not available. LiNGAM algorithm will fall back to PC.")
...
class CausalReasoningEngine:
    ...
    def __init__(self):
        ...
        self.graph = defaultdict(set)  # CRITICAL FIX: Add graph representation for cycle detection
        self.intervention_history = deque(maxlen=1000)  # CRITICAL FIX: Use deque with size limit
        
    def update_causal_link(self, cause: str, effect: str, 
    ...
    def detect_confounders(self, treatment: str, outcome: str) -> Set[str]:
        ...
        for node in self.causal_graph:
            if (node != treatment and node != outcome and
                outcome in self.causal_graph.get(node, {}) and
                treatment in self.causal_graph.get(node, {})):
    ...
    def topological_sort(self) -> List[str]:
        ...
        if cycles:
            logger.error(f"Cannot perform topological sort on graph with cycles: {cycles}")
        ...
        if len(result) != len(all_nodes):
            logger.warning(f"Topological sort incomplete: {len(result)}/{len(all_nodes)}")
...
class EnhancedCausalReasoning(CausalReasoningEngine):
    ...
    def __init__(self, enable_learning: bool = True):
        ...
        self.mechanisms = {}
        self.noise_models = {}
        self.data_store = defaultdict(lambda: deque(maxlen=10000))  # CRITICAL FIX: Limit data storage
    ...
    def add_causal_relationship(self, cause: str, effect: str,
        ...
        if self.causal_dag is not None and NETWORKX_AVAILABLE:
            ...
            if self.causal_dag.has_node(effect) and self.causal_dag.has_node(cause) and nx.has_path(self.causal_dag, effect, cause):
                logger.warning(f"Cannot add edge {cause}->{effect}: would create a cycle.")
                return
    ...
    def discover_causal_structure(self, data: Union[np.ndarray, 'pd.DataFrame'],
                                 variable_names: Optional[List[str]] = None,
                                 algorithm: str = 'pc',
                                 significance_level: float = 0.05) -> Optional[nx.DiGraph]:
        ...
        try:
            discovery_func = self.discovery_algorithms.get(algorithm, self._pc_algorithm)
    ...
    def _pc_algorithm(self, data: pd.DataFrame, variable_names: List[str],
                     alpha: float) -> Optional[nx.DiGraph]:
            ...
            for j in range(i+1, n_vars):
                ...
                for k in range(min(5, n_vars)):  # Limit conditioning set size
                    if self._test_conditional_independence(
                        data, variable_names[i], variable_names[j],
                        conditioning_set_size=k, alpha=alpha
                    ):
    ...
    def _test_conditional_independence(self, data: pd.DataFrame, x: str, y: str,
    ...
    def _ges_algorithm(self, data: pd.DataFrame, variable_names: List[str],
                      alpha: float) -> Optional[nx.DiGraph]:
    ...
    def _lingam_algorithm(self, data: pd.DataFrame, variable_names: List[str],
                        alpha: float) -> Optional[nx.DiGraph]:
        """Linear Non-Gaussian Acyclic Model discovery using lingam."""
        if not LINGAM_AVAILABLE:
            logger.warning("lingam library not found. LiNGAM falling back to PC algorithm.")
            return self._pc_algorithm(data, variable_names, alpha)
    ...
    def _fci_algorithm(self, data: pd.DataFrame, variable_names: List[str],
                      alpha: float) -> Optional[nx.DiGraph]:
    ...
    def perform_intervention(self, variable: str, value: Any,
                           use_do_calculus: bool = True) -> InterventionResult:
        """Perform causal intervention with do-calculus"""
        
        # CRITICAL FIX: Check if DAG exists AND has nodes, not if NetworkX is available
        if not self.causal_dag or (NETWORKX_AVAILABLE and self.causal_dag.number_of_nodes() == 0):
        ...
        self.record_intervention(variable, value, {
            'effects': len(effects),
            'confidence': confidence
        })
    ...
    def _identify_causal_paths(self, source: str,
                              graph: Optional[nx.DiGraph] = None) -> List[List[str]]:
    ...
    def _calculate_intervention_confidence(self, variable: str,
    ...
    def _explain_intervention(self, variable: str, value: Any,
    ...
    def test_conditional_independence(self, x: str, y: str,
                                     z: Set[str]) -> bool:
    ...
    def identify_confounders(self, treatment: str, outcome: str,
                           use_criteria: str = 'backdoor') -> Set[str]:
    ...
    def _find_backdoor_paths(self, treatment: str, outcome: str) -> List[List[str]]:
        ...
        try:
            ...
            all_paths = list(nx.all_simple_paths(undirected, treatment, outcome, cutoff=5))
    ...
    def compute_causal_effect(self, treatment: str, outcome: str,
        ...
        direct_effect = 0.0
        if self.causal_dag and NETWORKX_AVAILABLE and self.causal_dag.has_edge(treatment, outcome):
    ...
    def _compute_total_effect(self, treatment: str, outcome: str) -> float:
        ...
        try:
            ...
            paths = list(nx.all_simple_paths(self.causal_dag, source=treatment, target=outcome))
    ...
    def _is_valid_adjustment_set(self, treatment: str, outcome: str,
    ...
    def _regression_estimation(self, data: pd.DataFrame, treatment: str,
                             outcome: str, covariates: Set[str]) -> float:
    ...
    def _matching_estimation(self, data: pd.DataFrame, treatment: str,
                           outcome: str, covariates: Set[str]) -> float:
        ...
        try:
            ...
            if treated.empty or control.empty: return 0.0

            nn = NearestNeighbors(n_neighbors=1, algorithm='ball_tree').fit(control['propensity'].values.reshape(-1, 1))
            ...
            logger.error(f"Matching estimation failed: {e}. Falling back to regression.")
            return self._regression_estimation(data, treatment, outcome, covariates)

    def _weighting_estimation(self, data: pd.DataFrame, treatment: str,
                            outcome: str, covariates: Set[str]) -> float:
        """Estimate using Inverse Propensity Score Weighting (IPSW)."""
        if not PANDAS_AVAILABLE:
            logger.warning("Pandas not available. Weighting falling back to regression.")
            return self._regression_estimation(data, treatment, outcome, covariates)
        ...
        try:
            ...
            y1 = np.sum(df[df[treatment] == 1]['weight'] * df[df[treatment] == 1][outcome]) / np.sum(df[df[treatment] == 1]['weight'])
            y0 = np.sum(df[df[treatment] == 0]['weight'] * df[df[treatment] == 0][outcome]) / np.sum(df[df[treatment] == 0]['weight'])
    ...
    def _doubly_robust_estimation(self, data: pd.DataFrame, treatment: str,
                                outcome: str, covariates: Set[str]) -> float:
        """Doubly robust estimation"""
        # Combine regression and weighting
        try:
            reg_est = self._regression_estimation(data, treatment, outcome, covariates)
            weight_est = self._weighting_estimation(data, treatment, outcome, covariates)
    ...
    def _instrumental_variable(self, treatment: str, outcome: str) -> Set[str]:
        ...
        try:
            ...
            for node in self.causal_dag.nodes():
                if (self.causal_dag.has_edge(node, treatment) and
                    not self.causal_dag.has_edge(node, outcome)):
                    # Check if there's no path to outcome except through treatment
                    try:
                        # Find paths from instrument to outcome
                        paths = list(nx.all_simple_paths(self.causal_dag, source=node, target=outcome))
                        # If all paths go through treatment, it's a valid instrument
                        is_instrument = True
                        if not paths: # No path from instrument to outcome is also a condition
                            is_instrument = True
                        else:
                            for path in paths:
    ...
    def temporal_causal_analysis(self, time_series_data: Dict[str, List[float]],
            ...
            for var2 in variables:
                if var1 != var2:
                    try:
                        ...
                        logger.warning(f"Temporal analysis failed for {var1}->{var2}: {e}")
        
        return results
    
    def _granger_causality_test(self, x: List[float], y: List[float],
        ...
        if not STATSMODELS_AVAILABLE:
            logger.warning("statsmodels not available. Granger causality test is a placeholder.")
            return {'f_statistic': float(np.random.random()), 'p_value': float(np.random.random()), 'causality': bool(np.random.random() < 0.05)}
    ...
    def _lagged_correlation(self, x: List[float], y: List[float],
                           lag: int) -> float:
        ...
        try:
            ...
            if len(x_lagged) > 1 and len(y_future) > 1 and len(x_lagged) == len(y_future):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.causal_dag and NETWORKX_AVAILABLE:
            try:
                ...
                avg_degree = (sum(dict(self.causal_dag.degree()).values()) / num_nodes 
...
class CounterfactualReasoner:
    ...
    def compute_counterfactual(self,
                              ...
                              intervention: Dict[str, Any],
                              method: str = 'twin_network') -> CounterfactualResult:
        ...
        if cache_key:
            if len(self.counterfactual_cache) >= self.max_cache_size:
                ...
                keys_to_remove = list(self.counterfactual_cache.keys())[:self.max_cache_size//5]
    ...
    def _three_step_counterfactual(self,
                                  factual_state: Dict[str, Any],
        ...
        counterfactual_state = self._prediction(intervened_model, noise_terms, {**factual_state, **intervention})
    ...
    def _twin_network_counterfactual(self,
                                    factual_state: Dict[str, Any],
                                    intervention: Dict[str, Any]) -> CounterfactualResult:
    ...
    def _propagate_twin_network(self, twin_network: Dict[str, Any],
                    ...
                    for desc in descendants:
                        if desc not in result:
                            ...
                            if desc in self.structural_equations:
                                ...
                                parent_values = [result.get(p, twin_network.get(f"{p}_factual", 0))
    ...
    def _abduction(self, observed: Dict[str, Any]) -> Dict[str, float]:
        ...
        for var, value in observed.items():
            if var in self.structural_equations:
                ...
                try:
                    expected = eq_info['equation'](*parent_values, 0) # Assume noise is the last arg and is 0
    ...
    def _prediction(self, model: Dict, noise_terms: Dict[str, float],
    ...
    def _estimate_counterfactual_probability(self, factual: Dict[str, Any],
        ...
        try:
            ...
            for var in set(factual.keys()) | set(counterfactual.keys()):
                diff = abs(float(counterfactual.get(var, 0)) - float(factual.get(var, 0)))
    ...
    def _explain_counterfactual(self, factual: Dict[str, Any],
                               intervention: Dict[str, Any],
                               counterfactual: Dict[str, Any],
        ...
        for var, diff in sorted_diffs[:3]:
            ...
            explanation += f"  {var}: {fact_val:.2f} → {counter_val:.2f} (Δ={diff:.2f})\n"
        
        return explanation
    
    def necessity_sufficiency_analysis(self, cause: str, effect: str,
