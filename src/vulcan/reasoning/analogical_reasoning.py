...
class Entity:
    ...
    def similarity_to(self, other: 'Entity', use_embeddings: bool = True) -> float:
        ...
        if self is other or (self.name == other.name and self.entity_type == other.entity_type):
            return 1.0
        ...
        if use_embeddings and self.embedding is not None and other.embedding is not None:
            embedding_sim = self._embedding_similarity(self.embedding, other.embedding)
        else:
            embedding_sim = 0.0
        
        # Semantic category match
        category_match = 1.0 if (self.semantic_category and 
                                 self.semantic_category == other.semantic_category) else 0.0
...
class Relation:
    ...
    def semantic_similarity_to(self, other: 'Relation') -> float:
        ...
        # Semantic type match
        type_sim = 1.0 if (self.semantic_type and 
                          self.semantic_type == other.semantic_type) else 0.0
...
class SemanticEnricher:
    ...
    def enrich_entity(self, entity: Entity) -> Entity:
        ...
        entity.semantic_category = self._categorize_entity(entity.name, entity.entity_type)
        ...
        try:
            ...
            if len(self._tfidf_corpus) > 1:
                try:
                    ...
                    if not hasattr(self._tfidf_vectorizer, 'vocabulary_') or self._tfidf_vectorizer.vocabulary_ is None:
                        self._tfidf_vectorizer.fit(self._tfidf_corpus)
            ...
            logger.warning(f"TF-IDF embedding failed for '{text}': {e}, using character embedding")
            return self._character_embedding(text)
...
class GoalRelevanceAnalyzer:
    ...
    def analyze_goal_relevance(self, mappings: Dict[str, str], goal: Any,
                               source: Dict, target: Dict) -> Dict[str, float]:
        """
    ...
    def _compute_entity_goal_relevance(self, source_entity: str, target_entity: str,
                                      goal_text: str, goal_entities: Set[str],
                                      goal_concepts: Set[str], goal_embedding: np.ndarray,
                                      source: Dict, target: Dict) -> float:
        ...
        # Weighted average
        weights = [0.3, 0.2, 0.25, 0.15, 0.1] if len(scores) == 5 else [0.35, 0.25, 0.25, 0.15]
    ...
    def _compute_causal_relevance(self, source_entity: str, target_entity: str,
                                 goal_concepts: Set[str], source: Dict, target: Dict) -> float:
    ...
    def _compute_dependency_relevance(self, source_entity: str, target_entity: str,
                                     goal_text: str) -> float:
        ...
        try:
            ...
            for token in doc:
                if source_lower in token.text.lower() or target_lower in token.text.lower():
...
class AnalogicalReasoner(AbstractReasoner):
    ...
    def __init__(self, enable_caching: bool = True, enable_learning: bool = True):
        ...
        self.stats = {
            ...
            'embedding_method': 'sentence-transformers' if SENTENCE_TRANSFORMERS_AVAILABLE else 'tfidf'
        }
    ...
    def add_domain(self, domain_name: str, structure: Dict[str, Any]):
        ...
        logger.info(f"Added domain {domain_name} with {len(enriched_entities)} entities "
            ...
            for attr in intersection:
                try:
                    if source[attr] == target[attr]:
                        value_sim += 1.0
                    elif isinstance(source[attr], (int, float)) and isinstance(target[attr], (int, float)):
                        diff = abs(source[attr] - target[attr])
                        max_val = max(abs(source[attr]), abs(target[attr]), 1e-10)
                        value_sim += 1.0 - min(1.0, diff / max_val)
                    elif isinstance(source[attr], str) and isinstance(target[attr], str):
    ...
    def find_structural_analogy(self,
                               source_domain: str,
                               target_problem: Dict,
                               mapping_type: MappingType = MappingType.STRUCTURAL) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_str = json.dumps({
                'entities': sorted([e.name if isinstance(e, Entity) else str(e) for e in self._extract_entities(target_problem)]),
                'relations': sorted([str(r) for r in self._extract_relations(target_problem)]),
                'attributes': sorted([(k, sorted(v) if isinstance(v, list) else v) 
        ...
        try:
            if mapping_type == MappingType.STRUCTURAL:
                ...
                mapping = self._pragmatic_mapping(source, target_structure, target_problem)
            else:  # SEMANTIC
        ...
        if mapping.mapping_score >= self.similarity_threshold:
            solution = self._map_solution(
                source.get('structure', {}).get('solution'),
            ...
            self.update_cache(source_domain, target_problem, mapping.entity_mappings, mapping.confidence)
        else:
            result = {
                'found': False,
    ...
    def _structural_mapping(self, source: Dict, target: Dict) -> AnalogicalMapping:
        ...
        # Systematicity
        systematicity_score = self._compute_systematicity(best_mapping['relation_mappings'])
    ...
    def _find_entity_candidates_semantic(self, source_entities: Set[Entity],
                                        target_entities: Set[Entity]) -> Dict[str, List[Tuple[str, float]]]:
            ...
            for t_entity in target_entities:
                try:
                    if isinstance(s_entity, Entity) and isinstance(t_entity, Entity):
                        ...
                        similarity = s_entity.similarity_to(t_entity, use_embeddings=True)
                        if similarity > 0.3:  # Lower threshold for candidates
                            candidates[s_entity.name].append((t_entity.name, similarity))
                    else:
                        s_name = str(s_entity)
                        t_name = str(t_entity)
    ...
    def _compute_semantic_coherence(self, mapping: Dict, source: Dict, target: Dict) -> float:
        ...
        for s_name, t_name in mapping['entity_mappings'].items():
            ...
            s_entity = next((e for e in source['entities'] 
                           if isinstance(e, Entity) and e.name == s_name), None)
            t_entity = next((e for e in target['entities'] 
                           if isinstance(e, Entity) and e.name == t_name), None)
            
            if s_entity and t_entity and isinstance(s_entity, Entity) and isinstance(t_entity, Entity):
    ...
    def _surface_mapping(self, source: Dict, target: Dict) -> AnalogicalMapping:
        ...
        for s_entity in source['entities']:
            ...
            if best_match and best_score > 0.5:
                s_name = s_entity.name if isinstance(s_entity, Entity) else str(s_entity)
                t_name = best_match.name if isinstance(best_match, Entity) else str(best_match)
    ...
    def _pragmatic_mapping(self, source: Dict, target: Dict, 
                          target_problem: Dict) -> AnalogicalMapping:
        ...
        if goal:
            ...
            avg_relevance = np.mean(list(relevance_scores.values())) if relevance_scores else 0.5
            structural_mapping.mapping_score = (
                0.7 * structural_mapping.mapping_score + 
                0.3 * avg_relevance
            )
    ...
    def _semantic_mapping(self, source: Dict, target: Dict) -> AnalogicalMapping:
        ...
        for s_entity in source['entities']:
            ...
            for t_entity in target['entities']:
                try:
                    s_name = s_entity.name if isinstance(s_entity, Entity) else str(s_entity)
                    t_name = t_entity.name if isinstance(t_entity, Entity) else str(t_entity)
                    
                    # Pure semantic similarity
                    score = self.semantic_enricher.compute_semantic_similarity(s_name, t_name)
    ...
    def _find_identical_predicates(self, source_rels: List, target_rels: List) -> List[Dict]:
                    ...
                    if isinstance(s_rel, Relation) and isinstance(t_rel, Relation):
                        ...
                        if s_rel.predicate == t_rel.predicate:
                            mappings.append({
                                'source_rel': s_rel,
                                'target_rel': t_rel,
                                'entity_constraints': list(zip(s_rel.arguments, t_rel.arguments))
                            })
                        # Semantic similarity match
                        elif s_rel.semantic_type and s_rel.semantic_type == t_rel.semantic_type:
                            mappings.append({
                                'source_rel': s_rel,
                                'target_rel': t_rel,
                                'entity_constraints': list(zip(s_rel.arguments, t_rel.arguments))
                        ...
                        if len(s_rel) >= 3 and len(t_rel) >= 3 and s_rel[1] == t_rel[1]:
                            mappings.append({
                                'source_rel': s_rel,
                                'target_rel': t_rel,
                                'entity_constraints': [(s_rel[0], t_rel[0]), (s_rel[2], t_rel[2])]
                            })
                except Exception as e:
    ...
    def _extend_mappings_structurally(self, initial_mappings: List[Dict],
                                     source_rels: List, target_rels: List,
                                     entity_candidates: Dict) -> Dict:
    ...
    def _evaluate_structural_consistency(self, mapping: Dict, source: Dict, target: Dict) -> float:
        """Evaluate consistency"""
                ...
                for rel in structure['relations']:
                    if isinstance(rel, Relation):
                        ...
                        relations.append(Relation(
                            ...
                            arguments=[rel.get('subject', ''), rel.get('object', '')],
        ...
        try:
            ...
            for rel in relations:
                if isinstance(rel, Relation) and len(rel.arguments) >= 2:
                    G.add_edge(rel.arguments[0], rel.arguments[1],
    ...
    def find_multiple_analogies(self, target_problem: Dict, k: int = 5) -> List[Dict[str, Any]]:
    ...
    def save_model(self, name: str = "default"):
        ...
        model_data = {
            ...
            'mapping_patterns': dict(self.mapping_patterns) if self.enable_learning else None,
            'domain_similarities': dict(self.domain_similarities) if self.enable_learning else None,
            'stats': self.stats,
    ...
    def load_model(self, name: str = "default"):
        ...
        try:
            ...
            if self.enable_learning and model_data.get('learned_weights'):
                ...
                self.domain_similarities = defaultdict(float, model_data['domain_similarities'])
            
            if 'semantic_enricher_cache' in model_data:
                self.semantic_enricher.embedding_cache = model_data['semantic_enricher_cache']
            
            if 'tfidf_corpus' in model_data:
                self.semantic_enricher._tfidf_corpus = model_data['tfidf_corpus']
                self.semantic_enricher._tfidf_fitted = model_data.get('tfidf_fitted', False)
                
                # Refit vectorizer if available and if it was fitted before
                if (self.semantic_enricher._tfidf_vectorizer and 
                    len(self.semantic_enricher._tfidf_corpus) > 1 and 
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.enable_learning:
            top_patterns = sorted(
                self.mapping_patterns.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]
            
            top_domains = sorted(
                self.domain_similarities.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]
    ...
    def reason(self, input_data: Any, query: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if isinstance(input_data, dict):
            ...
            if 'problem' in input_data or 'target_problem' in input_data:
                target_problem = input_data.get('problem', input_data.get('target_problem'))
            ...
            # Specific domain analogy
                ...
                result = self.find_structural_analogy(source_domain, target_problem, mapping_type)
        ...
        return {
            'found': False,
            'reason': 'Unsupported input format',
            'confidence': 0.0
        }
    
    def analyze_text_analogy(self, source_text: str, target_text: str) -> Dict[str, Any]:
        ...
        if not structure['entities']:
            ...
            for word in words:
                if word and word[0].isupper():
                    structure['entities'].append(Entity(name=word, entity_type='object'))
...
def compute_conceptual_distance(concept1: str, concept2: str, 
                               enricher: SemanticEnricher) -> float:
    """
...
def find_conceptual_path(start_concept: str, end_concept: str,
                        knowledge_graph: nx.DiGraph) -> List[str]:
    """
...
def cluster_analogies(analogies: List[AnalogicalMapping], 
                     n_clusters: int = 3) -> Dict[int, List[AnalogicalMapping]]:
    """
...
def explain_analogy_differences(mapping1: AnalogicalMapping, 
                                mapping2: AnalogicalMapping) -> str:
    """
    ...
    if abs(score_diff) > 0.1:
        ...
        explanation += f"({mapping1.mapping_score:.2f} vs {mapping2.mapping_score:.2f}).\n"
    else:
    ...
    if mapping1.structural_depth != mapping2.structural_depth:
        deeper = "first" if mapping1.structural_depth > mapping2.structural_depth else "second"
...
def test_semantic_similarity():
    ...
    print(f"cat vs quantum: {enricher.compute_semantic_similarity('cat', 'quantum'):.3f}")
    print(f"running vs runner: {enricher.compute_semantic_similarity('running', 'runner'):.3f}")
    print(f"car vs automobile: {enricher.compute_semantic_similarity('car', 'automobile'):.3f}")
