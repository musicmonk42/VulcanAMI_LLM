...
class TableauProver(BaseProver):
    ...
    def _build_tableau(
        self, formulas: List[Clause], start_time: float, timeout: float
    ) -> Optional[Dict]:
        """
            ...
            if current["non_units"]:
                ...
                if expansions:
                    ...
                    for expansion in expansions:
                        ...
                        new_branch = {
                            ...
                            "instantiated_universals": current[
                                "instantiated_universals"
                            ].copy(),
                        }
                ...
                if formula_idx is not None and expansion_type == "universal":
                    ...
                    expansions = self._expand_universal(
                        formula, formula_idx, current["instantiated_universals"]
                    )
    ...
    def _expand_universal(
        self, clause: Clause, formula_idx: int, instantiated: Set[Tuple]
    ) -> List[List[Clause]]:
        """
    ...
    def _expand_existential(
        self, clause: Clause, formula_idx: int
    ) -> List[List[Clause]]:
        """
...
class ModelEliminationProver(BaseProver):
    ...
    def prove(
        ...
        for literal in goal.literals:
            ...
            result = self._me_solve(
                [literal], program, set(), {}, 0, start_time, timeout
            )
    ...
    def _clausify(self, clause: Clause) -> List[Dict]:
        ...
        for i, head_lit in enumerate(clause.literals):
            ...
            program_clause = {"head": head, "body": body, "original_clause": clause}
    ...
    def _me_solve(
        ...
        if not goals:
            return {"success": True, "substitution": substitution, "depth": depth}
        ...
        for prog_clause in program:
            ...
            if new_subst is not None:
                ...
                new_goals = [
                    self.unifier.apply_to_literal(lit, new_subst)
                    for lit in renamed_body
                ] + remaining_goals
    ...
    def _unify_literals(
        self, lit1: Literal, lit2: Literal, subst: Dict[str, Term]
    ...
    def _rename_variables_global(self, clause: Dict) -> Dict:
        ...
        def rename_term(term: Term) -> Term:
            if isinstance(term, Variable):
                ...
                return Function(
                    name=term.name, args=[rename_term(arg) for arg in term.args]
                )
            else:
                return term
...
class ConnectionMethodProver(BaseProver):
    ...
    def _build_matrix(self, clauses: List[Clause]) -> List[List[Dict]]:
        ...
        for clause in clauses:
            clause_paths = []
            for literal in clause.literals:
                path = {"literal": literal, "clause": clause, "connections": []}
    ...
    def _find_spanning_mating(
        ...
        result = self._search_mating(
            matrix, mating, covered_clauses, 0, start_time, timeout
        )
    ...
    def _search_mating(
        ...
        if len(covered) == len(matrix):
            ...
            if self._is_consistent_mating(mating):
                return {"success": True, "mating": mating.copy(), "depth": depth}
        ...
        for path in matrix[uncovered_clause]:
            for connection in path["connections"]:
                ...
                # Recursive search
                result = self._search_mating(
                    matrix, mating, new_covered, depth + 1, start_time, timeout
                )
    ...
    def _compose_substitutions(
        self, subst1: Dict[str, Term], subst2: Dict[str, Term]
    ) -> Optional[Dict[str, Term]]:
        """
...
class NaturalDeductionProver(BaseProver):
    ...
    def prove(
        self, goal: Clause, assumptions: List[Clause], timeout: float
    ) -> Tuple[bool, Optional[ProofNode], float]:
        """
        ...
        result = self._prove_recursive(goal, assumptions, [], 0, start_time, timeout)
    ...
    def _prove_recursive(
        ...
        for rule_name, rule_func in self.rules.items():
            result = rule_func(
                goal, assumptions, discharged, depth, start_time, timeout
            )
            if result is not None:
                return result
    ...
    def _modus_ponens(
        ...
        for assumption in assumptions:
            ...
            for i, lit in enumerate(assumption.literals):
                ...
                if (
                    lit.predicate == goal_lit.predicate
                    and lit.negated == goal_lit.negated
                ):
        ...
        for assumption1 in assumptions:
            ...
            for assumption2 in assumptions:
                ...
                for lit in assumption2.literals:
                    ...
                    if (
                        lit.predicate == p_lit.predicate
                        and lit.negated != p_lit.negated
                    ):
    ...
    def _and_introduction(
        ...
        for literal in goal.literals:
            ...
            proof = self._prove_recursive(
                part, assumptions, discharged, depth + 1, start_time, timeout
            )
            if proof is None:
                return None
    ...
    def _and_elimination(
        ...
        for assumption in assumptions:
            ...
            if len(assumption.literals) > 1 and target_lit in assumption.literals:
                proof = self._prove_recursive(
                    assumption, assumptions, discharged, depth + 1, start_time, timeout
                )
    ...
    def _double_negation_elimination(
        self,
        goal: Clause,
        assumptions: List[Clause],
        discharged: List[Clause],
        ...
        double_neg_proof = self._prove_recursive(
            double_neg_clause, assumptions, discharged, depth + 1, start_time, timeout
        )
        ...
        for i, assumption1 in enumerate(assumptions):
            ...
            for assumption2 in assumptions[i + 1 :]:
                ...
                if (
                    ...
                    and len(lit1.terms) == len(lit2.terms)
                ):
    ...
    def _universal_introduction(
        ...
        proof = self._prove_recursive(
            goal, assumptions, discharged, depth + 1, start_time, timeout
        )
...
class ParallelProver:
    ...
    def prove_parallel(
        self, goal: Clause, kb: List[Clause], timeout: float = 10.0
    ) -> Tuple[bool, Optional[ProofNode], float, str]:
        """
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for method_name, prover in self.provers.items():
                future = executor.submit(
                    self._prove_with_method, prover, method_name, goal, kb, timeout
                )
