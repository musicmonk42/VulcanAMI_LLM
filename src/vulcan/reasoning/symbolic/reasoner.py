...
from .core import (
    ...
    Unifier, ProofNode, KnowledgeBase
)
from .provers import (
    BaseProver, TableauProver, ResolutionProver, 
    ModelEliminationProver, ParallelProver
)
from .solvers import (
    BayesianNetworkReasoner, VariableType, CSPSolver
)
# FIX: This import will now work correctly after the move of the classes
from .parsing import Lexer, Parser, ASTConverter
...
class SymbolicReasoner:
    ...
    def _parse_simple_literal(self, literal_str: str, negated: bool) -> Optional[Literal]:
...
class ProbabilisticReasoner:
    ...
    def learn_structure(self, data: List[Dict[str, Any]], 
    ...
    def _set_single_rule_cpd(self, var_name: str, rule: Dict[str, Any]):
        ...
        if not var.parents:
            # No parents
            table = {
                (): {True: confidence, False: 1.0 - confidence}
            }
        else:
    ...
    def _set_noisy_or_cpd(self, var_name: str, rules: List[Dict[str, Any]]):
        ...
        if not var.parents:
            ...
            table = {
                (): {True: avg_conf, False: 1.0 - avg_conf}
            }
        else:
            table = {}
            parent_combos = self._generate_parent_combinations(var.parents)
    ...
    def _rule_satisfied(self, rule: Dict[str, Any], 
                       parent_values: Tuple, 
                       parent_names: List[str]) -> bool:
        """Check if rule conditions are satisfied by parent values."""
        # Build assignment
        assignment = {parent_names[i]: parent_values[i] 
    ...
    def intervention_query(self, query_var: str, 
                          intervention: Dict[str, Any],
                          evidence: Optional[Dict[str, Any]] = None) -> Dict[Any, float]:
        """
...
class HybridReasoner:
    ...
    def add_rule(self, rule_str: str, rule_type: str = "auto", 
    ...
    def query(self, query_str: str, evidence: Optional[Dict[str, Any]] = None,
    ...
    def _select_method(self, query_str: str, 
    ...
    def _combine_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'probabilistic' in results:
            ...
            if isinstance(prob, dict):
                ...
                if True in prob:
                    combined['confidence'] = max(combined.get('confidence', 0.0), prob[True])
