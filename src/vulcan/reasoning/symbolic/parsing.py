...
class Lexer:
    ...
    def _get_next_token(self) -> Token:
        ...
        if not match:
            raise SyntaxError(
                f"Lexer error: Unexpected character at line {self.line}, column {self.column}"
            )
        ...
        if kind == "IDENTIFIER":
            ...
            token_type = self.token_map.get(
                value,
                TokenType.IMPLIES if kind == "ARROW_OP" else self.token_map.get(value),
            )
            return Token(token_type, value, self.line, start_col)
        elif kind == "SYMBOL":
...
class Parser:
    ...
    def quantified(self) -> ASTNode:
        ...
        return ASTNode(quant_type, children=[body], metadata={"variables": variables})
    ...
    def term(self) -> ASTNode:
        ...
        if name[0].isupper():
            ...
            return ASTNode(
                NodeType.VARIABLE if name == "x" else NodeType.CONSTANT, value=name
            )
        else:
            # Lowercase multi-char = CONSTANT (john, mary, socrates)
...
class FormulaUtils:
    ...
    def get_free_variables(
        node: ASTNode, bound_vars: Optional[Set[str]] = None
    ) -> Set[str]:
        """
    ...
    @staticmethod
    def substitute_variable(node: ASTNode, old_var: str, new_term: ASTNode) -> ASTNode:
        """
        ...
        if node.node_type == NodeType.VARIABLE:
            ...
            # Substitute in body
            new_children = [
                FormulaUtils.substitute_variable(c, old_var, new_term)
    ...
    def is_clause(node: ASTNode) -> bool:
        ...
        if node.node_type == NodeType.OR:
            return all(
                FormulaUtils.is_literal(c) or FormulaUtils.is_clause(c)
                for c in node.children
            )
    ...
    def is_cnf(node: ASTNode) -> bool:
        ...
        if node.node_type == NodeType.AND:
            return all(
                FormulaUtils.is_clause(c) or FormulaUtils.is_cnf(c)
                for c in node.children
            )
    ...
    def _move_quantifiers_out(self, node: ASTNode) -> ASTNode:
        ...
        if node.node_type in [
            ...
            for quant_type, variables in reversed(left_quants + right_quants):
                result = ASTNode(
                    quant_type, children=[result], metadata={"variables": variables}
                )
...
class SkolemFunction:
    ...
    def to_term(self) -> ASTNode:
        ...
        if not self.arguments:
            ...
            # Skolem function
            return ASTNode(NodeType.FUNCTION, value=self.name, children=self.arguments)
...
class Skolemizer:
    ...
    def _skolemize_recursive(self, node: ASTNode, universal_vars: List[str]) -> ASTNode:
        """
        ...
        if node.node_type == NodeType.FORALL:
            ...
            for var in exist_vars:
                skolem_func = self._create_skolem_function(
                    var, dependencies.get(var, set()), set(universal_vars)
                )
            ...
            # Skolemize children
            new_children = [
                self._skolemize_recursive(c, universal_vars) for c in node.children
            ]
            result = node.copy()
            result.children = new_children
            return result
    ...
    def _analyze_dependencies(
        self, body: ASTNode, exist_vars: List[str], universal_vars: List[str]
    ) -> Dict[str, Set[str]]:
        """
    ...
    def _create_skolem_function(
        self, var: str, dependencies: Set[str], available_vars: Set[str]
    ) -> SkolemFunction:
        """
    ...
    def _move_negations_inward(self, node: ASTNode) -> ASTNode:
        ...
        if node.node_type == NodeType.NOT:
            ...
            # Quantifier negation
                ...
                return ASTNode(
                    NodeType.EXISTS, children=[body], metadata=inner.metadata
                )
                ...
                return ASTNode(
                    NodeType.FORALL, children=[body], metadata=inner.metadata
                )
    ...
    def _simplify(self, node: ASTNode) -> ASTNode:
        ...
        if node.node_type in [NodeType.AND, NodeType.OR]:
            ...
            if node.node_type == NodeType.OR:
                ...
                unique_children = [
                    c for c in unique_children if c.node_type != NodeType.TRUE
                ]

            # Simplify single child
            if len(unique_children) == 0:
                return (
                    ASTNode(NodeType.TRUE)
                    if node.node_type == NodeType.AND
                    else ASTNode(NodeType.FALSE)
                )
...
class ClauseExtractor:
    ...
    def _ast_to_literal(self, node: ASTNode) -> Literal:
        ...
        if node.node_type == NodeType.PREDICATE:
            ...
            return Literal(predicate=node.value, terms=terms, negated=False)
...
class FormulaBuilder:
    ...
    def predicate(name: str, *args: ASTNode) -> ASTNode:
        ...
        return ASTNode(NodeType.PREDICATE, value=name, children=list(args))
    ...
    def function(name: str, *args: ASTNode) -> ASTNode:
        ...
        return ASTNode(NodeType.FUNCTION, value=name, children=list(args))
    ...
    def forall(variables: List[str], body: ASTNode) -> ASTNode:
        ...
        return ASTNode(
            NodeType.FORALL, children=[body], metadata={"variables": var_nodes}
        )
    ...
    def exists(variables: List[str], body: ASTNode) -> ASTNode:
        ...
        var_nodes = [ASTNode(NodeType.VARIABLE, value=v) for v in variables]
        return ASTNode(
            NodeType.EXISTS, children=[body], metadata={"variables": var_nodes}
        )
