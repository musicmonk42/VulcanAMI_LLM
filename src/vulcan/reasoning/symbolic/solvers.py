...
class Factor:
    ...
    def multiply(self, other: 'Factor') -> 'Factor':
        ...
        for key1 in self.values:
            for key2 in other.values:
                # Merge keys
                merged_key = self._merge_keys(key1, key2, self.variables, other.variables, all_vars)
    ...
    def _merge_keys(self, key1: Tuple, key2: Tuple, vars1: List[str], 
                   vars2: List[str], all_vars: List[str]) -> Optional[Tuple]:
        """
...
class CPT:
    ...
    def set_probability(self, var_value: Any, parent_values: Tuple[Any, ...], prob: float):
...
class BayesianNetworkReasoner:
    ...
    def add_variable(self, name: str, var_type: VariableType, 
                    ...
                    parents: Optional[List[str]] = None) -> RandomVariable:
        """
    ...
    def set_gaussian_cpd(self, variable: str, coefficients: Dict[str, float],
    ...
    def query(self, query_var: str, evidence: Optional[Dict[str, Any]] = None,
             method: str = "variable_elimination") -> Dict[Any, float]:
        """
    ...
    def _variable_elimination(self, query_var: str, 
                             evidence: Dict[str, Any]) -> Dict[Any, float]:
        """
    ...
    def _create_factor_from_cpt(self, var_name: str, 
                                evidence: Dict[str, Any]) -> Optional[Factor]:
        """
        ...
        if var_name in evidence:
            ...
            for parent_combo in parent_combos:
                ...
                prob = cpt.get_probability(
                    evidence[var_name], 
                    tuple(parent_combo[p] for p in var.parents)
                )
                values[key] = prob
            ...
            if len(var.parents) == 0:
                ...
                for parent_combo in parent_combos:
                    ...
                    unobserved_parent_values = [parent_combo[p] for p in var.parents if p not in evidence]
                    
                    for val in var.domain:
                        prob = cpt.get_probability(val, tuple(parent_combo[p] for p in var.parents))
    ...
    def _get_parent_combinations(self, parents: List[str], 
    ...
    def _find_elimination_order(self, vars_to_eliminate: Set[str], 
    ...
    def _gaussian_inference(self, query_var: str, 
    ...
    def _hybrid_inference(self, query_var: str, 
                         evidence: Dict[str, Any]) -> Dict[Any, float]:
        """
        ...
        discrete_evidence = {k: v for k, v in evidence.items() 
                           if self.variables[k].var_type == VariableType.DISCRETE}
        continuous_evidence = {k: v for k, v in evidence.items()
                             if self.variables[k].var_type in [VariableType.CONTINUOUS, VariableType.GAUSSIAN]}
    ...
    def _mcmc_inference(self, query_var: str, evidence: Dict[str, Any],
                       num_samples: int = 10000, burn_in: int = 1000) -> Dict[Any, float]:
        """
        ...
        if var.var_type == VariableType.DISCRETE:
            ...
            distribution = {val: count / len(samples) 
    ...
    def learn_parameters_em(self, data: List[Dict[str, Any]], 
                           max_iterations: int = 100,
                           tolerance: float = 1e-4) -> List[float]:
        """
    ...
    def _update_counts_complete(self, sample: Dict[str, Any], 
                                expected_counts: Dict[str, Any]):
        """Update counts for complete data point."""
        for var_name, var in self.variables.items():
            if var.var_type == VariableType.DISCRETE:
                var_value = sample[var_name]
                parent_values = tuple(sample[p] for p in var.parents) if var.parents else ()
    ...
    def _update_counts_missing(self, observed: Dict[str, Any], 
                              missing: List[str],
                              expected_counts: Dict[str, Any]):
        ...
        for _ in range(num_samples):
            ...
            for var_name in missing:
                ...
                if var.var_type == VariableType.DISCRETE:
                    # Sample from conditional distribution
                    completed[var_name] = self._gibbs_sample_discrete(var_name, completed)
                else:
                    completed[var_name] = self._gibbs_sample_continuous(var_name, completed)
    ...
    def _update_counts_weighted(self, sample: Dict[str, Any],
                               expected_counts: Dict[str, Any],
                               weight: float):
        ...
        for var_name, var in self.variables.items():
            if var.var_type == VariableType.DISCRETE:
                var_value = sample[var_name]
                parent_values = tuple(sample[p] for p in var.parents) if var.parents else ()
                expected_counts['discrete'][var_name][parent_values][var_value] += weight
    ...
    def _fit_gaussian_cpd(self, var_name: str, X: List[List[float]], y: List[float]):
        ...
        try:
            ...
            self.set_gaussian_cpd(var_name, coefficients, intercept, max(variance, 1e-10))
        ...
        for sample in data:
            ...
            for var_name, var in self.variables.items():
                ...
                if var.var_type == VariableType.DISCRETE:
                    cpt = self.cpts.get(var_name)
                    if cpt:
                        parent_values = tuple(sample.get(p) for p in var.parents) if var.parents else ()
    ...
    def learn_structure_pc(self, data: List[Dict[str, Any]], 
                          alpha: float = 0.05) -> Dict[str, List[str]]:
        """
    ...
    def _test_conditional_independence(self, var_x: str, var_y: str,
        ...
        if var_x_obj.var_type == VariableType.DISCRETE and var_y_obj.var_type == VariableType.DISCRETE:
            return self._chi_square_test(var_x, var_y, conditioning_set, data, alpha)
        else:
            return self._partial_correlation_test(var_x, var_y, conditioning_set, data, alpha)
    
    def _chi_square_test(self, var_x: str, var_y: str,
                        conditioning_set: List[str],
                        data: List[Dict[str, Any]],
                        alpha: float) -> bool:
        ...
        if not conditioning_set:
            # Unconditional test
            counts = Counter((sample.get(var_x), sample.get(var_y)) 
                           for sample in data 
    ...
    def _partial_correlation_test(self, var_x: str, var_y: str,
                                  conditioning_set: List[str],
                                  data: List[Dict[str, Any]],
                                  alpha: float) -> bool:
    ...
    def learn_structure_k2(self, data: List[Dict[str, Any]], 
                          variable_order: Optional[List[str]] = None,
                          max_parents: int = 3) -> Dict[str, List[str]]:
        """
    ...
    def _compute_k2_score(self, var: str, parents: List[str], 
...
class CSPSolver:
    ...
    def _is_consistent(self, var: str, value: Any, 
    ...
    def _fc_backtrack(self, assignment: Dict[str, Any], 
                     domains: Dict[str, Set[Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _forward_check(self, var: str, value: Any,
                      assignment: Dict[str, Any],
                      domains: Dict[str, Set[Any]]) -> Optional[Dict[str, Set[Any]]]:
    ...
    def _min_conflicts_search(self, max_steps: int = 1000) -> Optional[Dict[str, Any]]:
        ...
        for step in range(max_steps):
            ...
            for value in self.variables[var].domain:
                assignment[var] = value
                conflicts = sum(1 for c in self.constraints if not c.is_satisfied(assignment))
