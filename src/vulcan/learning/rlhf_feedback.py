...
class RLHFManager:
    ...
    def receive_feedback(self, feedback: FeedbackData):
        ...
        with self._buffer_lock:
            ...
            if feedback.feedback_type == 'preference' and 'preferred_over' in feedback.metadata:
            ...
            should_process = len(self.feedback_buffer) >= self.config.reward_model_update_freq
    ...
    def _update_reward_model_direct(self, batch: List[FeedbackData]):
        ...
        logger.info(f"Updated reward model with {len(batch)} direct feedback, loss: {loss.item():.4f}")
    ...
    def _update_reward_model_preferences(self, batch: List[FeedbackData]):
        ...
        for feedback in batch:
            try:
                ...
                rejected_input = torch.cat([rejected_feat, context_feat]).to(self.device)
        ...
        if num_pairs > 0:
            ...
            logger.info(f"Updated reward model with {num_pairs} preference pairs, loss: {preference_loss.item():.4f}")
    ...
    def update_policy_with_ppo(self, trajectories: List[Dict[str, Any]]):
        ...
        for trajectory in trajectories:
            try:
                ...
                if not isinstance(states, torch.Tensor):
                    states = torch.stack([s.detach().clone() if isinstance(s, torch.Tensor) 
                                        else torch.tensor(s, dtype=torch.float32) for s in states])
                if not isinstance(actions, torch.Tensor):
                    actions = torch.stack([a.detach().clone() if isinstance(a, torch.Tensor)
                                         else torch.tensor(a, dtype=torch.float32) for a in actions])
                if not isinstance(log_probs, torch.Tensor):
                    log_probs = torch.stack([lp.detach().clone() if isinstance(lp, torch.Tensor)
                                           else torch.tensor(lp, dtype=torch.float32) for lp in log_probs])
        ...
        for epoch in range(self.config.ppo_epochs):
            ...
            for i in range(0, len(all_states), self.config.batch_size):
                ...
                new_log_probs, entropy = self._compute_log_probs(batch_states, batch_actions)
                ...
                surr2 = torch.clamp(ratio, 
                                  ...
                                  1 + self.config.ppo_clip) * batch_advantages
        ...
        logger.info(f"PPO update complete: policy_loss={policy_loss.item():.4f}, "
                   f"value_loss={value_loss.item():.4f}")
    
    def _compute_log_probs(self, states: torch.Tensor, 
        ...
        def process_loop():
            while not self._shutdown_event.is_set():
                try:
                    ...
                    with self._buffer_lock:
                        should_process = len(self.feedback_buffer) >= self.config.batch_size
    ...
    async def fetch_feedback_from_api(self, 
                                     since_timestamp: Optional[float] = None,
                                     limit: int = 100) -> List[FeedbackData]:
        ...
        try:
            ...
            async with self.api_session.get(url, headers=headers, params=params) as response:
    ...
    async def push_metrics_to_api(self, metrics: Dict[str, Any]):
        ...
        try:
            ...
            async with self.api_session.post(url, json=metrics, headers=headers) as response:
                if response.status == 200:
...
class LiveFeedbackProcessor:
    ...
    async def start_monitoring(self):
        ...
        if not self._monitoring_task:
            self._monitoring_task = asyncio.create_task(self.performance_monitoring_loop())
    ...
    async def _immediate_adjustment(self, feedback: Dict[str, Any]):
        ...
        with self._lock:
            if feedback.get('type') == 'error':
                ...
                logger.warning(f"Critical error feedback: reducing LR to {self.adaptive_lr}")
    ...
    def _get_current_accuracy(self) -> float:
        ...
        with self._lock:
            if self.performance_tracker['total_predictions'] > 0:
                return (self.performance_tracker['correct_predictions'] / 
                       self.performance_tracker['total_predictions'])
    ...
    async def _handle_alerts(self, alerts: List[Dict[str, Any]]):
        ...
        for alert in alerts:
            logger.warning(f"Performance alert: {alert['metric']} = {alert['value']:.2f} "
                         f"({alert['type']} threshold {alert['threshold']})")
            
            # Take action based on alert type
            if alert['metric'] == 'memory_usage_mb' and alert['type'] == 'above_threshold':
