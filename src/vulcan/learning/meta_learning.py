...
class TaskDetector:
    ...
    def __init__(self, threshold: float = 0.3, save_path: str = "task_signatures"):
        ...
        self.task_encoder_optimizer = optim.Adam(self.task_encoder.parameters(), lr=0.001)
    ...
    def detect_task(self, experience: Dict[str, Any]) -> str:
        ...
        with self._lock:
            ...
            signature_tensor = torch.tensor(signature, dtype=torch.float32, device=self.device)
            with torch.no_grad():
...
class MetaLearner:
    ...
    def __init__(self, base_model: nn.Module, 
        ...
        self.param_history = ParameterHistoryManager(base_path="meta_params", config=config)
    ...
    def adapt(self, support_set: Dict[str, torch.Tensor], 
             num_steps: Optional[int] = None,
             task_id: Optional[str] = None) -> Tuple[nn.Module, Dict[str, Any]]:
    ...
    def _adapt_maml(self, support_set: Dict[str, torch.Tensor], 
                   num_steps: int, inner_lr: float) -> Tuple[nn.Module, Dict]:
        ...
        stats = {
            ...
            'final_loss': adaptation_trajectory[-1]['loss'] if adaptation_trajectory else 0,
            'num_steps': num_steps
        }
        
        return adapted_model, stats
    
    def _adapt_fomaml(self, support_set: Dict[str, torch.Tensor], 
                     num_steps: int, inner_lr: float) -> Tuple[nn.Module, Dict]:
    ...
    def _adapt_reptile(self, support_set: Dict[str, torch.Tensor], 
                      num_steps: int, inner_lr: float) -> Tuple[nn.Module, Dict]:
    ...
    def _adapt_anil(self, support_set: Dict[str, torch.Tensor], 
                   num_steps: int, inner_lr: float) -> Tuple[nn.Module, Dict]:
        ...
        if not final_layer_params:
            ...
            return adapted_model, {'trajectory': [], 'final_loss': 0, 'num_steps': 0, 'algorithm': 'anil'}
    ...
    def _meta_update_reptile(self, tasks: List[Dict[str, Any]]):
        ...
        trajectory_id = self.param_history.start_trajectory(
            task_id=f"reptile_batch_{time.time()}",
            agent_id="meta_learner"
        )
        ...
        for task in tasks:
            # Adapt to task
            adapted_model, adapt_stats = self.adapt(task['support'], task_id=task.get('task_id'))
            ...
            for orig_param, adapted_param in zip(self.base_model.parameters(), 
                                                adapted_model.parameters()):
                orig_param.data = orig_param.data + epsilon * (adapted_param.data.detach() - orig_param.data)
    ...
    def _analyze_gradients(self, task_gradients: List[List[torch.Tensor]], 
        ...
        if len(task_gradients) > 1:
            ...
            for i in range(len(task_gradients)):
                for j in range(i + 1, len(task_gradients)):
                    sim = self._gradient_similarity(task_gradients[i], task_gradients[j])
                    similarities.append(sim)
            
            if similarities:
                self.gradient_stats['gradient_similarity'].append(np.mean(similarities))
    
    def _gradient_similarity(self, grads1: List[torch.Tensor], 
    ...
    def _create_batch_from_indices(self, buffer: List, indices: np.ndarray) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            'y': torch.stack(batch_y).squeeze(-1) if batch_y else torch.zeros(len(batch_x), device=self.device)
        }
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            stats = {
                ...
                'avg_task_loss': np.mean(list(self.task_losses)) if self.task_losses else 0,
