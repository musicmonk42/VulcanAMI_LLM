...
class ProgressiveNeuralNetwork(nn.Module):
    ...
    def add_column(self, task_id: str) -> None:
        ...
        self.columns.append(ProgressiveColumn(
        ...
        if column_idx > 0:
            for prev_idx in range(column_idx):
                ...
                if key in self.lateral_connections:
                    ...
                    for i, (prev_act, lat_conn) in enumerate(zip(prev_activations, lateral)):
                        if i < len(current_activations) - 1:
                            current_activations[i] = current_activations[i] + lat_conn(prev_act)
...
class EnhancedContinualLearner(nn.Module):
    ...
    def __init__(self, embedding_dim: int = EMBEDDING_DIM, 
        ...
        self._NON_PICKLABLE_TYPES = tuple(t for t in self._NON_PICKLABLE_TYPES if t is not None)
        # --- END BUG FIX ---
    ...
    def _increment_consolidation_counter(self) -> bool:
        ...
        with self._stats_lock:
            ...
            should_consolidate = self.consolidation_counter >= self.config.consolidation_threshold
        ...
        try:
            ...
            if self.config.audit_trail_enabled and 'embedding' in clean_experience:
                ...
                try: 
                    self.param_history.record_step(
                        state=embedding,
                        action=task_id,
                        reward=clean_experience.get('reward', 0.0), # Use clean_experience
            ...
            if self.rlhf_manager and 'feedback' in clean_experience:
                feedback = FeedbackData(
                    ...
                    human_preference=clean_experience.get('feedback'), # Use clean_experience
                    reward_signal=clean_experience.get('reward', 0.0) # Use clean_experience
                )
                # --- START BUG FIX: Wrap external call to catch PicklingError ---
                try:
                    self.rlhf_manager.receive_feedback(feedback)
                except (TypeError, pickle.PicklingError, AttributeError) as rlhf_e:
                    logger.warning(f"Failed to process RLHF feedback (pickle error): {rlhf_e}")
                # --- END BUG FIX ---
            
            # Online meta-learning
            # --- START BUG FIX: Wrap external call to catch PicklingError ---
            try:
                self.meta_learner.online_meta_update(clean_experience) # Use clean_experience
            except (TypeError, pickle.PicklingError, AttributeError) as meta_e:
                logger.warning(f"Failed to process meta-learning update (pickle error): {meta_e}")
            # --- END BUG FIX ---
                ...
                if self.config.checkpoint_frequency > 0:
                    ...
                    with self._stats_lock:
                        ...
                        components = {
                            ...
                            'replay_buffer': sanitize_buffer_items(list(self.replay_buffer)),
                            'task_buffers': {k: sanitize_buffer_items(list(v)) for k, v in self.task_buffers.items()},
                            'performance_tracker': {k: list(v) for k, v in self.performance_tracker.items()},
                        ...
                        for key, value in components.items():
                            try:
                                ...
                                logger.warning(f"Skipping non-picklable component '{key}': {e}")
    ...
    def _allocate_task_capacity(self, task_id: str):
        ...
        if total_params > self.free_capacity.numel():
            logger.warning(f"Resizing free_capacity tensor: {self.free_capacity.numel()} -> {total_params}")
        ...
        if len(free_indices) > 0:
            ...
            num_to_allocate = min(
                int(capacity_per_task * total_params),
                len(free_indices)
            )
    ...
    def _process_with_task(self, experience: Dict[str, Any], 
        ...
        with self._stats_lock:
            ...
            if task_id in self.task_info:
                recent_losses = list(self.performance_tracker.get(task_id, deque()))[-10:]
                if recent_losses:
                    self.task_info[task_id].performance = 1.0 / (1.0 + np.mean(recent_losses))
    ...
    def _compute_loss_with_ewc(self, output: torch.Tensor, 
                               target: torch.Tensor, 
    ...
    def _intelligent_experience_replay(self, current_task: str, n_samples: int = 32) -> float:
        ...
        for task, weight in task_weights.items():
            ...
            if n_task_samples > 0 and samples_per_task[task]:
                ...
                indices = np.random.choice(len(task_exp_list), min(n_task_samples, len(task_exp_list)), replace=False)
    ...
    def _update_continual_metrics(self):
        ...
        with self._stats_lock:
            ...
            self.continual_metrics.average_accuracy = np.mean(accuracies) if accuracies else 0
            ...
            if len(self.task_order) > 1:
                ...
                self.continual_metrics.backward_transfer = np.mean(old_task_perfs) if old_task_perfs else 0
            
            # Calculate forward transfer (performance on new task)
            if self.task_order:
                latest_task = self.task_order[-1]
                if latest_task in self.task_info:
                    self.continual_metrics.forward_transfer = self.task_info[latest_task].performance
            ...
            self.continual_metrics.forgetting_measure = forgetting / max(1, len(self.task_order) - 1)
            
            # Update task accuracies
            for task_id in self.task_order:
                if task_id in self.task_info:
                    self.continual_metrics.task_accuracies[task_id] = self.task_info[task_id].performance
    
    def adapt_to_distribution_shift(self, 
                                   new_distribution_samples: List[np.ndarray]) -> Dict:
        ...
        old_embeddings = [exp['embedding'] for exp in list(self.replay_buffer)[-100:]
                         if 'embedding' in exp]
        ...
        logger.info(f"Detected distribution shift (magnitude: {shift_magnitude:.3f}), creating task: {shift_task_id}")
        ...
        with self._lock:
            with self._stats_lock:
                stats = {
                    ...
                    'total_experiences': sum(self.task_info[t].samples_seen for t in self.task_info),
                    ...
                    'free_capacity': float(self.free_capacity.mean()) if hasattr(self, 'free_capacity') else 1.0
                }
                
                if self.hierarchical_memory:
                    stats['hierarchical_memory'] = {
                        'num_levels': len(self.hierarchical_memory.levels) if hasattr(self.hierarchical_memory, 'levels') else 0
                    }
        ...
        with self._lock:
            with self._stats_lock:
                ...
                state = {
                    ...
                    'task_buffers': {k: sanitize_buffer(list(v)) for k, v in self.task_buffers.items()},
                    'performance_tracker': {k: list(v) for k, v in self.performance_tracker.items()},
                    'task_performance_history': dict(self.task_performance_history),
                    'consolidation_counter': self.consolidation_counter,
                    'embedding_dim': self.embedding_dim,
    ...
    def load_state(self, path: str):
        ...
        with self._lock:
            with self._stats_lock:
                ...
                self.task_order = state['task_order'].copy() if isinstance(state['task_order'], list) else list(state['task_order'])
                self.task_info = {}
                ...
                try:
                    ...
                    logger.warning(f"Failed to load full state dict, attempting partial load: {e}")
                    # Try to load only matching keys
                    current_state = self.state_dict()
                    filtered_state = {k: v for k, v in state['model_state'].items() if k in current_state}
                ...
                if 'replay_buffer' in state:
                    self.replay_buffer = deque(state['replay_buffer'], maxlen=self.config.replay_buffer_size)
                
                if 'task_buffers' in state:
                    self.task_buffers = {k: deque(v, maxlen=1000) for k, v in state['task_buffers'].items()}
                
                if 'performance_tracker' in state:
                    self.performance_tracker = {k: deque(v, maxlen=100) for k, v in state['performance_tracker'].items()}
                
                if 'task_performance_history' in state:
                    self.task_performance_history = dict(state['task_performance_history'])
                    ...
                    for task_id, opt_state in state['optimizer_states'].items():
                        if task_id in self.optimizers:
                            try:
                                ...
                                logger.warning(f"Failed to load optimizer state for {task_id}: {e}")
                    ...
                    for task_id, sched_state in state['scheduler_states'].items():
                        if task_id in self.schedulers:
                            try:
                                ...
                                logger.warning(f"Failed to load scheduler state for {task_id}: {e}")
