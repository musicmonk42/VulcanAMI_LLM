...
class UnifiedWorldModel(nn.Module):
    ...
    def __init__(
        ...
        self.projection_head = nn.Sequential(
            nn.Linear(state_dim, HIDDEN_DIM), nn.ReLU(), nn.Linear(HIDDEN_DIM, 128)
        )
    ...
    def forward(
        self, state: torch.Tensor, action: torch.Tensor, model_idx: Optional[int] = None
        ...
        if model_idx is not None:
            ...
            for dynamics_model, reward_model in zip(
                self.dynamics_ensemble, self.reward_ensemble
            ):
    ...
    def train_step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        for i, (dynamics_model, reward_model) in enumerate(
            zip(self.dynamics_ensemble, self.reward_ensemble)
        ):
            # Bootstrap sampling for ensemble diversity
            mask = torch.bernoulli(
                torch.ones(len(state), device=self.device) * 0.8
            ).bool()
            ...
            reward_loss = F.mse_loss(
                pred_reward,
                reward[mask].unsqueeze(-1) if reward[mask].dim() == 1 else reward[mask],
            )
        ...
        # Update statistics
        with self._lock:
            self.training_stats["dynamics_losses"].append(
                losses.get("dynamics", torch.tensor(0.0)).item()
            )
            self.training_stats["reward_losses"].append(
                losses.get("reward", torch.tensor(0.0)).item()
            )
            self.training_stats["inverse_losses"].append(losses["inverse"].item())
            self.training_stats["contrastive_losses"].append(
                losses["contrastive"].item()
            )
            self.training_stats["curiosity_rewards"].append(
                curiosity_reward.mean().item()
            )
    ...
    def _contrastive_loss(
        self, state: torch.Tensor, next_state: torch.Tensor, temperature: float = 0.1
    ) -> torch.Tensor:
        """Contrastive loss for representation learning"""
    ...
    def imagine_rollout(
        ...
        for i in range(min(horizon, len(action_sequence)):
            ...
            with torch.no_grad():
                next_state, reward, uncertainty = self.forward(
                    current_state, action, model_idx
                )
            ...
            curiosity_rewards.append(
                curiosity.mean().item() if curiosity.numel() > 1 else curiosity.item()
            )
    ...
    def plan_actions(
        ...
        **kwargs,
        ...
        if algorithm == PlanningAlgorithm.GREEDY:
            ...
            return self._plan_beam_search(
                current_state, candidate_actions, horizon, **kwargs
            )
    ...
    def _plan_greedy(
        ...
        for action in candidate_actions:
            rollout = self.imagine_rollout(current_state, [action] * horizon, horizon)
        ...
        for step in range(1, horizon):
            ...
            for beam_item in beam:
                for action in candidate_actions:
                    ...
                    with torch.no_grad():
                        next_state, reward, uncertainty = self.forward(
                            beam_item["state"], action
                        )

                    new_item = {
                        "actions": beam_item["actions"] + [action],
                        "state": next_state,
                        "cumulative_reward": beam_item["cumulative_reward"]
                        + reward.item(),
            ...
            new_beam.sort(
                key=lambda x: x["cumulative_reward"] - 0.1 * x["uncertainty"],
                reverse=True,
            )
    ...
    def _plan_mcts(
        ...
        for _ in range(num_simulations):
            ...
            while node.is_expanded() and len(path) < horizon:
                ...
                best_child = max(
                    node.children.values(), key=lambda n: n.ucb_score(c_puct)
                )
                node = best_child
                path.append(node)
        ...
        for _ in range(num_iters):
            ...
            for _ in range(population_size):
                ...
                for t in range(horizon):
                    action = mean[t] + std[t] * torch.randn(
            ...
            mean = torch.stack(
                [
                    torch.stack(
                        [
                            a[t].squeeze(0) if a[t].dim() > 1 else a[t]
    ...
    def get_training_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self.training_stats["curiosity_rewards"]:
                stats["avg_curiosity_reward"] = sum(
                    self.training_stats["curiosity_rewards"]
                ) / len(self.training_stats["curiosity_rewards"])
...
class CuriosityModule(nn.Module):
    ...
    def __init__(self, state_dim: int):
        ...
        self.forward_model = nn.Sequential(
            nn.Linear(
                HIDDEN_DIM // 2 + state_dim, HIDDEN_DIM
    ...
    def __init__(self, state_dim: int, num_levels: int = 3):
        ...
        for level in range(num_levels):
            ...
            self.encoders.append(
                nn.Sequential(
                    nn.Linear(current_dim, next_dim), nn.LayerNorm(next_dim), nn.ReLU()
                )
            )
