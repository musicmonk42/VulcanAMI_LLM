...
class ParameterHistoryManager:
    ...
    def __init__(self, base_path: str = "parameter_history", config: LearningConfig = None):
    ...
    def async_checkpoint(self, model: Union[nn.Module, Dict[str, Any]], metadata: Dict[str, Any] = None):
        """
        ...
        try:
            ...
            if isinstance(model, nn.Module):
                ...
                try:
                    ...
                    pickle.dumps(model)
                    state_dict = model
                    model_class = metadata.get('model_class', 'Unknown') if metadata else 'Unknown'
                except Exception as e:
            ...
            self.checkpoint_queue.put(
                (state_dict, model_class, metadata),
                timeout=0.1
            )
        except queue.Full:
            with self._lock:
        ...
        try:
            ...
            if 'checksum' in checkpoint:
                ...
                if calculated_checksum != checkpoint['checksum']:
                    logger.warning(f"Checksum mismatch for checkpoint: {checkpoint_path}")
    ...
    def list_checkpoints(self, sort_by: str = 'timestamp', 
        ...
        with self._lock:
            ...
            if sort_by in ['timestamp', 'size_bytes', 'checkpoint_id']:
                try:
                    checkpoints.sort(key=lambda x: x.get(sort_by, 0), reverse=not ascending)
                except Exception as e:
        ...
        with self._lock:
            for checkpoint in self.parameter_history:
                ...
                try:
                    ...
                    logger.debug(f"Skipping checkpoint due to metadata comparison error: {e}")
                    continue
        ...
        with self._lock:
            for i, checkpoint in enumerate(self.parameter_history):
                if checkpoint['checkpoint_id'] == checkpoint_id:
                    ...
                    del history_list[i]
                    self.parameter_history = deque(history_list, maxlen=self.parameter_history.maxlen)
                    
                    logger.info(f"Deleted checkpoint: {checkpoint_id}")
                    return True
        return False
    
    def start_trajectory(self, task_id: str, agent_id: str, 
    ...
    def end_trajectory(self, save: bool = True, metadata: Dict[str, Any] = None) -> Optional[str]:
        ...
        with self._lock:
            ...
            if self.current_trajectory.rewards:
                self.current_trajectory.metadata['total_reward'] = sum(self.current_trajectory.rewards)
                self.current_trajectory.metadata['avg_reward'] = np.mean(self.current_trajectory.rewards)
            
            if self.current_trajectory.losses:
                self.current_trajectory.metadata['avg_loss'] = np.mean(self.current_trajectory.losses)
                self.current_trajectory.metadata['final_loss'] = self.current_trajectory.losses[-1]
    ...
    def import_checkpoint_history(self, import_path: str):
        ...
        with self._lock:
            ...
            for checkpoint in import_data.get('checkpoints', []):
                ...
                existing = any(c['checkpoint_id'] == checkpoint['checkpoint_id'] 
    ...
    def _cleanup_old_checkpoints(self):
        ...
        while len(self.parameter_history) > self.config.max_checkpoints:
            ...
            if old_path.exists():
                try:
                    old_path.unlink()
                    logger.debug(f"Deleted old checkpoint: {old_checkpoint['checkpoint_id']}")
                except Exception as e:
    ...
    def _load_checkpoint_history(self):
        ...
        if history_file.exists():
            try:
                ...
                with self._lock:
                    self.parameter_history = deque(history, maxlen=self.config.max_checkpoints)
                
                logger.info(f"Loaded {len(self.parameter_history)} checkpoints from history")
            except Exception as e:
