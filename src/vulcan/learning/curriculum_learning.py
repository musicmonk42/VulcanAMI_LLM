...
class CompositeDifficultyEstimator(DifficultyEstimator):
    ...
    def __init__(self, estimators: List[DifficultyEstimator], weights: Optional[List[float]] = None):
...
class LearnedDifficultyEstimator(DifficultyEstimator):
    ...
    def update(self, task: Any, performance: float):
        ...
        if len(self.performance_history) > 100 and len(self.performance_history) % 50 == 0:
            self._train_model()
...
class CurriculumLearner:
    ...
    def generate_curriculum(self, all_tasks: List[Any], 
                          auto_cluster: bool = True) -> List[List[Any]]:
        ...
        with self._lock:
            ...
            if self.pacing_strategy == PacingStrategy.EXPONENTIAL:
                ...
                # Linear distribution
                boundaries = [(i/n_stages, (i+1)/n_stages) for i in range(n_stages)]
            ...
            for i, (start_pct, end_pct) in enumerate(boundaries):
                ...
                stage_tasks = [task for task, _, _ in task_difficulties[start_idx:end_idx]]
                ...
                if stage_tasks:
                    ...
                    logger.info(f"Stage {i}: {len(stage_tasks)} tasks, "
    ...
    def _estimate_multi_objective_difficulty(self, task: Any) -> Dict[str, float]:
        ...
        # Example objectives
        objectives['complexity'] = task.get('complexity', 0.5) if isinstance(task, dict) else 0.5
        objectives['noise'] = task.get('noise_level', 0.0) if isinstance(task, dict) else 0.0
    ...
    def _combine_objectives(self, objectives: Dict[str, float]) -> float:
        ...
        combined = sum(objectives.get(k, 0) * self.objective_weights.get(k, 0)
                      for k in objectives)
    ...
    def _cluster_tasks(self, task_difficulties: List[Tuple[Any, float, Dict]]):
        ...
        if n_clusters < 2:
            logger.warning("Insufficient tasks for clustering (need at least 20 for 2 clusters)")
            return
        ...
        try:
            ...
            logger.info(f"Clustered {len(task_difficulties)} tasks into {n_clusters} clusters")
    ...
    def get_next_batch(self, performance: Optional[float] = None,
                ...
                if isinstance(self.difficulty_estimator, LearnedDifficultyEstimator):
                    if self.current_stage < len(self.curriculum_stages):
                        ...
                        if current_tasks and self.samples_in_stage > 0:
                            ...
                            sample_task = current_tasks[min(self.samples_in_stage-1, len(current_tasks)-1)]
    ...
    def _should_advance_stage(self, performance: Optional[float]) -> bool:
        ...
        if self.pacing_strategy == PacingStrategy.FIXED:
            ...
            if len(self.stage_performance) >= 5:  # Reduced from 10 for faster advancement
            ...
            if len(self.stage_performance) >= 10:  # Reduced from 20
                ...
                if avg_recent > 0.85:  # High performance -> advance
                    return True
                elif improvement < 0.02 and avg_recent > 0.65:  # Plateau with ok performance
                    return True
                elif self.samples_in_stage > self.min_samples_per_stage * 2:  # Too long in stage
            ...
            target_samples = self.min_samples_per_stage * (1.5 ** stage_num)  # Reduced from 2**
            ...
            if len(self.stage_performance) >= 5:  # Reduced from 10
                ...
                return performance_std < 0.15 and avg_performance > 0.75  # More permissive
    ...
    def _complete_current_stage(self):
        ...
        self.stage_history.append({
            ...
            'avg_performance': np.mean(self.stage_performance) if self.stage_performance else 0,
            'std_performance': np.std(self.stage_performance) if self.stage_performance else 0,
    ...
    def _select_tasks(self, batch_size: int) -> List[Any]:
        ...
        if self.current_stage < len(self.curriculum_stages):
            ...
            if self.difficulty_adjustments:
                ...
                if last_adjustment['adjustment'] == 'harder' and self.current_stage < len(self.curriculum_stages) - 1:
                    # Mix in harder tasks
                    ...
                    tasks.extend(self._sample_tasks(current_tasks, batch_size - harder_size))
                    
                elif last_adjustment['adjustment'] == 'easier' and self.current_stage > 0:
                    # Mix in easier tasks
                    ...
                    tasks.extend(self._sample_tasks(current_tasks, batch_size - easier_size))
                else:
                ...
                if self.current_stage > 0 and np.random.random() < 0.2:
                    ...
                    tasks.extend(self._sample_tasks(current_tasks, batch_size - review_size))
                else:
                    tasks = self._sample_tasks(current_tasks, batch_size)
        else:
    ...
    def get_curriculum_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            stats = {
                ...
                'avg_stage_performance': np.mean(self.stage_performance) if self.stage_performance else 0,
            ...
            for stage_id, info in self.stage_info.items():
                stage_stats[stage_id] = {
                    ...
                    'avg_performance': np.mean(info.performance_history) if info.performance_history else 0
                }
    ...
    def analyze_curriculum_effectiveness(self) -> Dict[str, Any]:
        ...
        analysis = {
            'total_time': sum(self.metrics.time_per_stage) if self.metrics.time_per_stage else 0,
            'avg_time_per_stage': np.mean(self.metrics.time_per_stage) if self.metrics.time_per_stage else 0,
