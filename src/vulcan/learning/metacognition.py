...
class MetaCognitiveMonitor:
    ...
    def __init__(self, model_ref: Optional[nn.Module] = None, 
        ...
        with self._lock:
            ...
            if 'predicted_confidence' in metrics and 'actual_performance' in metrics:
                self.confidence_history.append({
                    ...
                    'calibration_error': abs(metrics['predicted_confidence'] - metrics['actual_performance'])
                })
                
                # Update confidence calibration factor
                if len(self.confidence_history) > 10:
                    recent_errors = [h['calibration_error'] for h in list(self.confidence_history)[-10:]]
                    avg_error = np.mean(recent_errors)
                    
                    if avg_error > 0.2:
                        self.self_model['confidence_calibration'] *= 0.9
                    elif avg_error < 0.1:
                        self.self_model['confidence_calibration'] = min(1.5, self.self_model['confidence_calibration'] * 1.1)
            ...
            if 'cause' in metrics and 'effect' in metrics:
                self._update_causal_graph(metrics['cause'], metrics['effect'], 
                                         metrics.get('correlation', 0.5))
    
    def introspect_reasoning(self, reasoning_trace: List[Union[Dict, ReasoningStep]]) -> Dict[str, Any]:
        ...
        weights = {'consistency': 0.25, 'completeness': 0.2, 'efficiency': 0.2, 
                  'confidence_calibration': 0.15, 'logical_coherence': 0.2}
        ...
        suggestions = self._generate_reasoning_improvements(quality_metrics, trace_steps)
    ...
    def _check_consistency(self, trace: List[ReasoningStep]) -> float:
        ...
        for i in range(1, len(trace)):
            ...
            if prev_phase == ReasoningPhase.EXECUTION and curr_phase == ReasoningPhase.PERCEPTION:
                consistency_score *= 0.9
            elif prev_phase == ReasoningPhase.LEARNING and curr_phase == ReasoningPhase.PERCEPTION:
    ...
    def _check_completeness(self, trace: List[ReasoningStep]) -> float:
        ...
        expected_phases = {ReasoningPhase.PERCEPTION, ReasoningPhase.PLANNING, 
                          ReasoningPhase.EXECUTION, ReasoningPhase.LEARNING}
    ...
    def _check_logical_coherence(self, trace: List[ReasoningStep]) -> float:
        ...
        for i in range(1, len(trace)):
            ...
            if curr.phase == ReasoningPhase.EXECUTION and prev.phase == ReasoningPhase.PLANNING:
                coherence_score *= 1.1
            
            # Check if learning follows execution
            if curr.phase == ReasoningPhase.LEARNING and prev.phase == ReasoningPhase.EXECUTION:
                coherence_score *= 1.05
        
        return min(1.0, coherence_score)
    
    def _generate_reasoning_improvements(self, metrics: Dict[str, float], 
                                        trace: List[ReasoningStep]) -> List[str]:
        """Generate specific suggestions for reasoning improvement"""
        suggestions = []
        
        if metrics['consistency'] < 0.5:
            suggestions.append("Maintain more consistent confidence levels across reasoning steps")
            suggestions.append("Ensure logical phase transitions in reasoning")
        
        if metrics['completeness'] < 0.5:
            missing_phases = {ReasoningPhase.PERCEPTION, ReasoningPhase.PLANNING, 
                            ReasoningPhase.EXECUTION, ReasoningPhase.LEARNING}
            actual_phases = {step.phase for step in trace}
    ...
    def analyze_learning_efficiency(self) -> Dict[str, Any]:
        ...
        with self._lock:
            if len(self.learning_history) < 10:
                return {'status': 'insufficient_data', 'samples': len(self.learning_history)}
        ...
        for rec in analysis['recommendations']:
            if rec['auto_fix'] and rec['issue'] in self.improvement_strategies:
                ...
                audit_record = {
                    ...
                    'analysis': {k: v for k, v in analysis.items() if k != 'recommendations'},
                    'before_metrics': {
                ...
                logger.info(f"Applied improvement for {rec['issue']}: {audit_record['result']}")
    ...
    def _strategy_reduce_learning_rate(self) -> str:
        ...
        try:
            if not hasattr(self.optimizer_ref, 'param_groups') or not self.optimizer_ref.param_groups:
    ...
    def _strategy_increase_regularization(self) -> str:
        ...
        try:
            ...
            if self.optimizer_ref and hasattr(self.optimizer_ref, 'param_groups') and self.optimizer_ref.param_groups:
    ...
    def _strategy_increase_replay(self) -> str:
        ...
        return "Increased replay frequency recommendation (requires replay buffer access)"
    ...
    def _strategy_boost_confidence(self) -> str:
        ...
        try:
            with self._lock:
                old_calibration = self.self_model['confidence_calibration']
                self.self_model['confidence_calibration'] = min(1.5, self.self_model['confidence_calibration'] * 1.2)
                new_calibration = self.self_model['confidence_calibration']
...
class ConfidenceEstimator:
    ...
    def estimate_confidence(self, logits: torch.Tensor, 
                           method: str = 'softmax') -> float:
        ...
        try:
            ...
            if method == 'softmax':
                ...
                if len(sorted_logits) > 1:
                    ...
                    confidence = torch.sigmoid(torch.tensor(margin, device=self.device)).item()
                else:
                    confidence = 0.5
            else:
                confidence = 0.5
...
class CompositionalUnderstanding:
    ...
    def __init__(self, embedding_dim: int = EMBEDDING_DIM):
        ...
        self.composition_optimizer = optim.Adam(self.composition_net.parameters(), lr=0.001)
        self.decomposition_optimizer = optim.Adam(self.decomposition_net.parameters(), lr=0.001)
    ...
    def train_composition(self, concepts: List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]],
        ...
        for epoch in range(epochs):
            ...
            self.training_history.append({
                'epoch': epoch,
                'composition_loss': epoch_loss
            })
        
        return {'avg_loss': total_loss / epochs}
    
    def train_decomposition(self, concepts: List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]],
                          epochs: int = 10) -> Dict[str, float]:
        ...
        for epoch in range(epochs):
            ...
            self.training_history.append({
                'epoch': epoch,
                'decomposition_loss': epoch_loss
            })
        
        return {'avg_loss': total_loss / epochs}
    
    def compose_concepts_neural(self, concept1: torch.Tensor, 
                               concept2: torch.Tensor) -> torch.Tensor:
    ...
    def decompose_concept_neural(self, concept: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def learn_composition(self, concept: torch.Tensor, 
                         components: List[torch.Tensor]) -> Dict[str, Any]:
    ...
    def hierarchical_composition(self, primitives: List[torch.Tensor],
                                composition_tree: Dict) -> torch.Tensor:
        ...
        if composition_tree['op'] == 'primitive':
            ...
            return primitives[idx] if idx < len(primitives) else torch.zeros(self.embedding_dim, device=self.device)
    ...
    def analyze_compositionality(self, concept: torch.Tensor,
                                known_primitives: List[torch.Tensor]) -> Dict[str, Any]:
        ...
        for i, p1 in enumerate(known_primitives):
            for j, p2 in enumerate(known_primitives):
                if i != j:
                    ...
                    if error < best_error:
                        best_error = error
                        best_composition = {'primitives': [i, j], 'error': error, 'type': 'pairwise'}
            ...
            for i in range(len(known_primitives)):
                for j in range(i+1, len(known_primitives)):
                    for k in range(j+1, len(known_primitives)):
                        comp_ij = self.compose_concepts_neural(known_primitives[i], known_primitives[j])
                        composed = self.compose_concepts_neural(comp_ij, known_primitives[k])
                        error = torch.mean((composed - concept) ** 2).item()
                        
                        if error < best_error:
                            best_error = error
                            best_composition = {'primitives': [i, j, k], 'error': error, 'type': 'triple'}
        ...
        return {
            ...
            'compositionality_score': 1.0 / (1.0 + best_error) if best_composition else 0.0,
            'num_compositions_tried': len(compositions_tried),
            'is_primitive': best_error > 0.5
        }
    
    def discover_primitives(self, concepts: List[torch.Tensor], 
                          max_primitives: int = 10) -> List[torch.Tensor]:
        """FIXED: Discover primitive concepts with device management"""
        try:
            from sklearn.decomposition import PCA
            
            # Convert to numpy for analysis
            concept_matrix = torch.stack([c.to(self.device) for c in concepts]).detach().cpu().numpy()
            ...
            for component in pca.components_:
                primitives.append(torch.tensor(component, dtype=torch.float32, device=self.device))
