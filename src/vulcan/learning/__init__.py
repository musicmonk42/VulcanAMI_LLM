...
from .learning_types import (
    LearningConfig, TaskInfo, FeedbackData, LearningMode, LearningTrajectory
)
from .continual_learning import EnhancedContinualLearner, ContinualLearner
from .curriculum_learning import (
    CurriculumLearner, PacingStrategy, LearnedDifficultyEstimator
)
from .meta_learning import TaskDetector, MetaLearner, MetaLearningAlgorithm
from .metacognition import MetaCognitiveMonitor, CompositionalUnderstanding, ConfidenceEstimator
...
class UnifiedLearningSystem:
    ...
    def __init__(self, 
        ...
        logger.info(f"UnifiedLearningSystem initialized with {self._count_components()} active components")
    ...
    def _connect_metacognition(self):
        ...
        if 'general' in self.continual_learner.optimizers:
            ...
            logger.warning("Could not connect metacognitive monitor - no general optimizer found")
        ...
        class IntegratedDifficultyEstimator(LearnedDifficultyEstimator):
            ...
            def update(self, task: Any, performance: float):
                ...
                if isinstance(task, dict):
                    task_id = self.continual_learner.task_detector.detect_task(task)
                    if task_id in self.continual_learner.task_info:
                        self.continual_learner.task_info[task_id].difficulty = 1.0 - performance
    ...
    def process_experience(self, experience: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hasattr(self, 'curriculum_active') and self.curriculum_active and self.curriculum_learner:
            ...
            if curriculum_batch:
                ...
                experience['curriculum_batch'] = curriculum_batch[0] if curriculum_batch else None
            ...
            if embedding is not None:
                ...
                if embedding is not None:
                    ...
                    try:
                        ...
                        experience['world_model_value'] = self.world_model.predict_value(embedding).item()
                    except Exception as e:
        ...
        if hasattr(self.continual_learner, 'meta_cognitive'):
            ...
            if self.continual_learner.consolidation_counter % 50 == 0:
                efficiency_analysis = self.continual_learner.meta_cognitive.analyze_learning_efficiency()
    ...
    def plan_with_world_model(self, 
                              ...
                              horizon: int = 5) -> Tuple[torch.Tensor, Dict]:
    ...
    def train_world_model(self, num_steps: int = 100):
        ...
        for step in range(num_steps):
            ...
            indices = np.random.choice(len(self.world_model.transition_buffer), batch_size)
    ...
    def get_unified_stats(self) -> Dict[str, Any]:
        ...
        if hasattr(self.continual_learner, 'meta_learner'):
            stats['meta_learning'] = self.continual_learner.meta_learner.get_statistics()
        
        # Metacognition stats
        if hasattr(self.continual_learner, 'meta_cognitive'):
            stats['metacognition'] = {
                'self_model': self.continual_learner.meta_cognitive.self_model,
                'improvements_applied': len(self.continual_learner.meta_cognitive.applied_improvements),
                'improvement_audit': self.continual_learner.meta_cognitive.get_improvement_history()[-10:]
            }
        
        # RLHF stats
        if hasattr(self.continual_learner, 'rlhf_manager') and self.continual_learner.rlhf_manager:
    ...
    def _compute_integration_metrics(self) -> Dict[str, Any]:
        ...
        if getattr(self, 'curriculum_active', False) and self.curriculum_learner and self.continual_learner.task_info:
            curr_progress = self.curriculum_learner.current_stage / max(1, len(self.current_curriculum))
            avg_task_perf = np.mean([
                info.performance for info in self.continual_learner.task_info.values()
            ])
            metrics['curriculum_continual_alignment'] = 1.0 - abs(curr_progress - avg_task_perf)
    ...
    def shutdown(self, timeout: float = 15.0):
        ...
        if self.world_model and hasattr(self.world_model, 'param_history'):
            components.append(('world_model.param_history', self.world_model.param_history))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(components)) as executor:
            ...
            for name, component in components:
                if component and hasattr(component, 'shutdown'):
                    future = executor.submit(self._safe_component_shutdown, component, name)
                    futures[name] = future
            ...
            for name, future in futures.items():
                if future in not_done:
                    logger.error(f"Component '{name}' shutdown timed out after {timeout}s")
                    future.cancel()
                elif future.done():
                    try:
                        ...
                        if success:
                            ...
                            logger.warning(f"⚠ Component '{name}' shutdown completed with warnings")
                    except Exception as e:
