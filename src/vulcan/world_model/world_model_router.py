...
class UpdateDependencyGraph:
    ...
    def get_execution_order(self, required_updates: Set[UpdateType]) -> List[List[UpdateType]]:
...
class PatternLearner:
    ...
    def record_pattern(self, signature: ObservationSignature, 
    ...
    def _get_pattern_key(self, signature: ObservationSignature, 
                         updates: List[UpdateType]) -> str:
    ...
    def _rule_matches(self, rule: Dict[str, Any], signature: ObservationSignature) -> bool:
...
class CostModel:
    ...
    def estimate_cost(self, update_type: UpdateType, 
                     observation_size: int) -> float:
...
class WorldModelRouter:
    ...
    def __init__(self, world_model, config: Optional[Dict[str, Any]] = None, 
        ...
        if SAFETY_VALIDATOR_AVAILABLE:
            ...
            elif hasattr(world_model, 'safety_validator') and world_model.safety_validator:
            ...
            logger.warning("WorldModelRouter: Safety validator not available - operating without safety checks")
        
        logger.info("WorldModelRouter initialized with %d strategies", len(self.strategies))
        if self.self_improvement_drive and getattr(self.self_improvement_drive, '_csiu_enabled', False):
    ...
    def _csiu_predict_user_fit(self, plan: UpdatePlan) -> float:
        ...
        """
        if not self.self_improvement_drive or not getattr(self.self_improvement_drive, '_csiu_enabled', False):
            return 0.5
    ...
    def route(self, observation: Any, constraints: Optional[Dict[str, Any]] = None) -> UpdatePlan:
        """
        ...
        with self.lock:
            ...
            if self.self_improvement_drive and getattr(self.self_improvement_drive, '_csiu_enabled', False):
                try:
                    if hasattr(self.self_improvement_drive, '_collect_telemetry_snapshot'):
                        csiu_before = self.self_improvement_drive._collect_telemetry_snapshot()
                except Exception as e:
                    logger.debug(f"Failed to capture CSIU telemetry before routing: {e}")
            
            # SAFETY: Validate observation before routing
            if self.safety_validator:
                try:
                    if hasattr(self.safety_validator, 'analyze_observation_safety'):
                        obs_safety = self.safety_validator.analyze_observation_safety(observation)
                        if not obs_safety.get('safe', True):
                            logger.warning("Unsafe observation blocked from routing: %s", obs_safety.get('reason', 'unknown'))
            ...
            if self.safety_validator:
                ...
                if not sig_safety['safe']:
                    logger.warning("Unsafe signature detected: %s", sig_safety['reason'])
            ...
            proposal = {
                ...
                "action": "plan_updates",
                "details": {"constraints": constraints, "initial_updates": [u.value for u in required_updates]},
            }
            vr = None
                ...
                try:
                    ...
                    if not vr.valid and self.meta_soften_only:
                        ...
                        if UpdateType.INTERVENTION in required_updates:
                            ...
                            logger.debug("Meta-reasoning softening: Removed INTERVENTION update.")
                            ...
                            if UpdateType.CAUSAL in required_updates:
                                causal_deps = self.dependency_graph.get_dependencies(UpdateType.CAUSAL)
                                if not causal_deps.issubset(required_updates):
                                    required_updates.remove(UpdateType.CAUSAL)
                                    logger.debug("Meta-reasoning softening: Removed CAUSAL update due to missing dependency.")
            ...
            if self.safety_validator:
                ...
                if not plan_safety['safe']:
                    logger.warning("Unsafe execution plan detected: %s", plan_safety['reason'])
            ...
            if csiu_before is not None:
                try:
                    csiu_after = None
                    if hasattr(self.self_improvement_drive, '_collect_telemetry_snapshot'):
                        csiu_after = self.self_improvement_drive._collect_telemetry_snapshot()
                    ...
                    if csiu_after:
                        routing_record = {
                            ...
                            'signature': signature,
                            'updates_planned': len(plan.immediate) + sum(len(g) for g in plan.parallel_groups),
    ...
    def execute(self, plan: UpdatePlan) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.self_improvement_drive and getattr(self.self_improvement_drive, '_csiu_enabled', False):
                try:
                    if hasattr(self.self_improvement_drive, '_collect_telemetry_snapshot'):
                        csiu_exec_before = self.self_improvement_drive._collect_telemetry_snapshot()
                except Exception as e:
                    logger.debug(f"Failed to capture CSIU telemetry before execution: {e}")
                ...
                try:
                    ...
                    if self.safety_validator:
                        ...
                        if not update_safety['safe']:
                            logger.warning("Unsafe update %s blocked: %s", update_type, update_safety['reason'])
                    ...
                    if self.safety_validator:
                        result_safety = self._validate_result_safety(update_type, result)
                        if not result_safety['safe']:
                            logger.warning("Unsafe result from %s: %s", update_type, result_safety['reason'])
                            self.safety_corrections[f'result_{update_type.value}'] += 1
                            result = self._apply_result_corrections(result, result_safety)
            ...
            success = all('error' not in r and r.get('status') != 'error' for r in results.values() if isinstance(r, dict))
            ...
            if csiu_exec_before is not None:
                try:
                    if hasattr(self.self_improvement_drive, '_collect_telemetry_snapshot'):
                        csiu_exec_after = self.self_improvement_drive._collect_telemetry_snapshot()
                    ...
                    logger.debug(f"Failed to record CSIU telemetry after execution: {e}")
            ...
            if self.validation_tracker:
                try:
                    self.validation_tracker.record_validation(
                        proposal=proposal,
                        validation_result=vr if vr is not None else {"proposal_id": proposal["id"], "valid": True, "confidence": 0.0, "reasoning": "No pre-validation run."},
    ...
    def _extract_signature(self, observation: Any) -> ObservationSignature:
        ...
        has_intervention = bool(
            (hasattr(observation, 'intervention_data') and observation.intervention_data) or
            (hasattr(observation, 'is_intervention') and observation.is_intervention) or
            (hasattr(observation, 'metadata') and observation.metadata and observation.metadata.get('is_intervention', False))
        )
        
        has_temporal = (
            hasattr(observation, 'timestamp') and 
            hasattr(observation, 'sequence_id')
        )
        ...
        # Time delta
        time_delta = None
        if hasattr(observation, 'timestamp') and hasattr(self.world_model, 'last_observation_time'):
            last_time = self.world_model.last_observation_time
            if last_time:
                time_delta = observation.timestamp - last_time
        
        # Confidence
        confidence = observation.confidence if hasattr(observation, 'confidence') else 1.0
        
        # Domain
        domain = observation.domain if hasattr(observation, 'domain') else None
        
        # Create pattern hash
        pattern_str = f"{has_intervention}_{has_temporal}_{has_multi_variable}_{variable_count}"
    ...
    def _determine_required_updates(self, signature: ObservationSignature) -> Set[UpdateType]:
    ...
    def _apply_constraints(self, updates: Set[UpdateType], 
                          constraints: Dict[str, Any],
                          signature: ObservationSignature) -> Set[UpdateType]:
        """Apply constraints to filter updates with CSIU tie-breaking"""
        
        time_budget = constraints.get('time_budget_ms', self.time_budget_ms)
        priority_threshold = constraints.get('priority_threshold', UpdatePriority.NORMAL)
        
        # Estimate total cost
        observation_size = signature.variable_count * 100  # Rough estimate
        total_cost = sum(
            self.cost_model.estimate_cost(u, observation_size) 
            for u in updates
        )
        ...
        if (len(scored_updates) >= 2 and 
            self.self_improvement_drive and 
            getattr(self.self_improvement_drive, '_csiu_enabled', False)):
            ...
            while i < len(scored_updates) - 1:
                ...
                if abs(score_a - score_b) < epsilon:
                    ...
                    plan_a = UpdatePlan(
                        ...
                        deferred=[],
                        parallel_groups=[],
                        estimated_time_ms=self.cost_model.estimate_cost(update_a, observation_size),
                    ...
                    plan_b = UpdatePlan(
                        ...
                        deferred=[],
                        parallel_groups=[],
                        estimated_time_ms=self.cost_model.estimate_cost(update_b, observation_size),
                    ...
                    if adjusted_score_b > adjusted_score_a:
                        scored_updates[i], scored_updates[i + 1] = scored_updates[i + 1], scored_updates[i]
                        logger.debug(f"CSIU tie-break: {update_b.value} preferred over {update_a.value}")
    ...
    def _create_execution_plan(self, updates: Set[UpdateType], 
                              signature: ObservationSignature) -> UpdatePlan:
        ...
        for group in execution_groups:
            ...
            if len(group) > 1 and self.dependency_graph.can_parallelize(group):
                ...
                for update in group:
                    ...
                    if strategy.can_defer and strategy.priority.value > UpdatePriority.HIGH.value:
                        deferred.append(update)
                    else:
                        immediate.append(update)
                        total_estimated_time += self.cost_model.estimate_cost(update, observation_size)
        
        # Create reasoning explanation
        reasoning = self._generate_reasoning(immediate, deferred, parallel_groups, signature)
    ...
    def _execute_intervention_update(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'observation' in params:
                if 'intervention_data' in params and self.current_observation.intervention_data:
                     return method_to_call(intervention_data=self.current_observation.intervention_data, observation=self.current_observation)
                return method_to_call(observation=self.current_observation)
                 ...
                 if self.current_observation.intervention_data:
                    return method_to_call(intervention_data=self.current_observation.intervention_data)
                 else:
                    return method_to_call(intervention_data=self.current_observation)
            elif len(params) == 2:
                # Assume (intervention_data, observation)
                try:
                    return method_to_call(self.current_observation.intervention_data, self.current_observation)
                except Exception:
    ...
    def _execute_causal_update(self) -> Dict[str, Any]:
        ...
        if hasattr(self.world_model, 'causal_graph'):
            ...
            return {'status': 'causal_graph_updated'} # Assuming intervention handled it
    ...
    def _execute_confidence_update(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'observation' in params and 'prediction' in params:
                ...
                return tracker.update(observation=self.current_observation, prediction=None)
            elif 'observation' in params:
                # Only observation
                return tracker.update(observation=self.current_observation)
            elif len(params) == 2:
    ...
    def _execute_distribution_update(self) -> Dict[str, Any]:
        ...
        if not hasattr(self.world_model, 'distribution_monitor'):
            ...
            if hasattr(self.world_model, 'correlation_tracker') and hasattr(self.world_model.correlation_tracker, 'update_distributions'):
    ...
    def _execute_structural_update(self) -> Dict[str, Any]:
        ...
        if not hasattr(self.world_model, 'structural_analyzer'):
            ...
            if hasattr(self.world_model, 'causal_graph') and hasattr(self.world_model.causal_graph, 'analyze_structure'):
    ...
    def _detect_anomaly(self, observation: Any) -> bool:
        ...
        if hasattr(observation, 'variables') and hasattr(self.world_model, 'correlation_tracker'):
            try:
                for var, value in observation.variables.items():
                    if isinstance(value, (int, float)):
                        if hasattr(self.world_model.correlation_tracker, 'get_baseline'):
                            baseline = self.world_model.correlation_tracker.get_baseline(var)
                            if baseline is not None and hasattr(self.world_model.correlation_tracker, 'get_noise_level'):
                                noise_level = self.world_model.correlation_tracker.get_noise_level(var)
                                if noise_level > 0 and abs(value - baseline) > 3 * noise_level:
                                    return True
            except Exception as e:
        ...
        if hasattr(observation, 'variables') and hasattr(self.world_model, 'correlation_tracker'):
            try:
                ...
                if hasattr(self.world_model.correlation_tracker, 'correlation_matrix'):
                    known_vars = set(getattr(self.world_model.correlation_tracker.correlation_matrix, 'variables', []))
    ...
    def _generate_reasoning(self, immediate: List[UpdateType], 
                          deferred: List[UpdateType],
                          parallel: List[List[UpdateType]],
                          signature: ObservationSignature) -> str:
        ...
        if parallel:
            reasons.append(f"Parallel updates possible: {[[u.value for u in g] for g in parallel]}")
    ...
    def _validate_signature_safety(self, signature: ObservationSignature) -> Dict[str, Any]:
    ...
    def _apply_signature_corrections(self, signature: ObservationSignature, 
                                    safety_result: Dict[str, Any]) -> ObservationSignature:
        """Apply safety corrections to signature"""
        # Clamp confidence
    ...
    def _validate_plan_safety(self, plan: UpdatePlan, observation: Any) -> Dict[str, Any]:
    ...
    def _apply_plan_corrections(self, plan: UpdatePlan, 
                               safety_result: Dict[str, Any]) -> UpdatePlan:
        ...
        if total_updates > 50:
            ...
            plan.immediate = [u for u in plan.immediate if self.strategies[u].priority == UpdatePriority.CRITICAL]
    ...
    def _validate_update_safety(self, update_type: UpdateType, plan: UpdatePlan) -> Dict[str, Any]:
    ...
    def _validate_result_safety(self, update_type: UpdateType, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if exec_time > 10000:  # More than 10 seconds
            violations.append(f"Update {update_type.value} took excessive time: {exec_time}ms")
    ...
    def _apply_result_corrections(self, result: Dict[str, Any], 
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.self_improvement_drive and getattr(self.self_improvement_drive, '_csiu_enabled', False):
            metrics['csiu_tracking'] = {
                ...
                'execution_records': len([r for r in self.execution_history if 'csiu_before' in r]),
                'outcome_records': len([r for r in self.csiu_routing_history if 'success' in r]),
                'note': 'Internal tracking only, not for UX'
            }
    ...
    def save_state(self, path: str):
        ...
        state = {
            ...
            'cost_history': {k.value: list(v) for k, v in self.cost_model.cost_history.items()},
