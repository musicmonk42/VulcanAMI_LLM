...
"""
    ...
    def introspect(self, query: str, aspect: str = "general") -> Dict[str, Any]:
        ...
        # ========================================
        # These handle specific types of self-referential queries that were
        # previously returning generic "I am VULCAN" responses.
        
        # Handler for "what problems are you not suited for" / "classes of problems"
        unsuited_patterns = [
        ...
        if any(phrase in query_lower for phrase in unsuited_patterns):
            return {
                "confidence": 0.85,
                "response": self._explain_unsuited_problem_classes(query),
                "aspect": "limitations_specific",
                "reasoning": "Question about specific problem classes VULCAN is not suited for"
            }
        
        # Handler for "can modules disagree" / "module conflict" questions
        ...
        if any(phrase in query_lower for phrase in module_conflict_patterns):
            return {
                "confidence": 0.90,
                "response": self._explain_module_conflict_resolution(query),
                "aspect": "module_conflict",
                "reasoning": "Question about how VULCAN handles disagreement between reasoning modules"
            }
        
        # Handler for "weakest causal link" / "uncertainty in reasoning" questions  
        weakness_patterns = [
        ...
        if any(phrase in query_lower for phrase in weakness_patterns) and causal_context:
            return {
                "confidence": 0.85,
                "response": self._analyze_reasoning_weakness(query),
        ...
        # ========================================
        # These are NOT ethical queries - they're questions about VULCAN's own reasoning
        ...
        if any(phrase in query_lower for phrase in meta_reasoning_patterns):
            return {
                "confidence": 0.85,
                "response": self._analyze_own_reasoning_steps(query),
        ...
        # 
        # PROBLEM FIXED:
        # Query: "demonstrate how you use counterfactual reasoning"
        # Old behavior: Generic introspection text about capabilities
        # New behavior: Actually run CounterfactualObjectiveReasoner with example
        
        if question_type == "DEMONSTRATION":
            return self._handle_demonstration_query(query)
    ...
    def _handle_demonstration_query(self, query: str) -> Dict[str, Any]:
        ...
        - World model answered correctly (0.90 confidence)
        - But cache returned wrong result (MATHEMATICAL, 0.10 confidence)
        - User got LLM hallucination instead of actual demonstration
        ...
        if reasoning_to_demonstrate == "counterfactual":
            return self._demonstrate_counterfactual_reasoning()
        elif reasoning_to_demonstrate == "causal":
            return self._demonstrate_causal_reasoning()
    ...
    def _demonstrate_counterfactual_reasoning(self) -> Dict[str, Any]:
        ...
        if hasattr(self, 'counterfactual_reasoner') and self.counterfactual_reasoner is not None:
            try:
                ...
                outcome_speed = self.counterfactual_reasoner.predict_under_objective(
                    alternative_objective="speed",
                    context=context
                )
                
                # Run counterfactual analysis for "throughput" objective
                outcome_throughput = self.counterfactual_reasoner.predict_under_objective(
...
{self._format_counterfactual_outcome(outcome_speed, "speed")}

## Counterfactual 2: What if we optimized for THROUGHPUT instead?

{self._format_counterfactual_outcome(outcome_throughput, "throughput")}
...
"""
    ...
    # =========================================================================
    # BUG #14 FIX: Specific introspection response methods
    # =========================================================================
    # These methods provide specific, substantive answers to self-referential
    # queries that were previously returning generic "I am VULCAN" responses.
...
"""
    ...
    def _classify_introspection_type(self, query: str) -> str:
        ...
        # Examples:
        # - "demonstrate how you use counterfactual reasoning"
        # - "show me an example of causal reasoning"
        # - "give me a demo of your probabilistic reasoning"
