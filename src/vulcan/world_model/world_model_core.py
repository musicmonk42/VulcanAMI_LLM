...
import numpy as np
import json
from pathlib import Path as FilePath  # <-- FIX: Use alias 'FilePath' to avoid name conflict
import threading
import sys
import os
...
def _lazy_import_safety_validator():
    ...
    if EnhancedSafetyValidator is None:
        try:
            ...
            logger.critical("safety_validator not available: %s - System running without safety constraints!", e)
            return False
    return True
...
def _lazy_import_correlation_tracker():
    ...
    if CorrelationTracker is None:
        try:
            ...
            logger.critical("correlation_tracker module not available: %s - Cannot track correlations!", e)
            return False
    return True
...
def _lazy_import_causal_graph():
    ...
    if CausalDAG is None:
        try:
            ...
            logger.critical("causal_graph module not available: %s - Cannot build causal models!", e)
            return False
    return True
...
def _lazy_import_intervention_manager():
    ...
    if Correlation is None:
        try:
            from .intervention_manager import Correlation, InterventionPrioritizer, InterventionExecutor, InterventionResult, InterventionCandidate
            logger.info("Intervention manager components lazy loaded successfully")
            return True
        except ImportError as e:
            logger.critical("intervention_manager module not available: %s - Cannot run interventions!", e)
            return False
    return True
...
def _lazy_import_prediction_engine():
    ...
    if Path is None:
        try:
            ...
            logger.critical("prediction_engine module not available: %s - Cannot make predictions!", e)
            return False
    return True
...
def _lazy_import_invariant_detector():
    ...
    if InvariantRegistry is None:
        try:
            from .invariant_detector import InvariantRegistry, InvariantDetector, Invariant, InvariantType
...
def _lazy_import_confidence_calibrator():
    ...
    if ConfidenceCalibrator is None:
        try:
            from .confidence_calibrator import ConfidenceCalibrator, ModelConfidenceTracker
...
class ObservationProcessor:
    ...
    def detect_intervention_data(self, observation: Observation) -> Optional[Dict[str, Any]]:
            ...
            if self.safety_validator:
                try:
                    if hasattr(self.safety_validator, 'validate_intervention_data'):
                        validation_result = self.safety_validator.validate_intervention_data(observation.intervention_data)
                        if not validation_result.get('safe', True):
                            logger.warning("Unsafe intervention data detected: %s", validation_result.get('reason', 'unknown'))
                            return None
                except Exception as e:
                    logger.error("Safety validator error in validate_intervention_data: %s", e)
        ...
        if observation.metadata.get('is_intervention', False):
            ...
            if self.safety_validator:
                try:
                    if hasattr(self.safety_validator, 'validate_intervention_data'):
                        validation_result = self.safety_validator.validate_intervention_data(intervention_data)
                        if not validation_result.get('safe', True):
                            logger.warning("Unsafe intervention data in metadata: %s", validation_result.get('reason', 'unknown'))
                            return None
                except Exception as e:
                    logger.error("Safety validator error in validate_intervention_data: %s", e)
    ...
    def extract_temporal_patterns(self, observation: Observation) -> Dict[str, Any]:
        ...
        for var_name in observation.variables.keys():
            history = [obs.variables.get(var_name) for obs in self.observation_history 
        ...
        if self.safety_validator and patterns:
            try:
                if hasattr(self.safety_validator, 'validate_pattern'):
                    pattern_validation = self.safety_validator.validate_pattern(patterns)
                    if not pattern_validation.get('safe', True):
                        logger.warning("Unsafe pattern detected: %s", pattern_validation.get('reason', 'unknown'))
    ...
    def extract_linguistic_data(self, observation: Observation) -> Optional[str]:
        ...
        if 'text' in observation.variables and isinstance(observation.variables['text'], str):
            return observation.variables['text']
        if 'utterance' in observation.variables and isinstance(observation.variables['utterance'], str):
            return observation.variables['utterance']
        if observation.metadata.get('linguistic_input') and isinstance(observation.metadata['linguistic_input'], str):
            return observation.metadata['linguistic_input']
        
        return None

    def validate_observation(self, observation: Observation) -> Tuple[bool, Optional[str]]:
        ...
        if self.safety_validator:
            try:
                if hasattr(self.safety_validator, 'analyze_observation_safety'):
                    safety_result = self.safety_validator.analyze_observation_safety(observation)
                    if not safety_result.get('safe', True):
                        return False, f"Safety validation failed: {safety_result.get('reason', 'unknown')}"
            except Exception as e:
                logger.error("Safety validator error in analyze_observation_safety: %s", e)
    ...
    def _detect_statistical_intervention(self, observation: Observation) -> bool:
        ...
        for var_name, current_value in observation.variables.items():
            if isinstance(current_value, (int, float)):
                history = [obs.variables.get(var_name) for obs in list(self.observation_history)[-20:-1]
                          if var_name in obs.variables and isinstance(obs.variables[var_name], (int, float))]
    ...
    def _is_anomaly(self, value: float, history: List[float], z_threshold: float = 3.0) -> bool:
...
class InterventionManager:
    ...
    def schedule_interventions(self, correlations: List[Any], budget: float) -> List[Any]:
        """Schedule interventions based on correlations and budget"""
        
        if not INTERVENTION_MANAGER_AVAILABLE:
            logger.error("Cannot schedule interventions - InterventionManager not available")
            return []
        ...
        for candidate in candidates:
            ...
            if self.world_model.safety_validator:
                try:
                    if hasattr(self.world_model.safety_validator, 'validate_intervention'):
                        intervention_validation = self.world_model.safety_validator.validate_intervention(
                            candidate.correlation.var_a,
                            candidate.correlation.var_b,
                            candidate.intervention_type if hasattr(candidate, 'intervention_type') else 'simulated',
                            candidate.metadata if hasattr(candidate, 'metadata') else {}
                        )
                        
                        if not intervention_validation.get('safe', True):
                            logger.warning("Unsafe intervention blocked: %s -> %s: %s",
                                         candidate.correlation.var_a,
                                         candidate.correlation.var_b,
                                         intervention_validation.get('reason', 'unknown'))
                            continue
                except Exception as e:
                    logger.error("Safety validator error in validate_intervention: %s", e)
                    # Fail-safe: block on validator error
                    continue
    ...
    def execute_next_intervention(self) -> Optional['InterventionResult']:
        ...
        if not INTERVENTION_MANAGER_AVAILABLE:
            logger.error("Cannot execute intervention - InterventionManager not available")
            return None
        ...
        try:
            ...
            result = self.world_model.intervention_executor.execute_intervention(candidate)
    ...
    def process_intervention_observation(self, 
                                        intervention_data: Dict[str, Any],
                                        observation: Observation):
        """Process observation from an intervention"""
        
        if not CAUSAL_GRAPH_AVAILABLE:
            logger.error("Cannot process intervention observation - CausalDAG not available")
            return
        ...
        for effect_var, effect_value in observation.variables.items():
            ...
            if self._is_significant_effect(cause, effect_var, effect_value, intervention_data):
                # Calculate strength
                ...
                if strength > self.world_model.min_causal_strength:
                    ...
                    if self.world_model.safety_validator:
                        try:
                            if hasattr(self.world_model.safety_validator, 'validate_causal_edge'):
                                ...
                                if not edge_validation.get('safe', True):
                                    logger.warning("Unsafe causal edge blocked: %s -> %s: %s",
                                                 cause, effect_var, edge_validation.get('reason', 'unknown'))
                                    continue
                        except Exception as e:
                            logger.error("Safety validator error in validate_causal_edge: %s", e)
                            continue
        ...
        with self.world_model.lock:
            for cause, effect, strength in causal_edges_added:
                ...
                logger.info("Added causal edge: %s -> %s (strength=%.2f)",
                          cause, effect, strength)
    
    def _process_intervention_result(self, candidate: Any, result: 'InterventionResult'):
        ...
        if result.type == "success" and result.causal_strength:
            if result.causal_strength > self.world_model.min_causal_strength:
                ...
                if self.world_model.safety_validator:
                    try:
                        if hasattr(self.world_model.safety_validator, 'validate_causal_edge'):
                            ...
                            if not edge_validation.get('safe', True):
                                logger.warning("Unsafe causal edge from intervention blocked: %s -> %s: %s",
                                             candidate.correlation.var_a,
                                             candidate.correlation.var_b,
                ...
                logger.info("Added causal edge from intervention: %s -> %s (strength=%.2f)",
                          candidate.correlation.var_a,
                          candidate.correlation.var_b,
                          result.causal_strength)
    ...
    def _is_significant_effect(self, cause: str, effect: str, 
                              effect_value: Any,
                              intervention_data: Dict[str, Any]) -> bool:
        ...
        if isinstance(effect_value, (int, float)) and isinstance(baseline, (int, float)):
    ...
    def _calculate_causal_strength(self, cause: str, effect: str,
                                  intervention_data: Dict[str, Any],
...
class PredictionManager:
    ...
    def predict(self, action: Any, context: ModelContext) -> 'Prediction':
        ...
        if CAUSAL_GRAPH_AVAILABLE:
            paths = self.world_model.causal_graph.find_all_paths(action, context.targets)
        ...
        if self.world_model.safety_validator and paths:
            ...
            for path in paths:
                ...
                if hasattr(path, 'nodes') and (hasattr(path, 'strengths') or hasattr(path, 'get_strengths')):
                    try:
                        ...
                        if hasattr(self.world_model.safety_validator, 'validate_causal_path'):
                            path_validation = self.world_model.safety_validator.validate_causal_path(
                                path.nodes, strengths
                            )
                            if path_validation.get('safe', True):
                                validated_paths.append(path)
                            else:
                                logger.warning("Unsafe causal path blocked: %s", path_validation.get('reason', 'unknown'))
                        else:
                            validated_paths.append(path)
                    except Exception as e:
        ...
        if self.world_model.safety_validator:
            try:
                if hasattr(self.world_model.safety_validator, 'validate_prediction_comprehensive'):
                    pred_validation = self.world_model.safety_validator.validate_prediction_comprehensive(
                        prediction.expected,
                        prediction.lower_bound,
                        prediction.upper_bound,
                        {'domain': context.domain, 'targets': context.targets, 'target_variable': context.targets[0] if context.targets else 'unknown'}
                    )
                    
                    if not pred_validation.get('safe', True):
                        logger.warning("Unsafe prediction detected, applying safety corrections: %s", pred_validation.get('reason', 'unknown'))
                        # Use safe values from validation
                        prediction.expected = pred_validation.get('safe_expected', prediction.expected)
                        prediction.lower_bound = pred_validation.get('safe_lower', prediction.lower_bound)
                        prediction.upper_bound = pred_validation.get('safe_upper', prediction.upper_bound)
                        prediction.confidence *= 0.5  # Reduce confidence for corrected predictions
            except Exception as e:
                logger.error("Safety validator error in validate_prediction_comprehensive: %s", e)
    ...
    def _causal_prediction(self, action: Any, context: ModelContext, paths: List[Any]) -> 'Prediction':
        """
        ...
        if not PREDICTION_ENGINE_AVAILABLE or Path is None:
            logger.error("Cannot make causal prediction - Path or PredictionEngine unavailable")
            # Return a default Prediction
            return Prediction(expected=0.0, lower_bound=0.0, upper_bound=0.0, confidence=0.0, method="unavailable")
        ...
        for cp in paths:  # cp is a CausalPath
            try:
                ...
                logger.warning(f"Failed to convert CausalPath to Path: {e}. Skipping path.")
                continue
        ...
        return self.world_model.ensemble_predictor.predict_with_path_ensemble(
            action,
            context.to_dict() if hasattr(context, 'to_dict') else context, # Pass context
            converted_paths  # <-- Use the new list of converted paths
        )
    
    def _correlation_prediction(self, action: Any, context: ModelContext) -> 'Prediction':
        ...
        for target in context.targets:
            corr_strength = self.world_model.correlation_tracker.get_correlation(action, target)
            if corr_strength:
    ...
    def _apply_dynamics(self, prediction: 'Prediction', context: ModelContext) -> 'Prediction':
        ...
        if context.constraints.get('time_horizon'):
            try:
                ...
                context_dict = context.to_dict() if hasattr(context, 'to_dict') else context.__dict__.copy() # Use __dict__ fallback
                return self.world_model.dynamics.apply(
                    prediction,
                    context_dict,
                    context.constraints['time_horizon']
                )
            except Exception as e:
    ...
    def _check_invariants(self, prediction: 'Prediction') -> 'Prediction':
        ...
        state = {'value': prediction.expected} if isinstance(prediction.expected, (int, float)) else prediction.expected
    ...
    def _calibrate_confidence(self, prediction: 'Prediction', context: ModelContext) -> 'Prediction':
        """Calibrate prediction confidence"""
        
        if not CONFIDENCE_CALIBRATOR_AVAILABLE or self.world_model.confidence_calibrator is None:
            return prediction
...
class ConsistencyValidator:
    ...
    def validate(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'intervention_count': self.world_model.intervention_manager.intervention_count if INTERVENTION_MANAGER_AVAILABLE else 0
        }
    ...
    def _calculate_invariant_violation_rate(self) -> float:
        ...
        if not INVARIANT_DETECTOR_AVAILABLE or self.world_model.invariants is None or not self.world_model.prediction_manager.prediction_history:
            return 0.0
...
class WorldModel:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if missing_critical:
            error_msg = f"Critical components unavailable: {', '.join(missing_critical)}"
        ...
        if CorrelationTracker:
            ...
            self.correlation_tracker = None # This will be caught by the critical check above
        
        if INTERVENTION_MANAGER_AVAILABLE:
            self.intervention_prioritizer = InterventionPrioritizer(safety_config=safety_config)
            ...
            logger.warning("⚠ InterventionManager components unavailable - intervention testing disabled")
        ...
        if INVARIANT_DETECTOR_AVAILABLE:
            ...
            logger.warning("⚠ InvariantDetector unavailable - invariant detection disabled")
            self.invariants = None
            self.invariant_detector = None
        
        if CONFIDENCE_CALIBRATOR_AVAILABLE:
            self.confidence_calibrator = ConfidenceCalibrator(safety_config=safety_config)
            self.confidence_tracker = ModelConfidenceTracker(safety_config=safety_config)
            logger.info("✓ ConfidenceCalibrator components initialized")
        else:
            logger.warning("⚠ ConfidenceCalibrator unavailable - confidence calibration disabled")
        ...
        if META_REASONING_AVAILABLE and config.get('enable_meta_reasoning', True):
            ...
            try:
                ...
                self.value_evolution_tracker = getattr(self, "value_evolution_tracker", None) # As requested, not imported
                ...
                if self.meta_reasoning_enabled:
                    ...
                    logger.warning("⚠ Partial or incomplete meta-reasoning layer. `meta_reasoning_enabled` is False.")

            except Exception as e:
                logger.error("Failed to initialize meta-reasoning: %s", e, exc_info=True) # Added exc_info
        ...
        nested_enable = bool(config.get('world_model', {}).get('enable_self_improvement', False))
        nested_enabled_alt = bool(config.get('world_model', {}).get('self_improvement_enabled', False))
        env_enable = os.getenv('VULCAN_ENABLE_SELF_IMPROVEMENT', '0').lower() in ('1', 'true', 'yes', 'on')

        effective_self_improvement = root_enable or nested_enable or nested_enabled_alt or env_enable
        ...
        logger.info(
            ...
            root_enable, nested_enable, nested_enabled_alt, os.getenv('VULCAN_ENABLE_SELF_IMPROVEMENT', None), effective_self_improvement
        )
        ...
        if META_REASONING_AVAILABLE and SelfImprovementDrive and effective_self_improvement:
            try:
                self.self_improvement_drive = SelfImprovementDrive(
                    ...
                    state_path=config.get('self_improvement_state', "data/agent_state.json"),
                ...
                logger.error("Failed to initialize self-improvement drive: %s", e, exc_info=True)
        ...
        self.components = [
            'causal_graph', 'confidence_calibrator', 'correlation_tracker',
            'dynamics_model', 'intervention_manager', 'prediction_engine',
            'invariant_detector', 'world_model_core', 'world_model_router'
        ]
        ...
        logger.info("Safety: %s | Meta-reasoning: %s | Self-improvement: %s | Bootstrap: %s",
                   self.safety_mode, self.meta_reasoning_enabled, self.self_improvement_enabled, self.bootstrap_mode)
        logger.info("=" * 60)
    ...
    def _verify_component_interfaces(self):
        ...
        checks = [
            (self.correlation_tracker, ['get_baseline', 'get_noise_level', 'get_correlation', 'update', 'get_strong_correlations']),
            (self.causal_graph, ['add_edge', 'find_all_paths', 'has_cycles', 'has_edge']),
        ]
        
        if self.dynamics:
            checks.append((self.dynamics, ['apply', 'update']))
        if self.invariants:
            checks.append((self.invariants, ['check_invariant_violations', 'get_invariant_types']))
        if self.confidence_calibrator:
            checks.append((self.confidence_calibrator, ['calibrate', 'calculate_expected_calibration_error']))
        ...
        for component, methods in checks:
            if component is None: continue # Skip if component failed to load
    ...
    def _verify_safety_validator_interface(self):
        ...
        missing = [m for m in required_methods 
                  if not hasattr(self.safety_validator, m)]
    ...
    def start_autonomous_improvement(self):
        ...
        if not self.self_improvement_enabled:
            logger.warning("Cannot start autonomous improvement - self-improvement drive not initialized")
            return
    ...
    def _autonomous_improvement_loop(self):
        ...
        while self.improvement_running:
            try:
                ...
                logger.error(f"Error in autonomous improvement loop: {e}", exc_info=True)
                time.sleep(check_interval)
    ...
    def _build_improvement_context(self) -> Dict[str, Any]:
        ...
        with self.lock:
            # Check if startup
            is_startup = (time.time() - self.system_state['session_start']) < 300  # 5 min
            
            # Count recent errors
            current_time = time.time()
            window = 3600  # 1 hour
            recent_errors = [
                e for e in self.system_state['errors_in_window']
    ...
    def update_performance_metric(self, metric: str, value: float):
        ...
        with self.lock:
            ...
            if old_value is not None and old_value > 0:
                ...
                self.system_state['performance_metrics'][f'{metric}_degradation_percent'] = degradation
    ...
    def update_from_observation(self, observation: Observation) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            is_valid, error_msg = self.observation_processor.validate_observation(observation)
            if not is_valid:
            ...
            intervention_data = self.observation_processor.detect_intervention_data(observation)
            temporal_patterns = self.observation_processor.extract_temporal_patterns(observation)
            
            # --- START NEW LINGUISTIC PROCESSING ---
            linguistic_data = self.observation_processor.extract_linguistic_data(observation)
            if linguistic_data:
    ...
    def update_from_text(self, text: str, predictions: Dict[str, Any]):
        ...
        if not CAUSAL_GRAPH_AVAILABLE or self.dynamics is None:
            logger.warning("Causal DAG or Dynamics Model unavailable, skipping update from text.")
            return
                ...
                if self.safety_validator:
                    try:
                        if hasattr(self.safety_validator, 'validate_causal_edge'):
                            ...
                            if not edge_validation.get('safe', True):
                                logger.warning("Unsafe causal edge from text blocked: %s -> %s", cause, effect)
                                continue
                    except Exception as e:
                        logger.error("Safety validator error in validate_causal_edge (text): %s", e)
                        continue

                self.causal_graph.add_edge(cause, effect, strength=strength, evidence_type="linguistic")
                logger.info(f"Linguistic update: Added edge {cause} -> {effect} (strength={strength:.2f})")
    ...
    def validate_generation(self, proposed_token: str, context: Dict[str, Any]) -> bool:
        ...
        with self.lock:
            ...
            if self._would_create_contradiction(proposed_token, context):
                logger.warning(f"Generation blocked: Proposed token '{proposed_token}' creates causal contradiction.")
                return False
            
            # MOCK: Check against known safety invariants related to generation
            if self.invariants and self.invariants.check_invariant_violations({'generated_token': proposed_token, **context}):
                 logger.warning(f"Generation blocked: Proposed token '{proposed_token}' violates an invariant.")
                 return False
    ...
    def _extract_causal_relations(self, text: str) -> List[Dict[str, Any]]:
        ...
        if "causes" in text.lower():
            return [
                {'cause': 'text_mention', 'effect': 'hypothesized_effect', 'strength': 0.65},
                {'cause': 'linguistic_topic', 'effect': 'sentiment_variable', 'strength': 0.8}
            ]
        return []
    
    def _would_create_contradiction(self, proposed_token: str, context: Dict[str, Any]) -> bool:
    ...
    def run_intervention_tests(self, budget: float) -> List['InterventionResult']:
        ...
        if not INTERVENTION_MANAGER_AVAILABLE:
            logger.error("Cannot run intervention tests - InterventionManager not available")
            return []
        
        # SAFETY: Check if real interventions are allowed
        if self.safety_mode == 'disabled' and not getattr(self.intervention_executor, 'simulation_mode', True):
            raise RuntimeError(
    ...
    def predict_with_calibrated_uncertainty(self, 
                                           action: Any,
                                           context: ModelContext) -> 'Prediction':
    ...
    def evaluate_agent_proposal(self, proposal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.meta_reasoning_enabled:
            logger.warning("evaluate_agent_proposal called but meta-reasoning is disabled")
            return {
                'status': 'unavailable',
                'valid': True,
        ...
        with self.lock:
            try:
                ...
                return {
                    ...
                    'overall_status': validation.overall_status.value if hasattr(validation.overall_status, 'value') else str(validation.overall_status),
    ...
    def get_objective_state(self) -> Dict[str, Any]:
        ...
        if not self.meta_reasoning_enabled:
            return {
                'enabled': False,
                'reason': 'Meta-reasoning layer not available'
            }
        ...
        with self.lock:
            structure = {
                ...
                'statistics': {
                    ...
                    'strongly_connected_components': len(self.causal_graph.find_strongly_connected_components()),
            ...
            if self.invariants:
                structure['invariants'] = {
                    'count': len(self.invariants.invariants) if hasattr(self.invariants, 'invariants') else 0,
                    'types': self.invariants.get_invariant_types()
                }
            
            # Add confidence information
            if self.confidence_tracker:
                structure['model_confidence'] = self.confidence_tracker.get_model_confidence()
            ...
            if self.safety_validator:
                try:
                    if hasattr(self.safety_validator, 'get_safety_stats'):
                        structure['safety_stats'] = self.safety_validator.get_safety_stats()
                except Exception as e:
            ...
            if self.meta_reasoning_enabled:
                try:
                    structure['meta_reasoning'] = {
                        'enabled': True,
                        'objectives': list(self.motivational_introspection.active_objectives.keys()),
        ...
        for correlation in correlations[:10]:
            ...
            try:
                ...
                if hasattr(correlation, 'correlation'):
                    ...
                    logger.warning(f"Cannot find strength attribute in correlation object for {correlation.var_a} -> {correlation.var_b}")
                    continue
                
                # Create a simple wrapper object with the expected interface
                class CorrelationWrapper:
                    def __init__(self, var_a, var_b, strength, p_value, sample_size, metadata):
                        self.var_a = var_a
                        self.var_b = var_b
                ...
                # Estimate value
                info_gain = self.intervention_prioritizer.estimate_information_gain(wrapped_correlation)
                cost = self.intervention_prioritizer.estimate_intervention_cost(wrapped_correlation)
                
                # Queue if high value
                if info_gain / cost > 2.0:
                    self.intervention_prioritizer.queue_intervention(wrapped_correlation)
    ...
    def save_state(self, path: str):
        ...
        state = {
            ...
            'component_availability': {
                ...
                'router': ROUTER_AVAILABLE,
                'meta_reasoning': META_REASONING_AVAILABLE,
                'self_improvement': META_REASONING_AVAILABLE and SelfImprovementDrive is not None,
                'safety_validator': EnhancedSafetyValidator is not None
            }
        }
        ...
        if self.safety_validator:
            try:
                safety_state = {}
                if hasattr(self.safety_validator, 'get_safety_stats'):
                    safety_state['safety_stats'] = self.safety_validator.get_safety_stats()
                if hasattr(self.safety_validator, 'constraint_manager'):
                    if hasattr(self.safety_validator.constraint_manager, 'get_constraint_stats'):
                        safety_state['constraint_stats'] = self.safety_validator.constraint_manager.get_constraint_stats()
        ...
        if self.meta_reasoning_enabled:
            try:
                meta_reasoning_state = {
                    'objectives': self.motivational_introspection.explain_motivation_structure(),
                    'statistics': self.motivational_introspection.get_statistics(),
                    'validation_history_size': len(self.motivational_introspection.validation_history)
                }
    ...
    def load_state(self, path: str):
        ...
        if self.meta_reasoning_enabled and (load_path / 'meta_reasoning_state.json').exists():
            try:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'meta_reasoning_enabled': self.meta_reasoning_enabled,
            'self_improvement_enabled': self.self_improvement_enabled,
            'improvement_running': self.improvement_running if self.self_improvement_enabled else False,
            'components': {
                'causal_graph': {
                    'available': CAUSAL_GRAPH_AVAILABLE,
                    'nodes': len(self.causal_graph.nodes) if CAUSAL_GRAPH_AVAILABLE else 0,
                    'edges': len(self.causal_graph.edges) if CAUSAL_GRAPH_AVAILABLE else 0
                },
                'correlation_tracker': {
                    'available': CorrelationTracker is not None
                },
                ...
                'dynamics_model': {
                    'available': DynamicsModel is not None
                },
                'invariant_detector': {
                    'available': INVARIANT_DETECTOR_AVAILABLE
                },
                ...
                'self_improvement': {
                    'available': META_REASONING_AVAILABLE and SelfImprovementDrive is not None,
...
def print_diagnostics():
    ...
    print("\nOptional Components:")
    for name in ['dynamics_model', 'invariant_detector', 'confidence_calibrator', 
                 'router', 'meta_reasoning', 'self_improvement']:
