...
try:
    ...
    logging.warning("networkx not available, using comprehensive fallback graph implementation")
                ...
                for node in nodes:
                    for successor in self.edges_dict.get(node, []):
                        if successor in nodes:
                            sub.add_edge(node, successor, **self.edge_attrs.get((node, successor), {}))
            ...
            def find_cycles_from(start, node, stack, blocked, block_map, sccs):
                ...
                for neighbor in graph.edges_dict.get(node, []):
                    if neighbor == start:
                        ...
                        result = find_cycles_from(start, neighbor, stack, blocked, block_map, sccs)
    ...
    def sample(self) -> float:
        ...
        if self.distribution_type == "normal":
            return np.random.normal(
                self.parameters.get('mean', 0),
                self.parameters.get('std', 1)
            )
        elif self.distribution_type == "uniform":
            return np.random.uniform(
                self.parameters.get('low', 0),
                self.parameters.get('high', 1)
            )
        elif self.distribution_type == "beta":
            return np.random.beta(
                self.parameters.get('alpha', 1),
                self.parameters.get('beta', 1)
            )
        else:
...
class GraphStructure:
    ...
    def update_edge_strength(self, cause: str, effect: str, new_strength: float) -> bool:
...
class CycleDetector:
        ...
        with self.lock:
            if NETWORKX_AVAILABLE:
                try:
                    ...
                    logger.warning(f"NetworkX cycle finding failed: {e}, using fallback")
                    return []
            else:
    ...
    def find_strongly_connected_components(self) -> List[Set[str]]:
        ...
        with self.lock:
            if NETWORKX_AVAILABLE:
                try:
                    sccs = list(nx.strongly_connected_components(self.structure.nx_graph))
                ...
                sccs = list(MockNX.strongly_connected_components(self.structure.nx_graph))
                return [scc for scc in sccs if len(scc) > 1]
        ...
        with self.lock:
            while self.has_cycles():
                ...
                if min_edge:
                    ...
                    logger.info(f"Removed edge {min_edge[0]} -> {min_edge[1]} to break cycle")
                else:
    ...
    def _find_minimum_weight_edge_in_cycles(self, cycles: List[List[str]]) -> Optional[Tuple[str, str]]:
...
class PathFinder:
    ...
    def find_all_paths(self, source: str, targets: Union[str, List[str]]) -> List[CausalPath]:
        """
                ...
                if source in self.structure.nodes and target in self.structure.nodes:
                    if NETWORKX_AVAILABLE:
                        try:
                            ...
                            logger.warning(f"NetworkX all_simple_paths failed: {e}, using fallback")
                            raw_paths = self._find_paths_bfs(
                                source, 
                                target, 
                                len(self.structure.nodes)
                            )
                            for node_sequence in raw_paths:
                                causal_path = self._convert_to_causal_path(node_sequence)
                                if causal_path:
    ...
    def find_shortest_path(self, 
                          source: str, 
                          target: str, 
                          weighted: bool = True) -> Optional[CausalPath]:
        """
            ...
            try:
                if weighted:
                    ...
                    if NETWORKX_AVAILABLE:
                        ...
                        path = MockNX.shortest_path(self.structure.nx_graph, source, target)
    ...
    def get_shortest_path_length(self, 
                                 source: str, 
                                 target: str, 
                                 weighted: bool = True) -> Optional[float]:
        """
...
class DSeparationChecker:
    ...
    def _bayes_ball_d_separation(self, x: str, y: str, conditioning_set: Set[str]) -> bool:
        """
        ...
        with self.lock:
            if NETWORKX_AVAILABLE:
                try:
                    ...
                    logger.warning(f"NetworkX topological_sort failed: {e}, using fallback")
                    return self._topological_sort_kahn()
            else:
                try:
                    return MockNX.topological_sort(self.structure.nx_graph)
                except Exception as e:
                    logger.warning(f"MockNX topological_sort failed: {e}, using fallback")
                    return self._topological_sort_kahn()
...
class CausalDAG:
    ...
    def __init__(self, safety_config: Optional[Dict[str, Any]] = None):
        ...
        if SAFETY_VALIDATOR_AVAILABLE and EnhancedSafetyValidator is not None:
            ...
            self.safety_validator = None
            logger.warning("CausalDAG: Safety validator not available - operating without safety checks")
        ...
        with self.lock:
            ...
            if self.safety_validator:
                ...
                if not edge_check['safe']:
                    logger.warning(f"Blocked unsafe edge {cause} -> {effect}: {edge_check['reason']}")
            ...
            edge = CausalEdge(
                ...
                confidence_interval=confidence_interval or (strength * 0.9, min(1.0, strength * 1.1)),
    ...
    def add_stochastic_edge(self,
                           cause: str,
                           effect: str,
                           probability_distribution: Union[ProbabilityDistribution, Dict[str, Any]]) -> bool:
        """
        ...
        with self.lock:
            ...
            if self.safety_validator:
                dist_check = self._validate_distribution_safety(probability_distribution)
                if not dist_check['safe']:
                    logger.warning(f"Blocked unsafe stochastic edge {cause} -> {effect}: {dist_check['reason']}")
                    self.safety_blocks['stochastic_edge'] += 1
                    return False
            
            # EXAMINE: Check for cycle
            if self.cycle_detector.would_create_cycle(cause, effect):
                logger.warning(f"Stochastic edge {cause} -> {effect} would create cycle")
                return False
    ...
    def update_edge_strength(self, cause: str, effect: str, new_strength: float) -> bool:
        ...
        with self.lock:
            ...
            if self.safety_validator:
                if not np.isfinite(new_strength) or new_strength < 0 or new_strength > 1:
            ...
            if result:
                # Invalidate caches atomically
                self._invalidate_caches()
                logger.debug(f"Updated edge {cause} -> {effect} strength to {new_strength:.2f}")
    ...
    def _validate_edge_safety(self, cause: str, effect: str, strength: float) -> Dict[str, Any]:
        """
    ...
    def _validate_distribution_safety(self, distribution: ProbabilityDistribution) -> Dict[str, Any]:
        """
        ...
        if distribution.distribution_type not in valid_types:
            violations.append(f"Invalid distribution type: {distribution.distribution_type}")
    ...
    def find_paths(self, source: str, targets: Union[str, List[str]], max_length: int = 5) -> List[CausalPath]:
        """
    ...
    def find_shortest_path(self, source: str, target: str, weighted: bool = True) -> Optional[CausalPath]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'longest_path': self.get_longest_path_length() if not self.has_cycles() else -1,
            'cache_size': len(self.path_finder.path_cache)
        }
        
        # Add degree statistics
        if self.structure.nodes:
            in_degrees = [len(self.structure.get_parents(n)) for n in self.structure.nodes]
            out_degrees = [len(self.structure.get_children(n)) for n in self.structure.nodes]
    ...
    def import_from_dict(self, data: Dict[str, Any]) -> bool:
        ...
        with self.lock:
            try:
                ...
                for edge_dict in edges:
                    if edge_dict.get('is_stochastic') and 'probability_distribution' in edge_dict:
                        # Import stochastic edge
                        self.add_stochastic_edge(
                            cause=edge_dict['cause'],
                            effect=edge_dict['effect'],
                            probability_distribution=edge_dict['probability_distribution']
                        )
                    else:
                        # Import regular edge
                        self.add_edge(
                ...
                logger.info(f"Imported graph with {len(self.structure.nodes)} nodes and {self.edge_count} edges")
                return True
