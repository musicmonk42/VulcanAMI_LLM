...
if not SKLEARN_AVAILABLE:
    ...
    class KMeans:
        ...
        def fit_predict(self, X):
            ...
            for _ in range(self.n_init):
                ...
                # Calculate inertia
                inertia = sum(np.min([np.linalg.norm(x - c) for c in centers]) for x in X)
    ...
    class OPTICS:
        ...
        def fit_predict(self, X):
            # Fallback to DBSCAN
            dbscan = SimpleDBSCAN(eps=self.max_eps / 2, min_samples=self.min_samples, metric=self.metric)
...
class OptimizedPathClusterer:
    ...
    def _select_clustering_method(self, feature_matrix: np.ndarray, n_paths: int) -> str:
    ...
    def _cluster_hierarchical(self, feature_matrix: np.ndarray, n_paths: int) -> np.ndarray:
    ...
    def _evaluate_clustering_quality(self, feature_matrix: np.ndarray, labels: np.ndarray):
        """Evaluate clustering quality"""
        ...
        try:
            ...
            logger.info("Clustering quality: silhouette=%.3f, CH=%.1f, n_clusters=%d",
                       silhouette, calinski, len(set(labels)))
    ...
    def _create_clusters(self, 
                        paths: List[Path], 
                        labels: np.ndarray,
                        correlation_matrix: np.ndarray) -> List[PathCluster]:
...
class PathAnalyzer:
    ...
    def calculate_path_correlation(self, path_a: Path, path_b: Path) -> float:
        ...
        with self.lock:
            ...
            structural_similarity = self._calculate_structural_similarity(path_a, path_b)
    ...
    def _compare_strength_patterns(self, strengths_a: List[float], strengths_b: List[float]) -> float:
...
class PathEffectCalculator:
    ...
    def calculate_path_effect(self, 
                             path: Path,
                             initial_value: float,
    ...
    def calculate_chain_effects(self,
                               cause: str,
                               path: Path,
    ...
    def _calculate_edge_effect(self,
                              from_node: str,
                              to_node: str,
                              strength: float,
                              input_value: float,
    ...
    def _apply_moderators(self,
                         effect: float,
                         from_node: str,
                         to_node: str,
                         moderators: Dict[str, Any]) -> float:
...
class MonteCarloSampler:
    ...
    def sample_from_cluster(self,
                           cluster: PathCluster,
                           n_samples: int = None) -> List[Path]:
    ...
    def _sample_single_path(self, base_path: Path, n_samples: int) -> List[Path]:
        ...
        for _ in range(n_samples):
            ...
            total_strength = np.prod([e[2] for e in varied_edges]) if varied_edges else 1.0
    ...
    def _sample_multiple_paths(self, cluster: PathCluster, n_samples: int) -> List[Path]:
...
class PredictionCombiner:
    ...
    def combine(self,
               ...
               method: Union[str, CombinationMethod]) -> Prediction:
        ...
        if isinstance(method, str):
            method_enum = CombinationMethod(method) if method in [m.value for m in CombinationMethod] else CombinationMethod.WEIGHTED_QUANTILE
        else:
            method_enum = method
    ...
    def _extract_values_weights(self, 
                               predictions: List[Union[Prediction, Tuple[float, float]]]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _weighted_mean(self, values: np.ndarray, weights: np.ndarray, method_name: str) -> Prediction:
    ...
    def _weighted_median(self, values: np.ndarray, weights: np.ndarray, method_name: str) -> Prediction:
    ...
    def _weighted_quantile(self, values: np.ndarray, weights: np.ndarray, method_name: str) -> Prediction:
    ...
    def _bootstrap(self, values: np.ndarray, weights: np.ndarray, method_name: str) -> Prediction:
...
class EnsemblePredictor:
    ...
    def __init__(self,
        ...
        try:
            ...
            logger.warning(f"safety_validator not available: {str(e)}. Operating without safety checks")
        
        # Components - FULLY OPTIMIZED
        self.clusterer = OptimizedPathClusterer(
            self.path_tracer.analyzer,
        ...
        logger.info("EnsemblePredictor initialized (FULLY OPTIMIZED) with method=%s, clustering=%s", 
                   default_method, clustering_method)
    
    def predict_with_path_ensemble(self,
                                   action: Any,
                                   context: Dict[str, Any],
                                   paths: List[Path]) -> Prediction:
        """
        ...
        for i, path in enumerate(paths):
            if not isinstance(path, Path):
                raise TypeError(f"paths[{i}] must be Path object, got {type(path).__name__}")
        ...
        with self.lock:
            ...
            self.performance_metrics['clustering_quality'] = self.clusterer.get_quality_metrics()
            ...
            if cluster_predictions:
                ...
                final_prediction = self._fallback_prediction(safe_paths, action, context)
            ...
            if self.safety_validator:
                ...
                if not pred_check['safe']:
                    logger.warning("Unsafe prediction detected: %s", pred_check['reason'])
                    self.safety_corrections['prediction'] += 1
                    final_prediction = self._apply_prediction_corrections(final_prediction, pred_check)
    ...
    def _validate_prediction_safety(self, prediction: Prediction, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if prediction.expected < prediction.lower_bound or prediction.expected > prediction.upper_bound:
    ...
    def _apply_prediction_corrections(self, prediction: Prediction, validation: Dict[str, Any]) -> Prediction:
        """Apply safety corrections to prediction"""
        
        # Clamp values to safe ranges
        safe_expected = np.clip(prediction.expected, -1e6, 1e6) if np.isfinite(prediction.expected) else 0.0
        safe_lower = np.clip(prediction.lower_bound, -1e6, 1e6) if np.isfinite(prediction.lower_bound) else safe_expected - 1.0
        safe_upper = np.clip(prediction.upper_bound, -1e6, 1e6) if np.isfinite(prediction.upper_bound) else safe_expected + 1.0
        ...
        corrected = Prediction(
            ...
            metadata={**prediction.metadata, 'safety_corrected': True, 'correction_reason': validation['reason']}
        )
        
        return corrected
    
    def _generate_cluster_predictions(self,
    ...
    def _create_cluster_prediction(self,
                                  values: List[float],
                                  cluster: PathCluster,
    ...
    def _combine_cluster_predictions(self,
                                    cluster_predictions: List[Prediction],
                                    context: Dict[str, Any]) -> Prediction:
    ...
    def _no_path_prediction(self) -> Prediction:
        ...
        return Prediction(
            ...
            method="no_paths",
            supporting_paths=[],
            metadata={'note': 'No paths available - neutral prediction with reduced confidence'}
        )
    ...
    def _update_performance_metrics(self, prediction: Prediction):
        ...
        self.performance_metrics['average_confidence'] = (
            (old_avg_conf * (n - 1) + prediction.confidence) / n
        )
        
        # Average uncertainty
        old_avg_unc = self.performance_metrics['average_uncertainty']
        self.performance_metrics['average_uncertainty'] = (
            (old_avg_unc * (n - 1) + prediction.relative_uncertainty()) / n
        )
    
    def combine_predictions(self,
                           predictions: List[Union[Prediction, Tuple[float, float]]],
                           method: str = "weighted_quantile") -> Prediction:
    ...
    def optimize_parameters(self, 
                           validation_data: List[Tuple[Any, Dict[str, Any], List[Path], float]]):
        """
        ...
        logger.info("Optimizing predictor parameters with %d validation samples", len(validation_data))
        ...
        logger.info("Optimized clustering method: %s (error=%.4f)", best_method, best_error)
    ...
    def visualize_prediction_distribution(self, prediction: Prediction) -> Dict[str, Any]:
        """
