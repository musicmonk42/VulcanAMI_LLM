...
class SymbolicExpressionSystem:
    ...
    def parse(self, expr_str: str, variables: Optional[Set[str]] = None) -> 'SymbolicExpression':
        """
    ...
    def solve(self, 
             ...
             variable: Union[str, 'sp.Symbol', 'SimpleSymbol']) -> List[Any]:
        """
    ...
    def check_equality(self, expr1: 'SymbolicExpression', expr2: 'SymbolicExpression') -> bool:
        """
    ...
    def _parse_with_sympy(self, 
                         expr_str: str, 
                         variables: Optional[Set[str]] = None) -> 'SymbolicExpression':
...
class VariableExtractor(ast.NodeVisitor):
    ...
    def visit_Name(self, node):
        ...
        if node.id not in {'True', 'False', 'None'} and \
           node.id not in SymbolicExpressionSystem.SAFE_FUNCTIONS:
...
class SimpleExpression:
    ...
    def __init__(self, expr_str: str, variables: Set[str], ast_tree: Optional[ast.AST] = None):
        """
        ...
        if isinstance(node, ast.Expression):
            ...
            for op, comparator in zip(node.ops, node.comparators):
                ...
                if op_type not in SymbolicExpressionSystem.SAFE_COMPARISONS:
                    raise ExpressionSafetyError(f"Unsafe comparison: {op_type.__name__}")
            ...
            if isinstance(node.op, ast.And):
                ...
                raise ExpressionSafetyError(f"Unsafe boolean operation: {type(node.op).__name__}")
...
class InvariantEvaluator:
    ...
    def _evaluate_conservation(self, invariant: Invariant, state: Dict[str, Any]) -> bool:
...
class InvariantValidator:
    ...
    def validate(self, 
                invariant: Invariant,
                observations: List[Dict[str, Any]]) -> bool:
        """
...
class InvariantRegistry:
    ...
    def __init__(self, 
        ...
        if SAFETY_VALIDATOR_AVAILABLE:
            ...
            logger.warning("InvariantRegistry: Safety validator not available - operating without safety checks")
        ...
        logger.info("InvariantRegistry initialized (refactored with safety and full symbolic system)")
    ...
    def register(self, invariant: Invariant) -> str:
        ...
        with self.lock:
            ...
            logger.info("Registered invariant %s: %s", invariant_id, invariant.expression)
    ...
    def check_invariant_violations(self, state: Union[Dict[str, Any], float, int]) -> List[Tuple[str, Invariant]]:
        """
        ...
        if isinstance(state, (int, float)):
            ...
            logger.warning(f"Unexpected state type: {type(state).__name__}, converting to dict")
            try:
                state_dict = {'value': float(state)}
            except (TypeError, ValueError):
                logger.error(f"Cannot convert state of type {type(state).__name__} to dict")
                return []
            ...
            try:
                if hasattr(self.safety_validator, 'validate_state'):
                    ...
                    if not state_check.get('safe', True):
                        logger.warning("Unsafe state provided: %s", state_check.get('reason', 'unknown'))
            ...
            for inv_id, invariant in all_invariants.items():
                ...
                if not self.evaluator.evaluate(invariant, state_dict):
                    ...
                    if invariant.violation_count >= self.violation_threshold:
                        logger.warning("Removing invariant %s due to excessive violations", inv_id)
    ...
    def get_invariants_for_domain(self, domain: str) -> List[Invariant]:
        ...
        with self.lock:
            ...
            filtered = [
                inv for _, inv in domain_invariants 
    ...
    def get_invariants_for_variables(self, variables: List[str]) -> List[Invariant]:
        ...
        with self.lock:
            ...
            # SELECT: Filter by confidence
            filtered = [
                inv for _, inv in variable_invariants
                if inv.confidence >= self.confidence_threshold
            ]
    ...
    def _is_duplicate(self, inv1: Invariant, inv2: Invariant) -> bool:
        ...
        if inv1.symbolic_expr and inv2.symbolic_expr:
            return self.symbolic_system.check_equality(inv1.symbolic_expr, inv2.symbolic_expr)
...
class ConservationLawDetector:
    ...
    def _detect_constant_variables(self, variables: Dict[str, List[float]]) -> List[Invariant]:
    ...
    def _detect_conserved_pairs(self, 
                               variables: Dict[str, List[float]], 
                               var_names: List[str]) -> List[Invariant]:
...
class LinearRelationshipDetector:
    ...
    def _check_linear_relationship(self, 
                                  var1: str, 
                                  var2: str, 
                                  variables: Dict[str, List[float]]) -> Optional[Invariant]:
...
class InvariantDetector:
    ...
    def __init__(self,
        ...
        if SAFETY_VALIDATOR_AVAILABLE:
            ...
            self.safety_validator = None
            logger.warning("InvariantDetector: Safety validator not available - operating without safety checks")
        ...
        logger.info("InvariantDetector initialized (refactored with full symbolic system)")
    
    def check(self, observations: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        """
        ...
        with self.lock:
            ...
            if self.safety_validator:
                ...
                for obs in observations:
                    try:
                        if hasattr(self.safety_validator, 'analyze_observation_safety'):
                            obs_check = self.safety_validator.analyze_observation_safety(obs)
                ...
                if len(safe_observations) < self.min_samples:
                    logger.warning("Insufficient safe observations for invariant detection")
                    return []
            ...
            logger.info("Detected %d invariants from %d observations", 
                       len(filtered), len(safe_observations))
        
        return filtered
    
    def find_conservation_laws(self, variables: Dict[str, List[float]]) -> List[Invariant]:
    ...
    def test_invariant_hypothesis(self, 
                                 hypothesis: str,
                                 data: List[Dict[str, Any]]) -> Optional[Invariant]:
        """
    ...
    def _validate_variables_safety(self, variables: Dict[str, List[float]]) -> Dict[str, List[float]]:
        ...
        for var, values in variables.items():
            ...
            if len(safe_values) != len(values):
                self.safety_corrections['non_finite_values'] += (len(values) - len(safe_values))
    ...
    def _extract_variables(self, observations: List[Dict[str, Any]]) -> Dict[str, List[float]]:
    ...
    def _detect_constraints(self, 
                          ...
                          variables: Dict[str, List[float]]) -> List[Invariant]:
        """Detect constraint invariants"""
        
        return self.find_constraints(observations)
    
    def _detect_other_patterns(self,
                             observations: List[Dict[str, Any]],
                             variables: Dict[str, List[float]]) -> List[Invariant]:
            ...
            for var2 in var_names[i+1:]:
                if var1 in variables and var2 in variables:
                    ...
                    if len(vals1) == len(vals2):
                        ...
                        if np.mean(diff) < 0.01 * (np.mean(np.abs(vals1)) + np.mean(np.abs(vals2))):
                            inv = Invariant(
    ...
    def _detect_periodic_patterns(self, variables: Dict[str, List[float]]) -> List[Invariant]:
    ...
    def _detect_monotonic_relationships(self, variables: Dict[str, List[float]]) -> List[Invariant]:
                    ...
                    if len(vals1) == len(vals2) and len(vals1) >= self.min_samples:
                        ...
                        if abs(spearman_corr) > 0.95:
                            ...
                            inv = Invariant(
                                ...
                                parameters={'direction': direction, 'correlation': spearman_corr}
                            )
                            invariants.append(inv)
