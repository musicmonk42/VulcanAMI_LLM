...
try:
    ...
    logging.warning("sklearn.isotonic not available, using comprehensive fallback implementation")
...
try:
    ...
    logging.warning("sklearn.linear_model not available, using comprehensive fallback implementation")
...
class RobustIsotonicRegression:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        logger.debug(f"Isotonic regression fitted with {len(X)} samples, "
    ...
    def predict(self, X):
        ...
        if self.out_of_bounds == 'clip':
            ...
            y_pred = np.clip(y_pred, np.min(self.y_thresholds_), np.max(self.y_thresholds_))
            ...
            if np.any(X_flat < self.X_min_) or np.any(X_flat > self.X_max_):
                raise ValueError(f"X contains values outside fitted range "
    ...
    def _pava_increasing(self, y, weights):
        ...
        while i < len(blocks) - 1:
            ...
            if blocks[i]['value'] > blocks[i + 1]['value']:
                ...
                new_value = (block1['value'] * block1['weight'] + 
                           block2['value'] * block2['weight']) / total_weight
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for iteration in range(self.max_iter):
            ...
            cross_entropy = -np.mean(
                sample_weight * (y * np.log(predictions_clipped) + 
...
class BetaCalibrator:
    ...
    def fit(self, probabilities, outcomes):
        ...
        if len(probabilities) < 2:
            logger.warning("Too few samples for beta calibration, using default parameters")
            self._is_fitted = True
            return self
        ...
        self._is_fitted = True
        logger.debug(f"Beta calibration fitted with alpha={self.alpha:.3f}, beta={self.beta:.3f}")
...
class ConfidenceCalibrator:
    ...
    def __init__(self, 
        ...
        # Calibration models
        self.isotonic_model = IsotonicRegression(out_of_bounds='clip' if SKLEARN_ISOTONIC_AVAILABLE else 'clip',
                                                 y_min=0.0, y_max=1.0)
        self.platt_model = LogisticRegression(max_iter=100 if not SKLEARN_LOGISTIC_AVAILABLE else 100)
        ...
        logger.info(f"ConfidenceCalibrator initialized with method={method}, n_bins={n_bins}")
        ...
        if not self._safety_validator_initialized:
            try:
                ...
                if isinstance(self._safety_config, dict) and self._safety_config:
                    self._safety_validator_instance = EnhancedSafetyValidator(SafetyConfig.from_dict(self._safety_config))
                else:
                ...
                logger.warning(f"ConfidenceCalibrator: Safety validator not available: {e}")
                self._safety_validator_instance = None
    ...
    def calibrate(self, 
                  ...
                  safety_context: Optional[Dict[str, Any]] = None) -> float:
        """
        ...
        with self.lock:
            ...
            use_context_calibration = (
                context_features is not None and 
                self.context_model is not None and
                np.all(np.isfinite(context_features))
            )
            
            if use_context_calibration:
                # Context-aware calibration
                try:
                    calibrated = self._context_aware_calibration(raw_confidence, context_features)
                except Exception as e:
                    logger.debug(f"Context-aware calibration failed: {e}, using basic calibration")
            ...
            if self.safety_validator and safety_context:
                ...
                if not safety_check['safe']:
                    logger.debug(f"Reducing confidence due to unsafe region: {safety_check['reason']}")
    ...
    def update_calibration(self, 
        ...
        with self.lock:
            ...
            record = PredictionRecord(
                ...
                context_features=context_features if context_features is not None else None,
    ...
    def _validate_safety_context(self, safety_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _initialize_bins(self) -> List[CalibrationBin]:
        ...
        for i in range(self.n_bins):
            bins.append(CalibrationBin(
                min_confidence=i * bin_width,
                max_confidence=(i + 1) * bin_width
            ))
    ...
    def _retrain_calibrators(self):
        ...
        try:
            ...
            if not np.all(np.isfinite(X)) or not np.all(np.isfinite(y)):
                logger.warning("Non-finite values in training data, skipping retraining")
    ...
    def _context_aware_calibration(self, 
                                   confidence: float,
        ...
        try:
            if self.context_model is not None and hasattr(self.context_model, 'predict'):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            stats = {
                ...
                'avg_calibration_time': self.calibration_time_total / max(1, self.calibration_calls),
...
class ModelConfidenceTracker:
        ...
        if not self._safety_validator_initialized:
            try:
                ...
                if isinstance(self._safety_config, dict) and self._safety_config:
                    ...
                    self._safety_validator_instance = EnhancedSafetyValidator()
                logger.info("ModelConfidenceTracker: Safety validator initialized (lazy)")
            except ImportError as e:
                logger.warning(f"ModelConfidenceTracker: Safety validator not available: {e}")
                self._safety_validator_instance = None
    ...
    def update(self, 
               ...
               prediction: Optional[Any] = None) -> Dict[str, Any]:
        """
        ...
        with self.lock:
            ...
            try:
                ...
                self.model_confidence = self.decay_rate * self.model_confidence + \
                                       (1 - self.decay_rate) * (self.model_confidence + confidence_delta)
    ...
    def get_confidence_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            recent_errors = list(self.prediction_errors)[-10:] if self.prediction_errors else []
            ...
            stats = {
                ...
                'recent_mean_error': float(np.mean(recent_errors)) if recent_errors else None,
                'recent_max_error': float(np.max(recent_errors)) if recent_errors else None,
                'recent_min_error': float(np.min(recent_errors)) if recent_errors else None,
    ...
    def _update_rolling_metrics(self, error: float):
        ...
        alpha = 0.1
        self.rolling_accuracy = float((1 - alpha) * self.rolling_accuracy + alpha * accuracy)
    ...
    def save_state(self, path: str):
        ...
        with self.lock:
            state = {
                ...
                'domain_confidence': {k: float(v) for k, v in self.domain_confidence.items()},
                'rolling_accuracy': float(self.rolling_accuracy),
                'rolling_error': float(self.rolling_error),
    ...
    def load_state(self, path: str):
        ...
        with self.lock:
            ...
            self.domain_confidence = defaultdict(
                lambda: 0.5,
                {k: float(v) for k, v in domain_conf_dict.items()}
            )
...
class TemperatureScaling:
    ...
    def _compute_temperature_gradient(self, logits, labels, temperature):
        ...
        if labels.ndim == 1:
            loss_t = -np.mean(np.log(np.clip(probs[np.arange(len(labels)), labels], epsilon, 1)))
        else:
            loss_t = -np.mean(np.sum(labels * np.log(np.clip(probs, epsilon, 1)), axis=-1))
        
        # Forward pass at T + epsilon
        scaled_logits_plus = logits / (temperature + epsilon)
        exp_logits_plus = np.exp(scaled_logits_plus - np.max(scaled_logits_plus, axis=-1, keepdims=True))
        probs_plus = exp_logits_plus / np.sum(exp_logits_plus, axis=-1, keepdims=True)
        
        if labels.ndim == 1:
            loss_t_plus = -np.mean(np.log(np.clip(probs_plus[np.arange(len(labels)), labels], epsilon, 1)))
        else:
            loss_t_plus = -np.mean(np.sum(labels * np.log(np.clip(probs_plus, epsilon, 1)), axis=-1))
...
class CalibrationEnsemble:
    ...
    def predict(self, predictions):
        ...
        ensemble_pred = np.sum(
            calibrated_predictions * valid_weights[:, np.newaxis],
            axis=0
        )
...
class AdaptiveCalibrator:
    ...
    def _evaluate_and_adapt(self):
        ...
        if best_method and best_method != self.current_best_method:
            logger.info(f"Adaptive calibrator switched from {self.current_best_method} "
...
def plot_reliability_diagram(calibrator: ConfidenceCalibrator, 
...
def compare_calibration_methods(predictions: np.ndarray,
                               outcomes: np.ndarray,
    ...
    for method in methods:
        try:
            ...
            logger.info(f"{method}: ECE={metrics['ece']:.4f}, Brier={metrics['brier_score']:.4f}")
