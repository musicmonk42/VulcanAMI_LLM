...
class RESTAPIInterface(ExternalSystemInterface):
    ...
    async def execute_intervention(self,
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info("Attempting intervention execution (attempt %d/%d)", 
        ...
        raise RuntimeError(f"Failed to execute intervention after {self.max_retries} attempts. Last error: {last_error}")
    ...
    async def rollback_intervention(self, intervention_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning("Rollback failed for intervention %s: %s",
                             intervention_id, result.get('reason', 'unknown'))
...
class InterventionScheduler:
    ...
    def schedule(self, 
                ...
                cost_benefit_ratio: float = 2.0) -> List[InterventionCandidate]:
    ...
    def create_batches(self,
                      ...
                      max_batch_size: int = 5) -> List[List[InterventionCandidate]]:
    ...
    def _build_dependency_graph(self,
                               candidates: List[InterventionCandidate]) -> Dict[int, Set[int]]:
    ...
    def _depends_on(self,
                   ...
                   cand2: InterventionCandidate) -> bool:
    ...
    def _has_conflict(self,
                     candidate: InterventionCandidate,
                     batch: List[InterventionCandidate]) -> bool:
...
class ConfounderDetector:
    ...
    def identify_confounders(self,
                           correlation: Correlation,
                           result: InterventionResult) -> List[str]:
    ...
    def record_failure(self,
        ...
        with self.lock:
            ...
            if len(self.failure_reasons[failure_key]) >= 3:
                ...
                common_confounders = [c for c in set(all_confounders) 
                                     if all_confounders.count(c) >= 2]
    ...
    def _test_confounder(self, confounder: str, correlation: Correlation) -> bool:
        """Test if variable is a confounder"""
        # Simplified test - in practice would run partial correlation
...
class InterventionPrioritizer:
    ...
    def prioritize_interventions(self,
                                correlations: List[Correlation],
                                budget: float) -> List[InterventionCandidate]:
    ...
    def create_intervention_batch(self,
                                 candidates: List[InterventionCandidate],
                                 max_batch_size: int = 5) -> List[List[InterventionCandidate]]:
...
class InterventionExecutor:
    ...
    def __init__(self,
            ...
            try:
                ...
                if isinstance(safety_config, dict) and safety_config:
                    ...
                    try:
                        ...
                        logger.error(f"InterventionExecutor: SafetyConfig.from_dict failed: {str(e)}. Using default config.")
                ...
                logger.warning(f"safety_validator not available: {str(e)}. InterventionExecutor operating without safety checks")
                self.safety_validator = None
                self.safety_initialization_successful = False # Explicitly set false
            except Exception as e:
                logger.error(f"InterventionExecutor: Unexpected error initializing SafetyValidator: {str(e)}. Safety disabled.")
                self.safety_validator = None
                self.safety_initialization_successful = False # Explicitly set false
        ...
        if external_interface:
            ...
            logger.warning("No external interface provided, using MockExternalInterface")
        ...
        logger.info("InterventionExecutor initialized (REAL EXECUTION ENABLED) with simulation_mode=%s", simulation_mode)
    
    def execute_intervention(self,
                           intervention: Union[InterventionCandidate, Any]) -> InterventionResult:
        """
        ...
        with self.lock:
            ...
            if isinstance(intervention, InterventionCandidate):
                ...
                correlation = intervention.correlation
                intervention_type = getattr(intervention, 'intervention_type', InterventionType.DIRECT)
            else:
            ...
            if self.safety_validator:
                try:
                    if hasattr(self.safety_validator, 'validate_intervention'):
                        intervention_check = self.safety_validator.validate_intervention(
                            cause=correlation.var_a,
                            effect=correlation.var_b,
                            intervention_type=intervention_type.value if isinstance(intervention_type, InterventionType) else str(intervention_type),
                            metadata=correlation.metadata if hasattr(correlation, 'metadata') else {}
                        )
                        
                        if not intervention_check.get('safe', True):
                            logger.error("BLOCKED unsafe intervention: %s -> %s: %s",
                            ...
                            if hasattr(self.safety_validator, 'audit_logger') and self.safety_validator.audit_logger:
                                try:
                            ...
                            return InterventionResult(
                                ...
                                metadata={
                                    'blocked': True,
                                    'safety_blocked': True,
                                    'reason': intervention_check.get('reason', 'unknown'),
                                    'violations': intervention_check.get('violations', [])
                                }
                            )
                except Exception as e:
            ...
            if self.safety_validator:
                ...
                if not result_validation['safe']:
                    logger.warning("Unsafe intervention result detected: %s",
    ...
    def handle_intervention_failure(self,
                                   intervention: Any,
                                   result: InterventionResult):
        """Handle failed or inconclusive intervention - REFACTORED"""
        
        correlation = intervention.correlation if hasattr(intervention, 'correlation') else intervention
    ...
    def create_controlled_intervention(self,
                                      intervention: Any,
                                      control_for: List[str]) -> InterventionCandidate:
        ...
        controlled_correlation = Correlation(
            ...
            sample_size=correlation.sample_size if hasattr(correlation, 'sample_size') else 0,
            metadata={
        ...
        return InterventionCandidate(
            ...
            priority=intervention.priority if hasattr(intervention, 'priority') else 1.0,
            cost=controlled_cost,
            info_gain=intervention.info_gain if hasattr(intervention, 'info_gain') else 1.0,
            intervention_type=InterventionType.RANDOMIZED,
            metadata={'controls': control_for}
        )
    
    def _execute_simulated(self,
                         correlation: Correlation,
    ...
    def _execute_real(self,
        ...
        logger.critical("REAL INTERVENTION EXECUTION: %s -> %s (type: %s)",
                       correlation.var_a, correlation.var_b, 
                       intervention_type.value if isinstance(intervention_type, InterventionType) else str(intervention_type))
        ...
        intervention_id = f"intv_{int(time.time() * 1000)}_{np.random.randint(1000, 9999)}"
        ...
        try:
            ...
            if not status.get('available', False):
                raise RuntimeError(f"External system not available: {status.get('reason', 'unknown')}")
            ...
            with self.lock:
                self.active_interventions[intervention_id] = {
                    ...
                    'type': intervention_type.value if isinstance(intervention_type, InterventionType) else str(intervention_type)
                }
            
            logger.info("Executing intervention %s with params: %s", 
                       intervention_id, intervention_params)
            ...
            try:
                ...
                logger.error("Intervention %s timed out after %s seconds",
                           intervention_id, self.intervention_timeout)
            ...
            logger.info("Intervention %s completed: %s", intervention_id, execution_result)
            ...
            logger.info("Intervention %s result: type=%s, effect=%s, p=%s",
                       intervention_id, result_type, observed_effect, p_value)
            
            return result
            
        except Exception as e:
            logger.error("Error executing real intervention %s: %s", 
                        intervention_id, str(e), exc_info=True)
            ...
            try:
                ...
                logger.error("Rollback failed for %s: %s", 
                           intervention_id, rollback_error)
    ...
    def _prepare_intervention_params(self,
        ...
        if hasattr(correlation, 'metadata') and correlation.metadata:
            ...
            if 'controlled_variables' in correlation.metadata:
                params['control_variables'] = correlation.metadata['controlled_variables']
    ...
    def _validate_result_safety(self, result: InterventionResult) -> Dict[str, Any]:
        ...
        if result.causal_strength is not None:
            if not np.isfinite(result.causal_strength):
                violations.append(f"Non-finite causal strength: {result.causal_strength}")
            elif abs(result.causal_strength) > 10.0:
                violations.append(f"Excessive causal strength: {result.causal_strength}")
    ...
    def _apply_result_corrections(self,
                                 result: InterventionResult,
                                 validation: Dict[str, Any]) -> InterventionResult:
    ...
    def _calculate_actual_cost(self,
        ...
        # Sample size cost
        sample_cost = correlation.metadata.get('sample_size', 100) * 0.1 if hasattr(correlation, 'metadata') else 10.0
        
        # Complexity cost
        complexity = correlation.metadata.get('complexity', 1.0) if hasattr(correlation, 'metadata') else 1.0
    ...
    def get_safety_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'external_interface': type(self.external_interface).__name__ if self.external_interface else None
        }
