...
def _init_lazy_imports():
    ...
    """
    global ObjectiveHierarchy, CounterfactualObjectiveReasoner, GoalConflictDetector, \
           ValidationTracker, TransparencyInterface, ConflictType, ConflictSeverity, \
           ValidationOutcome, PatternType, ObjectiveBlocker, ValidationPattern
    ...
    ObjectiveHierarchy = getattr(mod_objective_hierarchy, 'ObjectiveHierarchy', MagicMock())
    CounterfactualObjectiveReasoner = getattr(mod_counterfactual_objectives, 'CounterfactualObjectiveReasoner', MagicMock())
    GoalConflictDetector = getattr(mod_goal_conflict_detector, 'GoalConflictDetector', MagicMock())
    ValidationTracker = getattr(mod_validation_tracker, 'ValidationTracker', MagicMock())
    TransparencyInterface = getattr(mod_transparency_interface, 'TransparencyInterface', MagicMock())
    
    # Assign Enums and other types
    ConflictType = getattr(mod_goal_conflict_detector, 'ConflictType', MagicMock(spec=Enum))
    ConflictSeverity = getattr(mod_goal_conflict_detector, 'ConflictSeverity', MagicMock(spec=Enum))
    ValidationOutcome = getattr(mod_validation_tracker, 'ValidationOutcome', MagicMock(spec=Enum))
    ...
    ValidationPattern = getattr(mod_validation_tracker, 'ValidationPattern', MagicMock())
    ...
    if isinstance(ConflictSeverity, MagicMock):
        ConflictSeverity.CRITICAL = "critical"; ConflictSeverity.HIGH = "high"; ConflictSeverity.MEDIUM = "medium"
    if isinstance(ValidationOutcome, MagicMock):
        ValidationOutcome.APPROVED = "approved"; ValidationOutcome.REJECTED = "rejected"; ValidationOutcome.UNKNOWN = "unknown"
    if isinstance(PatternType, MagicMock):
        PatternType.SUCCESS = "success"; PatternType.RISKY = "risky"
...
class MotivationalIntrospection:
    ...
    def __init__(self,
                 ...
                 config_path: str = "configs/intrinsic_drives.json"): # Kept optional with default
        """
        ...
        if config_path is None:
            ...
            if design_spec is None:
                ...
                logger.warning("config_path was None and design_spec was None, using default path: %s", default_path_str)
            else:
        ...
        if design_spec is not None:
            ...
            logger.info("MotivationalIntrospection using provided design_spec (legacy mode)")
        else:
        ...
        self._objective_negotiator: Optional['ObjectiveNegotiator'] = None # For lazy loading negotiator
        ...
        logger.info("MotivationalIntrospection initialized with %d objectives",
    ...
    def _load_design_spec_from_config(self) -> Dict[str, Any]:
        ...
        if self.config_path is None:
             logger.warning("config_path is None, cannot load design_spec from config. Using defaults or provided spec.")
        ...
        for enc in ("utf-8", "utf-8-sig"):
            try:
                ...
                if not motiv_config:
                    ...
                    drives_config = full_config.get('drives', {}).get('self_improvement', {})
                    ...
                    logger.warning("No motivational_introspection section in config, using defaults")
                    return self._default_design_spec()
                ...
                if not design_spec:
                    logger.warning("No design_spec in motivational_introspection config, using defaults")
                    return self._default_design_spec()

                logger.info("Loaded design_spec from unified config at %s", self.config_path)
                return design_spec
                ...
                logger.error(f"Failed to parse config JSON with {enc}: {e}, using defaults")
                return self._default_design_spec()
        ...
        try:
            ...
            logger.error(f"Failed to parse config JSON even with replacement: {e}, using defaults")
            return self._default_design_spec()
        except Exception as e:
            logger.error(f"Critical error loading config with replacement: {e}, using defaults")
            return self._default_design_spec()
    ...
    def objective_hierarchy(self) -> ObjectiveHierarchy:
        ...
        if self._objective_hierarchy is None:
            ...
            if ObjectiveHierarchy is None or isinstance(ObjectiveHierarchy, MagicMock):
                 logger.error("ObjectiveHierarchy failed to load during init. Using MagicMock.")
    ...
    def counterfactual_reasoner(self) -> CounterfactualObjectiveReasoner:
        ...
        if self._counterfactual_reasoner is None:
            if CounterfactualObjectiveReasoner is None or isinstance(CounterfactualObjectiveReasoner, MagicMock):
                logger.error("CounterfactualObjectiveReasoner failed to load during init. Using MagicMock.")
    ...
    def conflict_detector(self) -> GoalConflictDetector:
        ...
        if self._conflict_detector is None:
            if GoalConflictDetector is None or isinstance(GoalConflictDetector, MagicMock):
                logger.error("GoalConflictDetector failed to load during init. Using MagicMock.")
    ...
    def validation_tracker(self) -> ValidationTracker:
        if self._validation_tracker is None:
            ...
            if ValidationTracker is None or isinstance(ValidationTracker, MagicMock):
                ...
                si_drive = getattr(self.world_model, 'self_improvement_drive', None) if self.world_model else None
    ...
    def transparency_interface(self) -> TransparencyInterface:
        ...
        if self._transparency_interface is None:
            if TransparencyInterface is None or isinstance(TransparencyInterface, MagicMock):
    ...
    def objective_negotiator(self) -> 'ObjectiveNegotiator':
        ...
        if not hasattr(self, '_objective_negotiator') or self._objective_negotiator is None:
    ...
    def introspect_current_objective(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if task_objective and (not alignment['aligned'] or task_constraints):
                ...
                analysis['conflicts'] = [c.to_dict() if hasattr(c, 'to_dict') else c for c in conflicts] # Serialize if needed
    ...
    def detect_objective_pathology(self, proposal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if prediction['prediction'] == 'likely_rejected' and prediction['confidence'] > 0.7:
                pathologies.append({
    ...
    def reason_about_alternatives(self,
        ...
        with self.lock:
            ...
            if current_objective not in available_objectives:
                logger.warning("Current objective %s not in design spec", current_objective)

            # SELECT: Choose relevant alternatives
            alternatives = [obj for obj in available_objectives
            ...
            for alt_objective in alternatives:
                analysis = reasoner.predict_under_objective(
                    alt_objective,
                    context
                )
                ...
                alternative_analyses.append({
                    ...
                    'predicted_outcome': analysis.to_dict() if hasattr(analysis, 'to_dict') else analysis,
                    'comparison_to_current': comparison.to_dict() if hasattr(comparison, 'to_dict') else comparison,
            ...
            pareto_frontier_points = reasoner.find_pareto_frontier(
    ...
    def explain_motivation_structure(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for obj_name in self.active_objectives:
                dependencies[obj_name] = list(self.objective_hierarchy.get_dependencies(obj_name)) # Convert set to list

                # Find conflicts with other objectives
                obj_conflicts = []
                for other_obj in self.active_objectives:
                    if other_obj != obj_name:
                        conflict = self.objective_hierarchy.find_conflicts(obj_name, other_obj)
                        if conflict:
    ...
    def validate_proposal_alignment(self, proposal: Dict[str, Any]) -> ProposalValidation:
        ...
        with self.lock:
            ...
            proposal_id = proposal.get('id', f'proposal_{time.time_ns()}_{self.stats["validations_performed"]}')
            ...
            if pathology['has_pathology']:
                ...
                conflicts.extend([
                    p for p in pathology['pathologies']
                    if isinstance(p, dict) and p.get('type') in ['objective_conflict', 'unacceptable_tradeoff', 'constraint_violation', 'constraint_conflict'] # Added constraint types
                ])
            ...
            for c in conflicts:
                ...
                # Create a stable ID for the conflict (e.g., based on objectives)
                 obj_tuple = tuple(sorted(c.get('objectives', []))) if c.get('objectives') else ()
            ...
            valid = overall_status in [ObjectiveStatus.ALIGNED, ObjectiveStatus.ACCEPTABLE, ObjectiveStatus.DRIFT]

            # Adjust validity based on historical prediction
            if historical_prediction['prediction'] == 'likely_rejected' and historical_prediction['confidence'] > 0.8:
                # Strong historical signal for rejection
                if valid:
                    # Downgrade from acceptable to requiring review
                    overall_status = ObjectiveStatus.CONFLICT # Use CONFLICT as non-valid status
                    valid = False
                    logger.info("Proposal %s downgraded based on strong historical rejection signal", proposal_id)
    ...
    def update_validation_outcome(self, proposal_id: str, actual_outcome: str):
        ...
        with self.lock:
            ...
            logger.info("Updated validation outcome for %s: %s", proposal_id, actual_outcome)
        ...
        with self.lock:
            ...
            return {
                ...
                'performance': {
                    ...
                    'common_failure_features': failure_analysis.get('common_features', [])[:5],
                ...
                'recommendations': self._generate_objective_recommendations(
                    objective, blockers_objs, better_proxies, failure_analysis # Pass original objects here
                )
            }

    def _generate_objective_recommendations(self,
                                           objective: str,
        ...
        if blockers:
            ...
            if top_blocker.potential_solutions:
                recommendations.append(f"Address primary blocker: {top_blocker.potential_solutions[0]}")
    ...
    def _check_objective_alignment(self, task_objective: Optional[str]) -> Dict[str, Any]:
        """Check if task objective aligns with design objectives"""
    ...
    def _get_objective_current_value(self, obj_name: str) -> Optional[float]:
        ...
        if obj_name in self.active_objectives and 'metadata' in self.active_objectives[obj_name]:
            ...
            if hasattr(pm, 'prediction_history') and pm.prediction_history:
                ...
                if recent:
                    ...
                    if isinstance(first_record, dict):
                        ...
                        has_predicted = 'predicted' in first_record or 'expected' in first_record
                    else:
                        has_actual = hasattr(first_record, 'actual') or hasattr(first_record, 'metadata')
                        has_predicted = hasattr(first_record, 'expected') or hasattr(first_record, 'predicted')
                    ...
                    if has_actual and has_predicted:
                        ...
                        for p in recent:
                            if isinstance(p, dict):
                                ...
                                actual = getattr(p, 'actual', p.metadata.get('actual') if hasattr(p, 'metadata') else None)
                                predicted = getattr(p, 'expected', getattr(p, 'predicted', None))

                            if actual is not None and predicted is not None:
                                try:
                                    # 5% tolerance
                                    if abs(float(predicted) - float(actual)) < 0.05 * max(1.0, abs(float(actual))):
                                        correct += 1
                                    total += 1
    ...
    def _get_self_improvement_success_rate(self) -> Optional[float]:
        ...
        if self.world_model and hasattr(self.world_model, 'self_improvement_drive'):
            ...
            if drive is None: return None # Handle case where drive is disabled
            ...
            if hasattr(drive, 'validation_history') and drive.validation_history:
                ...
                successful_validations = sum(
                    1 for v in drive.validation_history
        ...
        if self.world_model and hasattr(self.world_model, 'drives') and isinstance(self.world_model.drives, dict):
    ...
    def _check_safety(self) -> bool:
        ...
        if self.world_model and hasattr(self.world_model, 'safety_validator'):
            ...
            if validator is None: return True # Validator disabled
        ...
        if self.world_model and hasattr(self.world_model, 'performance_tracker'):
            ...
            if hasattr(tracker, 'get_average_latency'):
                ...
                if callable(result):
                    value = result()
                    return float(value) if value is not None and isinstance(value, (int, float)) else None
                return float(result) if result is not None and isinstance(result, (int, float)) else None
    ...
    def _get_energy_efficiency(self) -> Optional[float]:
        ...
        if self.world_model and hasattr(self.world_model, 'performance_tracker'):
            ...
            if hasattr(tracker, 'energy_consumed') and hasattr(tracker, 'work_completed'):
                if tracker.work_completed > 0 and tracker.energy_consumed > 0: # Avoid division by zero
    ...
    def _generate_introspection_reasoning(self,
    ...
    def _check_constraint_violations(self, proposal: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for obj_name, constraints in self.objective_constraints.items():
            ...
            if predicted_value is not None:
                try:
                    ...
                    logger.warning(f"Non-numeric predicted outcome for {obj_name}: {predicted_value}")
                ...
                if 'min' in constraints:
                    ...
                    if value < (min_limit - tolerance):
                        violations.append({
                            ...
                            'severity': 'critical' if self.objective_hierarchy.objectives.get(obj_name, MagicMock(priority=1)).priority == 0 else 'high'
                        })
    ...
    def _check_objective_conflicts(self, proposal: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _check_goal_drift(self, proposal: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if 'objective_weights' in proposal:
            ...
            if not isinstance(proposed_weights, dict):
                logger.warning("Malformed objective_weights in proposal: expected dict, got %s", type(proposed_weights))
                return {
                    'type': 'goal_drift',
                    'severity': 'high',
    ...
    def _check_tradeoffs(self, proposal: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if 'tradeoffs' in proposal:
            for sacrifice, gain in proposal['tradeoffs'].items():
                ...
                if sacrifice in self.objective_hierarchy.objectives: # Ensure hierarchy initialized
    ...
    def _matches_risky_pattern(self,
                               proposal: Dict[str, Any],
                               risky_patterns: List[ValidationPattern]) -> bool:
    ...
    def _extract_proposal_features(self, proposal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'objectives' in proposal:
            ...
            features['num_objectives'] = len(objectives_list) if isinstance(objectives_list, list) else 1
        ...
        for k, v in features.items():
             try: hash(v); hashable_features[k] = v
             except TypeError: hashable_features[k] = str(v)
    ...
    def _find_similar_failures(self, proposal: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for past_validation in tracker.validation_records:
            ...
            if past_validation.actual_outcome == 'failure' or past_validation.outcome == ValidationOutcome.REJECTED:
                ...
                if similarity > 0.6:
                    similar.append({
                        ...
                        'failure_reason': self._safe_get_reasoning(past_validation.validation_result) # Safely get reasoning
                    })
    ...
    def _calculate_proposal_similarity(self,
                                       proposal_a: Dict[str, Any],
                                       proposal_b: Dict[str, Any]) -> float:
        ...
        if intersection:
            ...
            value_similarity = matching_values / len(intersection) if intersection else 0.0
    ...
    def _assess_overall_severity(self, pathologies: List[Dict[str, Any]]) -> str:
        ...
        severities_map = {'critical': 4, 'high': 3, 'medium': 2, 'low': 1, 'none': 0, 'negligible': 0}
        max_severity = 0

        for p in pathologies:
            sev_str = p.get('severity', 'medium')
            # Handle enums
            if hasattr(sev_str, 'value'): sev_str = sev_str.value
            ...
            p_type = p.get('type')
            # Handle enums
            if hasattr(p_type, 'value'): p_type = p_type.value

            # Check if it's one of the acceptable conflict/tradeoff types
            acceptable_conflict_types = [ConflictType.TRADEOFF, 'tradeoff', 'objective_conflict', 'unacceptable_tradeoff']
            ...
            if p_type in acceptable_conflict_types:
                ...
                if not is_constraint_related:
                    sev_val = min(sev_val, severities_map['medium']) # Cap pure tradeoffs/conflicts at medium

            max_severity = max(max_severity, sev_val)

        if max_severity == 4: return 'critical'
        if max_severity == 3: return 'high'
        if max_severity == 2: return 'medium'
        if max_severity == 1: return 'low'
        return 'none'
    ...
    def _identify_tradeoffs(self,
                           objective_a: str,
                           objective_b: str) -> Dict[str, Any]:
        ...
        tradeoff_estimate = reasoner.estimate_tradeoffs(
            objective_a,
            objective_b
        )
        ...
        return {
            ...
            'estimated_magnitude': tradeoff_estimate.get('tradeoff_score', 0.0) # Extract score
        }

    def _recommend_objective(self,
                            current_objective: str,
                            alternatives: List[Dict[str, Any]],
                            pareto_frontier: List[Any]) -> str: # Expects ParetoPoint objects
        ...
        for pf in pareto_frontier:
            ...
            if hasattr(pf, 'objectives'):
                ...
                if current_objective in pf.objective_weights and pf.objective_weights[current_objective] > 0.5:
                    current_on_frontier = True
                    break
    ...
    def _predict_proposal_outcomes(self, proposal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hasattr(self.world_model, 'prediction_manager'):
            try:
                pm = self.world_model.prediction_manager
                if pm is None: raise AttributeError("prediction_manager is None")
                ...
                if hasattr(pm, 'ensemble_predictor'):
                    ...
                    if predictor is None: raise AttributeError("ensemble_predictor is None")

                    # Make prediction
                    if hasattr(predictor, 'predict_with_path_ensemble') or hasattr(predictor, 'predict'):
                        # Create scenario from proposal
                        action = proposal.get('action', 'apply') # Predictor needs an action
                        ...
                        try:
                            prediction_result = predictor.predict(action, context)
                             ...
                             if hasattr(self.world_model, 'causal_graph') and hasattr(self.world_model.causal_graph, 'find_all_paths'):
                                 try:
                                     # Find paths from action (as source) to targets
                                     paths = self.world_model.causal_graph.find_all_paths(action, targets)
                                 except Exception as e:
                                     logger.debug(f"Could not find causal paths: {e}")
                             prediction_result = predictor.predict_with_path_ensemble(action, context, paths)


                        # Check if result is a Prediction object
                        if hasattr(prediction_result, 'expected') and hasattr(prediction_result, 'confidence'):
                            # Extract effects on objectives from Prediction metadata
                            effects = prediction_result.metadata.get('objective_effects', {})
                            ...
                            if not effects and targets:
                                ...
                                primary_target = targets[0] if targets else proposal.get('objective', 'unknown')
                        ...
                        elif isinstance(prediction_result, dict): # Handle dict return type
                            ...
                            for obj_name in self.active_objectives.keys():
                                ...
                                elif obj_name == proposal.get('objective') and 'prediction' in prediction_result:
                ...
                if hasattr(pm, 'predict_impact'):
                    ...
                    return {
                        'predicted': True,
                        'confidence': impact.get('confidence', 0.6), # impact is a Mock, so this returns a Mock
    ...
    def _analyze_objective_impact(self,
                                  obj_name: str,
                                  obj_config: Dict[str, Any],
                                  proposal: Dict[str, Any],
                                  predicted_outcomes: Dict[str, Any]) -> ObjectiveAnalysis:
        ...
        if not isinstance(pred_confidence, (int, float)):
            logger.warning(f"Predicted outcome confidence was not numeric (got {type(pred_confidence)}), defaulting to 0.5.")
            pred_confidence = 0.5
        ...
        if predicted_outcomes.get('predicted') and 'effects' in predicted_outcomes:
            ...
            if predicted_value is not None:
                ...
                if value is not None:
                    ...
                    min_violated = constraint_min is not None and value < (float(constraint_min) - tolerance)
                    max_violated = constraint_max is not None and value > (float(constraint_max) + tolerance)

                    if min_violated:
                        status = ObjectiveStatus.VIOLATION
                        reasoning = f"Predicted value {value:.3f} below minimum {constraint_min}"
                        confidence = pred_confidence # Use original prediction confidence
                    elif max_violated:
                        status = ObjectiveStatus.VIOLATION
                        ...
                        if target_value is not None:
                            ...
                            target_tolerance = obj_config.get('metadata', {}).get('tolerance', 0.05)
                            if abs(value - float(target_value)) <= target_tolerance:
                                ...
                                status = ObjectiveStatus.ACCEPTABLE # Within constraints but missed target
                                reasoning = f"Predicted value {value:.3f} within constraints but missed target {target_value}"
                        else:
                            # No target, just within constraints
                            status = ObjectiveStatus.ALIGNED
                            reasoning = f"Predicted value {value:.3f} within constraints"
    ...
    def _suggest_alternatives(self,
                             ...
                             pathology: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _determine_overall_status(self,
                                  ...
                                  conflicts: List[Dict[str, Any]]) -> ObjectiveStatus:
             ...
             if severity == 'critical':
                 ...
                 for p in pathology['pathologies']:
                     if p.get('severity') == 'critical':
                         ...
                         # Handle enums
                         if hasattr(p_type, 'value'): p_type = p_type.value
                         if p_type not in [ConflictType.TRADEOFF, 'tradeoff', 'objective_conflict', 'unacceptable_tradeoff']:
                             is_critical_non_tradeoff = True
                             break
                 if is_critical_non_tradeoff:
                     return ObjectiveStatus.CONFLICT # Invalid due to critical non-tradeoff pathology
             ...
             if severity == 'high':
                  ...
                  for p in pathology['pathologies']:
                      if p.get('severity') == 'high':
                            ...
                            if p_type not in [ConflictType.TRADEOFF, 'tradeoff', 'objective_conflict', 'unacceptable_tradeoff']:
                                is_high_non_tradeoff = True # Found a non-tradeoff high pathology
                                break
                  
                  if is_high_non_tradeoff: # If any high pathology is NOT just a tradeoff
                       return ObjectiveStatus.CONFLICT # High severity, non-tradeoff issue = invalid

        # If any *other* conflicts (e.g. from hierarchy), check severity
        if conflicts:
            # Check for explicit critical severity or critical enum
            if any(c.get('severity') in ['critical', ConflictSeverity.CRITICAL, getattr(ConflictSeverity.CRITICAL, 'value', 'critical')] for c in conflicts):
                 return ObjectiveStatus.CONFLICT # Invalid due to critical conflict
            
            # If high severity, check if it's just a tradeoff
            is_high_non_tradeoff_conflict = False
            for c in conflicts:
                 if c.get('severity') in ['high', ConflictSeverity.HIGH, getattr(ConflictSeverity.HIGH, 'value', 'high')]:
                    c_type = c.get('conflict_type', c.get('type'))
                    # Handle enums
                    if hasattr(c_type, 'value'): c_type = c_type.value
                    # Check if it's NOT one of the acceptable conflict types
                    if c_type not in [ConflictType.TRADEOFF, 'tradeoff', 'objective_conflict', 'unacceptable_tradeoff']:
                        is_high_non_tradeoff_conflict = True # Found a non-tradeoff high conflict
                        break

            if is_high_non_tradeoff_conflict: # If any high conflict is NOT just a tradeoff
                 return ObjectiveStatus.CONFLICT # High severity, non-tradeoff issue = invalid
        ...
        analyzed_active_objectives = [
            a for a in objective_analyses
            if a.objective_name in self.active_objectives
        ]
        if analyzed_active_objectives and all(a.status == ObjectiveStatus.ALIGNED for a in analyzed_active_objectives):
    ...
    def _generate_validation_reasoning(self,
                                       ...
                                       objective_analyses: List[ObjectiveAnalysis],
                                       overall_status: ObjectiveStatus,
                                       historical_prediction: Optional[Dict[str, Any]] = None) -> str:
        """Generate reasoning for validation decision"""
        base_reasoning = ""

        if overall_status == ObjectiveStatus.ALIGNED:
            base_reasoning = "Proposal aligns with all design objectives and constraints."
        elif overall_status == ObjectiveStatus.VIOLATION:
            violations = [a for a in objective_analyses if a.status == ObjectiveStatus.VIOLATION]
            ...
            if pathology['has_pathology'] and pathology.get('severity') in ['critical', 'high']:
                 # Get descriptions
                 pathology_descs = [p.get('description', 'pathology') for p in pathology['pathologies'] if p.get('severity') in ['critical', 'high']]
                 base_reasoning = f"Proposal has objective pathologies: {pathology.get('severity', 'high')} severity. {'; '.join(pathology_descs[:2])}"
            else:
                 conflict_reasons = [c.get('description', 'Conflict detected') for c in introspection.get('conflicts', [])]
            ...
            base_reasoning = "Proposal shows signs of goal drift from design specification"
            ...
            if pathology['has_pathology']:
                ...
                base_reasoning = "Proposal is acceptable but may not fully meet all targets."
        else:
    ...
    def _calculate_validation_confidence(self,
                                         ...
                                         historical_prediction: Optional[Dict[str, Any]] = None) -> float:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            approved_validations = tracker_stats.get('statistics', {}).get('outcome_approved', 0)
            # Also count actual successes if available
            actual_successes = tracker_stats.get('statistics', {}).get('actual_outcome_success', 0)
