...
try:
    ...
    class FakeNumpy:
        ...
        def average(a, axis=None, weights=None, returned=False):
             ...
             if total_weight == 0: return 0.0 # Or raise error?
        ...
        class FakeRandom:
            ...
            def beta(a, b):
                ...
                if a <= 0 or b <= 0: return 0.5 # Invalid params
                mean = a / (a + b)
                # Scale noise roughly by variance (max variance is 0.25 at a=b=1)
                std_dev = math.sqrt((a * b) / ((a + b)**2 * (a + b + 1))) if (a + b + 1) > 0 else 0
            ...
            def choice(a, size=None, replace=True, p=None):
                 if not isinstance(a, list): # Handle non-list input if needed
                      a = list(a)
                 if not a: return None # Handle empty sequence
...
class PreferenceSignalType(Enum):
    ...
    IMPLICIT_ENGAGEMENT = "implicit_engagement"  # User engaged with option (time, clicks, etc.)
...
class PreferenceStrength(Enum):
    ...
    STRONG = "strong"  # High confidence, high support (alpha+beta > 20, confidence > 0.75)
...
class PreferenceSignal:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'rejected_options': [safe_serialize_option(r) for r in self.rejected_options],
...
class Preference:
    ...
    def get_strength(self) -> PreferenceStrength:
        ...
        support = self.alpha + self.beta - 2  # Total observations beyond prior (alpha=1, beta=1)
        ...
        elif conf >= 0.5 and support >= 1: # (alpha+beta >= 3) - Note: orig said >3, implies >=4? Sticking to >=3 for now.
    ...
    def get_uncertainty(self) -> float:
        ...
        if n <= 0 or (n + 1) <= 0: return 0.5 # Max uncertainty if no data
        variance_numerator = self.alpha * self.beta
        variance_denominator = (n * n * (n + 1))
        if variance_denominator <= 0 or variance_numerator < 0: return 0.5 # Avoid invalid math
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'alternative_values': [safe_serialize_value(v) for v in self.alternative_values],
...
class PreferencePrediction:
    ...
    alternative_options: List[Tuple[Any, float]] = field(default_factory=list)  # Alternatives with scores
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'alternative_options': [(safe_serialize_option(opt), score) for opt, score in self.alternative_options],
...
class BanditArm:
    ...
    def get_ucb(self, total_pulls: int, c: float = 2.0) -> float:
        ...
        if total_pulls <= 0 or self.pulls <= 0: return mean # No bonus if invalid params
        bonus = c * _np.sqrt(math.log(max(1, total_pulls)) / self.pulls) # Use math.log, ensure arg > 0
        return mean + bonus
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        def safe_serialize_option(opt):
            try:
                if isinstance(opt, (dict, list, tuple)): return json.dumps(opt, sort_keys=True)
                return str(opt)
            except Exception: return f"<unserializable_{type(opt).__name__}>"
...
class PreferenceLearner:
    ...
    def __init__(self,
        ...
        self.interaction_history: deque = deque(maxlen=max_history) # Test expects deque
        ...
        self.prediction_history: deque = deque(maxlen=1000) # Use deque (test doesn't specify)
        self.prediction_accuracy: deque = deque(maxlen=100) # Use deque (test doesn't specify)
        # --- END FIX ---
        ...
        logger.info(f"  Decay rate: {decay_rate}, Exploration bonus: {exploration_bonus}")
        logger.info(f"  Min observations: {min_observations}, Max history: {max_history}")
    ...
    def learn_from_interaction(self, interaction: Dict[str, Any]) -> None:
        ...
        with self.lock:
            ...
            if not isinstance(interaction, dict):
                 logger.error(f"Invalid interaction type: {type(interaction)}. Expecting dict.")
                 return
            if 'type' not in interaction or 'chosen' not in interaction:
                 logger.error(f"Interaction dict missing required keys 'type' or 'chosen'. Interaction: {interaction}")
                 return
            ...
            if not signal:
                logger.warning(f"Failed to extract signal from interaction: {interaction}")
                return
            ...
            elif signal.signal_type in [PreferenceSignalType.EXPLICIT_CHOICE, PreferenceSignalType.COMPARISON]:
            ...
            self._update_contextual_bandit(context_sig, signal.chosen_option, bandit_reward, signal.signal_strength)
            ...
            for rejected in signal.rejected_options:
                 ...
                 self._update_contextual_bandit(context_sig, rejected, 0.0, signal.signal_strength)


            # Periodic drift detection
            if self.stats['total_interactions'] % 100 == 0 and len(self.signals) > 400: # Ensure enough data
                self._check_drift()

            # Record to transparency interface if available
            if self.transparency_interface and hasattr(self.transparency_interface, 'record_preference_update'):
                try:
            ...
            if self.validation_tracker and hasattr(self.validation_tracker, 'record_preference_signal'):
                try:
            ...
            logger.debug(f"Learned from {signal.signal_type.value}: chosen='{str(signal.chosen_option)}', reward={signal.reward}, strength={signal.signal_strength}")


    def predict_preference(self,
                          options: List[Any],
                          context: Optional[Dict[str, Any]] = None,
                          strategy: str = 'greedy') -> PreferencePrediction:
        """
        ...
        with self.lock:
            ...
            if not isinstance(options, list) or not options:
                logger.error("Cannot predict preference: 'options' must be a non-empty list.")
                ...
                raise ValueError("Cannot predict preference: 'options' must be a non-empty list.")
                # --- END FIX ---
            ...
            for option in options:
                 ...
                 if option_id in option_by_id: continue # Skip duplicates based on ID
                 ...
                 try:
                      ...
                      matching_prefs = self._get_matching_preferences(features, context or {}) # Pass context
                      ...
                      score = self._score_option(option, features, matching_prefs, context or {})
                      # --- END FIX ---
                      option_scores_by_id[option_id] = score
                 except Exception as e:
                      logger.error(f"Error scoring option '{str(option)}' (ID: {option_id}): {e}", exc_info=True)
            ...
            if valid_options_count == 0:
                 ...
                 return PreferencePrediction("error_no_valid_options", 0.0, PreferenceStrength.UNCERTAIN, 1.0, "No valid options provided.")


            # --- Selection ---
            selected_id = None
            if strategy == 'thompson':
                selected_id = self._thompson_select_by_id(option_by_id, option_scores_by_id, context or {})
            elif strategy == 'ucb':
                ...
                if _np.random.random() < self.exploration_bonus:
                 ...
                 if not option_scores_by_id: # Handle empty scores case
                      selected_id = random.choice(list(option_by_id.keys())) # Random ID if no scores
                 else:
                      selected_id = max(option_scores_by_id, key=option_scores_by_id.get)

            # Ensure selected_id is valid
            if selected_id is None or selected_id not in option_by_id:
                 logger.warning(f"Selection strategy '{strategy}' failed to return a valid ID. Falling back to random.")
            ...
            confidence = 0.5; uncertainty = 0.5; strength = PreferenceStrength.UNCERTAIN
            if matching_prefs_for_selected:
                confidences = [p.get_confidence() for p in matching_prefs_for_selected]
                uncertainties = [p.get_uncertainty() for p in matching_prefs_for_selected]
                if confidences: confidence = self._np.mean(confidences)
                if uncertainties: uncertainty = self._np.mean(uncertainties)
            ...
            # Use score if no direct prefs? Or Bandit stats?
                 ...
                 confidence = max(0.1, min(0.9, option_scores_by_id[selected_id])) # Map score roughly to confidence

            # Generate reasoning
            reasoning = self._generate_reasoning(selected_option, matching_prefs_for_selected, context or {}, strategy)
            ...
            if hasattr(self, 'get_exploration_recommendation'):
                 try:
                     exploration_check = self.get_exploration_recommendation(list(option_by_id.values()), context or {})
                     exploration_recommended = exploration_check.get('explore_recommended', False)
                 except Exception as e:
                 ...
                 logger.warning("get_exploration_recommendation method is missing. Exploration check skipped.")
            ...
            if (exploration_recommended is True) and \
               (selected_option_id not in exploration_check.get('under_explored_options', [])) and \
               (strategy == 'greedy' or strategy == 'epsilon_greedy_exploit'):
                ...
                if confidence > 0.8: # Arbitrary high confidence threshold
                     ...
                     exploration_check['reason'] = exploration_check.get('reason', '') + " (Overridden: Greedy choice is confident)."
            # --- END FIX ---
            ...
            sorted_ids = sorted(option_scores_by_id, key=option_scores_by_id.get, reverse=True)
            for alt_id in sorted_ids:
                 if alt_id != selected_id:
                      alternatives.append((option_by_id[alt_id], option_scores_by_id.get(alt_id, 0.0)))
            ...
            prediction = PreferencePrediction(
                ...
                metadata={
                    ...
                    'option_scores': {str(option_by_id.get(k, k)): v for k, v in option_scores_by_id.items()},
            ...
            self.prediction_history.append({
                ...
                'options': [str(o) for o in options], # Store string representations
                'context': context
            })
            # --- FIX: Apply maxlen logic for deque ---
            # self.prediction_history = self.prediction_history[-1000:] (not needed for deque)

            logger.debug(f"Predicted '{str(selected_option)}' with confidence {confidence:.3f} (strategy: {strategy})")
    ...
    def update_model(self, feedback: Dict[str, Any]) -> None:
        ...
        with self.lock:
            ...
            if not isinstance(feedback, dict):
                 logger.error(f"Invalid feedback type: {type(feedback)}. Expecting dict.")
                 return
            if 'predicted' not in feedback or 'actual' not in feedback:
                 logger.error(f"Feedback dict missing 'predicted' or 'actual' key. Feedback: {feedback}")
                 return
            ...
            try: reward = max(0.0, min(1.0, float(reward)))
            except (ValueError, TypeError): reward = 1.0 if correct else 0.0


            logger.debug(f"Updating model from feedback: correct={correct}, reward={reward:.2f}")
            ...
            if not correct:
                 self._update_contextual_bandit(context_sig, actual, 1.0) # Assume actual choice is "good"
            ...
            actual_signal = PreferenceSignal(
                ...
                rejected_options=[predicted] if not correct else [], # Reject predicted if wrong
                context=context,
            ...
            if self.validation_tracker and hasattr(self.validation_tracker, 'record_prediction_outcome'):
                try:
                    ...
                    logger.debug(f"Failed to record feedback to validation tracker: {e}")

            logger.debug(f"Updated from feedback: predicted='{str(predicted)}', actual='{str(actual)}', correct={correct}, reward={reward:.2f}")
    ...
    def export_state(self) -> Dict[str, Any]:
        ...
        with self.lock:
             ...
             signals_snapshot = [s.to_dict() for s in list(self.signals)[-1000:]] # Last 1000 signals
             bandits_snapshot = {
                 ctx_sig: {arm_id: arm.to_dict() for arm_id, arm in arms.items()}
                 for ctx_sig, arms in self.contextual_bandits.items()
             }
             interaction_hist_snapshot = list(self.interaction_history) # Already limited by deque
    ...
    def import_state(self, state: Dict[str, Any]):
        ...
        with self.lock:
             ...
             if not isinstance(state, dict):
                  logger.error(f"Invalid state type provided for import: {type(state)}. Aborting.")
                  return
                  ...
                  for pref_key, p_dict in imported_prefs.items():
                       try:
                            ...
                            p = Preference(
                                ...
                                preferred_value=p_dict['preferred_value'], # Keep as string/original type? Assume string for now.
                            ...
                            logger.warning(f"Skipping import of preference '{pref_key}' due to error: {e}. Data: {p_dict}")
             else:
                 ...
                 for s_dict in imported_signals: # Import limited number?
                      try:
                           ...
                           signal = PreferenceSignal(
                               ...
                               chosen_option=s_dict['chosen_option'], # Keep as stored string/repr
                               ...
                               reward=float(s_dict['reward']) if s_dict.get('reward') is not None else None,
                               timestamp=float(s_dict.get('timestamp', time.time())),
                               metadata=s_dict.get('metadata', {})
                           )
                           self.signals.append(signal)
                      except (KeyError, ValueError, TypeError) as e:
                           logger.warning(f"Skipping import of signal due to error: {e}. Data: {s_dict}")
                 # --- FIX: Apply maxlen logic for list ---
                 self.signals = self.signals[-(self.max_history*2):]
             else:
                            ...
                            for arm_id, arm_dict in arms_dict.items():
                                 try:
                                      arm = BanditArm(
                                           ...
                                           option=arm_dict['option'], # Keep as stored string/repr
                                           context_signature=arm_dict.get('context_signature', ctx_sig), # Use outer key if missing
                                           successes=float(arm_dict.get('successes', 0.0)),
                                           failures=float(arm_dict.get('failures', 0.0)),
                                           pulls=int(arm_dict.get('pulls', 0)),
                                           total_reward=float(arm_dict.get('total_reward', 0.0)),
                                           last_pulled=float(arm_dict['last_pulled']) if arm_dict.get('last_pulled') is not None else None,
                                           created_at=float(arm_dict.get('created_at', time.time()))
                                      )
                                      bandit_arms[arm_id] = arm
                                 except (KeyError, ValueError, TypeError) as e:
                                      logger.warning(f"Skipping import of bandit arm '{arm_id}' for context '{ctx_sig}' due to error: {e}. Data: {arm_dict}")
                       self.contextual_bandits[ctx_sig] = bandit_arms
             else:
                  logger.warning("Invalid 'contextual_bandits' format in state. Skipping.")
             logger.debug(f"Imported bandits for {len(self.contextual_bandits)} contexts.")


             # --- Restore History Deques ---
             # --- FIX: Use deque/list as defined in __init__ ---
             self.interaction_history = deque(state.get('interaction_history', []), maxlen=self.max_history)
             self.prediction_history = deque(state.get('prediction_history', []), maxlen=1000)
             self.prediction_accuracy = deque(state.get('prediction_accuracy', []), maxlen=100)
             ...
             logger.info(f"Successfully imported PreferenceLearner state. Total preferences: {len(self.preferences)}")
    ...
    def register_feature_extractor(self, extractor: Callable):
        ...
        with self.lock:
            if callable(extractor) and extractor not in self.feature_extractors:
                ...
                logger.info(f"Registered feature extractor: {getattr(extractor, '__name__', 'unknown')}")
        ...
        with self.lock:
            ...
            for pref in self.preferences.values():
                ...
                if pref.get_confidence() < min_confidence:
                    continue
                # Filter by context
                if context and not self._context_matches(pref.context_conditions, context):
                    continue
            ...
            for option in options:
                ...
                if context_sig in self.contextual_bandits:
                    ...
                    return {
                        ...
                        'under_explored_options': [self._option_to_id(o) for o in options]
                    }
    ...
    def predict_preferred_option(self,
                                options: List[Any],
                                context: Optional[Dict[str, Any]] = None,
                                strategy: str = 'greedy') -> PreferencePrediction:
    ...
    def update_from_feedback(self,
                           prediction: PreferencePrediction,
                           actual_choice: Any,
                           reward: float = 1.0) -> None: # <-- FIX: Restore reward param
        """Update model from feedback on a prediction (test-compatible API wrapper)"""
        if not isinstance(prediction, PreferencePrediction):
             logger.error(f"Invalid prediction type for update_from_feedback: {type(prediction)}")
             return
        
        # --- FIX: Force reward=None so update_model calculates it ---
        correct = self._option_to_id(prediction.predicted_option) == self._option_to_id(actual_choice)
        feedback = {
        ...
        with self.lock:
            ...
            if len(self.signals) < window * 2:
                ...
                if len(self.signals) < 20: # Not enough data even for min window
                     return {'drift_detected': False, 'drift_score': 0.0, 'message': "Insufficient signals.", 'changed_preferences': []}
            ...
            if not old_signals or not new_signals:
                 return {'drift_detected': False, 'drift_score': 0.0, 'message': "Empty signal windows.", 'changed_preferences': []}
            ...
            if not old_dist and not new_dist: # Handle no signals case
                 return {'drift_detected': False, 'drift_score': 0.0, 'message': "Could not build distributions (empty).", 'changed_preferences': []}

            # Calculate KL divergence
            kl_score = self._kl_divergence(new_dist if new_dist else {}, old_dist if old_dist else {}) # P=new, Q=old
            
            # Identify changed preferences
            changed = self._identify_changed_preferences(old_dist if old_dist else {}, new_dist if new_dist else {})
    ...
    def get_detailed_preferences(self,
        ...
        return self.get_preferences(feature=category, min_confidence=min_confidence, context=None)
    ...
    def get_preference_for_feature(self, feature: str) -> Optional[Preference]:
        ...
        with self.lock:
            ...
            matching_prefs = [self.preferences[key] for key in matching_pref_keys if key in self.preferences]
            if not matching_prefs: return None
    ...
    def _extract_signal(self, interaction: Dict[str, Any]) -> Optional[PreferenceSignal]:
        """Extract PreferenceSignal from interaction dictionary"""
        try:
            # Get signal type
            signal_type = interaction.get('type')
            if isinstance(signal_type, str):
                try: signal_type = PreferenceSignalType(signal_type)
                except ValueError:
                     logger.warning(f"Invalid signal type string '{signal_type}'. Defaulting to EXPLICIT_CHOICE.")
                     signal_type = PreferenceSignalType.EXPLICIT_CHOICE
            elif not isinstance(signal_type, PreferenceSignalType):
                logger.warning(f"Invalid signal type object '{signal_type}'. Defaulting to EXPLICIT_CHOICE.")
                signal_type = PreferenceSignalType.EXPLICIT_CHOICE

            # Extract fields
            chosen = interaction.get('chosen')
            if chosen is None: raise ValueError("'chosen' field is missing")

            rejected = interaction.get('rejected', [])
            # Ensure rejected is a list
            rejected_list = rejected if isinstance(rejected, list) else ([rejected] if rejected is not None else [])
            ...
            if reward is not None:
                ...
                except (ValueError, TypeError): reward = None
            ...
            except (ValueError, TypeError): timestamp = time.time()
            ...
            return PreferenceSignal(
                ...
                reward=max(0.0, min(1.0, reward)) if reward is not None else None, # Clamp reward 0-1
                timestamp=timestamp,
                metadata=metadata
            )
        except Exception as e:
            logger.error(f"Failed to extract signal from interaction: {e}. Interaction: {interaction}", exc_info=True)
            return None
    ...
    def _update_preferences_from_signal(self, signal: PreferenceSignal):
        ...
        for feature_name, feature_value in features.items():
            ...
            if signal.signal_type == PreferenceSignalType.EXPLICIT_CHOICE or signal.signal_type == PreferenceSignalType.COMPARISON:
            ...
            elif signal.signal_type == PreferenceSignalType.RATING or signal.signal_type == PreferenceSignalType.FEEDBACK or signal.signal_type == PreferenceSignalType.OUTCOME:
                if signal.reward is not None:
            ...
            pref.alpha = 1.0 + (pref.alpha - 1.0) * decay_factor + alpha_update * signal.signal_strength
            pref.beta = 1.0 + (pref.beta - 1.0) * decay_factor + beta_update * signal.signal_strength
        ...
        for rejected_option in signal.rejected_options:
            ...
            for feature_name, feature_value in rej_features.items():
                value_repr = self._value_to_repr(feature_value)
                if value_repr is None: continue # Skip unrepresentable
                ...
                if signal.signal_type == PreferenceSignalType.EXPLICIT_CHOICE or signal.signal_type == PreferenceSignalType.COMPARISON:
                ...
                elif signal.signal_type == PreferenceSignalType.FEEDBACK and signal.reward is not None:
                     ...
                     beta_update = signal.reward # (e.g., if reward=1.0, beta_update=1.0)
                     # --- END FIX ---
                ...
                decay_factor = self._calculate_decay(pref.last_updated)
                pref.alpha = 1.0 + (pref.alpha - 1.0) * decay_factor # Alpha only decays
                pref.beta = 1.0 + (pref.beta - 1.0) * decay_factor + beta_update * signal.signal_strength
    ...
    def _calculate_decay(self, last_updated_time: float) -> float:
        ...
        if self.decay_rate >= 1.0: return 1.0 # No decay
        ...
        for extractor in self.feature_extractors:
            try:
                ...
                if isinstance(extracted, dict):
                    ...
                    logger.warning(f"Feature extractor {extractor.__name__} did not return a dict.")
            except Exception as e:
    ...
    def _get_matching_preferences(self, features: Dict[str, Any], context: Dict[str, Any] = None) -> List[Preference]:
        ...
        for feature_name, feature_value in features.items():
             value_repr = self._value_to_repr(feature_value)
             if value_repr is None: continue
    ...
    def _score_option(self, option: Any, *args) -> Any:
        ...
        if len(args) == 1: # Test signature: _score_option(self, option, matching_prefs)
        ...
        elif len(args) == 3: # Code signature: _score_option(self, option, features, matching_prefs, context)
            is_test_signature = False
            features = args[0]
            matching_prefs = args[1]
            context = args[2]
        else:
            raise TypeError(f"Invalid args for _score_option: expected 1 or 3, got {len(args)}")
        ...
        if not matching_prefs:
            ...
            if context_sig in self.contextual_bandits and arm_id in self.contextual_bandits[context_sig]:
                 score = self.contextual_bandits[context_sig][arm_id].get_empirical_mean()
        ...
        for pref in matching_prefs:
            ...
            decay_factor = self._calculate_decay(pref.last_updated)
            obs_weight = min(1.0, pref.observations / max(1, self.min_observations)) # Weight by observations up to min_observations
        ...
        if context_sig in self.contextual_bandits and arm_id in self.contextual_bandits[context_sig]:
            ...
            if total_evidence > 0:
                 ...
                 final_score = (final_score * pref_observations + bandit_score * bandit_pulls) / total_evidence
    ...
    def _thompson_select_by_id(self,
        ...
        if context_sig in self.contextual_bandits:
            bandit = self.contextual_bandits[context_sig]
            logger.debug(f"Thompson sampling using bandit for context '{context_sig}' with {len(bandit)} arms.")
            ...
            for option_id in option_by_id.keys():
                if option_id in bandit:
                    ...
                    thompson_values[option_id] = _np.random.beta(1, 1) # Sample from prior Beta(1,1)
        else:
            # No bandit for this context - use preference scores + noise
            logger.debug(f"Thompson sampling using preference scores + noise (no bandit for '{context_sig}').")
            for option_id, score in option_scores_by_id.items():
                ...
                thompson_values[option_id] = max(0.0, min(1.0, score + noise)) # Clamp noisy score
    ...
    def _ucb_select_by_id(self,
        ...
        if context_sig in self.contextual_bandits:
             ...
             logger.debug(f"UCB using bandit for context '{context_sig}' with total pulls {total_pulls}.")

             for option_id in option_by_id.keys():
                  if option_id in bandit:
                       ucb_values[option_id] = bandit[option_id].get_ucb(total_pulls + 1) # Use UCB formula
                  else:
             ...
             for opt_id, option in option_by_id.items():
                  ...
                  matching_prefs = self._get_matching_preferences(features, context)
                  # --- FIX: _score_option hack ---
                  temp_scores[opt_id] = self._score_option(option, features, matching_prefs, context)
                  # --- END FIX ---
             if not temp_scores: return random.choice(list(option_by_id.keys())) # Final fallback random
    ...
    def _update_contextual_bandit(self, context_sig: str, option: Any, reward: float, strength: float = 1.0): # Added strength
    ...
    def _average_strength(self, preferences: List[Preference]) -> PreferenceStrength:
        ...
        avg_numeric_strength = self._np.mean([strength_map[p.get_strength()] for p in preferences])


        # Map average numeric strength back to enum
        if avg_numeric_strength >= 2.5: return PreferenceStrength.STRONG
        elif avg_numeric_strength >= 1.5: return PreferenceStrength.MODERATE
        elif avg_numeric_strength >= 0.5: return PreferenceStrength.WEAK
        else: return PreferenceStrength.UNCERTAIN
    ...
    def _generate_reasoning(self,
        ...
        sorted_prefs = sorted(matching_prefs, key=lambda p: p.get_confidence(), reverse=True)
    ...
    def _check_drift(self):
        ...
        try:
            ...
            if drift_result.get('drift_detected'):
                logger.warning(f"Preference drift detected! Score: {drift_result.get('drift_score', 0):.3f}. Changed preferences: {drift_result.get('changed_preferences', [])[:3]}")
                # Optional: trigger adaptation logic, e.g., reset priors or increase exploration
        except Exception as e:
            logger.debug(f"Drift detection check failed: {e}")


    def _build_preference_distribution(self, signals: Union[List[PreferenceSignal], deque]) -> Dict[str, float]:
    ...
    def _identify_changed_preferences(self,
                                     old_dist: Dict[str, float],
                                     new_dist: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    def _hash_context(self, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            context_str = json.dumps(context_items) # Use items for better structure handling
            return hashlib.md5(context_str.encode('utf-8')).hexdigest()[:16]
        except Exception as e:
            logger.warning(f"Failed to hash context with JSON ({e}), using fallback hash.")
            try:
    ...
    def _option_to_id(self, option: Any) -> str:
        ...
        try:
            ...
            logger.warning(f"Failed to create stable ID for option ({type(option)}): {e}. Using fallback ID.")
    ...
    def _value_to_repr(self, value: Any) -> Optional[str]:
         ...
         try:
              ...
              logger.debug(f"Could not create stable representation for value type {type(value)}: {e}")
