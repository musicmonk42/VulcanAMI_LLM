...
class Evaluation:
    ...
    def get_high_risks(self) -> List[Risk]:
        ...
        return [r for r in self.risks if r.severity in [RiskSeverity.CRITICAL, RiskSeverity.HIGH]]
...
class ComparisonResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        serializable_matrix = {f"{k[0]}_{k[1]}": v for k, v in self.comparison_matrix.items()}
        return {
...
class InternalCritic:
    ...
    def __init__(self,
        ...
        if total_weight > 0 and abs(total_weight - 1.0) > 1e-6: # Normalize if not already sum to 1
        ...
        # Learning parameters
        self.critique_effectiveness: Dict[str, float] = {}  # critique_type -> effectiveness
        ...
        logger.info(f"  Strict mode: {strict_mode}, Adaptive weights: {self.adaptive_weights}")
        logger.info(f"  Perspective weights: { {p.value: w for p, w in self.perspective_weights.items()} }")
    ...
    def evaluate_proposal(self,
        ...
        with self.lock:
            ...
            if not isinstance(proposal, dict):
                 logger.error(f"Invalid proposal type: {type(proposal)}. Cannot evaluate.")
                 ...
                 return Evaluation(
                      ...
                      perspective_scores=[], critiques=[], risks=[],
                      strengths=[], weaknesses=["Invalid proposal type"], improvements=[]
                 )
            ...
            for perspective, weight in self.perspective_weights.items():
                ...
                if eval_func_list:
                    ...
                    try:
                        ...
                        logger.error(f"Error during {perspective.value} evaluation: {e}", exc_info=True)
                        score = 0.0 # Penalize on error
                        critiques = [Critique(CritiqueLevel.CRITICAL, perspective, "evaluation_error", f"Evaluation failed with exception: {e}")]
                else:
                    logger.warning(f"No evaluation function found for perspective: {perspective.value}")
            ...
            if total_w > 0:
                 overall_score = sum(ps.score * ps.weight for ps in perspective_scores) / total_w
            else:
            ...
            if self.strict_mode and all_critiques:
                 critical_penalty = 0.2 * len([c for c in all_critiques if c.level == CritiqueLevel.CRITICAL])
                 major_penalty = 0.1 * len([c for c in all_critiques if c.level == CritiqueLevel.MAJOR])
                 overall_score = max(0.0, overall_score - critical_penalty - major_penalty)
            ...
            improvements = self._generate_improvements(all_critiques, all_risks, proposal)
            ...
            evaluation = Evaluation(
                ...
                metadata={
                    'context_keys': list(context.keys()), # Store keys instead of full context
            ...
            logger.info(f"Evaluated proposal {proposal_id}: score={overall_score:.3f}, recommendation={recommendation}")
    ...
    def generate_critique(self,
                         aspect: str,
                         data: Dict[str, Any],
                         perspective: Optional[EvaluationPerspective] = None) -> List[Critique]:
        """
        ...
        with self.lock:
            ...
            pseudo_proposal = {"id": f"critique_{aspect}_{time.time_ns()}", aspect: data}

            perspectives_to_check = [perspective] if perspective else list(self.perspective_weights.keys())
            ...
            for persp in perspectives_to_check:
                ...
                if eval_func_list:
                    ...
                    try:
                        ...
                        logger.error(f"Error during {persp.value} critique generation: {e}", exc_info=True)
                # --- END FIX ---
                        
                # Filter critiques to ensure they relate to the specified aspect
                # (Some generic checks might run even on the minimal proposal)
                related_critiques = [c for c in persp_critiques if c.aspect == aspect or aspect in c.description]
    ...
    def suggest_improvements(self,
                           proposal: Dict[str, Any],
                           evaluation: Optional[Evaluation] = None) -> List[str]:
        """
        ...
        with self.lock:
            if evaluation is None:
                ...
                if not isinstance(proposal, dict):
                     logger.error(f"Invalid proposal type for suggest_improvements: {type(proposal)}")
            ...
            if not isinstance(evaluation, Evaluation):
                 logger.error(f"Invalid evaluation object provided to suggest_improvements: {type(evaluation)}")
    ...
    def identify_risks(self,
                      proposal: Dict[str, Any],
                      context: Optional[Dict[str, Any]] = None) -> List[Risk]:
        """
        ...
        with self.lock:
             # Ensure proposal is a dict
            if not isinstance(proposal, dict):
                 logger.error(f"Invalid proposal type for identify_risks: {type(proposal)}")
                 return [Risk(category=RiskCategory.OPERATIONAL, severity=RiskSeverity.HIGH, description="Invalid proposal type provided.", likelihood=1.0, impact=0.5)]
    ...
    def _identify_safety_risks(self, proposal: Dict[str, Any], context: Dict[str, Any]) -> List[Risk]:
        """Placeholder: Identify safety risks"""
        risks = []
        if proposal.get("causes_physical_harm"):
            risks.append(Risk(RiskCategory.SAFETY, RiskSeverity.CRITICAL, "Proposal causes physical harm", 1.0, 1.0))
        if proposal.get("causes_psychological_harm"):
            risks.append(Risk(RiskCategory.SAFETY, RiskSeverity.HIGH, "Proposal causes psychological harm", 0.8, 0.8))
        return risks

    def _identify_security_risks(self, proposal: Dict[str, Any], context: Dict[str, Any]) -> List[Risk]:
        """Placeholder: Identify security risks"""
        risks = []
        if proposal.get("requires_network_access") and not proposal.get("has_security_review"):
            risks.append(Risk(RiskCategory.SECURITY, RiskSeverity.MEDIUM, "Network access required without security review", 0.6, 0.5))
        return risks

    def _identify_performance_risks(self, proposal: Dict[str, Any], context: Dict[str, Any]) -> List[Risk]:
        """Placeholder: Identify performance risks"""
        risks = []
        if proposal.get("complexity") == "exponential":
            risks.append(Risk(RiskCategory.PERFORMANCE, RiskSeverity.MEDIUM, "Exponential complexity may cause performance degradation", 0.7, 0.6))
        return risks

    def _identify_resource_risks(self, proposal: Dict[str, Any], context: Dict[str, Any]) -> List[Risk]:
        """Placeholder: Identify resource risks"""
        risks = []
        cost = proposal.get('estimated_cost', 0)
        budget = context.get('budget', 100)
        if cost > budget:
            risks.append(Risk(RiskCategory.RESOURCE, RiskSeverity.HIGH, f"Estimated cost {cost} exceeds budget {budget}", 0.9, 0.7))
        return risks

    def _identify_ethical_risks(self, proposal: Dict[str, Any], context: Dict[str, Any]) -> List[Risk]:
        """Placeholder: Identify ethical risks (integrates with monitor)"""
        risks = []
        if self.ethical_boundary_monitor:
            try:
                violations = self.ethical_boundary_monitor.detect_boundary_violations(proposal, context)
                for v in violations:
                    severity_map = {
                    ...
                    sev_key = v.severity.value if hasattr(v.severity, 'value') else str(v.severity)
                    ...
                    risks.append(Risk(
                        ...
                        likelihood=0.8,
                        impact=0.8,
                        metadata={'boundary_violated': getattr(v, 'boundary_violated', 'unknown')}
                    ))
            except Exception as e:
                logger.debug(f"Ethical boundary monitor check failed: {e}")
        return risks

    def _identify_operational_risks(self, proposal: Dict[str, Any], context: Dict[str, Any]) -> List[Risk]:
        """Placeholder: Identify operational risks"""
        risks = []
        if not proposal.get("has_rollback_plan"):
            risks.append(Risk(RiskCategory.OPERATIONAL, RiskSeverity.MEDIUM, "No rollback strategy defined", 0.5, 0.6))
        return risks
    # --- END FIX ---
    ...
    def compare_alternatives(self,
                           ...
                           context: Optional[Dict[str, Any]] = None) -> ComparisonResult:
        """
        ...
        with self.lock:
            ...
            if len(valid_proposals) != len(proposals):
                 logger.warning(f"Some items in proposals list were not dicts ({len(proposals) - len(valid_proposals)} ignored).")
            if not valid_proposals:
                return ComparisonResult(best_proposal_id="", ranking=[], comparison_matrix={}, rationale="No valid proposals provided", trade_offs=[])
    ...
    def learn_from_outcome(self,
        ...
        with self.lock:
            ...
            if not isinstance(outcome, dict):
                 logger.warning(f"Invalid outcome type for learn_from_outcome ({type(outcome)}). Cannot learn.")
                 return

            if proposal_id not in self.evaluations_by_id:
                logger.warning(f"No evaluation found for proposal {proposal_id} to learn from outcome.")
                return
            ...
            elif isinstance(outcome_success_val, str) and outcome_success_val.lower() == 'success':
                 success = True
            ...
            for critique in evaluation.critiques:
                 ...
                 if critique.perspective and critique.perspective.value and critique.aspect:
                      ...
                      current_effectiveness = self.critique_effectiveness.get(pattern_key, 0.5) # Default neutral

                      if success:
                          # If successful despite critique, maybe critique was too strict or irrelevant
                          # Reduce effectiveness slightly, bounded at 0.1
                          self.critique_effectiveness[pattern_key] = max(0.1, current_effectiveness * 0.95)
                      else:
                          # If failed and critique existed, critique might have been relevant
                          # Increase effectiveness slightly, bounded at 0.9
                          self.critique_effectiveness[pattern_key] = min(0.9, current_effectiveness * 1.05)
                      ...
                      logger.debug(f"Skipping critique effectiveness update due to invalid critique data: {critique}")
    ...
    def get_evaluation_history(self,
                              ...
                              min_score: Optional[float] = None) -> List[Evaluation]:
        """
        ...
        with self.lock:
            ...
            if min_score is not None:
                evaluations_snapshot = [e for e in evaluations_snapshot if e.overall_score >= min_score]
        ...
        with self.lock:
            ...
            for eval_item in eval_history_snapshot:
                for critique in eval_item.critiques:
                    ...
                    level_value = critique.level.value if hasattr(critique.level, 'value') else str(critique.level)
            ...
            return {
                ...
                'perspective_weights': {k.value: v for k, v in self.perspective_weights.items()},
                'strict_mode': self.strict_mode,
    ...
    def export_state(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            persp_weights_snapshot = {k.value: v for k, v in self.perspective_weights.items()}
    ...
    def import_state(self, state: Dict[str, Any]):
        ...
        with self.lock:
            ...
            if not isinstance(state, dict):
                 logger.error(f"Invalid state type provided for import: {type(state)}. Aborting import.")
                 return
            ...
            if isinstance(imported_weights, dict):
                 ...
                 for k_str, v in imported_weights.items():
                      try:
                           ...
                           logger.warning(f"Skipping invalid perspective weight during import: key='{k_str}', value='{v}'")
                 if valid_weights:
                      ...
                      if total_w > 0 and abs(total_w - 1.0) > 1e-6:
                           self.perspective_weights = {k: v / total_w for k, v in self.perspective_weights.items()}
            else:
                 logger.warning(f"Invalid 'perspective_weights' format in state: {type(imported_weights)}. Skipping.")
            ...
            if isinstance(imported_effectiveness, dict):
                self.critique_effectiveness = {str(k): float(v) for k, v in imported_effectiveness.items() if isinstance(k, str) and isinstance(v, (int, float))}
            else:
                logger.warning(f"Invalid 'critique_effectiveness' format in state: {type(imported_effectiveness)}. Skipping.")
            ...
            if isinstance(imported_patterns, dict):
                 self.successful_critique_patterns = defaultdict(int, {str(k): int(v) for k, v in imported_patterns.items() if isinstance(k, str) and isinstance(v, int)})
            else:
                 logger.warning(f"Invalid 'successful_critique_patterns' format in state: {type(imported_patterns)}. Skipping.")
            ...
            if isinstance(config_state, dict):
                ...
                self.adaptive_weights = config_state.get('adaptive_weights', self.adaptive_weights)
    ...
    def reset(self) -> None:
        ...
        with self.lock:
            ...
            logger.info("InternalCritic reset - evaluation history and learned patterns cleared")
    ...
    def _evaluate_logical_consistency(self,
                                      proposal: Dict[str, Any],
                                      context: Dict[str, Any]) -> Tuple[float, List[Critique], List[Risk]]:
        ...
        if proposal.get("contradicts_known_fact"):
            score -= 0.5
            critiques.append(Critique(CritiqueLevel.MAJOR, EvaluationPerspective.LOGICAL_CONSISTENCY, "contradiction", "Proposal contradicts established facts."))
        if not proposal.get("rationale"):
             score -= 0.1
             critiques.append(Critique(CritiqueLevel.MINOR, EvaluationPerspective.LOGICAL_CONSISTENCY, "rationale", "Rationale is missing or unclear."))
    ...
    def _evaluate_feasibility(self,
        ...
        if cost > budget:
            score -= 0.4
            critiques.append(Critique(CritiqueLevel.CRITICAL, EvaluationPerspective.FEASIBILITY, "budget", f"Cost {cost} exceeds budget {budget}."))
            risks.append(Risk(RiskCategory.RESOURCE, RiskSeverity.HIGH, "Budget overrun", 0.9, 0.7))

        duration = proposal.get('estimated_duration', 1)
        max_duration = context.get('max_duration', 10) # Example max duration
        if duration > max_duration:
             score -= 0.2
             critiques.append(Critique(CritiqueLevel.MAJOR, EvaluationPerspective.FEASIBILITY, "duration", f"Duration {duration} exceeds limit {max_duration}."))
    ...
    def _evaluate_safety(self,
        ...
        if any(r.severity == RiskSeverity.CRITICAL for r in safety_risks):
             score = 0.0
             critiques.append(Critique(CritiqueLevel.CRITICAL, EvaluationPerspective.SAFETY, "critical_safety_risk", "Proposal poses critical safety risk.", impact_if_ignored=1.0))
        elif any(r.severity == RiskSeverity.HIGH for r in safety_risks):
             score -= 0.4
             critiques.append(Critique(CritiqueLevel.MAJOR, EvaluationPerspective.SAFETY, "high_safety_risk", "Proposal poses high safety risk."))

        if any(r.severity == RiskSeverity.CRITICAL for r in ethical_risks):
            score -= 0.5 # Penalize heavily for critical ethical risks too
            critiques.append(Critique(CritiqueLevel.CRITICAL, EvaluationPerspective.SAFETY, "critical_ethical_risk", "Proposal poses critical ethical risk."))
        elif any(r.severity == RiskSeverity.HIGH for r in ethical_risks):
             score -= 0.3
             critiques.append(Critique(CritiqueLevel.MAJOR, EvaluationPerspective.SAFETY, "high_ethical_risk", "Proposal poses high ethical risk."))
        ...
        if self.ethical_boundary_monitor:
            try:
                ...
                if violations:
                     ...
                     highest_severity_num = max(self._severity_to_numeric(v.severity) for v in violations) if violations else 0
                     ...
                     added_boundaries = set(r.metadata.get('boundary_violated') for r in ethical_risks)
                     for violation in violations:
                          if violation.boundary_violated not in added_boundaries:
                               # --- FIX: Need _severity_to_numeric helper ---
                               sev_val = violation.severity.value if hasattr(violation.severity, 'value') else str(violation.severity)
                               level = CritiqueLevel.CRITICAL if sev_val in ["critical", "high"] else CritiqueLevel.MAJOR
                               critiques.append(Critique(
                                    level=level,
                ...
                logger.debug(f"Failed to check ethical boundaries during safety eval: {e}")


        return (max(0.0, min(1.0, score)), critiques, risks) # Return all identified risks
    ...
    def _evaluate_alignment(self,
        ...
        if prop_obj and ctx_goal and prop_obj != ctx_goal:
             score -= 0.3
             critiques.append(Critique(CritiqueLevel.MAJOR, EvaluationPerspective.ALIGNMENT, "goal_mismatch", f"Proposal goal '{prop_obj}' differs from system goal '{ctx_goal}'."))
             risks.append(Risk(RiskCategory.OPERATIONAL, RiskSeverity.MEDIUM, "Goal misalignment", 0.6, 0.6))
    ...
    def _evaluate_efficiency(self,
        ...
        if complexity == "high" or complexity == "exponential":
             score -= 0.3
             critiques.append(Critique(CritiqueLevel.MINOR, EvaluationPerspective.EFFICIENCY, "high_complexity", f"Complexity '{complexity}' may impact performance."))
             risks.append(Risk(RiskCategory.PERFORMANCE, RiskSeverity.LOW, "Potential performance issue due to complexity", 0.4, 0.3))

        resources = proposal.get("resources_needed", [])
        if len(resources) > 5: # Arbitrary limit
             score -= 0.1
             critiques.append(Critique(CritiqueLevel.SUGGESTION, EvaluationPerspective.EFFICIENCY, "resource_count", "High number of resources required."))
    ...
    def _evaluate_completeness(self,
        ...
        if missing:
             ...
             critiques.append(Critique(CritiqueLevel.MINOR, EvaluationPerspective.COMPLETENESS, "missing_sections", f"Missing sections: {', '.join(missing)}"))


        return (max(0.0, min(1.0, score)), critiques, risks)

    def _evaluate_clarity(self,
                         proposal: Dict[str, Any],
                         context: Dict[str, Any]) -> Tuple[float, List[Critique], List[Risk]]:
        ...
        if len(desc) < 10:
             score -= 0.4
             critiques.append(Critique(CritiqueLevel.MINOR, EvaluationPerspective.CLARITY, "description_length", "Description is too short."))
        elif "synergize" in desc or "paradigm shift" in desc: # Example jargon check
             score -= 0.1
             critiques.append(Critique(CritiqueLevel.SUGGESTION, EvaluationPerspective.CLARITY, "jargon", "Description may contain jargon."))
    ...
    def _evaluate_robustness(self,
        ...
        if not proposal.get("error_handling"):
             score -= 0.2
             critiques.append(Critique(CritiqueLevel.MINOR, EvaluationPerspective.ROBUSTNESS, "error_handling", "No explicit error handling described."))
             risks.append(Risk(RiskCategory.OPERATIONAL, RiskSeverity.LOW, "Potential failure on errors", 0.5, 0.4))
        if not proposal.get("edge_cases_considered"):
            score -= 0.1
            critiques.append(Critique(CritiqueLevel.SUGGESTION, EvaluationPerspective.ROBUSTNESS, "edge_cases", "Edge cases may not be fully considered."))
    ...
    def _generate_rationale(self,
        ...
        critique_summary = f"{len(critiques)} issues identified" if critiques else "no major issues identified"
    ...
    def _identify_strengths(self,
        ...
        for ps in perspective_scores:
            if ps.score >= 0.8:
                strengths.append(f"Strong {ps.perspective.value.replace('_', ' ').lower()}")

        # Explicit positive attributes from proposal (examples)
        if proposal.get('well_tested'): strengths.append("Well-tested")
        if proposal.get('includes_documentation'): strengths.append("Includes documentation")
        if proposal.get('simple_design'): strengths.append("Simple design")
    ...
    def _identify_weaknesses(self,
                            perspective_scores: List[PerspectiveScore],
                            critiques: List[Critique]) -> List[str]:
        ...
        for ps in perspective_scores:
            if ps.score < 0.5:
                weaknesses.append(f"Weak {ps.perspective.value.replace('_', ' ').lower()} (score: {ps.score:.2f})")

        # Critical/major critiques
        critical_critiques = [c for c in critiques if c.level in [CritiqueLevel.CRITICAL, CritiqueLevel.MAJOR]]
        # Summarize top critiques concisely
        for critique in sorted(critical_critiques, key=lambda c: (c.level.value != 'critical', c.impact_if_ignored), reverse=True)[:3]:
             weaknesses.append(f"{critique.level.value.capitalize()} issue in {critique.perspective.value.replace('_',' ')}: {critique.description}")
    ...
    def _generate_improvements(self,
        ...
        for risk in sorted(risks, key=lambda r: r.risk_score, reverse=True): # Prioritize higher risks
            if risk.mitigation_strategies:
                # Add prefix to clarify context
                improvements.add(f"Mitigate risk ({risk.category.value} - {risk.severity.value}): {risk.mitigation_strategies[0]}")
        ...
        if not proposal.get('validation_plan'): improvements.add("Add a validation plan.")
        if not proposal.get('rollback_strategy'): improvements.add("Define a rollback strategy.")
    ...
    def _generate_overall_assessment(self,
        ...
        high_risks = sum(1 for r in risks if r.severity in [RiskSeverity.CRITICAL, RiskSeverity.HIGH])

        assessment = f"Overall assessment: {quality} (Score: {overall_score:.2f}). "

        issues_summary = []
        if critical_issues > 0: issues_summary.append(f"{critical_issues} critical issue(s)")
        if major_issues > 0: issues_summary.append(f"{major_issues} major issue(s)")
        if high_risks > 0: issues_summary.append(f"{high_risks} high/critical risk(s)")
        ...
        if critical_issues > 0 or any(r.severity == RiskSeverity.CRITICAL for r in risks):
    ...
    def _generate_recommendation(self,
        ...
        if critical_issues or critical_risks:
            return "reject"
        elif major_issues or high_risks or overall_score < 0.5: # Include score threshold
            return "modify"
        else: # Score >= 0.5 and no critical/major issues or high/critical risks
            return "approve"


    def _compute_evaluation_confidence(self,
                                       perspective_scores: List[PerspectiveScore],
                                       critiques: List[Critique]) -> float:
        ...
        confidences = [ps.confidence for ps in perspective_scores if ps.confidence is not None]
        avg_confidence = _np.mean(confidences) if confidences else 0.7 # Default confidence if no scores
    ...
    def _identify_trade_offs(self, evaluations: List[Evaluation]) -> List[str]:
        ...
        for perspective in EvaluationPerspective:
            ...
            for eval_item in evaluations:
                 ps = next((ps for ps in eval_item.perspective_scores if ps.perspective == perspective), None)
                 if ps:
                      scores_for_perspective.append({'id': eval_item.proposal_id, 'score': ps.score})
            ...
            if len(scores_for_perspective) >= 2:
                 ...
                 if max_score_item['score'] - min_score_item['score'] > 0.3: # Threshold for significant difference
                      trade_offs.append(
    ...
    def _generate_comparison_rationale(self,
                                       sorted_evaluations: List[Evaluation],
        ...
        if trade_offs:
            ...
            if len(trade_offs) > 1: rationale += " among others."
        else:
        ...
        for critique in critiques:
             ...
             if critique.perspective and critique.perspective.value and critique.aspect:
                 ...
                 self.critique_patterns[pattern_key] += 1
                  ...
                  logger.debug(f"Skipping pattern update for invalid critique: {critique}")
        ...
        for risk in risks:
             ...
             if risk.category and risk.category.value and risk.description:
                  ...
                  if len(self.risk_patterns[risk.category]) < 100:
                       ...
                       if risk.description not in self.risk_patterns[risk.category][-10:]: # Check last 10
    ...
    def _adapt_perspective_weights(self, evaluation: Evaluation, success: bool):
        ...
        if not self.adaptive_weights: return
        ...
        if not success:
            ...
            if relevant_perspectives:
                logger.debug(f"Adapting weights due to failure. Increasing importance of: {relevant_perspectives}")
                ...
                for p, w in self.perspective_weights.items():
                    if p in relevant_perspectives:
                        self.perspective_weights[p] = min(0.5, w * (1 + learning_rate)) # Increase, cap at 0.5
                    else:
                        self.perspective_weights[p] = max(0.01, w * (1 - increase_factor / (len(self.perspective_weights) - len(relevant_perspectives) + 1e-6))) # Decrease slightly, floor at 0.01
                    total_weight += self.perspective_weights[p]
                
                # Renormalize to ensure weights sum to 1
                if total_weight > 0:
                     self.perspective_weights = {k: v / total_weight for k, v in self.perspective_weights.items()}
                
                logger.debug(f"New perspective weights: { {p.value: w for p,w in self.perspective_weights.items()} }")
    ...
    def _initialize_default_criteria(self):
        ...
        self.evaluation_criteria = {
            EvaluationPerspective.LOGICAL_CONSISTENCY: [self._evaluate_logical_consistency],
