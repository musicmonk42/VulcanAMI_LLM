...
try:
    ...
    class FakeNumpy:
        ...
        def zeros(shape):
             ...
             if len(shape) == 2: return [[0.0] * shape[1] for _ in range(shape[0])]
             raise NotImplementedError("FakeNumpy only supports 1D/2D zeros")
        @staticmethod
        def array(data): return list(data) # Just return list
        @staticmethod
        def mean(lst): return sum(lst) / len(lst) if lst else 0.0
        @staticmethod
        def max(a, axis=None, out=None, keepdims=False, initial=None, where=True):
             if not a or (isinstance(a, list) and not any(isinstance(row, list) for row in a) and not a): # Empty list or list of empty lists
...
try:
    ...
    class Objective: pass # Simple placeholder class
    class ObjectiveType(Enum): PRIMARY="primary"; SECONDARY="secondary"; DERIVED="derived"
    class HierarchyConflictType(Enum): DIRECT="direct"; INDIRECT="indirect"; CONSTRAINT="constraint"; TRADEOFF="tradeoff" # Added TRADEOFF
...
class Conflict:
    ...
    def __hash__(self):
        ...
        return hash((tuple(sorted(self.objectives)), self.conflict_type, self.severity, self.description))
    ...
    def __eq__(self, other):
        ...
        return (set(self.objectives) == set(other.objectives) and
...
class GoalConflictDetector:
    ...
    def __init__(self, objective_hierarchy: Optional[ObjectiveHierarchy] = None):
        ...
        # FIXED: Allow Mock/MagicMock types for testing
             ...
             raise TypeError(f"objective_hierarchy must be an instance of RealObjectiveHierarchy or a Mock, got {type(objective_hierarchy)}")
        elif not OBJECTIVE_HIERARCHY_AVAILABLE and not isinstance(objective_hierarchy, (Mock, MagicMock)):
             raise TypeError(f"objective_hierarchy must be an instance of a Mock (RealObjectiveHierarchy failed to import), got {type(objective_hierarchy)}")
    ...
    def _initialize_resolution_strategies(self) -> Dict[ConflictType, List[Dict[str, Any]]]:
    ...
    def detect_conflicts(self, proposal: Dict[str, Any]) -> List[Conflict]:
        ...
        """
        logger.debug("Using deprecated alias 'detect_conflicts'. Use 'detect_conflicts_in_proposal' instead.")
    ...
    def detect_conflicts_in_proposal(self, proposal: Dict[str, Any]) -> List[Conflict]:
        ...
        with self.lock:
            ...
            constraint_conflicts = self._detect_constraint_conflicts(proposal_objectives, proposal)
            ...
            if len(proposal_objectives) >= 2:
                ...
                direct_conflicts = self._detect_direct_conflicts(proposal_objectives, proposal)
                all_conflicts.extend(direct_conflicts)
                if direct_conflicts: logger.debug(f"Found {len(direct_conflicts)} direct conflicts.")

                # Check for resource conflicts
                resource_conflicts = self._detect_resource_conflicts(proposal_objectives, proposal)
                all_conflicts.extend(resource_conflicts)
                if resource_conflicts: logger.debug(f"Found {len(resource_conflicts)} resource conflicts.")

                # Check for priority violations
                priority_conflicts = self._detect_priority_violations(proposal_objectives, proposal)
                all_conflicts.extend(priority_conflicts)
                if priority_conflicts: logger.debug(f"Found {len(priority_conflicts)} priority conflicts.")
                ...
                if pattern_conflicts: logger.debug(f"Found {len(pattern_conflicts)} pattern conflicts.")
    ...
    def analyze_multi_objective_tension(self,
                                        objectives: List[str]) -> MultiObjectiveTension:
        """
        ...
        with self.lock:
            ...
            if isinstance(upper_triangle_indices, tuple) and len(upper_triangle_indices) == 2 and len(upper_triangle_indices[0]) > 0:
                 # Extract values using the indices
                 upper_triangle_values = [tension_matrix[r][c] for r, c in zip(upper_triangle_indices[0], upper_triangle_indices[1])]
            ...
            analysis = MultiObjectiveTension(
                ...
                metadata={
                    'analyzed_at': time.time(),
                    'num_objectives': n,
                    'max_tension': float(self._np.max(tension_matrix)) if n > 0 else 0.0 # Use self._np
                }
            )
    ...
    def suggest_resolution(self, conflict: Conflict) -> List[Dict[str, Any]]:
        ...
        if not isinstance(conflict, Conflict):
             logger.warning("suggest_resolution received non-Conflict object: %s", type(conflict))
             # Attempt to convert if it's a dict from serialization
             if isinstance(conflict, dict) and 'objectives' in conflict and 'conflict_type' in conflict and 'severity' in conflict:
                 try:
                 ...
                 logger.error("Input to suggest_resolution is not a Conflict object or valid dict.")
                 return []
    ...
    def check_constraint_violations(self,
        ...
        with self.lock:
            ...
            known_fields = {'id', 'type', 'description', 'objective', 'objectives', 'optimize_for',
                            'objective_weights', 'predicted_outcomes', 'constraints', 'implementation',
                            'estimated_cost', 'estimated_duration', 'complexity'}
            ...
            for key, value in proposal.items():
                ...
                elif key == 'objective' and isinstance(value, str) and value in proposal:
            ...
            logger.debug(f"Checking constraints for objectives with values: {objectives_to_check}")

            for obj_name in objectives_to_check:
                obj_data = hierarchy_objectives.get(obj_name) # Safely get objective data
                if obj_data:
                    ...
                    if value is None:
                        logger.debug(f"No target value for {obj_name}, skipping constraint check.")
                        continue

                    try:
                        value_num = float(value)
                    except (TypeError, ValueError):
                        logger.debug(f"Target value for {obj_name} is not numeric ({value}), skipping constraint check.")
                        continue
                    ...
                    elif isinstance(obj_data, dict) and 'constraints' in obj_data and isinstance(obj_data['constraints'], dict):
                        constraints = obj_data['constraints']
                    ...
                    if 'min' in constraints:
                        try:
                             ...
                             if value_num < min_limit:
                                 ...
                                 logger.debug(f"Constraint violation: {obj_name} value {value_num} < min {min_limit}")
                                 violations.append({
                                     'objective': obj_name,
                                     'constraint': 'minimum',
                                     'value': value_num,
                             ...
                             logger.warning(f"Invalid 'min' constraint value for {obj_name}: {constraints['min']}")
                    ...
                    if 'max' in constraints:
                        try:
                             ...
                             if value_num > max_limit:
                                 ...
                                 logger.debug(f"Constraint violation: {obj_name} value {value_num} > max {max_limit}")
                                 violations.append({
                                     'objective': obj_name,
                                     'constraint': 'maximum',
                                     'value': value_num,
                            ...
                            logger.warning(f"Invalid 'max' constraint value for {obj_name}: {constraints['max']}")
                else:
                    logger.debug(f"Objective {obj_name} not found in hierarchy during constraint check.")


            return violations

    def validate_tradeoff_acceptability(self,
                                        tradeoff: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        with self.lock:
            ...
            conflict = None
            if hasattr(self.objective_hierarchy, 'find_conflicts') and callable(self.objective_hierarchy.find_conflicts):
                 conflict = self.objective_hierarchy.find_conflicts(sacrifice_obj, gain_obj)
            ...
            acceptable = len([i for i in issues if i['severity'] in ['high', 'critical']]) == 0
            # Simple confidence calc - adjust based on severity and number of issues
            confidence = max(0.0, 1.0 - (len(issues) * 0.1) - sum(0.2 if i['severity']=='high' else 0.4 if i['severity']=='critical' else 0 for i in issues))
    ...
    def _extract_objectives_from_proposal(self, proposal: Dict[str, Any]) -> List[str]:
        ...
        objectives = []
        proposal_id_str = f"proposal {proposal.get('id', 'N/A')}" # More descriptive log prefix
        logger.debug(f"Extracting objectives from {proposal_id_str}")

        # Ensure proposal is a dict
        if not isinstance(proposal, dict):
            logger.warning(f"Proposal provided is not a dict ({type(proposal)}). Cannot extract objectives.")
            return []
        ...
        if isinstance(opt_field, str) and opt_field:
            ...
            if valid_opts: logger.debug(f"  [{proposal_id_str}] Found 'optimize_for' list: {valid_opts}")
        ...
        if isinstance(objs_field, list):
            ...
            if valid_objs: logger.debug(f"  [{proposal_id_str}] Found 'objectives' list: {valid_objs}")
        elif isinstance(objs_field, str) and objs_field: # Handle single string case
             objectives.append(objs_field)
             logger.debug(f"  [{proposal_id_str}] Found 'objectives' string: {objs_field}")
        ...
        if isinstance(weights_field, dict):
            ...
            if valid_keys: logger.debug(f"  [{proposal_id_str}] Found keys from 'objective_weights': {valid_keys}")
        elif weights_field is not None:
             logger.warning(f"Malformed objective_weights in {proposal_id_str}: expected dict, got {type(weights_field)}")
        ...
        if isinstance(outcomes_field, dict):
            ...
            objectives.extend(valid_keys)
            if valid_keys: logger.debug(f"  [{proposal_id_str}] Found keys from 'predicted_outcomes': {valid_keys}")
        elif outcomes_field is not None:
            logger.warning(f"Malformed predicted_outcomes in {proposal_id_str}: expected dict, got {type(outcomes_field)}")
        ...
        potential_obj_keys = [k for k in all_keys if k not in known_fields and k in hierarchy_objectives]
        if potential_obj_keys:
            objectives.extend(potential_obj_keys)
            logger.debug(f"  [{proposal_id_str}] Found keys from proposal root: {potential_obj_keys}")


        # Remove duplicates while preserving order roughly
        unique_objectives = list(dict.fromkeys(objectives))
        logger.debug(f"  [{proposal_id_str}] Final unique objectives: {unique_objectives}")

        return unique_objectives

    def _detect_direct_conflicts(self,
                                 objectives: List[str],
                                 proposal: Dict[str, Any]) -> List[Conflict]:
        ...
        for i, obj_i in enumerate(objectives):
            for obj_j in objectives[i+1:]:
                ...
                if hasattr(self.objective_hierarchy, 'find_conflicts') and callable(self.objective_hierarchy.find_conflicts):
                     hierarchy_conflict = self.objective_hierarchy.find_conflicts(obj_i, obj_j)
                ...
                if hierarchy_conflict and isinstance(hierarchy_conflict, dict):
                     ...
                     direct_type = HierarchyConflictType.DIRECT.value if hasattr(HierarchyConflictType, 'DIRECT') else 'direct'

                     if conflict_type_str == direct_type:
                         logger.debug(f"  Direct conflict found between {obj_i} and {obj_j} from hierarchy")
                         conflicts.append(Conflict(
                             objectives=[obj_i, obj_j],
                             conflict_type=ConflictType.DIRECT, # Use local ConflictType enum
                             severity=self._map_severity(hierarchy_conflict.get('severity', 'medium')),
                             description=hierarchy_conflict.get('description', 'Direct conflict based on hierarchy'),
    ...
    def _detect_constraint_conflicts(self,
        ...
        violations = self.check_constraint_violations(proposal) # Pass the whole proposal
            ...
            for i, obj_i_name in enumerate(objectives):
                for obj_j_name in objectives[i+1:]:
                     ...
                     if obj_i_data and obj_j_data:
                         ...
                         constraints_i = {}
                         if hasattr(obj_i_data, 'constraints') and isinstance(obj_i_data.constraints, dict): constraints_i = obj_i_data.constraints
                         elif isinstance(obj_i_data, dict) and 'constraints' in obj_i_data: constraints_i = obj_i_data.get('constraints', {})

                         constraints_j = {}
                         if hasattr(obj_j_data, 'constraints') and isinstance(obj_j_data.constraints, dict): constraints_j = obj_j_data.constraints
                         elif isinstance(obj_j_data, dict) and 'constraints' in obj_j_data: constraints_j = obj_j_data.get('constraints', {})

                         # Check if constraints are incompatible
                         if self._constraints_conflict(constraints_i, constraints_j):
                             logger.debug(f"  Incompatible constraints between {obj_i_name} and {obj_j_name}")
                             conflicts.append(Conflict(
    ...
    def _detect_resource_conflicts(self,
        ...
        for resource, competing_objectives in shared_resources.items():
            if len(competing_objectives) > 1:
                logger.debug(f"  Resource conflict found for '{resource}' involving {competing_objectives}")
                conflicts.append(Conflict(
    ...
    def _detect_priority_violations(self,
                                    objectives: List[str],
                                    proposal: Dict[str, Any]) -> List[Conflict]:
            ...
            for obj_name, weight in weights.items():
                ...
                if obj_data:
                    ...
                    if priority >= 2 and weight > 0.7:
                        logger.debug(f"  Priority violation: Low-priority '{obj_name}' (p={priority}) has high weight {weight}")
                        conflicts.append(Conflict(
                            objectives=[obj_name],
    ...
    def _check_conflict_patterns(self, objectives: List[str]) -> List[Conflict]:
        ...
        for rule in self.conflict_rules:
            ...
            if rule_objs.issubset(proposal_objs):
                logger.debug(f"  Matched pattern '{rule['name']}' for objectives {rule_objs}")
                conflicts.append(Conflict(
                    objectives=list(rule_objs),
                    conflict_type=rule['type'], # Assumes ConflictType enum value matches rule type string
                    severity=rule['severity'], # Assumes ConflictSeverity enum value matches rule severity string
    ...
    def _suggest_resolutions(self, conflict: Conflict) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"Suggesting resolutions for {conflict.conflict_type.value} conflict involving {conflict.objectives}")
        ...
        for strategy_def in strategies:
            suggestion = {
                ...
                'specific_actions': self._generate_specific_actions(conflict, strategy_def)
            }
    ...
    def _generate_specific_actions(self,
        ...
        try:
            if strategy_name == 'weighted_sum':
                ...
                ordered_objs = [obj for obj in full_priority_order if obj in conflict.objectives]
                if ordered_objs:
                    ...
                    actions.append(f"Determine priority order for {conflict.objectives} and optimize sequentially")


            elif strategy_name == 'pareto_optimization':
                actions.append(f"Find Pareto frontier for objectives: {conflict.objectives}")
                actions.append("Select point from frontier based on preferences")

            elif strategy_name == 'constraint_relaxation':
                actions.append(f"Identify non-critical constraints related to {conflict.objectives}")
                ...
                actions.append("Constrain secondary objectives within acceptable bounds")

            elif strategy_name == 'resource_allocation':
                 resource = conflict.metadata.get('resource', 'shared_resource')
                 actions.append(f"Allocate resource '{resource}' based on objective priorities among {conflict.objectives}")

            elif strategy_name == 'time_sharing':
                 actions.append(f"Implement time-sharing schedule for competing objectives: {conflict.objectives}")

            elif strategy_name == 'constraint_reformulation':
                 actions.append(f"Analyze and reformulate incompatible constraints for {conflict.objectives}")

            elif strategy_name == 'priority_reordering':
                 actions.append(f"Review and potentially adjust priorities for conflicting objectives: {conflict.objectives}")

        except Exception as e:
            # FIXED: Log the error with more context
            logger.error(f"Error generating specific actions for {strategy_name}: {e}", exc_info=True)
            actions.append("Error generating specific actions - review manually.")


        return actions if actions else ["Consult objective hierarchy and context to resolve."]
    ...
    def _find_shared_resources(self, objectives: List[str]) -> Dict[str, List[str]]:
        ...
        for obj_name in objectives:
            ...
            if obj_data:
                ...
                resources = []
                if hasattr(obj_data, 'metadata') and isinstance(obj_data.metadata, dict):
                    resources = obj_data.metadata.get('resources', [])
                elif isinstance(obj_data, dict) and 'metadata' in obj_data and isinstance(obj_data['metadata'], dict):
                    resources = obj_data['metadata'].get('resources', [])
                
                if isinstance(resources, list): # Ensure it's a list
                    for resource in resources:
                         if isinstance(resource, str) and resource: # Ensure resource is a non-empty string
                             shared[resource].append(obj_name)

        # Return only truly shared resources
        return {r: objs for r, objs in shared.items() if len(objs) > 1}

    def _constraints_conflict(self,
                             constraints_a: Dict[str, Any],
                             constraints_b: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            b_max = float(constraints_b.get('max', float('inf')))
             ...
             logger.warning("Invalid constraint value detected during conflict check. Assuming no conflict.")
        ...
        for i, obj_i in enumerate(objectives):
            for obj_j in objectives[i+1:]:
                 ...
                 if conflict and isinstance(conflict, dict):
                      ...
                      critical_val = ConflictSeverity.CRITICAL.value if hasattr(ConflictSeverity.CRITICAL, 'value') else 'critical'
                      if severity_str == critical_val:
                          logger.debug(f"Not Pareto optimal due to critical conflict between {obj_i} and {obj_j}")
                          return False

        return True # Assume optimal if no critical conflicts found

    def _generate_tension_recommendations(self,
                                          objectives: List[str],
                                          tension_matrix: np.ndarray, # Or list of lists
                                          conflicts: List[Conflict],
                                          pareto_optimal: bool) -> List[str]:
        ...
        if overall_tension < 0.3:
            recommendations.append("Low overall tension - objectives appear largely compatible.")
        elif overall_tension < 0.6:
            recommendations.append("Moderate tension detected - consider multi-objective optimization strategies (e.g., weighted sum, Pareto).")
        else:
            recommendations.append("High tension detected - prioritization, sequential optimization, or constraint relaxation may be necessary.")

        if not pareto_optimal:
            recommendations.append("Current objective set might not be Pareto optimal; investigate potential improvements.")

        if conflicts:
            high_severity = [c for c in conflicts if c.severity in [ConflictSeverity.CRITICAL, ConflictSeverity.HIGH]]
            if high_severity:
                # Format conflict details safely
                conflict_details = []
                for c in high_severity[:3]: # Show first 3
                     try:
                          conflict_details.append(f"({c.objectives}, {c.conflict_type.value})")
                     except AttributeError: # Handle case where conflict_type might not be enum
                          conflict_details.append(f"({c.objectives}, {c.conflict_type})")

                recommendations.append(f"Address {len(high_severity)} high-severity conflict(s) first. Primary conflicts: {', '.join(conflict_details)}")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            conflict_detection_rate = (conflicts_detected / proposals_analyzed) if proposals_analyzed > 0 else 0.0
