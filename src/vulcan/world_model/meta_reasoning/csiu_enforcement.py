...
class CSIUEnforcement:
    ...
    def __init__(self, config: Optional[CSIUEnforcementConfig] = None):
        ...
        logger.debug("[INTERNAL] CSIU Enforcement initialized with 5% single cap, 10% cumulative cap")
    ...
    def enforce_pressure_cap(self, pressure: float) -> float:
        ...
        pressure = max(-self.config.max_single_influence, 
        ...
        if self.config.alert_on_high_influence and abs(pressure) > self.config.alert_threshold:
            # INTERNAL ONLY: DEBUG level logging
            logger.debug(
    ...
    def check_cumulative_influence(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent = [r for r in self._influence_history 
    ...
    def record_influence(self, record: CSIUInfluenceRecord):
        ...
        with self._lock:
            ...
            if (self.config.log_all_influence or 
                abs(record.pressure) >= self.config.log_level_threshold):
                logger.debug(
    ...
    def apply_regularization_with_enforcement(
        ...
        plan["_internal_metadata"]["csiu_pressure_original"] = round(original_pressure, 3)
