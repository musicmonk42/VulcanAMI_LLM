...
class Policy:
    ...
    nso_requirements: Dict[str, Any] = field(default_factory=lambda: {"adversarial_detected": False, "risk_score_max": 0.3})
    policy_hash: str = ""
...
def load_policy(path: str | Path, default_root: Optional[Path] = None) -> Policy:
    ...
    if not YAML_AVAILABLE and not p.name.endswith(".json"):
        logger.warning(f"PyYAML not found, but policy file {p} is not .json. Attempting JSON load anyway.")
    ...
    for idx, entry in enumerate(commands):
        ...
        if isinstance(entry, dict):
            ...
            elif isinstance(cmd_spec, list) and all(isinstance(x, str) for x in cmd_spec):
                cmd = list(cmd_spec)
            else:
                raise PolicyError(f"Gate {idx}: 'cmd' must be a string or list of strings")
            timeout_s = int(entry.get("timeout_s", 60))
            cwd = entry.get("cwd")
            env = entry.get("env") or {}
            if not isinstance(env, dict) or not all(isinstance(k, str) and isinstance(v, str) for k, v in env.items()):
                raise PolicyError(f"Gate {idx}: 'env' must be a mapping of string->string")
            gate_specs.append(GateSpec(name=name, cmd=tuple(cmd), timeout_s=timeout_s, cwd=cwd, env=env))
        elif isinstance(entry, str):
            gate_specs.append(GateSpec(name=name, cmd=tuple(_split_cmd(entry)), timeout_s=60))
        else:
            raise PolicyError(f"Gate {idx}: must be string or mapping with 'cmd'")

    nso_requirements = gates_node.get("nso_requirements", {"adversarial_detected": False, "risk_score_max": 0.3})
...
def check_files_against_policy(
    ...
    ok = len(reasons) == 0
    return FileCheckResult(ok=ok, reasons=dedupe_stable(reasons), offending_files=sorted(set(offending)))
...
def evaluate_nso_requirements(nso_report: Dict[str, Any], policy: Policy) -> Tuple[bool, List[str]]:
    """
    ...
    if req.get("adversarial_detected") is False and nso_report.get("adversarial_detected", False):
