...
try:
    ...
    from .motivational_introspection import ProposalValidation, ObjectiveAnalysis, ObjectiveStatus
...
class TransparencyInterface:
    ...
    def __init__(self, world_model: Optional['WorldModel'] = None):
        ...
        if world_model and hasattr(world_model, 'self_improvement_drive'):
            self.self_improvement_drive = world_model.self_improvement_drive or MagicMock(_csiu_enabled=False)
        else:
    ...
    def _make_serializable(self, data: Any, seen: Optional[Set[int]] = None) -> Any:
        ...
        if is_hashable and not isinstance(data, (str, int, float, bool, type(None), Enum)):
            if data_id in seen:
        ...
        try:
            if isinstance(data, dict):
                ...
                return {self._make_serializable(k, seen.copy()): self._make_serializable(v, seen.copy()) for k, v in data.items()} # Serialize keys too
                ...
                return [self._make_serializable(item, seen.copy()) for item in data]
                 ...
                 return sorted([self._make_serializable(item, seen.copy()) for item in data])
                ...
                return data.value
                 ...
                 try:
                     ...
                     return self._make_serializable(asdict(data), seen.copy())
                      ...
                      logger.debug(f"Error calling asdict on {type(data)}: {e}, falling back to string.")
                      # Fallback: serialize fields manually
                      cleaned_dict = {}
                      if hasattr(data, '__dataclass_fields__'):
                            for f_name in data.__dataclass_fields__:
                                cleaned_dict[f_name] = self._make_serializable(getattr(data, f_name), seen.copy())
                      return cleaned_dict
                 ...
                 try:
                     ...
                     logger.debug(f"Error calling to_dict on {type(data)}: {e}, falling back to string.")
                     try: return str(data)
    ...
    def serialize_validation(self, validation_result: Union[ProposalValidation, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
             ...
             if not isinstance(validation_dict, dict):
                  logger.error(f"Serialization of validation result did not yield a dictionary (got {type(validation_dict)}). Result: {validation_dict}")
                  # Fallback to a minimal error structure
                  validation_dict = {
                       'proposal_id': 'serialization_error', 'valid': False,
                       'reasoning': f"Serialization failed, original type: {type(validation_result)}"
                  }
        except Exception as e:
            logger.error(f"Critical error during validation serialization: {e}", exc_info=True)
            validation_dict = {
                 'proposal_id': 'serialization_error', 'valid': False,
        ...
        if not isinstance(validation_data, dict):
            logger.warning(f"Validation data within serialized result is not a dict: {type(validation_data)}. Using defaults.")
        ...
        elif ObjectiveStatus and hasattr(ObjectiveStatus, 'unknown'): # Fallback for lowercase
            default_status = ObjectiveStatus.unknown.value
        ...
        timestamp = self._safe_float(validation_data.get('timestamp'), time.time()) # Ensure float
        objective_analyses = validation_data.get('objective_analyses', [])
        # Ensure objective_analyses is a list
        if not isinstance(objective_analyses, list): objective_analyses = []

        conflicts = validation_data.get('conflicts_detected', validation_data.get('conflicts', []))
        if not isinstance(conflicts, list): conflicts = []

        alternatives = validation_data.get('alternatives_suggested', validation_data.get('alternatives', []))
        ...
        serialized = {
            ...
            'validation': {
                ...
                'conflicts': self._serialize_conflicts_compact(conflicts), # Further compact serialization
                'alternatives': self._serialize_alternatives_compact(alternatives), # Further compact serialization
                'reasoning': {
                    'primary': reasoning,
        ...
        audit_entry_data = serialized.copy() # Copy data before adding internal audit for the log
        if internal_audit:
        ...
        if 'internal_audit' in serialized:
             logger.debug("Removing Internal audit data from externally serializable output.")
    ...
    def serialize_objective_state(self) -> Dict[str, Any]:
        ...
        if hasattr(self.introspection_engine, 'explain_motivation_structure'):
             try:
                 motivation_structure = self.introspection_engine.explain_motivation_structure()
                 # Ensure the result is a dict
                 if not isinstance(motivation_structure, dict):
                      logger.warning("introspection_engine.explain_motivation_structure did not return a dict.")
                      motivation_structure = {}
             except Exception as e:
                  logger.error(f"Error calling explain_motivation_structure: {e}")
                  motivation_structure = {} # Fallback empty
        else:
             logger.warning("introspection_engine missing 'explain_motivation_structure' method.")
        ...
        serialized = {
            ...
            'objectives': {
                ...
                'state': self._make_serializable(motivation_structure.get('current_state', {}), seen=None) # Serialize state values
            },
        ...
        if 'csiu' in serialized: serialized.pop('csiu', None)
        if 'internal_audit' in serialized: serialized.pop('internal_audit', None)
    ...
    def serialize_conflict(self, conflict_analysis: Union[Any, List[Any]]) -> Dict[str, Any]:
        """
        ...
        elif hasattr(conflict_analysis, 'to_dict') and callable(conflict_analysis.to_dict):
            conflicts_input = [conflict_analysis]
        elif isinstance(conflict_analysis, dict):
            conflicts_input = [conflict_analysis]
        elif conflict_analysis is not None: # Log unexpected types
             logger.warning(f"Unexpected conflict_analysis type: {type(conflict_analysis)}. Processing as empty list.")
             conflicts_input = []
        else:
            conflicts_input = []
        ...
        for conflict in conflicts_input:
            try:
                conflict_dict = self._make_serializable(conflict, seen=None) # Clean the input first
                if not isinstance(conflict_dict, dict):
                     logger.warning(f"Serialized conflict is not a dict: {type(conflict_dict)}. Skipping.")
                     continue

                # Ensure essential keys exist after cleaning, provide defaults
                objectives = conflict_dict.get('objectives', [])
                if not isinstance(objectives, list): objectives = [] # Ensure list

                conflict_type_val = conflict_dict.get('conflict_type', conflict_dict.get('type', 'unknown')) # Handle both keys
                ...
                serialized_conflict = {
                    ...
                    'description': str(conflict_dict.get('description', '')), # Ensure string
                    'quantitative_measure': self._safe_float(conflict_dict.get('quantitative_measure'), None), # Ensure float or None
                    'resolution_options': self._make_serializable(conflict_dict.get('resolution_options', []), seen=None), # Ensure serializable list
                    'metadata': self._make_serializable(conflict_dict.get('metadata', {}), seen=None) # Ensure serializable dict
                }
                serialized_conflicts.append(serialized_conflict)
            except Exception as e:
                 logger.error(f"Error serializing individual conflict: {e}. Conflict data: {conflict}", exc_info=True)
        ...
        serialized = {
            ...
            'summary': {
                ...
                'requires_resolution': any(
                    ...
                    c.get('severity') in ['critical', 'high', ViolationSeverity.CRITICAL.value, ViolationSeverity.HIGH.value]
    ...
    def serialize_negotiation_outcome(self, negotiation: Any) -> Dict[str, Any]:
        ...
        try:
             ...
             if not isinstance(negotiation_dict, dict):
                  logger.error(f"Serialization of negotiation result did not yield a dict: {type(negotiation_dict)}")
                  negotiation_dict = {'outcome': 'serialization_error', 'reasoning': 'Serialization failed'}
        except Exception as e:
             logger.error(f"Critical error during negotiation serialization: {e}", exc_info=True)
             negotiation_dict = {'outcome': 'serialization_error', 'reasoning': f'Critical error: {e}'}
        ...
        serialized = {
            ...
            'agreement': {
                'objectives': self._make_serializable(negotiation_dict.get('agreed_objectives', {}), seen=None), # Serialize nested dict
                'weights': self._make_serializable(negotiation_dict.get('objective_weights', {}), seen=None), # Serialize nested dict
                'participants': self._make_serializable(negotiation_dict.get('participating_agents', []), seen=None) # Serialize list
            },
            'process': {
                'strategy': str(negotiation_dict.get('strategy_used', 'unknown')),
                'iterations': int(negotiation_dict.get('iterations', 0)),
                'convergence_time_ms': self._safe_float(negotiation_dict.get('convergence_time_ms'), 0.0),
                'compromises': self._make_serializable(negotiation_dict.get('compromises_made', []), seen=None) # Serialize list
            },
            'rationale': {
    ...
    def export_for_consensus(self) -> Dict[str, Any]:
        ...
        consensus_export = {
            ...
            'voting_context': { # Example context
                'quorum_required': True, 'voting_mechanism': 'weighted_majority',
                'confidence_threshold': 0.7, 'timeout_seconds': 300
            }
        }
        # Signature, internal audit, guard, logging... (same pattern as others)
        try: signature = self._generate_signature(self._make_serializable(consensus_export, seen=None))
        except Exception: signature = "signature_error"
        consensus_export['signature'] = signature
        internal_audit = self._generate_internal_audit()
        audit_entry_data = consensus_export.copy()
        if internal_audit: audit_entry_data['internal_audit'] = internal_audit
        if 'csiu' in consensus_export: consensus_export.pop('csiu', None)
        if 'internal_audit' in consensus_export: consensus_export.pop('internal_audit', None)
    ...
    def get_csiu_longitudinal(self) -> Dict[str, Any]:
        ...
        tracker = None
        if hasattr(self.introspection_engine, 'validation_tracker') and \
           not isinstance(self.introspection_engine.validation_tracker, MagicMock):
        ...
        if isinstance(self.self_improvement_drive, MagicMock) or not getattr(self.self_improvement_drive, '_csiu_enabled', False):
            logger.debug("CSIU disabled or self_improvement_drive unavailable.")
            return {}

        try:
            # Call the tracker's summarize method if it exists
            if hasattr(tracker, 'summarize_longitudinal') and callable(tracker.summarize_longitudinal):
                 trends = tracker.summarize_longitudinal()
                 return self._make_serializable(trends, seen=None) # Ensure result is serializable
            else:
                 logger.warning("ValidationTracker missing 'summarize_longitudinal' method.")
                 return {}
        except Exception as e:
    ...
    def get_audit_log(self,
        ...
        try:
            if start_time is not None:
                filtered = [e for e in filtered if isinstance(e, dict) and e.get('timestamp', 0) >= start_time]
            if end_time is not None:
                filtered = [e for e in filtered if isinstance(e, dict) and e.get('timestamp', float('inf')) <= end_time]
            if event_type is not None:
                filtered = [e for e in filtered if isinstance(e, dict) and e.get('event_type') == event_type]
        except Exception as e:
    ...
    def verify_signature(self, serialized_data: Dict[str, Any]) -> bool:
        ...
        if not isinstance(serialized_data, dict): return False # Must be dict
        ...
        try:
             ...
             recalculated_signature = self._generate_signature(self._make_serializable(data_to_verify, seen=None))
             is_valid = (stored_signature == recalculated_signature)
             if not is_valid:
                  logger.warning(f"Signature mismatch! Stored: {stored_signature}, Calculated: {recalculated_signature}")
             return is_valid
        except Exception as e:
    ...
    def _safe_float(self, value: Any, default: Optional[float] = 0.0) -> Optional[float]:
        ...
        if NUMPY_AVAILABLE and isinstance(value, np.generic):
            try: return float(value.item())
            except (AttributeError, ValueError, TypeError): return default
    ...
    def _generate_internal_audit(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            cur_telemetry = {}
            if hasattr(self.self_improvement_drive, '_collect_telemetry_snapshot') and callable(self.self_improvement_drive._collect_telemetry_snapshot):
            ...
            if not isinstance(csiu_weights, dict): csiu_weights = {} # Ensure dict


            pressure = 0.0
            if hasattr(self.self_improvement_drive, '_csiu_u_ewma'):
                U_ewma = getattr(self.self_improvement_drive, '_csiu_u_ewma', 0.0)
                if hasattr(self.self_improvement_drive, '_csiu_pressure') and callable(self.self_improvement_drive._csiu_pressure):
                     try: pressure = self.self_improvement_drive._csiu_pressure(U_ewma)
                     except Exception: pass # Ignore pressure calc errors

            internal_audit = {
                'csiu': {
                    'pressure': round(self._safe_float(pressure, 0.0), 3),
                    'weights': {k: round(self._safe_float(v, 0.0), 3) for k, v in csiu_weights.items()},
                    'telemetry': {
                        k: round(self._safe_float(v, 0.0), 3) if isinstance(v, (int, float)) else str(v)
    ...
    def _serialize_conflicts_compact(self, conflicts: List[Any]) -> List[Dict[str, Any]]:
        """Serialize conflicts in compact format"""
        serialized = []
        if not isinstance(conflicts, list): return [] # Ensure list input
        ...
        for c in conflicts:
             try:
                 ...
                 if isinstance(c_dict, dict):
                     ...
                     serialized.append({
                         'objectives': sorted([str(o) for o in objectives]), # Sort for consistency
                         'type': str(c_dict.get('conflict_type', c_dict.get('type', 'unknown'))),
                         'severity': str(c_dict.get('severity', 'unknown'))
                     })
             except Exception as e:
                  logger.warning(f"Skipping conflict during compact serialization due to error: {e}")
        return serialized

    def _serialize_alternatives_compact(self, alternatives: List[Any]) -> List[Dict[str, Any]]:
        """Serialize alternatives in compact format"""
        serialized = []
        if not isinstance(alternatives, list): return []
            ...
            try:
                ...
                if isinstance(alt_dict, dict):
                    serialized.append({
                        ...
                        'confidence': self._safe_float(alt_dict.get('confidence'), 0.0) # Ensure float
                    })
            except Exception as e:
                 logger.warning(f"Skipping alternative during compact serialization due to error: {e}")
        return serialized
    ...
    def _extract_structured_reasoning(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        outcome_data = data.get('validation', {}).get('outcome', data.get('outcome', {}))
        if not isinstance(outcome_data, dict): outcome_data = {}
        ...
        if decision is None: # Fallback for negotiation outcome status
             ...
             if decision_str: decision = decision_str not in ['deadlock', 'dominated', 'unknown', 'serialization_error']


        confidence = self._safe_float(outcome_data.get('confidence', data.get('confidence')), 0.0) # Check multiple places

        key_factors = set()
        reasoning_text = str(data.get('reasoning', data.get('rationale', {}).get('reasoning', ''))) # Check multiple places

        # Simple keyword extraction from reasoning text
        if reasoning_text:
             text_lower = reasoning_text.lower()
             if 'conflict' in text_lower: key_factors.add('conflict_detected')
             if 'violation' in text_lower: key_factors.add('constraint_violation')
             if 'align' in text_lower: key_factors.add('objective_aligned')
             if 'historical' in text_lower or 'pattern' in text_lower: key_factors.add('historical_pattern_match')
             if 'tradeoff' in text_lower: key_factors.add('tradeoff_considered')
             if 'pareto' in text_lower: key_factors.add('pareto_optimality')
             if 'compromise' in text_lower: key_factors.add('compromise_reached')
             if 'deadlock' in text_lower: key_factors.add('negotiation_deadlock')


        # Add factors from specific structured fields
        if data.get('conflicts_detected') or data.get('conflicts'): key_factors.add('explicit_conflict')
        if any(oa.get('status') == 'violation' for oa in data.get('validation', {}).get('objectives', [])): key_factors.add('explicit_violation')
    ...
    def _extract_actionable_items(self, validation_dict: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if not valid:
            ...
            if conflicts:
                actionable.append({
                    ...
                    'details': {'summary': f"{len(conflicts)} conflict(s) detected", 'conflicts': conflicts} # Use pre-serialized conflicts
                })
            ...
            if alternatives:
                actionable.append({
                    ...
                    'details': {'summary': f"{len(alternatives)} alternative(s) suggested", 'alternatives': alternatives} # Use pre-serialized alts
                })

            # Suggest reviewing reasoning, provide structured factors
            structured_reasoning = validation_dict.get('validation', {}).get('reasoning', {}).get('structured', {})
            if not actionable: # Only if no specific actions above
                 actionable.append({
                     'action': 'review_reasoning',
                     'target': 'proposal_logic',
                     'details': {
                         'primary_reason': validation_dict.get('validation', {}).get('reasoning', {}).get('primary', 'No specific reason provided.'),
    ...
    def _serialize_active_objectives(self, objectives_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Serialize active objectives list with weights/constraints"""
        if not isinstance(objectives_data, dict): return []
        ...
        for obj_name in active:
             if isinstance(obj_name, str): # Ensure obj is a string name
                 serialized_list.append({
                     'name': obj_name,
                     'weight': self._safe_float(weights.get(obj_name), 1.0), # Default 1.0
                     'constraints': self._make_serializable(constraints.get(obj_name, {}), seen=None) # Ensure constraints are serializable
                 })
    ...
    def _serialize_constraints(self) -> Dict[str, Any]:
        ...
        if hasattr(self.introspection_engine, 'objective_constraints') and \
           isinstance(self.introspection_engine.objective_constraints, dict):
    ...
    def _count_by_severity(self, items: List[Dict[str, Any]], key: str = 'severity') -> Dict[str, int]:
    ...
    def _recommend_resolution_strategy(self, conflicts: List[Dict[str, Any]]) -> str:
        ...
        if not conflicts: return 'none_needed'
        # Check severities safely
        has_critical = any(c.get('severity') in ['critical', ViolationSeverity.CRITICAL.value] for c in conflicts if isinstance(c, dict))
        if has_critical: return 'immediate_resolution_required'
        has_high = any(c.get('severity') in ['high', ViolationSeverity.HIGH.value] for c in conflicts if isinstance(c, dict))
        if has_high: return 'priority_resolution_recommended'
    ...
    def _get_recent_validations(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if tracker is None or isinstance(tracker, MagicMock) or not hasattr(tracker, 'validation_records'):
            logger.debug("Validation tracker unavailable or invalid for recent validations.")
            return []

        # Access deque safely and convert to list snapshot
        try:
             history_deque = tracker.validation_records
             if not isinstance(history_deque, deque): return [] # Ensure it's a deque
             ...
             for v in recent:
                  ...
                  if hasattr(v, 'to_dict') and callable(v.to_dict):
                      try: v_dict = self._make_serializable(v.to_dict(), seen=None)
            ...
            logger.error(f"Error accessing or serializing validation history: {e}", exc_info=True)
            return []
    ...
    def _get_active_conflicts(self) -> List[Dict[str, Any]]:
        ...
        objectives_dict = {}
        if hierarchy and not isinstance(hierarchy, MagicMock) and hasattr(hierarchy, 'objectives') and isinstance(hierarchy.objectives, dict):
                 ...
                 for obj_b in objective_names[i+1:]:
                      try:
                           ...
                           if conflict and isinstance(conflict, dict):
                                # Serialize and add
                                active_conflicts.append(self._make_serializable(conflict, seen=None))
                      except Exception as e:
                           logger.debug(f"Error checking hierarchy conflict between {obj_a} and {obj_b}: {e}")
        ...
        if isinstance(conflict_history_deque, deque):
             try:
                 ...
                 recent_conflicts = list(conflict_history_deque)[-10:] # Snapshot last 10
    ...
    def _get_consensus_statistics(self) -> Dict[str, Any]:
        ...
        if tracker and not isinstance(tracker, MagicMock) and hasattr(tracker, 'validation_records'):
            try:
                history_deque = tracker.validation_records
                if not isinstance(history_deque, deque): raise TypeError("validation_records is not a deque")
                ...
                if num_records > 0:
                    ...
                    approved_count = sum(1 for v in history_list if getattr(v, 'valid', False))
                    stats['approval_rate'] = approved_count / num_records

                    # Calculate average confidence safely using getattr and safe_float
                    confidences = [self._safe_float(getattr(v, 'confidence', 0.5), 0.5) for v in history_list]
                    stats['avg_confidence'] = float(_np.mean(confidences)) if confidences else 0.5

            except Exception as e:
                logger.error(f"Error calculating consensus statistics from validation history: {e}", exc_info=True)
    ...
    def _generate_signature(self, data: Any) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to serialize data for signature generation: {e}. Data type: {type(data)}")
            ...
            try:
                 ...
                 logger.critical(f"CRITICAL: Could not generate signature even with fallback: {final_e}")
    ...
    def _audit(self, event_type: str, data: Dict[str, Any]):
        ...
        entry = {
            ...
            'data_signature': data.get('signature') or self._generate_signature(self._make_serializable(data, seen=None)),
            # Optionally include a snapshot of key data points for quick audit viewing
            'summary': {
                 'type': data.get('type'),
                 'id': data.get('validation', {}).get('id') or data.get('proposal_id') or data.get('id', 'N/A')
            }
        }
        ...
        if not isinstance(self.self_improvement_drive, MagicMock) and getattr(self.self_improvement_drive, '_csiu_enabled', False):
            ...
            if csiu_summary and isinstance(csiu_summary, dict):
                 entry['csiu_summary'] = { # Select key fields
                      ...
                      'top_weights': sorted(csiu_summary.get('weights', {}).items(), key=lambda item: item[1], reverse=True)[:3] # Top 3 weights
                 }
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if not isinstance(self.self_improvement_drive, MagicMock):
             ...
             if csiu_enabled:
                  csiu_calc_enabled = getattr(self.self_improvement_drive, '_csiu_calc_enabled', False)
                  csiu_regs_enabled = getattr(self.self_improvement_drive, '_csiu_regs_enabled', False)
                  csiu_hist_enabled = getattr(self.self_improvement_drive, '_csiu_hist_enabled', False)
        ...
        stats_output = {
            'statistics': {  # Wrap counters in 'statistics' key
                'validations_serialized': stats_snapshot.get('validations_serialized', 0),
                'objective_states_serialized': stats_snapshot.get('objective_states_serialized', 0),
                'conflicts_serialized': stats_snapshot.get('conflicts_serialized', 0),
                'negotiations_serialized': stats_snapshot.get('negotiations_serialized', 0),
