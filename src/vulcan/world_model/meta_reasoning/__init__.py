...
try:
    ...
    logger.warning(
        "auto_apply_policy module or dependencies not found. Auto-apply disabled."
    )
    ...
    def check_files_against_policy(*args, **kwargs):
        return FileCheckResult(
            ok=False, reasons=["module_unavailable"], offending_files=[]
        )

    def run_gates(*args, **kwargs):
        return GatesReport(ok=False, failures=["module_unavailable"], outputs={})
...
def create_meta_reasoning_system(
    world_model, design_spec: dict = None, config: dict = None
) -> MotivationalIntrospection:
    """
    ...
    mi = MotivationalIntrospection(world_model=world_model, design_spec=design_spec)
...
def check_integration() -> dict:
    ...
    """
    status = {"all_components_importable": True, "components": {}, "errors": []}
...
try:
    ...
    # MetaReasoner not defined, this is expected in some imports
    pass
