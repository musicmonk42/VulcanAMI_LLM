...
class Objective:
    ...
    def is_satisfied(self) -> bool:
        ...
        try:
             ...
             logger.warning(f"Invalid constraint value for objective {self.name}. Cannot check satisfaction.")
    ...
    def violates_constraints(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"Invalid constraint or current value for objective {self.name}. Cannot check violation.")
...
class ObjectiveHierarchy:
    ...
    def __init__(self, design_spec: Optional[Dict[str, Any]] = None):
        ...
        self.parent_child_graph: Dict[str, Set[str]] = defaultdict(set) # Maps parent -> set(children)

        # Computed properties
        self.priority_order: List[str] = []
        self.conflict_matrix: Optional[np.ndarray] = None # Could be list of lists if numpy failed
        ...
        logger.info("ObjectiveHierarchy initialized with %d objectives",
                   len(self.objectives))
    ...
    def _initialize_from_design_spec(self):
        ...
        if not objectives_spec:
            logger.warning("No objectives found in design_spec. Creating default objectives.")
            self._create_default_objectives()
        else:
            # Load from spec
            for obj_name, obj_config in objectives_spec.items():
                if not isinstance(obj_config, dict):
                     logger.warning(f"Skipping invalid objective config for '{obj_name}': not a dictionary.")
                     continue
                try:
                    ...
                    self.add_objective(
                        Objective(
                            name=obj_name,
                            description=obj_config.get('description', f'Objective: {obj_name}'),
                            ...
                            target_value=float(obj_config['target']) if obj_config.get('target') is not None else None, # Ensure float if present
                            maximize=bool(obj_config.get('maximize', True)), # Ensure bool
                            metadata=obj_config.get('metadata', {})
                        )
                    )
                except (ValueError, TypeError) as e:
                     logger.error(f"Failed to load objective '{obj_name}' from spec: {e}. Config: {obj_config}")
    ...
    def add_objective(self,
        ...
        with self.lock:
            ...
            logger.debug("Added objective: %s (priority=%d, type=%s)",
                        objective.name, objective.priority, objective.objective_type.value)
    ...
    def _invalidate_computed_properties(self):
        ...
        logger.debug("Invalidated computed hierarchy properties (priority_order, conflict_matrix, etc.)")
    ...
    def get_dependencies(self, objective: str) -> Set[str]:
        ...
        with self.lock:
            ...
            if objective not in self.objectives:
                logger.warning("Objective %s not found when getting dependencies", objective)
                return set()
    ...
    def get_transitive_dependencies(self, objective: str) -> Set[str]:
        ...
        with self.lock:
            ...
            if objective not in self.objectives:
                 logger.warning(f"Objective '{objective}' not found for transitive dependency calculation.")
                 return set()

            # Compute closure using DFS
            closure = set()
            stack = list(self.dependency_graph.get(objective, set())) # Start with direct dependencies
            ...
            while stack:
                ...
                if current_dep in self.objectives:
                    ...
                    direct_deps_of_current = self.dependency_graph.get(current_dep, set())
                    stack.extend(d for d in direct_deps_of_current if d not in visited_in_walk)
                else:
                    logger.warning(f"Dependency '{current_dep}' for objective '{objective}' not found in hierarchy.")
    ...
    def check_consistency(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if not consistent:
                logger.warning("Hierarchy inconsistency detected: %d issue categories found.", len(issues))
                for issue in issues:
                     logger.warning(f"  - Type: {issue['type']}, Severity: {issue['severity']}")
            else:
    ...
    def find_conflicts(self, obj_a: str, obj_b: str) -> Optional[Dict[str, Any]]:
        ...
        with self.lock:
            ...
            if not obj_a_data or not obj_b_data:
                logger.debug(f"One or both objectives not found for conflict check: {obj_a}, {obj_b}")
                return None
            ...
            if obj_b in self.conflict_graph.get(obj_a, set()):
                ...
                severity = 'high'
                # Check obj_a's spec
                conflict_meta_a = next((item.get('severity', severity) for item in obj_a_data.metadata.get('conflicts_meta', []) if item.get('objective') == obj_b), severity)
                # Check obj_b's spec (could be defined either way)
                conflict_meta_b = next((item.get('severity', severity) for item in obj_b_data.metadata.get('conflicts_meta', []) if item.get('objective') == obj_a), severity)
                # Use the more severe if defined differently
                severity_map = {'low': 1, 'medium': 2, 'high': 3, 'critical': 4}
                final_severity = max(conflict_meta_a, conflict_meta_b, key=lambda s: severity_map.get(s, 0))
            ...
            constraint_conflict_desc = self._check_constraint_conflict(obj_a_data, obj_b_data)
                ...
                for dep_name in shared_deps:
                    ...
                    if dep_obj and dep_obj.metadata.get('limited_resource', False):
                        return {
                            ...
                            'resource': dep_obj.metadata.get('resource_name', dep_name) # Add resource name if available
                        }
    ...
    def get_hierarchy_structure(self) -> Dict[str, Any]:
        ...
        with self.lock:
            # Ensure graphs are built
            if (not self.dependency_graph or not self.conflict_graph) and self.objectives:
                 self._build_relationship_graphs()
            ...
            for obj_name, obj in self.objectives.items():
                ...
                obj_dict['dependencies'] = sorted(list(self.dependency_graph.get(obj_name, set())))
                obj_dict['conflicts'] = sorted(list(self.conflict_graph.get(obj_name, set())))
                obj_dict['children'] = sorted(list(self.parent_child_graph.get(obj_name, set()))) # Add children from graph
            ...
            for key in structure:
                 structure[key].sort(key=lambda x: (x.get('priority', 99), -x.get('weight', 0), x['name']))
    ...
    def get_parents(self, objective: str) -> List[str]:
        ...
        with self.lock:
            ...
            obj = self.objectives.get(objective)
            if not obj:
                logger.warning(f"Objective '{objective}' not found when getting parents.")
                return []
            ...
            if obj.parent:
                 ...
                 if obj.parent in self.objectives:
                      ...
                      logger.warning(f"Explicit parent '{obj.parent}' for objective '{objective}' no longer exists.")
            ...
            for potential_parent, children in self.parent_child_graph.items():
                if objective in children:
                     ...
                     if potential_parent in self.objectives:
                          ...
                          logger.warning(f"Parent '{potential_parent}' from graph for objective '{objective}' no longer exists.")
    ...
    def get_children(self, objective: str) -> List[str]:
        ...
        with self.lock:
            ...
            obj = self.objectives.get(objective)
            if not obj:
                logger.warning(f"Objective '{objective}' not found when getting children.")
                return []

            # Explicit children
            if hasattr(obj, 'children') and isinstance(obj.children, list):
                 # Verify children still exist
                 valid_explicit_children = {c for c in obj.children if c in self.objectives}
                 if len(valid_explicit_children) != len(obj.children):
                      logger.warning(f"Some explicit children for '{objective}' no longer exist.")
            ...
            if len(valid_graph_children) != len(graph_children):
                 logger.warning(f"Some children from graph for '{objective}' no longer exist.")
        ...
        with self.lock:
            ...
            if obj:
                try:
                    ...
                    logger.debug("Updated %s current value to %.3f", objective, obj.current_value)
                except (TypeError, ValueError):
                    logger.error(f"Invalid value type '{type(value)}' provided for objective {objective}. Value not updated.")
            else:
                logger.warning(f"Attempted to update value for non-existent objective: {objective}")
    ...
    def get_unsatisfied_objectives(self) -> List[str]:
        ...
        with self.lock:
            ...
            # Sort by priority?
            unsatisfied.sort(key=lambda name: (self.objectives[name].priority, -self.objectives[name].weight, name))
            return unsatisfied
    ...
    def get_violated_objectives(self) -> List[str]:
        ...
        with self.lock:
            ...
            # Sort by priority?
            violated.sort(key=lambda name: (self.objectives[name].priority, -self.objectives[name].weight, name))
            return violated
            ...
            for obj_name, obj in self.objectives.items():
                ...
                for dep in getattr(obj, 'dependencies', []):
                    if dep in all_objective_names:
                        ...
                        logger.warning(f"Objective '{obj_name}' lists non-existent dependency '{dep}'.")
                if valid_deps:
                ...
                for conflict in getattr(obj, 'conflicts_with', []):
                    if conflict in all_objective_names:
                        ...
                        logger.warning(f"Objective '{obj_name}' lists non-existent conflict '{conflict}'.")
                if valid_conflicts:
                ...
                if parent_name:
                    if parent_name in all_objective_names:
                        ...
                        logger.warning(f"Objective '{obj_name}' lists non-existent parent '{parent_name}'.")
                ...
                if children_list:
                     ...
                     if len(valid_children) != len(children_list):
                          logger.warning(f"Objective '{obj_name}' lists some non-existent children.")
                     if valid_children:
        ...
        def find_cycles_util(node):
            ...
            for neighbor in self.dependency_graph.get(node, set()):
                if neighbor not in visited:
                    ...
                    logger.warning(f"Circular dependency detected involving edge {node} -> {neighbor}")
                    ...
                    cycles.append([node, neighbor]) # Report the edge causing cycle detection
    ...
    def _find_invalid_references(self) -> Tuple[List[Tuple[str, str]], List[Tuple[str, str]]]:
    ...
    def _find_constraint_conflicts(self) -> List[Dict[str, Any]]:
        ...
        for i, name_a in enumerate(all_objective_names):
             for name_b in all_objective_names[i+1:]:
                  ...
                  related = (name_b in self.dependency_graph.get(name_a, set()) or
                             name_a in self.dependency_graph.get(name_b, set()) or
                             name_b in self.conflict_graph.get(name_a, set()))
                  ...
                  if related:
                       ...
                       if conflict_desc:
                            conflicts.append({
                                'objectives': sorted([name_a, name_b]), # Sort for consistent reporting
    ...
    def _check_constraint_conflict(self,
                                   obj_a: Objective,
                                   obj_b: Objective) -> Optional[str]:
        ...
        try:
            ...
            a_min = float(constraints_a.get('min', float('-inf'))) if constraints_a.get('min') is not None else float('-inf')
            a_max = float(constraints_a.get('max', float('inf'))) if constraints_a.get('max') is not None else float('inf')
            b_min = float(constraints_b.get('min', float('-inf'))) if constraints_b.get('min') is not None else float('-inf')
            b_max = float(constraints_b.get('max', float('inf'))) if constraints_b.get('max') is not None else float('inf')
            ...
            if a_max < b_min or b_max < a_min:
             ...
             logger.warning(f"Could not compare constraints for {obj_a.name} and {obj_b.name} due to invalid numeric values.")
    ...
    def compute_conflict_matrix(self) -> np.ndarray: # Or list of lists
        ...
        with self.lock:
                ...
                for i, name_a in enumerate(obj_names):
                    for j, name_b in enumerate(obj_names):
                        ...
                        if conflict:
                            ...
                            severity_map = {'negligible': 0.1, 'low': 0.3, 'medium': 0.6, 'high': 0.9, 'critical': 1.0}
                            strength = severity_map.get(severity_str, 0.5) # Default to medium strength
            ...
            if self.conflict_matrix is None:
                ...
                return [row[:] for row in self.conflict_matrix] # Deep copy for list of lists
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for obj in self.objectives.values():
                  if obj.objective_type == ObjectiveType.PRIMARY: primary_count += 1
                  elif obj.objective_type == ObjectiveType.SECONDARY: secondary_count += 1
                  elif obj.objective_type == ObjectiveType.DERIVED: derived_count += 1
            ...
            total_conflicts = sum(len(conflicts) for conflicts in self.conflict_graph.values()) // 2
            ...
            return {
                ...
                'unsatisfied_count': len(self.get_unsatisfied_objectives()), # Call method for consistency
                'violated_count': len(self.get_violated_objectives()), # Call method for consistency
