...
try:
    ...
    class FakeNumpy:
        ...
        def var(lst):
             ...
             return sum((x - mean_val) ** 2 for x in lst) / len(lst) # Population variance
        @staticmethod
...
try:
    ...
    class HierarchyConflictType(Enum): DIRECT="direct"; INDIRECT="indirect"; CONSTRAINT="constraint"; TRADEOFF="tradeoff"
...
class ObjectiveNegotiator:
    ...
    def __init__(self, objective_hierarchy=None, world_model=None): # Made args optional
        """
        ...
        if objective_hierarchy is None:
             if not OBJECTIVE_HIERARCHY_AVAILABLE:
                  logger.warning("No ObjectiveHierarchy provided and import failed. Using MagicMock fallback.")
                  self.objective_hierarchy = ObjectiveHierarchy() # Instantiates the mock
             else:
                  logger.warning("No ObjectiveHierarchy provided. Using MagicMock fallback for now.")
                  self.objective_hierarchy = ObjectiveHierarchy() # Instantiates the mock
        
        # --- START FIX: Allow Mock objects from tests ---
        elif OBJECTIVE_HIERARCHY_AVAILABLE and not isinstance(objective_hierarchy, (RealObjectiveHierarchy, Mock, MagicMock)):
             raise TypeError(f"objective_hierarchy must be an instance of ObjectiveHierarchy or a Mock, but got {type(objective_hierarchy)}")
        # --- END FIX ---
    ...
    def negotiate_multi_agent_proposals(self,
                                        proposals: List[Dict[str, Any]]) -> NegotiationResult:
        """
        ...
        with self.lock:
            ...
            if not isinstance(proposals, list) or not all(isinstance(p, dict) for p in proposals):
                 logger.error(f"Invalid proposals format: expected List[Dict], got {type(proposals)}")
            ...
            try:
                if strategy == NegotiationStrategy.PARETO_OPTIMAL:
                    ...
                    result = self._negotiate_via_weighted_average(agent_proposals, analysis)
                elif strategy == NegotiationStrategy.NASH_BARGAINING:
                    result = self._negotiate_via_nash_bargaining(agent_proposals, analysis)
                elif strategy == NegotiationStrategy.LEXICOGRAPHIC:
                    result = self._negotiate_via_lexicographic(agent_proposals, analysis)
                else: # Default or MINIMAX
                    result = self._negotiate_via_minimax(agent_proposals, analysis)
            except Exception as e:
                 logger.error(f"Negotiation execution failed with strategy {strategy.value}: {e}", exc_info=True)
                 result = self._create_deadlock_result(agent_proposals, f"Negotiation execution error: {e}")
            ...
            if result.outcome != NegotiationOutcome.DEADLOCK:
                ...
                is_valid, validation_reason = self.validate_negotiated_objectives(result.agreed_objectives, return_reason=True)
                if not is_valid:
                    logger.warning(f"Negotiated objectives failed validation: {validation_reason}")
                    ...
                    result.reasoning = f"Negotiation deadlock: Validation failed - {validation_reason}"
                else:
            ...
            if result.outcome in [NegotiationOutcome.CONSENSUS, NegotiationOutcome.COMPROMISE]:
                self.strategy_performance[strategy]['successes'] += 1
            else: # DEADLOCK or DOMINATED
                self.strategy_performance[strategy]['failures'] += 1
                
            logger.info(f"Negotiation completed. Outcome: {result.outcome.value}, Confidence: {result.confidence:.2f}")


            return result

    def find_pareto_frontier(self, objective_space: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        with self.lock:
             # Validate input
            if not isinstance(objective_space, dict) or 'objectives' not in objective_space or not isinstance(objective_space['objectives'], list):
            ...
            if hasattr(self.world_model, 'motivational_introspection'):
                ...
                if not isinstance(mi, MagicMock) and hasattr(mi, 'counterfactual_reasoner'):
                     ...
                     if not isinstance(potential_reasoner, MagicMock) and hasattr(potential_reasoner, 'find_pareto_frontier'):
                          reasoner = potential_reasoner

            if reasoner:
                 try:
                      logger.debug(f"Using counterfactual_reasoner to find Pareto frontier for {objectives}")
                      ...
                      for point in pareto_points_objects:
                            ...
                            point_dict = {
                                ...
                                'is_pareto_optimal': getattr(point, 'is_pareto_optimal', False),
                      ...
                      logger.error(f"Counterfactual reasoner failed during Pareto calculation: {e}", exc_info=True)
    ...
    def resolve_objective_conflict(self,
                                   obj_a: str,
                                   obj_b: str,
        ...
        with self.lock:
            ...
            conflict = None
            if hasattr(self.objective_hierarchy, 'find_conflicts') and callable(self.objective_hierarchy.find_conflicts):
                 conflict = self.objective_hierarchy.find_conflicts(obj_a, obj_b)


            if not conflict:
                logger.info(f"No conflict found between '{obj_a}' and '{obj_b}'. Resolution not needed.")
                # No conflict - assume both can be maximally satisfied (or use base weights?)
                base_weight_a = getattr(self.objective_hierarchy.objectives.get(obj_a), 'weight', 1.0) if obj_a in getattr(self.objective_hierarchy, 'objectives', {}) else 1.0
                base_weight_b = getattr(self.objective_hierarchy.objectives.get(obj_b), 'weight', 1.0) if obj_b in getattr(self.objective_hierarchy, 'objectives', {}) else 1.0
                ...
                return ConflictResolution(
                    ...
                    agreed_weights={obj_a: norm_w_a, obj_b: norm_w_b}, # Use normalized base weights
                    # Estimate outcomes assuming no conflict (e.g., target or high value)
                    expected_outcomes={
                         obj_a: getattr(self.objective_hierarchy.objectives.get(obj_a), 'target_value', 1.0) if obj_a in getattr(self.objective_hierarchy, 'objectives', {}) else 1.0,
                         obj_b: getattr(self.objective_hierarchy.objectives.get(obj_b), 'target_value', 1.0) if obj_b in getattr(self.objective_hierarchy, 'objectives', {}) else 1.0
                    },
                    tradeoffs=[],
                    confidence=1.0,
            ...
            resolution = ConflictResolution(
                objectives=[obj_a, obj_b],
                resolution_type=conflict.get('type', 'unknown_conflict'), # Get type from conflict dict
            ...
            logger.info(f"Conflict resolved. Agreed weights: {resolution.agreed_weights}")


            return resolution

    def dynamic_objective_weighting(self,
        ...
        with self.lock:
            ...
            if not isinstance(system_state, dict):
                 logger.warning("Invalid system_state type provided for dynamic weighting. Returning base weights.")
            ...
            if hasattr(self.objective_hierarchy, 'objectives') and isinstance(self.objective_hierarchy.objectives, dict):
                 hierarchy_objectives = self.objective_hierarchy.objectives

            for obj_name, obj_data in hierarchy_objectives.items():
                 # Safely get weight
                 weight = 1.0 # Default weight
                 if hasattr(obj_data, 'weight'): weight = getattr(obj_data, 'weight', 1.0)
                 elif isinstance(obj_data, dict): weight = obj_data.get('weight', 1.0)
                 base_weights[obj_name] = float(weight) # Ensure float

            if not base_weights:
                 logger.warning("No objectives found in hierarchy. Cannot perform dynamic weighting.")
                 return {}
            ...
            if 'safety' in adjusted_weights and uncertainty > 0.7:
                adjusted_weights['safety'] *= (1.0 + (uncertainty - 0.7) * 2) # Scale increase

            # Rule 2: Increase efficiency when resources are low
            if 'efficiency' in adjusted_weights and resources < 0.3:
                adjusted_weights['efficiency'] *= (1.0 + (0.3 - resources) * 3) # Scale increase

            # Rule 3: Increase accuracy when confidence is needed
            # Use 'prediction_accuracy' or 'accuracy' if present
            accuracy_key = next((k for k in ['prediction_accuracy', 'accuracy'] if k in adjusted_weights), None)
            if accuracy_key and confidence_required > 0.8:
                adjusted_weights[accuracy_key] *= (1.0 + (confidence_required - 0.8) * 2.5) # Scale increase


            # Rule 4: Increase exploration when performance plateaus
            exploration_key = next((k for k in ['exploration', 'curiosity'] if k in adjusted_weights), None) # Allow 'curiosity' too
            ...
            for obj_name, obj_data in hierarchy_objectives.items():
                 ...
                 if hasattr(obj_data, 'priority'): priority = getattr(obj_data, 'priority', 1)
                 elif isinstance(obj_data, dict): priority = obj_data.get('priority', 1)
            ...
            if total_critical_minimum > 1.0:
                 logger.warning(f"Total minimum weight for critical objectives ({total_critical_minimum:.2f}) exceeds 1.0. Scaling down.")
                 scale_factor = 1.0 / total_critical_minimum
                 critical_objectives = {k: v * scale_factor for k, v in critical_objectives.items()}
                 total_critical_minimum = 1.0
            # If critical minimums are too high, ensure they don't leave negative weight for others
            elif total_critical_minimum > 0.95: # Leave at least 5% for non-critical if possible
                logger.warning(f"Critical objectives reserve {total_critical_minimum*100:.1f}% of weight. Limited room for others.")
            ...
            if total_non_critical_adjusted > 0 and remaining_weight > 0:
                ...
                final_weights.update(normalized_non_critical)
                 ...
                 logger.warning("No remaining weight for non-critical objectives after assigning critical minimums.")
            ...
            if abs(total_final_weight - 1.0) > 1e-6:
                if total_final_weight > 0:
                     logger.debug(f"Final weights sum {total_final_weight:.6f}, renormalizing.")
                     renorm_factor = 1.0 / total_final_weight
                     final_weights = {k: v * renorm_factor for k, v in final_weights.items()}
                else: # All weights ended up zero somehow, distribute equally
                     logger.warning("All final weights are zero. Distributing weight equally.")
            ...
            logger.debug(f"Dynamic weighting complete. Final weights: { {k: round(v, 4) for k, v in final_weights.items()} }")

            return final_weights

    # --- START FIX: Modified signature and logic ---
    def validate_negotiated_objectives(self,
                                       negotiated: Dict[str, Any],
                                       return_reason: bool = False) -> Union[bool, Tuple[bool, str]]:
        """
        ...
        with self.lock:
            ...
            if not isinstance(negotiated, dict):
                 validation_reason = f"Invalid input type: expected dict, got {type(negotiated)}"
            ...
            for obj_name in negotiated.keys():
                if obj_name not in hierarchy_objectives:
                    validation_reason = f"Unknown objective in negotiation result: '{obj_name}'"
                    logger.warning(validation_reason)
                    return (False, validation_reason) if return_reason else False
            ...
            all_le_one = all(isinstance(v, (int, float)) and v <= 1.0 + 1e-6 for v in negotiated.values())
            sum_is_one = abs(sum(v for v in negotiated.values() if isinstance(v, (int, float))) - 1.0) < 1e-6
            ...
            if is_values:
                logger.debug("Validating negotiated OBJECTIVE VALUES against constraints.")
                for obj_name, value in negotiated.items():
                    obj_data = hierarchy_objectives.get(obj_name)
                    if not obj_data: continue # Should have been caught by check 1

                    constraints = {}
                    if hasattr(obj_data, 'constraints') and isinstance(obj_data.constraints, dict):
                 ...
                 for obj_name, weight in negotiated.items():
                      try:
                           ...
                           if w_num < -1e-6: # Allow small negative tolerance
                                validation_reason = f"Negative weight assigned to {obj_name}: {w_num:.6f}"
                                logger.warning(validation_reason)
                                return (False, validation_reason) if return_reason else False
                           total_weight += w_num
                      except (TypeError, ValueError):
                           validation_reason = f"Non-numeric weight '{weight}' for objective '{obj_name}'."
                           logger.warning(validation_reason)
                           return (False, validation_reason) if return_reason else False

                 if abs(total_weight - 1.0) > 1e-6: # Allow tolerance for sum
                      validation_reason = f"Negotiated weights do not sum to 1.0 (sum={total_weight:.6f})"
                      logger.warning(validation_reason)
                      return (False, validation_reason) if return_reason else False
            ...
            min_critical_weight = 0.05 # Minimum acceptable weight for a critical objective if included
            
            for obj_name, negotiated_value in negotiated.items():
                obj_data = hierarchy_objectives.get(obj_name)
                if not obj_data: continue # Should be impossible due to Check 1
                ...
                if priority == 0:  # This is a critical objective that is *in the negotiated list*
                    if is_weights:
                        # Check if weight is too low
                        try:
                            weight_num = float(negotiated_value)
                            if weight_num < min_critical_weight - 1e-6: # Allow tolerance
                         ...
                         if hasattr(obj_data, 'target_value'): target_value = getattr(obj_data, 'target_value', 1.0)
                         elif isinstance(obj_data, dict): target_value = obj_data.get('target_value', 1.0)
    ...
    def _parse_proposal(self, proposal: Dict[str, Any]) -> AgentProposal:
        ...
        try:
             ...
             logger.error(f"Failed to parse proposal due to invalid value type: {e}. Proposal: {proposal}")
             # Return a default/dummy proposal or raise error?
             # Returning dummy allows processing to continue but might hide issues.
             return AgentProposal(agent_id='parse_error', objective='unknown', target_value=0, weight=0)



    def _analyze_proposal_space(self,
                                proposals: List[AgentProposal]) -> Dict[str, Any]:
        """Analyze space of proposals"""
        _np = self._np # Use internal numpy/FakeNumpy alias

        # Collect all proposed objectives
        objectives = sorted(list(set(p.objective for p in proposals if p.objective != 'unknown'))) # Exclude unknowns
        ...
        if hasattr(self.objective_hierarchy, 'find_conflicts') and callable(self.objective_hierarchy.find_conflicts):
            for i, obj_a in enumerate(objectives):
                for obj_b in objectives[i+1:]:
                    conflict = self.objective_hierarchy.find_conflicts(obj_a, obj_b)
                    if conflict and isinstance(conflict, dict): # Check if conflict found and is dict
                        conflicts.append({
                            'objectives': [obj_a, obj_b],
        ...
        flexibility = [p.flexibility for p in proposals if isinstance(p.flexibility, (int, float))]
        ...
        analysis['negotiation_difficulty'] = self._assess_difficulty(analysis) # Pass analysis dict
    ...
    def _select_negotiation_strategy(self,
                                     ...
                                     analysis: Dict[str, Any]) -> NegotiationStrategy:
        ...
        if analysis['avg_flexibility'] < 0.3:
             ...
             if priority_order and all(obj in priority_order for obj in analysis['objectives']):
    ...
    def _negotiate_via_pareto(self,
                             proposals: List[AgentProposal],
                             analysis: Dict[str, Any]) -> NegotiationResult:
    ...
    def _negotiate_via_weighted_average(self,
                                        proposals: List[AgentProposal],
                                        analysis: Dict[str, Any]) -> NegotiationResult:
        ...
        objective_proposer_weight_sum = defaultdict(float) # Sum of weights of agents proposing each obj

        total_agent_weight = sum(p.weight for p in proposals)

        # Guard against division by zero
        if total_agent_weight <= 0:
            logger.warning("All agent proposals have zero or negative weight in weighted average.")
            ...
            if num_objs > 0:
                 ...
                 for obj in analysis['objectives']:
                      ...
                      agreed_outcomes[obj] = _np.mean(targets) if targets else 0.5 # Default outcome

                 return NegotiationResult(outcome=NegotiationOutcome.COMPROMISE, agreed_objectives=agreed_outcomes, objective_weights=final_weights, participating_agents=[p.agent_id for p in proposals], strategy_used=NegotiationStrategy.WEIGHTED_AVERAGE, iterations=1, convergence_time_ms=0, compromises_made=[{'type':'equal_weight_fallback'}], pareto_optimal=False, confidence=0.5, reasoning="Fell back to equal weights due to zero agent weights.")
            else:
                 return self._create_deadlock_result(proposals, "All agent proposals have zero weight and no objectives identified.")
        ...
        for proposal in proposals:
            ...
            objective_target_weighted_sum[proposal.objective] += proposal.target_value * proposal.weight
        ...
        for obj, total_prop_weight in objective_proposer_weight_sum.items():
            if total_prop_weight > 0:
                agreed_objectives_outcomes[obj] = objective_target_weighted_sum[obj] / total_prop_weight
            else:
                 ...
                 targets = [p.target_value for p in proposals if p.objective == obj]
                 agreed_objectives_outcomes[obj] = _np.mean(targets) if targets else final_objective_weights.get(obj, 0.5) # Fallback
    ...
    def _negotiate_via_nash_bargaining(self,
                                       proposals: List[AgentProposal],
                                       analysis: Dict[str, Any]) -> NegotiationResult:
        """Negotiate using Nash bargaining solution (Placeholder)"""
        logger.warning("Nash bargaining strategy is not fully implemented - using fallback.")
        # Fallback to weighted average for now
        return self._negotiate_via_weighted_average(proposals, analysis)


    def _negotiate_via_lexicographic(self,
                                     proposals: List[AgentProposal],
                                     analysis: Dict[str, Any]) -> NegotiationResult:
        ...
        priority_order = []
        if hasattr(self.objective_hierarchy, 'get_priority_order') and callable(self.objective_hierarchy.get_priority_order):
             priority_order = self.objective_hierarchy.get_priority_order()

        # Order objectives present in proposals based on the hierarchy's priority order
        objectives_in_proposals = analysis['objectives']
        ordered_objectives = [obj for obj in priority_order if obj in objectives_in_proposals]
        # Add any proposal objectives not in the hierarchy order at the end (lower priority)
        ordered_objectives.extend([obj for obj in objectives_in_proposals if obj not in priority_order])


        if not ordered_objectives:
             return self._create_deadlock_result(proposals, "No valid objectives for lexicographic ordering.")
        ...
        agreed_weights = {obj: w / total_raw_weight for obj, w in raw_weights.items()} if total_raw_weight > 0 else {}
    ...
    def _negotiate_via_minimax(self,
                               proposals: List[AgentProposal],
                               analysis: Dict[str, Any]) -> NegotiationResult:
    ...
    def _generate_candidate_solutions(self, objectives: List[str]) -> List[Dict[str, Any]]:
        """Generate candidate solutions (weight combinations and estimated outcomes)"""

        candidates = []
        n_objectives = len(objectives)
        if n_objectives == 0: return candidates
        ...
        if hasattr(self.world_model, 'motivational_introspection'):
            ...
            if not isinstance(mi, MagicMock) and hasattr(mi, 'counterfactual_reasoner'):
                 potential_reasoner = mi.counterfactual_reasoner
                 if not isinstance(potential_reasoner, MagicMock) and hasattr(potential_reasoner, 'predict_under_objective'):
                      use_cf_reasoner = True
                      reasoner = potential_reasoner

        logger.debug(f"Generating candidate solutions for {n_objectives} objectives. Using reasoner: {use_cf_reasoner}")
        ...
        for weights_tuple in generated_weights_tuples:
            ...
            if use_cf_reasoner and reasoner:
                 for obj in objectives:
                      try:
                           ...
                           logger.warning(f"Prediction failed for objective {obj} with weights {weights}: {e}")
            ...
            avg_confidence = (confidence_sum / num_predicted) if num_predicted > 0 else 0.5
        ...
        logger.debug(f"Generated {len(candidates)} candidate solutions with estimated outcomes.")
        return candidates
    ...
    def _generate_weights(self, n: int, steps: int) -> List[Tuple[float, ...]]:
        ...
        for res_tuple in results:
            ...
            elif total > tolerance: # Normalize if sum is significantly off but positive
        ...
        if len(unique_results) < n + 1:
            corners = [tuple(1.0 if j == i else 0.0 for j in range(n)) for i in range(n)]
    ...
    def _generate_weights_recursive(self, n: int, steps: int, target_sum: float) -> List[Tuple[float, ...]]:
        ...
        if n == 1:
            ...
            if -tolerance < target_sum < 1.0 + tolerance:
                 ...
                 results.append((max(0.0, min(target_sum, 1.0)),)) # Clamp result just in case
            return results
        ...
        for i in range(steps + 1):
            ...
            if remaining_target > -tolerance:
                 ...
                 for rest_tuple in self._generate_weights_recursive(n - 1, steps, remaining_target):
    ...
    def _calculate_nash_product(self,
        ...
        for proposal in proposals:
            ...
            elif achieved > 0: # If target is 0 or less, any positive achievement is max utility? Or interpret differently? Assume 1.0 here.
                 utility = 1.0
    ...
    def _calculate_max_regret(self,
                              candidate: Dict[str, Any],
                              proposals: List[AgentProposal]) -> float:
    ...
    def _select_fairest_pareto_point(self,
        ...
        logger.debug(f"Selected fairest Pareto point with fairness score: {best_fairness:.4f}")
        return best_point
    ...
    def _analyze_binary_tradeoff(self,
                                 obj_a: str,
                                 obj_b: str,
        ...
        logger.debug(f"Analyzing tradeoff between {obj_a} and {obj_b}. Using reasoner: {reasoner is not None}")
            ...
            if reasoner:
                try:
                    ...
                    tradeoffs.append({
                     ...
                     logger.warning(f"Prediction failed during tradeoff analysis for weights ({weight_a:.2f}, {weight_b:.2f}): {e}")
                     # Append fallback point?
                     tradeoffs.append({
                        'weight_a': weight_a, 'weight_b': weight_b,
                        'outcome_a': weight_a, 'outcome_b': weight_b, # Fallback value = weight
                        'confidence': 0.3
                     })
    ...
    def _find_optimal_compromise(self,
                                obj_a: str,
                                obj_b: str,
                                tradeoffs: List[Dict[str, Any]],
        ...
        if not tradeoffs:
             logger.warning("No tradeoff points provided to find optimal compromise. Using default equal weights.")
        ...
        logger.debug(f"Optimal compromise weights for {obj_a}/{obj_b}: {best_point_weights} (product={best_utility_product:.4f})")
        return best_point_weights


    def _estimate_outcomes_at_point(self,
                                    obj_a: str,
                                    obj_b: str,
        ...
        if reasoner:
             try:
                 ...
                 logger.debug(f"Estimated outcomes at weights {weights} using reasoner: {estimated_outcomes}")
             except Exception as e:
                  logger.warning(f"Prediction failed for outcome estimation at weights {weights}: {e}. Using fallback.")
        ...
        if not estimated_outcomes:
            ...
            logger.debug(f"Estimated outcomes at weights {weights} using fallback: {estimated_outcomes}")
    ...
    def _calculate_resolution_confidence(self,
                                         ...
                                         estimated_outcomes: Dict[str, float]) -> float:
        ...
        avg_tradeoff_confidence = _np.mean(tradeoff_confidences) if tradeoff_confidences else 0.5
        ...
        closest_point = min(tradeoffs, key=lambda p: abs(p['weight_a'] - optimal_point_weights.get(list(optimal_point_weights.keys())[0], 0.5)) )
    ...
    def _generate_resolution_reasoning(self,
        ...
        reason = (f"Resolved conflict between '{obj_a}' and '{obj_b}'. "
    ...
    def _compute_pareto_frontier_simple(self,
                                       objectives: List[str],
                                       objective_space: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i, candidate in enumerate(candidates):
            ...
            if not is_dominated:
                ...
                pareto_points.append({
                     ...
                     'confidence': candidate.get('confidence', 0.5) # Include confidence
                })
    ...
    def _dominates(self,
        ...
        for obj in objectives:
            ...
            if obj_data:
                if hasattr(obj_data, 'maximize'): maximize = getattr(obj_data, 'maximize', True)
                elif isinstance(obj_data, dict): maximize = obj_data.get('maximize', True)
    ...
    def _create_deadlock_result(self,
        ...
        agent_ids = []
        for p in proposals:
             if isinstance(p, AgentProposal): agent_ids.append(p.agent_id)
             elif isinstance(p, dict): agent_ids.append(p.get('agent_id', 'unknown'))
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for strategy_enum, perf in self.strategy_performance.items():
                 ...
                 strategy_key = strategy_enum.value if hasattr(strategy_enum, 'value') else str(strategy_enum)
            ...
            for strategy_enum in NegotiationStrategy:
                 if strategy_enum.value not in strategy_stats:
                      strategy_stats[strategy_enum.value] = {'successes': 0, 'failures': 0, 'success_rate': 0.0, 'total_runs': 0}


            return {
                'total_negotiations': self.stats['negotiations_performed'],
                'outcomes': {k.replace('outcome_', ''): v for k, v in self.stats.items() if k.startswith('outcome_')},
