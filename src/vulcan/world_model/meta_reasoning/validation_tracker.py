...
try:
    ...
    class FakeNumpy:
        @staticmethod
        def mean(lst): return sum(lst) / len(lst) if lst else 0.0 # Return float
        @staticmethod
        ...
        def diff(a, n=1, axis=-1, prepend=None, append=None):
             ...
             if not isinstance(a, list) or n != 1 or axis != -1:
                 raise NotImplementedError("FakeNumpy diff only supports n=1, axis=-1 on lists")
             if len(a) < 2: return []
        ...
        def std(a, axis=None, dtype=None, out=None, ddof=0, keepdims=False):
             ...
             if not isinstance(a, list): raise NotImplementedError("FakeNumpy std only supports lists")
             n = len(a)
             if n <= ddof: return float('nan') # Cannot compute std dev
...
try:
    ...
    logger.warning("Failed to import types from motivational_introspection. Using dummy types.")
    # Fallback definition if run standalone or MI not found
    class ProposalValidation: pass
    class ObjectiveStatus(Enum):
        ALIGNED = "aligned"; CONFLICT = "conflict"; VIOLATION = "violation";
        DRIFT = "drift"; ACCEPTABLE = "acceptable"; UNKNOWN = "unknown" # Add UNKNOWN
...
class ValidationRecord:
    ...
    actual_outcome: Optional[str] = None  # What actually happened after execution ('success', 'failure')
    ...
    def get_features(self) -> Dict[str, Any]:
        ...
        if 'objectives' in proposal_data:
            ...
            if isinstance(objectives_list, list) and all(isinstance(o, str) for o in objectives_list):
            ...
            elif isinstance(objectives_list, str): # Handle single objective string case
                 # **************************************************************************
                 # FIX: Change key to 'objectives' to match test
        ...
        if isinstance(weights_data, dict):
            ...
            numeric_weights = [v for v in weights_data.values() if isinstance(v, (int, float))]
            if numeric_weights:
                 features['weight_diversity'] = self._calculate_weight_diversity(numeric_weights) # Pass list
    ...
    def _calculate_weight_diversity(self, weights_list: List[float]) -> float:
        ...
        _np = np if NUMPY_AVAILABLE else FakeNumpy # Use internal alias
        if not weights_list: return 0.0
        ...
        max_entropy = _np.log2(num_weights) if num_weights > 1 else 1.0 # Use internal alias
...
class ValidationPattern:
    ...
    features: Dict[str, Any] # Features dict (keys are strings, values should be hashable)
    ...
    def __hash__(self):
        ...
        try:
             ...
             logger.warning(f"Unhashable feature value found in pattern: {self.features}")
             feature_key = tuple(sorted({k: str(v) for k, v in self.features.items()}.items())) # Convert values to str
    ...
    def __eq__(self, other):
        ...
        try:
             ...
             return (self.pattern_type == other.pattern_type and
                     feature_key_self == feature_key_other)
        except TypeError:
             # Fallback comparison if features contain unhashable types
             return (self.pattern_type == other.pattern_type and
                     str(self.features) == str(other.features)) # Less reliable fallback
...
class ValidationTracker:
    ...
    def __init__(self,
        ...
        self.self_improvement_drive = self_improvement_drive or MagicMock(_csiu_enabled=False)
    ...
    def record_validation(self,
                         ...
                         actual_outcome: Optional[str] = None) -> ValidationRecord:
        """
        ...
        with self.lock:
            ...
            if isinstance(validation_result, dict):
                 ...
                 validation_dict = self._make_serializable(validation_result) # Serialize dict input
                 ...
                 if hasattr(validation_result, '_extract_mock_name'): # Check if it's a Mock
                    validation_dict = {
                        'valid': valid,
                        'reasoning': self._make_serializable(getattr(validation_result, 'reasoning', None))
                    }
                 elif hasattr(validation_result, 'to_dict'):
                      try: validation_dict = self._make_serializable(validation_result.to_dict())
                      except Exception: validation_dict = self._make_serializable(vars(validation_result)) # Fallback
                 elif is_dataclass(validation_result):
                      try: validation_dict = self._make_serializable(asdict(validation_result))
                      except Exception: validation_dict = self._make_serializable(vars(validation_result)) # Fallback
                 else: # Fallback for unknown objects
                      validation_dict = self._make_serializable(validation_result)


            outcome = ValidationOutcome.APPROVED if valid else ValidationOutcome.REJECTED
            ...
            # Create record ID
            proposal_id = str(proposal.get('id', f'proposal_{time.time_ns()}_{self.stats.get("total_validations", 0)}'))
            ...
            try:
                ...
                metadata = {'features': features} # Store extracted features in metadata
            except Exception as e:
                 logger.error(f"Failed to extract features for proposal {proposal_id}: {e}", exc_info=True)
                 metadata = {'features': {}, 'feature_error': str(e)}


            # Add CSIU metadata if available (internal only)
            if not isinstance(self.self_improvement_drive, MagicMock) and getattr(self.self_improvement_drive, '_csiu_enabled', False):
                 try:
                     csiu_meta = self._get_csiu_metadata() # Use helper
                     if csiu_meta: metadata['csiu'] = csiu_meta
                 except Exception as e:
                     logger.debug(f"Failed to add CSIU metadata to record {proposal_id}: {e}")
            ...
            if self.learning_enabled:
                ...
                if total_validations % self.relearn_interval == 0:
                     logger.info(f"Triggering comprehensive relearn at validation #{total_validations}")
                     self._comprehensive_relearn()
                     self.last_relearn = total_validations


            # Periodic CSIU logging (maintainer only)
            if (not isinstance(self.self_improvement_drive, MagicMock) and
                getattr(self.self_improvement_drive, '_csiu_enabled', False) and
                total_validations % self.csiu_log_interval == 0):
                 logger.info(f"Triggering CSIU trend logging at validation #{total_validations}")
            ...
            if not isinstance(self.transparency_interface, MagicMock) and hasattr(self.transparency_interface, '_audit'):
                try:
                     audit_data = {
                          'proposal_id': proposal_id,
                          ...
                          'valid': valid,
                          'actual_outcome': actual_outcome,
                          'features_hash': hash(tuple(sorted(metadata.get('features',{}).items()))) # Hash features for audit
                     }
                     # Include CSIU summary in audit if present
                     if 'csiu' in metadata: audit_data['csiu_audit'] = metadata['csiu']

                     self.transparency_interface._audit('validation_recorded', audit_data)
                except Exception as e:
                     logger.debug(f"Failed to audit validation record via transparency interface: {e}")
    ...
    def _get_csiu_metadata(self) -> Optional[Dict[str, Any]]:
         ...
         try:
              ...
              if hasattr(self.self_improvement_drive, '_collect_telemetry_snapshot'):
                   telemetry_result = self.self_improvement_drive._collect_telemetry_snapshot()
                   if isinstance(telemetry_result, dict): cur_telemetry = telemetry_result

              # Extract key features used in longitudinal summary
              features = {
                   'A': cur_telemetry.get('A', 0.85), 'H': cur_telemetry.get('H', 0.06),
                   'C': cur_telemetry.get('C', 0.88), 'E': cur_telemetry.get('E', 0.50),
                   'U': cur_telemetry.get('U', 0.70), 'M': cur_telemetry.get('M', 0.02)
              }
    ...
    def summarize_longitudinal(self, window: int = 500) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            def avg(key: str, default: float = 0.0) -> float:
                vals = [r.metadata.get("csiu", {}).get("features", {}).get(key) for r in last if r.metadata and "csiu" in r.metadata]
                vals = [float(v) for v in vals if isinstance(v, (int, float))]
                return _np.mean(vals) if vals else default

            summary = {'window_size': len(last), 'timestamp': time.time()}
            features_to_avg = {"A": 0.85, "H": 0.06, "C": 0.88, "E": 0.50, "U": 0.70, "M": 0.02}
            feature_names = {"A": "alignment_coherence", "H": "entropy", "C": "intent_clarity",
                             "E": "empathy", "U": "user_satisfaction", "M": "miscomm"}
            ...
            if len(last) >= 20:
                ...
                def avg_half(records, key: str, default: float = 0.0) -> float:
                    vals = [r.metadata.get("csiu", {}).get("features", {}).get(key) for r in records if r.metadata and "csiu" in r.metadata]
                    vals = [float(v) for v in vals if isinstance(v, (int, float))]
                    return _np.mean(vals) if vals else default

                summary['delta_alignment'] = avg_half(second_half, "A") - avg_half(first_half, "A")
                summary['delta_entropy'] = avg_half(first_half, "H") - avg_half(second_half, "H") # Lower is better
                summary['delta_clarity'] = avg_half(second_half, "C") - avg_half(first_half, "C")
                summary['delta_empathy'] = avg_half(second_half, "E") - avg_half(first_half, "E")
                summary['delta_satisfaction'] = avg_half(second_half, "U") - avg_half(first_half, "U")
                summary['delta_miscomm'] = avg_half(first_half, "M") - avg_half(second_half, "M") # Lower is better
    ...
    def _log_csiu_trends(self):
        ...
        try:
            ...
            logger.info("[CSIU_TRENDS] Longitudinal summary (window: %d):", trends.get('window_size', 0))
            # Log main trends
            log_items = [
            ...
            if not isinstance(self.transparency_interface, MagicMock) and hasattr(self.transparency_interface, '_audit'):
                try:
                    ...
                    self.transparency_interface._audit("csiu_trends_summary", self._make_serializable(trends))
                except Exception as e:
                    logger.debug(f"Failed to add CSIU trends to transparency audit: {e}")
        ...
        with self.lock:
            if proposal_id in self.records_by_id:
                ...
                if actual_outcome not in valid_outcomes:
                     logger.warning(f"Invalid actual_outcome '{actual_outcome}' for {proposal_id}. Expected 'success' or 'failure'.")
                ...
                logger.debug(f"Updated actual outcome for {proposal_id}: {actual_outcome}")
            else:
                logger.warning(f"Could not find record for proposal_id '{proposal_id}' to update actual outcome.")


    # --- identify_risky_patterns, identify_success_patterns ---
    # (Copied from previous version)
    def identify_risky_patterns(self, min_support: Optional[int] = None) -> List[ValidationPattern]:
        """Identify patterns associated with rejected proposals"""
        with self.lock:
            min_support = min_support if min_support is not None else self.min_pattern_support
            # Filter patterns (ensure type comparison is safe)
            risky_patterns = [p for p in self.patterns if p.pattern_type == PatternType.RISKY and p.support >= min_support]
            # Sort by confidence then support
            risky_patterns.sort(key=lambda p: (p.confidence, p.support), reverse=True)
            return risky_patterns

    def identify_success_patterns(self, min_support: Optional[int] = None) -> List[ValidationPattern]:
        """Identify patterns associated with approved proposals"""
        with self.lock:
            min_support = min_support if min_support is not None else self.min_pattern_support
            success_types = {PatternType.SUCCESS, PatternType.SAFE}
            success_patterns = [p for p in self.patterns if p.pattern_type in success_types and p.support >= min_support]
            success_patterns.sort(key=lambda p: (p.confidence, p.support), reverse=True)
            return success_patterns

    # --- identify_blockers, analyze_failure_patterns ---
    # (Copied from previous version, ensuring self._np usage)
    def identify_blockers(self, objective: Optional[str] = None) -> List[ObjectiveBlocker]:
    ...
    def analyze_failure_patterns(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            rejected = [r for r in history_list if r.outcome == ValidationOutcome.REJECTED]
            total_records = len(history_list)

            if not rejected:
                return {'total_failures': 0, 'failure_rate': 0.0, 'common_features': [], 'temporal_pattern': None, 'failure_reasons': {}}
            ...
            for record in rejected:
                 ...
                 for feature, value in features.items():
                      try: feature_key = f"{feature}={value}" # Basic key
                      except TypeError: feature_key = f"{feature}=<unhashable_{type(value).__name__}>"
            ...
            temporal_pattern = self._analyze_temporal_pattern(timestamps) # Uses self._np
            ...
            return {
                ...
                'failure_rate': len(rejected) / total_records if total_records > 0 else 0.0,
    ...
    def predict_validation_outcome(self, proposal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.lock:
            try:
                 ...
                 return {'prediction': 'unknown', 'confidence': 0.0, 'risk_score': 0.5, 'reasoning': 'Feature extraction failed'}
            ...
            if exact_match_pattern:
                 ...
                 p = exact_match_pattern[0] if isinstance(exact_match_pattern, list) else exact_match_pattern
                 matched_patterns.append(p)
                 if p.pattern_type in [PatternType.RISKY, PatternType.FAILURE]: matching_risky = p.confidence
                 elif p.pattern_type in [PatternType.SUCCESS, PatternType.SAFE]: matching_success = p.confidence
            else:
                 ...
                 for pattern in self.patterns:
                      if self._features_match_pattern(features, pattern.features): # Check if proposal features contain pattern features
                           matched_patterns.append(pattern)
                           logger.debug(f"  Matched subset pattern: type={pattern.pattern_type.value}, conf={pattern.confidence:.2f}, support={pattern.support}, features={pattern.features}")
                           if pattern.pattern_type in [PatternType.RISKY, PatternType.FAILURE]:
                                matching_risky += pattern.confidence # Accumulate confidence for multiple matches? Weighted sum? Max? Simple sum for now.
                           elif pattern.pattern_type in [PatternType.SUCCESS, PatternType.SAFE]:
                                matching_success += pattern.confidence


            total_confidence_signal = matching_risky + matching_success
            logger.debug(f"  Total matching confidence signal: Success={matching_success:.2f}, Risky={matching_risky:.2f}")

            # Prediction logic (revised thresholds)
            risk_score = 0.5; prediction = 'unknown'; confidence = 0.0;
            reasoning = 'No strong matching patterns found.'

            if total_confidence_signal > 0.1: # Need some signal
                risk_score = matching_risky / total_confidence_signal if total_confidence_signal > 0 else 0.5
                ...
                if matching_success / total_confidence_signal > approve_threshold and matching_risky / total_confidence_signal < (1 - approve_threshold):
                ...
                elif matching_risky / total_confidence_signal > reject_threshold and matching_success / total_confidence_signal < (1 - reject_threshold):
                     ...
                     confidence = 1.0 - abs(matching_success - matching_risky) / total_confidence_signal
            ...
            logger.debug(f"  Final Prediction: {prediction}, Risk score: {risk_score:.2f}, Confidence: {confidence:.2f}")
    ...
    def get_learning_insights(self, limit: int = 10) -> List[LearningInsight]:
        ...
        with self.lock:
            ...
            risky = self.identify_risky_patterns(min_support=max(self.min_pattern_support, 5)) # Higher support for insight
            if risky:
                p = risky[0]
                insights.append(LearningInsight('high_risk_pattern', f'Proposals like {p.features} often rejected ({p.confidence:.1%})', [{'pattern': p.features, 'support': p.support}], p.confidence, 'Avoid or revise proposals matching this pattern', 'high'))
            # Insight 2: Success patterns
            successful = self.identify_success_patterns(min_support=max(self.min_pattern_support, 5))
            if successful:
                p = successful[0]
                insights.append(LearningInsight('success_pattern', f'Proposals like {p.features} often approved ({p.confidence:.1%})', [{'pattern': p.features, 'support': p.support}], p.confidence, 'Prioritize similar proposals', 'medium'))
            # Insight 3: Common blockers
            blockers = self.identify_blockers()
            if blockers:
                b = blockers[0]
                insights.append(LearningInsight('common_blocker', f'Objective "{b.objective}" often blocked by "{b.blocker_type}" ({b.frequency} times)', [{'blocker': b.blocker_type, 'freq': b.frequency}], min(1.0, b.frequency / 10), b.potential_solutions[0] if b.potential_solutions else 'Investigate blocker', 'high' if b.severity > 0.7 else 'medium'))
            ...
            if analysis.get('temporal_pattern'):
                ...
                insights.append(LearningInsight('temporal_trend', 'Failure temporal pattern detected.', [analysis['temporal_pattern']], 0.5, 'Investigate failure timing', 'low'))
            ...
            for obj, perf in obj_perf.items():
                if perf['failure_rate'] > 0.6 and perf['total'] >= 5:
                    insights.append(LearningInsight('objective_difficulty', f'Objective "{obj}" has high failure rate ({perf["failure_rate"]:.1%})', [perf], min(1.0, perf['total'] / 10), f'Review strategy for "{obj}"', 'high'))

            priority_order = {'high': 0, 'medium': 1, 'low': 2}
            insights.sort(key=lambda i: (priority_order.get(i.priority, 2), -i.confidence))
    ...
    def suggest_better_proxies(self, objective: str) -> List[Dict[str, Any]]:
        ...
        with self.lock:
            history_list = list(self.validation_records) # Snapshot
            relevant_records = [r for r in history_list if (r.proposal.get('objective') == objective or objective in r.proposal.get('objectives', [])) and r.actual_outcome is not None]
            if not relevant_records: return []
            ...
            for record in relevant_records:
                ...
                for feature, value in features.items():
                    try: feature_key = f"{feature}={value}"
                    except TypeError: feature_key = f"{feature}=<unhashable_{type(value).__name__}>"
                    feature_success_rates[feature_key]['total'] += 1
                    if success: feature_success_rates[feature_key]['success'] += 1
            ...
            for feature_key, counts in feature_success_rates.items():
                if counts['total'] >= min_support_proxy:
                    ...
                    if predictive_power > 0.5: # Higher threshold for suggestion
                        proxies.append({'proxy': feature_key, 'success_rate': success_rate, 'support': counts['total'], 'predictive': predictive_power})
    ...
    def _safe_get_reasoning(self, validation_result: Any) -> Optional[str]:
        ...
        reasoning = None
        if hasattr(validation_result, 'reasoning'): reasoning = validation_result.reasoning
        elif isinstance(validation_result, dict): reasoning = validation_result.get('reasoning')
        try: return str(reasoning) if reasoning is not None else None
    ...
    def _extract_proposal_features(self, proposal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(proposal, dict): return {} # Handle invalid input

        features = {}
        # Objective features
        obj = proposal.get('objective')
        if isinstance(obj, str): features['objective'] = obj
        ...
        if isinstance(domain, str): features['domain'] = domain

        # Action type
        action = proposal.get('action')
        if isinstance(action, str): features['action_type'] = action
        ...
        if isinstance(weights, dict):
             ...
             if numeric_weights:
                  features['weight_diversity'] = self._calculate_weight_diversity(numeric_weights)
    ...
    def _incremental_pattern_update(self, record: ValidationRecord):
        ...
        try:
             features = record.metadata.get('features', self._extract_proposal_features(record.proposal)) # Use stored features
             feature_key = tuple(sorted(features.items())) # Hashable key
        except Exception as e:
             logger.error(f"Could not process features for incremental update (Proposal {record.proposal_id}): {e}")
             return

        outcome = record.outcome
        logger.debug(f"Incremental update: Proposal {record.proposal_id}, Outcome {outcome.value}, Features Key Hash {hash(feature_key)}")
        ...
        if matching_pattern:
             logger.debug(f"  Updating existing pattern: type={matching_pattern.pattern_type.value}, features_hash={hash(feature_key)}")
             ...
             if record.proposal_id not in pattern.examples:
                  ...
                  if len(pattern.examples) > 10: pattern.examples.pop(0)

             # Update Bayesian confidence
             alpha = pattern.metadata.get('alpha', 1.0)
             beta = pattern.metadata.get('beta', 1.0)
             if outcome == ValidationOutcome.APPROVED: alpha += 1.0
             else: beta += 1.0 # Treat REJECTED/MODIFIED/UNKNOWN as evidence against approval

             pattern.metadata['alpha'] = alpha
             pattern.metadata['beta'] = beta
             new_confidence = alpha / (alpha + beta) if (alpha + beta) > 0 else 0.5
             logger.debug(f"    Updated Bayesian params: alpha={alpha:.1f}, beta={beta:.1f}, Confidence={new_confidence:.3f} (was {pattern.confidence:.3f})")
             pattern.confidence = new_confidence

             # Check if pattern type should flip based on new confidence
             current_is_success = pattern.pattern_type in [PatternType.SUCCESS, PatternType.SAFE]
             should_be_success = new_confidence > 0.5 # Simple threshold for type flip
             if current_is_success != should_be_success:
                  new_type = PatternType.SUCCESS if should_be_success else PatternType.RISKY
                  logger.info(f"  Flipping pattern type for features {hash(feature_key)}: {pattern.pattern_type.value} -> {new_type.value} (Confidence: {new_confidence:.3f})")
             ...
             initial_alpha = 1.5; initial_beta = 1.5 # Start slightly uncertain prior
             if outcome == ValidationOutcome.APPROVED: initial_alpha += 1.0
             else: initial_beta += 1.0

             initial_confidence = initial_alpha / (initial_alpha + initial_beta)
             pattern_type = PatternType.SUCCESS if initial_confidence >= 0.5 else PatternType.RISKY
             ...
             logger.debug(f"  Created new pattern: type={new_pattern.pattern_type.value}, conf={new_pattern.confidence:.2f}, features_hash={hash(feature_key)}")
    ...
    def _update_patterns_with_outcome(self, record: ValidationRecord):
        ...
        try:
             features = record.metadata.get('features', self._extract_proposal_features(record.proposal))
             feature_key = tuple(sorted(features.items()))
        except Exception as e:
             logger.error(f"Could not process features for outcome update (Proposal {record.proposal_id}): {e}")
             return

        actual_success = (record.actual_outcome == 'success')
        logger.debug(f"Updating patterns with actual outcome for {record.proposal_id}: {'success' if actual_success else 'failure'}")
        ...
        if matching_pattern:
             ...
             update_strength = 2.0 # Give actual outcome more weight than initial validation

             if actual_success: alpha += update_strength
             else: beta += update_strength

             matching_pattern.metadata['alpha'] = alpha
             matching_pattern.metadata['beta'] = beta
             new_confidence = alpha / (alpha + beta) if (alpha + beta) > 0 else 0.5
             logger.debug(f"  Adjusted pattern confidence for features {hash(feature_key)} to {new_confidence:.3f} based on actual outcome.")
             matching_pattern.confidence = new_confidence

             # Check if type needs flipping
             current_is_success = matching_pattern.pattern_type in [PatternType.SUCCESS, PatternType.SAFE]
             should_be_success = new_confidence > 0.5
             if current_is_success != should_be_success:
                  new_type = PatternType.SUCCESS if should_be_success else PatternType.RISKY
                  logger.info(f"  Flipping pattern type based on actual outcome for {hash(feature_key)}: {matching_pattern.pattern_type.value} -> {new_type.value}")
                  matching_pattern.pattern_type = new_type

        else:
             logger.debug(f"  No existing pattern found for features {hash(feature_key)} to update with actual outcome.")
    ...
    def _comprehensive_relearn(self):
        ...
        with self.lock: # Ensure exclusive access during rebuild
             logger.info("Starting comprehensive relearning from %d validation records...", len(self.validation_records))
             ...
             end_time = time.time()
             logger.info("Comprehensive relearning complete in %.2f seconds: %d patterns, %d blockers found.",
                        end_time - start_time, len(self.patterns), len(self.blockers))
    ...
    def _rebuild_patterns(self):
        ...
        for record in history_snapshot:
             try:
                 ...
                 logger.warning(f"Skipping record {record.proposal_id} during pattern rebuild due to feature error: {e}")

        logger.debug(f"Grouped {processed_count} records into {len(feature_groups)} unique feature sets.")
        ...
        for feature_key, records in feature_groups.items():
            if len(records) >= self.min_pattern_support:
                ...
                for r in records:
                    ...
                    if r.actual_outcome:
                         outcome_used = ValidationOutcome.APPROVED if r.actual_outcome == 'success' else ValidationOutcome.REJECTED

                    if outcome_used == ValidationOutcome.APPROVED: alpha += 1.0
                    else: beta += 1.0
                ...
                if confidence > 0.6: pattern_type = PatternType.SUCCESS
                elif confidence < 0.4: pattern_type = PatternType.RISKY
                else: continue # Skip patterns in the uncertain middle ground during rebuild? Or classify? Classify for now.
                ...
                new_pattern_index[feature_key] = pattern # Map key to the single pattern
                patterns_created += 1
    ...
    # (Copied from previous version)
    def _find_similar_records(self, record: ValidationRecord, threshold: float = 0.7) -> List[ValidationRecord]:
        """Find records similar to given record"""
        try:
             features = record.metadata.get('features', self._extract_proposal_features(record.proposal))
        except Exception: return [] # Cannot find similar if features fail

        similar = []
        history_snapshot = list(self.validation_records)
        for other_record in history_snapshot:
            if other_record.proposal_id == record.proposal_id: continue
            try:
                 other_features = other_record.metadata.get('features', self._extract_proposal_features(other_record.proposal))
    ...
    def _calculate_feature_similarity(self, features_a: Dict[str, Any], features_b: Dict[str, Any]) -> float:
        """Calculate similarity between feature sets using Jaccard index on key-value pairs"""
        if not features_a or not features_b: return 0.0

        # Convert feature dicts to sets of (key, value) tuples for Jaccard
        try:
             # Ensure values are hashable for set conversion
             set_a = set((k, v if isinstance(v, (str, int, float, bool, tuple, type(None))) else str(v)) for k, v in features_a.items())
             set_b = set((k, v if isinstance(v, (str, int, float, bool, tuple, type(None))) else str(v)) for k, v in features_b.items())
        except Exception as e:
             logger.warning(f"Could not create hashable sets for feature similarity: {e}")
    ...
    # (Copied from previous corrected version)
    def _features_match_pattern(self,
        ...
        for key, pattern_value in pattern_features.items():
            ...
            try:
                 ...
                 if isinstance(prop_value, list): prop_value = tuple(sorted(prop_value))
                 if isinstance(pattern_value, list): pattern_value = tuple(sorted(pattern_value))
                 # Convert tuple values similarly if needed (though features should ideally be flattened)
                 if isinstance(prop_value, tuple): prop_value = tuple(sorted(prop_value))
                 if isinstance(pattern_value, tuple): pattern_value = tuple(sorted(pattern_value))
    ...
    # (Copied from previous version, ensuring self._np usage)
    def _analyze_temporal_pattern(self, timestamps: List[float]) -> Optional[Dict[str, Any]]:
        """Analyze temporal pattern in timestamps"""
        _np = self._np
        if len(timestamps) < 10: return None
        ...
        return {'mean_interval': float(mean_interval), 'std_interval': float(std_interval), 'regularity': float(regularity)}

    def _analyze_objective_performance(self) -> Dict[str, Dict[str, Any]]:
        """Analyze performance (approval/failure rate) by objective"""
        objective_stats = defaultdict(lambda: {'total': 0, 'approved': 0, 'rejected': 0})
        ...
        for record in history_snapshot:
            ...
            if 'objective' in features: objectives_in_proposal.add(features['objective'])
            # Assumes 'objectives_tuple' exists if multiple objectives were present
            # **************************************************************************
            # FIX: Read from 'objectives' key
            if 'objectives' in features: objectives_in_proposal.update(features['objectives'])
            # **************************************************************************

            # Determine outcome (prefer actual if available)
            outcome = record.outcome
            if record.actual_outcome:
                 outcome = ValidationOutcome.APPROVED if record.actual_outcome == 'success' else ValidationOutcome.REJECTED

            for obj in objectives_in_proposal:
                 if not obj or not isinstance(obj, str): continue # Skip invalid
    ...
    def detect_blockers_from_history(self):
        ...
        with self.lock:
             ...
             rejected = [r for r in self.validation_records if r.outcome == ValidationOutcome.REJECTED]
             blockers_by_objective = defaultdict(lambda: defaultdict(list)) # obj -> type -> instances
             ...
             for record in rejected:
                  ...
                  if reasoning:
                       ...
                       if blocker_type != 'unknown':
                            instance = {'reasoning': reasoning, 'proposal_id': record.proposal_id}
                            for obj in objectives_in_proposal:
                                 if obj and isinstance(obj, str): # Ensure valid objective name
                                      blockers_by_objective[obj][blocker_type].append(instance)


             # Create blocker objects, replacing existing ones
             new_blockers = []
             new_blocker_index = defaultdict(list)
             total_record_count = len(self.validation_records) # Use total count for severity normalization

             for obj, types in blockers_by_objective.items():
                  # Calculate total attempts for this objective to normalize severity
                  obj_attempts = sum(1 for r in self.validation_records if obj in r.metadata.get('features',{}).get('objectives',[]) or r.metadata.get('features',{}).get('objective') == obj)

                  for blocker_type, instances in types.items():
                       frequency = len(instances)
                       if frequency >= self.min_pattern_support: # Only consider blockers with minimum support
                            ...
                            most_common_desc = Counter(descriptions).most_common(1)[0][0] if descriptions else f'{blocker_type} blocks {obj}'

                            blocker = ObjectiveBlocker(
                                objective=obj, blocker_type=blocker_type, description=most_common_desc,
                                frequency=frequency, severity=severity_score,
                                examples=[i['proposal_id'] for i in instances[:5]],
                                potential_solutions=self._suggest_solutions(blocker_type, obj)
                            )
    ...
    def _classify_blocker(self, reasoning: str) -> str:
        ...
        if not isinstance(reasoning, str): return 'unknown'
        r = reasoning.lower()
        # Order checks from specific to general
        if 'constraint violation' in r or 'violates constraint' in r:
            if 'minimum' in r: return 'constraint_violation_min'
            if 'maximum' in r: return 'constraint_violation_max'
            return 'constraint_violation'
        # **************************************************************************
        # FIX: Check for both keywords
        if 'objective' in r and 'conflict' in r: return 'objective_conflict'
        # **************************************************************************
        if 'goal drift' in r: return 'goal_drift'
        if 'unacceptable tradeoff' in r: return 'unacceptable_tradeoff'
        if 'risky pattern' in r: return 'risky_pattern_match'
        if 'predicted failure' in r or ('rejected' in r and 'historical pattern' in r): return 'predicted_failure'
        if 'pathology' in r: return 'objective_pathology' # Generic pathology
        if 'insufficient resource' in r: return 'insufficient_resources'
        if 'insufficient data' in r: return 'insufficient_data'
        if 'insufficient' in r or 'lacking' in r: return 'insufficient_info' # Generic insufficient
        return 'unknown'
    ...
    def _suggest_solutions(self, blocker_type: str, objective: str) -> List[str]:
        ...
        solutions_map = {
            'objective_conflict': ['Use multi-objective optimization (e.g., Pareto)', 'Prioritize objectives explicitly', 'Negotiate compromise'],
            'constraint_violation': [f'Relax non-critical constraints for {objective}', f'Reformulate proposal for {objective}', f'Check feasibility for {objective} within limits'],
            'constraint_violation_min': [f'Ensure actions respect minimum for {objective}', f'Adjust proposal to increase {objective} value'],
            'constraint_violation_max': [f'Ensure actions respect maximum for {objective}', f'Adjust proposal to decrease {objective} value'],
            'goal_drift': ['Realign proposal with core design spec', 'Clarify objective definitions', 'Add drift monitoring'],
            'insufficient_resources': [f'Allocate more resources towards {objective}', 'Optimize resource usage', f'Adjust target for {objective}'],
            'insufficient_data': [f'Gather more data for {objective}', f'Initiate exploration for {objective} data'],
            'objective_pathology': [f'Review proposal for {objective} against spec', 'Analyze historical failures'],
            'unacceptable_tradeoff': [f'Re-evaluate tradeoff involving {objective}', 'Prioritize critical objectives'],
            'risky_pattern_match': [f'Analyze why pattern fails for {objective}', 'Modify proposal to avoid risky features'],
            'predicted_failure': [f'Review historical failures for {objective}', 'Modify proposal based on past failures'],
            'insufficient_info': [f'Gather more information relevant to {objective} or proposal context'],
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            stats_output = {
                ...
                'approval_rate': (approved_validations / total_validations) if total_validations > 0 else 0.0,
                'actual_success_rate': (actual_successes / total_actuals) if total_actuals > 0 else None, # Rate based on actuals
    ...
    def _make_serializable(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            ...
            if hasattr(data, '_extract_mock_name'): return f"<MagicMock name='{data._extract_mock_name()}' id='{id(data)}'>"
            return {self._make_serializable(k): self._make_serializable(v) for k, v in data.items()}
            ...
            return data.value
             ...
             try: return self._make_serializable(data.to_dict()) # Recurse on result
             except Exception: return str(data) # Fallback
        # Handle dataclasses AFTER to_dict check
        elif is_dataclass(data) and not isinstance(data, type):
             try: return self._make_serializable(asdict(data)) # Recurse on result
             except Exception: return str(data) # Fallback
