...
class EthicalBoundary:
    ...
    constraint_function: Optional[Callable[[Dict[str, Any]], bool]] = None  # Returns True if OK
    constraint_rules: List[Dict[str, Any]] = field(default_factory=list)  # Rule-based constraints
            ...
            if condition == 'exists':
                ...
                try:
                    return value < rule_value if rule_value is not None else False
                     ...
                     logger.warning(f"TypeError comparing field '{field_name}' ({type(value)}) with value ({type(rule_value)}) for rule {rule}")
                ...
                try:
                 ...
                 try:
                     ...
                     logger.warning(f"TypeError comparing field '{field_name}' ({type(value)}) with range ({type(min_val)}, {type(max_val)}) for rule {rule}")
                     return False
...
class EthicalBoundaryMonitor:
    ...
    def __init__(self,
        ...
        self.violations_by_boundary: Dict[str, List[EthicalViolation]] = defaultdict(list)
    ...
    def add_boundary(self,
                     ...
                     modification_function: Optional[Callable[[Dict[str, Any]], Dict[str, Any]]] = None,
                     applies_when: Optional[Callable[[Dict[str, Any]], bool]] = None,
                     metadata: Optional[Dict[str, Any]] = None) -> EthicalBoundary:
        """
    ...
    def get_boundaries(self, category: Optional[BoundaryCategory] = None) -> Dict[str, EthicalBoundary]:
        """
        ...
        with self.lock:
            ...
            return {name: self.boundaries[name] for name in self.boundary_index.get(category, []) if name in self.boundaries}
    
    def check_action(self,
                    action: Dict[str, Any],
                    context: Optional[Dict[str, Any]] = None) -> Tuple[bool, Optional[EthicalViolation]]:
        """
        ...
        with self.lock:
            ...
            for boundary_name in list(self.boundaries.keys()):
                ...
                if not boundary: continue # Boundary might have been removed concurrently
                ...
                if boundary.applies_when:
                     try:
                         if not boundary.applies_when(context):
                          ...
                          logger.error(f"Error checking applies_when for {boundary_name}: {e}")
            ...
            violations.sort(key=lambda v: self._severity_to_numeric(v.severity), reverse=True)
            ...
            if most_severe.action_modified:
                 most_severe.modified_action = final_action # Store the final modified action
    ...
    def detect_boundary_violations(self,
                                   proposal: Dict[str, Any],
                                   context: Optional[Dict[str, Any]] = None) -> List[EthicalViolation]:
        """
        ...
        with self.lock:
            ...
            temp_boundaries = {name: boundary for name, boundary in self.boundaries.items()}
                ...
                if boundary.applies_when:
                     try:
                         if not boundary.applies_when(context):
                          ...
                          logger.debug(f"Error checking applies_when for {boundary_name} during detection: {e}")
                          continue
                ...
                if boundary.constraint_function:
                     try:
                         is_ok = boundary.constraint_function(proposal)
                          ...
                          logger.debug(f"Error checking constraint_function for {boundary_name} during detection: {e}")
                          is_ok = False # Assume violation on error
                # Check rules otherwise
                elif boundary.constraint_rules:
                     try:
                          is_ok = all(boundary._check_rule(rule, proposal) for rule in boundary.constraint_rules)
                     except Exception as e:
                          logger.debug(f"Error checking rules for {boundary_name} during detection: {e}")
                          is_ok = False # Assume violation on error
    ...
    def get_violations(self,
                       ...
                       limit: Optional[int] = None) -> List[EthicalViolation]:
        """
        ...
        with self.lock:
            ...
            # Filter by severity
            if severity:
                violations_snapshot = [v for v in violations_snapshot if v.severity == severity]
            
            # Filter by category
            if category:
                violations_snapshot = [v for v in violations_snapshot if v.category == category]
    ...
    def get_violation_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            recent = sorted(violations_snapshot, key=lambda v: v.timestamp, reverse=True)[:10]
        ...
        with self.lock:
            ...
            if not self.shutdown_triggered:
                ...
                if self.shutdown_callback:
                 ...
                 logger.warning(f"Shutdown already triggered (Reason: {self.shutdown_reason}). Ignoring new trigger: {reason}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'actions_allowed_with_warnings': self.stats['actions_allowed_with_warnings'],
                'actions_blocked': self.stats['actions_blocked'],
                'total_violations': len(violations_snapshot),
                'violations_by_severity': self._count_by_severity(violations_snapshot), # Pass snapshot
                'violations_by_category': self._count_by_category(violations_snapshot), # Pass snapshot
                'boundaries_defined': len(self.boundaries),
                'boundaries_by_category': {
                    cat.value: len(bounds) for cat, bounds in self.boundary_index.items() if bounds
                },
                'shutdown_triggered': self.shutdown_triggered,
                'shutdown_reason': self.shutdown_reason,
                'enforcement_actions_recorded': len(enforcement_snapshot), # Use snapshot length
    ...
    def export_state(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'violations': [v.to_dict() for v in violations_snapshot[-1000:]],  # Last 1000
    ...
    def import_state(self, state: Dict[str, Any]):
        ...
        with self.lock:
            ...
            for name, boundary_dict in imported_boundaries.items():
                if name not in self.boundaries: # Avoid overwriting existing boundaries with only metadata
                    try:
                        ...
                        boundary = EthicalBoundary(
                            ...
                            enforcement_level=EnforcementLevel(boundary_dict['enforcement_level']),
                            severity_if_violated=ViolationSeverity(boundary_dict['severity_if_violated']),
                            metadata=boundary_dict.get('metadata', {})
                        )
                        # Restore stats from dict
                        boundary.check_count = boundary_dict.get('check_count', 0)
                        boundary.violation_count = boundary_dict.get('violation_count', 0)
                        ...
                        if name not in self.boundary_index[category]:
                             self.boundary_index[category].append(name)
                        logger.warning(f"Imported boundary {name} metadata only - functions not serializable")
                    except (KeyError, ValueError, TypeError) as e:
                         logger.error(f"Failed to import boundary '{name}' due to invalid data: {e}. Boundary dict: {boundary_dict}")
                else:
                    # Optionally update stats for existing boundaries
                    existing_boundary = self.boundaries[name]
                    existing_boundary.check_count = boundary_dict.get('check_count', existing_boundary.check_count)
                    existing_boundary.violation_count = boundary_dict.get('violation_count', existing_boundary.violation_count)
                    existing_boundary.last_checked = boundary_dict.get('last_checked', existing_boundary.last_checked)
            ...
            for viol_dict in imported_violations:
                try:
                    ...
                    viol = EthicalViolation(
                        action=viol_dict.get('action', {}), # Action might be complex/unserializable
                        boundary_violated=boundary_name,
                        category=category,
                        severity=severity,
                    ...
                    logger.error(f"Failed to import violation {viol_dict.get('violation_id')}: {e}. Violation dict: {viol_dict}")
            ...
            # Import config
            self.strict_mode = state.get('config', {}).get('strict_mode', self.strict_mode) # Use current as default
            
            logger.info(f"Imported state from persistence. Total boundaries: {len(self.boundaries)}, Violations in deque: {len(self.violations)}")
    ...
    def reset(self) -> None:
        ...
        with self.lock:
            ...
            logger.info("EthicalBoundaryMonitor reset - violations and stats cleared, boundaries kept")
    ...
    def _initialize_default_boundaries(self):
        ...
        except ValueError: pass # Ignore if already exists
        ...
        try:
             self.add_boundary(
                 ...
                 constraint_function=lambda action: not action.get('causes_psychological_harm', False)
             )
        ...
        try:
             self.add_boundary(
                 ...
                 enforcement_level=EnforcementLevel.BLOCK,
                 severity=ViolationSeverity.HIGH,
                 constraint_function=lambda action: not action.get('leaks_private_data', False)
             )
        except ValueError: pass
        ...
        try:
             self.add_boundary(
                 ...
                 constraint_function=lambda action: action.get('data_collection_justified', True)
             )
        except ValueError: pass
        ...
        try:
             self.add_boundary(
                 ...
                 enforcement_level=EnforcementLevel.BLOCK,
                 severity=ViolationSeverity.HIGH,
                 constraint_function=lambda action: not action.get('discriminates_by_demographics', False)
             )
        except ValueError: pass
        ...
        try:
             self.add_boundary(
                 ...
                 enforcement_level=EnforcementLevel.BLOCK,
                 severity=ViolationSeverity.HIGH,
                 constraint_function=lambda action: action.get('has_user_consent', True) or not action.get('requires_consent', False)
             )
        except ValueError: pass
        ...
        try:
             self.add_boundary(
                 ...
                 enforcement_level=EnforcementLevel.BLOCK,
                 severity=ViolationSeverity.HIGH,
                 constraint_function=lambda action: not action.get('is_deceptive', False)
             )
        except ValueError: pass
        ...
        logger.info(f"Initialized default ethical boundaries (if not already present). Total: {len(self.boundaries)}")
    ...
    def _record_violation(self, violation: EthicalViolation):
        ...
        # Log violation
        log_level = logging.CRITICAL if violation.severity == ViolationSeverity.CRITICAL else \
                   logging.ERROR if violation.severity == ViolationSeverity.HIGH else \
                   logging.WARNING if violation.severity == ViolationSeverity.MEDIUM else \
                   logging.INFO # Use INFO for LOW severity

        logger.log(log_level, f"Ethical violation detected: ID={violation.violation_id}, Boundary='{violation.boundary_violated}', Severity={violation.severity.value}, Desc='{violation.description}'")
        ...
        if self.validation_tracker:
            try:
                ...
                validation_result_data = {'valid': False, 'violation': violation.to_dict()}
                proposal_data = {'type': 'ethical_check', 'action_hash': hash(str(violation.action))} # Hash action if too large
        ...
        if self.transparency_interface:
            try:
                self.transparency_interface.record_ethical_violation(violation.to_dict())
            except Exception as e:
                logger.debug(f"Failed to record to transparency interface: {e}")
    
    def _enforce_violation(self, violation: EthicalViolation) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """
        ...
        if self.strict_mode and enforcement_level != EnforcementLevel.SHUTDOWN:
            logger.warning(f"Strict mode: Escalating enforcement for {violation.boundary_violated} from {enforcement_level.value} to BLOCK.")
            enforcement_level = EnforcementLevel.BLOCK
        
        # Store the effective enforcement level taken in the violation record
        violation.enforcement_action = enforcement_level # Update violation object directly
        ...
        if enforcement_level == EnforcementLevel.MONITOR:
            ...
            # Warn but allow
            logger.warning(f"Ethical warning issued for violation {violation.violation_id}: {violation.description}")
            is_allowed = True
            ...
            if boundary and boundary.modification_function:
                try:
                    ...
                    logger.info(f"Action modified by {violation.boundary_violated} due to violation {violation.violation_id}.")
                    is_allowed = True
                except Exception as e:
                    logger.error(f"Modification function for {violation.boundary_violated} failed: {e}. Blocking action.")
                ...
                logger.warning(f"No modification function for {violation.boundary_violated}. Blocking action for violation {violation.violation_id}.")
            ...
            # Block action
            logger.error(f"Action blocked due to ethical violation {violation.violation_id}: {violation.description}")
            ...
            # Trigger shutdown
            shutdown_reason = f"Critical violation {violation.violation_id}: {violation.description}"
            ...
            final_action = None # Action is blocked (system shutting down)
             ...
             logger.error(f"Unknown enforcement level: {enforcement_level}. Allowing action with warning.")
    ...
    def _severity_to_numeric(self, severity: ViolationSeverity) -> int:
        ...
        sev_member = severity if isinstance(severity, ViolationSeverity) else ViolationSeverity(severity)
    ...
    def _count_by_severity(self, violations_list: Optional[List[EthicalViolation]] = None) -> Dict[str, int]:
    ...
    def _count_by_category(self, violations_list: Optional[List[EthicalViolation]] = None) -> Dict[str, int]:
