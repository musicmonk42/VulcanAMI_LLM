...
try:
    ...
    class FakeNumpy:
        ...
        def std(a, axis=None, dtype=None, out=None, ddof=0, keepdims=False):
             ...
             variance = sum((x - mean_val) ** 2 for x in a) / max(1, (n - ddof)) # Avoid div by zero if n=ddof
        ...
        def ones(shape, dtype=None):
             ...
             if isinstance(shape, int): return [1.0] * shape
             if isinstance(shape, (list, tuple)) and len(shape) == 1: return [1.0] * shape[0]
        ...
        def diff(a, n=1, axis=-1, prepend=None, append=None):
             ...
             try: result = [a[i] - a[i-1] for i in range(1, len(a))]
             except TypeError:
        ...
        @staticmethod
        def sqrt(x):
             if isinstance(x, list): return [math.sqrt(i) if i >= 0 else float('nan') for i in x]
        ...
        def convolve(a, v, mode='full'):
             ...
             if mode != 'valid' or not isinstance(a, list) or not isinstance(v, list):
                 raise NotImplementedError("FakeNumpy convolve simplified for mode='valid' on lists only")
             if len(v) > len(a): return []
             # This is NOT a real convolution, just a placeholder
             k = len(v)
             return [sum(a[i:i+k]) for i in range(len(a) - k + 1)] # Incorrect math, placeholder only
        @staticmethod
        def exp(x):
             if isinstance(x, list): return [math.exp(i) for i in x]
             return math.exp(x)
        @staticmethod
        def linspace(start, stop, num=50, endpoint=True, retstep=False, dtype=None, axis=0):
             ...
             if num <= 0: return []
             if endpoint:
                 if num == 1: step = 0.0; result = [float(start)] # Handle num=1
                 else: step = (stop - start) / (num - 1)
             else:
                 step = (stop - start) / num
             ...
             if endpoint and num > 1: result[-1] = float(stop)

             if retstep: return result, float(step)
             return result
        @staticmethod
        def corrcoef(x, y=None, rowvar=True):
            # Simplified Pearson correlation for two 1D lists x and y
            if y is None: y = x # Correlation matrix of x with itself (diag=1)
            if not isinstance(x, list) or not isinstance(y, list) or len(x) != len(y) or len(x) < 2:
                 ...
                 if size > 0: return [[1.0 if i==j else 0.0 for j in range(size)] for i in range(size)]
        ...
        @staticmethod
        def isnan(x):
             if isinstance(x, list): return [math.isnan(i) for i in x]
             return math.isnan(x)
...
class ValueTrajectory:
    ...
    def add_point(self, timestamp: float, value: float):
        ...
        try:
             ...
             logger.warning(f"Invalid timestamp or value for trajectory {self.value_name}: ts={timestamp}, val={value}")
    ...
    def get_std(self) -> float:
        ...
        return float(_np.std(self.values, ddof=1)) if len(self.values) > 1 else 0.0 # Use sample std dev (ddof=1), ensure float
    ...
    def get_trend(self) -> Tuple[float, float]:
        ...
        try:
            ...
            slope, intercept = result[0] # First element of result tuple is the solution [slope, intercept]

            return (float(slope), float(intercept))
        except Exception as e:
            # Fallback if least squares fails (e.g., with FakeNumpy or other issues)
            logger.error(f"Linear regression failed for trajectory {self.value_name}: {e}")
    ...
    def get_volatility(self) -> float:
        ...
        if abs(mean) < 1e-9:
            ...
            return 0.0 if abs(std_dev) < 1e-9 else 1.0 # Return 1.0 as capped high volatility
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        def _make_serializable(data):
            ...
            if isinstance(data, dict):
                ...
                try: return data.to_dict() # Assumes to_dict returns serializable
                except Exception: return str(data)
            elif NUMPY_AVAILABLE and isinstance(data, (_np.ndarray, _np.generic)):
                if isinstance(data, _np.ndarray): return data.tolist()
                elif isinstance(data, _np.generic): return data.item() # Convert numpy scalars
        ...
        return {
            ...
            'drift_alerts': [_make_serializable(a) for a in self.drift_alerts], # Use helper on list items
...
class ValueEvolutionTracker:
    ...
    def __init__(self,
        ...
        self.cusum_state: Dict[str, Dict[str, float]] = {}  # value -> {cusum_pos, cusum_neg, baseline}

        # Drift alerts history
        self.drift_alerts: deque[DriftAlert] = deque(maxlen=1000) # Use deque for alerts too
        ...
        self.min_history_for_drift = 10  # Minimum history points before running drift detection
        ...
        logger.info(f"  CUSUM slack param (relative): {self.cusum_slack}, Anomaly Z-threshold: {self.anomaly_z_threshold}")


    def record_value_state(self,
                          values: Dict[str, float],
                          metadata: Optional[Dict[str, Any]] = None) -> ValueState:
        ...
        with self.lock:
             ...
             for k, v in values.items():
                  try: processed_values[k] = float(v)
                  except (ValueError, TypeError):
             ...
             for value_name, value in processed_values.items():
                 if value_name not in self.trajectories:
                     self.trajectories[value_name] = ValueTrajectory(value_name=value_name)
                 ...
                 if value_name not in self.cusum_state:
                     self.cusum_state[value_name] = {'cusum_pos': 0.0, 'cusum_neg': 0.0, 'baseline': value}

             # Set baseline automatically if first time reaching enough data
             if not self.baseline_set and len(self.value_history) >= 5: # Use small window for initial baseline
             ...
             if self.baseline_set and len(self.value_history) >= self.min_history_for_drift:
             ...
             if not isinstance(self.transparency_interface, MagicMock) and hasattr(self.transparency_interface, 'record_value_state'):
                 try:
                     # Pass serializable data
                     self.transparency_interface.record_value_state(values=processed_values, timestamp=state.timestamp)
                 except Exception as e:
                     logger.debug(f"Failed to record value state to transparency interface: {e}")

             logger.debug(f"Recorded value state at {state.timestamp}: {len(processed_values)} values.")
    ...
    def detect_drift(self,
        ...
        with self.lock:
            ...
            if len(self.value_history) < self.min_history_for_drift:
                return {'drift_detected': False, 'reason': 'Insufficient history', 'history_size': len(self.value_history), 'required': self.min_history_for_drift}

            # Use provided threshold or instance default
            effective_threshold = threshold if threshold is not None else self.drift_threshold
            ...
            if value_name:
                 if value_name not in self.trajectories:
                      return {'drift_detected': False, 'reason': f"Value '{value_name}' not tracked."}
                 values_to_check = [value_name]
            else:
                 values_to_check = list(self.trajectories.keys())

            if not values_to_check:
                 return {'drift_detected': False, 'reason': 'No values currently tracked.'}
            ...
            for vname in values_to_check:
                ...
                if not trajectory or len(trajectory.values) < self.min_history_for_drift:
                     drift_details[vname] = {'drift_detected': False, 'drift_score': 0.0, 'reason': 'Insufficient data'}
                     continue

                # Run multiple detection methods
                # Note: These methods return scores, not just booleans
                cusum_drift = self._detect_drift_cusum(vname, effective_threshold) # Uses internal state
                change_point_drift = self._detect_change_point(vname) # Uses trajectory
                trend_drift = self._detect_trend_drift(vname, effective_threshold) # Uses trajectory

                # Aggregate drift score (e.g., weighted average or max)
                # Using max emphasizes sensitivity to any drift type
                drift_score = max(cusum_drift['score'], change_point_drift['score'], trend_drift['score'])
                ...
                if drift_detected:
                    ...
                    logger.debug(f"Drift detected for '{vname}' (Score: {drift_score:.3f} > Threshold: {effective_threshold:.3f})")
    ...
    def analyze_evolution(self,
                         ...
                         use_cache: bool = True) -> ValueEvolutionAnalysis:
        """Comprehensive analysis of value evolution over a time window."""
        _np = self._np # Use internal alias
        with self.lock:
            current_time = time.time()
            # Check cache
            if use_cache and self.last_analysis and (current_time - self.last_analysis_time < self.analysis_cache_ttl):
                logger.debug("Returning cached evolution analysis.")
                return self.last_analysis

            # Determine time window
            if not self.value_history: return self._empty_analysis()
            end_time = self.value_history[-1].timestamp
            start_time = end_time - time_window if time_window else self.value_history[0].timestamp
            ...
            states_in_window = [s for s in history_snapshot if start_time <= s.timestamp <= end_time]


            if not states_in_window:
                 logger.warning("No value states found within the specified time window.")
                 return self._empty_analysis()

            logger.debug(f"Analyzing evolution over {len(states_in_window)} states (Window: {start_time:.1f} - {end_time:.1f}).")
            ...
            for vname in all_value_names:
                ...
                window_indices = [i for i, ts in enumerate(trajectory.timestamps) if start_time <= ts <= end_time]
                if len(window_indices) < 2: # Need at least 2 points for trend/stability
                ...
                slope, intercept = self._calculate_trend_slope_intercept(window_timestamps, window_values) # Helper using lstsq
                std_dev = _np.std(window_values, ddof=1) if len(window_values) > 1 else 0.0
                mean_val = _np.mean(window_values)
                time_range = window_timestamps[-1] - window_timestamps[0] if len(window_timestamps) > 1 else 1.0
                ...
                relative_change = (total_change / abs(mean_val)) if abs(mean_val) > 1e-9 else (float('inf') if total_change > 1e-9 else 0.0)
                std_normalized_change = (total_change / std_dev) if std_dev > 1e-9 else (float('inf') if total_change > 1e-9 else 0.0)

                # Determine if trend is significant
                is_significant_trend = (relative_change > relative_change_threshold or
                                        std_normalized_change > std_dev_change_threshold or
                                        total_change > min_absolute_change)
                ...
                # Check volatility
                volatility = abs(std_dev / mean_val) if abs(mean_val) > 1e-9 else (1.0 if std_dev > 1e-9 else 0.0)
                ...
                drift_score = drift_result_full.get('drift_details', {}).get(vname, {}).get('drift_score', 0.0)
            ...
            recent_alerts = [alert for alert in list(self.drift_alerts) if start_time <= alert.timestamp <= end_time]


            # Overall stability (average of individual stabilities)
            stability_scores = list(value_stability_scores.values())
            overall_stability = float(_np.mean(stability_scores)) if stability_scores else 1.0


            # Alignment consistency (based on correlations)
            correlation_matrix = self._compute_correlation_matrix(window_start_time=start_time, window_end_time=end_time) # Pass window times
            alignment_consistency = 0.0
            if correlation_matrix:
                abs_correlations = [abs(c) for c in correlation_matrix.values()]
                if abs_correlations: alignment_consistency = float(_np.mean(abs_correlations))
    ...
    def get_value_trajectory(self, value_name: str, time_window: Optional[float] = None) -> List[Tuple[float, float]]:
        ...
        with self.lock:
            ...
            filtered_data = [
                (ts, val) for ts, val in zip(trajectory.timestamps, trajectory.values)
                if ts >= cutoff_time
            ]
    ...
    def predict_future_value(self, value_name: str, steps_ahead: int = 1) -> List[float]:
        """Predict future value using linear extrapolation based on trajectory trend."""
        _np = self._np
        with self.lock:
            trajectory = self.trajectories.get(value_name)
            if not trajectory:
                logger.warning(f"Cannot predict future value: trajectory for '{value_name}' not found.")
                # **************************************************************************
            ...
            if len(trajectory.values) < 2:
                ...
                logger.debug(f"Insufficient data for trend prediction for '{value_name}'. Returning last value.")
            ...
            time_deltas = _np.diff(trajectory.timestamps[-10:]) if len(trajectory.timestamps) > 1 else [1.0] # Use diff
            
            avg_time_delta = _np.mean(time_deltas) if len(time_deltas) > 0 else 1.0
            
            if avg_time_delta <= 0: avg_time_delta = 1.0 # Ensure positive delta
            ...
            logger.debug(f"Predicted future values for '{value_name}' ({steps_ahead} steps): {predictions}")
            return predictions
    ...
    def set_baseline(self, values: Optional[Dict[str, float]] = None):
        ...
        with self.lock:
            if values is not None and isinstance(values, dict):
                 ...
                 self.baseline_values = {k: float(v) for k, v in values.items() if isinstance(v, (int, float))}
                 logger.info(f"Manually set baseline for {len(self.baseline_values)} values.")
            else:
                 logger.info("Auto-setting baseline from current trajectory means/EWMA...")
                 self._set_baseline() # Call internal method to calculate from history

            self.baseline_set = bool(self.baseline_values) # True if baseline has values
            if not self.baseline_set: logger.warning("Baseline could not be set (no values specified or calculated).")


    # --- get_alerts ---
    # (Copied from previous version)
    def get_alerts(self, severity: Optional[DriftSeverity] = None, limit: Optional[int] = None) -> List[DriftAlert]:
        ...
        with self.lock:
            ...
            if severity is not None:
                if not isinstance(severity, DriftSeverity):
                     try: severity = DriftSeverity(severity) # Allow string input
                     except ValueError: logger.warning(f"Invalid severity filter: {severity}"); return []
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'uptime_seconds': time.time() - stats_snapshot.get('initialized_at', time.time())
            }
    ...
    def import_state(self, state: Dict[str, Any]):
        ...
        with self.lock:
             ...
             if isinstance(config_state, dict):
                 self.max_history = int(config_state.get('max_history', self.max_history))
                 self.drift_threshold = float(config_state.get('drift_threshold', self.drift_threshold))
                 self.ewma_alpha = float(config_state.get('ewma_alpha', self.ewma_alpha))
                 self.cusum_slack = float(config_state.get('cusum_slack', self.cusum_slack))
                 self.anomaly_z_threshold = float(config_state.get('anomaly_z_threshold', self.anomaly_z_threshold))
                 self.min_history_for_drift = int(config_state.get('min_history_for_drift', self.min_history_for_drift))
                 ...
                 for state_dict in imported_history:
                      ...
                      try:
                           value_state = ValueState(
                               ...
                               values={k: float(v) for k,v in state_dict.get('values',{}).items()}, # Ensure floats
                               metadata=state_dict.get('metadata', {}),
                               state_id=state_dict.get('state_id', f"imported_{time.time_ns()}")
                           )
                           self.value_history.append(value_state)
                           # Rebuild trajectories incrementally
                           for vname, value in value_state.values.items():
                                if vname not in self.trajectories: self.trajectories[vname] = ValueTrajectory(vname)
                                self.trajectories[vname].add_point(value_state.timestamp, value)
                      except (KeyError, ValueError, TypeError) as e:
                           logger.warning(f"Skipping import of value state due to error: {e}. Data: {state_dict}")
             else: logger.warning("Invalid 'value_history' format in state.")


             # Import other state components
             current_vals = state.get('current_values')
             if isinstance(current_vals, dict): self.current_values = {k: float(v) for k,v in current_vals.items()}

             baseline_vals = state.get('baseline_values')
             if isinstance(baseline_vals, dict): self.baseline_values = {k: float(v) for k,v in baseline_vals.items()}

             self.baseline_set = bool(state.get('baseline_set', False))

             # Import CUSUM state
             cusum_s = state.get('cusum_state')
             if isinstance(cusum_s, dict): self.cusum_state = cusum_s # Assume format is correct
             ...
             if isinstance(imported_alerts, list):
                  for alert_dict in imported_alerts:
                       ...
                       try:
                            ...
                            logger.warning(f"Skipping import of drift alert due to error: {e}. Data: {alert_dict}")
             ...
             logger.info(f"Imported state. History size: {len(self.value_history)}, Values tracked: {len(self.trajectories)}")


    def reset(self) -> None:
        """Reset all tracked data, keeping initialization time."""
        with self.lock:
            init_time = self.stats.get('initialized_at', time.time()) # Preserve init time
    ...
    def _detect_drift_incremental(self, current_values: Dict[str, float]):
        ...
        for value_name, value in current_values.items():
            if value_name not in self.cusum_state or value_name not in self.baseline_values:
                # Need baseline to compare against
                continue

            cusum_info = self.cusum_state[value_name]
            baseline = self.baseline_values[value_name] # Compare against stable baseline
            ...
            slack = self.drift_threshold * max(0.1, abs(baseline)) # Use min baseline magnitude 0.1

            # Update CUSUM (positive and negative deviations)
            cusum_info['cusum_pos'] = max(0.0, cusum_info['cusum_pos'] + deviation - slack)
            cusum_info['cusum_neg'] = max(0.0, cusum_info['cusum_neg'] - deviation - slack)
            ...
            if cusum_pos > cusum_threshold or cusum_neg > cusum_threshold:
                ...
                alert = DriftAlert(
                    value_name=value_name, severity=severity, change_type=ValueChangeType.GRADUAL_DRIFT,
                    description=f"CUSUM drift detected for {value_name}",
                    old_value=baseline, new_value=value, change_magnitude=abs(value - baseline),
                    drift_score=drift_score, metadata={'method': 'cusum', 'threshold': cusum_threshold}
                )
    ...
    def _detect_drift_cusum(self, value_name: str, threshold: float) -> Dict[str, Any]:
        ...
        return {'detected': detected, 'score': score, 'threshold': cusum_threshold, 'cusum_pos': cusum_info['cusum_pos'], 'cusum_neg': cusum_info['cusum_neg']}
    ...
    def _detect_change_point(self, value_name: str) -> Dict[str, Any]:
        ...
        _np = self._np
        trajectory = self.trajectories.get(value_name)
        if not trajectory or len(trajectory.values) < self.min_history_for_drift: # Need enough points
            return {'detected': False, 'score': 0.0}

        values = trajectory.values
        n = len(values)
        # Split point (e.g., halfway or fixed recent window)
        split = max(n // 2, n - min(20, n // 2)) # Compare last ~20 points vs rest, or half vs half
        ...
        std_hist = _np.std(historical_values, ddof=1) if len(historical_values) > 1 else 0.0
        ...
        if n_hist + n_recent > 2:
            pooled_variance_num = (n_hist - 1) * std_hist**2 + (n_recent - 1) * std_recent**2
            pooled_variance_den = n_hist + n_recent - 2
            if pooled_variance_den > 0:
                 pooled_std = _np.sqrt(max(0.0, pooled_variance_num / pooled_variance_den)) # Ensure non-negative
        ...
        return {'detected': detected, 'score': score, 'recent_mean': float(mean_recent), 'historical_mean': float(mean_hist), 'z_like_diff': float(diff_score)}
    ...
    def _detect_trend_drift(self, value_name: str, threshold: float) -> Dict[str, Any]:
        ...
        values = trajectory.values
        slope, intercept = self._calculate_trend_slope_intercept(timestamps, values) # Use helper
        ...
        if value_range < 1e-9: value_range = 1.0 # Avoid division by zero if values are constant

        # Normalized slope magnitude (total change / value range)
        normalized_slope_magnitude = abs(slope * time_range / value_range) if time_range > 0 else 0.0
    ...
    def _calculate_trend_slope_intercept(self, timestamps: List[float], values: List[float]) -> Tuple[float, float]:
    ...
    def _classify_drift_severity(self, normalized_drift_score: float) -> DriftSeverity:
        ...
        if normalized_drift_score < 0.1: return DriftSeverity.NONE
        elif normalized_drift_score < 0.2: return DriftSeverity.MINOR
        elif normalized_drift_score < 0.4: return DriftSeverity.MODERATE
        elif normalized_drift_score < 0.7: return DriftSeverity.MAJOR
    ...
    def _handle_drift_alert(self, alert: DriftAlert):
        ...
        log_level = logging.CRITICAL if alert.severity == DriftSeverity.CRITICAL else \
                    logging.ERROR if alert.severity == DriftSeverity.MAJOR else \
                    logging.WARNING if alert.severity == DriftSeverity.MODERATE else \
                    logging.INFO # Use INFO for MINOR/NONE

        logger.log(log_level, f"Drift Alert ({alert.severity.value}): {alert.value_name} changed ({alert.change_type.value}). Score: {alert.drift_score:.3f}. Desc: {alert.description}")


        # Call alert callback if provided (outside lock)
        if self.alert_callback:
            try: self.alert_callback(alert)
            except Exception as e: logger.error(f"Drift alert callback failed: {e}")

        # Notify self-improvement drive if available and severe (outside lock)
        if not isinstance(self.self_improvement_drive, MagicMock) and alert.severity in [DriftSeverity.MAJOR, DriftSeverity.CRITICAL]:
            try:
                if hasattr(self.self_improvement_drive, 'handle_drift_alert'):
                    logger.info(f"Notifying SelfImprovementDrive about {alert.severity.value} drift in {alert.value_name}")
                    self.self_improvement_drive.handle_drift_alert(alert.to_dict()) # Pass dict
                else: logger.debug("SelfImprovementDrive missing handle_drift_alert method.")
            except Exception as e: logger.error(f"Failed to notify self-improvement drive about drift: {e}")

        # Record alert to transparency interface (outside lock)
        if not isinstance(self.transparency_interface, MagicMock) and hasattr(self.transparency_interface, 'record_drift_alert'):
             try: self.transparency_interface.record_drift_alert(alert.to_dict()) # Pass dict
             except Exception as e: logger.debug(f"Failed to record drift alert to transparency interface: {e}")
    ...
    def _compute_correlation_matrix(self, window_start_time=None, window_end_time=None) -> Dict[Tuple[str, str], float]:
        """Compute pairwise Pearson correlations between value trajectories within a window."""
        _np = self._np # Use internal alias
        correlation_matrix = {}
        value_names = list(self.trajectories.keys())
        if len(value_names) < 2: return {} # Need at least two values
        ...
        for vname in value_names:
            ...
            all_timestamps.extend(ts for ts in traj.timestamps if min_ts <= ts <= max_ts)
        ...
        for vname in value_names:
            ...
            if len(traj.timestamps) >= 2:
                ...
                valid_indices = [i for i, ts in enumerate(traj.timestamps) if min_ts <= ts <= max_ts]
                if not valid_indices and traj.timestamps: # Handle case where window is outside data range
                    ...
                    aligned_data[vname] = list(_np.interp(common_times, interp_times, interp_values))
                     ...
                     for target_t in common_times:
                          ...
                          if target_t < interp_times[0]: # Before first point
                               ...
                               t0, t1 = interp_times[current_idx], interp_times[current_idx+1]
                               v0, v1 = interp_values[current_idx], interp_values[current_idx+1]
                               # Avoid division by zero if timestamps are identical
                               interp_factor = (target_t - t0) / (t1 - t0) if (t1 - t0) != 0 else 0.0
        ...
        logger.debug(f"Calculating correlations for {num_valid_values} values with aligned data.")

        for i in range(num_valid_values):
            vname1 = valid_value_names[i]
            values1 = aligned_data[vname1]
            if len(values1) < 2: continue # Need at least 2 points

            for j in range(i + 1, num_valid_values):
                vname2 = valid_value_names[j]
                values2 = aligned_data[vname2]
                if len(values2) != len(values1): continue # Ensure same length after interpolation
                ...
                try:
                    ...
                    if not math.isnan(correlation):
                        correlation_matrix[(vname1, vname2)] = float(correlation) # Store as float
                except Exception as e:
                    logger.debug(f"Correlation calculation failed between {vname1} and {vname2}: {e}")
    ...
    def _empty_analysis(self) -> ValueEvolutionAnalysis:
        ...
        return ValueEvolutionAnalysis(
            time_window_start=ts, time_window_end=ts,
            value_trends={}, value_drift_scores={}, value_stability_scores={},
            drifted_values=[], drift_alerts=[],
            overall_stability=1.0, alignment_consistency=0.0,
            correlation_matrix={},
