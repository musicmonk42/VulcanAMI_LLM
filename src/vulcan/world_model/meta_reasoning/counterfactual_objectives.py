...
class CounterfactualObjectiveReasoner(SerializationMixin):
    ...
    def __init__(self, world_model=None, objective_estimates: Optional[Dict[str, float]] = None):
        ...
        self.learned_objective_correlations: Dict[Tuple[str, str], float] = defaultdict(
            float
        )
        self.prediction_accuracy_by_objective: Dict[str, List[float]] = defaultdict(
            list
        )
    ...
    def predict_under_objective(
        self, alternative_objective: str, context: Optional[Dict[str, Any]] = None
        ...
        with self.lock:
            ...
            predicted_outcome = self._simulate_objective_optimization(
                alternative_objective, relevant_vars, counterfactual_context
            )

            # Refine prediction using learned patterns
            predicted_outcome = self._refine_prediction_with_history(
                alternative_objective, predicted_outcome, counterfactual_context
            )
            ...
            confidence = self._calculate_prediction_confidence(
                alternative_objective, predicted_outcome, side_effects
            )
            ...
            outcome = CounterfactualOutcome(
                ...
                metadata={
                    ...
                    "learned_adjustments": predicted_outcome.get(
    ...
    def compare_objectives(
        self, obj_a: str, obj_b: str, scenario: Dict[str, Any]
    ) -> ObjectiveComparison:
        """
        ...
        with self.lock:
            ...
            for point_weights in candidate_points:
                ...
                evaluated_points.append(
                    {"weights": point_weights, "outcomes": outcomes}
                )

            # Find Pareto-optimal points
            pareto_points = self._identify_pareto_optimal(evaluated_points, objectives)
            ...
            logger.info(
                "Computed Pareto frontier with %d points in %.2fms",
                len(pareto_points),
    ...
    def estimate_tradeoffs(self, sacrifice_obj: str, gain_obj: str) -> Dict[str, Any]:
        """
        ...
        with self.lock:
            ...
            for sacrifice_weight in np.linspace(0.0, 1.0, 11):
                ...
                # Predict outcomes
                sacrifice_outcome = self.predict_under_objective(
                    sacrifice_obj, scenario
                )
            ...
            tradeoff_analysis = self._analyze_tradeoff_curve(
                scenarios, sacrifice_obj, gain_obj
            )
            ...
            if correlation_key in self.learned_objective_correlations:
                tradeoff_analysis["learned_correlation"] = (
                    self.learned_objective_correlations[correlation_key]
                )
    ...
    def generate_alternative_proposals(
    ...
    def update_prediction_accuracy(
        self, objective: str, predicted: float, actual: float
    ):
        """
        ...
        with self.lock:
            ...
            logger.debug(
                "Updated prediction accuracy for %s: error=%.3f", objective, error
            )

    def learn_objective_correlation(self, obj_a: str, obj_b: str, correlation: float):
        """
        ...
        with self.lock:
            ...
            logger.debug(
                "Learned correlation between %s and %s: %.3f", obj_a, obj_b, correlation
            )

    def _learn_objective_correlation(self, obj_a: str, obj_b: str, correlation: float):
        """
    ...
    def _refine_prediction_with_history(
        self, objective: str, predicted_outcome: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if objective in self.prediction_accuracy_by_objective:
            ...
            if accuracies:
                ...
                predicted_outcome["lower_bound"] = max(
                    0.0, predicted_value - uncertainty
                )
                predicted_outcome["upper_bound"] = min(
                    1.0, predicted_value + uncertainty
                )
    ...
    def _enhance_alternative_with_patterns(
        self, alternative: Dict[str, Any], objective: str
    ) -> Dict[str, Any]:
                ...
                if success_patterns and hasattr(success_patterns, "__iter__"):
                    try:
                        ...
                        relevant_patterns = [
                            ...
                            and p.features.get("objective") == objective
                        ]

                        # Apply highest-confidence pattern
                        if relevant_patterns:
                            best_pattern = max(
                                relevant_patterns, key=lambda p: p.confidence
                            )

                            # Apply pattern features to alternative
                            for feature, value in best_pattern.features.items():
                                if (
                                    feature not in alternative
                                    and feature != "objective"
                                ):
    ...
    def _simulate_objective_optimization(
        self, objective: str, variables: List[str], context: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                if hasattr(predictor, "predict_optimized"):
                    prediction = predictor.predict_optimized(
                        objective=objective, variables=variables, context=pred_context
                    )
                ...
                # Fallback: use standard prediction
                    ...
                    scenario = {**current_state, "optimize_for": objective}
    ...
    def _estimate_side_effects(
        ...
        for obj in available_objectives:
            ...
            if correlation_key in self.learned_objective_correlations:
                impact = (
                    self.learned_objective_correlations[correlation_key] * primary_value
                )
            else:
                # Estimate impact based on default relationships
                impact = self._estimate_objective_impact(
                    primary_objective, obj, primary_value
                )
    ...
    def _estimate_objective_impact(
        self, source_obj: str, target_obj: str, source_value: float
    ) -> float:
    ...
    def _calculate_prediction_confidence(
        ...
        value_range = predicted_outcome.get("upper_bound", 1.0) - predicted_outcome.get(
            "lower_bound", 0.0
        )
    ...
    def _generate_comparison_reasoning(
        ...
        if winner == obj_a:
            return (
                f"Optimizing for {obj_a} (value={outcome_a.predicted_value:.3f}) "
                f"achieves better outcome than {obj_b} (value={outcome_b.predicted_value:.3f})"
            )
        elif winner == obj_b:
            return (
                f"Optimizing for {obj_b} (value={outcome_b.predicted_value:.3f}) "
                f"achieves better outcome than {obj_a} (value={outcome_a.predicted_value:.3f})"
            )
        else:
            return (
                f"Objectives {obj_a} and {obj_b} achieve similar outcomes "
    ...
    def _generate_candidate_points(
        ...
        if n_objectives == 2:
            for w1 in np.linspace(0.0, 1.0, 11):
                weights = {objectives[0]: w1, objectives[1]: 1.0 - w1}
                candidates.append(weights)
        ...
        # For 3-4 objectives, use Dirichlet sampling
            ...
            for i in range(n_objectives):
                for j in range(i + 1, n_objectives):
                    weights = {
                        o: 0.5 if o in [objectives[i], objectives[j]] else 0.0
                        for o in objectives
                    }
                    candidates.append(weights)
    ...
    def _identify_pareto_optimal(
        self, evaluated_points: List[Dict[str, Any]], objectives: List[str]
    ) -> List[ParetoPoint]:
    ...
    def _analyze_tradeoff_curve(
        self, scenarios: List[Dict[str, Any]], sacrifice_obj: str, gain_obj: str
    ) -> Dict[str, Any]:
        ...
        tradeoff_score = (
            optimal_scenario["gain_value"] - optimal_scenario["sacrifice_value"]
        )
    ...
    def _generate_tradeoff_recommendation(
        self, sacrifice_obj: str, gain_obj: str, optimal_scenario: Dict[str, Any]
    ) -> str:
        ...
        return (
            f"Optimal trade-off: {sacrifice_weight:.0%} {sacrifice_obj}, "
    ...
    def _create_alternative_proposal(
        self, current_proposal: Dict[str, Any], alternative_objective: str
    ) -> Dict[str, Any]:
        ...
        if alternative_objective == "efficiency":
            ...
            if "target_accuracy" in alternative:
                alternative["target_accuracy"] = min(
                    0.99, alternative["target_accuracy"] * 1.05
                )
    ...
    def _is_valid_alternative(
        self, alternative: Dict[str, Any], original: Dict[str, Any]
    ) -> bool:
    ...
    def analyze_scenario(self, scenario: Any) -> Dict[str, Any]:
        ...
        with self.lock:
            try:
                ...
                # SECURITY NOTE: Using MD5 with usedforsecurity=False for cache key generation
                # This is acceptable because:
                # 1. This is NOT cryptographic use (just cache key generation)
                # 2. No security properties required (collision resistance not critical for cache)
                # 3. MD5 provides good distribution for cache keys
                # 4. usedforsecurity=False explicitly marks this as non-security use (Python 3.9+)
                # Alternative: Could use SHA-256 for better collision resistance or CRC32 for speed
