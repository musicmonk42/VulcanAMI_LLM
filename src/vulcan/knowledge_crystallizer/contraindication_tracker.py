...
class CascadeImpact:
    ...
    def add_affected(self, principle, impact_score: float, mitigation: Optional[str] = None):
    ...
    def estimate_recovery_time(self):
        ...
        self.recovery_time_estimate = base_time * severity_multiplier * depth_multiplier * blast_multiplier
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for serialization"""
        return {
            'affected_principles': [getattr(p, 'id', str(p)) for p in self.affected_principles],
...
class ContraindicationDatabase:
    ...
    def __init__(self, persistence_path: Optional[Path] = None):
        ...
        self.domain_contraindications = defaultdict(set)  # domain -> set of principle_ids
    ...
    def register(self, principle_id: str, contraindication: Contraindication):
        ...
        with self.lock:
            ...
            existing = self._find_similar_contraindication(principle_id, contraindication)
            
            if existing:
                # Update existing contraindication
                existing.update_frequency()
                logger.debug("Updated existing contraindication for principle %s", principle_id)
            else:
                ...
                if contraindication.domain:
                    self.domain_contraindications[contraindication.domain].add(principle_id)
                
                # Index by failure mode
                self.failure_patterns[contraindication.failure_mode].append(principle_id)
            ...
            logger.debug("Registered contraindication for principle %s: %s", 
                        principle_id, contraindication.condition)
    ...
    def get_by_condition(self, condition: str) -> Dict[str, List[Contraindication]]:
        ...
        with self.lock:
            ...
            for principle_id in self.condition_index.get(condition, set()):
                contras = [c for c in self.contraindications[principle_id] 
                          if c.condition == condition]
                if contras:
                    result[principle_id] = contras
            return result
    
    def check_domain_compatibility(self, principle_id: str, domain: str) -> Tuple[bool, List[Contraindication]]:
        """
        ...
        with self.lock:
            ...
            for contraindication in contraindications:
                ...
                elif contraindication.domain and self._domains_related(contraindication.domain, domain):
                    # Related domain contraindication
                    blocking.append(contraindication)
            
            # Check severity
            critical_blocking = [c for c in blocking if c.get_severity_level() == Severity.CRITICAL]
    ...
    def analyze_failure_patterns(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'high_risk_domains': sorted(domain_risks.items(), key=lambda x: x[1], reverse=True)[:5],
    ...
    def save(self, path: Optional[Path] = None):
        ...
        with self.lock:
            data = {
                'contraindications': {k: [c.to_dict() for c in v] 
    ...
    def _find_similar_contraindication(self, principle_id: str, 
                                      contraindication: Contraindication) -> Optional[Contraindication]:
        """Find similar existing contraindication"""
        for existing in self.contraindications.get(principle_id, []):
            if (existing.condition == contraindication.condition and
                existing.failure_mode == contraindication.failure_mode and
                existing.domain == contraindication.domain):
                return existing
        return None
...
class ContraindicationGraph:
    ...
    def __init__(self, persistence_path: Optional[Path] = None):
        ...
        logger.info("ContraindicationGraph initialized (networkx: %s)", NETWORKX_AVAILABLE)
    ...
    def add_edge(self, source_id: str, target_id: str, impact: float):
        ...
        with self.lock:
            ...
            logger.debug("Added edge %s -> %s (impact=%.2f)", source_id, target_id, impact)
    ...
    def remove_node(self, principle_id: str):
        ...
        with self.lock:
            ...
            logger.debug("Removed principle %s from contraindication graph", principle_id)
    ...
    def find_critical_nodes(self, threshold: float = 0.7) -> List[str]:
        ...
        with self.lock:
            ...
            return sorted(critical, key=lambda p: self.calculate_cascade_risk(p), reverse=True)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'avg_impact': np.mean(list(self.impact_weights.values())) if self.impact_weights else 0
            }
...
class CascadeAnalyzer:
    ...
    def analyze_cascade_impact(self, candidate, max_depth: int = 3) -> CascadeImpact:
        ...
        with self.lock:
            ...
            while queue:
                ...
                if self._affects_principle(failure_scenario, current_principle):
                    ...
                    if current_impact > self.min_impact_threshold:
                        ...
                        impact.add_affected(current_principle, current_impact, mitigation)
            ...
            if risk_level == "CRITICAL":
                impact.add_warning("CRITICAL cascade risk - immediate intervention required")
            elif risk_level == "HIGH":
                impact.add_warning("High cascade risk - implement circuit breakers")
            
            if len(impact.affected_principles) > 10:
                impact.add_warning(f"Wide cascade impact - {len(impact.affected_principles)} principles affected")
    ...
    def simulate_failure(self, candidate, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if contraindications:
            ...
            critical = [c for c in contraindications 
        ...
        if 'principle' in context:
            ...
            path, impact = self.graph.get_impact_path(scenario['principle_id'], target_id)
            if path:
    ...
    def get_cascade_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'total_edges': self.graph.graph.number_of_edges() if hasattr(self.graph.graph, 'number_of_edges') else 0,
