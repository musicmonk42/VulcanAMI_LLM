            ...
            for c in self.components:
                if isinstance(c, np.ndarray):
                    ...
                    serializable_components.append(
                        ("dict", {str(k): str(v) for k, v in c.items()})
                    )
                else:
                    serializable_components.append(("str", str(c)))
    ...
    def get_duration(self) -> float:
        ...
        try:
            if "execution_time" in self.outcomes:
                ...
                return float(self.metadata["end_time"]) - float(
    ...
    def calculate_generalizability(self) -> float:
        ...
        try:
            ...
            self.generalizability = (
                domain_diversity * 0.4 + context_diversity * 0.3 + success_rate * 0.3
            )
        ...
        try:
            ...
            if problem and isinstance(problem, dict) and "domain" in problem:
                ...
                if problem_domain in self.contraindicated_domains:
                    ...
                    problem_domain not in self.applicable_domains
                    and "general" not in self.applicable_domains
                ):
            ...
            return {"principle_id": self.id, "error": str(e), "confidence": 0.0}
...
class PrincipleExtractor:
    ...
    def __init__(
        ...
        logger.info(
            "PrincipleExtractor initialized (scipy: %s, strategy: %s)",
            SCIPY_AVAILABLE,
            strategy.value,
        )
    ...
    def extract_from_trace(
        ...
        with self.lock:
            try:
                ...
                valid_candidates = [
                    c for c in candidates if c and c.confidence >= self.min_confidence
                ]
                ...
                logger.info(
                    "Extracted %d principles from trace %s",
                    len(principles),
    ...
    def extract_from_batch(
        ...
        with self.lock:
            try:
                ...
                for candidate in merged:
                    try:
                        if (
                            ...
                            and len(candidate.evidence) >= self.min_evidence_count
                        ):
                            principle = self._crystallize_candidate(candidate)
                            if principle:
                                principles.append(principle)
                ...
                logger.info(
                    "Extracted %d principles from %d traces",
                    len(principles),
                    len(traces),
                )
                ...
                try:
                    ...
                    if signature in self.candidate_pool:
                        ...
                        candidate = PrincipleCandidate(
                            ...
                            required_metrics=self._identify_required_metrics(
                                execution_trace
                            ),
    ...
    def abstract_to_principle(
        self, success_factors: List[SuccessFactor]
    ) -> Optional[CrystallizedPrinciple]:
        """
        ...
        try:
            ...
            if abstracted and isinstance(abstracted, dict):
                principle = CrystallizedPrinciple(
                    ...
                    core_pattern=abstracted.get(
                        "pattern",
                        Pattern(
                            pattern_type=PatternType.SEQUENTIAL, components=["default"]
                        ),
                    ),
    ...
    def calculate_principle_confidence(self, evidence: List[ExecutionTrace]) -> float:
        ...
        try:
            ...
            if domains:
                domain_consistency = 1.0 - (len(set(domains)) - 1) / max(
                    1, len(domains)
                )
            else:
            ...
            logger.debug(
                "Calculated confidence %.2f from %d evidence items",
                confidence,
                len(evidence),
            )
    ...
    def _merge_candidates(
        self, candidates: List[PrincipleCandidate]
    ) -> List[PrincipleCandidate]:
        """Merge similar candidates"""
        try:
            if not candidates:
    ...
    def _crystallize_candidate(
        ...
        try:
            ...
            if (
                self.strategy != ExtractionStrategy.EXPLORATORY
                and candidate.generalizability < 0.4
            ):
                return None
            ...
            # Set confidence
            principle.confidence = self.calculate_principle_confidence(
                candidate.evidence
            )
    ...
    def _calculate_statistical_significance(
        self, evidence: List[ExecutionTrace]
    ) -> float:
        ...
        try:
            ...
            for trace in evidence:
                ...
                score = sum(
                    m.normalize_value() for m in trace.metrics if m and m.is_success
                )
                if trace.success:
        ...
        try:
            ...
            for window_size in range():
                ...
                for i in range(len(trace.actions) - window_size + 1):
                    ...
                    action_types = [
                        a.get("type", "action") if isinstance(a, dict) else str(a)
                        for a in sequence
                    ]
                    ...
                    if len(set(action_types)) > 1:  # More than one action type
                        ...
                        pattern = Pattern(
                            ...
                            structure={"sequence_length": window_size, "position": i},
                            frequency=1.0,
        ...
        try:
            ...
            for i, action in enumerate(trace.actions):
                if self._is_start_marker(action):
                    ...
                    if nested:
                        start_type = (
                            start_action.get("type", "start")
                            if isinstance(start_action, dict)
                ...
                for p2 in patterns[i + 1 :]:
                    ...
                    if self._patterns_compatible(p1, p2):
                        combined = Pattern(
                            ...
                            structure={
                                "sub_patterns": [
                                    p1.pattern_type.value,
                                    p2.pattern_type.value,
                                ],
...
class SuccessAnalyzer:
    ...
    def _analyze_action_factors(self, trace: ExecutionTrace) -> List[SuccessFactor]:
        ...
        try:
            ...
            first_type = (
                first_action.get("type", "unknown")
                if isinstance(first_action, dict)
                else str(first_action)
            )
    ...
    def _calculate_correlations(
        self, factors: List[SuccessFactor], trace: ExecutionTrace
    ):
...
class AbstractionEngine:
    ...
    def _group_factors(
        self, factors: List[SuccessFactor]
    ) -> Dict[str, List[SuccessFactor]]:
    ...
    def _identify_dominant(
        self, grouped: Dict[str, List[SuccessFactor]]
    ) -> Tuple[str, List[SuccessFactor]]:
    ...
    def _generate_description(
        self, factors: List[SuccessFactor], grouped: Dict[str, List[SuccessFactor]]
    ) -> str:
        ...
        try:
            ...
            return (
                ". ".join(descriptions)
                if descriptions
                else "General optimization principle"
            )
        except Exception as e:
            logger.error("Error generating description: %s", e)
            return "General principle"
    ...
    def _abstract_pattern(self, factors: List[SuccessFactor]) -> Pattern:
        ...
        try:
            ...
            if pattern_factors:
                ...
                pattern_type_str = main_pattern.factor_type.replace(
                    "pattern_", ""
                ).upper()
                try:
            ...
            return Pattern(
                ...
                structure={"abstracted": True, "factor_count": len(factors)},
    ...
    def _calculate_abstraction_confidence(self, factors: List[SuccessFactor]) -> float:
        ...
        try:
            ...
            correlation_scores = [
                abs(f.correlation) for f in valid_factors if f.correlation != 0
            ]
            correlation_score = (
                np.mean(correlation_scores) if correlation_scores else 0.5
            )

            # Combine scores
            confidence = (
                importance_score * 0.4 + evidence_score * 0.3 + correlation_score * 0.3
            )
