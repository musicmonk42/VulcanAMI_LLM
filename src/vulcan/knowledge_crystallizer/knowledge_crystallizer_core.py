...
class CrystallizationResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            'principles': [p.to_dict() if hasattr(p, 'to_dict') else str(p) for p in self.principles],
            'validation_results': [v.to_dict() if hasattr(v, 'to_dict') else str(v) for v in self.validation_results],
...
class ApplicationResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for serialization"""
        return {
            'principle_used': self.principle_used.to_dict() if self.principle_used and hasattr(self.principle_used, 'to_dict') else str(self.principle_used),
...
class KnowledgeCrystallizer:
    ...
    def __init__(self, vulcan_memory=None, semantic_bridge=None):
        ...
        logger.info("KnowledgeCrystallizer initialized (memory: %s, semantic: %s)", 
                   self.has_memory, self.has_semantic)
    
    def crystallize(self, execution_trace: ExecutionTrace,
                   context: Optional[Dict[str, Any]] = None) -> CrystallizationResult:
        """
        ...
        with self.lock:
            ...
            logger.info("Selected %s method with confidence %.2f: %s",
            ...
            try:
                if method_selection.method == CrystallizationMethod.CASCADE_AWARE:
                    ...
                    result = self._crystallize_incremental(execution_trace, method_selection.parameters)
                elif method_selection.method == CrystallizationMethod.BATCH:
                    result = self._crystallize_batch([execution_trace], method_selection.parameters)
                elif method_selection.method == CrystallizationMethod.ADAPTIVE:
                    result = self._crystallize_adaptive(execution_trace, method_selection.parameters)
                elif method_selection.method == CrystallizationMethod.HYBRID:
                    result = self._crystallize_hybrid(execution_trace, method_selection.parameters)
                else:
                ...
                logger.error("Crystallization failed with %s, trying fallback: %s",
                           method_selection.method.value, e)
                ...
                for fallback_method in method_selection.fallback_methods:
                    try:
                        ...
                        logger.error("Fallback %s also failed: %s", 
                                   fallback_method.value, fallback_error)
                        continue
                else:
    ...
    def crystallize_experience(self, execution_trace: ExecutionTrace) -> CrystallizationResult:
        """
        ...
        if not principles:
            logger.warning("No principles extracted from trace %s", execution_trace.trace_id)
        ...
        for principle in principles:
            ...
            if validation.is_valid and validation.confidence >= self.min_confidence_threshold:
        ...
        logger.info("Crystallized %d principles from trace %s (confidence: %.2f)",
                   len(valid_principles), execution_trace.trace_id, avg_confidence)
        
        return result
    
    def crystallize_with_cascade_detection(self, 
                                          execution_trace: ExecutionTrace) -> CrystallizationResult:
        """
        ...
        for principle in result.principles:
            ...
            # Check cascade risk
            cascade_risk = self.contraindication_graph.calculate_cascade_risk(principle.id)
        ...
        result.metadata['cascade_filtered'] = len(result.principles) - len(filtered_principles)
        
        logger.info("Cascade-aware crystallization: %d principles passed cascade check",
                   len(filtered_principles))
        
        return result
    
    def _crystallize_incremental(self, execution_trace: ExecutionTrace,
                                parameters: Dict[str, Any]) -> CrystallizationResult:
        """
        ...
        for principle in merged_principles:
            ...
            if validation.is_valid:
                ...
                if hasattr(principle, 'confidence'):
                    principle.confidence = min(0.99, principle.confidence * (1 + 0.05 * iteration))
                valid_principles.append(principle)
        ...
        self.incremental_state[trace_signature] = {
            ...
            'principles': valid_principles,
            'confidence': np.mean([p.confidence for p in valid_principles]) if valid_principles else 0.0
        }
        
        # Calculate overall confidence
        avg_confidence = np.mean([v.confidence for v in validation_results]) if validation_results else 0.0
        ...
        logger.info("Incremental crystallization iteration %d: %d principles (confidence: %.2f)",
                   iteration, len(valid_principles), avg_confidence)
        
        return result
    
    def _crystallize_batch(self, traces: List[ExecutionTrace],
                          parameters: Dict[str, Any]) -> CrystallizationResult:
        """
        ...
        logger.info("Batch crystallization: %d principles from %d traces (confidence: %.2f)",
                   len(valid_principles), batch_size, avg_confidence)
        
        return result
    
    def _crystallize_adaptive(self, execution_trace: ExecutionTrace,
                             parameters: Dict[str, Any]) -> CrystallizationResult:
        """
        ...
        for principle in principles:
            ...
            if validation.is_valid and validation.confidence >= self.min_confidence_threshold:
                valid_principles.append(principle)
            elif validation.confidence >= self.min_confidence_threshold * (1 - exploration_ratio):
        ...
        logger.info("Adaptive crystallization: %d principles (including %d exploratory)",
                   len(valid_principles), num_exploratory)
        
        return result
    
    def _crystallize_hybrid(self, execution_trace: ExecutionTrace,
                           parameters: Dict[str, Any]) -> CrystallizationResult:
        """
        ...
        for method_name in secondary_methods:
            if method_name == CrystallizationMethod.ADAPTIVE.value:
                result = self._crystallize_adaptive(execution_trace, {'adaptation_rate': 0.1})
        ...
        logger.info("Hybrid crystallization: %d principles from %d methods",
                   len(valid_principles), len(results))
    ...
    def validate_stratified(self, candidate: Principle) -> ValidationResult:
        ...
        if hasattr(candidate, 'domain'):
            domain_compatible, blocking = self.contraindication_db.check_domain_compatibility(
        ...
        logger.debug("Stratified validation for %s: valid=%s, confidence=%.2f",
                    candidate.id, all_valid, avg_confidence)
        
        return combined_validation
    
    def apply_knowledge(self, problem: Dict[str, Any], 
                        confidence_required: float = 0.7) -> ApplicationResult:
        """
        ...
        if not confident_principles:
            logger.info("No principles meet confidence requirement %.2f", confidence_required)
            max_conf = max(getattr(p, 'confidence', 0) for p in applicable)
            return ApplicationResult(
                principle_used=None,
                solution=None,
                confidence=max_conf,
                warnings=[f"No principles meet confidence requirement {confidence_required}"]
            )
        
        # Select best principle
        best_principle = max(confident_principles, key=lambda p: getattr(p, 'confidence', 0))
        
        # Check for contraindications
        if problem.get('context'):
            compatible, blocking = self.contraindication_db.check_domain_compatibility(
                best_principle.id,
                problem['context'].get('domain', 'general')
            )
            
            if not compatible and blocking:
                # Try to find alternative
                for principle in confident_principles[1:]:
                    alt_compatible, _ = self.contraindication_db.check_domain_compatibility(
                        principle.id,
                        problem['context'].get('domain', 'general')
                    )
                    if alt_compatible:
        ...
        logger.info("Applied principle %s to problem (confidence: %.2f)",
                   best_principle.id, result.confidence)
    ...
    def update_from_feedback(self, principle_id: str, outcome: Dict[str, Any]):
        ...
        with self.lock:
            ...
            if not principle:
                logger.warning("Principle %s not found for feedback update", principle_id)
                return
            ...
            if any(score > 0.5 for score in imbalances.values()):
                ...
                logger.info("Knowledge imbalance detected. Suggestions: %s", suggestions)
            
            logger.debug("Updated principle %s from feedback (new confidence: %.2f)",
                        principle_id, getattr(principle, 'confidence', 0.5))
    ...
    def _merge_principles_weighted(self, old_principles: List[Principle],
                                   new_principles: List[Principle],
                                   old_weight: float) -> List[Principle]:
    ...
    def _merge_principles_simple(self, old_principles: List[Principle],
                                new_principles: List[Principle]) -> List[Principle]:
    ...
    def _get_recent_success_rate(self) -> float:
        ...
        successes = sum(1 for h in recent if h.get('result_summary', {}).get('success', False))
        
        return successes / len(recent)
    
    def _fuse_results_weighted(self, results: List[CrystallizationResult],
                               weights: List[float]) -> List[Principle]:
        ...
        for pid, weighted_list in all_principles.items():
            if len(weighted_list) == 1:
                ...
                avg_confidence = np.mean([getattr(p, 'confidence', 0.5) for p in weighted_list])
    ...
    def _fuse_results_simple(self, results: List[CrystallizationResult]) -> List[Principle]:
        ...
        for result in results:
            for principle in result.principles:
                ...
                if principle.id not in all_principles:
                    ...
                    existing_conf = getattr(all_principles[principle.id], 'confidence', 0.5)
                    new_conf = getattr(principle, 'confidence', 0.5)
    ...
    def _analyze_contraindications(self, principle: Principle, 
                                   trace: ExecutionTrace) -> List[Contraindication]:
        """Analyze potential contraindications"""
        contraindications = []
    ...
    def _validate_against_history(self, candidate: Principle) -> ValidationResult:
        ...
        return ValidationResult(
            ...
            warnings=[] if success_rate > 0.5 else ["Below average historical performance"]
        )
    
    def _apply_principle(self, principle: Principle, 
                        problem: Dict[str, Any]) -> Any:
...
class KnowledgeApplicator:
    ...
    def adapt_principle_to_context(self, principle: Principle, 
                                   context: Dict[str, Any]) -> Principle:
        """
        ...
        # Adjust for domain
        if 'domain' in context and hasattr(adapted, 'domain') and context['domain'] != adapted.domain:
            adapted.domain = context['domain']
            if hasattr(adapted, 'confidence'):
                adapted.confidence *= 0.9  # Reduce confidence for cross-domain
            adaptations.append(f"Adapted from {getattr(principle, 'domain', 'unknown')} to {context['domain']}")
        ...
        logger.debug("Adapted principle %s with %d adaptations", 
                    principle.id, len(adaptations))
    ...
    def combine_principles(self, principles: List[Principle]) -> Optional[Principle]:
        ...
        combined = Principle(
            ...
            confidence=np.mean([getattr(p, 'confidence', 0.5) for p in principles]) * 0.9,
            domain="combined"
        )
    ...
    def monitor_application(self, principle: Principle, execution: Dict[str, Any]):
        ...
        if issues:
            ...
            for issue in issues:
                logger.warning("Application issue for principle %s: %s", 
                             principle.id, issue)
        else:
    ...
    def _matches_problem_type(self, principle: Principle, 
        ...
        return problem_type in principle.problem_types or 'general' in principle.problem_types
    
    def _satisfies_constraints(self, principle: Principle, 
                              problem: Dict[str, Any]) -> bool:
    ...
    def _calculate_relevance(self, principle: Principle, 
