...
try:
    ...
    logging.warning("resource module not available (Windows?), resource limits will be disabled")
...
class Principle:
    ...
    def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.execution_logic is None:
            raise NotImplementedError(f"Principle {self.id} has no execution logic defined")
        
        if self.execution_type == "function":
            if callable(self.execution_logic):
                return self.execution_logic(inputs)
            else:
                raise ValueError(f"Principle {self.id} execution_type is 'function' but execution_logic is not callable")
            ...
            try:
                ...
                raise RuntimeError(f"Failed to execute code string for principle {self.id}: {e}")
    ...
    def _execute_rules(self, inputs: Dict[str, Any], rules: Dict[str, Any]) -> Dict[str, Any]:
        """Execute rule-based logic"""
                ...
                if rule_type == 'direct':
                    ...
                    if callable(condition):
                        try:
                            ...
                            raise RuntimeError(f"Condition evaluation failed for rule {key}: {e}")
...
class ValidationResults:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'failure_analyses': {k: v.to_dict() for k, v in self.failure_analyses.items()},
...
class KnowledgeValidator:
    ...
    def validate(self, principle) -> ValidationResult:
        ...
        try:
            ...
            if not hasattr(principle, 'execution_logic') or principle.execution_logic is None:
                warnings.append("No execution logic defined - principle cannot be executed")
        ...
        with self.lock:
            try:
                ...
                if hasattr(principle, 'core_pattern'):
                    pattern_consistency = self._validate_pattern_consistency(principle.core_pattern)
                    consistency_scores.append(pattern_consistency)
                    if pattern_consistency < self.consistency_threshold:
                        warnings.append(f"Pattern consistency below threshold: {pattern_consistency:.2f}")
                
                # Check domain consistency
                if hasattr(principle, 'applicable_domains') and hasattr(principle, 'contraindicated_domains'):
                    # Check for overlap
                    applicable_domains = getattr(principle, 'applicable_domains', [])
                    contraindicated_domains = getattr(principle, 'contraindicated_domains', [])
                ...
                if hasattr(principle, 'success_count') and hasattr(principle, 'failure_count'):
                ...
                result = ValidationResult(
                    is_valid=len(errors) == 0 and overall_consistency >= self.consistency_threshold,
    ...
    def validate_across_domains(self, candidate, domains: List[str]) -> ValidationResults:
        """
        ...
        try:
            ...
            results = ValidationResults(validation_level=ValidationLevel.DOMAIN_SPECIFIC)
            
            for domain in domains:
                try:
                    # Generate domain-specific test
                    test_case = self.domain_validator.generate_domain_test(candidate, domain)
                    
                    # Run test
                    test_result = self.domain_validator.run_sandboxed_test(candidate, test_case)
            ...
            logger.info("Cross-domain validation: %d/%d domains passed",
                       len(results.successful_domains), len(domains))
    ...
    def test_generalization(self, principle, test_domains: List[str] = None) -> ValidationResults:
        """
        ...
        try:
            ...
            if not test_domains:
                test_domains = self.domain_validator.select_diverse_domains(principle, count=7)
            ...
            for domain in test_domains:
                try:
                    ...
                    # Run test
                    test_result = self.domain_validator.run_sandboxed_test(principle, test_case)
                    
                    # Evaluate generalization
                    if self._passes_generalization_criteria(test_result, principle):
                        score = self._calculate_generalization_score(test_result, principle)
                    ...
                    logger.error("Error testing generalization for domain %s: %s", domain, e)
                    failure = FailureAnalysis(
            ...
            results.metadata['generalization_score'] = self._calculate_overall_generalization(results)
            
            logger.info("Generalization test: %.2f success rate across %d domains",
                       results.success_rate, len(test_domains))
    ...
    def validate_principle_multilevel(self, principle, context: Optional[Dict[str, Any]] = None) -> Dict[str, ValidationResult]:
        """
        ...
        try:
            ...
            logger.info("Running validation levels: %s for principle %s", 
                       levels_to_run, getattr(principle, 'id', 'unknown'))
            ...
            if ValidationLevel.BASIC in levels_to_run:
                ...
                if not results['basic'].is_valid and results['basic'].confidence < 0.3:
                    logger.warning("Basic validation failed critically, skipping further validation")
                    results['skipped'] = ValidationResult(
                        is_valid=False,
                        confidence=0.0,
                        warnings=["Further validation skipped due to critical basic validation failure"],
                        metadata={'levels_skipped': [l.value for l in levels_to_run if l != ValidationLevel.BASIC]}
                    )
                    return results
            ...
            if ValidationLevel.DOMAIN_SPECIFIC in levels_to_run:
                ...
                if test_domains:
                    results['domain_specific'] = self.validate_across_domains(principle, test_domains)
                else:
                    results['domain_specific'] = ValidationResult(
                        is_valid=True,
                        confidence=0.5,
                        warnings=["No domains available for testing"],
                        metadata={'validation_type': 'domain_specific', 'skipped': True}
                    )
            ...
            if ValidationLevel.GENERALIZATION in levels_to_run:
                ...
                if not gen_domains:
                    gen_domains = self.domain_validator.select_diverse_domains(principle, count=7)
                
                results['generalization'] = self.test_generalization(principle, gen_domains)
            ...
            self.validation_history.append({
                ...
                'results': {k: v.is_valid for k, v in results.items() if hasattr(v, 'is_valid')},
                'timestamp': time.time()
            })
    ...
    def _select_validation_levels(self, principle, context: Dict[str, Any]) -> List[ValidationLevel]:
        """
        ...
        logger.debug("Selected validation levels for principle (confidence=%.2f, criticality=%.2f): %s",
                    confidence, domain_criticality, [l.value for l in levels])
        ...
        for domain in domains:
            ...
            if domain_normalized in self.domain_criticality:
                ...
                for known_domain, criticality in self.domain_criticality.items():
                    if known_domain in domain_normalized or domain_normalized in known_domain:
    ...
    def _get_domains_for_testing(self, principle, context: Dict[str, Any]) -> List[str]:
        ...
        return self.domain_validator.select_diverse_domains(principle, count=max_domains)
                ...
                for req2 in requirements:
                    if req1 != req2:
                        ...
                        for conflict_pair in conflicts:
                            if (conflict_pair[0] in req1_lower and conflict_pair[1] in req2_lower) or \
                               (conflict_pair[1] in req1_lower and conflict_pair[0] in req2_lower):
                                consistency -= 0.2
    ...
    def _passes_generalization_criteria(self, test_result: Dict[str, Any], principle) -> bool:
    ...
    def _calculate_generalization_score(self, test_result: Dict[str, Any], principle) -> float:
...
class DomainValidator:
    ...
    def select_diverse_domains(self, candidate, count: int = 7) -> List[str]:
        ...
        try:
            ...
            middle_domains = [d for d, _ in sorted_domains[len(sorted_domains)//4:3*len(sorted_domains)//4]]
    ...
    def categorize_domains_by_data_availability(self) -> Dict[DomainCategory, List[str]]:
        """
    ...
    def generate_domain_test(self, principle, domain: str) -> DomainTestCase:
        ...
        try:
            ...
            expected = None
            if hasattr(principle, 'compute_expected') and callable(principle.compute_expected):
                try:
                    expected = principle.compute_expected(inputs)
                except Exception as e:
                    logger.debug("Could not compute expected outputs for principle %s: %s",
                               getattr(principle, 'id', 'unknown'), e)
    ...
    def run_sandboxed_test(self, principle, test_case: DomainTestCase) -> Dict[str, Any]:
        """
        ...
        with self._create_sandbox() as sandbox_dir:
            try:
                # Create test script
                script_path = self._create_test_script(principle, test_case, sandbox_dir)
                
                # Run test with resource limits based on platform
                if platform.system() == "Linux" and RESOURCE_AVAILABLE:
                    result = self._run_with_resource_limits(script_path, test_case.timeout)
                else:
                    result = self._run_with_subprocess_limits(script_path, test_case.timeout)
        ...
        logger.debug("Sandboxed test for domain %s: %s",
                    test_case.domain, 'passed' if test_result.get('success') else 'failed')
    ...
    def _generate_domain_inputs(self, domain: str, domain_info: Dict[str, Any]) -> Dict[str, Any]:
        """Generate domain-specific inputs"""
        try:
            inputs = {
                'domain': domain,
                'test_type': 'domain_validation'
            }
...
"""
            elif execution_type == "code_string" and hasattr(principle, 'execution_logic'):
...
"""
    ...
    def _run_with_resource_limits(self, script_path: str, timeout: float) -> Dict[str, Any]:
        ...
        try:
            ...
            def set_limits():
                ...
                try:
                    # CPU time limit
                    resource.setrlimit(resource.RLIMIT_CPU, (int(timeout), int(timeout) + 5))
                    # Memory limit
    ...
    def _run_with_subprocess_limits(self, script_path: str, timeout: float) -> Dict[str, Any]:
...
class ImbalanceHandler:
    ...
    def detect_imbalance(self, validation_results: ValidationResults) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if validation_results.failure_analyses:
                failure_types = [fa.failure_type for fa in validation_results.failure_analyses.values()]
                if failure_types:
                    type_counts = Counter(failure_types)
                    most_common = type_counts.most_common(1)[0]
                    if most_common[1] > len(failure_types) * 0.7:
                        imbalances['failure_clustering'] = most_common[1] / len(failure_types)
    ...
    def handle_imbalanced_validation(self, results: ValidationResults, domain: str) -> ValidationResult:
        """
    ...
    def _handle_low_success(self, results: ValidationResults, domain: str) -> ValidationResult:
        """Handle low success rate"""
        try:
            # Relax validation criteria
            adjusted_confidence = results.domain_scores.get(domain, 0.0) * 1.5
            adjusted_confidence = min(0.7, adjusted_confidence)  # Cap at moderate confidence
    ...
    def _handle_high_success(self, results: ValidationResults, domain: str) -> ValidationResult:
    ...
    def _handle_high_variance(self, results: ValidationResults, domain: str) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                ...
                metadata={'adjusted': True, 'reason': 'high_variance', 'used_median': True}
            )
        except Exception as e:
    ...
    def _generate_test_inputs(self, characteristics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            complexity = complexity_map.get(characteristics.get('complexity', 'medium'), 5)
