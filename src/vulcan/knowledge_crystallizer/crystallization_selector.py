...
class SelectionStrategy:
    ...
    def evaluate(self, characteristics: TraceCharacteristics, 
...
class StandardStrategy(SelectionStrategy):
    ...
    def evaluate(self, characteristics: TraceCharacteristics, 
        ...
        if characteristics.complexity in [TraceComplexity.SIMPLE, TraceComplexity.MODERATE]:
            score += 0.3
        ...
        parameters = {
            'validation_level': 'basic' if characteristics.complexity == TraceComplexity.SIMPLE else 'comprehensive'
        }
...
class CascadeAwareStrategy(SelectionStrategy):
    ...
    def evaluate(self, characteristics: TraceCharacteristics, 
        ...
        if characteristics.complexity in [TraceComplexity.COMPLEX, TraceComplexity.HIGHLY_COMPLEX]:
            score += 0.2
...
class IncrementalStrategy(SelectionStrategy):
    ...
    def evaluate(self, characteristics: TraceCharacteristics, 
        ...
        parameters = {
            ...
            'merge_strategy': 'weighted' if (characteristics.iteration_count or 0) > 5 else 'simple',
...
class BatchStrategy(SelectionStrategy):
    ...
    def evaluate(self, characteristics: TraceCharacteristics, 
        ...
        parameters = {
            ...
            'aggregation_method': 'voting' if characteristics.batch_size > 5 else 'averaging',
...
class AdaptiveStrategy(SelectionStrategy):
    ...
    def evaluate(self, characteristics: TraceCharacteristics, 
        ...
        if characteristics.complexity in [TraceComplexity.MODERATE, TraceComplexity.COMPLEX]:
        ...
        parameters = {
            'adaptation_rate': 0.1,
            'exploration_ratio': 0.2 if characteristics.domain_type == DomainType.NOVEL else 0.1,
...
class HybridStrategy(SelectionStrategy):
    ...
    def _select_secondary_methods(self, characteristics: TraceCharacteristics) -> List[str]:
...
class CrystallizationSelector:
    ...
    def __init__(self):
        ...
        logger.info("CrystallizationSelector initialized with %d strategies", len(self.strategies))
    
    def select_method(self, 
                      trace: Any,
                      context: Optional[Dict[str, Any]] = None) -> MethodSelection:
        """
        ...
        with self.lock:
            ...
            if confidence < self.min_confidence_threshold:
                ...
                confidence = 0.5
                logger.warning("Low confidence %.2f, falling back to standard method", confidence)
            ...
            logger.info("Selected %s method with confidence %.2f", method.value, confidence)
            
            return selection
    
    def _analyze_trace(self, trace: Any, context: Dict[str, Any]) -> TraceCharacteristics:
        """
    ...
    def _count_unique_patterns(self, actions: List[Any]) -> int:
        ...
        for i in range(len(actions)):
            ...
            action_type = str(actions[i].get('type', actions[i])) if isinstance(actions[i], dict) else str(actions[i])
            patterns.add(action_type)
            
            # Bigram patterns
            if i < len(actions) - 1:
                next_type = str(actions[i+1].get('type', actions[i+1])) if isinstance(actions[i+1], dict) else str(actions[i+1])
    ...
    def _detect_loops(self, actions: List[Any]) -> bool:
        ...
        action_strs = [str(a.get('type', a)) if isinstance(a, dict) else str(a) for a in actions]
    ...
    def _classify_domain(self, domain: str, context: Dict[str, Any]) -> DomainType:
        ...
        known_domains = context.get('known_domains', ['general', 'optimization', 'analysis', 'control'])
    ...
    def _apply_learning_adjustment(self, method: CrystallizationMethod, base_score: float) -> float:
    ...
    def _generate_reasoning(self, method: CrystallizationMethod, 
                           characteristics: TraceCharacteristics,
        ...
        if method == CrystallizationMethod.CASCADE_AWARE:
            if characteristics.has_failures:
                reasons.append(f"Detected failures with {characteristics.failure_rate:.1%} failure rate")
            if characteristics.has_dependencies:
                reasons.append("Dependencies require cascade analysis")
        
        elif method == CrystallizationMethod.INCREMENTAL:
            if characteristics.is_incremental:
                reasons.append(f"Incremental pattern with {characteristics.iteration_count} iterations")
            if characteristics.has_loops:
                reasons.append("Loop patterns benefit from incremental approach")
        
        elif method == CrystallizationMethod.BATCH:
            if characteristics.batch_size > 1:
                reasons.append(f"Batch of {characteristics.batch_size} traces available")
            ...
            reasons.append(f"Moderate confidence level ({characteristics.confidence_level:.2f}) suits adaptive approach")
        
        elif method == CrystallizationMethod.HYBRID:
            reasons.append(f"High complexity ({characteristics.complexity.value}) requires hybrid approach")
        
        else:  # STANDARD
            reasons.append(f"Standard crystallization suitable for {characteristics.complexity.value} complexity")
        ...
        return "; ".join(reasons) if reasons else "Default selection based on trace characteristics"
    
    def _identify_fallbacks(self, strategy_scores: Dict[CrystallizationMethod, float],
                           selected_method: CrystallizationMethod) -> List[CrystallizationMethod]:
        ...
        if CrystallizationMethod.STANDARD not in fallbacks and selected_method != CrystallizationMethod.STANDARD:
            fallbacks.append(CrystallizationMethod.STANDARD)
        
        return fallbacks
    
    def _estimate_cost(self, method: CrystallizationMethod,
                      characteristics: TraceCharacteristics) -> float:
        ...
        batch_mult = 1.0 if method != CrystallizationMethod.BATCH else (0.7 + characteristics.batch_size * 0.05)
        
        return base_cost * complexity_mult * action_mult * batch_mult
    
    def _estimate_time(self, method: CrystallizationMethod,
                      characteristics: TraceCharacteristics) -> float:
    ...
    def _calculate_priority(self, characteristics: TraceCharacteristics,
    ...
    def _track_selection(self, selection: MethodSelection,
                        characteristics: TraceCharacteristics):
    ...
    def update_performance(self, method: CrystallizationMethod, success: bool):
        ...
        with self.lock:
            ...
            logger.debug("Updated performance for %s: success=%s", method.value, success)
