...
class MockGraphixVulcanLLM:
    ...
    def generate(self, prompt: str, max_tokens: int) -> str:
        ...
        self.logger.info(f"Generating response for prompt: '{prompt[:30]}...' (max_tokens: {max_tokens})")
...
class Settings(BaseSettings):
    ...
    improvement_max_cost_usd: float = Field(default=10.0, env="IMPROVEMENT_MAX_COST_USD")
    improvement_check_interval_seconds: int = Field(default=120, env="IMPROVEMENT_CHECK_INTERVAL_SECONDS")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if not isinstance(config, AgentConfig):
            logger.error(f"Invalid config type returned: {type(config)}, creating default config")
            config = AgentConfig()
    ...
    if not hasattr(config, 'self_improvement_config'):
        ...
        if not hasattr(config, 'world_model'):
            ...
            WorldModelConfig = make_dataclass("WorldModelConfig", [("self_improvement_config", str, settings.self_improvement_config)])
            config.world_model = WorldModelConfig()
        elif not hasattr(config.world_model, 'self_improvement_config'):
             setattr(config.world_model, 'self_improvement_config', settings.self_improvement_config)
             
    if not hasattr(config, 'self_improvement_state'):
        # Ensure world_model config section exists if needed
        if not hasattr(config, 'world_model'):
             from dataclasses import make_dataclass
             WorldModelConfig = make_dataclass("WorldModelConfig", [("self_improvement_state", str, settings.self_improvement_state)])
             config.world_model = WorldModelConfig()
        elif not hasattr(config.world_model, 'self_improvement_state'):
            setattr(config.world_model, 'self_improvement_state', settings.self_improvement_state)
    ...
    try:
        ...
        if settings.checkpoint_path:
            if os.path.exists(settings.checkpoint_path) and os.path.getsize(settings.checkpoint_path) > 0:
                checkpoint_to_load = settings.checkpoint_path
                logger.info(f"Will load checkpoint from {checkpoint_to_load}")
            else:
                logger.warning(f"Checkpoint file {settings.checkpoint_path} does not exist or is empty, starting fresh")
        ...
        llm_instance = initialize_component(
            'llm',
            lambda: GraphixVulcanLLM(config_path="configs/llm_config.yaml")
        )
        ...
        try:
            ...
            logger.warning(f"Could not ensure data/configs/checkpoints directories: {e}")
...
improvement_attempts = Counter('vulcan_improvement_attempts_total', 'Total improvement attempts', ['objective_type'])
improvement_successes = Counter('vulcan_improvement_successes_total', 'Successful improvements', ['objective_type'])
improvement_failures = Counter('vulcan_improvement_failures_total', 'Failed improvements', ['objective_type'])
improvement_cost = Counter('vulcan_improvement_cost_usd_total', 'Total improvement cost in USD')
improvement_approvals_pending = Gauge('vulcan_improvement_approvals_pending', 'Number of pending approvals')
...
async def validate_api_key(request: Request, call_next):
    ...
    provided_key = (
        ...
        or hdrs.get("x-api-key")
        or (hdrs.get("Authorization")[7:] if hdrs.get("Authorization", "").startswith("Bearer ") else None)
    )
    ...
    if not provided_key or not hmac.compare_digest(provided_key, settings.api_key):
        ...
        return JSONResponse(
            ...
            content={
                ...
                "accepted_headers": ["X-API-Key", "X-API-KEY", "Authorization: Bearer <key>"],
...
async def rate_limiting(request: Request, call_next):
    ...
    if settings.api_key:
        api_key = (
            ...
            or (request.headers.get("Authorization")[7:] if request.headers.get("Authorization", "").startswith("Bearer ") else None)
        )
        if api_key:
    ...
    with rate_limit_lock:
        ...
        if len(rate_limit_storage[client_id]) >= settings.rate_limit_requests:
            ...
            return JSONResponse(
                status_code=429,
                content={"error": "Rate limit exceeded", "retry_after": settings.rate_limit_window_seconds}
            )
...
async def execute_step(request: StepRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail=f"Execution timeout after {timeout}s")
...
async def stream_execution():
    ...
    async def generate():
        ...
        try:
            ...
            logger.critical(f"Unexpected stream generator error (outside loop): {e}", exc_info=True)
...
async def create_plan(request: PlanRequest):
    ...
    try:
        ...
        return {"plan": plan.to_dict() if hasattr(plan, 'to_dict') else str(plan), "status": "created"}
...
async def start_self_improvement():
    ...
    try:
        ...
        if not hasattr(world_model, 'self_improvement_enabled') or not world_model.self_improvement_enabled:
            raise HTTPException(status_code=400, detail="Self-improvement not enabled in configuration")
        
        if hasattr(world_model, 'improvement_running') and world_model.improvement_running:
            return {"status": "already_running", "message": "Self-improvement drive is already running"}
...
async def stop_self_improvement():
    ...
    try:
        ...
        if not hasattr(world_model, 'improvement_running') or not world_model.improvement_running:
            return {"status": "not_running", "message": "Self-improvement drive is not running"}
...
async def approve_improvement(request: ApprovalRequest):
    ...
    try:
        ...
        if not world_model or not hasattr(world_model, 'self_improvement_drive'):
            raise HTTPException(status_code=503, detail="Self-improvement drive not available")
        ...
        if result:
            logger.info(f"Improvement {request.approval_id} {'approved' if request.approved else 'rejected'}")
...
async def update_performance_metric(metric: str, value: float):
    ...
    try:
        ...
        await loop.run_in_executor(
            None,
            world_model.update_performance_metric,
            metric,
            value
        )
...
async def health_check(response: Response):
    ...
    try:
        ...
        health_checks = {
            ...
            "memory_usage": status['health'].get('memory_usage_mb', 0) < settings.max_memory_mb * 0.9,
...
async def system_status():
    ...
    try:
        ...
        # Add LLM status
        status['llm'] = {
            'initialized': hasattr(app.state, 'llm') and not isinstance(app.state.llm, MagicMock),
            'mocked': isinstance(app.state.llm, MagicMock) if hasattr(app.state, 'llm') else False
        }
...
def test_basic_functionality(deployment: ProductionDeployment) -> bool:
    ...
    test_contexts = [
        ...
        {"high_level_goal": "optimize", "raw_observation": {"text": "Multi", "data": [1, 2, 3]}},
    ...
    for i, context in enumerate(test_contexts):
        try:
            ...
            assert ("action" in result) or ("output" in result), f"Test {i}: Missing action/output in result"
            assert result.get("error") is None or "stub" in str(result.get("error", "")).lower(), f"Test {i}: Error occurred: {result.get('error')}"
...
def test_memory_systems(deployment: ProductionDeployment) -> bool:
    ...
    try:
        for i in range(5):
            context = {"high_level_goal": "explore", "raw_observation": f"Memory test {i}"}
        ...
        if hasattr(deployment.collective.deps, 'am') and deployment.collective.deps.am:
            try:
                ...
                logger.info(f"Memory test passed: {memory_stats['total_episodes']} episodes stored")
...
def test_resource_limits(deployment: ProductionDeployment) -> bool:
    ...
    try:
        ...
        assert memory_usage < settings.max_memory_mb, f"Memory limit exceeded: {memory_usage}MB"
...
def test_self_improvement(deployment: ProductionDeployment) -> bool:
    ...
    try:
        ...
        if '_initialized_components' not in globals() or 'self_improvement_drive' not in _initialized_components:
...
def test_llm_integration() -> bool:
    ...
    try:
        ...
        reasoning_response = llm.bridge.reasoning.reason("Why is the sky blue?", {}, 'hybrid')
...
def run_all_tests(config: AgentConfig) -> bool:
    ...
    tests = [
        ...
        ("Self-Improvement", test_self_improvement), # Will use the global drive instance
...
class IntegrationTestSuite:
    ...
    async def test_end_to_end_async(self):
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                if result.get('success'):
                    print(f"✅ Task {i} completed successfully: action={result.get('action', 'unknown')}")
                    successful += 1
                else:
        ...
        assert success_rate > 0.8, f"Low success rate: {success_rate:.2%} ({successful}/{total} tasks succeeded)"
        
        return {"success_rate": success_rate, "results": results, "successful": successful, "failed": failed}
    ...
    async def _execute_test_step(self, context: Dict) -> Dict:
        ...
        try:
            ...
            has_critical_error = result.get("error") and "critical" in str(result.get("error", "")).lower()
    ...
    async def test_concurrent_operations(self):
        ...
        return {
            'planning': results[0] if not isinstance(results[0], Exception) else {'success': False, 'error': str(results[0])},
            'memory': results[1] if not isinstance(results[1], Exception) else {'success': False, 'error': str(results[1])},
            'reasoning': results[2] if not isinstance(results[2], Exception) else {'success': False, 'error': str(results[2])},
            'learning': results[3] if not isinstance(results[3], Exception) else {'success': False, 'error': str(results[3])}
        }
    ...
    async def _test_planning(self):
        ...
        try:
            ...
            return {"success": plan is not None, "steps": len(getattr(plan, 'steps', []))}
        except Exception as e:
...
class PerformanceBenchmark:
    ...
    def _benchmark_robustness(self, iterations: int) -> Dict:
        ...
        edge_cases = [
            ...
            {'high_level_goal': 'explore', 'raw_observation': {'nested': {'deep': {'very': 'deep'}}}},
        ...
        report = {
            'summary': {
                ...
                'config': {
                    ...
                    'self_improvement': getattr(self.config, 'enable_self_improvement', False),
    ...
    def _analyze_results(self) -> Dict:
        ...
        if 'latency' in self.results:
            ...
            analysis['latency_stability'] = self.results['latency']['std'] / self.results['latency']['mean']
        
        if 'memory' in self.results:
            growth = self.results['memory']['growth_rate']
            analysis['memory_stable'] = abs(growth) < 0.1
            analysis['memory_within_limit'] = self.results['memory']['rss_max'] < settings.max_memory_mb
        
        if 'scalability' in self.results:
            throughputs = [v['throughput'] for v in self.results['scalability'].values()]
            if len(throughputs) > 1:
                scalability_factor = throughputs[-1] / throughputs[0]
                ideal_factor = int(list(self.results['scalability'].keys())[-1].split('_')[1])
                analysis['scalability_efficiency'] = scalability_factor / ideal_factor
        
        if 'robustness' in self.results:
            analysis['robustness_acceptable'] = self.results['robustness']['robustness_score'] > 0.95
...
def run_interactive(config: AgentConfig):
    ...
    print("="*50)
    print("Commands: 'step', 'status', 'save', 'load', 'improve', 'llm', 'help', 'quit'")
    print("="*50 + "\n")
    ...
    while True:
        try:
            ...
            if cmd in ["quit", "exit"]:
                ...
                print("  llm         - LLM interaction commands (chat, reason, explain)")
                ...
                observation = input("Observation: ").strip() or "Interactive observation"
                ...
                is_success = result.get('success', False) or result.get('status') == 'completed'
                print(f"\nSuccess: {is_success}")
...
def _handle_llm_command(deployment: ProductionDeployment):
    ...
    llm = _initialized_components.get('llm')
    if llm is None:
        print("LLM component not available. Run full test suite to confirm initialization.")
        return
...
def find_available_port(host: str, port: int) -> int:
    ...
    while True:
        try:
            ...
            if e.errno == 98 or "Address already in use" in str(e) or "only one usage" in str(e): # 98 is EADDRINUSE
                logger.warning(f"Port {port} is already in use. Trying next port...")
                port += 1
                if port - original_port > 100: # Stop after 100 tries
                    logger.error(f"Could not find an available port after 100 attempts from base {original_port}")
                    raise RuntimeError(f"Could not find an available port after 100 attempts from base {original_port}")
            else:
    ...
    if config.enable_self_improvement:
        try:
            ...
            if hasattr(config, 'world_model') and config.world_model is not None:
                ...
                self_improvement_config_path = getattr(config.world_model, 'self_improvement_config', default_config_path)
            else:
                 logger.warning("AgentConfig has no 'world_model' attribute or it is None, using default self-improvement config path.")


            # Verify the config file exists
            if not Path(self_improvement_config_path).exists():
                 logger.warning(f"Self-improvement config file not found at {self_improvement_config_path}, using default config settings.")
                 # Initialize with default config dictionary if file not found
                 try:
                    self_improvement_drive = SelfImprovementDrive(config={'enabled': True})
                 except Exception as e:
                    self_improvement_drive = MagicMock()
                    logger.error(f"Failed to initialize SelfImprovementDrive with default config, using MagicMock: {e}")
            else:
                # Initialize the self-improvement drive from file path
                try:
                    self_improvement_drive = SelfImprovementDrive(config_path=self_improvement_config_path)
                except Exception as e:
                    self_improvement_drive = MagicMock()
            ...
            logger.info("✓ Meta-reasoning self-improvement drive initialized successfully")
        except Exception as e:
            self_improvement_drive = MagicMock()
            logger.error(f"Failed to initialize meta-reasoning self-improvement drive: {e}", exc_info=True)
            logger.warning("Continuing without meta-reasoning self-improvement (using MagicMock)")
    ...
    if args.mode == "test":
        ...
        try:
            ...
            if available_port != args.port:
                logger.warning(f"Original port {args.port} was busy. Using {available_port} instead.")
