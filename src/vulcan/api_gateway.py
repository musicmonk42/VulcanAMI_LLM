    ...
    async def register_service(self, service: ServiceEndpoint):
        ...
        async with self._lock:
            ...
            if self.consul_client:
                ...
                logger.info(f"Registered service locally: {service.name} at {service.url}")

    async def get_service(self, name: str, version: Optional[str] = None) -> Optional[ServiceEndpoint]:
        ...
        try:
            ...
            async with self._lock:
                for service_name in services:
                    ...
                    _, service_instances = self.consul_client.health.service(service_name, passing=True)
...
class UserStore:
    ...
    def __init__(self):
        ...
        if not ARGON2_AVAILABLE and not BCRYPT_AVAILABLE:
            logger.warning("Argon2 and Bcrypt not found. Falling back to PBKDF2. This is NOT recommended for production.")

    async def add_user(self, username: str, password: str, roles: Optional[List[str]] = None, scopes: Optional[List[str]] = None):
        ...
        if self.argon2:
            ...
            pwd_hash = BcryptLib.hashpw(password.encode('utf-8'), salt_bytes).decode('utf-8')
            algo = 'bcrypt'
    ...
    async def verify_user(self, username: str, password: str) -> Tuple[bool, Optional[Dict[str, Any]]]:
        ...
        if not user:
            ...
            if self.argon2:
                ...
                try:
                    BcryptLib.hashpw("dummy_password_for_timing_attack".encode('utf-8'), BcryptLib.gensalt(rounds=4))
                except Exception: pass
            else:
        ...
        try:
            if algo == 'argon2' and self.argon2:
                ...
                valid = BcryptLib.checkpw(password.encode('utf-8'), user['password_hash'].encode('utf-8'))
    ...
    def _pbkdf2_hash(self, password: str, salt_b64: str, iterations: int = 200_000) -> str:
...
class AuthManager:
    ...
    def __init__(self, user_store: UserStore, redis_client: Optional[aioredis.Redis] = None):
        ...
        if self.private_key and self.public_key:
            ...
            if env_secret:
                ...
                logger.warning("Generated random JWT secret - set JWT_SECRET env var for persistence (or JWT_PRIVATE_KEY/JWT_PUBLIC_KEY for RS256)")
            self.verify_key = self.key
            if self.private_key or self.public_key:
                logger.warning("JWT_PRIVATE_KEY or JWT_PUBLIC_KEY missing. Falling back to HS256.")

        self.kid = os.getenv('JWT_KID', 'vulcan-key-1')
        self.token_expiry = timedelta(minutes=int(os.getenv('ACCESS_TOKEN_TTL_MIN', '30')))
        self.refresh_expiry = timedelta(days=int(os.getenv('REFRESH_TOKEN_TTL_DAYS', '7')))
        self.permissions_cache = TTLCache(maxsize=1000, ttl=300)

        # In-memory fallbacks
        self._revoked_cache = TTLCache(maxsize=5000, ttl=int(self.refresh_expiry.total_seconds()))
        # Active refresh token JTI per user
        self._active_refresh: Dict[str, Tuple[str, float]] = {}  # user_id -> (jti, exp_ts)
    ...
    async def _set_revoked(self, jti: str, exp_ts: float):
        ...
        if self.redis and not self.degraded_mode:
            try:
                ...
                logger.error(f"Redis connection failed in _set_revoked: {e}. Falling back to in-memory store.")
    ...
    async def _is_revoked(self, jti: str) -> bool:
        if self.redis and not self.degraded_mode:
            try:
                ...
                logger.error(f"Redis connection failed in _is_revoked: {e}. Falling back to in-memory check.")
                self.degraded_mode = True
    ...
    async def _set_active_refresh(self, user_id: str, jti: str, exp_ts: float):
        ...
        if self.redis and not self.degraded_mode:
            try:
                ...
                logger.error(f"Redis connection failed in _set_active_refresh: {e}. Falling back to in-memory store.")
                self.degraded_mode = True
    ...
    async def _get_active_refresh(self, user_id: str) -> Optional[str]:
        if self.redis and not self.degraded_mode:
            try:
                ...
                logger.error(f"Redis connection failed in _get_active_refresh: {e}. Falling back to in-memory check.")
                self.degraded_mode = True
        # fallback
    ...
    def _make_access_claims(self, user_id: str, roles: List[str], scopes: List[str]) -> Dict[str, Any]:
        now = datetime.utcnow()
        return {
            **self._base_claims(),
            'sub': user_id,
    ...
    async def create_tokens(self, user_id: str, roles: List[str], scopes: List[str]) -> Tuple[str, str, Dict[str, Any]]:
        ...
        access_token = jwt.encode(access_claims, self.key, algorithm=self.algorithm, headers=headers)
        refresh_token = jwt.encode(refresh_claims, self.key, algorithm=self.algorithm, headers=headers)

        exp_ts = float(refresh_claims['exp'].timestamp() if hasattr(refresh_claims['exp'], 'timestamp') else time.time() + self.refresh_expiry.total_seconds())
        await self._set_active_refresh(user_id, refresh_claims['jti'], exp_ts)

        return access_token, refresh_token, {'access_jti': access_claims['jti'], 'refresh_jti': refresh_claims['jti']}

    async def verify_token(self, token: str, expected_type: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    async def revoke_tokens(self, access_token: Optional[str], refresh_token: Optional[str]):
        """Revoke provided tokens (if any)."""
        for token, ttype in [(access_token, 'access'), (refresh_token, 'refresh')]:
            if not token:
                continue
            try:
                payload = jwt.decode(token, self.verify_key, algorithms=[self.algorithm], options={"verify_signature": False, "verify_exp": False, "verify_aud": False, "verify_iss": False})
                jti = payload.get('jti')
                exp = payload.get('exp', time.time() + 60)
                exp_ts = float(exp) if isinstance(exp, (int, float)) else (exp.timestamp() if hasattr(exp, 'timestamp') else time.time() + 60)
                if jti:
    ...
    def check_permission(self, user_id: str, permission: str) -> bool:
        ...
        has_permission = (permission in default_permissions) or (user_id in admin_users and permission in ['admin', 'delete'])
...
class RateLimiter:
    ...
    async def check_limit(self, key: str, limit_type: str = 'user', override: Optional[Tuple[int, int]] = None) -> bool:
        """Check if request is within rate limit. Optional override for tokens/window."""
        max_tokens, window = override if override else self.default_limits.get(limit_type, (100, 60))
...
class RequestTransformer:
    ...
    async def transform_request(self, request: APIRequest,
                               target_format: str = 'json') -> Any:
    ...
    async def transform_response(self, response: Any,
...
class CacheManager:
    ...
    async def invalidate(self, pattern: str) -> int:
        ...
        # Invalidate LRU
        keys_to_remove_lru = [k for k in list(self.lru_cache.keys())
        ...
        # Invalidate TTL
        keys_to_remove_ttl = [k for k in list(self.memory_cache.keys())
                         if re.match(pattern, k)]
        ...
        if self.redis and not self.degraded_mode:
            try:
                ...
                logger.error(f"Redis cache invalidation failed: {e}. Entering degraded mode.")
                self.degraded_mode = True
            except Exception as e:
...
class APIGateway:
    ...
    def __init__(self, config: AgentConfig = None):
        ...
        self.allowed_origins = [o.strip() for o in env_origins.split(',')] if env_origins else default_origins
    ...
    def _setup_middleware(self):
        ...
        async def logging_middleware(request, handler):
            ...
            try:
                ...
                logger.info(f"[{request_id}] {request.method} {request.path} - {response.status} - {duration:.3f}s")
                ...
                duration = time.time() - start_time
                logger.error(f"[{request_id}] {request.method} {request.path} - ERROR - {duration:.3f}s - {e}")
                raise
        ...
        async def auth_middleware(request, handler):
            ...
            payload = await self.auth_manager.verify_token(token, expected_type='access')
            ...
            if required_roles and not any(role in user_roles for role in required_roles):
                ...
                return web.json_response({'error': 'Forbidden: missing role'}, status=403)
            ...
            if required_scopes and not any(scope in user_scopes for scope in required_scopes):
                ...
                return web.json_response({'error': 'Forbidden: missing scope'}, status=403)
        ...
        async def rate_limit_middleware(request, handler):
            ...
            ip = request.remote or request.headers.get('X-Forwarded-For', 'unknown').split(',')[0].strip()
            endpoint = request.path
            ...
            if endpoint == '/auth/login':
                ...
                if not await self.rate_limiter.check_limit(endpoint, 'endpoint'):
                    return web.json_response({'error': 'Endpoint rate limit exceeded'}, status=429)
        ...
        async def cache_cleanup():
            ...
            while True:
                try:
                    ...
                    error_count += 1
                    logger.error(f"Cache cleanup task error (count: {error_count}): {e}")
                    if error_count > 10:
                        logger.critical("Too many cache cleanup failures, stopping task")
                        break
    ...
    async def _seed_default_users(self):
        ...
        if not admin_pass:
            ...
            logger.warning(f"GATEWAY_ADMIN_PASS not set. Generated temporary admin password for user '{admin_user}': {admin_pass}. CHANGE IMMEDIATELY in production.")
        await self.user_store.add_user(admin_user, admin_pass, roles=['admin'], scopes=['read', 'write', 'admin'])

        # Optional default user
        user_user = os.getenv('GATEWAY_DEFAULT_USER')
        user_pass = os.getenv('GATEWAY_DEFAULT_PASS')
        if user_user and user_pass:
            await self.user_store.add_user(user_user, user_pass, roles=['user'], scopes=['read', 'write'])
            ...
            try:
                ...
                for user_entry in users_data:
                    try:
                        ...
                        logger.error(f"Failed to load user {user_entry.get('username')} from file: {e}")
    ...
    async def login(self, request):
        ...
        access_token, refresh_token, meta = await self.auth_manager.create_tokens(username, roles, scopes)
    ...
    async def refresh_token(self, request):
        ...
        payload = await self.auth_manager.verify_token(refresh_token, expected_type='refresh')
        if not payload:
        ...
        access_token, new_refresh_token, meta = await self.auth_manager.create_tokens(user_id, roles, scopes)
    ...
    async def create_plan(self, request):
        ...
        try:
            ...
            if planner is None:
                return web.json_response(
                    {'error': 'Planner not available', 'status': 'stub'},
                    status=503
                )

            plan = planner.generate_plan(goal, context)

            return web.json_response(plan.to_dict() if hasattr(plan, 'to_dict') else {'plan': str(plan)})

        except Exception as e:
            logger.error(f"Planning failed: {e}\n{traceback.format_exc()}")
            return web.json_response(
                {'error': 'Planning failed', 'message': str(e)},
                status=500
            )
    ...
    async def execute_plan(self, request):
        ...
        try:
            ...
            return web.json_response(
                {'error': 'Execution failed', 'message': str(e)},
                status=500
            )
    ...
    async def learn(self, request):
        ...
        try:
            ...
            if learner is None:
                return web.json_response(
                    {'error': 'Learner not available', 'status': 'stub'},
                    status=503
                )
            ...
            return web.json_response(
                {'error': 'Learning failed', 'message': str(e)},
                status=500
            )
    ...
    async def reason(self, request):
        ...
        try:
            if reasoning_type == 'symbolic':
                ...
                if reasoner is None:
                    return web.json_response(
                        {'error': 'Symbolic reasoner not available'},
                        status=503
                    )
                ...
                if reasoner is None:
                    return web.json_response(
                        {'error': 'Causal reasoner not available'},
                        status=503
                    )
                ...
                if reasoner is None:
                    return web.json_response(
                        {'error': 'Probabilistic reasoner not available'},
                        status=503
                    )
            ...
            return web.json_response(
                {'error': 'Reasoning failed', 'message': str(e)},
                status=500
            )
    ...
    async def store_memory(self, request):
        ...
        try:
            ...
            if memory is None or processor is None:
                return web.json_response(
                    {'error': 'Memory or processor not available'},
                    status=503
                )
            ...
            return web.json_response(
                {'error': 'Store failed', 'message': str(e)},
                status=500
            )
    ...
    async def configure_system(self, request):
        ...
        try:
            ...
            return web.json_response(
                {'error': 'Configuration failed', 'message': str(e)},
                status=500
            )
    ...
    async def websocket_handler(self, request):
        ...
        async with self.ws_lock:
            if len(self.websocket_connections) >= self.max_ws_connections:
                logger.warning(f"Max WebSocket connections ({self.max_ws_connections}) reached. Rejecting new connection.")
        ...
        ws.rate_limit_bucket = {'tokens': 20.0, 'last_refill': time.time()} # 20 tokens burst, 10 tokens/sec
        ...
        try:
            async for msg in ws:
                if msg.type == web.WSMsgType.TEXT:
                    ...
                    ws.rate_limit_bucket['tokens'] = min(ws.rate_limit_burst, ws.rate_limit_bucket['tokens'] + tokens_to_add)
    ...
    async def graphql_handler(self, request):
        ...
        try:
            ...
            return web.json_response(
                {'error': 'GraphQL execution failed', 'message': str(e)},
                status=500
            )
