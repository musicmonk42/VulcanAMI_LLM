    ...
    def decompose_goal(self, goal: str, context: Dict) -> List[Dict]:
        ...
        if "optimize" in goal.lower():
            subgoals.extend(
                [
                    {
                        "subgoal": "explore_solution_space",
                        "priority": 0.8,
                        "resources_required": {"cpu": 1},
                    },
                    {
                        "subgoal": "learn_patterns",
                        "priority": 0.9,
                        "resources_required": {"memory": 2},
                    },
                    {
                        "subgoal": "optimize_parameters",
                        "priority": 1.0,
                        "resources_required": {"compute": 3},
                    },
                ]
            )
            ...
            subgoals.extend(
                [
                    {
                        "subgoal": "explore_data",
                        "priority": 0.7,
                        "resources_required": {"cpu": 1},
                    },
                    {
                        "subgoal": "learn_representation",
                        "priority": 0.9,
                        "resources_required": {"memory": 3},
                    },
                ]
            )
        else:
...
class Plan:
    ...
    def optimize(self):
        ...
        self.steps = [
            step_map[step_id] for step_id in optimized_order if step_id in step_map
        ]
...
class EnhancedResourceMonitor:
        ...
        for host, port in test_endpoints:
            try:
                ...
                with socket.create_connection(
                    (host, port), timeout=NETWORK_TEST_TIMEOUT_SECONDS
                ) as sock:
    ...
    def _determine_operational_mode(
        self, cpu: float, memory: float, gpu: Optional[float]
    ) -> OperationalMode:
    ...
    def get_resource_availability(self) -> Dict[str, float]:
        ...
        with self._state_lock:
            ...
            try:
                ...
                disk_usage_percent = getattr(
                    self.current_state, "disk_usage_percent", 50
                )
...
class SurvivalProtocol:
    ...
    def change_mode(self, new_mode: OperationalMode):
        ...
        logger.info(
            f"Changing operational mode: {self.current_mode.value} -> {new_mode.value}"
        )

        # Record change
        self.mode_history.append(
            {"timestamp": time.time(), "from": self.current_mode, "to": new_mode}
        )
    ...
    def detect_network_failure(self) -> Dict[str, Any]:
        ...
        state = self.resource_monitor.current_state
        if not state:
            return {"failure_detected": False, "connectivity": "unknown", "actions": []}
        ...
        failure_detected = (
            connectivity in ["offline", "intermittent"]
            or recent_failures >= NETWORK_FAILURE_THRESHOLD
        )
    ...
    def _disable_network_capabilities(self):
        ...
        disabled = []
        for cap_name, cap_info in self.capabilities.items():
            if cap_info.get("network_required", False) and cap_info.get(
                "enabled", False
            ):
                cap_info["enabled"] = False
    ...
    def check_thermal_status(self, current_temp: Optional[float]) -> bool:
        ...
        if current_temp > thermal_limit:
            if not self.thermal_throttle_active:
                logger.warning(
                    f"Thermal throttling activated: {current_temp}°C > {thermal_limit}°C"
                )
...
class MonteCarloTreeSearch:
    ...
    def search(
        self, initial_state: Any, available_actions: Optional[List[Any]] = None
    ) -> Tuple[Any, float]:
...
class EnhancedHierarchicalPlanner(HierarchicalGoalSystem):
    ...
    def generate_plan(
        ...
        goal: str,
        context: Dict[str, Any],
        method: PlanningMethod = PlanningMethod.HIERARCHICAL,
    ) -> Plan:
        ...
        if method == PlanningMethod.MCTS:
            if not self.survival_protocol.is_capability_available("mcts_planning"):
                logger.info(
                    "MCTS not available in current mode, falling back to hierarchical"
                )
        ...
        if SafetyValidator and AgentConfig:
            try:
                ...
                if not is_safe:
                    logger.error(
                        f"Generated plan for goal '{goal}' failed safety validation: {reason}"
                    )
    ...
    def _create_hierarchical_plan(self, goal: str, context: Dict[str, Any]) -> Plan:
        ...
        with self._lock:
            ...
            plan = Plan(
                plan_id=f"plan_{goal}_{int(time.time())}", goal=goal, context=context
            )
    ...
    def _create_mcts_plan(self, goal: str, context: Dict[str, Any]) -> Plan:
        ...
        plan = Plan(
            plan_id=f"mcts_plan_{goal}_{int(time.time())}", goal=goal, context=context
        )
    ...
    def _identify_dependencies(
        self, subgoal: str, previous_subgoals: List[Dict]
    ) -> List[str]:
        dependencies = []
        if "learn" in subgoal:
            for prev in previous_subgoals:
                if "explore" in prev["subgoal"]:
                    dependencies.append(
                        f"step_{previous_subgoals.index(prev)}_{prev['subgoal']}"
                    )
    ...
    def _apply_planning_action(
        self, state: "PlanningState", action: Any
    ) -> "PlanningState":
        ...
        if "explore" in action_str.lower():
            new_state.resources_used["time"] = (
                new_state.resources_used.get("time", 0) + 1.0
            )
        elif "optimize" in action_str.lower():
            new_state.resources_used["compute"] = (
                new_state.resources_used.get("compute", 0) + 2.0
            )
    ...
    def _reconstruct_path(
        self, came_from: Dict, current: "PlanningState"
    ) -> List[Tuple["PlanningState", Any]]:
        path = []
    ...
    def _path_to_plan(
        self, path: List[Tuple["PlanningState", Any]], goal: str, context: Dict
    ) -> Plan:
        plan = Plan(
            plan_id=f"astar_plan_{goal}_{int(time.time())}", goal=goal, context=context
        )
    ...
    def execute_plan(
        self, plan: Plan, executor: Optional[Callable] = None
    ) -> Dict[str, Any]:
...
class PlanningState:
    ...
    def __eq__(self, other):
        return self.goal == other.goal and self.steps_taken == other.steps_taken
...
class PlanRepairer:
    ...
    def create_recovery_plan(
        self, original_plan: Plan, failed_step: PlanStep, error: str
    ) -> Optional[Plan]:
    ...
    def _repair_preconditions_strategy(self, plan: Plan, failed_step: PlanStep) -> Plan:
    ...
    def _repair_resources_strategy(self, plan: Plan, failed_step: PlanStep) -> Plan:
    ...
    def _repair_timeout_strategy(self, plan: Plan, failed_step: PlanStep) -> Plan:
...
class ResourceAwareCompute:
    ...
    def plan_with_budget(
        self, problem: Dict, time_budget_ms: float, energy_budget_nJ: float
    ) -> Dict[str, Any]:
        ...
        with self._lock:
            if cache_key in self.cache:
                ...
                if (
                    "result" in cached
                    and time.time() - cached["timestamp"] < CACHE_TTL_SECONDS
                ):
        ...
        try:
            ...
            plan = self._execute_plan(
                problem, time_budget_ms - (time.time() - start_time) * 1000
            )

            # Track actual usage
            actual_time = (time.time() - start_time) * 1000
            power = (
                self.resource_monitor.current_state.power_watts
                if self.resource_monitor.current_state
                else 10
            )
            ...
            plan["resource_usage"] = {
                ...
                "within_budget": actual_time <= time_budget_ms
                and actual_energy <= energy_budget_nJ,
            ...
            # Cache result
            with self._lock:
                self.cache[cache_key] = {"result": plan, "timestamp": time.time()}
    ...
    def _estimate_requirements(self, problem: Dict, use_gpu: bool) -> Dict[str, float]:
        ...
        return {"time_ms": time_ms, "memory_mb": memory_mb, "energy_nJ": energy_nJ}
    ...
    def _select_optimization_strategy(
        ...
        if self.survival_protocol.current_mode in [
            OperationalMode.SURVIVAL,
            OperationalMode.EMERGENCY,
        ]:
            return "cpu_only"
...
class ResourceAllocator:
    ...
    def allocate(
        self, requirements: Dict[str, float], available: Dict[str, float]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "feasible": feasible,
            "utilization": {
                r: allocation.get(r, 0) / available.get(r, 1) for r in available
            },
        }
...
class DistributedCoordinator:
    ...
    def distribute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.lock:
            active_agents = [
                aid for aid, info in self.agents.items() if info["status"] == "active"
            ]
    ...
    def _assign_subtasks(
        self, subtasks: List[Dict], agents: List[str]
    ) -> Dict[str, Dict]:
...
if __name__ == "__main__":
    ...
    result = compute.plan_with_budget(
        problem, time_budget_ms=100, energy_budget_nJ=10000
    )
