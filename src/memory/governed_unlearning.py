    ...
    def evaluate_proposal(self, proposal: IRProposal) -> GovernanceResult:
        ...
        if self.auto_approve:
            return GovernanceResult(
                ...
                details={
                    "auto_approved": True,
                    "reason": "Simple consensus auto-approval",
                },
...
class GovernedUnlearning:
    ...
    def submit_ir_proposal(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Consensus evaluation failed for proposal {proposal_id}: {e}"
                )
            ...
            logger.info(
                f"Submitted proposal {proposal_id} with status {governance_result.status.value}"
            )
            return proposal_id
    ...
    def submit_governance_vote(
        self, proposal_id: str, voter_id: str, approve: bool, justification: str = ""
    ) -> bool:
        """
        ...
        try:
            with self._lock:
                if proposal_id not in self.governance_results:
                    logger.warning(
                        f"Cannot vote on non-existent proposal {proposal_id}"
                    )
                    return False
                ...
                if result.status != ProposalStatus.PENDING:
                    logger.warning(
                        f"Cannot vote on proposal {proposal_id} with status {result.status.value}"
                    )
                    return False
                ...
                if len(result.votes) >= 2:  # Require at least 2 votes
                    ...
                    if approve_votes > total_votes / 2:
                        ...
                        if proposal_id in self.pending_proposals:
                            self._queue_for_execution(
                                self.pending_proposals[proposal_id], result
                            )
                        self.metrics.approved_requests += 1
                    else:
            ...
            logger.info(
                f"Vote recorded for proposal {proposal_id} by {voter_id}: {'approve' if approve else 'reject'}"
            )
            return True
                        ...
                        active_proposals.append(
                            {
                                ...
                                "pattern": proposal.ir_content.get(
                                    "pattern", "unknown"
                                ),
                            }
                        )
    ...
    def _queue_for_execution(
        self, proposal: IRProposal, result: GovernanceResult
    ) -> None:
        """Queue approved proposal for execution with error handling."""
        try:
            # Extract pattern and affected packs from IR content
            pattern = proposal.ir_content.get(
                "pattern", f"proposal_{proposal.proposal_id}"
            )
            ...
            if method_str:
                try:
                    ...
                    logger.warning(
                        f"Invalid method '{method_str}' in ir_content, using urgency-based selection"
                    )
            ...
            logger.info(
                f"Queued task {task.task_id} for pattern '{pattern}' using method {method.value}"
            )
        ...
        try:
            ...
            try:
                ...
                if task.proof is None:
                    logger.warning(
                        f"Task {task.task_id} completed without proof verification"
                    )
            except Exception as e:
            ...
            with self._lock:
                ...
                self.metrics.total_packs_processed += (
                    len(task.affected_packs) if task.affected_packs else 0
                )
    ...
    def _execute_gradient_surgery(self, task: UnlearningTask) -> None:
        ...
        for i, pack in enumerate(task.affected_packs):
            try:
                if hasattr(self.memory, "unlearning_engine") and hasattr(
                    self.memory.unlearning_engine, "gradient_surgery"
                ):
                    self.memory.unlearning_engine.gradient_surgery(
                        packfile=pack, pattern=task.pattern, fast_lane=is_urgent
                    )
                else:
                    logger.warning(
                        f"Memory system lacks unlearning_engine.gradient_surgery for task {task.task_id}"
                    )
                    return
        ...
        for i, pack in enumerate(task.affected_packs):
            try:
                ...
                if hasattr(self.memory, "unlearning_engine") and hasattr(
                    ...
                    self.memory.unlearning_engine.exact_removal(
                        packfile=pack, pattern=task.pattern
                    )
                else:
                    ...
                    if hasattr(self.memory, "unlearning_engine") and hasattr(
                        ...
                        self.memory.unlearning_engine.gradient_surgery(
                            packfile=pack, pattern=task.pattern, fast_lane=True
                        )
                    else:
                        logger.warning(
                            f"Memory system lacks unlearning capabilities for task {task.task_id}"
                        )
                        return
    ...
    def _execute_retraining(self, task: UnlearningTask) -> None:
        ...
        try:
            if hasattr(self.memory, "retrain_without_pattern"):
                ...
                # Fallback to gradient surgery
                logger.info(
                    f"retrain_without_pattern not available, falling back to gradient surgery"
                )
                self._execute_gradient_surgery(task)
        except Exception as e:
    ...
    def _execute_cryptographic_erasure(self, task: UnlearningTask) -> None:
        ...
        try:
            if hasattr(self.memory, "unlearning_engine") and hasattr(
                ...
                self.memory.unlearning_engine.cryptographic_erasure(
                    pattern=task.pattern, affected_packs=task.affected_packs
                )
            else:
                # Fallback to exact removal
                logger.info(
                    f"cryptographic_erasure not available, falling back to exact removal"
                )
                self._execute_exact_removal(task)
        except Exception as e:
    ...
    def _execute_differential_privacy(self, task: UnlearningTask) -> None:
        ...
        try:
            if hasattr(self.memory, "unlearning_engine") and hasattr(
                ...
                # Fallback to gradient surgery
                logger.info(
                    f"differential_privacy_unlearn not available, falling back to gradient surgery"
                )
                self._execute_gradient_surgery(task)
        except Exception as e:
            logger.error(
                f"Differential privacy unlearning failed for task {task.task_id}: {e}"
            )
            raise

    def _generate_unlearning_proof(
        self, task: UnlearningTask
        ...
        try:
            ...
            proof = self.memory.zk_prover.generate_unlearning_proof(
                pattern=task.pattern, affected_packs=task.affected_packs or []
            )
