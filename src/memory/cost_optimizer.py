...
class CostAnalyzer:
    ...
    def analyze_current_costs(self) -> CostBreakdown:
        ...
        try:
                ...
                if isinstance(stats, dict):
                    try:
                        breakdown.storage_cost = (
                            float(stats.get('hot_gb', 0)) * self.pricing['storage_hot_gb_month'] +
                            float(stats.get('warm_gb', 0)) * self.pricing['storage_warm_gb_month'] +
                            float(stats.get('cold_gb', 0)) * self.pricing['storage_cold_gb_month']
                        )
            ...
            # FIXED: Fallback method using tier-based storage
                ...
                try:
                    hot_gb = self.memory.get_tier_storage_gb(ResourceTier.HOT) if hasattr(self.memory, 'get_tier_storage_gb') else 0
                    warm_gb = self.memory.get_tier_storage_gb(ResourceTier.WARM) if hasattr(self.memory, 'get_tier_storage_gb') else 0
                    cold_gb = self.memory.get_tier_storage_gb(ResourceTier.COLD) if hasattr(self.memory, 'get_tier_storage_gb') else 0
            ...
            # FIXED: Second fallback using total storage
                ...
                try:
                    ...
                    avg_price = (self.pricing['storage_hot_gb_month'] + 
                               self.pricing['storage_warm_gb_month'] + 
                               self.pricing['storage_cold_gb_month']) / 3
            ...
            if hasattr(self.memory, 'get_bandwidth_stats'):
                ...
                if isinstance(stats, dict):
                    try:
                        breakdown.bandwidth_cost = float(stats.get('total_gb', 0)) * self.pricing['bandwidth_gb']
                    except (ValueError, TypeError) as e:
            ...
            # FIXED: Fallback method
                ...
                try:
                    ...
                    breakdown.bandwidth_cost = bandwidth_gb * self.pricing['bandwidth_gb']
                except Exception as e:
            ...
            if hasattr(self.memory, 'get_compute_stats'):
                ...
                if isinstance(stats, dict):
                    try:
                        breakdown.compute_cost = float(stats.get('hours', 0)) * self.pricing['compute_hour']
                    except (ValueError, TypeError) as e:
            ...
            if hasattr(self.memory, 'get_api_stats'):
                ...
                if isinstance(stats, dict):
                    try:
                        breakdown.api_cost = float(stats.get('total_calls', 0)) * self.pricing['api_call']
                    except (ValueError, TypeError) as e:
            ...
            if hasattr(self.memory, 'get_cdn_stats'):
                ...
                if isinstance(stats, dict):
                    try:
                        breakdown.cdn_cost = float(stats.get('requests', 0)) * self.pricing['cdn_request']
                    except (ValueError, TypeError) as e:
        ...
        try:
                ...
                try:
                    ...
                    # FIXED: Fallback - estimate from duplication rate
                        ...
                        if isinstance(stats, dict) and 'duplication_rate' in stats:
                            total_storage = self.memory.get_total_storage_gb() if hasattr(self.memory, 'get_total_storage_gb') else 0
                            savings = total_storage * float(stats.get('duplication_rate', 0))
                        else:
                            savings = 0
                    else:
            ...
            if hasattr(self.memory, 'tier_c'):
                try:
                    ...
                    if hasattr(self.memory.tier_c, 'get_cold_candidates'):
                        ...
                        if hasattr(self.memory, 'get_stats'):
                            ...
                            if isinstance(stats, dict):
                                ...
                                if util_rate < 0.5:  # Low utilization suggests migration opportunity
                                    # Estimate number of candidates
                                    migration_candidates = ['estimated'] * int(100 * (1 - util_rate))
                                else:
                    ...
                    if migration_candidates and hasattr(migration_candidates, '__len__'):
                        candidate_count = len(migration_candidates)
                        if candidate_count > 0:
                            opportunities.append({
                                'type': 'tier_migration',
                                'items': candidate_count,
                                'priority': 'medium' if candidate_count > 50 else 'low',
            ...
            if hasattr(self.memory, 'tier_c'):
                try:
                    ...
                    if hasattr(self.memory.tier_c, 'count_low_utility'):
                        low_utility = self.memory.tier_c.count_low_utility(threshold=0.1)
                    else:
                        ...
                        if hasattr(self.memory, 'get_stats'):
                            ...
                            if isinstance(stats, dict):
                                ...
                                util_rate = float(stats.get('utilization_rate', 1.0))
                                if util_rate < 0.7:
                                    low_utility = int(1000 * (1 - util_rate))  # Rough estimate
                                else:
                                    low_utility = 0
                            else:
                                low_utility = 0
                    ...
                    if hasattr(self.memory, 'get_bandwidth_stats'):
                        try:
                            ...
                            if isinstance(stats, dict):
                                ...
                                total_savings += current_bandwidth * 0.2 * self.pricing['bandwidth_gb']
                        except (ValueError, TypeError):
                            pass  # Skip if can't calculate
                    # FIXED: Fallback bandwidth calculation
                    elif hasattr(self.memory, 'get_bandwidth_usage_gb'):
                        try:
                            current_bandwidth = float(self.memory.get_bandwidth_usage_gb())
                            total_savings += current_bandwidth * 0.2 * self.pricing['bandwidth_gb']
                        except Exception:
                            pass
    ...
    def get_cost_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        if len(self.cost_history) < 2:
            return {'trend': 'insufficient_data', 'average_cost': 0, 'change_percentage': 0}
        ...
        mid = len(costs) // 2
        if mid == 0:
            return {'trend': 'insufficient_data', 'average_cost': avg_cost, 'change_percentage': 0}
        
        first_half_avg = sum(costs[:mid]) / mid if mid > 0 else 0
        second_half_avg = sum(costs[mid:]) / (len(costs) - mid) if mid < len(costs) else 0
...
class CostOptimizer:
    ...
    def optimize_storage(self, 
                        ...
                        dry_run: bool = False) -> OptimizationReport:
        """
        ...
        try:
            ...
            if report.cost_before and report.cost_after:
                report.savings = report.cost_before.total_cost - report.cost_after.total_cost
                if report.cost_before.total_cost > 0:
                    report.savings_percentage = (report.savings / report.cost_before.total_cost) * 100
                ...
                if report.savings > 0:
                    ...
                    if self.metrics.successful_optimizations > 0:
                        self.metrics.average_savings_percentage = (
                            self.metrics.total_savings / self.metrics.successful_optimizations
                        )
                else:
            ...
            logger.info(f"Storage optimization completed. Savings: ${report.savings:.2f}")
    ...
    def optimize_retrieval(self,
        ...
        try:
            logger.info(f"Starting retrieval optimization with {strategy.value} strategy")
            report.cost_before = self.analyzer.analyze_current_costs()
            ...
            if hasattr(self.memory, 'persistent_store') and hasattr(self.memory.persistent_store, 'enable_adaptive_range'):
                try:
            ...
            if hasattr(self.memory, 'prefetcher') and hasattr(self.memory.prefetcher, 'enable_intelligent_prefetch'):
                try:
    ...
    def optimize_full(self,
        ...
        try:
            ...
            if consolidated_report.cost_before and consolidated_report.cost_after:
                ...
                if consolidated_report.cost_before.total_cost > 0:
                    consolidated_report.savings_percentage = (
                        consolidated_report.savings / consolidated_report.cost_before.total_cost
                    ) * 100
            
            # Add metadata
            consolidated_report.metadata['storage_report_id'] = storage_report.optimization_id
            consolidated_report.metadata['retrieval_report_id'] = retrieval_report.optimization_id
            ...
            logger.info(f"Full optimization completed. Total savings: ${consolidated_report.savings:.2f}")
    ...
    def check_budget(self) -> Dict[str, Any]:
        ...
        try:
            ...
            usage_percentage = monthly_cost / self.budget_config.monthly_budget if self.budget_config.monthly_budget > 0 else 0
            ...
            status = {
                ...
                'remaining_budget': max(0, self.budget_config.monthly_budget - monthly_cost),
                'status': 'ok'
            }
        ...
        try:
            ...
            if 'reduce_storage_by_gb' in changes:
                try:
                    ...
                    storage_savings = reduction_gb * self.analyzer.pricing['storage_cold_gb_month']
                    projected_costs.storage_cost = max(0, projected_costs.storage_cost - storage_savings)
                except (ValueError, TypeError) as e:
            ...
            if 'reduce_storage_by_percent' in changes:
                try:
                    reduction_percent = float(changes['reduce_storage_by_percent']) / 100.0
                    if 0 <= reduction_percent <= 1:
                        projected_costs.storage_cost = current_costs.storage_cost * (1 - reduction_percent)
                    else:
                        logger.warning(f"Invalid storage reduction percentage: {reduction_percent * 100}%")
                except (ValueError, TypeError) as e:
            ...
            if 'reduce_bandwidth_by_gb' in changes:
                try:
                    ...
                    bandwidth_savings = reduction_gb * self.analyzer.pricing['bandwidth_gb']
                    projected_costs.bandwidth_cost = max(0, projected_costs.bandwidth_cost - bandwidth_savings)
                except (ValueError, TypeError) as e:
            ...
            if 'reduce_bandwidth_by_percent' in changes:
                try:
                    reduction_percent = float(changes['reduce_bandwidth_by_percent']) / 100.0
                    if 0 <= reduction_percent <= 1:
                        projected_costs.bandwidth_cost = current_costs.bandwidth_cost * (1 - reduction_percent)
                    else:
                        logger.warning(f"Invalid bandwidth reduction percentage: {reduction_percent * 100}%")
                except (ValueError, TypeError) as e:
            ...
            if 'increase_cdn_hit_rate' in changes:
                try:
                    ...
                    if 0 <= hit_rate_increase <= 1:
                        ...
                        bandwidth_savings = current_costs.bandwidth_cost * hit_rate_increase
                        projected_costs.bandwidth_cost = max(0, projected_costs.bandwidth_cost - bandwidth_savings)
                    else:
                        logger.warning(f"Invalid CDN hit rate value: {hit_rate_increase}")
                except (ValueError, TypeError) as e:
            ...
            if 'reduce_api_calls_by_percent' in changes:
                try:
                    reduction_percent = float(changes['reduce_api_calls_by_percent']) / 100.0
                    if 0 <= reduction_percent <= 1:
                        projected_costs.api_cost = current_costs.api_cost * (1 - reduction_percent)
                    else:
                        logger.warning(f"Invalid API reduction percentage: {reduction_percent * 100}%")
                except (ValueError, TypeError) as e:
            ...
            if 'migrate_to_cold_tier_gb' in changes:
                try:
                    ...
                    projected_costs.storage_cost = max(0, projected_costs.storage_cost - savings)
                except (ValueError, TypeError) as e:
            ...
            savings_percentage = (potential_savings / current_costs.total_cost * 100) if current_costs.total_cost > 0 else 0
    ...
    def _create_optimization_plan(self,
        ...
        for opp in opportunities:
            ...
            if strategy == OptimizationStrategy.AGGRESSIVE:
                ...
                should_execute = opp.get('priority') == 'high' and opp.get('type') != 'pruning'
            elif strategy == OptimizationStrategy.PERFORMANCE_FOCUSED:
                # Only do optimizations that improve performance
                should_execute = opp.get('type') in ['cdn_optimization', 'tier_migration']
    ...
    def _execute_storage_optimizations(self,
        ...
        for step in plan:
            try:
                ...
                if action_type == 'deduplication':
                    if hasattr(self.memory, 'dedup_engine') and hasattr(self.memory.dedup_engine, 'fold_ir_atoms'):
                        try:
                    ...
                    if hasattr(self.memory, 'quantizer') and hasattr(self.memory.quantizer, 'apply_rotational_quantization'):
                        try:
                            self.memory.quantizer.apply_rotational_quantization(error_correction=True)
                    ...
                    if hasattr(self.memory, 'tier_c') and hasattr(self.memory.tier_c, 'convert_to_binary_embeddings'):
                        try:
                            self.memory.tier_c.convert_to_binary_embeddings()
                            report.actions_taken.append("Converted to binary embeddings")
                        except Exception as e:
                            report.warnings.append(f"Tier migration failed: {str(e)}")
                
                elif action_type == 'pruning':
                    if hasattr(self.memory, 'tier_c') and hasattr(self.memory.tier_c, 'prune_low_utility'):
                        try:
                            threshold = 0.1 if strategy == OptimizationStrategy.AGGRESSIVE else 0.05
                            self.memory.tier_c.prune_low_utility(threshold=threshold, disk_based=True)
                            report.actions_taken.append(f"Pruned low utility items (threshold={threshold})")
                        except Exception as e:
                            report.warnings.append(f"Pruning failed: {str(e)}")
                
                elif action_type == 'cdn_optimization':
                    if hasattr(self.memory, 'cdn') and hasattr(self.memory.cdn, 'optimize_cache_policy'):
                        try:
                            self.memory.cdn.optimize_cache_policy()
