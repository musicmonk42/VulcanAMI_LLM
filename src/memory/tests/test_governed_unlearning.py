...
from governed_unlearning import (
    ProposalStatus, UnlearningMethod, UrgencyLevel,
    IRProposal, GovernanceResult, UnlearningTask,
    UnlearningMetrics, UnlearningAuditLogger,
    GovernedUnlearning
)
...
class TestUnlearningTask:
    ...
    def test_get_duration(self):
        proposal = IRProposal("prop-1", {}, "user-1")
        task = UnlearningTask("task-1", proposal, UnlearningMethod.EXACT_REMOVAL, "test", [])
...
class TestGovernedUnlearning:
    ...
    def mock_memory(self):
        ...
        memory.zk_prover.generate_unlearning_proof = Mock(return_value={'proof': 'test'})
        return memory
    ...
    def test_propose_unlearning(self, mock_memory):
        ...
        proposal_id = system.submit_ir_proposal(
            ir_content={"pattern": "sensitive_data"},
            proposer_id="user-1"
        )
    ...
    def test_multiple_proposals(self, mock_memory):
        ...
        for i in range(5):
            system.submit_ir_proposal(
                ir_content={"pattern": f"data_{i}"},
                proposer_id=f"user-{i}"
            )
