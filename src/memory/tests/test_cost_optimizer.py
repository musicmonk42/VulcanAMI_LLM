...
from cost_optimizer import (
    OptimizationStrategy, OptimizationPhase, CostBreakdown,
    OptimizationReport, OptimizationMetrics, BudgetConfig,
...
class TestOptimizationReport:
    ...
    def test_get_duration_complete(self):
        start = time.time()
        report = OptimizationReport(
            optimization_id="test", strategy=OptimizationStrategy.BALANCED,
            phase=OptimizationPhase.COMPLETED, started_at=start, completed_at=start + 5.0
        )
...
class TestOptimizationMetrics:
    def test_get_success_rate(self):
        metrics = OptimizationMetrics(total_optimizations=10, successful_optimizations=8)
...
class TestCostOptimizer:
    ...
    def mock_memory(self):
        ...
        memory.dedup_engine.estimate_duplicates.return_value = {'potential_savings_gb': 20.0}
