...
ROLLING_WINDOW_MULTIPLIER = (
    2  # Multiplier for deque maxlen (2x rate limit provides adequate rolling window)
)
...
class RateLimiter:
    ...
    def check_limit(self, provider_name: str) -> bool:
        ...
        if not limit_info:
            logger.debug(
                f"No rate limits defined for provider '{provider_name}'. Allowing call."
            )
...
class OpenAIProvider(AIProvider):
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            if AIOHTTP_AVAILABLE:
                ...
                raise ImportError(
                    "aiohttp is required for async OpenAI calls but not installed."
                )
        ...
        try:
            if task.operation == "EMBED":
                ...
                result = await self._classify(
                    task, contract
                )  # Typically uses generate/complete
            else:
                result = AIResult(
                    status="FAILED",
            ...
            if result:
                result.latency_ms = latency_ms  # Set latency on the result object
            ...
            if contract.max_latency_ms and latency_ms > contract.max_latency_ms:
                if result:
                    ...
                    result.metadata["warning"] = (
                        f"Latency exceeded contract: {latency_ms:.2f}ms > {contract.max_latency_ms}ms"
                    )
                else:
                    result = AIResult(
                        status="FAILED",
                        error="Result object missing after operation",
            ...
            logger.error(
                f"OpenAI provider error during {task.operation}: {e}", exc_info=True
            )
            latency_ms = (time.time_ns() - start_time_ns) / 1_000_000.0
            return AIResult(
                status="FAILED",
    ...
    async def _embed(self, task: AITask, contract: AIContract) -> AIResult:
        ...
        if not AIOHTTP_AVAILABLE:
            return AIResult(
                status="FAILED",
                error="aiohttp is required for OpenAI provider",
                error_code=AI_ERRORS.AI_INTERNAL_ERROR.value,
                trace_id=task.trace_id,
            )

        api_payload = {"input": text, "model": task.model}
        ...
        if requested_dim is not None:
            try:
                ...
                logger.info(
                    f"Requesting OpenAI embedding with dimensions: {api_payload['dimensions']}"
                )
            except (ValueError, TypeError):
                logger.warning(
                    f"Invalid 'dim' value ({requested_dim}) provided in task payload, ignoring."
                )
        ...
        try:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    error_code = error_info.get(
                        "code", AI_ERRORS.AI_PROVIDER_ERROR.value
                    )
                    status_map = {
                        401: AI_ERRORS.AI_UNAUTHORIZED,
                        429: AI_ERRORS.AI_RATE_LIMIT,
                        500: AI_ERRORS.AI_INTERNAL_ERROR,
                    }
                    api_error_code = status_map.get(
                        response.status, AI_ERRORS.AI_PROVIDER_ERROR
                    ).value
                    logger.error(
            ...
            return AIResult(
                ...
                error_code=AI_ERRORS.AI_NETWORK_ERROR.value,
                trace_id=task.trace_id,
            )
            ...
            logger.error(
                f"Unexpected error during OpenAI embedding: {e}", exc_info=True
            )
            return AIResult(
                status="FAILED",
                error=str(e),
    ...
    async def _generate_or_complete(
        self, task: AITask, contract: AIContract
    ) -> AIResult:
        """Generate text completion with real OpenAI API call"""
    ...
    async def _classify(self, task: AITask, contract: AIContract) -> AIResult:
        ...
        gen_result = await self._generate_or_complete(
            generation_task, classification_contract
        )
...
class AnthropicProvider(AIProvider):
    ...
    async def execute(self, task: AITask, contract: AIContract) -> AIResult:
        ...
        if task.operation not in self.supported_operations:
            return AIResult(
                status="FAILED",
                error=f"Operation {task.operation} not supported",
                error_code=AI_ERRORS.AI_UNSUPPORTED.value,
                trace_id=task.trace_id,
            )
...
class GrokProvider(AIProvider):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        )  # Note: Actual URL will differ when API is released
    ...
    async def execute(self, task: AITask, contract: AIContract) -> AIResult:
        ...
        # Since Grok API doesn't exist yet, return a clear error for production use
        logger.warning(
        ...
        if task.operation == "REASON":
            ...
            # Placeholder: Would call real Grok embedding API when available
            dim = 768
            if NUMPY_AVAILABLE:
        ...
        elif task.operation in ["GENERATE", "ANALYZE"]:
            # Placeholder: Would call real Grok generation API when available
            text_response = (
                f"[PLACEHOLDER - Grok API not available] "
                f"Analysis of: {task.payload.get('prompt', 'No input')[:50]}..."
            )
            ...
            return AIResult(
                status="FAILED",
                error=f"Unsupported Grok operation: {task.operation}",
                error_code=AI_ERRORS.AI_UNSUPPORTED.value,
                trace_id=task.trace_id,
            )
        ...
        if isinstance(response_content, str):
            ...
            and len(response_content) > 0
            and isinstance(response_content[0], str)
        ):
            tokens = sum(len(s.split()) for s in response_content)
        elif isinstance(response_content, list):
            tokens = len(response_content)

        placeholder_response = {
            "id": f"grok_placeholder_{random.randint(1000, 9999)}",
            "response": response_content,
            "model": task.model,
            "usage": {"tokens": tokens},
            "note": "Placeholder - Grok API not publicly available",
        }
        ...
        return AIResult(
            ...
            * (tokens / 1000),  # Estimated cost - will change when API is available
            metadata={
                "provider": "xAI",
                "version": "placeholder",
                "warning": "Grok API not publicly available",
            },
            provider_response=placeholder_response,
            trace_id=task.trace_id,
        )

    def _grok_reasoning_steps(self, payload: Dict[str, Any]) -> List[str]:
        """Simulate Grok's reasoning capability (placeholder)"""
        prompt = payload.get("prompt", "")
        steps = [
            f"Analyzing: {prompt[:30]}",
            "Applying principles",
        ...
        if task.operation == "EMBED":
            ...
            try:
                ...
                if not (1 <= dim <= 8192):  # Clamp to a reasonable range
                    logger.warning(
                        f"MockProvider received unreasonable dim {dim}, clamping to 768."
                    )
                    dim = 768
            except (ValueError, TypeError):
...
class LocalGPTAIProvider(AIProvider):
    ...
    async def execute(self, task: AITask, contract: AIContract) -> AIResult:
        ...
        max_new_tokens = int(
            task.payload.get("max_new_tokens")
            or task.payload.get("max_tokens")
            or (contract.max_tokens or 128)
        )
        temperature = float(
            task.payload.get("temperature") or contract.temperature or 0.7
        )
        top_p = float(
            task.payload.get("top_p")
            or (contract.top_p if contract.top_p is not None else 0.95)
        )
        ...
        with self._lock:
            ...
            if cached_data:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to reconstruct cached AIResult for key {key}: {e}"
                    )
                    del self.cache[key]
                    return None
        return None
...
class AIRuntime:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"AIRuntime initialized with providers: {list(self.providers.keys())}"
        )
    ...
    def _initialize_providers(self):
        ...
        try:
            ...
            if api_key:
                ...
                logger.warning(
                    "OPENAI_API_KEY not found, OpenAI provider not registered."
                )
        except Exception as e:
        ...
        try:
            artifacts_dir = (
                self.config.get("local_gpt_artifacts_dir")
                or os.getenv("LOCAL_GPT_ARTIFACTS_DIR")
                or ""
            )
            if artifacts_dir and os.path.isdir(artifacts_dir):
                try:
                    ...
                    logger.info(
                        f"Registered LocalGPT provider from artifacts: {artifacts_dir}"
                    )
                except Exception as e:
                ...
                if artifacts_dir:
                    logger.error(
                        f"Configured local_gpt_artifacts_dir does not exist: {artifacts_dir}"
                    )
                else:
                    logger.info(
                        "No LocalGPT artifacts configured; skipping local-gpt registration."
                    )
        except Exception as e:
        ...
        if "default" not in self.providers:
            ...
            if desired_default and desired_default in self.providers:
                ...
                logger.info(
                    f"Registered {desired_default} provider as default (from config)."
                )
    ...
    def execute_task(self, task: AITask, contract: AIContract) -> AIResult:
        ...
        if task.is_expired():
            logger.warning(
                f"Task {task.trace_id} is expired (deadline: {task.deadline})."
            )
        ...
        if not ok:
            ...
            latency_ms = (
                time.time_ns() - start_t_ns
            ) / 1_000_000.0  # Calculate latency even for validation failure
            ...
            self.metrics.record_execution(
                task.provider, task.operation, validation_result
            )
        ...
        if not provider:
            ...
            self.metrics.record_execution(
                task.provider or "unknown", task.operation, no_provider_result
            )
        ...
        if contract.allow_cached:
            ...
            if cached_result:
                ...
                self.metrics.record_execution(
                    task.provider, task.operation, cached_result
                )
        ...
        try:
            if not asyncio.iscoroutinefunction(provider.execute):
                logger.warning(
                    f"Provider {task.provider} execute method is synchronous."
                )
                # Directly call the sync method
                result = provider.execute(
                    task, contract
                )  # Type checker might complain here
            else:
                try:
                    ...
                    logger.error(
                        f"execute_task (sync) called from within an async context for task {task.trace_id}. Use execute_task_async.",
                        exc_info=True,
                    )
                    raise RuntimeError(
                        "Synchronous execute_task cannot be called directly from an async function. Use await runtime.execute_task_async() instead or call from sync context."
                    )

                except (
                    RuntimeError
                ):  # No running event loop, so asyncio.run is appropriate
                    logger.debug(
                        f"Creating new event loop for provider.execute task {task.trace_id}"
                    )
                    result = asyncio.run(provider.execute(task, contract))

            if not isinstance(result, AIResult):
                raise TypeError(
                    f"Provider {task.provider} returned unexpected type {type(result)}"
                )
            ...
            logger.error(
                f"Error executing task {task.trace_id} with provider {task.provider}: {e}",
                exc_info=True,
            )
            latency_ms = (time.time_ns() - start_t_ns) / 1_000_000.0
            ...
            self.metrics.record_execution(
                task.provider or "unknown", task.operation, error_result
            )
            return error_result
    ...
    async def execute_task_async(self, task: AITask, contract: AIContract) -> AIResult:
        ...
        try:
            ...
            if not isinstance(result, AIResult):
                raise TypeError(
                    f"Provider {task.provider} returned invalid type {type(result)}"
                )
            ...
            logger.error(
                f"Async error executing task {task.trace_id} with provider {task.provider}: {e}",
                exc_info=True,
            )
            latency_ms = (time.time_ns() - start_t_ns) / 1_000_000.0
            error_result = AIResult(
    ...
    async def shutdown(self):
        ...
        for provider_name, provider in self.providers.items():
            try:
                if hasattr(provider, "close_session") and asyncio.iscoroutinefunction(
    ...
    def cleanup(self):
        ...
        try:
            ...
            logger.warning(
                f"Could not run async shutdown during sync cleanup: {e}. Proceeding with sync cleanup."
            )
            ...
            for provider in self.providers.values():
                if hasattr(provider, "sync_cleanup"):
                    provider.sync_cleanup()
        except Exception as e:
    ...
    def _route_to_provider(self, task: AITask) -> Optional[AIProvider]:
        ...
        if (
            not task.provider or task.provider.lower() == "default"
        ) and self.routing_strategy == "round_robin":
            available_providers = [
                p
                for name, p in self.providers.items()
                if name != "default" and p.supports_operation(task.operation)
            ]
            if available_providers:
                ...
                logger.debug(
                    f"Round-robin routing task {task.trace_id} to provider index {provider_idx}"
                )
                return selected_provider
            else:
                logger.warning(
                    f"Round-robin failed: No non-default providers support operation '{task.operation}'. Falling back."
                )
    ...
    def execute_task_sync(
        self, task: AITask, contract: Optional[AIContract] = None
    ) -> AIResult:
    ...
    async def batch_execute(
        self, tasks: List[AITask], contract: Optional[AIContract] = None
    ) -> List[AIResult]:
        ...
        for i, res in enumerate(results):
            if isinstance(res, Exception):
                logger.error(
                    f"Exception during batch execution for task {tasks[i].trace_id}: {res}",
                    exc_info=res,
                )
                ...
                logger.error(
                    f"Unexpected result type in batch execution for task {tasks[i].trace_id}: {type(res)}"
                )
                final_results.append(
                    AIResult(
                        status="FAILED",
...
class AIMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            summary["avg_latency_ms_overall"] = (
                (total_latency_all / total_execs) if total_execs > 0 else 0.0
            )
