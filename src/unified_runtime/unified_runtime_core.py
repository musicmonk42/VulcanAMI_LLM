...
try:
    from .execution_engine import ExecutionEngine, ExecutionContext, ExecutionMode, ExecutionStatus
except ImportError:
...
class UnifiedRuntime:
    ...
    def __init__(self, config: Optional[RuntimeConfig] = None):
        ...
        if self.config.learned_subgraphs_dir:
            ...
            if learned_path_obj.is_absolute() or learned_path_obj.exists():
                self.learned_subgraphs_path = learned_path_obj
                 ...
                 self.learned_subgraphs_path = base_path / self.config.learned_subgraphs_dir
        else:
        ...
        logger.info(f"Runtime initialized. Grammar {self.schema.get('version', 'unknown')} loaded")
        logger.info(f"Components: Hardware={bool(self.hardware_dispatcher)}, "
    ...
    def _initialize_components(self):
        ...
        if RuntimeExtensions:
            ...
            enable_autonomous_flag = getattr(self.config, 'enable_autonomous', True) # Get flag safely
        ...
        if self.config.enable_distributed:
            self.sharder = DistributedSharder() if SHARDER_AVAILABLE and DistributedSharder else None
            self.obs_manager = ObservabilityManager() if OBSERVABILITY_AVAILABLE and ObservabilityManager else None
        else:
            self.sharder = None
        ...
        if hasattr(self.config, 'enable_vulcan_agi') and self.config.enable_vulcan_agi:
            self.multimodal_processor = MultimodalProcessor() if MULTIMODAL_AVAILABLE and MultimodalProcessor else None
            self.cross_modal_reasoner = CrossModalReasoner() if REASONER_AVAILABLE and CrossModalReasoner else None
            self.continual_learner = ContinualLearner() if LEARNER_AVAILABLE and ContinualLearner else None
            self.safety_validator = SafetyValidator() if SAFETY_AVAILABLE and SafetyValidator else None
            self.autobiographical_memory = AutobiographicalMemory() if MEMORY_AVAILABLE and AutobiographicalMemory else None
        else:
            ...
            if self.learned_subgraphs_path.exists():
                for f in self.learned_subgraphs_path.glob("*.json"):
                    with open(f, "r") as fh:
                        ...
                        if key:
                            self.subgraph_definitions[key] = data.get("graph_definition", data)

            # Also load from extensions
            if self.extensions:
                subgraphs = self.extensions.load_learned_subgraphs()
                self.subgraph_definitions.update(subgraphs)
                logger.info(f"Loaded {len(self.subgraph_definitions)} total learned subgraphs")
        except Exception as e:
    ...
    def set_verbosity(self, level: float):
        ...
        level = float(level) if not isinstance(level, bool) and level is not None else 0.0
    ...
    async def execute_graph(self, graph: Dict[str, Any], mode: Optional['ExecutionMode']=None, **kwargs) -> Dict[str, Any]:
        """
        ...
        # 4. Run engine
        exec_metrics = ExecutionMetrics() if ExecutionMetrics and self.config.enable_metrics else None
        ...
        if exec_metrics:
            ...
            if hasattr(exec_metrics, 'metadata') and isinstance(exec_metrics.metadata, dict): # Check if metadata exists
                exec_metrics.metadata["hardware"] = (
                    self.hardware_dispatcher.get_health_snapshot()
                    if self.hardware_dispatcher and hasattr(self.hardware_dispatcher, 'get_health_snapshot') else {}
                )
            if self._metrics_aggregator:
                self._metrics_aggregator.record_metrics(exec_metrics) # Corrected method name
        ...
        if self.extensions:
            try:
                ...
                if hasattr(self.extensions, 'on_run_complete') and callable(self.extensions.on_run_complete):
        ...
        if isinstance(graph_result, dict):
            ...
            logger.error(f"Execution engine returned unexpected type: {type(graph_result)}")
            return {"status": "failed", "errors": {"_graph": "Internal error: Unexpected execution result type"}}
        ...
        try:
            ...
            if hasattr(self.extensions, 'trigger_autonomous_cycle'):
                ...
                if report.get('optimizations_applied'):
                    logger.info(f"Applied {len(report['optimizations_applied'])} optimizations")
    ...
    async def execute_stream(self, graph: Dict[str, Any]) -> AsyncIterator[Dict[str, Any]]:
        """
        ...
        if self.validator:
            ...
            if hasattr(self.validator, "validate_with_timeout"):
                validation = self.validator.validate_with_timeout(
                    graph,
                    manifest_node_types=manifest_node_types,
                    timeout_s=timeout_s
                )
            else:
                validation = self.validator.validate_graph(
            ...
            validation = ValidationResult(is_valid=True) if ValidationResult else {'valid': True, 'errors': [], 'warnings': []}

        is_valid = getattr(validation, 'is_valid', validation.get('valid', True)) if validation else True
        errors = getattr(validation, 'errors', validation.get('errors', ['Unknown validation error'])) if validation else []
    ...
    async def execute_batch(self, graphs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i in range(0, len(graphs), batch_size):
            ...
            if total_nodes > max_nodes:
                mid = len(batch) // 2
                if mid == 0:
                    logger.warning(f"Single graph exceeds node limit ({total_nodes} > {max_nodes}). Skipping.")
                    results.append({'status': 'failed', 'error': 'Graph exceeds node limit', 'batch_index': i // batch_size, 'item_index': 0})
                    continue
                ...
                if isinstance(result, Exception):
                    ...
                    if isinstance(result, dict):
                        ...
                        results.append(result)
                         ...
                         logger.error(f"Batch execution got unexpected result type {type(result)}")
                         results.append({
                            'status': 'failed',
    ...
    def learn_subgraph(self, graph_definition: Dict[str, Any]) -> bool:
        ...
        if success:
            ...
            if isinstance(nodes_info, list):
                 ...
                 logger.info(f"Learned subgraph (details unavailable): {graph_definition.get('id', 'unknown')}")
        return success
    ...
    async def _dispatch_to_hardware(self, operation: str, *args, **kwargs) -> Any:
        ...
        if hasattr(self.hardware_dispatcher, 'run_tensor_op'):
            ...
            def op_closure():
                ...
                logger.warning("Generic _dispatch_to_hardware closure used. Node should provide its own.")
            ...
            dispatch_result = await self.hardware_dispatcher.run_tensor_op(op=op_closure)
    ...
    def introspect(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            'node_types': {
                'core': sorted([k for k in self.node_executors.keys() # Use self.node_executors
            ...
            'components': {
                ...
                'metrics': self._metrics_aggregator is not None, # Use self._metrics_aggregator
        ...
        if self._metrics_aggregator and hasattr(self._metrics_aggregator, 'get_summary'):
        ...
        if self.vulcan_bridge:
            try:
                get_stats = getattr(self.vulcan_bridge, 'get_integration_statistics', None)
    ...
    def get_hardware_metrics(self) -> Dict[str, Any]:
        ...
        if self.hardware_dispatcher and hasattr(self.hardware_dispatcher, 'get_metrics_summary'):
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        if self._metrics_aggregator and hasattr(self._metrics_aggregator, 'get_summary'): # Check aggregator
            return self._metrics_aggregator.get_summary()
        # Fallback if metrics are disabled or the object/method is missing
        return {"execution_count_total": 0, "cache_hits_total": 0, "cache_misses_total": 0, "nodes_executed_total": 0, "avg_latency_ms_per_execution": 0.0}


    def validate_graph(self, graph: Dict[str, Any]) -> Dict[str, Any]:
        """Validate a graph structure"""
        if self.validator:
            manifest_node_types = self.manifest.get('node_types', {}) # Should probably be manifest.get('nodes', {})
            ...
            if hasattr(result, 'to_dict'):
                return result.to_dict()
                 ...
                 return {'valid': False, 'errors': ['Validation returned unexpected result type'], 'warnings': []}

        else:
            return {'valid': False, 'errors': ['Graph validation disabled (Validator not initialized)'], 'warnings': []}
    ...
    async def shutdown(self):
        ...
        try:
            ...
            # ai runtime cleanup
            if hasattr(self, "ai_runtime") and self.ai_runtime and hasattr(self.ai_runtime, "shutdown"):
            ...
            if hasattr(self, "hardware_dispatcher") and self.hardware_dispatcher:
                ...
                if hasattr(self.hardware_dispatcher, "executor") and self.hardware_dispatcher.executor: # Corrected attribute name
                    self.hardware_dispatcher.executor.shutdown(wait=False) # Corrected attribute name
    ...
    def cleanup(self):
        ...
        if loop and is_running:
            ...
            logger.debug("No event loop found, running async shutdown via asyncio.run()...")
            try:
...
async def execute_graph(graph: Dict[str, Any], mode: Optional['ExecutionMode']=None) -> Dict[str, Any]:
