...
class HardwareProfileManager:
    ...
    def select_backend(
        self, tensor_size_mb: float, strategy: DispatchStrategy
    ) -> Optional[HardwareBackend]:
...
class HardwareDispatcherIntegration:
    ...
    def __init__(
        ...
        if self.enable_hardware:
            try:
                self.hardware_dispatcher = HardwareDispatcher(
                    enable_metrics=True, enable_health_checks=True, prefer_grpc=False
                )
        ...
        logger.info(
            f"HardwareDispatcherIntegration initialized. Hardware: {self.enable_hardware}, Emulator: {self.enable_emulator}"
        )

    async def dispatch_to_hardware(
        ...
        if (
            ...
            and self.hardware_dispatcher
            and backend != HardwareBackend.EMULATOR
        ):
            try:
                result = await self._dispatch_to_real_hardware(
                    operation, backend, *args, **kwargs
                )
                if not result.error:
        ...
        if self.enable_emulator:
            ...
            result = await self.dispatch_to_emulator_fallback(
        ...
        logger.warning(
            f"All dispatch methods failed, using CPU fallback for {operation}"
        )
        result = await self._cpu_fallback(operation, *args, **kwargs)
        self._update_metrics(result)
        return result

    async def _dispatch_to_real_hardware(
        self, operation: str, backend: HardwareBackend, *args, **kwargs
    ) -> DispatchResult:
    ...
    async def dispatch_to_emulator_fallback(
        self, operation: str, *args, **kwargs
    ) -> DispatchResult:
        """
        ...
        try:
            if operation == "photonic_mvm" and len(args) >= 2:
                result = await self._emulate_photonic_mvm(
                    args[0], args[1], kwargs.get("params", {})
                )
                backend = HardwareBackend.PHOTONIC
                ...
                result = await self.dispatch_to_emulator(
                    subgraph, backend="photonic", **kwargs
                )
                backend = HardwareBackend.PHOTONIC
            else:
            ...
            if profile:
                ops_count = self._estimate_operation_count(operation, args)
                energy_nj = (
                    profile.energy_per_op_nj * ops_count * 1.5
                )  # Emulation overhead
    ...
    async def _emulate_photonic_mvm(
        self, matrix: Any, vector: Any, params: Dict[str, Any]
    ) -> Any:
        """Emulate photonic matrix-vector multiplication"""
        if EMULATOR_AVAILABLE and analog_photonic_emulator:
            # Apply compression if specified
            if (
                params.get("compression") == "ITU-F.748-quantized"
                and COMPRESSOR_AVAILABLE
            ):
                logger.debug("Applying ITU-F.748 compression")
                matrix = llm_compressor.quantize_tensor(
                    matrix, config={"precision": "8bit"}
                )
            ...
            if hasattr(vector, "ndim") and vector.ndim == 1:
                ...
                isinstance(vector, list) and vector and not isinstance(vector[0], list)
            ):
            ...
            return self._emulate_mvm_pure(
                matrix, vector, noise_std=params.get("noise_std", 0.01)
            )
        ...
        if EMULATOR_AVAILABLE and analog_photonic_emulator:
            ...
            if TORCH_AVAILABLE:
                ...
                result = analog_photonic_emulator.emulate_memristor_cim(
                    tensor1, op=lambda t: t @ op_tensor
                )
                return result.cpu().numpy() if hasattr(result, "cpu") else result
            else:
                # Numpy-based emulation
                return analog_photonic_emulator.emulate_memristor_cim(
                    tensor1, op=lambda t: np.dot(t, tensor2)
                )
        else:
            # Pure fallback
            return self._emulate_mvm_pure(
                tensor1, tensor2, noise_std=0.02, scaling_factor=0.98
            )

    def _emulate_mvm_pure(
        self,
        tensor1: Any,
        tensor2: Any,
        ...
        if np:
            ...
            if tensor1.size == 0 or tensor2.size == 0:
                raise ValueError(
                    "Cannot perform matrix multiplication on empty tensors"
                )
            ...
            if N != len(tensor2):
                raise ValueError(
                    f"Matrix dimensions incompatible: {N} vs {len(tensor2)}"
                )
            ...
            for i in range(M):
                ...
                for j in range(P):
                    ...
                    for k in range(N):
                        a = (
                            tensor1[i][k]
                            if isinstance(tensor1[i], list)
                            else tensor1[i]
                        )
                        b = (
                            tensor2[k][j]
                            if isinstance(tensor2[k], list)
                            else tensor2[k]
                        )
    ...
    async def dispatch_to_emulator(
        self, subgraph: Dict, backend: str = "photonic", **kwargs
    ) -> Any:
        """
        ...
        if len(inputs) < 2:
            raise ValueError(
                f"Emulator dispatch requires at least 2 tensors, got {len(inputs)}"
            )
        ...
        if backend == "photonic":
            if hasattr(tensor2, "ndim") and tensor2.ndim == 1:
                ...
                isinstance(tensor2, list)
                and tensor2
                and not isinstance(vector[0], list)
            ):
                tensor2 = [[v] for v in tensor2]

        logger.info(
            f"Dispatching to {backend} emulator: shapes {getattr(tensor1, 'shape', 'N/A')} x {getattr(tensor2, 'shape', 'N/A')}"
        )
        ...
        if backend == "photonic":
            ...
            if (
                hasattr(analog_photonic_emulator, "train_in_situ")
    ...
    async def optimize_and_dispatch(
        self, subgraph: Dict, metrics: Dict[str, float]
    ) -> DispatchResult:
        """
        ...
        logger.info(
            f"Dynamic dispatch chose backend='{backend.value}' for {tensor_size_mb:.2f}MB tensor, strategy={strategy.value}"
        )
        ...
        try:
            if backend in [HardwareBackend.PHOTONIC, HardwareBackend.MEMRISTOR]:
                ...
                result = await self.dispatch_to_emulator(
                    subgraph, backend=backend.value
                )
                latency_ms = (time.perf_counter() - start_time) * 1000
                return DispatchResult(
                    backend=backend,
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_backends": [
                b.value for b in self.profile_manager.get_available_backends()
            ],
            "backend_profiles": {
    ...
    def _estimate_tensor_size_mb(self, tensor: Any) -> float:
        ...
        try:
            ...
            logger.warning(
                f"Could not estimate tensor size for type {type(tensor)}: {e}"
            )
        return 1.0  # Default 1MB

    async def run_tensor_op(
        self, op: Callable[[], Any], estimated_tensor_mb: Optional[float] = None
    ) -> DispatchResult:
        """
        ...
        try:
            ...
            backend_choice = self.profile_manager.select_backend(
                tensor_mb, self.strategy
            )
...
async def dispatch_to_emulator(
    subgraph: Dict, backend: str = "photonic", **kwargs
) -> Any:
