...
class ExecutionScheduler:
    ...
    def _build_dependencies(self) -> Dict[str, Set[str]]:
        ...
        for edge in self.edges:
            ...
            if (
                from_node
                and to_node
                and from_node in self.node_map
                and to_node in self.node_map
            ):  # Check nodes exist
        ...
        if not NETWORKX_AVAILABLE:
            ...
            for node_id in self.node_map:
                if node_id not in visited:
                    if has_cycle(node_id):
                        logger.warning(
                            f"Cycle detected involving node {node_id} (using DFS)"
                        )
                        return True
            ...
            try:
                ...
                if not is_dag:
                    ...
                    logger.warning(
                        f"Cycle detected (using NetworkX): {cycles[0] if cycles else 'unknown'}"
                    )
                return not is_dag
            except Exception as e:
                logger.error(
                    f"NetworkX cycle detection failed: {e}. Falling back to DFS."
                )
    ...
    def get_ready_nodes(self) -> List[str]:
        ...
        for node_id in self.node_map:
            ...
            if (
                node_id in self.executed
                or node_id in self.executing
    ...
    def get_execution_layers(self) -> List[List[str]]:
        ...
        while len(nodes_in_layers) < len(self.node_map):
            ...
            if not layer:
                ...
                if unreachable:
                    logger.warning(
                        f"Could not schedule all nodes. Unreachable/Cyclic?: {unreachable}"
                    )
                break
    ...
    def get_topological_order(self) -> List[str]:
        ...
        if NETWORKX_AVAILABLE:
            try:
                ...
                logger.error(
                    f"NetworkX topological sort failed: {e}. Falling back to manual."
                )
        ...
        if len(result) == len(self.node_map):
            ...
            logger.warning(
                "Manual topological sort failed - graph may contain cycles or be disconnected."
            )
            return []
...
class ExecutionEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"ExecutionEngine initialized with max_parallel={max_parallel}, timeout={self.timeout_seconds}s"
        )  # Log timeout
    ...
    async def run_graph(
        ...
        context.add_audit_entry(
            {"event": "run_graph_start", "mode": mode.value if mode else "unknown"}
        )
        ...
        try:
            ...
            final_result = GraphExecutionResult(
                ...
                nodes_executed=len(scheduler.executed)
                + len(scheduler.failed),  # Count nodes attempted
                metadata={
        ...
        if isinstance(result_or_iterator, GraphExecutionResult):
            ...
            if metrics:
                metrics.finalize_graph()  # Finalize metrics here too
            return result
    ...
    async def _run_parallel(
        ...
        while len(scheduler.executed) + len(scheduler.failed) < len(context.node_map):
            ...
            if not nodes_to_run and not scheduler.executing:
                ...
                unprocessed_nodes = (
                    set(context.node_map.keys()) - scheduler.executed - scheduler.failed
                )
                if unprocessed_nodes:
                    logger.warning(
                        f"Execution stuck? Ready: {ready_nodes}, Executing: {scheduler.executing}, Unprocessed: {unprocessed_nodes}"
                    )
                break  # Exit loop
            ...
            for node_id in nodes_to_run:
                if node_id not in tasks:
                    ...
                    task = asyncio.create_task(
                        self._run_single_node(node_id, context, scheduler, metrics)
                    )
                    tasks[node_id] = task
                    ...
                    try:
                        ...
                        if result.status == ExecutionStatus.SUCCESS:
                            ...
                            scheduler.mark_failed(
                                node_id, result.error or "Unknown node error"
                            )
                            ...
                            if self._is_critical_node(node_id, context):
                                logger.warning(
                                    f"Critical node {node_id} failed. Stopping parallel execution."
                                )
                        ...
                        if self._is_critical_node(node_id, context):
                            logger.warning(
                                f"Critical node {node_id} failed unexpectedly. Stopping parallel execution."
                            )
                            self._shutdown_event.set()  # Signal shutdown
        ...
        status = (
            ...
            if not scheduler.failed and not self._shutdown_event.is_set()
            else ExecutionStatus.FAILED
        )
        if self._shutdown_event.is_set():
            status = ExecutionStatus.CANCELLED  # Or FAILED? Let's use CANCELLED

        output_nodes_map = self._get_output_nodes(context)
        final_outputs = {
            out_id: context.outputs.get(out_id)
            for out_id in output_nodes_map
            if out_id in context.outputs
        }
    ...
    async def _run_sequential(
        ...
        if not exec_order and context.node_map:
            ...
            logger.error(
                "Sequential execution failed: Could not determine valid execution order (cycle?)."
            )
            return GraphExecutionResult(
                status=ExecutionStatus.FAILED,
                errors={
                    "_graph": "Could not determine valid execution order (cycle?)."
                },
                nodes_executed=0,
                metadata={"recursion_depth": context.recursion_depth},
            )
        ...
        for node_id in exec_order:
            ...
            scheduler.mark_executing(node_id)
            try:
                result = await self._run_single_node(
                    node_id, context, scheduler, metrics
                )
                nodes_executed += 1
                if result.status == ExecutionStatus.SUCCESS:
                    scheduler.mark_executed(node_id)
                    ...
                    if self._is_critical_node(node_id, context):
                        logger.warning(
                            f"Critical node {node_id} failed. Stopping sequential execution."
                        )
                ...
                if self._is_critical_node(node_id, context):
                    logger.warning(
                        f"Critical node {node_id} failed unexpectedly. Stopping sequential execution."
                    )
                    break  # Stop processing
    ...
    async def _run_streaming(
        ...
        while len(scheduler.executed) + len(scheduler.failed) < len(context.node_map):
            ...
            if not nodes_to_run and not scheduler.executing:
                ...
                if unprocessed_nodes:
                    logger.warning(
                        f"Streaming execution stuck? Unprocessed: {unprocessed_nodes}"
                    )
                break
            ...
            if processed_task or (current_time - last_yield_time > yield_interval):
                ...
                if scheduler.failed:
                    status = ExecutionStatus.FAILED  # If any failed, mark as failed
        ...
        context.add_audit_entry(
            {"event": "run_graph_end", "status": final_status.value}
        )
    ...
    async def _run_single_node(
            ...
            with self._cache_lock:
                if cache_key in self.execution_cache:
                    ...
                    if metrics and NodeExecutionStats:
                        stats = metrics.record_node_start(
                            node_id=node_id, node_type=node_type, cache_hit=True
                        )
                    # <<< --- END FIX --- >>>
        ...
        if metrics and NodeExecutionStats and not is_cached:
            stats = metrics.record_node_start(
                node_id=node_id, node_type=node_type, cache_hit=False
            )
        # <<< --- END FIX --- >>>
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(handler):
                ...
                output = await loop.run_in_executor(
                    self.executor, handler, node, context.to_dict(), inputs_dict
                )
            ...
            try:
                ...
                logger.warning(
                    f"Output of node {node_id} ({node_type}) is not JSON serializable: {json_err}. Storing raw."
                )
            ...
            context.add_audit_entry(
                {"event": "node_success", "node_id": node_id, "node_type": node_type}
            )
            ...
            logger.error(
                f"Node {node_id} ({node_type}) failed: {err_msg}", exc_info=True
            )
        ...
        if stats and metrics:
            metrics.record_node_end(
                stats, status=result_status.value, error_message=err_msg
            )
    ...
    async def _gather_inputs(
        ...
        for edge in context.graph.get("edges", []):
            edge_id = edge.get(
                "id", f"{edge.get('from')}_{edge.get('to')}"
            )  # Create a unique ID for the edge if none exists
            ...
            if to_node_id == node_id and edge_id not in processed_edges:
                ...
                if from_node_id and from_node_id in context.outputs:
                    ...
                    logger.warning(
                        f"Input edge for {node_id} from {from_node_id} has no output data yet."
                    )
        ...
        if node and node.get("type") in ("INPUT", "InputNode"):  # Match common names
            ...
            if input_key in context.inputs:
                ...
                inputs["output"] = context.inputs[
                    input_key
                ]  # Assuming 'output' is the standard port
                logger.debug(
                    f"Injected graph input '{input_key}' into InputNode {node_id}"
                )
            elif (
                "input" in context.inputs and input_key == "default_input"
            ):  # Handle simple 'input' key
    ...
    def _get_node_executor(
        self, node: Dict[str, Any], context: ExecutionContext
    ) -> Optional[Callable]:
        """
        ...
        logger.warning(
            f"Could not find node executor for type '{node_type}' in runtime."
        )
    ...
    def _compute_cache_key(
        ...
        scheduler = ExecutionScheduler(
            context.graph
        )  # Need scheduler to access dependencies
        ...
        try:
            ...
            params_repr = {
                k: type(v).__name__ for k, v in node.get("params", {}).items()
            }
    ...
    def _is_deterministic_node(self, node: Dict[str, Any]) -> bool:
        ...
        non_deterministic_types = [
            ...
            "SchedulerNode",
            "SearchNode",
            "ExternalAPINode",  # Common examples
            "INPUT",
            "InputNode",  # Input nodes depend on external state
        ]
    ...
    def _should_stop_execution(
        self, context: ExecutionContext, scheduler: ExecutionScheduler
    ) -> bool:
        ...
        for failed_id in scheduler.failed:
            if self._is_critical_node(failed_id, context):
                logger.warning(
                    f"Stopping execution because critical node '{failed_id}' failed."
                )
                return True
        return False
    ...
    def cleanup(self):
        ...
        if not self._shutdown_event.is_set():
            try:
                ...
                logger.warning(
                    f"Could not run async shutdown cleanly (event loop closed?): {e}"
                )
...
def get_global_engine() -> ExecutionEngine:
    ...
    if _global_engine is None:
        ...
        raise RuntimeError(
            "Global engine cannot be initialized directly. Use UnifiedRuntime."
        )
...
async def execute_graph(*args, **kwargs):
    ...
    logger.warning(
        "Module-level execute_graph is deprecated. Use runtime.execute_graph()."
    )
