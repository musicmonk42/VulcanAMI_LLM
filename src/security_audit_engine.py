...
class SecurityAuditEngine:
    ...
    def __init__(self, db_path: str = "audit.db", max_connections: int = 5):
        ...
        if slack_token and self.slack_channel:
            ...
            self.logger.info("Slack environment variables not set. Real-time alerting is disabled.")
        ...
        self.logger.info(f"Audit engine initialized. Logging to database: {self.db_path}")
        ...
        try:
            ...
            if table_exists:
                ...
                if 'severity' not in columns:
                    self.logger.warning("Old schema detected (missing 'severity' column)")
                    conn.close()
                    ...
                    self.logger.info("Old database removed, will create new one with current schema")
                else:
    ...
    def _send_alert(self, event_type: str, details: Dict, timestamp: str, severity: str = 'critical'):
        ...
        try:
            ...
            message = f"*Critical Security Event*\n" \
            ...
            with self.stats_lock:
                self.stats['errors'] += 1
            self.logger.error(f"Failed to send Slack alert: {e.response.get('error', 'Unknown error')}")
        except Exception as e:
            with self.stats_lock:
                self.stats['errors'] += 1
            self.logger.error(f"An unexpected error occurred while sending Slack alert: {e}")
...
if __name__ == "__main__":
    ...
    with SecurityAuditEngine(db_path="demo_audit.db") as audit:
        ...
        # Log some events
        audit.log_event("user_login", {"user_id": "user123", "ip": "192.168.1.1"}, severity="info")
        audit.log_event("graph_submitted", {"graph_id": "g001", "size": 100}, severity="info")
