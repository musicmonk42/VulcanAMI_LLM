...
class ExplainabilityValidator:
    ...
    def validate_tensor(
        ...
        if isinstance(tensor, list):
            ...
            return (
                False,
                f"Tensor must be list or numpy array, got {type(tensor)}",
                None,
            )
    ...
    def validate_baseline(
        baseline: Any, tensor_shape: Tuple[int, ...]
    ) -> Tuple[bool, Optional[str], Optional[np.ndarray]]:
        """
        ...
        if isinstance(baseline, list):
            ...
            return (
                False,
                f"Baseline must be list or numpy array, got {type(baseline)}",
                None,
            )
...
class ExplainabilityNode:
    ...
    def execute(
        self, tensor: Any, params: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.interpretability_engine is not None:
                try:
                    explanation = self.interpretability_engine.explain_tensor(
                        validated_tensor, baseline=validated_baseline, method=method
                    )
                    ...
                    explanation = {"error": str(e), "fallback": True}
            else:
                logger.info(
                    "InterpretabilityEngine not available, skipping explanation"
                )
                explanation = {
                    "available": False,
            ...
            if self.hardware is not None and hasattr(self.hardware, "get_last_drift"):
                try:
                    ...
                    # Validate drift data
                    is_valid, validated_drift = self.validator.validate_drift_data(
                        drift_data
                    )
            ...
            logger.info(
                f"ExplainabilityNode success: {method}, coverage={coverage:.4f}"
            )
...
class CounterfactualNode:
    ...
    def execute(
        ...
        counterfactual = (
            validated_tensor
            + np.random.randn(*validated_tensor.shape) * perturbation_scale
        )
...
if __name__ == "__main__":
    ...
    cf_node = {
        ...
        "params": {"target_class": 1, "perturbation_scale": 0.2},
    }
    ...
    print(
        f"   All have timestamp: {all('timestamp' in e for e in context4['audit_log'])}"
    )
