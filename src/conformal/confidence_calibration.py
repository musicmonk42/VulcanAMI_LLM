...
class BetaCalibration:
    ...
    def fit(self, predictions: np.ndarray, labels: np.ndarray):
        ...
        if len(pos_preds) > 0 and len(neg_preds) > 0:
            ...
            self.fitted = True
            logger.info(f"Beta calibration fitted: α={self.alpha:.3f}, β={self.beta_param:.3f}")
...
class ConformalPredictor:
    ...
    def fit(self, predictions: np.ndarray, labels: np.ndarray):
        ...
        logger.info(f"Conformal predictor fitted with {len(self.calibration_scores)} scores")
...
class CalibratedDecisionMaker:
    ...
    def add_observation(self, tool_name: str, prediction: float, 
                       actual: bool, features: Optional[np.ndarray] = None):
    ...
    def fit_calibration(self, tool_name: str, method: Optional[str] = None):
        ...
        logger.info(f"Calibration fitted for {tool_name}: ECE={self.metrics[tool_name].ece:.4f}")
    
    def calibrate_confidence(self, tool_name: str, raw_confidence: float,
                           features: Optional[np.ndarray] = None,
    ...
    def get_prediction_set(self, tool_name: str, confidence: float,
                          alpha: float = 0.1) -> Tuple[bool, bool, float]:
        """
    ...
    def compute_metrics(self, predictions: np.ndarray, 
                       labels: np.ndarray) -> CalibrationMetrics:
    ...
    def _reliability_resolution_uncertainty(self, predictions: np.ndarray,
