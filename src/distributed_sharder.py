...
class DistributedSharder:
        ...
        *,
        ...
        if compress:
            try:
                ...
                logger.warning(f"Invalid compression '{compression_type}', using 'gzip'")
        ...
        *,
        ...
        # Structured pruning (channel/row-wise)
            ...
            if input_tensor.ndim < 2:
                logger.warning("Structured pruning requires 2D+ tensor, using magnitude")
                return self.prune_tokens(
                    input_tensor, threshold=threshold,
                    strategy="magnitude", target_sparsity=target_sparsity
                )
    ...
    def dynamic_batch(
        ...
        *,
        ...
        if validate_shapes and tensors:
            ...
            for i, t in enumerate(tensors[1:], 1):
                if t.shape != first_shape:
                    raise ValueError(
                        f"Tensor {i} has incompatible shape: "
        ...
        for t in tensors:
            ...
            would_exceed_memory = (
                max_memory_mb is not None and
                cur_memory + t_memory > max_memory_mb
            )
    ...
    def load_checkpoint(self, name: str = "checkpoint") -> Tuple[List[Any], ShardMetadata]:
        """
        ...
        if self.dry_run:
            ...
            return [], ShardMetadata(
                axis=0, original_shape=(0,), shard_shapes=[],
                shard_slices=[], dtype='float32',
                compressed=False, compressor='none', num_nodes=0
            )
...
if __name__ == "__main__":
    ...
    print(f"   Reconstruction error: {np.max(np.abs(tensor - reconstructed_comp)):.10f}")
