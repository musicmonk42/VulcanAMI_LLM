...
class HardwareDispatcher:
    ...
    LIGHTMATTER_REST_URL = os.getenv("LIGHTMATTER_REST_URL", "https://api.lightmatter.co")
    ...
    def __init__(self, 
        ...
        if use_mock:
            ...
            self.lightmatter_api_key = lightmatter_api_key or os.getenv("LIGHTMATTER_API_KEY")
        ...
        logger.info(f"HardwareDispatcher initialized with {len(self.hardware_registry)} backends discovered")
    ...
    def _discover_hardware(self):
        ...
        if self.aim_api_key:
            self.hardware_registry[HardwareBackend.AIM_PHOTONICS] = HardwareCapabilities(
        ...
        if TORCH_AVAILABLE and torch is not None and torch.cuda.is_available():
            for i in range(torch.cuda.device_count()):
                ...
                backend = HardwareBackend.NVIDIA_GPU if "NVIDIA" in props.name else HardwareBackend.AMD_GPU
    ...
    def _initialize_grpc_channels(self):
        ...
        try:
            # Lightmatter gRPC
            if self.lightmatter_api_key and HardwareBackend.LIGHTMATTER in self.hardware_registry:
            ...
            # AIM Photonics gRPC
            if self.aim_api_key and HardwareBackend.AIM_PHOTONICS in self.hardware_registry:
                credentials = grpc.ssl_channel_credentials()
        ...
        try:
            ...
            with self.lock:
                if response.status_code == 200:
                    ...
                    capabilities.last_health_check = datetime.now()
                    logger.warning(f"{backend.value} health check failed: {response.status_code}")
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        for backend in HardwareBackend:
            ...
            if backend_metrics:
                backend_stats[backend.value] = {
                    ...
                    "avg_energy_nj": float(np.mean([m.energy_nj for m in backend_metrics])),
                    "avg_latency_ms": float(np.mean([m.latency_ms for m in backend_metrics])),
                    "avg_throughput": float(np.mean([m.throughput_ops_per_sec for m in backend_metrics])),
        ...
        return {
            ...
            "hardware_capabilities": {
                backend.value: asdict(cap) for backend, cap in self.hardware_registry.items()
            }
        }
    ...
    def get_last_metrics(self, key_id: str) -> Dict[str, Any]:
        ...
        with self.metrics_lock:
            ...
            for metric in reversed(self.metrics_history):
                if (key_id == metric.operation or 
                    key_id == metric.backend.value or
                    key_id == f"{metric.backend.value}_{metric.operation}"):
                    ...
                    return {
                        ...
                        "timestamp": metric.end_time.isoformat() if metric.end_time else None
                    }
    ...
    def validate_photonic_params(self, params: Dict[str, Any]) -> Optional[Dict[str, str]]:
        """
        ...
        required_fields = ["noise_std", "multiplexing", "compression", "bandwidth_ghz", "latency_ps"]
        ...
        valid_compressions = ["ITU-F.748-quantized", "ITU-F.748-sparse", "ITU-F.748", "none"]
        ...
        valid_multiplexing = ["microwave-lightwave", "wavelength", "space-time-wavelength"]
    ...
    def _select_backend(self, op: str, input_size: Tuple[int, ...], params: Dict[str, Any]) -> HardwareBackend:
        ...
        for backend, capabilities in registry_copy.items():
            ...
            if op in ["photonic_mvm", "memristor_mvm"] and not capabilities.supports_mvm:
                continue
            ...
            # Calculate score
            base_score = 1.0 / (capabilities.energy_per_op_nj + capabilities.latency_per_op_us + 1e-6)
        ...
        logger.debug(f"Selected backend {selected.value} for operation {op} (score={scores[best_idx]:.4f})")
    ...
    def _dispatch_to_lightmatter_rest(self, op: str, *args, params: Dict[str, Any]) -> Any:
        ...
        if op == "photonic_mvm" and len(args) == 2:
            ...
            request_data = {
                "args": [arg.tolist() if hasattr(arg, 'tolist') else arg for arg in args],
                "params": params
            }
        ...
        try:
            ...
            # Update RL weights
            if self.weights is not None and self.opt and TORCH_AVAILABLE and torch is not None:
                reward = 1.0 / (energy + latency + 1e-6)
                backend_idx = list(HardwareBackend).index(HardwareBackend.LIGHTMATTER)
                loss = -torch.tensor(reward) * torch.log_softmax(self.weights, dim=0)[backend_idx]
                self.opt.zero_grad()
                loss.backward()
                self.opt.step()
    ...
    def _dispatch_to_lightmatter_grpc(self, op: str, *args, params: Dict[str, Any]) -> Any:
    ...
    def _dispatch_to_emulator(self, op: str, *args, params: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if params and "noise_std" in params:
                ...
                try:
                    emulator = HardwareEmulator(noise_std=noise_std, noise_type=noise_type)
                except Exception as e:
                    logger.warning(f"Failed to create custom emulator: {e}, using default")
    ...
    def dispatch(self, op: str, *args, **kwargs: Any) -> Any:
        ...
        for i, arg in enumerate(args):
            if arg is None:
                logger.error(f"Dispatch failed: argument {i} is None for operation {op}")
                return {
                    "error_code": AI_ERRORS.AI_INVALID_REQUEST,
        ...
        input_size = (
            args[0].shape if hasattr(args[0], 'shape')
            else (len(args[0]),) if hasattr(args[0], '__len__')
            else (1,)
        )
        ...
        if isinstance(result, dict) and "error_code" in result:
            logger.warning(f"Primary backend {backend.value} failed: {result['message']}")
            ...
            for fallback in fallback_order:
                ...
                if fallback not in self.hardware_registry or not self.hardware_registry[fallback].available:
                    continue
    ...
    def shutdown(self):
        ...
        if self.enable_metrics:
            try:
                ...
                logger.info(f"Final metrics summary: {json.dumps(metrics_summary, indent=2)}")
            except Exception as e:
...
if __name__ == "__main__":
    ...
    if NUMPY_AVAILABLE and np is not None:
        ...
        # Test photonic MVM
        result = dispatcher.dispatch("photonic_mvm", mat, vec, params={
            "noise_std": 0.01,
            "multiplexing": "wavelength",
            "compression": "ITU-F.748-quantized",
            "bandwidth_ghz": 100,
        ...
        print(f"   Photonic MVM result shape: {result.shape if isinstance(result, np.ndarray) else 'error'}")
