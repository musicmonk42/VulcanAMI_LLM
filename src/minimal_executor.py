...
try:
    ...
    logging.warning("ObservabilityManager not found. Monitoring hooks will be disabled.")
...
class GraphValidator:
    ...
    def validate_graph(graph: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        for i, edge in enumerate(edges):
            ...
            if from_node not in node_ids:
                return False, f"Edge {i} references non-existent 'from' node: {from_node}"
    ...
    @staticmethod
    def detect_cycles(nodes: Dict[str, Any], edges: List[Dict[str, Any]]) -> Tuple[bool, Optional[List[str]]]:
        """
...
class MinimalExecutor:
    ...
    def _log_node_execution(self, node_id: str, node_type: str, 
    ...
    async def _execute_input_node(self, node: Dict, context: ThreadSafeContext) -> Any:
        ...
        try:
            ...
            self.audit_logger.log("input_node_executed", {
                "node_id": node_id,
            ...
            duration_ms = (time.time() - start_time) * 1000
            self._log_node_execution(node_id, "InputNode", "failed", duration_ms, str(e))
            raise
    ...
    async def _execute_output_node(self, node: Dict, context: ThreadSafeContext) -> Any:
        ...
        try:
            ...
            self.audit_logger.log("output_node_executed", {
                "node_id": node_id,
                "data": str(output_data)[:1000]
            })
            ...
            duration_ms = (time.time() - start_time) * 1000
            self._log_node_execution(node_id, "OutputNode", "failed", duration_ms, str(e))
            raise
    
    async def _execute_generative_node(self, node: Dict, context: ThreadSafeContext) -> Any:
        ...
        try:
            ...
            self.audit_logger.log("generative_node_executed", {
                "node_id": node_id,
            ...
            duration_ms = (time.time() - start_time) * 1000
            self._log_node_execution(node_id, "GenerativeNode", "failed", duration_ms, str(e))
            raise
    
    async def _execute_combine_node(self, node: Dict, context: ThreadSafeContext) -> Any:
        ...
        try:
            ...
            for ref in input_refs:
                if not isinstance(ref, str):
                    raise ValidationError(f"CombineNode {node_id} input ref must be string")
            ...
            self.audit_logger.log("combine_node_executed", {
                "node_id": node_id,
                "data": combined_data[:1000]
            })
            
            self.logger.info(f"CombineNode {node_id} executed with {len(input_refs)} inputs")
            ...
            duration_ms = (time.time() - start_time) * 1000
            self._log_node_execution(node_id, "CombineNode", "failed", duration_ms, str(e))
            raise
    
    async def _execute_transform_node(self, node: Dict, context: ThreadSafeContext) -> Any:
        ...
        try:
            ...
            if not input_ref:
                raise ValidationError(f"TransformNode {node_id} missing input reference")
            ...
            duration_ms = (time.time() - start_time) * 1000
            self._log_node_execution(node_id, "TransformNode", "failed", duration_ms, str(e))
            raise
    ...
    async def _execute_filter_node(self, node: Dict, context: ThreadSafeContext) -> Any:
        ...
        try:
            ...
            duration_ms = (time.time() - start_time) * 1000
            self._log_node_execution(node_id, "FilterNode", "failed", duration_ms, str(e))
            raise
                ...
                while len(executed_nodes) + len(failed_nodes) < len(nodes):
                    ...
                    for node_id in current_layer_nodes:
                        ...
                        if not executor:
                            ...
                            self.audit_logger.log("execution_failed", {
                                "node_id": node_id,
                                "error": error_msg
                            })
                    ...
                    try:
                        ...
                        self.logger.error(f"Layer execution timed out for nodes: {task_nodes}")
                    ...
                    for node_id in executed_nodes:
                        if node_id in current_layer_nodes:
                            for dependent_id in dependents.get(node_id, []):
                                if dependent_id not in executed_nodes and dependent_id not in failed_nodes:
    ...
    async def test_execution():
        ...
        test_graph = {
            ...
            "nodes": [
                ...
                {"id": "gen1", "type": "GenerativeNode", "prompt": "Summarize", "in": "in1"},
                {"id": "gen2", "type": "GenerativeNode", "prompt": "Expand", "in": "in2"},
