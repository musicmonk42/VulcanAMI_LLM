...
class AgentProfile:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AgentProfile":
        ...
        return cls(
            ...
            certificates=[
                AgentCertificate.from_dict(c) for c in data.get("certificates", [])
            ],
...
class KeyManager:
    ...
    def decrypt_key(
        self, encrypted_data: bytes, password: str, salt_iv: bytes
    ) -> bytes:
...
class CertificateAuthority:
    ...
    def __init__(
        self, ca_key_path: Optional[str] = None, ca_cert_path: Optional[str] = None
    ):
        ...
        if (
            ca_key_path
            and ca_cert_path
            and Path(ca_key_path).exists()
            and Path(ca_cert_path).exists()
        ):
    ...
    def issue_certificate(
        self, agent_id: str, public_key_pem: bytes, valid_days: int = 365
    ) -> AgentCertificate:
...
class AgentRegistry:
    ...
    def register_agent(
        ...
        with self.agents_lock:
            ...
            if self.vulcan_bridge:
                ...
                try:
                    validation = self.vulcan_bridge.world_model.evaluate_agent_proposal(
                        proposal
                    )
                    if not validation.get("valid", True):
                        logger.warning(
                            f"VULCAN flagged agent registration: {validation.get('reasoning')}"
                        )
                except Exception as e:
    ...
    def verify_signature(
        ...
        with self.agents_lock:
            ...
            if not agent:
                ...
                self.logger.warning(
                    f"Verification failed: Agent '{agent_id}' not found"
                )
            ...
            if isinstance(signature, str):
                ...
                try:
                    ...
                    if agent.failed_attempts >= MAX_FAILED_ATTEMPTS:
                        agent.locked_until = datetime.utcnow() + timedelta(
                        ...
                        self.audit_logger.log_event(
                            ...
                            agent_id,
                            {
                                ...
                                "locked_until": agent.locked_until.isoformat(),
                            },
                            success=False,
                            ip_address=ip_address,
                        )
            ...
            is_valid = self.key_manager.verify_signature(
                message, signature, key.public_key, key.algorithm
            )
    ...
    def revoke_certificate(
        self, agent_id: str, cert_id: str, reason: str = "unspecified"
    ):
        ...
        with self.agents_lock:
            ...
            if not cert_found:
                raise ValueError(
                    f"Certificate '{cert_id}' not found for agent '{agent_id}'"
                )
    ...
    def _get_default_permissions(self, roles: List[AgentRole]) -> Dict[str, bool]:
        ...
        for role in roles:
            if role == AgentRole.ADMIN:
                ...
                permissions.update(
                    {"execute_graph": True, "submit_graph": True, "view_status": True}
                )
            elif role == AgentRole.VIEWER:
                permissions.update({"view_status": True, "view_graph": True})
                ...
                permissions.update(
                    {"view_audit": True, "view_status": True, "view_graph": True}
                )
            elif role == AgentRole.SERVICE:
                permissions.update({"execute_graph": True, "submit_graph": True})
                    ...
                    with self.agents_lock:
                        ...
                        for agent in self.agents.values():
                            ...
                            for key in expired_keys:
                                ...
                                self.logger.info(
                                    f"Expired key {key.key_id} for agent {agent.agent_id}"
                                )
    ...
    def list_agents(
        self, role: Optional[AgentRole] = None, active_only: bool = True
    ) -> List[Dict[str, Any]]:
    ...
    def export_ca_certificate(self) -> str:
        ...
        return self.cert_authority.ca_cert.public_bytes(
    ...
    def import_agent_certificate(self, agent_id: str, cert_pem: bytes) -> bool:
        ...
        with self.agents_lock:
            ...
            if not self.cert_authority.verify_certificate(cert_pem):
                self.logger.warning(
                    f"Certificate verification failed for agent '{agent_id}'"
                )
                return False
