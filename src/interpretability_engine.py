...
class InterpretabilityEngine(metaclass=_SingletonMeta):
    ...
    def __init__(
        ...
        if model is not None and CAPTUM_AVAILABLE and TORCH_AVAILABLE:
            try:
                ...
                self.logger.warning(f"Model validation failed: {e}. Explanations may not work properly.")
        
        # Log availability of optional dependencies
        if not CAPTUM_AVAILABLE:
            self.logger.warning("Captum not available. Using fallback attribution methods.")
    ...
    def explain_tensor(
        ...
        if method not in ("integrated_gradients", "saliency"):
            raise ValueError(f"method must be 'integrated_gradients' or 'saliency', got {method}")
        ...
        try:
            ...
            if np.sum(np.abs(shap_scores)) > 0:
                shap_scores = (np.abs(shap_scores) / np.sum(np.abs(shap_scores))).tolist()
            else:
    ...
    def visualize_attention(
        ...
        try:
            ...
            if not isinstance(attn_weights, np.ndarray):
                self.logger.error(f"attn_weights must be numpy array, got {type(attn_weights)}")
                return
            
            if attn_weights.ndim != 2 or attn_weights.shape[0] != attn_weights.shape[1]:
                self.logger.error(f"attn_weights must be square matrix, got shape {attn_weights.shape}")
                return
            ...
            for sign in [+1, -1]:
                ...
                if (model is not None or self.model is not None) and TORCH_AVAILABLE:
                    ...
                    if current_model is not None:
                        model_input = torch.tensor(perturbed, dtype=torch.float32).unsqueeze(0)
                        model_output = current_model(model_input).detach().cpu().numpy().flatten()
                        outputs.append(model_output)
                    else:
                        outputs.append(perturbed) # Should not happen if self.model is None but TORCH_AVAILABLE
                else:
            ...
            if (model is not None or self.model is not None) and TORCH_AVAILABLE:
                ...
                if current_model is not None:
                    if baseline_output is None:
                        baseline_input = torch.tensor(base, dtype=torch.float32).unsqueeze(0)
                        baseline_output = current_model(baseline_input).detach().cpu().numpy().flatten()
                    
                    output_diff = float(np.max([
    ...
    def adversarial_explain(
        ...
        try:
            ...
            if not scores_orig or not scores_adv:
                ...
                diff = float(np.linalg.norm(np.array(scores_adv) - np.array(scores_orig)))
    ...
    def trace_relations(
        ...
        if embedding_func is not None and not callable(embedding_func):
            raise TypeError(f"embedding_func must be callable, got {type(embedding_func)}")
        ...
        if embedding_func is None:
            ...
            try:
                ...
                if not isinstance(emb, np.ndarray):
                    self.logger.warning("Embedding function did not return numpy array, using flatten")
                    emb = tensor.flatten()
            except Exception as e:
            ...
            if not isinstance(nodes, list):
                ...
                for node in nodes:
                    ...
                    try:
                        ...
                        self.logger.debug(f"Failed to process node {node.get('id')}: {e}")
                        continue
...
if __name__ == "__main__":
    ...
    print(f"   Scores shape: {len(result['shap_scores']) if result['shap_scores'] else 0}")
    ...
    relations = engine.trace_relations(tensor, test_graph, threshold=0.5, save_json=False)
