...
class HardwareEmulator:
    ...
    def __init__(
        ...
        if noise_std < MIN_NOISE_STD or noise_std > MAX_NOISE_STD:
            raise ValueError(f"noise_std must be in [{MIN_NOISE_STD}, {MAX_NOISE_STD}], got {noise_std}")
    ...
    def matmul(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
        ...
        if a.ndim < 2 or b.ndim < 2:
            raise ValueError(f"Both arrays must be at least 2D, got shapes {a.shape} and {b.shape}")
        
        if a.shape[-1] != b.shape[-2]:
            raise ValueError(
                f"Shape mismatch: cannot multiply {a.shape} and {b.shape}"
            )
    ...
    def batch_emulate(
        ...
        **kwargs
        ...
        if tensor_count > MAX_BATCH_SIZE:
            raise ValueError(f"Batch size {tensor_count} exceeds maximum {MAX_BATCH_SIZE}")
        ...
        if op_type == "mvm":
            ...
            if (tensors_arr.ndim == 3 and vectors_arr.ndim == 2 and 
                tensors_arr.shape[0] == vectors_arr.shape[0]):
                ...
                if self.debug:
                    logger.info(f"Batch MVM with einsum: {tensors_arr.shape} @ {vectors_arr.shape}")
