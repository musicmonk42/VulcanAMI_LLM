...
def generate_stress_test_graphs(
    ...
    if not isinstance(min_density, (int, float)) or not isinstance(max_density, (int, float)):
        raise TypeError("min_density and max_density must be numeric")
    
    if min_density < MIN_DENSITY or max_density > MAX_DENSITY or min_density > max_density:
        raise ValueError(
...
def generate_specific_topology(
    ...
    if not isinstance(topology_type, str):
        raise TypeError(f"topology_type must be string, got {type(topology_type).__name__}")
    ...
    if not isinstance(max_mesh_nodes, int):
        raise TypeError(f"max_mesh_nodes must be int, got {type(max_mesh_nodes).__name__}")
    ...
    try:
        ...
        if topology_type == 'star':
            ...
            deviation = abs(actual_nodes - num_nodes) / num_nodes if num_nodes > 0 else 0
            if deviation > 0.3:
                logger.warning(
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i, g in enumerate(stress_graphs):
            print(f"   Graph {i}: {g['metadata']['num_nodes']} nodes, "
    ...
    for topo in ['star', 'ring', 'mesh', 'tree', 'random']:
        try:
            ...
            print(f"   {topo.capitalize()}: {graph['metadata']['actual_nodes']} nodes, "
    ...
    for target_nodes in [10, 50, 100, 500, 1000]:
        try:
            ...
            print(f"   Target: {target_nodes}, Actual: {actual}, "
    ...
    try:
        ...
        print(f"   Requested: 150, Max allowed: 75, "
