...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("validation_test_logs.log"),
...
class ValidationTestSuite:
    ...
    def __init__(self, agent_id: str = "validation_agent", enable_caching: bool = True):
        ...
        try:
            ...
            logger.info("ValidationTestSuite initialized with agent_id=%s, caching=%s", agent_id, enable_caching)
    ...
    def _validate_schema(self, graph: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for node in nodes:
                ...
                schema = self.manifest.get("node_types", {}).get(node_type, {}).get("json_schema")
                if not schema:
            ...
            for edge in edges:
                ...
                schema = self.manifest.get("edge_types", {}).get(edge_type, {}).get("json_schema")
                if not schema:
                    logger.error(f"No schema for edge type: {edge_type}")
                    return False
                validate(instance=edge, schema=schema)
            
            logger.info(f"Schema validation passed for graph with grammar_version {grammar_version}")
            return True
    ...
    def _validate_signature(self, graph: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"Signature verification {'passed' if verified else 'failed'} for graph {graph.get('id')}")
            return verified
        except Exception as e:
    ...
    async def _validate_photonic_params(self, graph: Dict[str, Any]) -> bool:
        ...
        if not self.hardware:
            logger.warning("HardwareDispatcher not available; skipping photonic validation")
            return True
            ...
            for node in graph.get("nodes", []):
                if node.get("type") == "PhotonicMVMNode":
                    ...
                    if result.get("error_code"):
                        logger.error(f"Photonic params invalid: {result.get('message')}")
                        return False
            logger.info(f"Photonic params validation passed for graph {graph.get('id')}")
            return True
        except Exception as e:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if result.get("status") == "completed":
                    logger.info(f"Execution successful for graph {graph.get('id')} on attempt {attempt + 1}")
                    return {
                        "status": "completed",
            ...
            if attempt < max_retries - 1:
                ...
                if self.obs and "validation_retries" in self.obs.metrics:
                    self.obs.metrics["validation_retries"].labels(test_type="execution").inc()
    ...
    async def validate_all_aspects(self, graph: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async_results = await asyncio.gather(*[task[1] for task in async_tasks], return_exceptions=True)
    ...
    def _log_metrics(self, test_name: str, result: Dict[str, Any]):
        ...
        try:
            ...
            if "validation_pass" in self.obs.metrics and result.get("status") == "completed":
            ...
            if "validation_latency" in self.obs.metrics and "duration_ms" in result:
                self.obs.metrics["validation_latency"].labels(test_type=test_name).observe(result["duration_ms"] / 1000)
    ...
    def _log_audit(self, test_name: str, details: Dict[str, Any]):
        ...
        try:
            self.audit.log_event(f"validation_test_{test_name}", {
...
async def test_graph_validation(golden_file: str):
    ...
    with ValidationTestSuite() as suite:
        ...
        suite._log_metrics(f"schema_{test_name}", {"status": "completed", "duration_ms": schema_duration})
        suite._log_audit(f"schema_{test_name}", {"file": golden_file, "valid": schema_result})

        # Signature verification
        sig_result = suite._validate_signature(graph)
        assert sig_result, f"Signature verification failed for {golden_file}"
        suite._log_metrics(f"signature_{test_name}", {"status": "completed", "duration_ms": 0})
        suite._log_audit(f"signature_{test_name}", {"file": golden_file, "valid": sig_result})

        # Ethical validation
        ethics_result = suite._validate_ethics(graph)
        assert ethics_result["status"] != "error", f"Ethics validation failed: {ethics_result.get('error')}"
        assert ethics_result["ethical"] == "safe", f"Graph {golden_file} not ethically safe: {ethics_result['ethical']}"
        ...
        assert exec_result["status"] == "completed", f"Execution failed: {exec_result.get('error')}"
        ...
        suite._log_metrics(f"photonic_{test_name}", {"status": "completed", "duration_ms": 0})
        suite._log_audit(f"photonic_{test_name}", {"file": golden_file, "valid": photonic_result})
...
async def test_parallel_validation():
    ...
    with ValidationTestSuite() as suite:
        if GOLDEN_FILES:
            ...
            expected_aspects = {'schema', 'signature', 'ethics', 'execution', 'photonic'}
            assert set(results.keys()) >= expected_aspects, f"Missing validation aspects: {expected_aspects - set(results.keys())}"
...
async def test_stress_validation():
    ...
    with ValidationTestSuite() as suite:
        ...
        exec_result = await suite._validate_execution(graph)
        assert exec_result["status"] == "completed", f"Stress test failed: {exec_result.get('error')}"
...
async def test_adversarial_validation():
    ...
    with ValidationTestSuite() as suite:
        ...
        assert ethics_result["ethical"] == "risky", "Adversarial graph not flagged as risky"
...
async def test_tournament_validation():
    ...
    with ValidationTestSuite() as suite:
        ...
        proposals = [
            {
                "id": f"p{i}", 
                "nodes": [{"id": "n1", "type": "InputNode", "params": {"value": f"data{i}"}}], 
                "edges": []
            } 
            for i in range(5)
        ]
        fitness = np.random.rand(5).tolist()
        winners = tm.run_adaptive_tournament(proposals, fitness, lambda x: np.random.rand(16))
        
        for idx in winners:
            graph = proposals[idx]
            assert suite._validate_schema(graph), f"Tournament winner {graph['id']} invalid schema"
...
def test_manifest_completeness():
    ...
    with ValidationTestSuite() as suite:
        ...
        assert used_node_types.issubset(manifest_nodes), f"Missing node types: {used_node_types - manifest_nodes}"
        assert used_edge_types.issubset(manifest_edges), f"Missing edge types: {used_edge_types - manifest_edges}"
        suite._log_metrics("manifest_completeness", {"status": "completed", "duration_ms": 0})
        suite._log_audit("manifest_completeness", {"node_types": list(used_node_types), "edge_types": list(used_edge_types)})
...
def test_caching_functionality():
    ...
    if second_cache_time > 0 and first_cache_time > 0:
        assert second_cache_time <= first_cache_time, "Cache did not improve performance"
    else:
        assert graph3 == graph2, "Cached content doesn't match"

    logger.info(f"Cache performance: no_cache={no_cache_time:.6f}s, first={first_cache_time:.6f}s, cached={second_cache_time:.6f}s")
...
async def demo_validation(golden_files: List[str] = None):
    ...
    with ValidationTestSuite() as suite:
        ...
        total_passed = sum(1 for results in all_results.values() 
                          if all(r.get("status") == "completed" if isinstance(r, dict) else r 
                                for r in results.values()))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Run interactive demo instead of pytest"
    )
    parser.add_argument(
        "--parallel",
        action="store_true",
