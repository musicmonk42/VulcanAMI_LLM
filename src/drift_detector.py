...
drift_events = Counter('drift_detector_realignments_total', 'Total drift-triggered realignments')
drift_checks = Counter('drift_detector_checks_total', 'Total drift checks performed')
drift_latency = Histogram('drift_detector_check_latency_seconds', 'Latency of drift detection checks')
realignment_operations = Counter('drift_detector_realignment_operations', 'Realignment operations performed')
validation_errors = Counter('drift_detector_validation_errors', 'Validation errors encountered')
...
class DriftDetector:
    ...
    def __init__(
        ...
        if not isinstance(drift_threshold, (int, float)):
            raise ValueError(f"drift_threshold must be numeric, got {type(drift_threshold)}")
    ...
    def _validate_embeddings(self, embeddings: np.ndarray) -> Tuple[bool, Optional[str]]:
        """
    ...
    def _realign_pca(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        ref_centered = self.reference_embeddings - np.mean(self.reference_embeddings, axis=0)
    ...
    def realign_if_drift(
        ...
        with self.lock:
            ...
            result = {
                ...
                "last_realignment": (
                    self.last_realignment.isoformat()
                    if self.last_realignment else None
                )
            }
