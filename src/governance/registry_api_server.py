...
try:
    ...
    HAS_GRPC = False
    logging.warning("gRPC not available. Install 'grpcio' and 'protobuf' for full gRPC server functionality.")
...
class Node:
    def __init__(self, **kwargs):
        ...
        self.validation_type = kwargs.get('validation_type', '') # Used in ValidationNode
        ...
        self.proposal_id = kwargs.get('proposal_id', self.id or self.target or '') # Try id or target if proposal_id missing

class AuditLogEntry:
    def __init__(self, **kwargs):
        # Use a default Timestamp or a string representation if no gRPC
        self.timestamp = kwargs.get('timestamp', Timestamp() if HAS_GRPC else datetime.utcnow().isoformat()+'Z')
...
class DeployGrammarVersionRequest:
    def __init__(self, agent_id='', signature='', proposal_id='', new_grammar_version=''):
        self.agent_id = agent_id
        self.signature = signature
        self.proposal_id = proposal_id
...
class QueryProposalsRequest:
    def __init__(self, agent_id='', status=None, proposed_by=None, limit=None, offset=0):
        self.agent_id = agent_id
    ...
    @property
    def proposed_by(self): return self._proposed_by
    @property
    def limit(self): return self._limit

    def HasField(self, field_name):
        # Check if the internal attribute holding the optional value is not None
        if field_name == "status": return self._status is not None
        if field_name == "proposed_by": return self._proposed_by is not None
        if field_name == "limit": return self._limit is not None
...
class RegistryServiceServicer(ABC):
    ...
    @abstractmethod
    def RegisterGraphProposal(self, request, context): pass
    @abstractmethod
    def SubmitLanguageEvolutionProposal(self, request, context): pass
    @abstractmethod
    def RecordVote(self, request, context): pass
    @abstractmethod
    def RecordValidation(self, request, context): pass
    @abstractmethod
    def DeployGrammarVersion(self, request, context): pass
    @abstractmethod
    def QueryProposals(self, request, context): pass
    @abstractmethod
    def GetFullAuditLog(self, request, context): pass
    @abstractmethod
    def VerifyAuditLogIntegrity(self, request, context): pass
...
class DatabaseConnectionPool:
    ...
    def __init__(self, db_path: str, pool_size: int = DB_POOL_SIZE, timeout: float = 5.0):
        ...
        if not self._connections and pool_size > 0:
            logging.error(f"Failed to initialize any database connections for path: {db_path}")
    ...
    def _create_connection(self):
        ...
        conn = sqlite3.connect(self._db_path, check_same_thread=False, timeout=max(10.0, self._timeout + 5.0))
        ...
        with self._condition:
            while not acquired: # Loop until connection acquired or timeout
                if self._connections:
                    ...
                    if remaining_time <= 0:
                        raise RuntimeError(f"Connection pool exhausted - timeout ({self._timeout}s) waiting for available connection")
...
class DatabaseManager:
    ...
    def __init__(self, db_path: str = DB_PATH):
        ...
        try:
            ...
            self.logger.exception(f"FATAL: DatabaseManager failed to initialize pool or DB at {db_path}: {e}")
    ...
    def _init_database(self):
        ...
        try:
            with self.pool.get_connection() as conn:
                ...
                conn.commit()
                self.logger.info("Database schema initialized (tables created if not exists).")
        except sqlite3.Error as e:
            self.logger.error(f"Database schema initialization failed: {e}")
        except RuntimeError as e: # Catch pool exhaustion during init
             self.logger.error(f"Database schema initialization failed - connection pool issue: {e}")
            ...
            try:
                ...
                if "database is locked" in str(e):
                    if attempt < max_retries - 1:
                        ...
                        self.logger.warning(f"Database locked, retrying query in {delay:.2f}s (attempt {attempt+1}/{max_retries}). Query: {query[:100]}...")
                        time.sleep(delay)
                    else:
                        self.logger.error(f"Database locked after {max_retries} attempts. Query: {query[:100]}")
                        raise # Reraise if retries exhausted
                else:
                    self.logger.error(f"Database operational error: {e}. Query: {query[:100]}")
                    raise # Reraise other operational errors immediately
            except Exception as e:
                self.logger.exception(f"Unexpected database error on query: {query[:100]}. Error: {e}")
    ...
    def get_record(self, table_name: str, record_id: str) -> Optional[Dict]:
        ...
        try:
            row = self._exec_query(f"SELECT {data_column} FROM {table_name} WHERE {id_column} = ?", (record_id,), fetch_one=True)
            if row and len(row) > 0 and row[0] is not None:
                try:
                    return json.loads(row[0])
                except json.JSONDecodeError as e:
                    self.logger.error(f"Failed to decode JSON data for {table_name} ID {record_id}: {e}")
                    return None # Return None if data is corrupt
            return None # Record not found
        except Exception as e:
             self.logger.error(f"Error getting record {record_id} from {table_name}: {e}")
    ...
    def save_record(self, table_name: str, record_id: str, data: Dict):
        ...
        data_column = "profile_data" if table_name == "agents" else "data"
        try:
            self._exec_query(f"INSERT OR REPLACE INTO {table_name} ({id_column}, {data_column}) VALUES (?, ?)", (record_id, json.dumps(data)))
        except Exception as e:
             self.logger.error(f"Error saving record {record_id} to {table_name}: {e}")
             # Optionally re-raise depending on desired error handling

    def query_records(self, table_name: str, where_clause: str = "", params=(), limit: Optional[int] = None, offset: int = 0) -> List[Dict]:
        """Query records with optional filters, limit, and offset, parsing JSON data."""
        data_column = "profile_data" if table_name == "agents" else "data"
        id_column = "agent_id" if table_name == "agents" else "id" # Needed for ORDER BY
                ...
                for row in rows:
                    if row and len(row) > 0 and row[0] is not None:
                        try:
                            ...
                            self.logger.error(f"Failed to decode record data from {table_name} during query: {e}")
    ...
    def log_audit(self, data: Dict):
        ...
        try:
            self._exec_query("INSERT INTO audit_log (timestamp, data) VALUES (?, ?)", (ts, json.dumps(data)))
        except Exception as e:
...
class RegistryAPI:
    ...
    def submit_proposal(self, proposal_node: Dict) -> str:
        prop_id = proposal_node.get("id", f"graph_prop_{hashlib.sha256(json.dumps(proposal_node, sort_keys=True).encode()).hexdigest()[:8]}")
        # Ensure 'id' is in the node itself before saving
        if "id" not in proposal_node: proposal_node["id"] = prop_id
        proposal_data = {"node": proposal_node, "status": "submitted", "timestamp": datetime.utcnow().isoformat() + 'Z'}
    ...
    def query_proposals(self, status: Optional[str] = None, proposed_by: Optional[str] = None, limit: Optional[int] = None, offset: int = 0) -> List[Dict]:
        all_proposals_data = self.db.query_records("graph_proposals") # Fetches full data objects
        results = []
        # Filter in Python
        for pdata in all_proposals_data:
            ...
            match_proposer = (proposed_by is None or node_data.get("proposed_by") == proposed_by)
...
class LanguageEvolutionRegistry:
    ...
    def submit_proposal(self, proposal_node: Dict) -> str:
        prop_id = proposal_node.get("id", f"lang_prop_{hashlib.sha256(json.dumps(proposal_node, sort_keys=True).encode()).hexdigest()[:8]}")
        # Ensure 'id' is in the node itself
        if "id" not in proposal_node: proposal_node["id"] = prop_id
        proposal_data = {"node": proposal_node, "status": "pending", "timestamp": datetime.utcnow().isoformat() + 'Z', "votes": {}, "validations": {}}
    ...
    def record_vote(self, consensus_node: Dict) -> bool:
        ...
        with self._proposal_lock:
            ...
            if "votes" not in proposal or not isinstance(proposal["votes"], dict): proposal["votes"] = {}
            ...
            rejected = no_votes >= yes_votes and num_votes > 0 # Reject on tie or majority no

            original_status = proposal.get("status") # Keep track if status changes
            status_changed = False

            if approved and proposal.get("status") != "approved":
                proposal["status"] = "approved"; self.logger.info(f"Proposal {prop_id} approved by vote.")
                status_changed = True
            elif rejected and proposal.get("status") != "rejected":
                proposal["status"] = "rejected"; self.logger.info(f"Proposal {prop_id} rejected by vote.")
                status_changed = True
            ...
            if consensus_node.get("votes", {}) or status_changed:
                ...
                self.logger.debug(f"No change in votes or status for {prop_id}, skipping save.")
    ...
    def record_validation(self, validation_node: Dict) -> bool:
        ...
        if "validations" not in proposal or not isinstance(proposal["validations"], dict): proposal["validations"] = {}
        validator_key = validation_node.get("validator_id", validation_node.get("validation_type", str(time.time())))
    ...
    def deploy_grammar_version(self, proposal_id: str, new_version: str) -> bool:
        ...
        if not proposal or proposal.get("status") != "validated":
            self.logger.error(f"Cannot deploy: Proposal {proposal_id} not in 'validated' state. Status is {proposal.get('status') if proposal else 'not found'}.")
            return False
        ...
        self.logger.info(f"Grammar version {new_version} deployed for proposal {proposal_id}.")
        return True
...
class AgentRegistry:
    ...
    def register_agent(self, agent_data: Dict[str, Any]):
        ...
        try:
            profile = { # Build the profile dict to be stored
                ...
                "is_active": agent_data.get("is_active", True), # Assume active by default
                "updated_at": datetime.utcnow().isoformat() + 'Z',
            }
            existing_profile = self.get_agent_info(agent_id) # Check if updating
            profile["created_at"] = existing_profile.get("created_at") if existing_profile else profile["updated_at"]
    ...
    def authenticate_agent(self, agent_id: str, message: str, signature_hex: str) -> bool:
        ...
        if is_valid: self.logger.debug(f"Agent {agent_id} authenticated successfully.")
        else: self.logger.warning(f"Authentication failed for {agent_id}: Invalid signature.")
        return is_valid

    def verify_agent_signature(self, agent_id: str, message: bytes, signature_hex: str) -> bool:
    ...
    def query_agents(self, role: Optional[str] = None, min_trust_level: float = 0.0, status: Optional[str] = "active", limit: Optional[int] = None) -> List[Dict]:
...
class SecurityAuditEngine:
    ...
    def log_audit(self, action: str, details: Dict, entity_id: Optional[str] = None, entity_type: Optional[str] = None):
        ...
        try:
            ...
            self.logger.info(f"AUDIT: {action} logged for {entity_type or 'N/A'}:{entity_id or 'N/A'}")
        except Exception as e:
    ...
    def enforce_policies(self, node: Dict) -> bool:
        ...
        for key in ['content', 'proposal_content', 'rationale', 'code']:
            ...
            if isinstance(content, (str, dict, list)): content_to_scan += json.dumps(content) + " "
            elif content is not None: content_to_scan += str(content) + " "
        content_lower = content_to_scan.lower()
        malicious_patterns = ['malicious', 'exploit', 'hack', "os.system('rm -rf /')", '<script>', 'onerror=']
        if any(pattern in content_lower for pattern in malicious_patterns):
            self.log_audit("policy_violation", {"reason": "malicious_content_detected"}, node.get("id"), node.get("type"))
            self.logger.warning(f"Security policy violation: Malicious pattern in node {node.get('id')}")
            return False
        return True
    ...
    def validate_trust_policy(self, node: Dict, trust_level: float) -> bool:
        ...
        if trust_level < required_trust:
            self.log_audit("trust_violation", {"reason": "insufficient_trust"}, node.get("proposed_by"), "agent")
            self.logger.warning(f"Trust policy violation: Agent {node.get('proposed_by')} trust {trust_level} < {required_trust}")
            return False
        return True
...
class RegistryServicer(RegistryServiceServicer):
    ...
    def _authenticate_request(self, agent_id: str, message_content: str, signature: str) -> bool:
        """Authenticate a request using AgentRegistry."""
        return self.agent_registry.authenticate_agent(agent_id, message_content, signature)
    ...
    def _authorize_request(self, agent_id: str, required_roles: List[str]) -> bool:
        ...
        if not has_required_role:
            self.logger.warning(f"Authorization failed: Agent {agent_id} lacks required roles {required_roles}. Has: {list(agent_roles)}")
            return False
        self.logger.debug(f"Authorization successful for {agent_id} (required: {required_roles})")
        return True
        ...
        if HAS_GRPC and hasattr(node_metadata, 'DESCRIPTOR'):
            try:
                ...
                return MessageToDict(node_metadata)
                 ...
                 self.logger.warning(f"Could not convert proto metadata map to dict: {e}")
                 return {} # Fallback to empty dict
        elif isinstance(node_metadata, dict):
            return node_metadata # It's already a Python dict
        else:
            self.logger.debug(f"Metadata field was not a dict or proto map, received {type(node_metadata)}. Defaulting to empty dict.")
    ...
    def RegisterGraphProposal(self, request, context):
        ...
        self.logger.info(f"Received RegisterGraphProposal request from agent: {agent_id}")

        try:
            # Safely parse proposal_content (assuming JSON bytes)
            proposal_content_dict = {}
            if request.proposal_node.proposal_content:
                proposal_content_dict = json.loads(request.proposal_node.proposal_content.decode('utf-8'))
            ...
            return RegisterGraphProposalResponse(status="error", message=f"Invalid proposal content JSON: {e}")
        except Exception as e: # Catch other unexpected errors during prep
            self.logger.error(f"Error processing proposal data from {agent_id}: {e}", exc_info=True) # Log stack trace
            context.set_code(StatusCode.INTERNAL)
            context.set_details(f"Error processing proposal data: {e}")
            return RegisterGraphProposalResponse(status="error", message=f"Error processing proposal data: {e}")

        # --- Authentication & Authorization ---
        if not self._authenticate_request(agent_id, message_for_auth, signature):
            context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Authentication failed.")
            return RegisterGraphProposalResponse(status="error", message="Authentication failed.")
        if not self._authorize_request(agent_id, ["proposer", "governor"]):
            context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Authorization failed: Missing role.")
            return RegisterGraphProposalResponse(status="error", message="Authorization failed.")

        # --- Security & Trust Policies ---
        try:
            if not self.security_audit_engine.enforce_policies(proposal_node_dict):
                context.set_code(StatusCode.FAILED_PRECONDITION); context.set_details("Violates security policies.")
                return RegisterGraphProposalResponse(status="error", message="Security policy violation.")

            agent_info = self.agent_registry.get_agent_info(agent_id)
            trust_level = agent_info.get("trust_level", 0.0) if agent_info else 0.0
            if not self.security_audit_engine.validate_trust_policy(proposal_node_dict, trust_level):
                 context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Agent trust level too low.")
                 return RegisterGraphProposalResponse(status="error", message="Agent trust level too low.")

            # --- Submit Proposal ---
            proposal_id = self.registry_api.submit_proposal(proposal_node_dict)
            self.security_audit_engine.log_audit("graph_proposal_registered", {"id": proposal_id}, agent_id, "agent")
            self.logger.info(f"Graph proposal {proposal_id} registered by {agent_id}.")
            return RegisterGraphProposalResponse(status="success", proposal_id=proposal_id)

        except Exception as e:
            self.logger.exception(f"Internal error in RegisterGraphProposal for {agent_id}: {e}")
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Internal server error: {e}")
            return RegisterGraphProposalResponse(status="error", message=f"Internal server error: {e}")
    ...
    def SubmitLanguageEvolutionProposal(self, request, context):
        ...
        agent_id = request.agent_id
        signature = request.signature
        self.logger.info(f"Received SubmitLanguageEvolutionProposal request from agent: {agent_id}")
        ...
        try:
            ...
            self.logger.error(f"Invalid proposal content JSON from {agent_id}: {e}")
            context.set_code(StatusCode.FAILED_PRECONDITION); context.set_details(f"Invalid proposal content JSON: {e}")
            return SubmitLanguageEvolutionProposalResponse(status="error", message=f"Invalid proposal content JSON: {e}")
        except Exception as e:
            self.logger.error(f"Error processing proposal data from {agent_id}: {e}", exc_info=True) # Log stack trace
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Error processing proposal data: {e}")
            return SubmitLanguageEvolutionProposalResponse(status="error", message=f"Error processing proposal data: {e}")

        # --- Auth & Security ---
        if not self._authenticate_request(agent_id, message_for_auth, signature):
            context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Authentication failed.")
            return SubmitLanguageEvolutionProposalResponse(status="error", message="Authentication failed.")
        if not self._authorize_request(agent_id, ["proposer", "governor"]):
            context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Authorization failed.")
            return SubmitLanguageEvolutionProposalResponse(status="error", message="Authorization failed.")
        try:
            if not self.security_audit_engine.enforce_policies(proposal_node_dict):
                context.set_code(StatusCode.FAILED_PRECONDITION); context.set_details("Violates security policies.")
                return SubmitLanguageEvolutionProposalResponse(status="error", message="Security policy violation.")
            agent_info = self.agent_registry.get_agent_info(agent_id)
            trust_level = agent_info.get("trust_level", 0.0) if agent_info else 0.0
            if not self.security_audit_engine.validate_trust_policy(proposal_node_dict, trust_level):
                 context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Agent trust level too low.")
                 return SubmitLanguageEvolutionProposalResponse(status="error", message="Agent trust level too low.")

            # --- Submit ---
            proposal_id = self.lang_evolution_registry.submit_proposal(proposal_node_dict)
            self.security_audit_engine.log_audit("language_proposal_submitted", {"id": proposal_id}, agent_id, "agent")
            self.logger.info(f"Language proposal {proposal_id} submitted by {agent_id}.")
            return SubmitLanguageEvolutionProposalResponse(status="success", proposal_id=proposal_id)

        except Exception as e:
            self.logger.exception(f"Internal error in SubmitLanguageEvolutionProposal for {agent_id}: {e}")
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Internal server error: {e}")
            return SubmitLanguageEvolutionProposalResponse(status="error", message=f"Internal server error: {e}")
    ...
    def RecordVote(self, request, context):
        ...
        try:
            ...
            votes_dict = dict(request.consensus_node.votes) if request.consensus_node.votes else {}
            consensus_node_dict = {
                "proposal_id": request.consensus_node.id or request.consensus_node.proposal_id, # Allow id or proposal_id
                "votes": votes_dict,
                "quorum": request.consensus_node.quorum
            }
            if not consensus_node_dict["proposal_id"]: raise ValueError("Missing proposal_id in consensus node")
            ...
            context.set_code(StatusCode.FAILED_PRECONDITION); context.set_details(f"Invalid vote data: {e}")
            return RecordVoteResponse(status="error", message=f"Invalid vote data: {e}")

        if not self._authenticate_request(agent_id, message_for_auth, signature) or \
           not self._authorize_request(agent_id, ["voter", "governor"]):
            context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Auth failed.")
            return RecordVoteResponse(status="error", message="Auth failed.")
        try:
            consensus_reached = self.lang_evolution_registry.record_vote(consensus_node_dict)
            self.security_audit_engine.log_audit("vote_recorded", {"proposal_id": proposal_id, "voter": agent_id, "vote": votes_dict.get(agent_id), "consensus": consensus_reached}, proposal_id, "language_proposal")
            return RecordVoteResponse(status="success", consensus_reached=consensus_reached)
        except Exception as e:
            self.logger.exception(f"Internal error in RecordVote for {agent_id} on {proposal_id}: {e}")
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Internal error: {e}")
    ...
    def RecordValidation(self, request, context):
        ...
        try:
            ...
            if not validation_node_dict["target"]: raise ValueError("Missing target proposal_id")
            ...
            context.set_code(StatusCode.FAILED_PRECONDITION); context.set_details(f"Invalid validation data: {e}")
            return RecordValidationResponse(status="error", message=f"Invalid validation data: {e}")

        if not self._authenticate_request(agent_id, message_for_auth, signature) or \
           not self._authorize_request(agent_id, ["validator", "governor"]):
            context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Auth failed.")
            return RecordValidationResponse(status="error", message="Auth failed.")
        try:
            validation_passed = self.lang_evolution_registry.record_validation(validation_node_dict)
            self.security_audit_engine.log_audit("validation_recorded", {"proposal_id": proposal_id, "validator": agent_id, "passed": validation_passed}, proposal_id, "language_proposal")
            return RecordValidationResponse(status="success", validation_passed=validation_passed)
        except Exception as e:
            self.logger.exception(f"Internal error in RecordValidation for {agent_id} on {proposal_id}: {e}")
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Internal error: {e}")
            return RecordValidationResponse(status="error", message=f"Internal error: {e}")
    ...
    def DeployGrammarVersion(self, request, context):
        ...
        self.logger.info(f"Received DeployGrammarVersion request: {agent_id} for {proposal_id} -> {new_grammar_version}")
        try:
            request_dict = {"agent_id": agent_id, "proposal_id": proposal_id, "new_grammar_version": new_grammar_version}
            ...
            context.set_code(StatusCode.INTERNAL); context.set_details("Internal error.")
            return DeployGrammarVersionResponse(status="error", message="Internal error.")

        if not self._authenticate_request(agent_id, message_for_auth, signature) or \
           not self._authorize_request(agent_id, ["governor", "deployer"]):
            context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Auth failed.")
            return DeployGrammarVersionResponse(status="error", message="Auth failed.")
        try:
            deployed = self.lang_evolution_registry.deploy_grammar_version(proposal_id, new_grammar_version)
            if deployed:
                self.security_audit_engine.log_audit("grammar_deployed", {"proposal_id": proposal_id, "version": new_grammar_version}, agent_id, "agent")
                return DeployGrammarVersionResponse(status="success", deployed=True)
            else:
                 context.set_code(StatusCode.FAILED_PRECONDITION); context.set_details("Deployment failed (proposal not validated?).")
                 return DeployGrammarVersionResponse(status="error", message="Deployment failed (proposal not validated?).", deployed=False)
        except Exception as e:
            self.logger.exception(f"Internal error in DeployGrammarVersion for {agent_id} on {proposal_id}: {e}")
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Internal error: {e}")
            return DeployGrammarVersionResponse(status="error", message=f"Internal error: {e}")
    ...
    def QueryProposals(self, request, context):
        ...
        if not agent_info:
             ...
             context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Agent not registered.")
             return QueryProposalsResponse(status="error", message="Agent not registered.")

        self.logger.info(f"Received QueryProposals request from {agent_id}")
        filters = {"status": request.status if request.HasField("status") else None,
                   "proposed_by": request.proposed_by if request.HasField("proposed_by") else None}
        self.security_audit_engine.log_audit("proposals_queried", {"querier": agent_id, "filters": filters}, agent_id, "agent_query")
        try:
            ...
            for prop_dict in proposals_data:
                 ...
                 content_bytes = json.dumps(content if isinstance(content, dict) else {}).encode('utf-8')
                 pb_prop = Node(id=prop_dict.get("id", ""), type=prop_dict.get("type", "ProposalNode"),
                                metadata=prop_dict.get("metadata", {}), proposed_by=prop_dict.get("proposed_by", ""),
                                rationale=prop_dict.get("rationale", ""), proposal_content=content_bytes)
                 pb_proposals.append(pb_prop)
            return QueryProposalsResponse(status="success", proposals=pb_proposals)
        except Exception as e:
            self.logger.exception(f"Internal error during QueryProposals for {agent_id}: {e}")
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Internal server error: {e}")
            return QueryProposalsResponse(status="error", message=f"Internal server error: {e}")
    ...
    def GetFullAuditLog(self, request, context):
        ...
        if not self._authenticate_request(agent_id, message_for_auth, signature) or \
           not self._authorize_request(agent_id, ["auditor", "governor"]):
            context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Auth failed.")
        ...
        try:
            ...
            for entry_dict in audit_log_data:
                ...
                details_bytes = json.dumps(details if isinstance(details, dict) else {}).encode('utf-8')
                pb_entry = AuditLogEntry(action=entry_dict.get("action", ""), details=details_bytes,
                                         entity_id=entry_dict.get("entity_id", ""), entity_type=entry_dict.get("entity_type", ""))
                ts_str = entry_dict.get("timestamp")
                if ts_str:
                    try:
                        if HAS_GRPC:
                            ts = Timestamp(); ts.FromJsonString(ts_str); pb_entry.timestamp = ts
                        else:
                            pb_entry.timestamp = ts_str # Store string in mock attribute
                    except Exception as ts_err:
                         self.logger.warning(f"Could not parse timestamp '{ts_str}' for audit log: {ts_err}")
                         # Keep default timestamp
                pb_audit_log.append(pb_entry)
            return GetFullAuditLogResponse(status="success", audit_log=pb_audit_log)
        except Exception as e:
            self.logger.exception(f"Internal error during GetFullAuditLog for {agent_id}: {e}")
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Internal error: {e}")
            return GetFullAuditLogResponse(status="error", message=f"Internal error: {e}")
    ...
    def VerifyAuditLogIntegrity(self, request, context):
        ...
        if not agent_info:
             ...
             context.set_code(StatusCode.PERMISSION_DENIED); context.set_details("Agent not registered.")
             return VerifyAuditLogIntegrityResponse(status="error", message="Agent not registered.")

        self.logger.info(f"Received VerifyAuditLogIntegrity request from {agent_id}")
        self.security_audit_engine.log_audit("integrity_check_requested", {"requester": agent_id}, agent_id, "agent_action")
        try:
            ...
            return VerifyAuditLogIntegrityResponse(status="success", integrity_valid=is_valid)
        except Exception as e:
            self.logger.exception(f"Internal error during VerifyAuditLogIntegrity for {agent_id}: {e}")
            context.set_code(StatusCode.INTERNAL); context.set_details(f"Internal error: {e}")
            return VerifyAuditLogIntegrityResponse(status="error", message=f"Internal error: {e}")
...
def serve(port: int = 50051, db_path: str = DB_PATH):
    ...
    example_agents = [
        ("agent-alice", ["proposer", "voter", "validator", "governor", "auditor", "deployer"], 0.9),
    ...
    for agent_id, roles, trust_level in example_agents:
         if not agent_registry.get_agent_info(agent_id): # Check before registering
             try:
                 agent_registry.register_agent({"id": agent_id, "roles": roles, "trust_level": trust_level})
             except Exception as e:
    ...
    try:
        ...
        server.start()
        logging.info(f"Registry API Server started on port {port} using database {db_path}")
        server.wait_for_termination() # Keep server running until interrupted

    except ImportError:
        logging.error("Could not import 'registry_pb2_grpc'. Ensure protobuf files are compiled.")
        logging.error("Example compile command (run from root):")
        logging.error("python -m grpc_tools.protoc -I./src/governance/protos --python_out=./src/governance --grpc_python_out=./src/governance ./src/governance/protos/registry.proto")
