...
class UncertaintyEstimator:
    ...
    def estimate_uncertainty(self, features: np.ndarray, 
                            predictions: Optional[np.ndarray] = None) -> float:
        """
...
class InformationGainCalculator:
    ...
    def calculate_gain(self, 
                      ...
                      expected_posterior: np.ndarray) -> float:
        """
    ...
    def expected_gain_features(self, 
                              current_features: np.ndarray,
                              feature_tier: int) -> float:
        """
...
class CostEstimator:
    ...
    def estimate_cost(self, source: InformationSource, 
                     context: Optional[Dict[str, Any]] = None) -> InformationCost:
        """
...
class ValueCalculator:
    ...
    def expected_value_current(self, 
                              confidence: float,
                              expected_quality: float) -> float:
        """
    ...
    def expected_value_with_info(self,
...
class ValueOfInformationGate:
    ...
    def should_probe_deeper(self,
                          features: np.ndarray,
                          predictions: Optional[np.ndarray],
                          budget_remaining: Dict[str, float]) -> Tuple[bool, Optional[str]]:
        """
        ...
        state = DecisionState(
            features=features,
            uncertainty=self.uncertainty_estimator.estimate_uncertainty(features, predictions),
    ...
    def evaluate_information_source(self,
                                  state: DecisionState,
                                  source: InformationSource) -> Optional[InformationValue]:
        """
        ...
        # Estimate cost
        cost = self.cost_estimator.estimate_cost(source, {
        ...
        # Calculate expected information gain
        if source in [InformationSource.TIER2_FEATURES, 
            ...
            info_gain = self.gain_calculator.expected_gain_features(state.features, tier)
        elif source == InformationSource.PROBE_TOOL:
            info_gain = self.gain_calculator.expected_gain_probe(
                state.current_best_tool or "unknown",
    ...
    def calculate_evpi(self, 
                      current_distribution: np.ndarray,
                      tool_utilities: np.ndarray) -> float:
        """
    ...
    def multi_stage_voi(self, 
                       state: DecisionState,
                       horizon: int = 3) -> List[InformationValue]:
        """
        ...
        if self.myopic or horizon <= 1:
            ...
            for source in InformationSource:
                value = self.evaluate_information_source(state, source)
                if value and (best_value is None or value.net_value > best_value.net_value):
                    best_value = value
    ...
    def _backward_induction(self, 
                           state: DecisionState,
                           horizon: int) -> List[InformationValue]:
        """
    ...
    def _record_decision(self, state: DecisionState, 
                        value: Optional[InformationValue],
                        gather: bool):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_value_per_gather': self.total_value_gained / max(1, self.gather_decisions),
    ...
    def visualize_decisions(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'gather_pattern': {
                ...
                ]) if any(u > 0.5 for u in uncertainties) else 0,
                'gather_when_confident': np.mean([
                    g for g, c in zip(gather_decisions, confidences) if c > 0.7
                ]) if any(c > 0.7 for c in confidences) else 0
            },
            'source_distribution': dict(source_counts),
            'value_distribution': {
                'positive_value_rate': sum(1 for v in net_values if v > 0) / len(net_values),
                'mean_positive_value': np.mean([v for v in net_values if v > 0]) if any(v > 0 for v in net_values) else 0
            }
        }
