    ...
    def detect(self, current_data: np.ndarray) -> List[DriftDetection]:
        ...
        for i in range(n_features):
            ...
            statistic, p_value = stats.ks_2samp(
                self.reference_samples[:, i], current_data[:, i]
            )
    ...
    def detect(self, current_data: np.ndarray) -> List[DriftDetection]:
        ...
        for i in range(n_features):
            ...
            distance = wasserstein_distance(
                self.reference_distribution[:, i], current_data[:, i]
            )
...
class DistributionMonitor:
    ...
    def set_reference(self, features: np.ndarray, performance: Optional[float] = None):
        """Set reference distribution"""
    ...
    def update(self, features: np.ndarray, performance: Optional[float] = None) -> bool:
        """
    ...
    def detect_shift(self, features: np.ndarray, result: Optional[Any] = None) -> bool:
        """
        ...
        with self.lock:
            ...
            moderate_drifts = [
                d for d in detections if d.severity == DriftSeverity.MODERATE
            ]
    ...
    def save_state(self, path: str):
        ...
        with self.lock:
            ...
            if self.reference_distribution is not None:
                np.save(
                    save_path / "reference_distribution.npy",
