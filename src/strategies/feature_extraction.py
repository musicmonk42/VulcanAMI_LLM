...
class SyntacticFeatureExtractor(FeatureExtractor):
    ...
    def __init__(self):
        self.feature_names = [
            'length', 'num_tokens', 'num_unique_tokens', 'avg_token_length',
            'num_numbers', 'num_symbols', 'num_uppercase', 'num_punctuation',
            'has_equation', 'has_logic', 'has_probability', 'has_graph',
            'entropy', 'complexity_score', 'type_indicator'
        ]
    ...
    def extract(self, problem: Any) -> np.ndarray:
        ...
        features.append(1.0 if any(op in text for op in ['=', '+', '-', '*', '/', '^']) else 0.0)
        features.append(1.0 if any(op in text for op in ['∧', '∨', '¬', '→', '⊢', 'and', 'or', 'not']) else 0.0)
        features.append(1.0 if any(term in text.lower() for term in ['probability', 'likely', 'chance', 'random']) else 0.0)
        features.append(1.0 if any(term in text.lower() for term in ['node', 'edge', 'graph', 'network', 'connected']) else 0.0)
...
class StructuralFeatureExtractor(FeatureExtractor):
    ...
    def _extract_structural_complexity(self, structure: ProblemStructure) -> List[float]:
        ...
        if structure.text:
            ...
            features.append(float(np.mean([len(s.split()) for s in sentences]) if sentences else 0))
            ...
            clause_count = sum(marker in structure.text.lower() for marker in clause_markers)
    ...
    def _extract_dependency_features(self, structure: ProblemStructure) -> List[float]:
        ...
        if structure.tokens:
            ...
            if pos_tags:
                ...
                noun_ratio = sum(pos_counts.get(tag, 0) for tag in ['NN', 'NNS', 'NNP', 'NNPS']) / len(pos_tags)
                verb_ratio = sum(pos_counts.get(tag, 0) for tag in ['VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ']) / len(pos_tags)
                adj_ratio = sum(pos_counts.get(tag, 0) for tag in ['JJ', 'JJR', 'JJS']) / len(pos_tags)
...
class SemanticFeatureExtractor(FeatureExtractor):
    ...
    def _extract_semantic_similarity(self, structure: ProblemStructure) -> List[float]:
        ...
        problem_types = {
            ...
            'probabilistic': ['probability', 'random', 'distribution', 'likelihood', 'chance'],
    ...
    def _extract_conceptual_features(self, structure: ProblemStructure) -> List[float]:
        ...
        if structure.tokens:
            ...
            for concept_type, concept_words in concepts.items():
                presence = len(token_set.intersection(concept_words)) / len(concept_words)
        ...
        if structure.tokens:
            token_set = set(t.lower() for t in structure.tokens)
            abstract_score = len(token_set.intersection(abstract_indicators)) / len(abstract_indicators)
            concrete_score = len(token_set.intersection(concrete_indicators)) / len(concrete_indicators)
    ...
    def _extract_inference_complexity(self, structure: ProblemStructure) -> List[float]:
        ...
        if structure.text:
            ...
            inference_steps = sum(1 for marker in inference_markers if marker in structure.text.lower())
            features.append(inference_steps)
            
            # Logical connectives
            connectives = ['and', 'or', 'not', 'if', 'then', 'iff']
            connective_count = sum(1 for conn in connectives if conn in structure.text.lower())
            features.append(connective_count)
            
            # Quantifier complexity
            quantifiers = ['all', 'some', 'none', 'exists', 'every']
            quantifier_count = sum(1 for q in quantifiers if q in structure.text.lower())
            features.append(quantifier_count)
            
            # Negation complexity
            negations = ['not', 'no', 'none', 'neither', 'never']
            negation_count = sum(1 for neg in negations if neg in structure.text.lower())
...
class MultimodalFeatureExtractor(FeatureExtractor):
    ...
    def _extract_modality_features(self, problem: Any, modality: str) -> List[float]:
        ...
        if modality == 'text':
            if isinstance(problem, str):
                ...
                return list(self.modality_extractors['text'].extract(problem['text'])[:50])
    ...
    def _extract_cross_modal_features(self, problem: Any, modalities: List[str]) -> List[float]:
    ...
    def _extract_deep_reasoning_features(self, problem: Any) -> List[float]:
        ...
        inference_count = sum(1 for marker in inference_markers if marker in text.lower())
...
class MultiTierFeatureExtractor:
    ...
    def extract_adaptive(self, problem: Any, time_budget_ms: float) -> np.ndarray:
        ...
        for tier in [FeatureTier.TIER4_MULTIMODAL, FeatureTier.TIER3_SEMANTIC,
                     FeatureTier.TIER2_STRUCTURAL, FeatureTier.TIER1_SYNTACTIC]:
