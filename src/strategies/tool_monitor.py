...
class ToolMonitor:
    ...
    def get_health_status(self) -> HealthStatus:
        ...
        # System error rate
        system_error_rate = (self.system_metrics.total_failures / 
        ...
        if format == 'json':
            ...
            with open(export_path, 'w', newline='') as f:
                if self.tool_metrics:
                    fieldnames = list(next(iter(self.tool_metrics.values())).to_dict().keys())
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'health_status': self.get_health_status().value,
            'buffer_sizes': {
                name: len(buffer.data)
                for name, buffer in self.time_series.items()
            }
        }
