...
class GradientSurgeryUnlearner:
    ...
    def unlearn_batch(
        ...
        logger.info(f"Starting gradient surgery: forget={len(forget_set)}, retain={len(retain_set)}")
    ...
    def _gradient_surgery(
        self,
        forget_grads: np.ndarray,
        retain_grads: np.ndarray,
...
class UnlearningEngine:
    ...
    def unlearn(
        ...
        if self.method == "gradient_surgery":
            result = self._unlearn_gradient_surgery(forget_bytes, retain_bytes, fast_lane)
    ...
    def _unlearn_sisa(
        ...
        logger.info(f"SISA unlearning: {len(affected_shards)}/{self.shard_count} shards affected")
    ...
    def _verify_unlearning(
        ...
        verification_passed = (
            avg_forget_score < 0.3 and  # Forgotten
            avg_retain_score > 0.7      # Retained
        )
    ...
    def _extract_pattern_from_packfile(
        self,
        packfile: str,
        pattern: str
    ) -> List[Any]:
    ...
    async def unlearn_async(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        return await loop.run_in_executor(
            self.executor,
            lambda: self.unlearn(*args, **kwargs)
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_unlearnings": len(self.unlearning_log),  # Alias for backward compatibility
            "verified_removals": len(self.verified_removals),
            "shard_count": self.shard_count,
            "enable_verification": self.enable_verification,
            "recent_operations": self.unlearning_log[-10:] if self.unlearning_log else []
        }
