...
class S3Store:
    ...
    def get_object(self, key: str, byte_range: Optional[Tuple[int, int]] = None) -> bytes:
        """
        ...
        **kwargs
        ...
        try:
            ...
            while offset < len(data):
                ...
                logger.debug(f"Uploaded part {part_number - 1}/{(len(data) + part_size - 1) // part_size}")
    ...
    def _sign_request(
        ...
        credential = f"{self.access_key}/{now.strftime('%Y%m%d')}/{self.region}/s3/aws4_request"
...
class PackfileStore:
    ...
    upload_stats: Dict[str, Any] = field(default_factory=lambda: {
        "total_uploads": 0,
        "total_bytes": 0,
        "failed_uploads": 0
    })
    download_stats: Dict[str, Any] = field(default_factory=lambda: {
        "total_downloads": 0,
        "total_bytes": 0,
        "cache_hits": 0
    })
    ...
    def upload(
        ...
        content_type = "application/zstd" if self.compression == "zstd" else "application/octet-stream"
    ...
    def download(
        ...
        # Check cache first
        if use_cache and self.enable_cache and path in self.cache and byte_range is None:
        ...
        try:
            ...
            if use_cache and self.enable_cache and byte_range is None and len(data) < 10 * 1024 * 1024:  # < 10MB
    ...
    async def upload_async(self, pack_bytes: bytes, **kwargs) -> str:
        ...
        return await loop.run_in_executor(
            self.executor,
            lambda: self.upload(pack_bytes, **kwargs)
        )
    ...
    async def download_async(self, path: str, **kwargs) -> bytes:
        ...
        return await loop.run_in_executor(
            self.executor,
            lambda: self.download(path, **kwargs)
        )
