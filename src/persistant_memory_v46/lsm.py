...
class MerkleLSM:
    ...
    def compact_level(self, level: int) -> List[str]:
        ...
        logger.info(f"Compacting level {level} with {len(self.packfiles[level])} packfiles")
        ...
        try:
            ...
            logger.info(f"Level {level} compaction completed in {elapsed:.2f}s, created {len(new_packfiles)} packfiles")
    ...
    def find_pattern(self, pattern: str) -> List[str]:
        ...
        for level in sorted(self.packfiles.keys()):
            for packfile in self.packfiles[level]:
                ...
                if pattern.endswith("*"):
                    prefix = pattern[:-1]
                    if packfile.bloom_filter and not packfile.bloom_filter.contains(prefix):
                        continue
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_size = sum(
            sum(p.size_bytes for p in packs)
            for packs in self.packfiles.values()
        )
        
        # Calculate total items across all levels
        total_items = sum(
            sum(p.item_count for p in packs)
            for packs in self.packfiles.values()
        )
    ...
    def _is_tombstone(self, value: Any) -> bool:
        ...
        return isinstance(value, tuple) and len(value) == 2 and value[0] == "__TOMBSTONE__"
