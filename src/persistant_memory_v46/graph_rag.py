...
class GraphRAG:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(f"GraphRAG initialized (FAISS: {self.use_faiss}, NX: {NX_AVAILABLE})")
    ...
    def add_document(
        ...
        **kwargs
        ...
        # Create parent node
        parent_embedding = embedding if embedding is not None else self._get_text_embedding(content_str[:1000])
        ...
        for chunk_id, chunk_text, chunk_emb in chunk_list:
            ...
            if self.use_faiss:
                try:
                    ...
                    logger.error(f"Failed adding embedding to FAISS index for {chunk_id}: {e}")
            else:
    ...
    def add_edge(self, src: str, dst: str, rel_type: str = "related", metadata: Optional[Dict] = None):
    ...
    def _vector_search_numpy(self, query_emb: np.ndarray, k: int) -> List[RetrievalResult]:
    ...
    def _merge_results(self, results1: List[RetrievalResult], results2: List[RetrievalResult]) -> List[RetrievalResult]:
    ...
    def _rerank(self, query: str, results: List[RetrievalResult]) -> List[RetrievalResult]:
        ...
        try:
            pairs = [(query, r.content[:500]) for r in results]  # Truncate for efficiency
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            **self.stats,
            ...
            "index_size": self.vector_index.ntotal if (self.use_faiss and self.vector_index) else len(getattr(self, "embeddings_list", [])),
        ...
        if os.path.exists(meta_path):
            try:
                with open(meta_path) as f:
                    meta = json.load(f)
                    self.index_to_node_id = {int(k): v for k, v in meta.get("index_to_node_id", {}).items()}
