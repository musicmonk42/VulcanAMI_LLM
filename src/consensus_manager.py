...
class LeaderElector:
    ...
    def elect_leader(self, agents: List[str], timeout: float = 1.0) -> str:
        ...
        for agent_id in agents[1:]:
            ...
            granted, _ = self.request_vote(candidate_id, self.state.current_term, agents)
...
class ConsensusManager:
    ...
    def __init__(
        ...
        if deadlock_threshold < 1:
            raise ValueError(f"Deadlock threshold must be >= 1, got {deadlock_threshold}")
        ...
        try:
            ...
            for a in agents:
                pending[a] = executor.submit(
                    _simulate_vote, a, proposal_id,
                    self.chaos_params, self.timeout, self._base_seed
                )
                    ...
                    try:
                        ...
                        if attempts[agent] <= self.max_retries:
                            ...
                            pending[agent] = executor.submit(
                                _simulate_vote, agent, proposal_id,
                                self.chaos_params, self.timeout, self._base_seed
                            )
                        else:
                    ...
                    try:
                        ...
                        if attempts[agent] <= self.max_retries:
                            pending[agent] = _vote_task.options(num_cpus=0).remote(
                                agent, proposal_id, self.chaos_params,
                                self.timeout, self._base_seed
                            )
                        else:
                            pending.pop(agent, None)
