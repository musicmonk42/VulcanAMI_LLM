...
class KeyManager:
    ...
    def _save_keys(self, private_key: ec.EllipticCurvePrivateKey, 
...
class ConnectionPool:
    ...
    def __init__(self, db_path: Path, max_connections: int = DEFAULT_MAX_CONNECTIONS,
    ...
    def get_connection(self) -> sqlite3.Connection:
        ...
        with self.lock:
            ...
            if len(self.in_use) < self.max_connections:
                ...
                conn.execute("PRAGMA journal_mode=WAL" if WAL_MODE_ENABLED else "PRAGMA journal_mode=DELETE")
...
class PersistenceLayer:
    ...
    def __init__(self, db_path: str = "./graphix_memory/graphix_storage.db",
    ...
    def _verify_signature(self, data: bytes, signature: str, use_cache: bool = True) -> bool:
    ...
    def verify_integrity(self, force: bool = False):
        ...
        with self._transaction() as conn:
            ...
            for table, (id_col, data_col) in tables_and_cols.items():
                ...
                query = f"SELECT {id_col}, {data_col}, signature FROM {table}{where_clause}"
                
                for item_id, data, signature in cursor.execute(query).fetchall():
                    if self._verify_signature(data.encode('utf-8'), signature, use_cache=False):
                        # Mark as verified
                        cursor.execute(
                        ...
                        failed_count += 1
                        logger.error(f"Integrity failure in table '{table}' for ID {item_id}")
                        raise IntegrityError(f"Integrity failure in table '{table}' for ID {item_id}")
    ...
    def backup(self):
        ...
        backup_db_path = self.backup_path / f"backup_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}.db"
        ...
        with self.audit_lock:
            ...
            if self.audit_log_path.exists():
                if self.audit_log_path.stat().st_size > AUDIT_LOG_MAX_SIZE:
                    ...
                    rotated_path = self.audit_log_path.with_suffix(f".{timestamp}.jsonl")
    ...
    def _extract_features(self, graph: Dict) -> Dict:
        ...
        return {
            ...
            "has_cycles": self._detect_cycles(nodes, edges) if NETWORKX_AVAILABLE else None
        }
    ...
    def recall_graph(self, graph_id: str, use_cache: bool = True) -> Optional[Dict]:
        ...
        try:
            ...
            if result:
                ...
                if not verified:
                    if not self._verify_signature(graph_data.encode('utf-8'), signature):
                        self._audit_log("integrity_failure", {"type": "graph", "id": graph_id})
                        raise IntegrityError(f"Signature verification failed for graph {graph_id}")
            ...
            if result:
                ...
                if not verified:
                    if not self._verify_signature(evo_data.encode('utf-8'), signature):
                        self._audit_log("integrity_failure", {"type": "evolution", "id": evolution_id})
                        raise IntegrityError(f"Signature verification failed for evolution {evolution_id}")
    ...
    def recall_knowledge(self, knowledge_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if result:
                ...
                if not verified:
                    if not self._verify_signature(knowledge_data.encode('utf-8'), signature):
                        self._audit_log("integrity_failure", {"type": "knowledge", "id": knowledge_id})
                        raise IntegrityError(f"Signature verification failed for knowledge {knowledge_id}")
        ...
        try:
            with self._get_connection() as conn:
                ...
                for knowledge_data, signature in cursor.fetchall():
                    if self._verify_signature(knowledge_data.encode('utf-8'), signature):
    ...
    def query_graphs_by_features(self, node_count: Optional[int] = None, 
                                 op: str = '>', 
...
if __name__ == "__main__":
    ...
    print(f"✓ Recalled graph: {recalled is not None and recalled['id'] == 'test_graph_001'}")
    ...
    knowledge_items = [
        ...
        {"type": "optimization", "name": "perf_tip_1", "description": "Cache intermediate results"}
    ]
    ...
    print(f"✓ Stored in cache: {persistence.working_memory.recall('test_key') is not None}")
    
    import time
    time.sleep(1.1)
    print(f"✓ Expired from cache: {persistence.working_memory.recall('test_key') is None}")
