...
class EncryptNode:
    ...
    def _validate_data(self, data: Any) -> None:
        ...
        if isinstance(data, (dict, list)):
            ...
            if len(data) > MAX_STRING_LENGTH:
                raise ValueError(f"String data exceeds maximum length of {MAX_STRING_LENGTH}")
        ...
        try:
            ...
            if isinstance(tensor, (list, tuple)):
                ...
                if total > MAX_TENSOR_ELEMENTS:
                    logger.warning(f"Tensor has {total} elements, exceeds max {MAX_TENSOR_ELEMENTS}")
                    return False
    ...
    def _get_encryption_key(self, key_id: str, algorithm: str) -> Any:
        ...
        key_data = self.key_manager.get_key(key_id) if hasattr(self.key_manager, 'get_key') else None
        
        if key_data is None:
            # Fallback: try direct access
            key_data = self.key_manager.keys.get(key_id) if hasattr(self.key_manager, 'keys') else None
        ...
        if algorithm == 'AES':
            ...
            try:
                ...
                raise ValueError(f"Invalid key format for AES. Expected a valid Fernet key. Error: {e}")
    ...
    def execute(self, data: Any, params: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Executing EncryptNode with algorithm={algorithm}, key_id={key_id}")
            ...
            if self.compressor and tensor is not None and self._validate_tensor(tensor):
                try:
                    ...
                    if not compression_ok:
                        raise ValueError("ITU F.748.53 compression validation failed (AI_COMPRESSION_INVALID)")
                except Exception as e:
...
class PolicyNode:
    ...
    def execute(self, data: Any, params: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Executing PolicyNode with policy={policy}, enforcement={enforcement}")
        ...
        try:
            ...
            if policy == 'ITU-F.748.53' and self.compressor:
                try:
                    ...
                    if not compression_ok:
                        ...
                        if enforcement == 'restrict':
                            raise ValueError("ITU-F.748.53 non-compliance detected (AI_COMPRESSION_INVALID)")
                except Exception as e:
                    logger.error(f"Compression error: {e}")
                    compliance_result = 'non-compliant'
                    compression_ok = False
            ...
            result = {
                ...
                'compression_meta': compression_meta if policy == 'ITU-F.748.53' else {},
                'audit': {
                    'timestamp': datetime.utcnow().isoformat(),
...
def dispatch_security_node(node: Dict[str, Any], data: Any, context: Dict[str, Any]) -> Dict[str, Any]:
    """
