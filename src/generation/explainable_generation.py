...
class ExplainableGeneration:
    ...
    def explain(
        ...
        chosen_prob = None
        if probs is not None and chosen_index is not None and 0 <= chosen_index < len(probs):
    ...
    def explain_sequence(
        ...
        **kwargs
        ...
        return {
            ...
            "total_entropy": sum(e["decision"]["entropy"] or 0 for e in per_token_explanations),
            "avg_confidence": sum(e["decision"]["confidence"] or 0 for e in per_token_explanations) / len(tokens),
        }
    ...
    def _digest_chain(self, chain: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        out["logits"] = last.get("logits") if isinstance(last.get("logits"), list) else reasoning.get("logits")
        ...
        for step in chain:
            if isinstance(step, dict):
                inter = step.get("intervention") or step.get("reasoning", {}).get("intervention")
    ...
    def _decision_factors(
        ...
        if attributions:
            top_attrs = sorted(attributions, key=lambda a: a.importance, reverse=True)[:3]
    ...
    def _causal_chain_from_trace(
        ...
        for i, step in enumerate(chain):
            ...
            if step.get("safety_event"):
                ev = step["safety_event"]
                events_at_step.append(CausalEvent(
                    timestep=i,
                    event_type="safety",
    ...
    def _top_k_alternatives(
        ...
        for i in idxs:
            ...
            rejection_reason = self._infer_rejection_reason(
                prob, 
                chosen_prob, 
                rank
            )
    ...
    def _generate_counterfactuals(
        ...
        for alt in alternatives[:3]:
            ...
            if alt.token_str.lower() in ("not", "never", "no"):
                outcome = "The statement would be negated, changing meaning significantly"
    ...
    def _compute_uncertainty(
        ...
        if len(top_probs) > 1:
            prob_variance = sum((p - top_probs[0]) ** 2 for p in top_probs[1:]) / len(top_probs)
    ...
    def _compute_coherence_score(
        ...
        # Penalize variance
        conf_variance = sum((c - avg_confidence) ** 2 for c in confidences) / len(confidences)
        
        coherence = avg_confidence * (1.0 - min(1.0, avg_entropy / 10.0)) * (1.0 - min(1.0, conf_variance))
    ...
    def _render_narrative_explanation(
        ...
        if callable(self._custom_rationale):
            try:
                return self._custom_rationale(
                    summary=summary, factors=factors, alts=alts, safety=safety,
                    consensus=consensus, interventions=interventions, context=context
                )
            except Exception:
                pass
        ...
        if summary.confidence is not None:
            conf_label = "high" if summary.confidence > 0.7 else "moderate" if summary.confidence > 0.4 else "low"
        ...
        if context:
            ...
            parts.append(f" Primary context from {top_context.source} "
    ...
    def _render_technical_explanation(
        ...
        parts = [
            ...
            f"Logit: {summary.confidence:.6f}" if summary.confidence else "",  # Placeholder
            f"Entropy: {summary.entropy:.4f}" if summary.entropy else "",
            f"Perplexity: {summary.perplexity:.4f}" if summary.perplexity else "",
            f"Calibration: {summary.calibration_score:.4f}" if summary.calibration_score else "",
        ]
        ...
        if attributions:
            ...
            for attr in attributions[:5]:
                parts.append(f"  - {attr.feature_name}: {attr.importance:.3f} "
        ...
        if alts:
            parts.append("\nAlternative Probabilities:")
            for alt in alts[:5]:
                parts.append(f"  {alt.rank}. '{alt.token_str}': {alt.prob:.6f} "
        ...
        if counterfactuals:
            ...
            for cf in counterfactuals[:3]:
                parts.append(f"  - If '{self._token_to_str(cf.alternative_token)}' "
    ...
    def _render_conceptual_explanation(
        ...
        if context:
            ...
            for i, ctx in enumerate(context[:3], 1):
                parts.append(f"   {i}. {ctx.source} contributed {ctx.contribution_score:.1%} "
        ...
        if summary.confidence is not None:
            ...
            if summary.confidence > 0.8:
                ...
                parts.append("   The model has low confidence; alternative tokens were plausible.")
        
        # Safety
        safety_factors = [f for f in factors if f.get("type") == "safety"]
        if safety_factors:
            parts.append(f"\n4. Safety Validation:")
            parts.append(f"   {safety_factors[0]['events']} safety check(s) were applied "
    ...
    def _answer_why_chosen(self, explanation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for factor in factors:
            if factor.get("type") == "strategy":
                ...
                reasons.append(f"{factor['feature']} had high importance ({factor['importance']:.2f})")
    ...
    def _answer_factors(self, explanation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for f in factors:
            if f.get("type") == "attribution":
                top_factors.append(f"{f['feature']} (importance: {f['importance']:.2f})")
    ...
    def _answer_context(self, explanation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not context_contributions:
            return {"query": "How was context used?", "answer": "No context information available"}
    ...
    def _answer_safety(self, explanation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not safety_events:
            return {"query": "Were there safety concerns?", "answer": "No safety interventions required"}
