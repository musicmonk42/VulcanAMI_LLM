...
class UnifiedGeneration:
    ...
    def __init__(self, config: Optional[UnifiedGenConfig] = None) -> None:
        ...
        self._module_performance: Dict[str, deque] = defaultdict(lambda: deque(maxlen=100))
    ...
    def generate_candidates(
        ...
        temperature = float(reasoning_modules.get("temperature") or self.cfg.temperature)
        diversity_penalty = float(reasoning_modules.get("diversity_penalty") or self.cfg.diversity_penalty)
        ...
        if self.cfg.enable_dynamic_weights:
            weights = self._adapt_weights(weights, per_module_proposals, module_confidences, context)
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if self._generation_history:
            avg_time = sum(h["processing_time"] for h in self._generation_history) / len(self._generation_history)
    ...
    def _detect_modules(self, modules: Dict[str, Any]) -> List[str]:
        ...
        # Known module keys
        known = [
            "symbolic", "probabilistic", "causal", "analogical", "language",
            "meta_cognitive", "evolutionary", "adversarial", "hierarchical"
        ]
        present = [k for k in known if k in modules and self._looks_like_module(modules[k])]
        
        # Include any additional keys that look like modules
        reserved = ("context", "weights", "max_candidates", "temperature", "diversity_penalty")
            ...
            for p in proposals:
                ...
                if entry is None or contrib > entry["score"]:
                    fused[tok] = {
                        ...
                        "module_agreement": 1 if entry is None else entry["module_agreement"] + 1,
                    }
    ...
    def _fuse_rank_fusion(
        ...
        for mod_name, proposals in per_module.items():
            ...
            sorted_props = sorted(proposals, key=lambda p: p.get("module_prob", 0), reverse=True)
