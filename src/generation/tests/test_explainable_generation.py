...
class TestExplainableGenerationBasics(unittest.TestCase):
    ...
    def test_minimal_explanation(self):
        ...
        explainer = ExplainableGeneration(explanation_level=ExplanationLevel.MINIMAL)

        explanation = explainer.explain(
            token=1, chain=[{"step": "reasoning", "strategy": "symbolic"}]
        )
    ...
    def test_basic_explanation(self):
        ...
        explainer = ExplainableGeneration(
            explanation_level=ExplanationLevel.BASIC, vocab=self.vocab
        )

        logits = [0.1, 0.5, 0.3, 0.2]

        explanation = explainer.explain(
            token=1, chain=[{"step": "reasoning"}], logits=logits
        )
    ...
    def test_standard_explanation(self):
        ...
        explanation = self.explainer.explain(
            token=1, chain=chain, logits=[0.1, 0.6, 0.2, 0.1]
        )
    ...
    def test_comprehensive_explanation(self):
        ...
        explanation = explainer.explain(
            ...
            candidates=[{"token": 1, "score": 0.9}, {"token": 2, "score": 0.1}],
        )
...
class TestAlternatives(unittest.TestCase):
    ...
    def test_alternatives_ranking(self):
        ...
        explanation = self.explainer.explain(token=1, chain=[], logits=logits)
        ...
        if len(alternatives) > 1:
            for i in range(len(alternatives) - 1):
                self.assertGreaterEqual(
                    alternatives[i]["prob"], alternatives[i + 1]["prob"]
                )
    ...
    def test_alternatives_limit(self):
        ...
        explanation = self.explainer.explain(token=0, chain=[], logits=logits)
...
class TestCounterfactuals(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.vocab = MockVocab()
        self.explainer = ExplainableGeneration(
            vocab=self.vocab, enable_counterfactuals=True
        )
...
class TestAttribution(unittest.TestCase):
    ...
    def test_attribution_from_candidates(self):
        ...
        candidates = [
            ...
            },
            {
                "token": 2,
                "score": 0.2,
                "provenance": [{"module": "language", "weight": 1.0}],
            },
        ]

        explanation = self.explainer.explain(token=1, chain=[], candidates=candidates)
    ...
    def test_attention_attribution(self):
        ...
        explainer = ExplainableGeneration(
            transformer=transformer, enable_attribution=True, enable_attention_viz=True
        )

        explanation = explainer.explain(
            token=1, chain=[], attention_weights=[[0.2, 0.5, 0.3]]
        )
...
class TestExplanationFormats(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.explanation = self.explainer.explain(
            token=1, chain=self.chain, logits=[0.1, 0.7, 0.2, 0.1]
        )
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_no_chain(self):
        ...
        explanation = self.explainer.explain(token=1, chain=[])
    ...
    def test_no_logits(self):
        """Test without logits"""
        explanation = self.explainer.explain(
            token=1, chain=[{"step": "reasoning"}], logits=None
        )
    ...
    def test_invalid_token(self):
        ...
        explanation = self.explainer.explain(token=None, chain=[{"step": "reasoning"}])

        self.assertIsInstance(explanation, dict)

    def test_empty_logits(self):
        """Test with empty logits"""
        explanation = self.explainer.explain(token=1, chain=[], logits=[])

        self.assertIsInstance(explanation, dict)

    def test_single_logit(self):
        """Test with single logit value"""
        explanation = self.explainer.explain(token=0, chain=[], logits=[1.0])
    ...
    def test_all_zero_logits(self):
        ...
        explanation = self.explainer.explain(token=1, chain=[], logits=[0.0, 0.0, 0.0])
    ...
    def test_negative_logits(self):
        ...
        explanation = self.explainer.explain(
            token=1, chain=[], logits=[-2.0, -0.5, -1.0]
        )
...
class TestExplanationHistory(unittest.TestCase):
    ...
    def test_history_accumulation(self):
        ...
        for i in range(5):
            self.explainer.explain(token=i, chain=[{"step": f"test_{i}"}])

        # History should grow
        self.assertEqual(len(self.explainer._explanation_history), initial_len + 5)
