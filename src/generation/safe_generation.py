...
try:
    ...
    class ToxicityValidator:  # type: ignore
        ...
        def __init__(self) -> None:
            self._severe_words = {
                "kill", "murder", "genocide", "torture", "rape", "assault"
            }
            self._moderate_words = {
                "hate", "stupid", "idiot", "dumb", "moron", "loser",
                "worthless", "disgusting", "terrible", "awful"
            }
            self._mild_words = {
                "annoying", "boring", "lame", "silly"
            }
        ...
        def check(self, token: Any, context: Any) -> bool:
            ...
            if any(w in s for w in self._moderate_words):
                ...
                if not any(marker in ctx_str for marker in ["discuss", "analyze", "explain", "research"]):
                    return False
            return True
    ...
    class HallucinationValidator:  # type: ignore
        ...
        def __init__(self) -> None:
            ...
            self._confidence_keywords = {
                "certainly", "definitely", "absolutely", "proven", "fact"
            }
    ...
    class PromptInjectionValidator:  # type: ignore
        ...
        def _detect_injection_pattern(self, sequence: str) -> bool:
            ...
            instruction_keywords = ["ignore", "override", "disregard", "system", "admin"]
    ...
    class BiasValidator:  # type: ignore
        ...
        def _is_educational_context(self, context: Any) -> bool:
            ...
            return any(marker in ctx_str for marker in [
                "discuss", "analyze", "study", "research", "examine", "critique"
            ])
...
class SafetyMetrics:
    ...
    risk_distribution: Dict[RiskLevel, int] = field(default_factory=lambda: defaultdict(int))
    category_violations: Dict[ValidationCategory, int] = field(default_factory=lambda: defaultdict(int))
...
class SafeGeneration:
    ...
    def __init__(
        ...
        self.validators = validators or getattr(self._enhanced, "validators", []) or [
            ToxicityValidator(),
            HallucinationValidator(),
            PromptInjectionValidator(),
            PIIValidator(),
    ...
    def filter(
        ...
        for idx, c in enumerate(norm):
            ...
            if self.enable_caching and cache_key in self._cache:
                ...
                # Full validation
                safe_tok, reasons, violations = self._validate_and_repair(tok, context, wm)
        ...
        self._obs("safe_generation.filter", {
            "mode": mode,
        ...
        self._audit("safe_generation.filter", {
    ...
    def _validate_and_repair(
        ...
        for v in self.validators:
            ...
            try:
                ...
                if not safe:
                    ...
                    severity = getattr(v, "get_severity", lambda t: RiskLevel.MEDIUM)(out)
        ...
        if isinstance(self._enhanced, EnhancedSafetyValidator):
            if hasattr(self._enhanced, "validate_generation"):
                try:
                    final = self._enhanced.validate_generation(out, context, world_model)
                    if final != out:
        ...
        # World model validation
        if world_model:
            if hasattr(world_model, "validate_generation") and hasattr(world_model, "suggest_correction"):
                try:
    ...
    def _detect_sequence_injection(self, sequence: str) -> bool:
        ...
        injection_keywords = [
            "ignore previous", "disregard above", "override system",
            "act as admin", "forget instructions"
        ]
                ...
                if not is_coherent and hasattr(world_model, "repair_sequence"):
                    ...
                    if repaired:
                        self._emit_event(
                            "sequence_repaired",
                            ValidationCategory.CONSISTENCY,
                            {"original_length": len(tokens), "repaired_length": len(repaired)},
                            RiskLevel.LOW
                        )
                        return repaired
            except Exception:
    ...
    def _select_candidates(
        self,
        indexed: List[Tuple[int, RiskAssessment]],
        mode: str,
        top_k: int
    ) -> List[int]:
    ...
    def _apply_adaptive_adjustment(self, score: float, context: Dict[str, Any]) -> float:
