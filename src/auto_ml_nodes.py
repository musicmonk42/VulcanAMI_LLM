...
class RandomNode:
    ...
    def execute(self, params: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Executing RandomNode with distribution={distribution}, range={range_vals}")
        ...
        try:
            ...
            if tensor is not None:
                if isinstance(tensor, (list, np.ndarray)):
                    ...
                    if tensor_array.size > MAX_TENSOR_SIZE:
                        raise ValueError(f"Tensor too large: {tensor_array.size} > {MAX_TENSOR_SIZE}")
                else:
            ...
            if distribution == 'uniform':
                ...
                if len(range_vals) < 1:
                    raise ValueError("Discrete distribution requires at least one value")
            ...
            if self.compressor and tensor is not None:
                try:
                    ...
                    compression_meta = {
                        ...
                        "compressed_size": len(compressed) if hasattr(compressed, '__len__') else None
                    }
            ...
            error_result = {
                ...
                'photonic_meta': photonic_meta if photonic_meta else {'error': 'Not attempted'},
                'ethical_label': ethical_label,
                'audit': {
                    'timestamp': datetime.utcnow().isoformat(),
                    'node_type': 'RandomNode',
    ...
    def execute(self, params: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(space) > MAX_SPACE_DIMENSIONS:
                raise ValueError(f"Too many dimensions: {len(space)} > {MAX_SPACE_DIMENSIONS}")
        ...
        try:
            ...
            if kernel is not None:
                ...
                if len(kernel) > MAX_KERNEL_LENGTH:
                    raise ValueError(f"Kernel too long: {len(kernel)} > {MAX_KERNEL_LENGTH}")
            ...
            result = {
                ...
                'audit': {
                    ...
                    'energy_nj': energy_nj,
                    'compression_ok': compression_ok,
                    'kernel_audit': kernel_audit if kernel_audit and 'error' not in kernel_audit else None,
                    'ethical_label': ethical_label
                }
            }
            ...
            error_result = {
                ...
                'compression_meta': compression_meta if compression_meta else {'error': 'Not attempted'},
                'photonic_meta': photonic_meta if photonic_meta else {'error': 'Not attempted'},
                'kernel_audit': kernel_audit if kernel_audit else {'error': 'Not attempted'},
                'ethical_label': ethical_label,
                'audit': {
                    'timestamp': datetime.utcnow().isoformat(),
                    'node_type': 'SearchNode',
...
def dispatch_auto_ml_node(node: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"   Compression: {result.get('compression_ok', False)}")
        print(f"   Energy: {result.get('energy_nj', 'N/A')} nJ")
        print(f"   Kernel audit: {result.get('kernel_audit', {}).get('available', 'N/A')}")
    except Exception as e:
        print(f"   Error: {e}")
    ...
    for i, entry in enumerate(context['audit_log'], 1):
        print(f"   {i}. {entry['node_type']}: {entry['status']} at {entry['timestamp']}")
