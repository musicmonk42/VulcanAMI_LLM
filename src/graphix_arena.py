...
os.environ.setdefault(
    "VULCAN_AUTO_APPLY_POLICY",
    str(_repo_root / "configs" / "auto_apply_policy.yaml")
)
...
try:
    ...
    set_config("intrinsic_drives_config.auto_apply_policy", os.environ["VULCAN_AUTO_APPLY_POLICY"])
    print("✅ Applied runtime config overrides for auto-apply (approval_required=False).")
except ModuleNotFoundError as e:
    # More specific handling for module not found
    print(f"⚠️  Could not apply runtime config overrides for auto-apply: {e}. Relying on environment variables.")
except Exception as e:
...
# Prometheus metrics
agent_dropout = Counter('agent_dropout', 'Number of agents who failed consensus or dropped out')
bias_detections = Counter('bias_detections', 'Number of detected bias or risky proposals')
...
class GraphSpec(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Parameters for the generator agent")
    ...
    def validate_spec_id(cls, v):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', v):
            ...
            raise ValueError("spec_id must contain only alphanumeric, underscore, or hyphen characters")
        return v
...
class GraphixIRGraph(BaseModel):
    ...
    def validate_graph_id(cls, v):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', v):
            raise ValueError("graph_id must contain only alphanumeric, underscore, or hyphen characters")
        return v
...
class GraphixArena:
    ...
    def __init__(self, port: int = 8181):
        ...
        logger.info(f"Graphix Arena initialized with {len(self.agents)} agents: {list(self.agents.keys())}")
        ...
        self.interpret_engine = InterpretabilityEngine() if INTERPRETABILITY_AVAILABLE and InterpretabilityEngine else None
        # FIX: use the correct class symbol 'NSOAligner' guarded by availability
        self.nso_aligner = NSOAligner() if NSO_ALIGNER_AVAILABLE and (NSOAligner is not None) else None
        ...
        if SLACK_AVAILABLE and WebClient:
            ...
            if slack_token:
                try:
                    ...
                    self.slack_channel = os.getenv("SLACK_ALERT_CHANNEL", "#graphix-arena-alerts")
                    logger.info(f"Slack alerts enabled for channel {self.slack_channel}")
                except Exception as e:
        ...
        if YAML_AVAILABLE:
            try:
                config_path = Path(__file__).parent.parent / "configs" / "tool_selection.yaml"
                if config_path.exists():
                    with open(config_path, 'r') as f:
                        self.tool_selection_config = yaml.safe_load(f)
                    logger.info("Loaded tool_selection.yaml configuration.")
                else:
                    logger.warning(f"tool_selection.yaml not found at {config_path}, using default task routing.")
            except Exception as e:
                logger.error(f"Failed to load or parse tool_selection.yaml: {e}")
        else:
            logger.warning("PyYAML not installed, tool selection rules will not be applied.")
    ...
    async def _run_agent(self, agent_id: str, task: str, data: Dict) -> Dict:
        ...
        if agent_id in ("generator", "evolver") and VLLM_AVAILABLE and SHARDER_AVAILABLE:
            if LLM is not None and DistributedSharder is not None:
                ...
                if "input_tensor" in data and NUMPY_AVAILABLE:
                    try:
                        pruned_tensor = rebert_prune(data["input_tensor"], threshold=0.1)
                        input_data["input_tensor"] = pruned_tensor
                        logger.info(f"ReBERT-pruned {agent_id} input tensor (threshold=0.1)")
                    except Exception as e:
        ...
        try:
            ...
            if proc.returncode != 0:
                ...
                self.audit.log_event("agent_error", {
                    "agent_id": agent_id,
                    "error": error_output
                })
                logger.error(f"Agent '{agent_id}' failed. Return Code: {proc.returncode}. Error: {error_output}")
    ...
    def run_transparent_task(self, agent_id: str, task_prompt: str, payload: Dict) -> Dict:
        """
    ...
    async def run_shadow_task(self, agent_id: str, task_prompt: str, payload: Dict) -> Dict:
        """
        ...
        Returns:
            Shadow task result
        """
        logger.info(f"Running shadow task for agent {agent_id} (graph_id={payload.get('graph_id', 'N/A')})")
        ...
        try:
            ...
            if hallucination_rate > 0.05:
                logger.warning(f"Hallucination rate {hallucination_rate:.2%} > 5%, rolling back mutation")
                
                if checkpoint is not None and hasattr(self.runtime, "restore_checkpoint"):
                    self.runtime.restore_checkpoint(checkpoint)
                
                self.audit.log_event("shadow_rollback", {
            ...
            if self.registry and hasattr(self.registry, "find_similar_topologies"):
                try:
                    ...
                    logger.info(f"Queried LTM for similar topologies, found {len(ltm_results)} matches")
                except Exception as e:
    ...
    async def rollback_failed_task(self, payload: Dict, reason: str = "") -> Dict:
        ...
        if hasattr(self.runtime, "restore_checkpoint") and hasattr(self.runtime, "get_checkpoint"):
            try:
                checkpoint = self.runtime.get_checkpoint(payload)
                if checkpoint:
                    self.runtime.restore_checkpoint(checkpoint)
    ...
    async def run_agent_task(self, agent_id: str, request: Request):
        ...
        if schema_name and schema_name in SCHEMA_MAP:
            try:
                ...
                graph_id = payload.get("graph_id") or payload.get("spec_id", "[unknown_id]")
            except Exception as e:
                raise HTTPException(
                    status_code=422,
        ...
        drift_info = None
        if self.registry and hasattr(self.registry, "get_embeddings") and self.drift_detector is not None:
            try:
                ...
                if drift_info and drift_info.get("realignment_needed"):
                    ...
                    self.audit.log_event("drift_realignment", {
        ...
        try:
            transparent_result = self.run_transparent_task(agent_id, task_prompt, payload)
        except Exception as e:
        ...
        if self.tool_selection_config:
            ...
            for rule in self.tool_selection_config.get('rules', []):
                ...
                agent_match = conditions.get('agent_id') is None or conditions.get('agent_id') == agent_id
                ...
                if agent_match and content_match:
                    ...
                    if handler_name:
                        ...
                        logger.info(f"Matched tool selection rule: '{rule.get('name', 'Unnamed')}'. Using handler: '{handler_name}'.")
                        break
        ...
        try:
            ...
            # Log completion
            self.audit.log_event("agent_task_completed", {
                "graph_id": graph_id,
                "agent": agent_id,
    ...
    async def run_tournament_task(self, request: Request):
        ...
        if not self.tournament_manager:
            raise HTTPException(
                status_code=503,
                detail="TournamentManager not available"
            )
        ...
        try:
            ...
            if len(proposals) != len(fitness):
                raise ValueError(f"proposals and fitness length mismatch: {len(proposals)} vs {len(fitness)}")
            ...
            if embedding_func is None and NUMPY_AVAILABLE:
                ...
                raise ValueError("NumPy not available and no embedding function provided")
    ...
    async def feedback_ingestion(self, request: Request):
        ...
        try:
            ...
            if missing_keys:
                raise HTTPException(
                    status_code=422,
                    detail=f"Missing required fields: {missing_keys}"
                )
            ...
            if feedback.get("score", 1.0) < 0.0 or feedback.get("failed_consensus"):
                await self.rollback_failed_task(
                    feedback,
                    reason="Negative feedback/consensus failure"
                )
    ...
    def start(self):
        ...
        logger.info("Security is ENABLED. Use the 'X-API-KEY' header for authentication.")
        logger.info("Rate limiting is ENABLED. Default: 100 requests per minute per IP.")
...
@app.post("/api/feedback_dispatch", dependencies=[Depends(get_api_key)], summary="Dispatch feedback protocol")
...
async def arena_health():
    ...
    try:
        ...
        components = {
            ...
            "tournament_manager": bool(getattr(_ARENA_INSTANCE, "tournament_manager", None)),
            "interpretability": bool(getattr(_ARENA_INSTANCE, "interpret_engine", None)),
