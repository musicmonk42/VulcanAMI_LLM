    ...
    def build_ir(
        ...
        nodes = [
            ...
            # RMSNorm uses variance/square mean, standard LayerNorm uses mean and variance
            {
                ...
                "type": "reduce_var_sq",
                "params": {"axis": -1},
            },  # Using reduce_var_sq for RMS
            {
                "id": "ln_norm",
                "type": "transform",
                "params": {"operation": "normalize", "eps": eps},
            },
        ...
        if not is_rmsnorm:
            ...
            nodes.insert(
                1, {"id": "ln_mean", "type": "reduce_mean", "params": {"axis": -1}}
            )
            nodes.append(
                {
                    "id": "ln_shift",
            ...
            nodes.append(
                {
                    "id": "ln_group",
                    "type": "transform",
                    "params": {"operation": "group_norm", "groups": num_groups},
                }
            )
        ...
        return {
            ...
            "metadata": {
                ...
                "note": (
                    ...
                    if num_groups
                    else f"Base {norm_type} implementation."
                ),
            },
        }
