...
class IREmbeddings:
    def build_ir(self, vocab_size: int, hidden_size: int, max_positions: int, dropout_p: float = 0.1) -> Dict[str, Any]:
        return {
            ...
            "nodes": [
                ...
                {"id": "emb_tokens", "type": "lookup", "params": {"table": "token", "size": hidden_size}},
                ...
                {"id": "emb_combine", "type": "combine", "params": {"mode": "none"}}, # No combination needed if no position vector
                
                # New Node: Dropout for Regularization
                {"id": "emb_dropout", "type": "dropout", "params": {"p": dropout_p}},

                {"id": "emb_norm", "type": "transform", "params": {"operation": "norm_scale"}}
            ],
            "edges": [
                # Connect token lookup directly to dropout/normalization path
                {"src": "emb_tokens", "dst": "emb_combine"}, 
                {"src": "emb_combine", "dst": "emb_dropout"},
                {"src": "emb_dropout", "dst": "emb_norm"}     # Connect dropout to normalization
            ],
            "metadata": {
