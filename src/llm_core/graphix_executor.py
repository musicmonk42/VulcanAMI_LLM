...
class KVCacheManager:
    ...
    def get(self, layer_idx: int, head_idx: int, position: int) -> Optional[KVCacheEntry]:
    ...
    def put(self, layer_idx: int, head_idx: int, position: int, 
            keys: List[float], values: List[float]) -> None:
        """Put entry in cache."""
        key = self._make_key(layer_idx, head_idx, position)
...
class QuantizationManager:
    ...
    def quantize(self, tensor: List[float], key: str = "") -> Tuple[List[int], float, float]:
        ...
        # Quantize
        quantized = [
            int(min(max(round(v / scale + zero_point), qmin), qmax))
            for v in tensor
        ]
...
class PerformanceProfiler:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            'timings': {},
            'memory_peak_mb': self.current_memory
        }
...
class GraphixExecutor:
    ...
    def __init__(self,
        ...
        ) if self.config.use_kv_cache else None
        ...
        self.auditor = AuditLogger(
            enabled=self.config.enable_audit
        )
        ...
        logger.info(f"GraphixExecutor initialized: {hidden_size}d, {num_layers}L, {num_heads}H")
    ...
    def execute(self, graph_ir: Dict[str, Any], inputs: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        # Audit log
        self.auditor.log('execution_start', {
        ...
        try:
            ...
            hidden_states = self._execute_embeddings(tokens, graph_ir.get('embedding', {}))
            ...
            # Audit log
            self.auditor.log('execution_complete', {
                'time_ms': exec_time,
                'tokens': len(tokens)
            })
    ...
    def _execute_embeddings(self, tokens: List[Any], emb_ir: Dict[str, Any]) -> List[float]:
        ...
        for token_idx in range(len(tokens)):
            ...
            token_id = int(tokens[token_idx]) if not isinstance(tokens[token_idx], int) else tokens[token_idx]
            ...
            token_emb = emb_weights[start_idx:end_idx] if start_idx < len(emb_weights) else [0.0] * self.hidden_size
    ...
    def _execute_ffn(self,
        ...
        # Gate projection
        gate = self._linear(hidden_states, gate_weight, self.hidden_size, intermediate_size)
    ...
    def _apply_layer_norm(self, hidden_states: List[float], weight_key: str) -> List[float]:
        ...
        normalized = [
            (h / rms) * w
            for h, w in zip(hidden_states, weights)
        ]
    ...
    def get_logits(self, hidden_state: Any, tokens: List[Any]) -> List[float]:
        ...
        logits = self._linear(hidden_state, lm_head_weight, self.hidden_size, self.vocab_size)
    ...
    def apply_update(self, proposal: Dict[str, Any]) -> None:
        ...
        self.auditor.log('weight_update', {
        ...
        for key, grad in gradients.items():
            if key in self.weights and isinstance(grad, list):
                ...
                self.weights[key] = [
                    w - lr * g
                    for w, g in zip(self.weights[key], grad)
                ]
