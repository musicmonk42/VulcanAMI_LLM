...
class IRAttention:
    def build_ir(self, num_heads: int, hidden_size: int, num_kv_heads: int | None = None) -> Dict[str, Any]:
        ...
        kv_hidden_size = hidden_size if not is_gqa else hidden_size // num_heads * num_kv_heads
        ...
        for h in range(num_heads):
            ...
            nodes.append({"id": f"{prefix}_q", "type": "transform", "params": {"operation": "linear_q", "size": hidden_size}})
            
            # K projection and KV cache (Only define these once per kv_head if using GQA)
            if h < num_kv_proj:
                nodes.extend([
                    {"id": f"{kv_prefix}_k", "type": "transform", "params": {"operation": "linear_k", "size": hidden_size}},
                    {"id": f"{kv_prefix}_v", "type": "transform", "params": {"operation": "linear_v", "size": hidden_size}},
                    # Enhancement 2: KV cache nodes
                    {"id": f"{kv_prefix}_cache_k", "type": "cache", "params": {"axis": 0}},
                    {"id": f"{kv_prefix}_cache_v", "type": "cache", "params": {"axis": 0}},
                ])
                edges.extend([
            ...
            nodes.append({"id": f"{prefix}_scores", "type": "generative", "params": {
            ...
            nodes.extend([
                {"id": f"{prefix}_softmax", "type": "softmax", "params": {"dim": -1}},
                {"id": f"{prefix}_attn_drop", "type": "dropout"},
                {"id": f"{prefix}_weighted", "type": "transform", "params": {"operation": "weight_values"}},
                {"id": f"{prefix}_out", "type": "transform", "params": {"operation": "linear_o", "size": hidden_size}},
                
                # Enhancement: Spiking Gate
                {"id": f"{prefix}_spike_gate", "type": "spike", "params": {"threshold": 0.3}}
            ])
            
            # Edges for attention head calculation (using KV Cache outputs)
            edges.extend([
                {"src": f"{prefix}_q", "dst": f"{prefix}_scores"},
                {"src": f"{kv_prefix}_cache_k", "dst": f"{prefix}_scores"}, # From KV Cache K
                
                {"src": f"{prefix}_scores", "dst": f"{prefix}_softmax"},
                {"src": f"{prefix}_softmax", "dst": f"{prefix}_attn_drop"}, # Dropout after Softmax
                
                {"src": f"{kv_prefix}_cache_v", "dst": f"{prefix}_weighted"}, # From KV Cache V
                {"src": f"{prefix}_attn_drop", "dst": f"{prefix}_weighted"}, # Use dropped scores for weighting
                
                {"src": f"{prefix}_weighted", "dst": f"{prefix}_spike_gate"}, # Output goes into spike gate
                {"src": f"{prefix}_spike_gate", "dst": f"{prefix}_out"}, # Spiked output goes into final projection
            ])

        # Head Merge (Combine)
        nodes.append({"id": "attn_merge", "type": "combine", "params": {"mode": "mean", "heads": num_heads}})
        for h in range(num_heads):
