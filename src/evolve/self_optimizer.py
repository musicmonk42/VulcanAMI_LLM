...
class SelfOptimizer:
    ...
    def __init__(self,
        ...
        self.target_memory_mb = max(self.MIN_MEMORY_MB, min(self.MAX_MEMORY_MB, target_memory_mb))
        ...
        self.strategies = {
            'caching': OptimizationStrategy(
                'caching',
                parameters={'cache_size': 1000, 'ttl_seconds': 300}
            ),
            'batching': OptimizationStrategy(
                'batching', 
                parameters={'batch_size': 32, 'timeout_ms': 10}
            ),
            'parallelization': OptimizationStrategy(
                'parallelization',
                parameters={'num_workers': 4, 'chunk_size': 100}
            ),
            'pruning': OptimizationStrategy(
                'pruning',
                parameters={'threshold': 0.01, 'sparsity': 0.1}
            ),
            'quantization': OptimizationStrategy(
                'quantization',
                parameters={'bits': 8, 'symmetric': True}
            ),
            'compilation': OptimizationStrategy(
                'compilation',
                parameters={'backend': 'jit', 'optimize': True}
            )
        }
    ...
    def _should_optimize(self, metrics: PerformanceMetrics) -> bool:
        ...
        needs_optimization = (
            metrics.latency_ms > self.target_latency_ms * 1.1 or
            metrics.memory_mb > self.target_memory_mb * 1.1 or
            metrics.cpu_percent > self.resource_limits['max_cpu_percent'] or
            metrics.score() < self.baseline_metrics.score() * 0.9
        )
    ...
    def _run_optimization_cycle(self):
        ...
        try:
            ...
            for name, strategy in self.strategies.items():
                ...
                # Apply strategy
                before_score = self.current_metrics.score() if self.current_metrics else 0
                ...
                if success:
                    ...
                    if improvement > 0:
                        ...
                        logger.info(f"Strategy {name} improved performance by {improvement:.2f}")
                    else:
    ...
    def _optimize_caching(self, params: Dict) -> bool:
        ...
        if hit_rate < 0.5 and cache_size < self.resource_limits['max_cache_size']:
            # Increase cache size
            new_size = min(int(cache_size * 1.5), self.resource_limits['max_cache_size'])
            ...
            new_size = max(int(cache_size * 0.8), self.resource_limits['min_cache_size'])
            params['cache_size'] = new_size
    ...
    def _optimize_batching(self, params: Dict) -> bool:
        ...
        current_batch_size = max(self.MIN_BATCH_SIZE, min(self.MAX_BATCH_SIZE, current_batch_size))
        
        # Adjust based on memory usage
        if self.current_metrics and self.current_metrics.memory_mb > self.target_memory_mb:
    ...
    def _optimize_quantization(self, params: Dict) -> bool:
        ...
        if self.current_metrics and self.current_metrics.latency_ms > self.target_latency_ms:
    ...
    def _auto_tune_parameters(self):
        ...
        for param_name, param_info in self.tunable_parameters.items():
            ...
            current_value = max(param_info['min'], min(param_info['max'], current_value))
        ...
        for param_name, param_info in self.tunable_parameters.items():
            ...
            step_size = param_info.get('step', (param_info['max'] - param_info['min']) * 0.1)
            ...
            if best_value != current_value:
                logger.info(f"Auto-tuned {param_name} from {current_value} to {best_value}")
    ...
    def _update_strategy_priorities(self, improvements: List[Tuple[OptimizationStrategy, float]]):
    ...
    def _cleanup_resources(self):
        ...
        if hasattr(self, 'thread_executor') and hasattr(self.thread_executor, '_threads'):
    ...
    def load_state(self, filepath: str):
        ...
        try:
            ...
            if 'strategies' in state:
                for name, strategy_data in state['strategies'].items():
                    if name in self.strategies:
                        ...
                        self.strategies[name].success_count = strategy_data['success_count']
                        self.strategies[name].failure_count = strategy_data['failure_count']
            ...
            if 'tunable_parameters' in state:
                for name, param_data in state['tunable_parameters'].items():
                    if name in self.tunable_parameters:
                        ...
                        current = param_data.get('current', self.tunable_parameters[name]['current'])
                        current = max(param_data['min'], min(param_data['max'], current))
            ...
            if 'optimization_interval_s' in state:
                self.optimization_interval_s = max(1, min(3600, state['optimization_interval_s']))
            if 'target_latency_ms' in state:
                self.target_latency_ms = max(1, min(10000, state['target_latency_ms']))
            if 'target_memory_mb' in state:
                self.target_memory_mb = max(self.MIN_MEMORY_MB, min(self.MAX_MEMORY_MB, state['target_memory_mb']))
