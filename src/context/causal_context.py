...
class CausalContext:
    ...
    def select(
        ...
        if isinstance(query, dict):
            ...
            include_confounders = bool(query.get("include_confounders", include_confounders))
        ...
        for e in episodic:
            result = self._score_episodic_causal(
                e, concepts, causal_related, confounders, mediators,
                world_model, causal_graph, now, temporal_window
            )
            if result:
        ...
        for s in semantic:
            result = self._score_semantic_causal(
                s, concepts, causal_related, confounders, mediators,
                world_model, causal_graph, now, temporal_window
            )
            if result:
                scored.append(result)
        ...
        for p in procedural:
            result = self._score_procedural_causal(
                p, concepts, causal_related, confounders, mediators,
                world_model, causal_graph, now, temporal_window
            )
            if result:
                scored.append(result)
        ...
        statistics = CausalStatistics(
            ...
            max_path_length=max((len(d.get("causal_path", [])) for d in top_scored), default=0),
        ...
        result = {
            ...
            "causal_graph": self._serialize_causal_graph(causal_graph) if causal_graph else None,
        ...
        if hasattr(world_model, "compute_intervention_effect"):
            try:
                original_outcome = world_model.compute_intervention_effect(
                    intervention if isinstance(intervention, dict) else {variable: original_value},
                    outcome_variable
                )
    ...
    def track_intervention(self, variable: str, value: Any, effect_on: Optional[Dict[str, float]] = None, metadata: Optional[Dict[str, Any]] = None) -> None:
    ...
    def _get_causal_graph(
        ...
        """
        # Check cache
        if self._causal_graph_cache and (time.time() - self._graph_cache_time < self._graph_cache_ttl):
    ...
    # Alias for compatibility
    def _identify_confounders(self, concepts: List[str], causal_graph: Dict[str, Any]) -> List[str]:
    ...
    # Alias for compatibility
    def _identify_mediators(self, concepts: List[str], causal_graph: Dict[str, Any]) -> List[str]:
    ...
    def _score_semantic_causal(
        ...
        terms = [str(t) for t in (item.get("terms") or [])] or self._tokenize(concept_text)
        ...
        is_confounder = concept_text in confounders or any(t in confounders for t in terms)
    ...
    def _score_procedural_causal(
        ...
        terms = [str(t) for t in (item.get("signature_terms") or [])] or self._tokenize(name)
    ...
    def _score_episodic_item(self, item, qterms, concepts, world_model):
        ...
        return self._score_episodic_causal(
            item, concepts, qterms, [], [], world_model, None, time.time(), self.temporal_window
        )["score"]
    ...
    def _generate_causal_explanations(
        ...
        recent_items = [item for item in scored_items if item.get("temporal_relevance", 0) > 0.7]
        if recent_items:
            explanations.append(
