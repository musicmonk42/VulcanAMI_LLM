...
class TestMemoryConsolidation(unittest.TestCase):
    ...
    def test_consolidate_from_episodic_to_semantic(self):
        ...
        for i in range(10):
            self.memory.store(
                f"Tell me about machine learning {i}",
                f"ML is great {i}",
                {}
            )
    ...
    def test_consolidation_marks_episodic_items(self):
        ...
        consolidated_count = sum(1 for item in self.memory.episodic if item.consolidated)
...
def run_performance_benchmark():
    ...
    print(f"2. Retrieve 100 queries: {retrieve_time:.3f}s ({100/retrieve_time:.1f} ops/sec)")
