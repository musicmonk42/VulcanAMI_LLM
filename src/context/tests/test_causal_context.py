...
class TestCausalGraphTraversal(unittest.TestCase):
    ...
    def test_traverse_causal_graph(self):
        ...
        result = self.causal_ctx._traverse_causal_graph(
            concepts,
            graph,
            max_depth=2
        )
...
class TestTemporalReasoning(unittest.TestCase):
    ...
    def test_temporal_decay_hyperbolic(self):
        ...
        ctx = CausalContext(
            decay_function=TemporalDecayFunction.HYPERBOLIC
        )
    ...
    def test_temporal_decay_power_law(self):
        ...
        ctx = CausalContext(
            decay_function=TemporalDecayFunction.POWER_LAW
        )
    ...
    def test_temporal_decay_linear(self):
        """Test linear decay"""
        ctx = CausalContext(
            decay_function=TemporalDecayFunction.LINEAR,
            decay_half_life_hours=24.0
        )
    ...
    def test_temporal_window_filtering(self):
        ...
        memory_data = {
            "episodic": [
                {"prompt": "recent", "token": "val", "trace": {}, "ts": now - 1800},  # 30 min ago
                {"prompt": "old", "token": "val", "trace": {}, "ts": now - 7200}  # 2 hours ago
            ]
        }
        
        result = ctx.select(
            world_model=world_model,
            query={"text": "test", "memory": memory_data}
        )
...
class TestInterventions(unittest.TestCase):
    ...
    def test_track_intervention(self):
        ...
        ctx.track_intervention(
            variable="X",
            value=10,
            effect_on={"Y": 0.8, "Z": 0.3}
        )
...
class TestScoring(unittest.TestCase):
    ...
    def test_score_episodic_item(self):
        ...
        score = self.ctx._score_episodic_item(
            item,
            qterms,
            concepts,
            self.world_model
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_very_large_memory(self):
        ...
        large_memory = {
            ...
            "semantic": [
                {"concept": f"c{i}", "terms": [f"t{i}"], "freq": 1}
                for i in range(500)
            ],
            "procedural": [
...
class TestExplanationGeneration(unittest.TestCase):
    ...
    def test_generate_explanations(self):
        ...
        explanations = ctx._generate_causal_explanations(
            scored_items,
            concepts,
            graph
        )
...
def run_performance_benchmark():
    ...
    large_memory = {
        "episodic": [
            {"prompt": f"prompt {i}", "token": f"token {i}", "trace": {}, "ts": time.time()}
            for i in range(500)
        ],
        "semantic": [
    ...
    for i in range(20):
        ctx.select(
            world_model,
            {"text": f"test {i}", "memory": large_memory, "limit": 10}
        )
    memory_time = time.time() - start
    print(f"2. With large memory (20 queries): {memory_time:.3f}s ({20/memory_time:.1f} ops/sec)")
    ...
    print(f"3. Deep traversal (20 queries, depth=5): {deep_time:.3f}s ({20/deep_time:.1f} ops/sec)")
