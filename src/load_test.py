...
try:
    ...
    def generate_large_graph(
        num_nodes: int = 100, density: float = 0.1, seed: Optional[int] = None
    ) -> Dict[str, Any]:
...
if api_key:
    ...
    if not _TESTING_MODE:
        logger.warning(
            "GRAPHIX_API_KEY environment variable not set. Requests will likely fail."
        )
...
class MetricsCollector:
    ...
    def _categorize_error(self, status: int, exception: Optional[Exception]) -> str:
    ...
    def on_quit(environment: Environment, **kwargs) -> None:
        ...
        try:
            if csv_success and prom_success:
                notify_success(
                    "Load test run complete. All reports written successfully."
                )
            else:
...
class GraphixArenaUser(HttpUser):
    ...
    def on_start(self) -> None:
        ...
        for i in range(num_graphs):
            try:
                ...
                graph = generate_large_graph(
                    num_nodes=num_nodes, density=density, seed=None
                )
        ...
        logger.debug(
            f"User {self.agent_id} initialized with {len(self.large_graphs)} graphs"
        )
