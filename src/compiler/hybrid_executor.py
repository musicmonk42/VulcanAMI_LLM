...
class GraphProfile:
    ...
    def update_best_mode(self):
        ...
        avg_interpreted = np.mean([m.duration_ms for m in self.interpreted_metrics[-10:]])
        ...
        if self.compiled_metrics:
            total_compilation_time = sum(m.compilation_time_ms for m in self.compiled_metrics)
            amortized_compilation = total_compilation_time / max(1, len(self.compiled_metrics))
...
class HybridExecutor:
    ...
    def _compile_graph(self, graph: Dict[str, Any], graph_hash: str) -> Optional[bytes]:
        ...
        try:
            ...
            future = self.thread_executor.submit(
                self.compiler.compile_graph, graph
            )
            ...
            # Cache the binary
            if binary:
                self.binary_cache.put(graph_hash, binary, {
        ...
        try:
            ...
            if lib_path and os.path.exists(lib_path):
                try:
                    ...
                    self.logger.error(f"Failed to clean up temporary file {lib_path}: {cleanup_e}")
        ...
        for node in graph.get("nodes", []):
            if node.get("type") == "InputNode":
                ...
                if isinstance(value, (list, np.ndarray)):
                    arr = np.asarray(value, dtype=np.float32)
                    setattr(inputs, node["id"], arr.ctypes.data_as(ctypes.POINTER(ctypes.c_float)))
                else:
    ...
    def _allocate_outputs(self, graph: Dict[str, Any]):
        ...
        output_nodes = [n for n in graph.get("nodes", []) if n.get("type") == "OutputNode"]
        ...
        for node in output_nodes:
            ...
            setattr(outputs, node["id"], ctypes.cast(buffer, ctypes.POINTER(ctypes.c_double)))
    ...
    async def execute_with_profiling(
        ...
        if force_mode:
            ...
            mode = ExecutionMode.INTERPRETED if profile.execution_count % 2 == 1 else ExecutionMode.COMPILED
        else:
            # Use best known mode
    ...
    async def benchmark_graph(
        ...
        compilation_start = time.perf_counter()
        binary = self._compile_graph(graph, graph_hash)
        results["compilation_time_ms"] = (time.perf_counter() - compilation_start) * 1000
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "compiled_graphs": sum(1 for p in self.profiles.values() if p.compilation_success),
            "average_speedup": np.mean([p.speedup for p in self.profiles.values()]) if self.profiles else 1.0
        }
        ...
        if self.execution_history:
            ...
            stats["recent_avg_duration_ms"] = np.mean([e["duration_ms"] for e in recent])
