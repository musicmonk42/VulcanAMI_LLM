...
def initialize_llvm():
    ...
    except Exception as e:        pass  # Need to initialize
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: Could not create default target during initialization: {e}")
    except Exception as e:
...
class LLVMBackend:
    ...
    def _declare_intrinsics(self):
        ...
        self.intrinsics['sqrt'] = ir.Function(self.module, sqrt_sig, name="llvm.sqrt.f64")
        ...
        fma_sig = ir.FunctionType(ir.DoubleType(), [ir.DoubleType(), ir.DoubleType(), ir.DoubleType()])
    ...
    def _create_execution_engine(self):
        ...
        try:
            ...
            self.execution_engine = llvm.create_mcjit_compiler(backing_mod, target_machine)
    ...
    def compile_node(self, node_type: str, node_params: Dict) -> CompiledFunction:
        ...
        cache_key = hashlib.md5(
            json.dumps({"type": node_type, "params": node_params}, sort_keys=True).encode()
        ).hexdigest()
        ...
        if node_type == "ADD":
            ...
            raise NotImplementedError(f"Node type {node_type} not supported for compilation")
    ...
    def _compile_matrix_mul(self, params: Dict) -> CompiledFunction:
        ...
        func_type = ir.FunctionType(
            ir.VoidType(),
            [self.ptr_f32, self.ptr_f32, self.ptr_f32, 
             ir.IntType(32), ir.IntType(32), ir.IntType(32)]
        )
        func = ir.Function(self.module, func_type, name=f"matmul_{len(self.func_cache)}")
    ...
    def _compile_softmax(self) -> CompiledFunction:
        ...
        func_type = ir.FunctionType(
            ir.VoidType(),
            [self.ptr_f64, self.ptr_f64, ir.IntType(32)]
        )
        func = ir.Function(self.module, func_type, name=f"softmax_{len(self.func_cache)}")
    ...
    def _compile_photonic_mvm(self, params: Dict) -> CompiledFunction:
        ...
        func = ir.Function(self.module, func_type, name=f"photonic_mvm_{len(self.func_cache)}")
    ...
    def _compile_conv2d(self, params: Dict) -> CompiledFunction:
        ...
        func_type = ir.FunctionType(
            ir.VoidType(),
            [self.ptr_f32, self.ptr_f32, self.ptr_f32,
             ir.IntType(32), ir.IntType(32), ir.IntType(32), ir.IntType(32),
             ir.IntType(32), ir.IntType(32), ir.IntType(32)]
        )
        func = ir.Function(self.module, func_type, name=f"conv2d_{len(self.func_cache)}")
        
        # Arguments: input, kernel, output, height, width, channels_in, channels_out, kernel_h, kernel_w, stride
        input_ptr, kernel_ptr, output_ptr, height, width, c_in, c_out, k_h, k_w, stride_val = func.args
        ...
        out_h = builder.sdiv(
            builder.sub(builder.add(height, ir.Constant(ir.IntType(32), 2 * padding)),
        ...
        out_w = builder.sdiv(
            builder.sub(builder.add(width, ir.Constant(ir.IntType(32), 2 * padding)),
                       ir.Constant(ir.IntType(32), kernel_size)),
            ir.Constant(ir.IntType(32), stride)
        )
        ...
        ih = builder.add(builder.mul(oh, stride_val), builder.sub(kh, ir.Constant(ir.IntType(32), padding)))
        iw = builder.add(builder.mul(ow, stride_val), builder.sub(kw, ir.Constant(ir.IntType(32), padding)))
    ...
    def _compile_batch_norm(self) -> CompiledFunction:
        ...
        func_type = ir.FunctionType(
            ir.VoidType(),
            [self.ptr_f32, self.ptr_f32, self.ptr_f32, self.ptr_f32,
             self.ptr_f32, self.ptr_f32, ir.IntType(32), ir.FloatType()]
        )
        func = ir.Function(self.module, func_type, name=f"batch_norm_{len(self.func_cache)}")
        
        # Parameters: input, output, mean, variance, gamma, beta, size, epsilon
        input_ptr, output_ptr, mean_ptr, var_ptr, gamma_ptr, beta_ptr, size, epsilon = func.args
    ...
    def _compile_embedding(self, params: Dict) -> CompiledFunction:
        ...
        func_type = ir.FunctionType(
            ir.VoidType(),
            [self.ptr_i32, self.ptr_f32, self.ptr_f32, 
             ir.IntType(32), ir.IntType(32), ir.IntType(32)]
        )
        func = ir.Function(self.module, func_type, name=f"embedding_{len(self.func_cache)}")
        ...
        return CompiledFunction(
            ...
            llvm_func=func,
            entry_point=func.name,
            input_types=[DataType.INT32, DataType.FLOAT32, DataType.FLOAT32, 
    ...
    def _compile_attention(self, params: Dict) -> CompiledFunction:
        ...
        func = ir.Function(self.module, func_type, name=f"attention_{len(self.func_cache)}")
