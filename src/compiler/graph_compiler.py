...
class GraphOptimizer:
    ...
    def _matches_pattern(self, graph: nx.DiGraph, start_node: str, pattern: List[str]) -> bool:
    ...
    def _eliminate_dead_code(self, graph: nx.DiGraph) -> nx.DiGraph:
        ...
        input_nodes = [n for n in graph.nodes() if graph.nodes[n].get('type') == 'InputNode']
        reachable = set()
    ...
    def _constant_folding(self, graph: nx.DiGraph) -> nx.DiGraph:
        ...
        const_nodes = [n for n in graph.nodes() if graph.nodes[n].get('type') == 'CONST']
...
class GraphCompiler:
    ...
    def __init__(self, optimization_level: int = 2):
        ...
        self.compilable_nodes = {
            NodeType.CONST, NodeType.ADD, NodeType.MUL,
            NodeType.MATRIX_MUL, NodeType.RELU, NodeType.SOFTMAX,
            NodeType.PHOTONIC_MVM, NodeType.LOAD, NodeType.STORE,
            NodeType.REDUCE_SUM, NodeType.REDUCE_MEAN,
            NodeType.TRANSPOSE, NodeType.RESHAPE
        }
        ...
        try:
            ...
            for node in graph.get("nodes", []):
                ...
                try:
                    ...
                    if node_type not in self.compilable_nodes and \
                       node_type not in {NodeType.INPUT, NodeType.OUTPUT}:
    ...
    def _create_main_function(self, graph: nx.DiGraph, exec_order: List[str]) -> ir.Function:
        ...
        main_func = ir.Function(self.llvm_backend.module, func_type, name="graphix_main")
    ...
    def _compile_node(self, graph: nx.DiGraph, node_id: str) -> CompiledNode:
        ...
        if node_type == NodeType.INPUT:
            ...
            compiled_func = self.llvm_backend.compile_node(backend_type, compiled.params)
    ...
    def _link_nodes(self, graph: nx.DiGraph, compiled_nodes: Dict[str, CompiledNode], main_func: ir.Function):
    ...
    def _link_matrix_mul(self, node: CompiledNode, args: List[ir.Value], value_map: Dict, builder: ir.IRBuilder):
        ...
        if len(args) >= 2:
            ...
            # Call function
            builder.call(node.llvm_function, [mat_a_ptr, mat_b_ptr, output_alloca, m, n, k])
            
            # Store result
            value_map[node.node_id] = output_alloca
    
    def _link_photonic_mvm(self, node: CompiledNode, args: List[ir.Value], value_map: Dict, builder: ir.IRBuilder):
        ...
        if len(args) >= 2:
            ...
            builder.call(node.llvm_function, [matrix_ptr, vector_ptr, output_alloca, rows, cols])
            value_map[node.node_id] = output_alloca
    ...
    def compile_subgraph(self, graph: Dict[str, Any], subgraph_nodes: List[str]) -> bytes:
        """Compile a subgraph for fusion"""
        # Extract subgraph
        subgraph = {
            "nodes": [n for n in graph["nodes"] if n["id"] in subgraph_nodes],
            "edges": [e for e in graph["edges"] if 
                     self._edge_in_subgraph(e, subgraph_nodes)]
        }
