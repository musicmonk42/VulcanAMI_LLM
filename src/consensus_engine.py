...
class ConsensusEngine:
    ...
    def propose(self, proposal_graph: Dict, agent_id: str,
    ...
    def propose_weight_update(self, gradient_update: Dict, agent_id: str,
                              layer: str, current_loss: float) -> str:
        """
    ...
    def evaluate_consensus(self, proposal_id: str) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if proposal.status == ProposalStatus.OPEN:
                ...
                if proposal.status not in [ProposalStatus.APPLIED, ProposalStatus.FAILED]:
            ...
            result = {
                ...
                "total_agents": total_agents,
                "vote_breakdown": {
                    "approve": sum(1 for v in proposal.votes.values() if v.vote == VoteType.APPROVE),
                    "reject": sum(1 for v in proposal.votes.values() if v.vote == VoteType.REJECT),
                    "abstain": sum(1 for v in proposal.votes.values() if v.vote == VoteType.ABSTAIN)
                },
                "trust_weighted": {
            ...
            if self.proposals.get(proposal_id):
                ...
                if vulcan_assessment:
                    result['vulcan_assessment'] = {
                        ...
                        'safety_level': vulcan_assessment.get('safety_level', 'unknown'),
                        'reasoning': vulcan_assessment.get('reasoning', 'No reasoning available')
                    }
    ...
    def _validate_weight_update(self, proposal_content: Dict) -> Tuple[bool, Optional[str]]:
        """
        ...
        if 'layer' not in proposal_content or not isinstance(proposal_content['layer'], str):
    ...
    def _validate_proposal(self, proposal_graph: Dict) -> Tuple[bool, Optional[str]]:
        ...
        proposal_node = next(
            (node for node in nodes if node.get("type") == "ProposalNode"),
            None
        )
        ...
        if "add" in proposal_content or "remove" in proposal_content or "modify" in proposal_content:
            return True, None
        
        # Fallback if content type is unknown or missing generic change keys
        return False, "ProposalNode content must define a recognized change ('add', 'remove', 'modify') or a 'type' (e.g., 'weight_update')"
    ...
    def _start_cleanup_thread(self):
        ...
        thread = threading.Thread(target=cleanup_loop, daemon=True, name="ConsensusCleanup")
        thread.start()
