...
Mutator = Callable[[Graph, Match], Graph]  # A function that rewrites a graph based on a match
...
class PatternMatcher:
    ...
    def _validate_graph_structure(self, data: Dict, graph_type: str = "graph") -> GraphValidationResult:
        """
        ...
        if not isinstance(data, dict):
            return GraphValidationResult(
                False, 
                f"{graph_type} must be a dictionary, got {type(data).__name__}"
            )
        ...
        for i, node in enumerate(nodes):
            if not isinstance(node, dict):
                return GraphValidationResult(
                    False,
                    f"{graph_type} node at index {i} is not a dictionary"
                )
            
            if 'id' not in node:
                return GraphValidationResult(
                    False,
                    f"{graph_type} node at index {i} missing 'id' field"
                )
            ...
            if node_id in node_ids:
                return GraphValidationResult(
                    False,
                    f"{graph_type} has duplicate node id: {node_id}"
                )
            ...
            if 'type' not in node:
                return GraphValidationResult(
                    False,
                    f"{graph_type} node '{node_id}' missing 'type' field"
                )
            ...
            if 'params' in node and not isinstance(node['params'], dict):
                return GraphValidationResult(
                    False,
                    f"{graph_type} node '{node_id}' has non-dict params"
                )
        ...
        for i, edge in enumerate(edges):
            if not isinstance(edge, dict):
                return GraphValidationResult(
                    False,
                    f"{graph_type} edge at index {i} is not a dictionary"
                )
            
            if 'from' not in edge:
                return GraphValidationResult(
                    False,
                    f"{graph_type} edge at index {i} missing 'from' field"
                )
            
            if 'to' not in edge:
                return GraphValidationResult(
                    False,
                    f"{graph_type} edge at index {i} missing 'to' field"
                )
                    ...
                    for op, p_val in p_constraint.items():
                        try:
                            if op == '>':
                                ...
                                logger.warning(f"Unknown operator '{op}' for parameter '{p_key}'")
                                if self.strict_mode:
                                    return False
    ...
    async def find_matches(self, graph: Graph, pattern: Pattern) -> AsyncGenerator[Match, None]:
        """
        ...
        try:
            ...
            matcher = isomorphism.DiGraphMatcher(G, P, node_match=self._node_semantic_match)
            ...
            for i, match in enumerate(matches):
                ...
                inverted_match = {pattern_id: graph_id for graph_id, pattern_id in match.items()}
    ...
    async def rewrite_graph(self, graph: Graph, pattern: Pattern, mutator: Mutator) -> Graph:
        """
        ...
        try:
            async for match in self.find_matches(graph, pattern):
                ...
                if self.enable_validation:
                    validation = self._validate_graph_structure(proposed_graph, "proposed_graph")
            ...
            logger.warning("No approved matches found for the given pattern. Graph remains unchanged.")
            return graph
        ...
        with self.stats_lock:
            return {
                ...
                "approval_rate": (
                    ...
                    if self.stats.rewrites_proposed > 0 else 0.0
                ),
...
async def main():
    ...
    try:
        async for match in matcher.find_matches(target_graph, pattern_to_find):
            found_count += 1
            logger.info(f"Found match: Pattern node '?p1' maps to graph node '{match['?p1']}'")
    except Exception as e:
    ...
    try:
        approved_graph = await matcher.rewrite_graph(target_graph, pattern_to_find, safe_optimizer_mutator)
    ...
    if NSO_ALIGNER_AVAILABLE:
        ...
        try:
            rejected_graph = await matcher.rewrite_graph(target_graph, pattern_to_find, risky_optimizer_mutator)
    ...
    invalid_pattern = {
        "nodes": [
            {"id": "?p1", "type": "CONST", "params": {"value": "> 5"}}
        ],
        "edges": []
    }
