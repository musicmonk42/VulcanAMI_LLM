...
class LowRankDraftTransformer(nn.Module):
    ...
    def encode(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        assert input_ids.device == next(self.parent.parameters()).device, "Input and parent model device mismatch in encode."
        
        self.parent.eval()
        with torch.autocast(device_type=input_ids.device.type, dtype=torch.float16, enabled=(input_ids.device.type == 'cuda')):
    ...
    def get_logits(self, low_rank_hidden: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
        """
    ...
    def train_distill(self, dataloader, target_model: nn.Module, scheduler=None, epochs: int = 1):
        """
        ...
        for epoch in range(epochs):
            for step, batch in enumerate(dataloader):
                ...
                with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=(device.type == 'cuda')):
                    ...
                    expanded_all = self.up_proj(low_rank_hidden_all) # (B, S, draft_size)
                ...
                if n_steps % 100 == 0:
                    self._logger.info(f"Epoch {epoch}, Step {n_steps}, MSE Loss: {loss.item():.4f}")
    ...
    @torch.no_grad()
    def check_entropy_and_fallback(self, logits: torch.Tensor, entropy_threshold: Optional[float] = None) -> bool:
        """
...
def speculative_sampling_and_verify(
    ...
    with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=(device.type == 'cuda')):
        ...
        entropy_init = -torch.sum(probs_init * torch.log(probs_init + 1e-10), dim=-1).mean().item()
        ...
        for k in range(K):
            ...
            p_draft_token = q_draft.gather(dim=-1, index=drafted_token_ids_k).squeeze(-1) # [B]
            p_main_token = p_main.gather(dim=-1, index=drafted_token_ids_k).squeeze(-1) # [B]
        ...
        for i in range(B):
            ...
            if accepted_len_i > 0:
                ...
                draft_logits_accepted = torch.stack(draft_logits_list[:accepted_len_i])[:, i, :] 
            ...
            sample_logits = main_logits_seq[i, sample_dist_idx, :] if sample_dist_idx < K else full_output.logits[i, L + K - 1, :]

            p_main_k = F.softmax(sample_logits / temperature, dim=-1)
            
            # Draft logits for the position *after* the last accepted token
            q_draft_k = F.softmax(draft_logits_list[sample_dist_idx][i] / temperature, dim=-1) if sample_dist_idx < K else torch.zeros_like(p_main_k)
            ...
            final_sequence_i = torch.cat([input_ids[i], accepted_tokens_i, next_token_i.unsqueeze(0)], dim=0) 
