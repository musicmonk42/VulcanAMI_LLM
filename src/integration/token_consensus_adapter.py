...
class TokenConsensusAdapter:
    ...
    def __init__(
        ...
        observability: Optional[Callable[[str, Dict[str, Any]], Union[None, Any]]] = None,
    ):
        self.engine = engine
    ...
    async def approve(self, proposal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.obs("consensus.validation_error", {"error": str(e), "proposal": proposal})
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if attempt == self.max_retries:
                    self.obs("consensus.timeout_final", {"proposal": proposal_dict, "max_retries": self.max_retries})
                    if not fallback_result: 
                        log.error("Consensus failed after retries: Timeout") # ENHANCEMENT: Error log severity
                ...
                self.obs("consensus.timeout_retry", {"attempt": attempt, "backoff_sleep": backoff_sleep})
                await asyncio.sleep(backoff_sleep) 
                
            except Exception as e:
                attempt_times.append((time.time() - attempt_start_time) * 1000)
                
                self.obs("consensus.engine_error", {"error": str(e), "attempt": attempt, "proposal_type": validated.type})
                if attempt == self.max_retries:
                    if not fallback_result: 
                        log.error(f"Consensus failed after retries: Engine Error: {e}") # ENHANCEMENT: Error log severity
                ...
                self.obs("consensus.engine_error_retry", {"attempt": attempt, "backoff_sleep": engine_backoff_sleep})
    ...
    def _emit_final_obs(self, approved: bool, proposal: Dict[str, Any], attempt: int, start_time: float, attempt_times: List[float], error_type: Optional[str]):
    ...
    async def _attempt_approval(self, proposal_dict: Dict[str, Any], validated_proposal: ConsensusProposal) -> bool:
        """Helper function to contact the engine, normalizing async/sync calls."""
        if not self.engine:
            self.obs("consensus.engine_missing", {"proposal_type": validated_proposal.type})
        ...
        self.obs("consensus.engine_no_method", {"proposal_type": validated_proposal.type})
        return True
    ...
    def _validate_proposal(self, raw: Dict[str, Any]) -> ConsensusProposal:
        ...
        if not isinstance(raw["type"], str):
             raise TypeError(f"Field 'type' must be a string, got {type(raw['type']).__name__}")
             
        # Token validation - ENHANCED 
        token = raw["token"]
        if not isinstance(token, (str, int)):
            raise TypeError(f"Field 'token' must be a string or int, got {type(token).__name__}")
        ...
        if not isinstance(position, int):
            raise TypeError(f"Field 'position' must be an int, got {type(position).__name__}")
        if position < 0:
        ...
        if chosen_index is not None:
            if not isinstance(chosen_index, int):
                raise TypeError(f"Field 'chosen_index' must be an int if provided, got {type(chosen_index).__name__}")
            if chosen_index < 0:
                raise ValueError(f"chosen_index must be non-negative if provided, got {chosen_index}")
...
async def demo_consensus_adapter():
    ...
    for proposal in proposals:
        ...
        print(f"Proposal {proposal['token']}: {'✓ Approved' if approved else '✗ Rejected'}")
    ...
    fail_closed_adapter = TokenConsensusAdapter(
        engine=None,
        config=ConsensusAdapterConfig(fail_closed=True)
    )
    ...
    fail_open_adapter = TokenConsensusAdapter(
        engine=None,
        config=ConsensusAdapterConfig(fail_closed=False)
    )
