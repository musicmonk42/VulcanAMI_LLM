...
class TestVulcanUnlearn:
    ...
    def test_unlearn_with_packfile(self):
        ...
        result = run_vulcan_unlearn(
            ["test_pattern", "--packfile", "test.pack"], timeout=30
        )
        assert result.returncode == 0
    ...
    def test_unlearn_gradient_surgery_strategy(self):
        ...
        result = run_vulcan_unlearn(
            ["test_pattern", "--strategy", "gradient_surgery"], timeout=30
        )
        assert result.returncode == 0
        output = result.stdout + result.stderr
        assert (
            "gradient" in output.lower()
            or "surgery" in output.lower()
    ...
    def test_unlearn_deletion_strategy(self):
        ...
        result = run_vulcan_unlearn(
            ["test_pattern", "--strategy", "deletion"], timeout=30
        )
        assert result.returncode == 0
    ...
    def test_unlearn_perturbation_strategy(self):
        ...
        result = run_vulcan_unlearn(
            ["test_pattern", "--strategy", "perturbation"], timeout=30
        )
        assert result.returncode == 0
    ...
    def test_unlearn_with_json_output(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_vulcan_unlearn(
                ["test_pattern", "--json", json_output], timeout=30
            )
    ...
    def test_unlearn_multiple_patterns(self):
        ...
        for pattern in patterns:
            result = run_vulcan_unlearn(
                [pattern, "--fast-lane", "--no-proof"], timeout=30
            )
            assert result.returncode == 0
    ...
    def test_unlearn_invalid_strategy(self):
        ...
        result = run_vulcan_unlearn(
            ["test_pattern", "--strategy", "invalid_strategy"], timeout=30
        )
        assert result.returncode != 0
    ...
    def test_unlearn_secure_erase_flag(self):
        ...
        result = run_vulcan_unlearn(
            ["test_pattern", "--secure-erase", "--no-proof"], timeout=30
        )
        assert result.returncode == 0
        output = result.stdout + result.stderr
        assert (
            "secure" in output.lower()
            or "erase" in output.lower()
            or "SUCCESS" in output
        )
    ...
    def test_unlearn_secure_erase_with_verification(self):
        ...
        result = run_vulcan_unlearn(
            ["test_pattern", "--secure-erase", "--verify"], timeout=30
        )
        assert result.returncode == 0
        output = result.stdout + result.stderr
        assert "verif" in output.lower() or "SUCCESS" in output
    ...
    def test_unlearn_shows_warning_about_simplified_zk(self):
        ...
        output = result.stdout + result.stderr
        assert (
            "groth16" in output.lower()
            or "zk" in output.lower()
            or "proof" in output.lower()
        )
    ...
    def test_unlearn_secure_erase_in_help(self):
        ...
        assert result.returncode == 0
        assert (
            "--secure-erase" in result.stdout or "secure erase" in result.stdout.lower()
        )
