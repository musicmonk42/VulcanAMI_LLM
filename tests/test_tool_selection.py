...
class TestUtilityWeights:
    ...
    def test_weight_ratios_make_sense(self, utility_weights):
        ...
        assert (
            quality > 0 and risk > 0 and time > 0 and energy > 0
...
class TestCalibration:
    ...
    def test_temperature_scaling_is_boolean(self, calibration):
        ...
        assert isinstance(temp_scaling, bool), "temperature_scaling should be boolean"
    ...
    def test_isotonic_regression_is_boolean(self, calibration):
        ...
        assert isinstance(iso_reg, bool), "isotonic_regression should be boolean"
...
class TestMissingFeatures:
    ...
    def test_no_exploration_strategy(self, tool_selection_config):
        ...
        assert (
            "exploration" not in tool_selection_config
    ...
    def test_no_cost_budget(self, tool_selection_config):
        ...
        assert (
            "budget_constraints" not in tool_selection_config
    ...
    def test_no_fallback_strategy(self, portfolio_strategies):
        ...
        assert (
            "fallback" not in portfolio_strategies
    ...
    def test_no_timeout_config(self, tool_selection_config):
        ...
        assert "timeout_ms" not in tool_selection_config.get(
            "defaults", {}
...
class TestHardwareIntegration:
    ...
    def test_risk_penalty_defined_for_exotic_hardware(self, utility_weights):
        ...
        assert (
            "risk_penalty" in utility_weights
    ...
    def test_strategy_parameters_missing(self, portfolio_strategies):
        ...
        for strategy in enabled:
            if strategy == "speculative_parallel":
                assert (
                    "max_parallel" not in portfolio_strategies
                ), "Strategy parameters not yet implemented"
            if strategy == "sequential_refinement":
                assert (
                    "max_iterations" not in portfolio_strategies
                ), "Strategy parameters not yet implemented"
...
class TestOptimizationParameters:
    ...
    def test_can_compute_utility_score(self, utility_weights):
        ...
        quality_component = (
            utility_weights["quality"] * simulated_metrics["quality_score"]
        )
        time_component = (
            utility_weights["time_penalty"] / simulated_metrics["time_cost"]
        )
        energy_component = (
            utility_weights["energy_penalty"] / simulated_metrics["energy_cost"]
        )
        risk_component = utility_weights["risk_penalty"] * (
            1 - simulated_metrics["risk_score"]
        )
...
class TestRecommendedEnhancements:
    ...
    def test_should_add_exploration_strategy(self, tool_selection_config):
        ...
        recommended_config = {
            "exploration": {"epsilon": 0.1, "decay_rate": 0.99, "min_epsilon": 0.01}
        }
    ...
    def test_should_add_strategy_parameters(self, portfolio_strategies):
        ...
        recommended_config = {
            "strategy_config": {
                "speculative_parallel": {"max_parallel": 3, "timeout_multiplier": 1.5},
                "sequential_refinement": {
                    "max_iterations": 5,
...
class TestDocumentation:
    def test_config_is_self_documenting(self, tool_selection_config):
        ...
        if not config_path.exists():
            config_path = (
                Path(__file__).parent / ".." / "configs" / "tool_selection.yaml"
            )
