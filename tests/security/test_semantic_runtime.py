...
async def test_kernel_computes_only_through_typed_ingress_and_records_claims():
    ...
    kernel = CognitiveKernel(
        state_authority=SimpleNamespace(version="1"), finalizer=_Finalizer()
    )
    result = await kernel.handle(KernelRequest(utterance, None), case)
    assert result.response == "The computed result is 14."
    head = kernel._transactions.epistemic_head(case.case_id)
    assert head is not None
    assert head.claims[0].derivation_ids == (head.derivations[0].derivation_id,)
...
def test_provider_fields_and_bad_unicode_spans_fail_closed():
    ...
    proposal = InterpretationProposal(
        "semantic-ingress/2",
        (ProposedCandidate("arithmetic", "é + 1", SourceSpan(0, 99)),),
        "provider",
    )
    with pytest.raises(ValueError):
        validate_proposal(utterance, proposal)
