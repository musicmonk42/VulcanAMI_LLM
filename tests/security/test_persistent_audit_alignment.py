...
def data(case="c", digest="0" * 64):
    return {"case_id": case, "request_id": "r", "request_digest": digest}


def test_audit_append_reopen_tamper_truncate_duplicate_sequence_close_symlink(tmp_path):
    p = tmp_path / "audit.jsonl"
    a = CanonicalAudit(p)
    ev = a.append("case.started", data())
    payload = {"x": ["y"]}
    a.append("case.interpreted", {**data(), "payload": payload})
    payload["x"].append("mutated")
    a.close()
    a.close()
    b = CanonicalAudit(p)
    assert len(b.events_for_case("c")) == 2
    b.close()
    with pytest.raises(AuditError):
        b.append("case.started", data("x"))
    ...
    with pytest.raises(AuditError):
        CanonicalAudit(tmp_path / "sym.jsonl")


def test_second_writer_invalid_type_timestamp_and_verified_case_retrieval(tmp_path):
    a = CanonicalAudit(tmp_path / "a.jsonl")
    a.append("case.started", data())
    with pytest.raises(AuditError):
        a.append("bad.event", data())
    with pytest.raises(AuditError):
        CanonicalAudit(tmp_path / "a.jsonl")
    assert a.events_for_case("c")[0].event_type == "case.started"
    a.close()
    line = (tmp_path / "a.jsonl").read_text()
    o = json.loads(line)
    o["timestamp"] = "2020-01-01T00:00:00"
    o["event_hash"] = "0" * 64
...
def test_alignment_policy_load_update_cas_lease_and_fail_closed(tmp_path):
    a = CanonicalAudit(tmp_path / "audit.jsonl")
    r = AlignmentRegistry(tmp_path / "policy.json", audit=a)
    old = r.active()
    lease = r.lease()
    cand = default_policy().__dict__.copy()
    cand.update(revision=2, max_claims_per_response=4, policy_digest="")
    cand["permitted_epistemic_statuses"] = list(cand["permitted_epistemic_statuses"])
    cand["policy_digest"] = hashlib.sha256(
        json.dumps(
            {k: v for k, v in sorted(cand.items()) if k != "policy_digest"},
            ensure_ascii=False,
            sort_keys=True,
            separators=(",", ":"),
            allow_nan=False,
    ...
    new = r.update(cand, expected_previous_digest=old.policy_digest)
    assert (
        lease.policy.policy_digest == old.policy_digest
        and r.active().policy_digest == new.policy_digest
    )
    with pytest.raises(ValueError):
        r.update(cand, expected_previous_digest=old.policy_digest)
    lease.close()
    r.close()
    a.close()
    (tmp_path / "policy.json").write_text("{}")
    with pytest.raises(ValueError):
        AlignmentRegistry(tmp_path / "policy.json")
...
def test_alignment_blocks_bad_claims_and_passes_valid(tmp_path):
    ...
    assert r.decide(_commit((c,), (ev,), (d,))).accepted
    assert not r.decide(
        _commit(
            (c,),
            (
                EvidenceArtifact(
                    ...
                    "case",
                    observed_at=now,
                    source_integrity="bad",
                ),
            ),
            (d,),
        )
    ).accepted
    expired = EvidenceArtifact(
        "evidence:one",
        EvidenceKind.RETRIEVED_RECORD,
        canonical_digest("v"),
        "ref",
    ...
    assert not r.decide(_commit((c,), (expired,), (d,))).accepted
    assert not r.decide(_commit(tuple([c] * 9), (ev,), (d,))).accepted
...
def test_terminal_audit_failure_keeps_case_open_and_blocks_positive_response(tmp_path):
    ...
    audit = FailingAudit(tmp_path / "a.jsonl")
    align = AlignmentRegistry(tmp_path / "p.json")
    k = CognitiveKernel(
        state_authority=SimpleNamespace(version="1"),
        finalizer=Finalizer(),
        audit=audit,
        alignment=align,
    )
    ...
    case = CognitiveCase.create(
        request_id="r", conversation_id=None, input_digest=u.digest
    )
    with pytest.raises(AuditError):
        asyncio.run(k.handle(KernelRequest(u, None), case))
    assert case.terminal_status is CognitiveCaseStatus.OPEN
    assert [e.event_type for e in audit.events_for_case(case.case_id)][
        -1
    ] == "case.finalized"
...
def test_no_raw_prompt_token_or_secret_in_case_events(tmp_path):
    audit = CanonicalAudit(tmp_path / "a.jsonl")
    align = AlignmentRegistry(tmp_path / "p.json")
    k = CognitiveKernel(
        state_authority=SimpleNamespace(version="1"),
        finalizer=Finalizer(),
    ...
    u = Utterance.from_text("secret token 2+2")
    case = CognitiveCase.create(
        request_id="r", conversation_id=None, input_digest=u.digest
    )
    asyncio.run(k.handle(KernelRequest(u, None), case))
...
def test_activation_audit_failures_leave_prior_domain_and_policy_active(tmp_path):
    ...
    class Fail:
        ...
        def append(self, t, d):
            if self.fail:
                raise AuditError("audit down")

    fail = Fail()
    from vulcan.runtime.domain_registry import PersistentDomainRegistry

    dom = PersistentDomainRegistry(tmp_path / "dom", audit=fail)
    sid1 = dom.load_bundle(bundle("geo", 1, "Paris"))
    fail.fail = True
    with pytest.raises(AuditError):
        dom.load_bundle(
            bundle("geo", 2, "Lyon"),
            expected_previous_digest=dom._active.domains["geo"].digest,
        )
    assert (
        dom.domain_snapshot_id == sid1
        and dom.lookup_exact("france.capital").value == "paris"
    )
    fail.fail = False
    ar = AlignmentRegistry(tmp_path / "policy2.json", audit=fail)
    old = ar.active()
    cand = default_policy().__dict__.copy()
    cand.update(revision=2, max_claims_per_response=4, policy_digest="")
    cand["permitted_epistemic_statuses"] = list(cand["permitted_epistemic_statuses"])
    ...
    fail.fail = True
    with pytest.raises(AuditError):
        ar.update(cand, expected_previous_digest=old.policy_digest)
