...
from vulcan.graphix.runtime import LanguageInputPort, Utterance
...
from vulcan.runtime.output import (
    LanguageOutputPort,
    ResponseIRProjection,
...
class _TextSwappingFinalizer:
    async def finalize(self, artifact):
        return FinalizationResult(
            FinalizationDecision.ALLOW, artifact, "The computed result is 5."
        )
...
async def test_failed_output_adapter_falls_back_to_strict_renderer():
    ...
    case = CognitiveCase.create(
        request_id="r", conversation_id=None, input_digest=utterance.digest
    )
    result = await CognitiveKernel(
        state_authority=object(),
        finalizer=_Finalizer(),
        language_output=_FailingOutput(),
    ).handle(KernelRequest(utterance, None), case)
...
@pytest.mark.asyncio
async def test_finalizer_cannot_allow_one_artifact_and_publish_different_text():
    utterance = Utterance.from_text("2 + 2")
    case = CognitiveCase.create(
        request_id="r-text-swap", conversation_id=None, input_digest=utterance.digest
    )
    with pytest.raises(RuntimeError, match="outside the committed render"):
        await CognitiveKernel(
            state_authority=object(), finalizer=_TextSwappingFinalizer()
        ).handle(KernelRequest(utterance, None), case)


import json

from vulcan.local_language import (
    SpanProposalError,
    VerifiedAdapterMetadata,
    VerifiedLocalSpanCompletion,
    parse_transformer_span_proposal,
)


def _proposal(operation="arithmetic", s=0, e=5, conf=0.9):
    obj = {
        "schema_version": "transformer-span-proposal/1",
        "candidates": [
            {
                "operation": operation,
                "span": {"start": s, "end": e},
                "argument_spans": {"expression": {"start": s, "end": e}},
                "confidence": conf,
            }
        ],
    }
...
def test_strict_transformer_json_rejects_duplicates_trailing_and_nan(raw):
    with pytest.raises(SpanProposalError):
        parse_transformer_span_proposal(raw, Utterance.from_text("2 + 2"))


@pytest.mark.parametrize(
    "field", ["answer", "domain_hint", "graphix_plan", "code", "evidence"]
)
...
@pytest.mark.parametrize(
    "span", [{"start": 5, "end": 1}, {"start": 0, "end": 99}, {"start": 0, "end": 0}]
)
def test_transformer_invalid_spans_rejected(span):
    obj = json.loads(_proposal())
    obj["candidates"][0]["argument_spans"]["expression"] = span
    raw = json.dumps(obj, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    with pytest.raises(SpanProposalError):
        parse_transformer_span_proposal(raw, Utterance.from_text("2 + 2"))
...
async def test_verified_adapter_close_and_context_overflow():
    adapter = VerifiedLocalSpanCompletion(
        provider=_Provider(_proposal()),
        tokenizer=_Tok(),
        metadata=VerifiedAdapterMetadata("0" * 64),
        context_length=10,
        max_generated_tokens=10,
    )
    with pytest.raises(SpanProposalError):
        await adapter.propose(Utterance.from_text("2 + 2"))
    adapter.close()
...
async def test_malformed_proposal_falls_back_deterministically():
    utterance = Utterance.from_text("2 + 2")
    case = CognitiveCase.create(
        request_id="r2", conversation_id=None, input_digest=utterance.digest
    )
    adapter = VerifiedLocalSpanCompletion(
        provider=_Provider('{"bad":true}'),
        tokenizer=_Tok(),
        metadata=VerifiedAdapterMetadata("0" * 64),
    ...
    result = await CognitiveKernel(
        state_authority=object(), finalizer=_Finalizer(), language_input=adapter
    ).handle(KernelRequest(utterance, None), case)
    assert result.response == "The computed result is 4."
    assert [event.stage for event in case.events].count(
        "input_proposal_unavailable"
    ) == 1
