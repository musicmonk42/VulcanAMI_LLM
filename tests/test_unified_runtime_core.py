...
try:
    ...
    try:
        ...
        class RealRuntimeConfig(object):
            ...
            def __init__(
                self,
                manifest_path=None,
                learned_subgraphs_dir=None,
                enable_hardware_dispatch=True,
                enable_streaming=True,
                enable_batch=True,
                batch_size=100,
                enable_metrics=True,
                enable_governed_io=True,
                ...
                enable_vulcan_agi=True,
                enable_vulcan_integration=True,
                vulcan_validation=True,
                vulcan_consensus=True,
                vulcan_semantic_transfer=True,
                vulcan_safety_checks=True,
                **kwargs,
            ):
                self.manifest_path = (
                    manifest_path if manifest_path else "type_system_manifest.json"
                )
                self.learned_subgraphs_dir = (
                    learned_subgraphs_dir
                    if learned_subgraphs_dir
                    else "learned_subgraphs"
                )
    ...
    try:
        ...
        if GraphValidator_class is None:
            ...
            from src.unified_runtime.graph_validator import (
                GraphValidator as GraphValidator_class,
            )  # type: ignore
            from src.unified_runtime.graph_validator import (
                ValidationResult,
            )  # type: ignore
    ...
    pytest.skip(
        f"Skipping tests, failed to import unified_runtime_core: {e}",
    ...
    def __init__(self, *args, config: Optional[RealRuntimeConfig] = None, **kwargs):
        ...
        if passed_config is not None:
            ...
            try:
                ...
                print(
                    f"Warning: RealRuntimeConfig instantiation failed ({e}), using MagicMock fallback for config."
                )
                self.config = MagicMock(
                    name="mock.default_config", spec=RealRuntimeConfig
                )
        ...
        try:
            self._effective_max_node_count = int(
                getattr(self.config, "max_node_count", 10000)
            )
        ...
        if GraphValidator_class is not None and not isinstance(
            GraphValidator_class, (Mock, MagicMock)
        ):
            validator_spec = GraphValidator_class
        self.validator = MagicMock(name="mock.validator", spec=validator_spec)
        self.validator.validate_graph = Mock(
            return_value=ValidationResult(True, [], {})
        )
        self.validator.validate_graph.return_value.to_dict = Mock(
        ...
        self.execute_streaming.__aiter__ = (
            lambda self: self._async_stream_mock().__aiter__()
        )

        # execute_batch setup
        async def mock_execute_batch_side_effect(
            graphs: List[Dict[str, Any]], *args, **kwargs
        ) -> List[Dict[str, Any]]:
            if not getattr(self.config, "enable_batch", False):
                raise RuntimeError("Batch mode is not enabled")
            return [
                {"status": "success", "graph_index": i} for i, _ in enumerate(graphs)
            ]

        self.execute_batch = AsyncMock(side_effect=mock_execute_batch_side_effect)
        self._execute_batch_side_effect_func = (
            mock_execute_batch_side_effect  # Store for potential reuse
        )
        ...
        def introspect_side_effect():
            ...
            return {
                ...
                "node_types": {
                    "core": sorted(list(self.node_executors.keys())),
                    "learned": [],
                },
                "components": {
        ...
        self.set_verbosity = Mock(
            side_effect=lambda v: setattr(self, "io_verbosity", float(v))
        )
        ...
        self.get_execution_metrics = Mock(
            return_value={"current": {}, "aggregated": {}, "engine": {}}
        )
        self.validate_graph = Mock(
            return_value={"valid": True, "errors": [], "warnings": []}
        )
...
def patch_and_cleanup_runtime():
    ...
    with patch(
        "src.unified_runtime.unified_runtime_core.UnifiedRuntime", MockRuntime
    ) as mock_rt:
        yield mock_rt
    urc._global_runtime = None
...
class TestUnifiedRuntime:
    ...
    def simple_graph(self):
        ...
        return {
            ...
            "edges": [{"from": "c1", "to": {"node": "out", "port": "default"}}],
        }
    ...
    async def test_execute_graph_validation_failure(self, runtime):
        ...
        async def execute_graph_side_effect(graph_arg):
            val_res = runtime.validator.validate_graph(
                graph_arg
    ...
    def test_set_verbosity_bounds(self, runtime):
        runtime.set_verbosity(1.5)
        assert runtime.io_verbosity == 1.5
        runtime.set_verbosity(-0.5)
        assert runtime.io_verbosity == -0.5
    ...
    async def test_execute_streaming_disabled(self, runtime):
        ...
        runtime.execute_streaming.side_effect = RuntimeError(
            "Streaming mode is not enabled"
        )
        ...
        with pytest.raises(RuntimeError, match="Streaming mode is not enabled"):
            async for _ in runtime.execute_streaming(gen()):
    ...
    async def test_execute_streaming_enabled(self, runtime):
        ...
        runtime.execute_streaming.__aiter__ = (
            lambda: runtime._async_stream_mock().__aiter__()
        )
    ...
    def test_register_node_type(self, runtime):
        async def custom_executor(**kwargs):
            pass  # pragma: no cover

        success = runtime.register_node_type("CustomNode", custom_executor)
        runtime.register_node_type.assert_called_once_with(
            "CustomNode", custom_executor
        )
    ...
    def test_introspect_node_types(self, runtime):
        info = runtime.introspect()
        assert sorted(info["node_types"]["core"]) == sorted(
            ["CONST", "ADD", "MUL", "OutputNode"]
        )
    ...
    def test_validate_graph_invalid(self, runtime):
        ...
        runtime.validate_graph.return_value = {
            "valid": False,
            "errors": ["err"],
            "warnings": [],
        }
...
class TestModuleLevelFunctions:
    ...
    async def test_execute_batch_function(self):
        ...
        runtime_instance.execute_batch.side_effect = (
            runtime_instance._execute_batch_side_effect_func
        )
...
class TestIOGovernance:
    ...
    def runtime_io(self, temp_dir_io):  # Unique name
        manifest_path = temp_dir_io / "m.json"
        manifest_path.touch()
        config = urc.RuntimeConfig(
            manifest_path=str(manifest_path), enable_governed_io=True
        )
...
class TestComponentIntegration:
    ...
    def full_config_ci(self, temp_dir_ci):
        manifest_path = temp_dir_ci / "m.json"
        manifest_path.touch()
...
class TestErrorHandling:
    ...
    def runtime_eh(self, temp_dir_eh):
        manifest_path = temp_dir_eh / "m.json"
        manifest_path.touch()
...
class TestComplexScenarios:
    ...
    def runtime_cs(self, temp_dir_cs):
        manifest_path = temp_dir_cs / "m.json"
        manifest_path.touch()
        ...
        rt.execute_batch.side_effect = (
            rt._execute_batch_side_effect_func
        )  # Ensure side effect
        yield rt
...
class TestEdgeCases:
    ...
    def runtime_ec(self, temp_dir_ec):
        manifest_path = temp_dir_ec / "m.json"
        manifest_path.touch()
        config = urc.RuntimeConfig(manifest_path=str(manifest_path))
        rt = MockRuntime(config=config)
        rt.execute_graph.return_value = {"status": "success"}
