...
class TestYAMLStructure:
    ...
    def test_all_resources_have_apiversion(self, helm_chart):
        ...
        for doc in helm_chart:
            assert 'apiVersion' in doc, f"Resource missing apiVersion: {doc.get('kind')}"
...
class TestResourceManagement:
    ...
    def test_requests_less_than_limits(self, deployment):
        ...
        if 'cpu' in requests and 'cpu' in limits:
            assert parse_cpu(requests['cpu']) <= parse_cpu(limits['cpu']), \
                "CPU request should not exceed limit"

        if 'memory' in requests and 'memory' in limits:
            assert parse_memory(requests['memory']) <= parse_memory(limits['memory']), \
...
class TestHealthChecks:
    def test_liveness_probe_defined(self, deployment):
        ...
        assert 'livenessProbe' in container, \
            "Liveness probe is required for production"
    ...
    def test_liveness_probe_has_endpoint(self, deployment):
        ...
        assert 'httpGet' in probe or 'tcpSocket' in probe or 'exec' in probe, \
        ...
        for env in env_vars:
            if 'value' in env:
                ...
                if any(pattern in var_name for pattern in dangerous_patterns):
                    assert 'valueFrom' in env or 'value' not in env, \
...
class TestService:
    ...
    def test_service_selector_matches_deployment(self, service, deployment):
        ...
        for key, value in svc_selector.items():
            assert key in dep_labels, \
                f"Service selector {key} not in deployment labels"
            assert dep_labels[key] == value, \
...
class TestSecurityBestPractices:
    ...
    def test_run_as_non_root(self, deployment):
        ...
        run_as_non_root = (
            pod_security.get('runAsNonRoot') or
            container_security.get('runAsNonRoot')
        )
...
class TestProductionReadiness:
    ...
    def test_pod_disruption_budget_recommended(self, helm_chart):
        ...
        pdb = next((doc for doc in helm_chart if doc.get('kind') == 'PodDisruptionBudget'), None)
    ...
    def test_hpa_recommended(self, helm_chart):
        ...
        hpa = next((doc for doc in helm_chart if doc.get('kind') == 'HorizontalPodAutoscaler'), None)
    ...
    def test_resource_quotas_recommended(self, helm_chart):
        ...
        quota = next((doc for doc in helm_chart if doc.get('kind') == 'ResourceQuota'), None)
    ...
    def test_network_policy_recommended(self, helm_chart):
        ...
        net_policy = next((doc for doc in helm_chart if doc.get('kind') == 'NetworkPolicy'), None)
...
class TestLabelsAndAnnotations:
    ...
    def test_consistent_app_name_across_resources(self, namespace, deployment, service):
        ...
        if app_label in dep_labels and app_label in svc_labels:
            assert dep_labels[app_label] == svc_labels[app_label], \
...
class TestAntiPatterns:
    def test_no_latest_tag(self, deployment):
        ...
        assert ':latest' not in image and not image.endswith('latest'), \
