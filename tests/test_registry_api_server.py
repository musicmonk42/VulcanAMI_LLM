...
from src.governance.registry_api_server import StatusCode  # Import mocked StatusCode
from src.governance.registry_api_server import Timestamp  # Import mocked Timestamp
from src.governance.registry_api_server import (
    AgentRegistry,
    AuditLogEntry,
...
@pytest.fixture
def registry_servicer(
    registry_api, lang_evolution_registry, agent_registry, security_audit_engine
):
...
class TestRequestResponseMessages:
    ...
    def test_register_graph_proposal_request(self):
        ...
        request = RegisterGraphProposalRequest(
            agent_id="agent1", signature="sig", proposal_node=node
        )
    ...
    def test_register_graph_proposal_response(self):
        response = RegisterGraphProposalResponse(
            status="success", message="Created", proposal_id="prop123"
        )
...
class TestDatabaseConnectionPool:
    ...
    def test_connection_pool_exhaustion_timeout(self, temp_db):
        ...
        try:
            ...
            assert (
                not connection_acquired_unexpectedly.is_set()
            ), "Connection acquired unexpectedly"
            assert not error_occurred.is_set(), "An unexpected error occurred in thread"
            assert (
                timed_out_event.is_set()
            ), "Expected RuntimeError (pool exhausted) was not raised"
            ...
            if ctx_manager and conn1:
                try:
                    ...
                    pass  # Ignore errors during cleanup __exit__
...
class TestDatabaseManager:
    ...
    def test_query_records_all(self, db_manager):
        ...
        for i in range(3):
            ...
            db_manager.save_record(
                "graph_proposals", record_id, {"num": i, "id": record_id}
            )

        results = db_manager.query_records(
            "graph_proposals", where_clause="id LIKE ?", params=(f"{prefix}_%",)
        )
    ...
    def test_query_records_with_offset(self, db_manager):
        ...
        for i in range(5):
            db_manager.save_record("graph_proposals", f"{prefix}_{i}", {"num": i})
        all_relevant = db_manager.query_records(
            "graph_proposals", where_clause="id LIKE ?", params=(f"{prefix}_%",)
        )
    ...
    def test_get_full_audit_log_ordered(self, db_manager):
        ...
        logs = db_manager.get_full_audit_log()
        timestamps_read = [
            log["timestamp"] for log in logs if log.get("action", "").startswith(prefix)
        ]
...
class TestRegistryAPI:
    ...
    def test_get_proposal(self, registry_api):
        ...
        submitted_id = registry_api.submit_proposal(proposal_node)
        retrieved_data = registry_api.get_proposal(
            submitted_id
        )  # Retrieves full {node:..., status:...} dict
    ...
    def test_query_proposals_empty(self, registry_api):
        results = registry_api.query_proposals(
            proposed_by=f"nonexistent_agent_{uuid.uuid4()}"
        )
    ...
    def test_query_proposals_with_filter(self, registry_api):
        ...
        for i in range(3):
            ...
            if proposer == proposer1:
                ids_by_proposer1.add(prop_id)
        results = registry_api.query_proposals(
...
class TestLanguageEvolutionRegistry:
    ...
    def test_record_vote_nonexistent(self, lang_evolution_registry):
        consensus_node = {
            "proposal_id": f"nonexistent_vote_{uuid.uuid4()}",
            "votes": {"agent1": "yes"},
        }
        result = lang_evolution_registry.record_vote(consensus_node)
        assert result is False
    ...
    def test_record_validation_pass(self, lang_evolution_registry):
        proposal_id = lang_evolution_registry.submit_proposal(
            {"id": f"lang_prop_valid_pass_{uuid.uuid4()}"}
        )
        validation_node = {
            "target": proposal_id,
            "result": True,
    ...
    def test_record_validation_fail(self, lang_evolution_registry):
        proposal_id = lang_evolution_registry.submit_proposal(
            {"id": f"lang_prop_valid_fail_{uuid.uuid4()}"}
        )
        validation_node = {
            "target": proposal_id,
            "result": False,
    ...
    def test_deploy_grammar_version_validated(self, lang_evolution_registry):
        proposal_id = lang_evolution_registry.submit_proposal(
            {"id": f"lang_prop_deploy_valid_{uuid.uuid4()}"}
        )
        lang_evolution_registry.record_validation(
            {"target": proposal_id, "result": True}
        )
        new_version = f"3.1.{uuid.uuid4().int % 1000}"
        result = lang_evolution_registry.deploy_grammar_version(
    ...
    def test_deploy_grammar_version_not_validated(self, lang_evolution_registry):
        proposal_id = lang_evolution_registry.submit_proposal(
            {"id": f"lang_prop_deploy_invalid_{uuid.uuid4()}"}
        )
        initial_version = lang_evolution_registry.active_grammar_version
        result = lang_evolution_registry.deploy_grammar_version(
            proposal_id, f"3.2.{uuid.uuid4().int % 1000}"
        )
        assert result is False
...
class TestAgentRegistry:
    ...
    def test_authenticate_agent_invalid_signature(self, agent_registry):
        ...
        agent_registry.register_agent({"id": agent_id})
        result = agent_registry.authenticate_agent(
            agent_id, "message", "wrong_signature_hash"
        )
        assert result is False

    def test_authenticate_agent_not_found(self, agent_registry):
        result = agent_registry.authenticate_agent(
            f"nonexistent_auth_{uuid.uuid4()}", "message", "sig"
        )
        assert result is False
...
class TestSecurityAuditEngine:
    ...
    def test_log_audit(self, security_audit_engine):
        ...
        security_audit_engine.log_audit(
            action=action,
            details={"key": "value"},
            entity_id="entity1",
            entity_type="agent",
...
class TestRegistryServicer:
    ...
    def test_register_graph_proposal_success(self, registry_servicer, agent_registry):
        ...
        agent_registry.register_agent(
            {"id": agent_id, "roles": ["proposer"], "trust_level": 0.8}
        )
        ...
        message_dict = {
            "id": node.id,
            "type": node.type,
            "metadata": node.metadata,  # Use node.metadata (which is a dict)
        ...
        request = RegisterGraphProposalRequest(
            agent_id=agent_id, signature=signature, proposal_node=node
        )
        context = MagicMock()
    ...
    def test_register_graph_proposal_auth_failed(self, registry_servicer):
        ...
        node = Node(
            id=f"test_prop_auth_fail_{uuid.uuid4()}",
            proposed_by=agent_id,
            metadata={"a": "b"},
        )
        # Signature is intentionally wrong
        request = RegisterGraphProposalRequest(
            agent_id=agent_id, signature="invalid_short_sig", proposal_node=node
        )
        context = MagicMock()
        response = registry_servicer.RegisterGraphProposal(request, context)
        ...
        assert (
            "Authentication failed" in response.message
    ...
    def test_submit_language_evolution_proposal(
        self, registry_servicer, agent_registry
    ):
        agent_id = f"agent_lang_evol_{uuid.uuid4()}"
        agent_registry.register_agent(
            {"id": agent_id, "roles": ["proposer", "governor"], "trust_level": 0.8}
        )
        ...
        message_dict = {
            "id": node.id,
            "type": node.type,
            "metadata": node.metadata,  # Use node.metadata (dict)
            "proposed_by": node.proposed_by,
            "rationale": node.rationale,
            "proposal_content": {},
        }
    ...
    def test_record_vote(
        self, registry_servicer, agent_registry, lang_evolution_registry
    ):
        agent_id = f"agent_vote_{uuid.uuid4()}"
        agent_registry.register_agent(
            {"id": agent_id, "roles": ["voter"], "trust_level": 0.8}
        )
        proposal_id = lang_evolution_registry.submit_proposal(
            {"id": f"prop_vote_{uuid.uuid4()}"}
        )
        consensus_node_dict = {
        ...
        request = RecordVoteRequest(
            agent_id=agent_id, signature=signature, consensus_node=consensus_node_pb
        )
        context = MagicMock()
    ...
    def test_record_validation(
        ...
        agent_registry.register_agent(
            {"id": agent_id, "roles": ["validator"], "trust_level": 0.8}
        )
        proposal_id = lang_evolution_registry.submit_proposal(
            {"id": f"prop_validate_{uuid.uuid4()}"}
        )
        ...
        request = RecordValidationRequest(
            agent_id=agent_id, signature=signature, validation_node=validation_node_pb
        )
        context = MagicMock()
    ...
    def test_deploy_grammar_version(
        self, registry_servicer, agent_registry, lang_evolution_registry
    ):
        agent_id = f"agent_deploy_{uuid.uuid4()}"
        agent_registry.register_agent(
            {"id": agent_id, "roles": ["governor", "deployer"], "trust_level": 0.8}
        )
        proposal_id = lang_evolution_registry.submit_proposal(
            {"id": f"prop_deploy_{uuid.uuid4()}"}
        )
        lang_evolution_registry.record_validation(
            {"target": proposal_id, "result": True}
        )
        ...
        signature = hashlib.sha256(message_str.encode()).hexdigest()
        request = DeployGrammarVersionRequest(
            agent_id=agent_id,
            signature=signature,
            proposal_id=proposal_id,
            new_grammar_version=new_version,
    ...
    def test_query_proposals(
        ...
        prop1_id = lang_evolution_registry.submit_proposal(
            {"id": f"prop_q1_{uuid.uuid4()}", "proposed_by": agent_id}
        )
        prop2_id = lang_evolution_registry.submit_proposal(
            {"id": f"prop_q2_{uuid.uuid4()}", "proposed_by": "other_agent"}
        )
    ...
    def test_get_full_audit_log(self, registry_servicer, agent_registry):
        ...
        agent_registry.register_agent(
            {"id": agent_id, "roles": ["auditor", "governor"], "trust_level": 0.9}
        )
...
class TestAuthenticationAuthorization:
    ...
    def test_authenticate_request_invalid(self, registry_servicer, agent_registry):
        ...
        agent_registry.register_agent({"id": agent_id})
        result = registry_servicer._authenticate_request(
            agent_id, "message", "wrong_sig_format_different"
        )
        assert result is False

    def test_authenticate_request_unknown_agent(self, registry_servicer):
        result = registry_servicer._authenticate_request(
            f"unknown_aa_{uuid.uuid4()}", "message", "sig_hash_placeholder"
        )
        assert result is False
    ...
    def test_authorize_request_unknown_agent(self, registry_servicer):
        result = registry_servicer._authorize_request(
            f"unknown_authz_{uuid.uuid4()}", ["proposer"]
        )
        assert result is False
...
class TestEdgeCases:
    ...
    def test_empty_proposal_content(self, registry_api):
        ...
        assert "proposal_content" not in retrieved.get(
            "node", {}
        )  # It shouldn't exist if not provided
    ...
    def test_concurrent_database_access(self, db_manager):
        ...
        def write_data(index):
            try:
                ...
                db_manager.save_record(
                    "graph_proposals", f"{prefix}_{index}", {"num": index}
                )
            except Exception as e:
    ...
    def test_malformed_json_in_proposal(self, registry_servicer, agent_registry):
        ...
        message_dict = {
            ...
            "rationale": node.rationale,
            "proposal_content": {},
        }  # Assume empty dict as it fails parsing
        message_str = json.dumps(message_dict, sort_keys=True)
