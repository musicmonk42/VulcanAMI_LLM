...
def test_snapshot_bundle_is_immutable_and_canonical():
    ps = providers()
    bundle = construct_snapshot_bundle(
        episode_id="ep1", providers=ps, clock=clock, lifetime=timedelta(minutes=5)
    )
    ...
    raw = canonical_json(bundle.to_json())
    assert canonical_json(AdmittedContext.from_bytes(raw).to_json()) == raw
    mutated = bundle.to_json()
    mutated["refs"][0]["material"]["payload"]["source"] = "tampered"
    with pytest.raises(ValueError, match="material digest mismatch"):
        AdmittedContext.from_bytes(canonical_json(mutated))
    malformed = bundle.to_json()
    malformed["refs"][0]["owner"] = 7
    with pytest.raises(ValueError, match="metadata must be text"):
        AdmittedContext.from_bytes(canonical_json(malformed))
    with pytest.raises(ValueError, match="non-empty bytes"):
        AdmittedContext.from_bytes(b"")
...
def test_mixed_snapshot_rejection_cannot_be_bypassed_by_reason_text():
    ...
    with pytest.raises(EpisodeTransitionError, match="typed authorized rebase"):
        ep.transition(
            EpisodeState.INTERPRETED,
            reason="explicit rebase to new bundle",
            authority="microkernel",
            snapshot_ids=["9" * 64],
            clock=clock,
        )
...
def test_policy_domain_update_during_active_episode_keeps_old_leases_pinned():
    ...
    ps[4].digest = "f" * 64
    newer = construct_snapshot_bundle(
        episode_id="ep2", providers=ps, clock=clock, lifetime=timedelta(minutes=5)
    )
    assert bundle.domain.digest == old_domain
    assert newer.domain.digest != old_domain
...
def test_restart_reconciliation_rejects_digest_mismatch_and_malformed_authority():
    with pytest.raises(ValueError, match="full sha256"):
        construct_snapshot_bundle(
            episode_id="ep1",
            providers=[Provider(k, bad_digest=(k == "domain")) for k in KINDS],
            clock=clock,
            lifetime=timedelta(minutes=5),
        )
    good = default_snapshot_ref(
        "world",
        "owner",
        1,
        {"rev": 1},
