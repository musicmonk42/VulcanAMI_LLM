...
class TestVulcanPack:
    ...
    def test_build_pack_from_json(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_vulcan_pack(
                ["-i", input_file, "-o", output_file, "--no-dqs"], timeout=30
            )
    ...
    def test_build_pack_verbose_mode(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_vulcan_pack(
                ["-i", input_file, "-o", output_file, "--verbose", "--no-dqs"],
                timeout=30,
            )
    ...
    def test_build_pack_quiet_mode(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_vulcan_pack(
                ["-i", input_file, "-o", output_file, "--quiet", "--no-dqs"], timeout=30
            )
    ...
    def test_build_pack_from_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(3):
                with open(
                    os.path.join(data_dir, f"file{i}.txt"), "w", encoding="utf-8"
                ) as f:
            ...
            result = run_vulcan_pack(
                ["-d", data_dir, "-o", output_file, "--no-dqs"], timeout=30
            )
    ...
    def test_build_pack_from_directory_recursive(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_vulcan_pack(
                ["-d", data_dir, "-o", output_file, "--recursive", "--no-dqs"],
                timeout=30,
            )
    ...
    def test_file_list_input(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(3):
                with open(
                    os.path.join(tmpdir, f"data{i}.txt"), "w", encoding="utf-8"
                ) as f:
                    f.write(f"data {i}\n")
            ...
            result = run_vulcan_pack(
                ["-f", file_list, "-o", output_file, "--no-dqs"], timeout=30
            )
    ...
    def test_missing_input_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = run_vulcan_pack(
                ["-i", "/nonexistent/file.json", "-o", output_file], timeout=30
            )
