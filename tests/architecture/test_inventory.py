...
def test_fixture_inventory_detects_entrypoints_aliases_cycles_and_fallbacks(
    tmp_path: Path,
) -> None:
    write(
        tmp_path / "src" / "vulcan" / "runtime" / "app.py",
        """
...
""",
    ...
    write(
        tmp_path / "src" / "vulcan" / "world_model" / "cycle_a.py",
        "from vulcan.world_model.cycle_b import b\n",
    )
    write(
        tmp_path / "src" / "vulcan" / "world_model" / "cycle_b.py",
        "from vulcan.world_model.cycle_a import a\n",
    )
    write(
        tmp_path / "docker" / "api" / "Dockerfile",
    ...
    write(
        tmp_path / "helm" / "vulcanami" / "templates" / "deployment.yaml",
        'image: vulcan:test\ncommand: ["python"]\nlivenessProbe: {}\nreadinessProbe: {}\n',
    )
    ...
    inventory = build_inventory(
        InventoryConfig(
            ...
            require_truth_map=False,
        )
    )
    assert any(item["kind"] == "python_main_guard" for item in inventory["entrypoints"])
    assert any(
        item["module"].startswith("src.vulcan")
        for item in inventory["import_identities"]
    )
    assert any(
        item["module"].startswith("vulcan") for item in inventory["import_identities"]
    )
    assert any(
        item["id"] == "duplicate_package_identities" for item in inventory["findings"]
    )
...
def test_docker_compose_and_helm_entrypoints_are_separate() -> None:
    ...
    assert text_entrypoints[
        "docker"
    ], "Docker CMD/ENTRYPOINT directives must be represented"
    assert text_entrypoints[
        "compose"
    ], "Compose image/command/entrypoint keys must be represented"
...
def test_inventory_marks_reachability_and_known_architecture_findings() -> None:
    ...
    reachability = {
        item["reachability"]
        for section in ("routes", "fallbacks", "mocks", "workers")
...
def test_inventory_has_bounded_refs_and_no_arbitrary_import_side_effect_contract() -> (
    None
):
    inventory = json.loads(INVENTORY_JSON.read_text(encoding="utf-8"))
