...
MAX_CONTENT_LENGTH_BYTES = int(
    os.environ.get("MAX_CONTENT_LENGTH_BYTES", 16 * 1024 * 1024)
)  # 16MB default
...
ENFORCE_HTTPS_BOOTSTRAP = (
    os.environ.get("ENFORCE_HTTPS_BOOTSTRAP", "true").lower() != "false"
)
...
if not jwt_secret or jwt_secret.strip() in {
    "super-secret-key",
    "insecure-dev-secret",
    "default-super-secret-key-change-me",
}:
    raise RuntimeError(
        "Refusing to start: JWT_SECRET_KEY must be set to a strong, non-default value in the environment."
    )
...
def revoke_jti(jti: str, exp_ts: Optional[int] = None):
    if redis_client:
        ...
        try:
            redis_client.setex(
                f"{JWT_REVOCATION_PREFIX}{jti}", ttl if ttl else 3600, "1"
            )
            return
        except Exception:
            pass
    ...
    return jsonify(
        {
            ...
            "cors": {"enabled": bool(cors_origins), "origins": cors_origins},
            "limits": {
...
def openapi_spec():
    spec = {
        "openapi": "3.0.0",
        "info": {"title": "Graphix Registry", "version": APP_VERSION},
        "paths": {
            ...
            "/registry/bootstrap": {
                "post": {"summary": "Bootstrap first agent or with key"}
            },
...
def auth_nonce():
    ...
    if existing:
        log_audit(
            None,
            f"Nonce reissued (existing) for agent_id={agent_id}",
    ...
    if not agent:
        log_audit(
            None, f"Nonce issued for agent_id={agent_id} (existence not disclosed)"
        )
    else:
...
def login():
    ...
    if wait_remaining > 0:
        abort(
            429,
            description=f"Login temporarily blocked. Try again in {int(wait_remaining)}s due to repeated failed attempts.",
        )
    ...
    if not stored_nonce or not secrets.compare_digest(stored_nonce, nonce):
        ...
        log_audit(
            agent_id,
            "Login failed: nonce invalid or expired",
    ...
    if not verified:
        ...
        log_audit(
            agent_id,
            "Login failed: signature invalid",
            {"ip": ip, "backoff_seconds": backoff},
        )
        time.sleep((5 + secrets.randbelow(25)) / 1000.0)
        abort(401, description="Invalid credentials")
    ...
    claims = {"iss": JWT_ISSUER, "aud": JWT_AUDIENCE, "jti": secrets.token_hex(16)}
...
def registry_bootstrap():
    ...
    if wait_remaining > 0:
        abort(
            429,
            description=f"Bootstrap temporarily blocked. Try again in {int(wait_remaining)}s seconds.",
        )
    ...
    if not allow_without_key:
        if not key_valid:
            ...
            log_audit(
                None, "Bootstrap failed: invalid key", {"ip": ip, "backoff": backoff}
            )
...
def propose_ir():
    ...
    if actor_agent.trust < 0.4 and (len(nodes) > 2000 or len(edges) > 4000):
        abort(
            403,
            description="Low trust agent cannot submit IR exceeding 2000 nodes / 4000 edges",
        )
    ...
    log_audit(
        actor_id,
        "Proposed IR",
        {"target_agent_id": agent_id, "proposal_id": proposal.id},
    )
    return jsonify(
        {
...
def get_audit_logs():
    ...
    return jsonify(
        {"count": len(result), "logs": result, "offset": offset, "limit": limit}
    )
        ...
        jsonify(
            {
                ...
                "detail": "Request payload exceeds configured maximum size",
            }
        ),
        413,
    )
...
if __name__ == "__main__":
    ...
    # For containerized deployments (e.g., Railway, Docker), bind to 0.0.0.0 to accept external connections
