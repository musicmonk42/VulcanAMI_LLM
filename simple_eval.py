...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        required=True,
        help="Checkpoint path for the model weights/state",
    )
    parser.add_argument(
        "--model-class",
        required=True,
        help="Dotted path to model class e.g. llm_core.graphix_transformer.GraphixTransformer",
    )
    parser.add_argument(
        "--tokenizer-class",
        required=True,
        help="Dotted path to tokenizer class e.g. local_llm.tokenizer.simple_tokenizer.SimpleTokenizer",
    )
    parser.add_argument(
        "--vocab", required=True, help="Path to vocab JSON or tokenizer weights"
    )
    parser.add_argument(
        "--vfile", required=True, help="Validation file (one text per line)"
    )
    ...
    with open(args.vfile, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if hasattr(model, "forward"):
                ...
                raise ValueError(
                    "Model missing forward/encode/callable method for inference"
                )

            if hasattr(model, "executor") and hasattr(model.executor, "get_logits"):
                logits = model.executor.get_logits(
                    out.get("hidden_states", []), ids[:-1]
                )
            elif "logits" in out:
                logits = out["logits"]
            else:
                raise ValueError(
                    "Model output missing 'logits' or model.executor.get_logits()"
                )
    ...
    stats["perplexity"] = (
        math.exp(stats["avg_loss"]) if total_tokens > 0 else float("nan")
    )
